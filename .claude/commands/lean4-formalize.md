...
team lead（你）——轻量级，只做：
  ├── 创建团队 + spawn agents
  ├── cron 监控（10 分钟检查 orchestrator 状态）
...
- ✅ 设置 cron jobs（10 分钟状态检查 + 2 小时全刷新）
- ✅ 响应 cron：检查 orchestrator 是否在工作，如果无响应则 respawn
- ✅ 响应用户指令（方向调整、暂停、关闭等）
...
- ✅ **流水线监控**（由 orchestrator 执行）：team lead 仅监控 orchestrator 是否活跃，不直接操作流水线
...
**首先 spawn orchestrator**——它是可刷新的协调中心：
...
你将通过 SendMessage 收到 orchestrator 或其他 teammate 的分析任务。
...
收到任务后按 lean4-formalizer 规格实现证明，完成后将结果通过 SendMessage 发回 orchestrator。
...
完成后将登记报告通过 SendMessage 发回 orchestrator。
...
5. 如果连续 3 轮产出 ≤ 2 定理，orchestrator 应主动要求 analyst 选取中/高难度目标（而非继续低难度扫尾），并在 formalizer 遇到阻塞时积极 spawn codex-consultant
6. **禁止接受 "低垂果实耗尽" 作为停止理由**——当 analyst 报告耗尽时，orchestrator 必须立即要求转向更高难度方向（S_3 递推、fiber factorization、D(m) 闭式、结论章节、圆维度 Real 分析、SPG 测度扩展等）
...
|------|-----------|----------|----------|
| **orchestrator** | lean4-orchestrator | **每 2 小时刷新** | team lead 管理；shutdown 前 orchestrator 保存状态到 memory，respawn 后从 memory 恢复 |
| analyst | lean4-analyst | **每 10 轮刷新**（由 orchestrator 管理） | orchestrator 在 round_count % 10 == 0 时刷新 |
| formalizer | lean4-formalizer | **每 2 小时刷新**（与 orchestrator 同步） | team lead 的 2h cron 同时刷新 |
| registrar | lean4-registrar | **每 2 小时刷新**（与 orchestrator 同步） | team lead 的 2h cron 同时刷新 |
