...
- ❌ 不读论文、不分析定理、不写代码
- ✅ **编译串行原则**：formalizer、optimizer、reviewer 不得并行 `lake build`。**registrar 默认不处理 proof-bearing Lean 代码的编译门禁**；formalizer 必须先完成 full `lake build` clean 并创建 proof commit，registrar 之后只做独立核对、更新追踪文件与 `.tex` 标注，可在不触发新的 proof build 的前提下并行处理登记工作
- ✅ **编译性能硬限制**（见下方"编译性能规范"）
- ✅ **实时状态表**：由 orchestrator 维护并输出

**流程硬规则（新增）**：proof-bearing Lean 代码必须先由 formalizer 在 full `lake build` clean 后提交 proof commit；registrar 只能在拿到该 proof commit hash 后，独立核对并提交 IMPLEMENTATION_PLAN / `.tex` 标注等登记 commit，再执行 push。禁止 registrar 代替 formalizer 提交 proof-bearing 代码。
...
**重要流程约束**：`formalizer` 的 prompt 必须明确写入“proof-bearing Lean 代码由 formalizer 先在 full `lake build` clean 后提交 proof commit”；`registrar` 的 prompt 必须明确写入“registrar 不得提交 proof-bearing Lean 代码，只能在收到 formalizer proof commit hash 后提交登记 commit”。
...
**proof-bearing Lean 代码必须由 formalizer 先提交 commit**：只有在 full `lake build` clean 后才允许提交 proof commit；之后再由 registrar 基于该 commit 进行登记。
实现完成后，可直接通知 registrar 准备登记（抄送 orchestrator，附 proof commit hash）。
...
**proof-bearing 代码边界**：formalizer 必须先提交可编译的 proof commit；registrar 只基于该 proof commit 做独立核对、更新 IMPLEMENTATION_PLAN / `.tex` 标注，并创建单独的登记 commit + push。
...
**编译串行约束**：registrar 不得接管 formalizer 的 proof gate；若任务要求文件级验证，只能在 formalizer 已暂停后亲自运行 `lake env lean <目标文件>` 做独立核对。
