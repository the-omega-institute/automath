...
| **章节多样性** | **连续 3 轮不得全部来自同一章节方向（如 POM/Folding）；team lead 标记方向"饱和"后，analyst 必须从未饱和章节选至少 1 个目标** | 退回 analyst |
...
- ✅ 只做：建团队、建任务、分配任务、转发消息、路由结果
- ✅ 每个 phase 必须收到 teammate 回复后才进入下一 phase
- ✅ 发现论文错误时直接修改论文 .tex 源文件（spawn paper-writer 或自行修改），不建勘误文件
