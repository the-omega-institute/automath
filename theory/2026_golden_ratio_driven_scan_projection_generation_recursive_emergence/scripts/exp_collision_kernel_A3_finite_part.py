#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Abel/Mertens finite-part constants for the triple-collision kernel A3.

We treat the 3-state kernel A3 as an SFT adjacency matrix and compute the Abel/Hadamard
finite-part constant log 𝔐 and the orbit-Mertens constant 𝕄 = γ + log 𝔐, using:

  log 𝔐 = log C + Σ_{k>=2} μ(k)/k * log ζ(z_*^k),

where:
  - ζ(z) = 1/det(I - z A3),
  - z_* = 1/ρ(A3) is the main pole,
  - C = lim_{z->z_*} (1 - ρ z) ζ(z) = 1/(-z_* Δ'(z_*)), Δ(z)=det(I-zA3).
...
def det_delta(z: float) -> float:
    # Δ(z)=det(I - z A3) = 1 - 2z - 4z^2 + 2z^3
    return 1.0 - 2.0 * z - 4.0 * z * z + 2.0 * z * z * z


def det_delta_prime(z: float) -> float:
    # Δ'(z) = -2 - 8z + 6z^2
    return -2.0 - 8.0 * z + 6.0 * z * z
...
def perron_root_r3(iters: int = 50) -> float:
...
def write_table_tex(path: Path, fp: FinitePart) -> None:
    ...
    lines.append(
        "\\caption{Abel/Mertens finite-part constants for the triple-collision kernel $A_3$. "
        "Here $z_\\star=1/\\rho(A_3)$, $C=\\lim_{z\\to z_\\star}(1-\\rho z)\\zeta_{A_3}(z)$, "
        "$\\log\\mathfrak{M}=\\log C+\\sum_{k\\ge2}\\mu(k)\\log\\zeta_{A_3}(z_\\star^k)/k$, and "
        "$\\mathsf{M}=\\gamma+\\log\\mathfrak{M}$.}"
    )
    lines.append("\\label{tab:collision_kernel_A3_finite_part}")
    lines.append("\\begin{tabular}{r r}")
    lines.append("\\toprule")
    lines.append("Quantity & Value\\\\")
    lines.append("\\midrule")
    lines.append(f"$r_3=\\rho(A_3)$ & {fp.r3:.12f}\\\\")
    lines.append(f"$z_\\star=1/r_3$ & {fp.z_star:.12f}\\\\")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compute Abel/Mertens finite-part for triple-collision kernel A3.")
    ...
    parser.add_argument(
        "--json-out",
        type=str,
        default=str(export_dir() / "collision_kernel_A3_finite_part.json"),
        help="Output JSON path.",
    )
    parser.add_argument(
        "--tex-out",
        type=str,
        default=str(generated_dir() / "tab_collision_kernel_A3_finite_part.tex"),
        help="Output LaTeX table path.",
    )
    args = parser.parse_args()

    prog = Progress("collision-A3-finite-part", every_seconds=20.0)
    ...
    print(f"[collision-A3-finite-part] wrote {jout}", flush=True)

    write_table_tex(Path(args.tex_out), fp)
    print(f"[collision-A3-finite-part] wrote {args.tex_out}", flush=True)
