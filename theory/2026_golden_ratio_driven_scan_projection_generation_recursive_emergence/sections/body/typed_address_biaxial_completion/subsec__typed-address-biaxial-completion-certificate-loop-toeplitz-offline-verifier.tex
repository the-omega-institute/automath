...
\begin{definition}[branch-pure 的嵌套证书走廊]\label{def:distill-typed-branch-pure-certificate-corridor}
\textbf{依赖状态：construction.}
沿定义 \ref{def:typed-address-biaxial-completion-unified-offline-verifier}，记
\[
\mathsf{Cert}^{\mathrm{off}}
=\mathsf{Cert}_{\mathrm{def}}
\sqcup\mathsf{Cert}_{\mathrm{psd}}
\sqcup\mathrm{Addr}_{\mathrm{US}}.
\]
令
\[
\tau\in\{\mathrm{def},\mathrm{psd},\mathrm{addr}\}.
\]
一个深度区间 \(M<N\) 上的 active certificate corridor 称为 branch-pure，若存在有限集
\[
\mathcal E_N^\tau\subset
\begin{cases}
\mathsf{Cert}_{\mathrm{def}},&\tau=\mathrm{def},\\
\mathsf{Cert}_{\mathrm{psd}},&\tau=\mathrm{psd},\\
\mathrm{Addr}_{\mathrm{US}},&\tau=\mathrm{addr},
\end{cases}
\]
以及同一分支内的 coarse map
\[
\pi_{N,M}^\tau:\mathcal E_N^\tau\longrightarrow \mathcal E_M^\tau
\]
其中 \(\mathcal E_M^\tau\) 仍位于同一个直和 summand。也就是说，一条 branch-pure 走廊不允许把 Jensen 缺陷证书、Toeplitz--PSD 证书与地址查询混合成同一个 active fibre。
\end{definition}

\begin{proposition}[branch-pure 证书走廊的 coarse--fine 多重度乘积]\label{prop:distill-typed-branch-pure-multiplicity-product}
\textbf{依赖状态：rigidity.}
沿定义 \ref{def:distill-typed-branch-pure-certificate-corridor}。令 \(E_N\subset\mathcal E_N^\tau\) 为非空 active fine certificate set，并定义 coarse support
\[
C_M(E_N):=\pi_{N,M}^\tau(E_N)
\]
与 fine-overlap multiplicity
\[
\mu_{N|M}^\tau(E_N):=\max_{c\in C_M(E_N)}
\bigl|E_N\cap (\pi_{N,M}^\tau)^{-1}(c)\bigr|.
\]
非空假设保证最大值存在。则
\[
|E_N|\le |C_M(E_N)|\,\mu_{N|M}^\tau(E_N).
\]
因此，对任意整数预算 \(B_{\rm coarse},B_{\rm fine}\ge1\)，若
\[
|E_N|>B_{\rm coarse}B_{\rm fine},
\]
则必有一个同分支的有限过载见证：或者
\[
|C_M(E_N)|>B_{\rm coarse},
\]
或者存在 \(c\in C_M(E_N)\) 使
\[
\bigl|E_N\cap (\pi_{N,M}^\tau)^{-1}(c)\bigr|>B_{\rm fine}.
\]
该见证属于同一个 verifier syntax branch \(\tau\)：当 \(\tau=\mathrm{def}\) 时是 Jensen 缺陷证书走廊中的过载；当 \(\tau=\mathrm{psd}\) 时是 Toeplitz--PSD 证书走廊中的过载；当 \(\tau=\mathrm{addr}\) 时是地址逆系统走廊中的过载。
\end{proposition}
\begin{proof}
由于 \(E_N\) 非空，\(C_M(E_N)\) 亦非空。按 coarse certificate 分块，得到不交并
\[
E_N=\bigsqcup_{c\in C_M(E_N)}
\bigl(E_N\cap (\pi_{N,M}^\tau)^{-1}(c)\bigr).
\]
每个分块大小不超过 \(\mu_{N|M}^\tau(E_N)\)，故
\[
|E_N|\le |C_M(E_N)|\mu_{N|M}^\tau(E_N).
\]
若两个预算同时满足 \(|C_M(E_N)|\le B_{\rm coarse}\) 与 \(\mu_{N|M}^\tau(E_N)\le B_{\rm fine}\)，则上式给出 \(|E_N|\le B_{\rm coarse}B_{\rm fine}\)，与假设矛盾。故至少一个预算失败。branch-pure 条件保证 \(\pi_{N,M}^\tau\) 的定义域、值域以及所有 fibre 都位于同一 summand 中，所以得到的过载见证不会混合 Jensen、Toeplitz 与地址三种验证语法。证毕。
\end{proof}
