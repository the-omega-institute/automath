...
\begin{definition}[有限逆系统窗口中的 active nested carrier]\label{def:distill-recursive-active-nested-carrier}
\textbf{依赖状态：construction.}
令
\[
A_h \xleftarrow{\rho_{h+1,h}} A_{h+1}\xleftarrow{}\cdots\xleftarrow{} A_n
\]
为有限集合的逆系统窗口，其中 \(h<n\)，并记复合映射为 \(\rho_{b,a}:A_b\to A_a\)（\(h\le a<b\le n\)）。对非空 active fine set \(E_n\subset A_n\)，定义其第 \(a\) 层 coarse support
\[
C_a(E_n):=\rho_{n,a}(E_n),
\]
以及相对最大细纤维多重度
\[
M_{n|a}(E_n):=\max_{u\in C_a(E_n)}
\bigl|E_n\cap \rho_{n,a}^{-1}(u)\bigr|.
\]
非空假设保证该最大值在通常意义下存在；本文不采用空集最大值约定。
\end{definition}

\begin{lemma}[nested carrier 的 coarse--fine 多重度乘积]\label{lem:distill-recursive-nested-carrier-multiplicity-product}
\textbf{依赖状态：rigidity.}
沿定义 \ref{def:distill-recursive-active-nested-carrier}，并令 \(h<k<n\)。设 \(E_n\subset A_n\) 非空，记
\[
E_k:=\rho_{n,k}(E_n)\subset A_k.
\]
则
\[
|E_n|\le |C_h(E_n)|\,M_{n|h}(E_n),
\]
并且
\[
M_{n|h}(E_n)\le M_{k|h}(E_k)\,M_{n|k}(E_n).
\]
因此对任意整数预算 \(B_h,B_{n|k},B_{k|h}\ge1\)，若
\[
|E_n|>B_hB_{k|h}B_{n|k},
\]
则三类有限见证中至少有一类出现：
\[
|C_h(E_n)|>B_h,\qquad
M_{k|h}(E_k)>B_{k|h},\qquad
M_{n|k}(E_n)>B_{n|k}.
\]
\end{lemma}
\begin{proof}
第一式来自按 \(h\)-层 coarse carrier 分块：
\[
E_n=\bigsqcup_{u\in C_h(E_n)}\bigl(E_n\cap\rho_{n,h}^{-1}(u)\bigr),
\]
每块大小不超过 \(M_{n|h}(E_n)\)。

对第二式，固定 \(u\in C_h(E_n)\)。在 \(E_k\cap\rho_{k,h}^{-1}(u)\) 中的每个 \(v\) 上，fine fibre
\[
E_n\cap\rho_{n,k}^{-1}(v)
\]
大小至多为 \(M_{n|k}(E_n)\)；而这样的 \(v\) 的数目至多为 \(M_{k|h}(E_k)\)。故
\[
|E_n\cap\rho_{n,h}^{-1}(u)|
\le M_{k|h}(E_k)M_{n|k}(E_n).
\]
对 \(u\) 取最大即得第二式。最后一项是两条乘积不等式的反证形式：若三个预算均不超标，则 \(|E_n|\le B_hB_{k|h}B_{n|k}\)，与假设矛盾。证毕。
\end{proof}
