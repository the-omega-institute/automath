...
\Gamma_S\text{ 为森林}.
\]
\leanpartial{paper\_conclusion\_screen\_realizability\_cycle\_space\_obstruction}{The current Lean signature is false over `Nat` without an extra hypothesis such as `imageDim ≤ edgeCount`, so the paper statement is registered as partial rather than with a fabricated proof.}
\end{corollary}
