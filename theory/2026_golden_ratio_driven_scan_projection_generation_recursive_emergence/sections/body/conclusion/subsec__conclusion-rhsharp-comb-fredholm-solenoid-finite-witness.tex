...
\]
则对任意为每个原子选取的 \(n_j\)-次根 \(\alpha_j\in\CC\)（\(\alpha_j^{n_j}=\beta_j\)），\(L\) 的非零谱多重集恰为
\[
...
\]
完整地嵌入到 \(L\) 的非零谱中；若不同原子在同一点重合，其代数重数只会相加，而不会相互抵消。
...
\end{theorem}
\begin{proof}
推论 \ref{thm:general-support-optimality} 已经给出：一旦
\[
\det(I-rL)^{-1}=\zeta_{\mathcal D}(r),
\]
则 \(L\) 的非零谱多重集精确等于
...
\biguplus_{j\in J}\bigl\{\alpha_j\omega:\omega^{n_j}=1\bigr\}^{m_j}.
\]
因此若某个原子位于边界模长 \(\sqrt{\lambda}\)，则这一整簇边界模态已经被零点结构刚性锁定；若不同原子在同一点相遇，其代数重数只会相加，而不会相互抵消。
...
\qquad (|r|<1).
\]
再由推论 \ref{thm:general-support-optimality}，任何其他满足同一 Fredholm \(\zeta\)-接口的迹类实现 \(L\) 都拥有与 \(L_{\mathcal D}\) 完全相同的非零谱多重集。因而凡由寄存器侧原子谱决定的边界模态与临界梳齿，都已经在有限阶段 \(\Sigma_S\) 被完全固定。
...
\input{thm__conclusion-solenoidal-visible-hull-growth}
\input{thm__conclusion-majorarc-gram-dirichlet-wavepacket-slice-collapse}
\input{thm__conclusion-hzom-hardy-spectralflow-kadic-renormalization}
