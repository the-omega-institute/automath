...
定理 \ref{thm:conclusion-bigwitt-exact-spectral-budget} 给出的是最小非零谱支撑预算；其解析伴随由 \Cref{thm:kyfan-dominance,thm:trace-norm-minimality} 提供。对同一有限原子数据 \(\mathcal D\) 的任意迹类实现 \(L\)，循环块正规模型 \(L_{\mathcal D}\) 同时最小化全部 Ky Fan 预算，进而最小化一切酉不变范数；特别地，
\[
