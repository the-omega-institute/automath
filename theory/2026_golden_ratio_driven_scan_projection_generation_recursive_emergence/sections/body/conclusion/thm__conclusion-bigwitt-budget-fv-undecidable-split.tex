...
\begin{remark}[精确秩预算的解析伴随]
定理 \ref{thm:conclusion-bigwitt-exact-spectral-budget} 给出的是最小非零谱支撑预算；其解析伴随由定理~\ref{thm:kyfan-dominance} 与定理~\ref{thm:trace-norm-minimality} 提供。对同一有限原子数据 \(\mathcal D\) 的任意迹类实现 \(L\)，循环块正规模型 \(L_{\mathcal D}\) 同时最小化全部 Ky Fan 预算，进而最小化一切酉不变范数；特别地，
\[
\|L\|_1\ge \sum_{j=1}^{N}m_jn_j|\beta_j|^{1/n_j},
\]
且等号当且仅当 \(L\) 正规。故“最小秩预算”与“最小迹范数预算”在同一 block-cyclic 模型上同时闭合。
\end{remark}
