...
\input{thm__conclusion-realinput40-ghost-hurwitz-rootprobe-incompleteness}

进一步说，这个单原子机制在根值单位方向只会把 periodic/ghost 层析压缩成严格的 Dirichlet 梳齿，并把全部有限周期标量阴影锁死在 Hurwitz 残数账本中；即使把素数模根值单位探针的角分辨率推进到 \(p^{-2}\) 级，它所细化的也仍只是 ghost 整除几何，而不是宿主的 sheet 结构。于是 atom/core 分裂、primitive surgery、residue 壳、ghost 梳齿与 Hurwitz 有限闭包便共同构成了二维 realinput40 核在 cyclotomic 方向上的完整可见边界。
