...
\begin{theorem}[时钟线丛的下降平凡化与 holonomy 见证]\label{distill:grothendieck-descent_to_closure_theorem_chains-clock-line-descent-holonomy-normal-form}
沿用定理 \ref{thm:physical-spacetime-clock-potential-cech-cocycle} 的 admissible good refinement 与局域时钟势记号。令
\[
g_{\alpha\beta}:=e^{-c_{\alpha\beta}}\in\RR_{>0}
\]
为二重交上的乘法过渡数据。则以下二者等价：
\begin{enumerate}
  \item 存在正数 \(b_\alpha\in\RR_{>0}\)，使重整化局域 redshift 因子
  \[
  \widetilde N_\alpha:=b_\alpha^{-1}N_\alpha
  \]
  在所有非空二重交上严格相等，并因而粘接为全局正函数 \(\widetilde N\)；
  \item \((g_{\alpha\beta})\) 的 \(\RR_{>0}\)-值 \v{C}ech 类为零，即存在 \(b_\alpha\in\RR_{>0}\) 使
  \[
  g_{\alpha\beta}=b_\alpha b_\beta^{-1}
  \qquad(U_{\alpha\beta}\ne\varnothing).
  \]
\end{enumerate}
若该类非零，则局域势仍定义一个可审计的时钟线对象，但不存在仅由顶点重标定给出的全局 redshift 函数。更精确地，对 nerve 一骨架中的任意闭路
\[
\gamma=(\alpha_0,\alpha_1,\ldots,\alpha_m=\alpha_0)
\]
定义
\[
\operatorname{Hol}_N(\gamma):=\prod_{r=0}^{m-1}g_{\alpha_r\alpha_{r+1}}.
\]
则 \(\operatorname{Hol}_N\) 只依赖于 \(\gamma\) 的同调类；若存在闭路使 \(\operatorname{Hol}_N(\gamma)\ne1\)，则任何沿该闭路传播的局域频率比都会获得非平凡乘法误差，因而失败应记录为时钟线 holonomy 障碍，而不是局域时钟势不存在。
\end{theorem}

\begin{theorem}[时钟势边界 collar 的相对 holonomy 账本]\label{distill:grothendieck-compactified_boundary_degeneration_control-clock-collar-relative-holonomy-ledger}
沿用定理 \ref{thm:physical-spacetime-clock-potential-cech-cocycle} 与推论 \ref{cor:physical-spacetime-global-clock-potential} 的记号。设 \(\mathfrak U^\circ\) 是 \(M_{\mathrm{adm}}\) 上的有限 admissible good refinement，\(N^\circ\) 为其 nerve。设
\[
\overline{\mathfrak U}=\mathfrak U^\circ\cup\{V_b\}_{b\in B}
\]
为一次有限 collar 型边界紧化图册，其 nerve \(\bar N\) 含 \(N^\circ\) 为满子复形。假设每个顶点 \(v\in\bar N_0\) 上给出局域对数时钟势 \(\psi_v\)，并且每条非空二重交上存在常数
\[
a_{vw}\in\RR,\qquad \psi_v-\psi_w=a_{vw},\qquad a_{wv}=-a_{vw},
\]
且三重交上满足
\[
a_{uv}+a_{vw}+a_{wu}=0.
\]
在内部子复形 \(N^\circ\) 上，推论 \ref{cor:physical-spacetime-global-clock-potential} 给出全局时钟势，因而 \(a|_{N^\circ}\) 为余边界。取 \(\beta\in C^0(N^\circ;\RR)\) 使
\[
a|_{N^\circ}=\delta\beta,
\]
并将 \(\beta\) 在边界顶点上零延拓为 \(\bar\beta\)。定义相对边界时钟余循环
\[
\vartheta:=a-\delta\bar\beta\in Z^1(\bar N,N^\circ;\RR).
\]
则其相对类
\[
\operatorname{Hol}_{\partial N}:=[\vartheta]\in\check H^1(\bar N,N^\circ;\RR)
\]
与内部平凡化选择无关。并且以下二者等价：
\begin{enumerate}
  \item 存在顶点常数重整化，使 \(e^{-\psi_v}\) 在整个 collar 紧化图册 \(\overline{\mathfrak U}\) 上粘接为一个正的全局 redshift 函数；
  \item \(\operatorname{Hol}_{\partial N}=0\)。
\end{enumerate}
若 \(\operatorname{Hol}_{\partial N}\ne0\)，则 \(M_{\mathrm{adm}}\) 内部的 redshift 单值化仍然成立；失败仅发生在把该内部对象延拓到边界 collar 时，且该失败由相对边界 holonomy 类完全分类。
\end{theorem}
\begin{proof}
由假设，\(a=(a_{vw})\) 是 \(\bar N\) 上的 \(\RR\)-值 \(1\)-余循环。内部全局时钟势的存在说明内部过渡常数可由顶点常数杀死，即存在 \(\beta\in C^0(N^\circ;\RR)\) 满足 \(a|_{N^\circ}=\delta\beta\)。将 \(\beta\) 零延拓后，
\[
\vartheta:=a-\delta\bar\beta
\]
在所有内部边上为零，且 \(\delta\vartheta=0\)。故 \(\vartheta\) 是相对余循环，定义 \(\check H^1(\bar N,N^\circ;\RR)\) 中的类。

若换用另一组内部平凡化 \(\beta'\)，则 \(\beta'-\beta\) 在 \(N^\circ\) 上为 \(0\)-余循环。其零延拓的余边界在内部边上为零，因此改变 \(\vartheta\) 的只是一个相对余边界。相对类由此良定。

若存在全局正 redshift 函数，则存在常数 \(s_v\in\RR\) 使重整化势
\[
\widetilde\psi_v:=\psi_v-s_v
\]
在每条边上严格相等。等价地，
\[
a_{vw}=s_v-s_w=-(\delta s)_{vw}
\]
按本文的余边界号约定即表示 \(a\) 为一个全局余边界。于是与上一段相同，\(a-\delta\bar\beta\) 是相对余边界，故 \(\operatorname{Hol}_{\partial N}=0\)。

反过来，若 \(\operatorname{Hol}_{\partial N}=0\)，则存在相对 \(0\)-上链 \(\lambda\in C^0(\bar N,N^\circ;\RR)\) 使
\[
\vartheta=\delta\lambda.
\]
因此
\[
a=\delta(\bar\beta+\lambda).
\]
用顶点常数 \(\bar\beta+\lambda\) 重整化各 \(\psi_v\) 后，每条边上的差常数均变为零。于是局域函数 \(e^{-\psi_v}\) 在所有重叠上严格一致，按分离性唯一粘接为整个 collar 紧化图册上的正函数。

最后，\(\vartheta|_{N^\circ}=0\) 表明该类完全看不见内部 admissible 域；内部单值化已经由推论 \ref{cor:physical-spacetime-global-clock-potential} 完成。非零类只阻止边界 collar 延拓，因而应作为闭包边界的 holonomy 预算，而不是作为内部时钟势不存在的证据。证毕。
\end{proof}


\begin{proof}
由定理 \ref{thm:physical-spacetime-clock-potential-cech-cocycle}，在每个非空二重交上有
\[
N_\alpha=g_{\alpha\beta}N_\beta,
\]
且 \((g_{\alpha\beta})\) 满足乘法 \v{C}ech \(1\)-余循环条件。先设存在 \(b_\alpha\) 使
\[
g_{\alpha\beta}=b_\alpha b_\beta^{-1}.
\]
则在 \(U_{\alpha\beta}\) 上
\[
\widetilde N_\alpha=b_\alpha^{-1}N_\alpha
=b_\alpha^{-1}g_{\alpha\beta}N_\beta
=b_\beta^{-1}N_\beta
=\widetilde N_\beta.
\]
故这些 \(\widetilde N_\alpha\) 由分离性唯一粘接为全局正函数 \(\widetilde N\)。

反过来，若某组 \(b_\alpha\) 使 \(\widetilde N_\alpha=b_\alpha^{-1}N_\alpha\) 在二重交上严格相等，则
\[
b_\alpha^{-1}N_\alpha=b_\beta^{-1}N_\beta.
\]
再代入 \(N_\alpha=g_{\alpha\beta}N_\beta\)，并用 \(N_\beta>0\)，得
\[
b_\alpha^{-1}g_{\alpha\beta}=b_\beta^{-1},
\]
即 \(g_{\alpha\beta}=b_\alpha b_\beta^{-1}\)。因此过渡余循环正是余边界。

现在考虑非平凡类。闭路 holonomy 的定义只使用边过渡数据。若把 \(g\) 改为同调的余循环 \(g'_{\alpha\beta}=a_\alpha g_{\alpha\beta}a_\beta^{-1}\)，则闭路乘积中相邻顶点因子逐项望远镜消去，故 holonomy 不变。又由于 \(g\) 是 \v{C}ech 余循环，经过二单形边界的初等闭路乘积为 \(1\)，故 \(\operatorname{Hol}_N\) 只依赖于闭路同调类。若某闭路 holonomy 非平凡，则不存在顶点重标定同时杀死所有边过渡；否则第一段会给出全局 \(\widetilde N\)，并使每个闭路乘积等于 \(1\)，矛盾。局域势的存在已由定理 \ref{thm:physical-spacetime-clock-potential-cech-cocycle} 保证，所以该失败不是局域构造失败，而是从局域时钟线到全局 redshift 函数的下降平凡化失败。推论 \ref{cor:physical-spacetime-global-clock-potential} 对应于 admissible 相容性进一步强制 \(g_{\alpha\beta}=1\) 的特殊情形。证毕。
\end{proof}
