...
\end{proposition}
\begin{theorem}[window-\(6\) 群块图册的刚性 \texorpdfstring{$\ZZ_2$}{Z2} 曲率门]\label{distill:hrushovski_zilber_zariski_geometries_and_elekes_szabo_group_extraction-group_chunk_reconstruction-window6-rigid-z2-curvature-gate}
令
\[
\mathfrak A_6=(X_6,\pi_6,P_6,\langle\sigma_{\mathrm{geo}}\rangle)
\]
为推论 \ref{cor:terminal-window6-finite-completeness-template} 中的 window-\(6\) 可观测结构，并记
\(\tau^\epsilon:=\sigma_{\mathrm{geo}}^\epsilon\)（\(\epsilon\in\mathbb F_2\)）。
假设通过 incidence-to-configuration extraction 与 definable-envelope replacement，若干重复的 window-fiber 复合约束已经被压缩为如下有限图册：
\(\Lambda\) 是有限连通无向图，边的两个取向同时存在；每个顶点 \(v\in V(\Lambda)\) 携带一个 \(\tau\)-不变的 generic carrier
\(U_v^\circ\subseteq X_6\)、一个部分二元运算
\[
*_v:D_v\longrightarrow U_v^\circ,
\qquad D_v\subseteq U_v^\circ\times U_v^\circ,
\]
以及单位元 \(e_v\in U_v^\circ\) 与逆映射 \(\iota_v:U_v^\circ\to U_v^\circ\)。
假设每个顶点图册均通过 group-configuration completion 的局部审计：
\(D_v\) 在所声明的 generic 对上闭合，\(e_v\) 为双侧单位，\(\iota_v(x)\) 为 \(x\) 的双侧逆，并且
\[
(x*_v y)*_v z=x*_v(y*_v z)
\]
对所有两边有定义的 generic 三元组成立。
进一步假设边集恰好记录可能导致两个局部运算同时作用于同一 generic 输入的重叠；并且对每条边 \(\{v,w\}\) 存在唯一的 \(h_{vw}=h_{wv}\in\mathbb F_2\)，使得 \(\tau^{h_{vw}}\) 在重叠上给出群块同构：只要 \(x,y\in U_v^\circ\cap U_w^\circ\)，便有
\[
(x,y)\in D_v
\Longleftrightarrow
(\tau^{h_{vw}}x,\tau^{h_{vw}}y)\in D_w,
\]
且在上述等价条件下
\[
\tau^{h_{vw}}(x*_v y)=(\tau^{h_{vw}}x)*_w(\tau^{h_{vw}}y),
\qquad
\tau^{h_{vw}}e_v=e_w,
\qquad
\tau^{h_{vw}}\iota_v(x)=\iota_w(\tau^{h_{vw}}x).
\]
则边标号 \(h=(h_{vw})\) 定义一个规范的 \(\mathbb F_2\)-曲率类
\([h]\)，其中两个标号 \(h,h'\) 等价当且仅当存在顶点函数
\(b:V(\Lambda)\to\mathbb F_2\) 使
\[
h'_{vw}=h_{vw}+b_v+b_w
\qquad(\{v,w\}\in E(\Lambda)).
\]
该类具有以下性质。
\begin{enumerate}
  \item \([h]\) 不依赖局部图册平凡化的选择，并且在保持 \(\mathfrak A_6\) 的任何有限可观测重标号下不发生变化。
  \item \([h]=0\) 当且仅当存在顶点符号 \(b_v\in\mathbb F_2\)，使归一化运算
  \[
  D_v^b:=\{(x,y):(\tau^{b_v}x,\tau^{b_v}y)\in D_v\},
  \]
  \[
  x*_v^b y:=\tau^{b_v}\bigl((\tau^{b_v}x)*_v(\tau^{b_v}y)\bigr)
  \]
  在所有重叠上相同。因此它们粘接为 \(\bigcup_v U_v^\circ\) 上的一个局部可审计群块运算；该运算满足 generic closure、局部结合律、单位元与逆元审计。
  \item 若 \([h]\ne0\)，则存在闭边路 \(\gamma=(v_0,v_1,\ldots,v_r=v_0)\) 满足
  \[
  \sum_{i=0}^{r-1}h_{v_i v_{i+1}}=1\in\mathbb F_2.
  \]
  此时所有顶点局部群块审计仍可同时成立，但不存在将这些局部群块同时 untwist 为单一全局群块证书的顶点平凡化。换言之，经 exceptional-set stratification 排除非闭合、非逆、非结合或非唯一边稳定子之后，剩余坏例子被压缩为有限的 \(\sigma_{\mathrm{geo}}\)-holonomy 骨架。
\end{enumerate}
特别地，若 \(\Lambda\) 为树，则任一满足上述条件的图册自动下降为全局局部群块证书；若某一回路的 \(h\)-和为 \(1\)，则该回路本身给出不可由重标号吸收的有限反证。
\end{theorem}
\begin{proof}
首先说明曲率类的定义与平凡化无关。若在顶点 \(v\) 将局部图册整体以 \(\tau^{b_v}\) 改写，则沿边 \(\{v,w\}\) 的过渡符号由 \(h_{vw}\) 变为
\(h_{vw}+b_v+b_w\)。因此闭边路上的和保持不变，而上述等价关系正是所有这种顶点平凡化所生成的关系。
反过来，在连通图中，若每条闭边路上的 \(h\)-和均为 \(0\)，取一棵生成树并指定根 \(v_0\)，令 \(b_v\) 为从 \(v_0\) 到 \(v\) 的树路径上 \(h\)-和；闭路和为零保证该定义与路径无关，并给出 \(h_{vw}=b_v+b_w\)。故 \([h]=0\) 等价于所有闭路 holonomy 为零。

其次证明规范性。定理 \ref{thm:foldbin6-geo-stabilizer-z2} 已将 stabilizer extraction 的输出固定为唯一几何稳定子
\(\langle\sigma_{\mathrm{geo}}\rangle\cong\ZZ_2\)。命题 \ref{prop:terminal-gamma6-rigidity} 及推论 \ref{cor:terminal-gamma6-multiplicity-rigidity} 又排除了保持 window-\(6\) 加权邻接读出的非平凡类型重标号；推论 \ref{cor:terminal-window6-finite-completeness-template} 将这一点提升为有限可判定同构模板。因此，除顶点处选择是否施加 \(\tau\) 之外，边过渡符号没有额外 gauge 可吸收。由此 \([h]\) 是 \(\mathfrak A_6\) 的可观测不变量。

设 \([h]=0\)，取 \(b_v\) 使 \(h_{vw}=b_v+b_w\)。对每个顶点按定理陈述定义 \(*_v^b\)。若 \(x,y\) 同时落在相邻顶点 \(v,w\) 的归一化定义域中，令
\(a=\tau^{b_v}x\)、\(c=\tau^{b_v}y\)。由于 \(b_w+h_{vw}=b_v\)，边同构给出
\[
(\tau^{b_w}x)*_w(\tau^{b_w}y)
=(\tau^{h_{vw}}a)*_w(\tau^{h_{vw}}c)
=\tau^{h_{vw}}(a*_v c).
\]
再施加 \(\tau^{b_w}\) 得
\[
...
=x*_v^b y.
\]
边集按假设覆盖所有会造成同一输入拥有两个局部读出的重叠，故这些归一化局部运算粘接为单值部分运算 \(*\)。同一计算应用于单位元与逆元，得到所有
\(\tau^{b_v}e_v\) 粘接为同一个局部单位元，所有 \(\tau^{b_v}\iota_v(\tau^{b_v}x)\) 粘接为同一个逆映射。结合律、闭合性、单位元律与逆元律均为局部语句；每个参与审计的 generic 三元组按假设被某个顶点图册覆盖，故从该顶点的 group-configuration completion 审计直接传递到粘接后的运算。于是得到所称的全局局部群块证书。

反之，若存在单一局部群块运算 \(*\) 与顶点符号 \(b_v\)，使每个 \(*_v\) 经 \(\tau^{b_v}\) 归一化后等于 \(*\) 在对应 carrier 上的限制，则沿任意边 \(\{v,w\}\) 比较两个归一化限制可知，\(\tau^{b_v+b_w}\) 与原边过渡具有相同的重叠同构作用。边符号的唯一性给出 \(h_{vw}=b_v+b_w\)，故 \([h]=0\)。

最后，若 \([h]\ne0\)，由第一段的闭路判别存在 \(h\)-和为 \(1\) 的闭边路。若仍可全局 untwist，则上一段推出 \([h]=0\)，矛盾。因此非零闭路 holonomy 精确阻断全局平凡化。该阻断不否定各顶点内的局部 closure、inverse 与 associativity；它只否定把这些局部群块同时解释为同一 window-fiber composition 的全局证书。由于可观测重标号已由 \(\mathrm{Aut}(\Gamma_6)=\{1\}\) 排除，且唯一剩余稳定子正是 \(\sigma_{\mathrm{geo}}\)，该坏例子不能被伪不变量或重新标号隐藏。证毕。
\end{proof}
