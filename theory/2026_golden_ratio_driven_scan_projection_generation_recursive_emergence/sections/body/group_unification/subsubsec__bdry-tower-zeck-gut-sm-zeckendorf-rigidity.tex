...
\end{theorem}
\begin{proposition}[SM 签名的谱作用二次型缺口]\label{distill:connes_spectral_triples_and_chamseddine_connes_spectral_action-spectral_action_closure_family-gaugeq}
设命题 \ref{prop:sm-zeckendorf-lie-algebra-rigidity} 的逐项对齐结论已经给出
\[
\mathfrak g_{\mathrm{SM}}\cong\mathfrak u(1)\oplus\mathfrak{su}(2)\oplus\mathfrak{su}(3).
\]
令 \(V\) 为一个有限内部 Hilbert 表示，\(\pi_V:\mathfrak g_{\mathrm{SM}}\to\mathfrak u(V)\) 为其表示，并令内涨落规范曲率写作
\[
F_{\mu\nu}\in\Omega^2(M_{\mathrm{adm}};\mathfrak g_{\mathrm{SM}}).
\]
假设对应涨落 Dirac 算子平方满足四维 Laplace 型热核公式，且 \(a_4\) 的规范部分为
\[
\mathsf Q_V(F)=c\int_{M_{\mathrm{adm}}}\kappa_V(F_{\mu\nu},F^{\mu\nu})\,d\mu,
\qquad
\kappa_V(X,Y):=-\Tr_V(\pi_V(X)\pi_V(Y)),
\]
其中 \(c>0\) 为与截断函数和规范化约定有关的共同常数。则对任意两个有限内部表示 \(V,V'\)，下列条件等价：
\[
\mathsf Q_V(F)=\mathsf Q_{V'}(F)\text{ for all admissible local curvatures }F
\]
与
\[
\kappa_V=\kappa_{V'}\text{ on }\mathfrak g_{\mathrm{SM}}.
\]
因此，Zeckendorf 维数签名 \(D=12\)、三层标签 \(\Sigma(\mathfrak g_{\mathrm{SM}})=\{4,6,8\}\) 以及共同循环识别 \(\ZZ_6\) 至多确定抽象规范代数及其全局形态候选；它们不确定谱作用中的规范动能系数。更精确地，若 \(W\) 是任一非平凡有限表示并令 \(V':=V\oplus W\)，则 \(V\) 与 \(V'\) 具有同一抽象 \(\mathfrak g_{\mathrm{SM}}\) 读出和同一 Zeckendorf 计数接口，但
\[
\kappa_{V'}=\kappa_V+\kappa_W\ne\kappa_V,
\]
从而 \(a_4\) 规范系数账本不同。维数签名式规范闭包若未额外审计 \(\kappa_V\)，其坏例子骨架恰由不变量
\[
\kappa_{V'}-\kappa_V
\]
给出。
\end{proposition}
\begin{proposition}[SM 挠谱读出的 \texorpdfstring{$\mathrm{SL}_2$}{SL2} 乘积增长预算门槛]\label{distill:breuillard_green_tao_approximate_groups_and_helfgott_growth-product_growth_dichotomy-sm_torsion_sl2_budget_gate}
沿用定理 \ref{thm:sm-signature-common-cyclic-identification-divides-6} 的记号，令
\(\Sigma(\mathfrak g_{\mathrm{SM}})=\{4,6,8\}\)。存在绝对常数 \(\varepsilon>0\) 与 \(k\in\mathbb N\)，使得如下断言成立。
令 \(p\ge 5\) 为素数。设 \(C\cong\ZZ/n\ZZ\)，并且对每个 \(m\in\Sigma(\mathfrak g_{\mathrm{SM}})\) 给定单射
\(\jmath_m:C\hookrightarrow \pi_1(\mathcal G_m)\) 与群同态
\(\theta_m:\pi_1(\mathcal G_m)\to \mathrm{SL}_2(\mathbb F_p)\)。定义 SM 三层挠谱读出集
\[
B_p:=\bigcup_{m\in\Sigma(\mathfrak g_{\mathrm{SM}})}\theta_m(\jmath_m(C))
\subseteq \mathrm{SL}_2(\mathbb F_p).
\]
则 \(B_p\) 为含单位元的对称集，且 \(|B_p|\le 18\)。若 \(B_p\) 生成 \(\mathrm{SL}_2(\mathbb F_p)\)，则
\[
|B_p^3|\ge |B_p|^{1+\varepsilon}
\quad\text{或}\quad
B_p^k=\mathrm{SL}_2(\mathbb F_p).
\]
因此，若
\[
p(p^2-1)>18^k
\]
且 \(|B_p^3|<|B_p|^{1+\varepsilon}\)，则 \(B_p\) 不能生成 \(\mathrm{SL}_2(\mathbb F_p)\)；等价地，所有这种非增长读出均被迫落入 \(\mathrm{SL}_2(\mathbb F_p)\) 的真子群载体。特别地，在任意素数层上，生成性非增长只能发生在有界深度全闭包情形，并必满足
\[
|\mathrm{SL}_2(\mathbb F_p)|\le 18^k.
\]
\end{proposition}
\begin{proof}
由定理 \ref{thm:sm-signature-common-cyclic-identification-divides-6}，共同循环识别的阶数满足 \(n\mid 6\)。因此每个 \(\theta_m(\jmath_m(C))\) 都是 \(\mathrm{SL}_2(\mathbb F_p)\) 中阶数至多为 \(6\) 的循环子群像，故其含单位元且对取逆封闭。三层并集 \(B_p\) 仍含单位元并对取逆封闭，并且
\[
|B_p|\le \sum_{m\in\{4,6,8\}}|\theta_m(\jmath_m(C))|\le 3n\le 18.
\]

调用 Helfgott 关于 \(\mathrm{SL}_2(\mathbb F_p)\) 的 Growth-or-structure dichotomy：存在绝对常数 \(\varepsilon>0\) 与 \(k\in\mathbb N\)，使得任意含单位元的有限对称生成集 \(A\subseteq \mathrm{SL}_2(\mathbb F_p)\) 均满足
\[
|A^3|\ge |A|^{1+\varepsilon}
\quad\text{或}\quad
A^k=\mathrm{SL}_2(\mathbb F_p).
\]
将其应用于 \(A=B_p\)，得到命题中的增长--闭包二分。

若 \(|B_p^3|<|B_p|^{1+\varepsilon}\) 且 \(B_p\) 生成 \(\mathrm{SL}_2(\mathbb F_p)\)，则由二分只能有 \(B_p^k=\mathrm{SL}_2(\mathbb F_p)\)。于是
\[
|\mathrm{SL}_2(\mathbb F_p)|=|B_p^k|\le |B_p|^k\le 18^k.
\]
但 \(|\mathrm{SL}_2(\mathbb F_p)|=p(p^2-1)\)，这与 \(p(p^2-1)>18^k\) 矛盾。因此在该范围内，非增长读出不可能生成整个 \(\mathrm{SL}_2(\mathbb F_p)\)，只能位于其生成的真子群 \(\langle B_p\rangle<\mathrm{SL}_2(\mathbb F_p)\) 中。最后一个断言即为同一估计在不假定 \(p(p^2-1)>18^k\) 时的形式。证毕。
\end{proof}

\begin{proof}
若 \(\kappa_V=\kappa_{V'}\)，则代入 \(a_4\) 的规范部分公式立即得到 \(\mathsf Q_V(F)=\mathsf Q_{V'}(F)\) 对所有 \(F\) 成立。

反设 \(\mathsf Q_V(F)=\mathsf Q_{V'}(F)\) 对所有可允许局部曲率成立。取 \(M_{\mathrm{adm}}\) 中一个读出坐标片，并取非零二形式 \(\beta\) 支撑于该片。对任意 \(X,Y\in\mathfrak g_{\mathrm{SM}}\) 与任意实数 \(s,t\)，令
\[
F_{s,t}:=\beta\otimes(sX+tY).
\]
由假设，
\[
0=\mathsf Q_V(F_{s,t})-\mathsf Q_{V'}(F_{s,t})
=c\left(\int_{M_{\mathrm{adm}}}\langle\beta,\beta\rangle\,d\mu\right)
\bigl(\kappa_V-\kappa_{V'}\bigr)(sX+tY,sX+tY).
\]
括号中的积分为正，故
\[
\bigl(\kappa_V-\kappa_{V'}\bigr)(sX+tY,sX+tY)=0
\]
对所有 \(s,t\) 成立。比较多项式的 \(st\) 系数，并用对称双线性形式的极化恒等式，得到
\[
(\kappa_V-\kappa_{V'})(X,Y)=0.
\]
由于 \(X,Y\) 任意，故 \(\kappa_V=\kappa_{V'}\)。等价性得证。

若 \(W\) 非平凡，则存在 \(X\in\mathfrak g_{\mathrm{SM}}\) 使 \(\pi_W(X)\ne0\)。因 \(\pi_W(X)\) 为斜 Hermitian 算子，
\[
\kappa_W(X,X)=-\Tr_W(\pi_W(X)^2)>0.
\]
于是 \(\kappa_{V\oplus W}=\kappa_V+\kappa_W\ne\kappa_V\)。然而加入 \(W\) 并不改变命题 \ref{prop:sm-zeckendorf-lie-algebra-rigidity} 所识别的抽象紧李代数直和，也不改变定理 \ref{thm:sm-signature-common-cyclic-identification-divides-6} 中只依赖三层标签的共同循环整除条件。故这些离散签名不能决定 \(a_4\) 中的规范二次型。由已经证明的等价性，所有此类谱作用差异完全由 \(\kappa_{V'}-\kappa_V\) 分类。证毕。
\end{proof}
