...
\end{proof}
\begin{definition}[window-\(11\) 边界的有限内部涨落接口]\label{def:distill-connes-z34-finite-internal-fluctuation-interface}
沿用推论 \ref{cor:m11-z34-rotation-part-complex-structure} 的记号，把
\[
W=\bigoplus_{k=1}^{16}W_k
\]
视为 \(16\) 维复 Hilbert 空间，并记 \(P_k:W\to W_k\) 为对应的复正交投影。一个有限内部涨落接口由有限维 \(*\)-代数 \(\mathcal A_F\)、忠实 \(*\)-表示 \(\pi:\mathcal A_F\to\mathrm{End}_{\CC}(W)\)、自伴算子 \(D_F\in\mathrm{End}_{\CC}(W)\)，以及可选的反酉实结构 \(C\) 组成。其一形式空间定义为
\[
\Omega^1_{D_F}(\mathcal A_F):=\operatorname{span}_{\CC}\{\pi(a_0)[D_F,\pi(a_1)]:a_0,a_1\in\mathcal A_F\}.
\]
自伴内涨落方向为
\[
\Omega^{1,\mathrm{sa}}_{D_F}(\mathcal A_F):=\{A\in\Omega^1_{D_F}(\mathcal A_F):A=A^*\}.
\]
若给定 \(C\)，则实结构对称后的方向为 \(A+CAC^{-1}\)；若未给定 \(C\)，则只使用 \(A\)。
\end{definition}

\begin{proposition}[对角内部代数的内涨落支撑限制]\label{prop:distill-connes-z34-diagonal-internal-fluctuation-support}
取对角内部代数
\[
\mathcal A_{\mathrm{diag}}:=\bigoplus_{k=1}^{16}\CC P_k\subset\mathrm{End}_{\CC}(W).
\]
对任意自伴复线性算子 \(D_F\)，定义无向支撑图 \(\mathcal E(D_F)\) 的顶点为 \(\{1,\ldots,16\}\)，并在 \(i\ne j\) 且 \(P_iD_FP_j\ne0\) 时连边。则每个
\[
T\in\Omega^1_{D_F}(\mathcal A_{\mathrm{diag}})
\]
满足
\[
P_iTP_i=0\qquad(1\le i\le16),
\]
并且若 \(\{i,j\}\notin\mathcal E(D_F)\)，则
\[
P_iTP_j=0.
\]
特别地，若 \(D_F\) 与 \(\mathcal A_{\mathrm{diag}}\) 对易，则 \(\Omega^1_{D_F}(\mathcal A_{\mathrm{diag}})=0\)。更一般地，任何要求候选 Lie 代数的切向量均来自 \(i\Omega^{1,\mathrm{sa}}_{D_F}(\mathcal A_{\mathrm{diag}})\) 的一阶内涨落验收规则，都不能从该对角接口中验收非零对角 Cartan 方向
\[
i\left\{\sum_{k=1}^{16}t_kP_k:\ t_k\in\RR,\ \sum_{k=1}^{16}t_k=0\right\}\subset\mathfrak{su}(W,J).
\]
因此，定理 \ref{thm:m11-z34-uplift-intrinsic-su16} 给出的 \(\mathrm{SU}(16)\) 包络只有在另行给出非对角内部代数、非平凡一形式账本及相应酉商关系后，才能被解释为内涨落产生的候选场方向。
\end{proposition}
\begin{proof}
任取
\[
a=\sum_{k=1}^{16}a_kP_k\in\mathcal A_{\mathrm{diag}}.
\]
对 \(i,j\) 有
\[
P_i[D_F,a]P_j=(a_j-a_i)P_iD_FP_j.
\]
特别地，当 \(i=j\) 时该压缩为零；当 \(P_iD_FP_j=0\) 时该压缩也为零。再取
\[
a_0=\sum_{k=1}^{16}b_kP_k,
\]
则
\[
P_i\,a_0[D_F,a]\,P_j=b_i(a_j-a_i)P_iD_FP_j.
\]
线性张成后得到对所有 \(T\in\Omega^1_{D_F}(\mathcal A_{\mathrm{diag}})\) 的两条支撑结论。

若 \(D_F\) 与 \(\mathcal A_{\mathrm{diag}}\) 对易，则每个 \([D_F,a]\) 为零，故一形式空间为零。最后，任一非零对角 Cartan 元 \(X=i\sum_kt_kP_k\) 至少有某个 \(P_iXP_i=it_iP_i\ne0\)。但上面已证 \(i\Omega^{1,\mathrm{sa}}_{D_F}(\mathcal A_{\mathrm{diag}})\) 的所有元素在每个对角块上的压缩均为零，故 \(X\) 不属于该一阶内涨落切空间。由于 \(\mathfrak{su}(W,J)\) 含有非零对角 Cartan 方向，单纯的 \(\mathrm{SU}(16)\) 包络不能由该对角接口的一形式账本推出。证毕。
\end{proof}
