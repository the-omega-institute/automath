...
\begin{proposition}[Čech 见证向 Hard 见证的线性预算下界]\label{prop:distill-cdim-cech-hard-linear-budget}
\textbf{依赖状态：obstruction.}
设 \(\mathcal U_0,\dots,\mathcal U_{k-1}\) 为有限前缀站点，\(\mathcal A_i\) 为 \(\mathcal U_i\) 上的阿贝尔群层，并令
\[
[\alpha_i]\in \check H^2(\mathcal U_i,\mathcal A_i)
\]
为第 \(i\) 层 \(\mathrm{Cech}\)-型 \(\mathsf{NULL}\) 见证的障碍类。
假设存在有限生成阿贝尔群 \(M\) 与账本同态
\[
\lambda_i:\check H^2(\mathcal U_i,\mathcal A_i)\longrightarrow M
\]
使得
\[
\lambda_0([\alpha_0]),\dots,\lambda_{k-1}([\alpha_{k-1}])
\]
在 \(M\otimes_{\mathbb Z}\mathbb Q\) 中线性无关。

若某一外置方案只允许 \(b\) 个直接和自由寄存器，即其可取消障碍的账本像包含在某个同态
\[
\chi:\mathbb Z^b\longrightarrow M
\]
的像中，则该方案同时取消全部 \([\alpha_i]\) 的必要条件是
\[
b\ge k.
\]
因此，当可用直接和寄存器预算 \(b<k\) 时，持续存在的 \(k\) 个相互独立 Čech 障碍不能被解释为非 \(\mathsf{NULL}\) 地址；它们必被编译为 \(\mathrm{Hard}\) 型预算不足见证。
\end{proposition}
\begin{proof}
若外置方案能同时取消全部障碍，则对每个 \(i\) 都有
\[
\lambda_i([\alpha_i])\in\mathrm{im}(\chi).
\]
张量 \(\mathbb Q\) 后，\(\mathrm{im}(\chi\otimes\mathbb Q)\) 是 \(M\otimes\mathbb Q\) 的子空间，维数不超过 \(b\)。
但该子空间包含 \(k\) 个线性无关向量 \(\lambda_i([\alpha_i])\otimes1\)，故其维数至少为 \(k\)。
于是 \(b\ge k\)。
当 \(b<k\) 时，直接和寄存器的自由预算不足以吸收这些二阶粘合障碍；按照本节的三类失败标签，只能转化为硬阈值资源不足见证。证毕。
\end{proof}
