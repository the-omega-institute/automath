...
% --- Distillation writeback: exact container shadow for unitary-slice audits ---
\begin{definition}[结果 19：unitary-slice exact container shadow]\label{distill:wang_zahl-convex_container_non_concentration_implies_maximal_support-cdim-exact-container-shadow}
\textbf{日期时间：2026-04-22T17:42:33+08:00. 依赖状态：construction.}
固定一个与 SPG 端完全相同的 dyadic 网格维数与分辨率 \((m,n)\)，并令
\[
\mathcal G_{m,n}=\{0,\ldots,2^m-1\}^n.
\]
设
\[
\mathfrak S=(\mathfrak C_m,B,S,\mathscr A_m)
\]
为定义 \ref{distill:wang_zahl-convex_container_non_concentration_implies_maximal_support-spg-support-separating-datum} 中同一 \((m,n)\)-网格上的 support-separating SPG container audit datum。对 unitary-slice 预过滤后的有限地址族 \(E\subset\mathcal A_m\)，称映射
\[
\tau:E\longrightarrow\mathcal G_{m,n}
\]
给出 \(E\) 的 exact \(\mathfrak S\)-container shadow，若其 active shadow
\[
\operatorname{Supp}_{m,n}^{\tau}(E):=\tau(E)\subset\mathcal G_{m,n}
\]
属于 \(\mathscr A_m\)，并且每个 container occupancy 被定义为支撑占据数
\[
\operatorname{Occ}_{\mathcal K}^{\tau}(E):=\bigl|\tau(E)\cap\mathcal K\bigr|\qquad(\mathcal K\in\mathfrak C_m).
\]
若某个查询 \(q\) 的 unitary-slice Hard-axis verifier \(\mathcal V_{\mathrm{Hard}}(q)\) 明确把上述资料 \((m,n,\mathfrak S,\tau)\) 注册为其合法支撑预算子审计，并以全部不等式
\[
\operatorname{Occ}_{\mathcal K}^{\tau}(E)\le B_{\mathcal K}\qquad(\mathcal K\in\mathfrak C_m)
\]
作为 Hard-axis pass 条件的一部分，则称该 exact container shadow 对 \(q\) 是 authorized 的。未作此注册时，container overload 只是一份支撑预算审计资料，而不自动构成定义 \ref{def:cdim-null-witness-tag} 意义下的 \(\mathrm{Hard}\)-型 \(\mathsf{NULL}\) 失败见证。
\end{definition}
% --- End distillation writeback ---
