...
本小节从完成化 resultant 到端点 jet、根分离与大 \(m\) 渐近式的结论，对应独立子论文定理 1.2 的渐近包
...
\]
继续隐式微分至六阶（或在 \((w,s)=(1/3,2)\) 处作隐函数展开）可得到“色散曲线”的局部 Taylor 展开：令 \(\delta:=2-s\)，则
\[
\boxed{\;
\begin{aligned}
\rho(2-\delta)
...
\end{aligned}\;}
\]
结合端点逼近 \(s_m=2\cos(\pi/m)\)，
对 \(\delta=2-s_m\) 作 Taylor 展开并代入上式，即可恢复
\[
\rho_m
...
+O(m^{-8}),
\]
以及推论 \ref{cor:sync-rho-gap-m8-diffusive} 中列出的 \(m^{-12}\) 展开；这些系数与独立子论文命题 3.8 的六阶 jet 一致
\cite{Internal2026SelfDualSynchronisationKernelCyclotomicTwists}。
\end{remark}
...
\begin{lemma}[根分离：\texorpdfstring{$R_m$}{Rm} 的最小模根来自端点分支]\label{lem:sync-root-separation}
令 \(w^\ast(s)\) 为方程 \(\widehat\Delta(w,s)=0\) 在 \((w,s)=(1/3,2)\) 附近的解析根分支，并置
\(\rho(s):=1/w^\ast(s)\)。则对所有充分大的 \(m\)，\(R_m(w)\) 的最小模零点模长由
\(\widehat\Delta(w,s_m)=0\) 中靠近 \(w=1/3\) 的分支达到。特别地，
\[
\boxed{\;\rho_m=\rho(s_m),\qquad s_m=2\cos(\pi/m).\;}
\]
\leanverified{paper\_self\_dual\_sync\_root\_separation}
\end{lemma}
\begin{proof}[证明要点]
令
\[
A_m:=\left\{2\cos\frac{\pi a}{m}:a\in(\ZZ/2m\ZZ)^\times/\{\pm1\}\right\}
\]
为 \(s_m\) 的共轭集合。由结式的根分解公式，
\[
...
\]
其中 \(w=1/3\) 为单根，且其余根与该根在模长上有正间隔。
因此由根的连续性与隐函数定理，存在 \(\varepsilon>0\)，使得当
\(|s-2|<\varepsilon\) 时，\(\widehat\Delta(\cdot,s)\) 在 \(w=1/3\) 附近有唯一简单根
\(w^\ast(s)\)，而其余根的模长一致地远离该局部分支。
\par
另一方面，函数
\[
...
\]
在 \([-2,2]\) 上连续，且由对称性
\(\widehat\Delta(w,-s)=\widehat\Delta(-w,s)\) 得 \(\lambda(-s)=\lambda(s)\)。
在去掉 \(\pm2\) 的小邻域后的紧集上，\(\lambda(s)\) 与端点局部分支之间存在一致正间隔。
仍可能靠近端点的共轭可直接比较：除 \(\alpha=\pm s_m\) 外，靠近 \(\pm2\) 的共轭满足
\(|\alpha|\le 2\cos(3\pi/m)<s_m\)。又
\[
(w^\ast)'(2)=-\frac{11}{153}<0,
\]
故在端点邻域内 \(w^\ast(s)\) 随实参数 \(s\) 增大而严格减小，因而这些共轭对应的端点分支根模长仍严格大于 \(w^\ast(s_m)\)。
若 \(-s_m\) 也是共轭，则由 \(s\mapsto -s\) 对称性只给出相反号的同模根，不改变最小模值。
结合紧集上的一致间隔，所有非端点因子
\(\widehat\Delta(w,\alpha)\) 的根模长都严格大于 \(w^\ast(s_m)\) 的模长。
故 \(R_m\) 的最小模值由 \(\widehat\Delta(w,s_m)\) 的端点分支达到，进而
\(\rho_m=1/w^\ast(s_m)=\rho(s_m)\)。
完整的根分离证书见独立子论文引理 3.9
\cite{Internal2026SelfDualSynchronisationKernelCyclotomicTwists}。证毕。
\end{proof}
...
由引理 \ref{lem:sync-root-separation}，对充分大的 \(m\) 有
\(\rho_m=\rho(s_m)\)，其中 \(s_m=2\cos(\pi/m)=2\cos(t/2)\)。
...
由引理 \ref{lem:sync-root-separation}，可先把充分大 \(m\) 下的最小模 resultant 根识别为端点解析分支，即
\(\rho_m=\rho(s_m)\)、\(s_m=2\cos(\pi/m)=2\cos(t/2)\)。
