...
\leanverified{paper\_xi\_kernel\_dirichlet\_euler\_rational\_smith\_completeness}
设 \(A\in M_{m\times n}(\ZZ)\) 满足 \(\operatorname{rank}_{\QQ}(A)=m\)，其 Smith 不变量为
\[
...
\begin{corollary}[整数轴对齐椭圆半群不存在任何有限加法账本的忠实同态实现]\label{cor:xi-integer-ellipse-no-faithful-finite-additive-ledger}\leanverified{paper\_xi\_integer\_ellipse\_no\_faithful\_finite\_additive\_ledger}
