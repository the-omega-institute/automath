...
(1+o(1))\,n\log n.
\]
\leanpartial{paper\_xi\_time\_part9zl\_gcd\_lcm\_compiler\_primorial\_lower\_bound}{The current Lean signature is false for an arbitrary strictly increasing prime sequence `p`; a counterexample exists unless `p` is fixed to the first `n-1` primes or given an explicit minimality hypothesis.}
\end{theorem}
\begin{proof}
