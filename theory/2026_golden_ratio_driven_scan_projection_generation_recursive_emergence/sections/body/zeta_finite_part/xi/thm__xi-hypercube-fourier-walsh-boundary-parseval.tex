...
\leanpartial{paper\_xi\_hypercube\_fourier\_walsh\_boundary\_parseval\_zero}{仅 $n=0$ 平凡实例 + indicator 辅助引理；通用 $n$ Parseval 需要对偶正交 $\sum_I \chi_I(w)\chi_I(v) = [w=v]\cdot 2^n$ 待后轮，论文 (a) 边界形式同样未形式化}
\leanverified{parseval\_zero}
\leanverified{indicatorInt}
\leanverified{indicatorInt\_sq\_eq}
\leanverified{sum\_indicatorInt}
\end{proof}
