...
\leanpartial{paper\_pom\_diagonal\_rate\_coarsegraining\_bounds}{Requested Lean signature is false as stated: it quantifies arbitrary functions \(R_w,R_W:\mathbb R\to\mathbb R\), but from only \(0<c_\pi\le 1\) one cannot derive \(R_W(\delta)\le R_w(\delta)\) for all \(\delta\in[0,1]\); for example \(R_W\equiv 1\), \(R_w\equiv 0\), \(\delta=0\).}
