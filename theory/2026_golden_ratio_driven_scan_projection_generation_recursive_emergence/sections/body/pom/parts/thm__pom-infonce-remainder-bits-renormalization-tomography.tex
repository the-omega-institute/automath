...
\leanpartial{paper\_pom\_infonce\_fiber\_spectrum\_tomography\_minimality}{the requested Lean signature is not derivable from its abstract assumptions alone: from \texttt{recoverableFromNMinusOne} and \texttt{continuousInjectionFromFewer : \mathbb N \to Prop} one cannot prove \(\forall r < N-1,\ \neg\,\texttt{continuousInjectionFromFewer}\ r\) without an additional minimality hypothesis}
固定分辨率 \(m\)，记
\[
X_m=\{x_1,\dots,x_N\},
\qquad
