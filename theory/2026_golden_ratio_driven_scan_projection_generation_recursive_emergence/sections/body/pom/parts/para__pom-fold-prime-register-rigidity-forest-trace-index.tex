...
\leanpartial{paper\_pom\_prime\_register\_strategy\_gauge\_groupoid}{the requested Lean signature is false: with \(\Omega=\mathbb{N}\), \(X=\{\ast\}\), \(R=\mathbb{N}\), \(\mathrm{foldS}(n)=(\ast,n)\), \(\mathrm{unliftS}(\ast,n)=n\), and \(\mathrm{foldS'}(n)=(\ast,n+1)\), the transport map misses \((\ast,0)\)}
