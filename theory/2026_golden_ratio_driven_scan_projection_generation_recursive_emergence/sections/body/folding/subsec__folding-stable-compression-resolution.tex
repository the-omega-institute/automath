...
\begin{definition}[折叠纤维的整数深度 sticky 测试]\label{def:distill-folding-integer-depth-sticky-test}
\textbf{依赖状态：construction.}
固定分辨率 \(m\)，并沿用 \(\Fold_m:\Omega_m\to X_m\) 与 \(d_m(x)=|\Fold_m^{-1}(x)|\) 的记号。令
\[
\gamma_{m,h}:X_m\longrightarrow Y_h
\]
为一个有限 coarse carrier 映射，其中 \(Y_h\) 是有限集合，\(h<m\)。给定非空 \(E\subset X_m\) 与整数 \(q\ge 1\)，定义 escort 质量
\[
\nu_{E,q}(A):=
\frac{\sum_{x\in A\cap E}d_m(x)^q}{\sum_{x\in E}d_m(x)^q}
\qquad(A\subset X_m).
\]
给定整数 \(s\ge1\)，称 \(E\) 在 \(\gamma_{m,h}\) 上为 \(s\)-sticky，若存在 \(y\in Y_h\) 使
\[
\nu_{E,q}\bigl(E\cap\gamma_{m,h}^{-1}(y)\bigr)>2^{-s}.
\]
若对所有 \(y\in Y_h\) 都有上述质量不超过 \(2^{-s}\)，则称 \(E\) 为 \(s\)-non-sticky。
\end{definition}

\begin{proposition}[non-sticky 分支给出 coarse-carrier 深度增益]\label{prop:distill-folding-nonsticky-carrier-depth-gain}
\textbf{依赖状态：rigidity.}
沿定义 \ref{def:distill-folding-integer-depth-sticky-test}。若非空有限族 \(E\subset X_m\) 为 \(s\)-non-sticky，则其 occupied coarse carrier 集
\[
C_{m,h}(E):=\gamma_{m,h}(E)
\]
满足
\[
|C_{m,h}(E)|\ge 2^s,
...
且 carrier 熵
\[
H_{m,h}(E):=-\sum_{y\in C_{m,h}(E)}p_y\log p_y,\qquad
p_y:=\nu_{E,q}\bigl(E\cap\gamma_{m,h}^{-1}(y)\bigr),
\]
满足
\[
H_{m,h}(E)\ge s\log 2.
\]
等价地，若 \(|C_{m,h}(E)|<2^s\)，则 \(E\) 必为 \(s\)-sticky。因而在给定 coarse-fine carrier 映射后，non-sticky 不是单纯的非集中描述，而是一个精确的整数 residual-depth gain：至少 \(s\) 个二进深度单位必须在 coarse carrier 支撑中显影。
\end{proposition}
\begin{proof}
设 \(E\) 为 \(s\)-non-sticky。则对每个 \(y\in C_{m,h}(E)\) 都有 \(p_y\le 2^{-s}\)，且 \(\sum_y p_y=1\)。于是
\[
1=\sum_{y\in C_{m,h}(E)}p_y\le |C_{m,h}(E)|\,2^{-s},
\]
从而 \(|C_{m,h}(E)|\ge2^s\)。熵下界由标准有限概率不等式
\[
H_{m,h}(E)=\sum_y p_y\log\frac1{p_y}\ge \sum_y p_y\log(2^s)=s\log2
\]
直接得到。最后一句是第一项的逆否命题：若 occupied carrier 少于 \(2^s\)，则不可能所有 carrier 质量都不超过 \(2^{-s}\)，故存在一个 carrier 质量严格大于 \(2^{-s}\)，即为 \(s\)-sticky。证毕。
\end{proof}
