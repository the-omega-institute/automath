...
\leanpartial{paper\_fold\_gauge\_anomaly\_chernoff\_nonasymptotic}{the requested Lean signature quantifies over an arbitrary rate function \(I : \mathbb{R} \to \mathbb{R}\) and arbitrary tail functions `upperTail` and `lowerTail`, but the paper argument uses the concrete gauge-anomaly laws together with a finite-dimensional mgf/Jordan bound, so the stated universal conclusion is not derivable from `D.legendreFenchelFormula` alone}
...
\leanpartial{paper\_fold\_gauge\_anomaly\_two\_constants\_closure}{Current Lean signature quantifies over an arbitrary proposition `gcDefectTaylor3`, so the stated conjunction is not derivable without an additional hypothesis witnessing that proposition.}
