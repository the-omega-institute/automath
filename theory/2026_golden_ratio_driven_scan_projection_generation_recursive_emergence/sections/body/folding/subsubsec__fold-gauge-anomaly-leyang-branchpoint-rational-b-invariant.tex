...
\end{proof}
\begin{proposition}[分歧有理图上的 alien--fold 桥刚性]\label{distill:ecalle_resurgence_mould_calculus_and_alien_derivatives-alien_fold_bridge_family-fold-branchpoint-rational-chart-bridge-rigidity}
在特化 \(t=2\) 下，限制到命题 \ref{prop:fold-gauge-anomaly-branchpoint-rational-reduction} 的开图 \(\mu^2+1\neq0\)，并令
\[
\widetilde{\mathcal A}:=\CC[\mu,u,(\mu^2+1)^{-1}],\qquad
r:=u-\mathcal R(\mu),\qquad
\mathcal A_{\mathrm{br}}:=\widetilde{\mathcal A}/(r).
\]
把 native fold evolution 写成 \(\nabla_{\mathrm{br}}\)，其中 \(\nabla_{\mathrm{br}}\mu=1\)，故在 \(\mathcal A_{\mathrm{br}}\cong \CC[\mu,(\mu^2+1)^{-1}]\) 上 \(\nabla_{\mathrm{br}}=d/d\mu\)。若 residual 地址 \(\alpha\) 上的 alien residual extraction \(\Delta_\alpha\) 是 \(\mathcal A_{\mathrm{br}}\) 的 \(\CC\)-导子，则存在唯一的
\[
f_\alpha:=\Delta_\alpha(\mu)\in\mathcal A_{\mathrm{br}}
\]
使得对所有 \(a\in\mathcal A_{\mathrm{br}}\) 都有 bridge-equation transfer
\[
\Delta_\alpha a=f_\alpha\nabla_{\mathrm{br}}a.
\]
特别地，分歧坐标满足强制桥律
\[
\Delta_\alpha u=f_\alpha\mathcal R'(\mu).
\]

反之，设 \(\widetilde\Delta_\alpha\) 是 \(\widetilde{\mathcal A}\) 上的候选 \(\CC\)-导子。定义其分歧桥缺陷类为
\[
\mathfrak o_\alpha:=[\widetilde\Delta_\alpha r]
=[\widetilde\Delta_\alpha u-\mathcal R'(\mu)\widetilde\Delta_\alpha\mu]
\in \mathcal A_{\mathrm{br}}.
\]
则 \(\widetilde\Delta_\alpha\) 下降为 \(\mathcal A_{\mathrm{br}}\) 上的 alien residual extraction 当且仅当 \(\mathfrak o_\alpha=0\)。若 \(\mathfrak o_\alpha\neq0\)，则该失败不是 \(u\)-坐标规范化造成的：对任意满足 \(G(r)=(r)\) 且在商环上诱导自同构的分歧规范化 \(G\)，传输导子 \(G\widetilde\Delta_\alpha G^{-1}\) 的缺陷类为 \(G_\ast(h\mathfrak o_\alpha)\)，其中 \([h]\in\mathcal A_{\mathrm{br}}^\times\) 为单位。因此其零与非零状态不变。
\end{proposition}
\begin{proof}
由命题 \ref{prop:fold-gauge-anomaly-branchpoint-rational-reduction}，在 \(\mu^2+1\neq0\) 的分歧图上有 \(u=\mathcal R(\mu)\)，故
\[
\mathcal A_{\mathrm{br}}\cong \CC[\mu,(\mu^2+1)^{-1}].
\]
设 \(\Delta_\alpha\) 是该环上的 \(\CC\)-导子，并令 \(f_\alpha=\Delta_\alpha\mu\)。对任意多项式 \(p(\mu)\)，导子规则给出
\[
\Delta_\alpha p(\mu)=f_\alpha p'(\mu).
\]
对局部化元，由
\[
0=\Delta_\alpha(1) = \Delta_\alpha((\mu^2+1)(\mu^2+1)^{-1})
\]
得
\[
\Delta_\alpha((\mu^2+1)^{-1})=-2\mu f_\alpha(\mu^2+1)^{-2},
\]
因而同一公式延拓到整个局部化环。于是 \(\Delta_\alpha=f_\alpha\nabla_{\mathrm{br}}\)，唯一性由 \(f_\alpha=\Delta_\alpha\mu\) 立即得到。代入 \(u=\mathcal R(\mu)\) 便得
\[
\Delta_\alpha u=\Delta_\alpha\mathcal R(\mu)=f_\alpha\mathcal R'(\mu).
\]

对反向断言，候选导子 \(\widetilde\Delta_\alpha\) 能下降到商环 \(\mathcal A_{\mathrm{br}}=\widetilde{\mathcal A}/(r)\) 当且仅当它保持理想 \((r)\)，即 \(\widetilde\Delta_\alpha r\in(r)\)。这等价于其在商环中的类
\[
[\widetilde\Delta_\alpha r]=\mathfrak o_\alpha
\]
为零。若该类为零，则下降导子存在，且由前半部分必满足 bridge law；若该类非零，则任何下降导子都会与前半部分矛盾，故不存在。

最后考察规范化。设 \(G\) 为保持分歧理想的环自同构。由于 \((r)\) 是主理想，存在 \(h\in\widetilde{\mathcal A}\) 使
\[
G^{-1}r=hr.
\]
因为 \(G^{-1}\) 在商环上仍诱导自同构，\([h]\) 必为 \(\mathcal A_{\mathrm{br}}\) 中的单位。传输导子的缺陷为
\[
[G\widetilde\Delta_\alpha G^{-1}r]
=[G\widetilde\Delta_\alpha(hr)]
=[G(\widetilde\Delta_\alpha(h)r+h\widetilde\Delta_\alpha r)].
\]
第一项含因子 \(r\)，在商环中为零；故缺陷类等于
\[
G_\ast([h]\,[\widetilde\Delta_\alpha r])=G_\ast(h\mathfrak o_\alpha).
\]
由于 \([h]\) 是单位且 \(G_\ast\) 是自同构，\(G_\ast(h\mathfrak o_\alpha)\) 为零当且仅当 \(\mathfrak o_\alpha\) 为零。于是非零桥缺陷不能通过分歧坐标规范化消去。证毕。
\end{proof}
