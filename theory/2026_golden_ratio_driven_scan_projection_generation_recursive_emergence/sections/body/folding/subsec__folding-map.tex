...
\end{proposition}
\begin{proposition}[小倍增 Fold 影像的 Ruzsa 纤维覆盖]\label{prop:distill-fold-ruzsa-fiber-atlas}
固定 \(m\geq 1\)，记 \(M:=F_{m+2}\)。令
\(
\alpha_m:\ZZ/(M\ZZ)\to X_m
\)
为 \(V_m:X_m\to \ZZ/(M\ZZ)\) 的逆映射；其存在性由命题 \ref{prop:fold-inverse-limit-xm-xinfty} 的范围双射给出。对非空集合 \(A\subset\Omega_m\)，定义其 Fold 值影像
\[
B_A:=\{\,V_m(\Fold_m(\omega))\bmod M:\ \omega\in A\,\}\subset\ZZ/(M\ZZ).
\]
若在循环群 \(\ZZ/(M\ZZ)\) 中满足小倍增条件
\[
|B_A+B_A|\leq K|B_A|,
\]
则存在 \(T\subset B_A\) 使得 \(|T|\leq K\) 且
\[
B_A\subset T+(B_A-B_A).
\]
因此，若 \(d_m(x):=|\Fold_m^{-1}(x)|\)，则
\[
A\subset
\bigcup_{t\in T}\ \bigcup_{b,b'\in B_A}
\Fold_m^{-1}\bigl(\alpha_m(t+b-b')\bigr),
\]
并有纤维预算估计
\[
|A|\leq
\sum_{t\in T}\sum_{b,b'\in B_A}
 d_m\bigl(\alpha_m(t+b-b')\bigr)
\leq K|B_A|^2\max_{x\in X_m}d_m(x).
\]
所有加减法均在 \(\ZZ/(M\ZZ)\) 中进行。
\end{proposition}
\begin{theorem}[折叠重写的活动支撑预算与有限面账本]\label{thm:distill-khovanskii-fold-active-support-finite-chamber}
对 \(a\in\mathcal D\)，置
\[
b(a):=\sum_{k\ge1}a_k,\qquad
K(a):=
\begin{cases}
0,&\mathrm{Val}(a)=0,\\
\max\{k:\ F_{k+1}\le \mathrm{Val}(a)\},&\mathrm{Val}(a)>0.
\end{cases}
\]
令 \(\mathcal R(a)\) 为从 \(a\) 出发经本小节四类局部重写规则可达的全部配置集合。则对任意 \(a'\in\mathcal R(a)\)，有
\[
\mathrm{Val}(a')=\mathrm{Val}(a),\qquad
b(a')\le b(a),\qquad
\operatorname{supp}(a')\subseteq\{1,\ldots,K(a)\}.
\]
因此可达支撑族满足有限估计
\[
\#\{\operatorname{supp}(a'):\ a'\in\mathcal R(a)\}
\le
\sum_{j=0}^{b(a)}\binom{K(a)}{j}.
\]
进一步，若对有限支撑 \(S\subset\mathbb N\) 与 valuation \(\xi\in\mathbb R\) 定义
\[
\operatorname{in}_\xi(S):=\{k\in S:\ \xi k=\min_{s\in S}\xi s\},
\]
则全部可达支撑的 valuation-selected face survivor 集合
\[
\{\operatorname{in}_\xi(\operatorname{supp}(a')):\ a'\in\mathcal R(a),\ \xi\in\mathbb R\}
\]
也是有限的。换言之，在固定初始活动质量与固定 Fibonacci 值界下，折叠的支撑切换只需要有限 chamber audit。
\end{theorem}
\begin{proof}
值保持性已由四类重写规则对应的 Fibonacci 恒等式给出。考察活动质量 \(b\)：相邻合并与底位去重使总质量减少一，第二位去重与一般位去重保持总质量不变。因此每一步重写均满足 \(b(a_{new})\le b(a_{old})\)，沿任意可达链得到 \(b(a')\le b(a)\)。

若 \(a'_k>0\)，则由于所有数字非负，
\[
F_{k+1}\le a'_kF_{k+1}\le \sum_{i\ge1}a'_iF_{i+1}=\mathrm{Val}(a')=\mathrm{Val}(a).
\]
按 \(K(a)\) 的定义可得 \(k\le K(a)\)。故 \(\operatorname{supp}(a')\subseteq\{1,\ldots,K(a)\}\)。又 \(|\operatorname{supp}(a')|\le b(a')\le b(a)\)，所以可能出现的支撑集合至多为 \(\{1,\ldots,K(a)\}\) 中大小不超过 \(b(a)\) 的子集数，得到所述估计。

最后，对每一个已经固定的有限支撑 \(S\)，集合 \(\operatorname{in}_\xi(S)\) 随 \(\xi\) 只能取有限多个值；而可达支撑族本身有限，故全部 face survivor 也有限。这正是 Newton support 先行、valuation 只在有限支撑上选面的有限性断言。
\end{proof}

\begin{proof}
这是 Ruzsa 覆盖引理在 Fold 值影像上的直接有限化。取 \(T\subset B_A\) 极大，使得平移族 \(\{t+B_A:t\in T\}\) 两两不交。由于这些平移都包含于 \(B_A+B_A\)，有
\[
|T|\,|B_A|\leq |B_A+B_A|\leq K|B_A|,
\]
故 \(|T|\leq K\)。由极大性，对任意 \(y\in B_A\)，平移 \(y+B_A\) 必与某个 \(t+B_A\) 相交，于是存在 \(b,b'\in B_A\) 使
\[
y+b'=t+b,
\qquad\text{即}\qquad y=t+b-b'.
\]
这给出 \(B_A\subset T+(B_A-B_A)\)。

若 \(\omega\in A\)，令 \(y=V_m(\Fold_m(\omega))\)。由上式可写成 \(y=t+b-b'\)。由于 \(\alpha_m\) 是 \(V_m\) 的逆映射，得到
\[
\Fold_m(\omega)=\alpha_m(t+b-b'),
\]
从而 \(\omega\) 落入所列纤维并得到覆盖。最后的计数估计由并集上界和 \(|T|\leq K\) 给出。证毕。
\end{proof}
