...
\end{enumerate}
\end{corollary}
\begin{definition}[shift Newton 支撑与稀疏判别账本]\label{def:distill-khovanskii-gauge-shift-newton-denominator}
将 \(\widetilde M_m(u)\) 的四阶递推移到左端，并以 \(z\) 记录 shift mode，定义稀疏 Newton 支撑
\[
A:=\{(4,0),(3,0),(2,0),(2,1),(1,1),(1,3),(0,1)\}\subset\mathbb Z^2
\]
及其 shift Newton 多项式
\[
D_A(z,u):=z^4-z^3-(2u+1)z^2-(u^3-2u)z+2u.
\]
定义支撑判别账本为
\[
\Delta_A(u):=\operatorname{Res}_z\bigl(D_A(z,u),\partial_zD_A(z,u)\bigr)\in\mathbb Z[u].
\]
\end{definition}

\begin{proposition}[shift mode 碰撞的稀疏 resultant 障碍]\label{prop:distill-khovanskii-gauge-moment-sparse-resultant-obstruction}
沿用定义 \ref{def:distill-khovanskii-gauge-shift-newton-denominator}。若 \(u_0\in\mathbb C\) 满足 \(\Delta_A(u_0)\ne0\)，则方程
\[
D_A(z,u_0)=0
\]
的四个根均为单根，并且在 \(u_0\) 的某个邻域内可写成四条全纯分支
\[
z_1(u),z_2(u),z_3(u),z_4(u).
\]
反之，若存在 \(z_0\in\mathbb C\) 使
\[
D_A(z_0,u_0)=0,\qquad \partial_zD_A(z_0,u_0)=0,
\]
则 \(\Delta_A(u_0)=0\)。因此，任何把该有限长度矩闭式提升为单一 shift-mode 分支控制的论证，都必须排除 \(\Delta_A=0\) 上的 denominator-level 碰撞；该障碍由稀疏支撑 \(A\) 与对应系数决定，而不是由初值枚举决定。
\end{proposition}
\begin{proof}
resultant 的基本性质给出
\[
\operatorname{Res}_z(D_A,\partial_zD_A)(u_0)=0
\]
当且仅当 \(D_A(z,u_0)\) 与其 \(z\)-导数在 \(\mathbb C[z]\) 中有公共根。由于 \(D_A\) 关于 \(z\) 是首一四次多项式，\(\Delta_A(u_0)\ne0\) 时它与 \(\partial_zD_A\) 无公共根，故所有根均为单根。对每个单根 \(z_i(u_0)\)，有 \(\partial_zD_A(z_i(u_0),u_0)\ne0\)，由全纯隐函数定理得到邻域内唯一全纯根分支。四个根在 \(u_0\) 处互异，缩小邻域后可同时枚举为四条分支。反向断言同样直接由 resultant 的公共根判据得到。
\end{proof}
