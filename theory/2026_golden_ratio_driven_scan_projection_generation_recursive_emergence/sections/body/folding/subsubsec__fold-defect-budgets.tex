...
\]
\leanverified{paper\_fold\_hamming\_lipschitz\_budget}
\end{corollary}
...
\)。
取期望得第二不等式。证毕。
\leanverified{paper\_hamming\_metric\_axioms}
\end{proof}
...
\leanverified{globalDefect\_triangle}
\leanverified{paper\_globalDefect\_theory\_package}
\leanverified{paper\_fold\_defect\_cocycle}
\end{proposition}
...
\)
对所有 \(m\ge M\) 成立。
\leanverified{paper\_fold\_naive\_prefix\_lift\_seeds}
\leanverified{paper\_fold\_naive\_prefix\_lift}
\leanverified{paper\_fold\_naive\_prefix\_lift\_package}
\end{proposition}
...
\leanpartial{paper\_fold\_naive\_prefix\_lift\_summable\_core}{仅形式化纯 $\mathbb{N}$ 序列代数核（部分和有界 $\Rightarrow$ 尾部为零）；具体 $\mathrm{Fold}_m / X_\infty / \mathrm{restriction}$ 逆系统应用留后轮}
\leanverified{partial\_sum\_mono}
\leanverified{card\_pos\_support\_le}
\leanverified{posSupportRange}
\leanverified{eventually\_zero\_of\_partial\_sum\_bounded}
\end{proof}
