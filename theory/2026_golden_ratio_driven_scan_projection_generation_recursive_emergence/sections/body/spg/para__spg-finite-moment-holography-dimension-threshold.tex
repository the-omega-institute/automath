...
\begin{definition}[dyadic polycube 的有限 container-budget 泛函]\label{def:distill-spg-container-budget-functional}
\textbf{依赖状态：construction.}
固定定理 \ref{thm:spg-linear-moment-holography-minimal-dimension} 中的 \(m,n\)，令 \(N=2^m\)。对任意指标集合
\[
\mathcal K\subset\{0,\ldots,N-1\}^n
\]
定义 dyadic container
\[
C_{\mathcal K}:=\bigcup_{\mathbf k\in\mathcal K} C_{\mathbf k},
\qquad
\ind_{\mathcal K}:=\sum_{\mathbf k\in\mathcal K}\ind_{C_{\mathbf k}}\in\mathcal V_m.
\]
其 container occupancy 泛函定义为
\[
L_{\mathcal K}(f):=2^{mn}\int_{I^n} f(x)\ind_{\mathcal K}(x)\,\dd x
\qquad(f\in\mathcal V_m).
\]
当 \(f\) 是 dyadic polycube 的示性函数时，\(L_{\mathcal K}(f)\) 正是 \(f\) 在 \(C_{\mathcal K}\) 内占据的 \(m\)-级小立方数。
\end{definition}

\begin{proposition}[完整 tensor moment box 审计所有有限 container budget]\label{prop:distill-spg-container-budget-linear-audit}
\textbf{依赖状态：rigidity.}
沿定义 \ref{def:distill-spg-container-budget-functional}。对每个 container \(\mathcal K\)，存在唯一系数族
\[
\bigl(c_{\mathcal K,\alpha}\bigr)_{0\le \alpha_i\le N-1}
\]
使得对所有 \(f\in\mathcal V_m\) 都有
\[
L_{\mathcal K}(f)=
\sum_{0\le\alpha_i\le N-1} c_{\mathcal K,\alpha}\,\mu_\alpha(f).
\]
因此，完整张量积矩盒 \(\mathcal M_m^{\Box}\) 能够离线核验任意有限 dyadic container family 的预算不等式
\[
L_{\mathcal K}(f)\le B_{\mathcal K}.
\]
反之，若某个有限线性观测族 \(\Lambda_m=\{\ell_1,\ldots,\ell_L\}\subset\mathcal V_m^\ast\) 的线性张成不包含 \(L_{\mathcal K}\)，则存在 signed density \(g\in\mathcal V_m\) 使
\[
\ell_i(g)=0\quad(1\le i\le L),
\qquad
L_{\mathcal K}(g)\ne0.
\]
换言之，任何不张成该 container 泛函的低维观测族都存在 signed container camouflage；若要在该观测族下使用 container-budget 谓词，必须把 \(L_{\mathcal K}\) 的可张成性作为额外证书输入。
\end{proposition}
\begin{proof}
定理 \ref{thm:spg-linear-moment-holography-minimal-dimension} 第 (2) 条说明
\[
f\longmapsto (\mu_\alpha(f))_{0\le\alpha_i\le N-1}
\]
是 \(\mathcal V_m\) 到 \(\mathbb R^{N^n}\) 的线性同构。因此其坐标泛函 \(\mu_\alpha\) 构成 \(\mathcal V_m^\ast\) 的一组基。任意线性泛函，特别是 \(L_{\mathcal K}\)，都可唯一写成这些坐标泛函的线性组合，得到第一式。

预算核验只需先由矩盒计算 \(L_{\mathcal K}(f)\)，再与给定整数或有理预算 \(B_{\mathcal K}\) 比较。
最后，设 \(E_\Lambda:\mathcal V_m\to\mathbb R^L\) 为 \(f\mapsto(\ell_1(f),\ldots,\ell_L(f))\)。有限维线性代数给出
\[
(\ker E_\Lambda)^\perp=\operatorname{span}\{\ell_1,\ldots,\ell_L\}.
\]
若 \(L_{\mathcal K}\) 不属于该张成，则它不在 \((\ker E_\Lambda)^\perp\) 中，故存在 \(g\in\ker E_\Lambda\) 使 \(L_{\mathcal K}(g)\ne0\)。证毕。
\end{proof}
