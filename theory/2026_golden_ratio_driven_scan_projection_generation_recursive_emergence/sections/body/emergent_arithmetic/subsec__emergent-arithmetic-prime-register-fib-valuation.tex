...
\leanpartial{paper\_prime\_register\_monoid\_realization}{The stated Lean theorem quantifies over an arbitrary endofunction \(\Gamma\), so it is false for the current concrete definition \(\Omega.EA.DigitCfg = \mathbb{N} \to_0 \mathbb{N}\).}
映射 \(\Gamma\) 是交换幺半群同构。
...
\input{subsubsec__emergent-arithmetic-dynamic-prime-register-godel-dynamics}

在此基础上，外置尺度不再停留在离散指数账本层，而被进一步送入一个可解析观测的复几何门结构。
下一小节给出与该外置寄存器严格兼容的 Gödel--Joukowsky 椭圆化，并据此讨论解析不可见性、径向恢复与零点输运。

\input{subsec__emergent-arithmetic-godel-joukowsky-gate}
