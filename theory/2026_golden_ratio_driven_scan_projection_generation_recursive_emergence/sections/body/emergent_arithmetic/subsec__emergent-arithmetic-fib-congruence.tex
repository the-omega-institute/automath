...
本节的关键点是：\textbf{算术结构并非预置}，而是从尺度计数的自由结构在 Fibonacci 关系下取商之后被强制导出；Zeckendorf 规范形只是该商结构上的一个可计算横截面。
因此，本节处理的不是“先有自然数，再找一种编码来表示它”，而是相反：先有尺度计数的自由交换结构，再对其施加 Fibonacci 关系并取商；自然数结构正是该商幺半群的涌现结果，而 Zeckendorf 规范形则是该商上的唯一规范代表。
...
这里的 \(\mathcal D\) 不是值域，而是值的前对象层：它只记录“第 \(k\) 级单位出现了多少次”，而尚未对不同尺度间的递推兼容性作任何折叠。
...
\]
\leanverified{value\_invariant\_step}
\leanverified{paper\_val\_invariant}
...
\leanverified{paper\_ea\_ring\_iso\_core}
\leanverified{paper\_ea\_ring\_char\_small}
\end{theorem}
...
\]
是一个可计算的截面（选择规范代表）。因此，折叠并不是”把一个外在对象压进一个盒子”，而是先在自由尺度计数层上按 Fibonacci 关系取商，再在每个值类中选取唯一合法代表。这正是”folding = 取不变量/取商 + 选规范形”的抽象机制在 Fibonacci 情形下的具体实现。
...
\begin{remark}[与对象层 forcing 语义的衔接]\label{rem:emergent-arithmetic-object-value-interface}
第 \ref{sec:recursive-addressing} 节讨论的是：一个地址在对象层上何时可读、何时因缺乏合法局部截面而退化为 \(\mathsf{NULL}\)。
本节讨论的则是：一旦对象已经具备合法读出，其值结构如何从尺度计数的自由交换幺半群中内生涌现。
前者是对象层资格问题，后者是值层规范化问题；两者在逻辑层次上彼此衔接，但不可混同。
\end{remark}
