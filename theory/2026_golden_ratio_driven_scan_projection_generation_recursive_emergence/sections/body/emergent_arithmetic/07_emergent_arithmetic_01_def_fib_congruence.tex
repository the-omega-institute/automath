...
\end{theorem}
\begin{remark}[方法算子的有限翻译]\label{distill:connes_consani_arithmetic_site-point_spectrum_obstruction_classification-operator-bridge}
在以下审计中，\texttt{siteify monoid dynamics} 被具体化为：只允许把 \(\mathcal D\) 的平移幺半群动力学读成模 \(q\) 的同态截面 \(\eta_q:\mathcal D\to\mathbb Z/q\mathbb Z\)。\texttt{recover numbers from point classification} 被具体化为：通过 rank-one 观察纤维 \(\mathbb Z/p^m\mathbb Z\) 的点标签恢复 \(\overline{\mathrm{Val}}\) 的剩余类。跨尺度对象只经由 \(r_{q',q}\) 的稳定逆系统比较。
\end{remark}

\begin{theorem}[有限 fold-aware 点谱审计的首次失败分类与 rank-one 尺度预算]\label{distill:connes_consani_arithmetic_site-point_spectrum_obstruction_classification-finite-fold-aware-rank-one-spectrum-audit}
令 \(\mathcal Q\) 为有限非空的模数集合，且若 \(q\in\mathcal Q\)、\(d\mid q\)、\(d\ge2\)，则 \(d\in\mathcal Q\)。对每个 \(q\in\mathcal Q\)，给定交换幺半群同态
\[
\eta_q:\mathcal D\longrightarrow \mathbb Z/q\mathbb Z,
\qquad
u_q:=\eta_q(e_1).
\]
对 \(q\mid q'\) 仍记 \(r_{q',q}:\mathbb Z/q'\mathbb Z\to\mathbb Z/q\mathbb Z\) 为自然约化。定义 Fibonacci 下降缺陷
\[
\delta^{\rm fib}_{q,0}:=\eta_q(e_2)-2\eta_q(e_1),
\qquad
\delta^{\rm fib}_{q,k}:=\eta_q(e_{k+2})-\eta_q(e_{k+1})-\eta_q(e_k)\quad(k\ge1),
\]
以及尺度转移缺陷
\[
\delta^{\rm sc}_{q',q}:=r_{q',q}(\nu_{q'})-\nu_q\qquad(q\mid q',\ q,q'\in\mathcal Q).
\]
按先检查 Fibonacci 下降缺陷、再检查尺度转移缺陷、最后检查 rank-one 纤维尺度的优先级，任一族 \((\eta_q)_{q\in\mathcal Q}\) 恰落入以下三类之一。
\begin{enumerate}
  \item 存在最小的 \((q,i)\) 使 \(\delta^{\rm fib}_{q,i}\ne0\)。此时 \((q,i,\delta^{\rm fib}_{q,i})\) 是首次下降失败点；该族不能下降到 \(\mathcal D/\!\equiv\) 上的观察截面。
  \item 所有 \(\delta^{\rm fib}_{q,i}\) 均为零，但存在最小的可除箭头 \(q\mid q'\) 使 \(\delta^{\rm sc}_{q',q}\ne0\)。此时 \((q',q,\delta^{\rm sc}_{q',q})\) 是首次尺度失败纤维；该族不能给出稳定逆系统截面。
  \item 所有上述缺陷均为零。则每个 \(\eta_q\) 唯一下降为
  \[
  \overline\eta_q([a])=\nu_q\,\overline\rho_q([a])=\nu_q\,[\mathrm{Val}(a)]_q,
  \]
  且所有复合约化自动相容。若每个 \(\nu_q\) 都是 \(\mathbb Z/q\mathbb Z\) 的单位，则存在唯一的相容单位尺度证书 \(v_q\in(\mathbb Z/q\mathbb Z)^\times\) 满足 \(v_q\nu_q=1\)，并且
  \[
  a\longmapsto v_q\eta_q(a)
  \]
  正是标准观察截面 \(\rho_q\)。若某个 \(\nu_q\) 非单位，则存在素数 \(p\) 与 \(m\ge1\) 使 \(p^m\in\mathcal Q\) 且 \(\nu_{p^m}\) 非单位；对最小这样的 rank-one 纤维，令
  \[
  t:=\min\{v_p(\widetilde\nu),m\},
  \]
  其中 \(\widetilde\nu\) 是 \(\nu_{p^m}\) 的任一整数代表。则 \(1\le t\le m\)，且 \(\overline\eta_{p^m}\) 在标准值纤维 \(\mathbb Z/p^m\mathbb Z\) 上恰好压缩模 \(p^{m-t}\) 的陪集；特别地
  \[
  a_0:=0,
  \qquad
  a_1:=p^{m-t}e_1
  \]
  满足 \(\rho_{p^m}(a_0)\ne\rho_{p^m}(a_1)\)，但 \(\eta_{p^m}(a_0)=\eta_{p^m}(a_1)\)。进一步，若允许细化到 \(p^{m+s}\) 并给定相容的下降截面 \(\widetilde\eta_{p^{m+s}}\) 满足 \(\widetilde\eta_{p^{m+s}}(e_1)=U_s\) 与 \(r_{p^{m+s},p^m}(U_s)=\nu_{p^m}\)，则该细化纤维能区分全部 \(p^m\)-值类，当且仅当
  \[
  \min\{v_p(\widetilde U_s),m+s\}\le s,
  \]
  其中 \(\widetilde U_s\) 为 \(U_s\) 的任一整数代表。在此条件成立时，记录 \(p\)-进深度 \(w_s:=\min\{v_p(\widetilde U_s),m+s\}\) 与单位部分逆元就是恢复 \(\rho_{p^m}\) 所需且充分的尺度证书；若该条件不成立，则不存在仅依赖细化点标签 \(\widetilde\eta_{p^{m+s}}(a)\) 的函数能够在所有 \(p^m\)-值类上恢复 \(\rho_{p^m}(a)\)。
\end{enumerate}
因此，在该有限 fold-aware 审计语义中，坏重构的最小见证只能是下降点、尺度箭头或 rank-one 尺度纤维；rank-one 缺陷的闭合必须通过满足上述不等式的观察细化与尺度证书完成，否则该纤维缺陷不能在当前接纳的观察模数内被消去。
\end{theorem}
\begin{proof}
若某个 \(\delta^{\rm fib}_{q,i}\) 非零，则由于 \(\mathcal Q\) 有限且指标集 \(\{0,1,2,\ldots\}\) 良序，存在最小的失败对 \((q,i)\)。Fibonacci 同余 \(\equiv\) 正由 \(e_2\equiv2e_1\) 与 \(e_{k+2}\equiv e_{k+1}+e_k\) 生成；故该非零缺陷说明 \(\eta_q\) 对生成关系不恒定，因而不可能下降到 \(\mathcal D/\!\equiv\)。这给出第一类，并且排除了后续分类的适用性。

以下假设所有 Fibonacci 下降缺陷均为零。由 \(\eta_q\) 的加法性与缺陷为零可归纳得到
\[
\eta_q(e_k)=F_{k+1}\nu_q\qquad(k\ge1).
\]
确实，\(k=1\) 为定义，\(k=2\) 由 \(\eta_q(e_2)=2\eta_q(e_1)=F_3\nu_q\) 给出；若结论对 \(k\) 与 \(k+1\) 成立，则
\[
\eta_q(e_{k+2})=\eta_q(e_{k+1})+\eta_q(e_k)=(F_{k+2}+F_{k+1})\nu_q=F_{k+3}\nu_q.
\]
于是对任意 \(a=\sum_k a_ke_k\in\mathcal D\)，有限支撑性给出
\[
\eta_q(a)=\sum_k a_k\eta_q(e_k)=\left(\sum_k a_kF_{k+1}\right)\nu_q
=\nu_q[\mathrm{Val}(a)]_q.
\]
因此 \(\eta_q\) 对 \(\equiv\) 恒定，并唯一诱导 \(\overline\eta_q([a])=\nu_q\overline\rho_q([a])\)。

在上述表达式下，对任意 \(q\mid q'\) 与任意 \(a\in\mathcal D\)，有
\[
r_{q',q}(\eta_{q'}(a))-\eta_q(a)
=[\mathrm{Val}(a)]_q\bigl(r_{q',q}(\nu_{q'})-\nu_q\bigr).
\]
取 \(a=e_1\) 可知转移相容等价于 \(\delta^{\rm sc}_{q',q}=0\)。若存在非零尺度缺陷，则有限可除箭头集中存在最小失败箭头，得到第二类。若所有尺度缺陷均为零，则 \(r_{q',q}(\nu_{q'})=\nu_q\)，从而对任何复合 \(q\mid q'\mid q''\) 亦有
\[
r_{q'',q}(\nu_{q''})=r_{q',q}(r_{q'',q'}(\nu_{q''}))=r_{q',q}(\nu_{q'})=\nu_q,
\]
所以不存在额外的复合约化失败。
...
=r_{q',q}(v_{q'}\nu_{q'})=1,
\]
故 \(r_{q',q}(v_{q'})=v_q\)。于是 \((v_q)_q\) 是唯一相容尺度证书，并且
\[
v_q\eta_q(a)=v_q\nu_q[\mathrm{Val}(a)]_q=[\mathrm{Val}(a)]_q=\rho_q(a).
\]

剩余情形中某个 \(\nu_q\) 非单位。取素数 \(p\mid\gcd(\widetilde\nu_q,q)\)。由于 \(\mathcal Q\) 对除数封闭，\(p^m\in\mathcal Q\)，其中 \(m=v_p(q)\ge1\)；并且 \(\nu_{p^m}=r_{q,p^m}(\nu_q)\) 仍非单位。对最小的此类 \((p,m)\)，令 \(t=\min\{v_p(\widetilde\nu),m\}\)。映射
\[
T:\mathbb Z/p^m\mathbb Z\longrightarrow\mathbb Z/p^m\mathbb Z,
\qquad
[n]\longmapsto\nu_{p^m}[n]
\]
的核由条件 \(p^m\mid \widetilde\nu n\) 描述，等价于 \(p^{m-t}\mid n\)。故 \(T\) 恰好压缩模 \(p^{m-t}\) 的陪集。因为 \(t\ge1\)，元素 \([p^{m-t}]\) 在 \(\mathbb Z/p^m\mathbb Z\) 中非零，而
\[
T([p^{m-t}])=0=T([0]).
\]
取 \(a_0=0\)、\(a_1=p^{m-t}e_1\)，并用 \(\mathrm{Val}(e_1)=F_2=1\)，得到
\[
\rho_{p^m}(a_1)-\rho_{p^m}(a_0)=[p^{m-t}]_{p^m}\ne0,
\qquad
\eta_{p^m}(a_1)=\eta_{p^m}(a_0).
\]
这就是 rank-one 点谱中的可见压缩见证。

最后考虑细化 \(p^{m+s}\)。令
\[
w_s:=\min\{v_p(\widetilde U_s),m+s\}.
\]
细化标签在 \(p^m\)-值类上的作用为
\[
T_s:\mathbb Z/p^m\mathbb Z\longrightarrow\mathbb Z/p^{m+s}\mathbb Z,
\qquad
[n]\longmapsto U_s[n].
\]
若 \(w_s\le s\)，则 \(T_s([n])=0\) 蕴含 \(p^{m+s}\mid\widetilde U_sn\)，从而 \(p^{m+s-w_s}\mid n\)。由于 \(m+s-w_s\ge m\)，得 \([n]=0\) 于 \(\mathbb Z/p^m\mathbb Z\)，故 \(T_s\) 单射。写 \(U_s=p^{w_s}W_s\)，其中 \(W_s\) 在模 \(p^{m+s-w_s}\) 下为单位。对标签 \(y=T_s([n])\)，数值 \(y/p^{w_s}\) 在模 \(p^{m+s-w_s}\) 下良定义，且
\[
[W_s^{-1}(y/p^{w_s})]_{p^m}=[n]_{p^m}.
\]
因此 \((w_s,W_s^{-1})\) 给出恢复 \(\rho_{p^m}\) 的尺度证书。

反之，若 \(w_s>s\)，则 \(m+s-w_s<m\)。取 \(d=p^{m+s-w_s}\)；若 \(w_s=m+s\)，该式给出 \(d=1\)。于是 \([d]\ne0\) 于 \(\mathbb Z/p^m\mathbb Z\)，但
\[
U_s[d]=0=U_s[0]\quad\text{于 }\mathbb Z/p^{m+s}\mathbb Z.
\]
两个不同的 \(p^m\)-值类具有同一细化点标签，因此任何仅依赖该标签的函数都必须给出同一输出，不能同时恢复二者不同的标准剩余类。细化判据与尺度预算下界随之成立。
\end{proof}
