...
\]
where \(\lambda_q\) is Sanna's partition constant \cite{Sanna2025}.  Hence
\[
...
\]
Sanna's theorem therefore gives
\[
  T_q^\dagger(N)\asymp_q N^{(\log \lambda_q)/\log\varphi}.
\]
...
\[
  S_q(m)\asymp_q \lambda_q^m.
\]
If \(q\ge 2\), \Cref{thm:collision-kernel} gives
\[
  \lim_{m\to\infty}\frac{1}{m}\log S_q(m)=\log r_q.
\]
Comparing this with \(S_q(m)\asymp_q \lambda_q^m\) proves \(r_q=\lambda_q\).
\end{proof}
