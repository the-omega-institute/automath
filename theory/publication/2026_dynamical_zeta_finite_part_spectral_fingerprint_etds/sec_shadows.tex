...
\begin{theorem}[Exact boundary of cyclic detection]
  ...
  Let \(\mathscr C_{\mathrm{cyc}}(G)\) be the linear span of the
  pullbacks \(q^\ast f\), where \(q:G\twoheadrightarrow \ZZ/m\ZZ\)
  ranges over all cyclic quotients and \(f\) ranges over the class
  functions on \(\ZZ/m\ZZ\). Then
  \[
...
\begin{proof}
  ...
  \]
  and every \(\psi\circ q\) is a one-dimensional character of \(G\).
  Hence \(\mathscr C_{\mathrm{cyc}}(G)\subset V_{\mathrm{ab}}(G)\).

  Conversely, let \(\chi\in\Irr(G)\) be one-dimensional. Its image
  \(\chi(G)\subset\CC^\times\) is a finite subgroup of the
  multiplicative group of complex numbers, hence cyclic. Let
  ...
  \]
  so \(\chi\in\mathscr C_{\mathrm{cyc}}(G)\). Therefore
...
\begin{proof}
  ...
  \(A_\varepsilon\), the characteristic polynomial of \(A_\rho\), the
  spectral-radius identity \eqref{eq:s3-example-eta-check}, and the
  determinant formulas follow by direct calculation.
