...
> **TICK 47 补 — A2 四项修复齐、A7 投稿件补齐;八篇投稿包首次全项齐备**。
>
> **A2**(`95afe5be7`):唯一可能走弱路的一项走了强路。审稿人给乘性归一化留了两条路 —— 证出 $o(s^{-2r})$ 等价,或干脆声明 "proxy convention" 只指加性质量归一化。**选了证,并证成了**,现在定理同时覆盖加性与乘性,不是把问题定义掉。另三项:一对剖面 $((\chi^
u,\chi^\eta),H)$(两剖面间不要求任何关系);**原点锐性反例入正文**($\chi\equiv\kappa\in(0,1]$ 于零点邻域即破坏结论)——**把看似的局限变成已证的锐性**;第二条命题改题 *Asymptotic coefficient rigidity inside a prescribed ansatz*,引言明确否认对所有合理约定作分类,**同时订正了我先前"第 4 条到手"的夸大**。交替清构建 77+33 页、缺陷全 0、三 verifier PASS、14 测试、清单 13/13。
>
> **A7**(`c925515f9`):补上 cover letter 与 `submission_metadata.md` —— 八篇中唯一两样皆无的一篇,而它恰是**拒稿重投**,投稿信必须交代改了什么。FJMV 两问题按**稿件原文**核过,非转述:`sec_introduction.tex:72–76` 逐字写明两问及"以更强的素数形式"作答,FJMV 引用齐备。
>
> **至此八篇投稿包七项首次全部齐备。** 仅余 `submission_bundle/` 装配(现只有 A4、A5 有),这一步等投稿时间确定再做,以免过早固化快照。
