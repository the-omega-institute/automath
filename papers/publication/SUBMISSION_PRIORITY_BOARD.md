...
- t504 — codex 503，池空，内存 1.47 GB
- t505 — codex 503，池空，内存 1.62 GB
- t506 — codex 503，池空，内存 1.79 GB
- t507 — codex 503，池空，内存 1.48 GB
- t508 — codex 503，池空，内存 1.83 GB
- t509 — codex 503，池空，内存 1.55 GB
- t510 — codex 503，池空，内存 1.40 GB
- t511 — codex 503，池空，内存 1.38 GB
- t512 — codex 503，池空，内存 1.58 GB
- t513 — codex 503，池空，内存 1.36 GB
- t514 — codex 503，池空，内存 1.47 GB
- t515 — codex 503，池空，内存 1.80 GB
- t516 — codex 503，池空，内存 1.71 GB
- t517 — codex 503，池空，内存 1.72 GB
- t518 — codex 503，池空，内存 1.64 GB
- t519 — codex 503，池空，内存 1.60 GB
- t520 — codex 503，池空，内存 1.78 GB
- t521 — codex 503，池空，内存 1.71 GB
- t522 — codex 503，池空，内存 1.51 GB

## tick 523 — **codex 回来了**；已派出清单上的第一条阻塞项

内存 1.00 GB（偏低，故只派一个 agent）。Oracle 池空闲。

### 通道恢复

`codex exec` 连续三次返回 OK（OK / OK1 / OK2，确认非偶发）。
自 t476 起的四十六个连续无变化 tick 到此结束；折叠日志停在 t522。
中断从 08-18 04:53 起算，约 48 小时。

### 立刻派工：window6 的标题与摘要

按 `PRE_SUBMISSION_ACTIONS.md` 的顺序，这是第一条 —— 最强一篇、卡在 desk 阶段、
且改动不依赖任何未证结论。任务文件 `sprint/task_window6_abstract.txt`，
输出 `sprint/tier2/w6_abstract_out.txt`。

任务里写明的要点：正文已有一般 m 的定义与 {3,6,8,9} 分类（导入 Bugeaud-Cipu-Mignotte）、
闭式 `3*2^(m-2)`、以及 cover letter 早已改过 —— 只剩标题与摘要停在被拒版本。
要求以分类领起、把六维谱结果降为工作案例而非删除、并如实区分已证与未证。
一并处理 t440 查出的两处：补 "perfect colouring" 术语与相应文献、参考文献仅五条。

**明确禁止的**：动可复现节（已验证六个脚本全通过）、改任何定理陈述或证明、
出现任何修订痕迹措辞。要求跑 latexmk 确认可编译。

**并已预授权直接编辑** —— 依据既往教训：codex 会卡在"是否可以动手"的确认关上、
最后零改动退出。
...
## tick 527 — 内存一度 0.12 GB（复测为瞬时尖峰）；修掉 brocot 两个 cwd bug，中途把自己搞坏又修回

### 先处理内存

首测 **0.12 GB**，远低于 0.6 阈值。按第 5 条查：codex 进程 0 个，python 全是 uv 缓存的 MCP
server（0-10 MB），**无孤儿、无失控**；占用前十二名合计仅约 4.2 GB / 15.7 GB，
说明压力是分散的与缓存性的，不来自任何单一进程。

**没有可减的并发，因此正确动作是不派新 agent。** 复测得 **1.74 GB** ——
那是瞬时尖峰，不是持续状态。**只看一个采样就行动会得出错误结论**，这次没有。

### brocot 的两个工作目录 bug（t456 记录）

两者把输出路径默认写成 `artifacts/...`，因而只能从论文根目录跑、从 artifacts/ 里跑就失败。
已改为按脚本自身位置推导，现在**两个目录都 exit 0**，且仍写出各自的产物
（6925 与 23800 字节）。

### 中途我把它们弄坏了，检查抓住了

我第一版补丁把 `import os` 插在 `from __future__ import annotations` **之前** ——
后者必须是文件首条语句，于是两个脚本在**任何目录**下都抛 SyntaxError，
**比原来的 bug 更糟**。把 import 移到 `__future__` 之后即修复。
若不是坚持"改完两个方向都重跑"，这次会把一个可用脚本换成一个不可用脚本并提交。
...
## tick 529 — 内存与硬缺页**双双越线**，定位到单一元凶（我自己的脚本）并处置

### 这次不是尖峰

采样 0.33 / 0.36 / 0.35 / 0.53 GB，**持续低于 0.6**；硬缺页 **5227、4085 /秒**，
远超 1000/秒 的阈值。**两条阈值同时触发。**

### 元凶是一个进程，而且是我的

codex 本身只占 35 MB，不是它。占用前十名第一位是一个 **4360 MB 的 python**：

    verify_Zn_large_n.py     —— 我 t458 写的，已运行 6.1 分钟

**这正是我在 t459 因同样原因亲手 kill 掉的那个脚本。** 可复现任务要求 agent
"点名前先跑每个脚本"，它照做，于是跑到了这一个。

**杀掉这一个进程后**：内存 0.35 -> **5.25 GB**，硬缺页 5227 -> **924**（回到阈值以下），
**agent 存活并继续**。处置对象是肇事进程，不是 agent —— 这才是相称的。

### 根因是我把警告写错了地方

t459 我把"这脚本吃内存、n>24 就爆"写进了**文件末尾的注释块**，
而任何人（或 agent）在启动它之前都不会读到那里。

已改：默认值降为 `n=22, vmax=20000`，**裸跑 12 秒、几百 MB**，并在裸跑时打印
"正在用安全默认值，要更远请显式传参"；代价说明移进 `main()` 的 docstring —— 看得见的地方。

（修的过程中我又一次把文件改坏：print 字符串里的换行塌了、抛 SyntaxError。跑一遍就抓到，已修，
裸跑 exit 0。）
...
## tick 532 — 修正 tick 计数器的一处错乱；submission-gap agent 仍在跑

内存 2.26 GB。

### 计数器错乱，已更正

本 tick 首次读取时计数器给出 **531 —— 与上一个 tick 重号**。查明：
board 的 `## tick` 标题到 531 为止（各一条、无重复），磁盘上的计数器却退回了 530，
提交进 HEAD 的那份也是 530。也就是 t531 那次写入没有落到提交里，
于是本 tick 的自增又生成了一个 531。

已把计数器直接置为 **532** 并以此为本 tick 编号。**board 的编号序列本身没有断裂或重复**，
错的只是计数器文件与它脱节。写在这里以免日后对不上账。

### agent 未完成，故本 tick 不收割

四份 metadata 与三份 checklist 已出现在工作区，但 agent 仍在运行（`tokens used` 尚未出现）。
按纪律不在它退出前采信任何产物，因此**不提交**。

**已先查了那条硬约束：四份新 metadata 中"接受概率"类数字命中数均为 0** ——
它没有编造概率，这一条守住了。checklist 里标 PASS 的条目要等它跑完再逐条复核。

## tick 533 — 投稿材料补齐；一处"页数不符"查到底是引擎差异，我的更正被自己撤回

内存 2.21 GB。

### 补齐结果

metadata 补四篇、checklist 补三篇。**六篇现在各自都有 PDF、cover letter、metadata、
checklist、可复现声明。**

硬约束守住了：**四份新 metadata 无一编造接受概率**。checklist 逐条记证据，
且**如实标注未达成项** —— window6 那份直接写明本稿未声明 MSC 代码、未声明关键词。

### 一处页数不符，追到根因

window6 的 metadata 写 9 页，我清空重建数出 **10 页**，于是把它改成了 10。

**根因是编译引擎**：`latexmk -pdfxe`（XeLaTeX）出 **9 页**，`latexmk -pdf`（pdfLaTeX）出 **10 页**；
仓库里**没有 latexmkrc 钉住引擎**，源码也不含任何 XeLaTeX 专属包，两条路都能干净编译。

agent 本来是自洽的（9 页 + 明写 `-pdfxe`），**是我的改动让 metadata 与 checklist 互相打架** ——
比其中之一单独错更糟。已**撤回我的改动**（回到 9），并在两份文件里都写明所用引擎
与另一引擎的结果，使这个数字可复现而非含糊。

**教训**：核对一个数字之前先确认双方用的是同一条命令。我用的构建方式和它不同，
却把差异当成了它的错误。

## tick 534 — 冲刺组可执行清单清零；只剩一个需要人来定的选择

内存 0.80 GB，无 agent、池子空闲、无孤儿。

### 终检（材料完备性）

六篇**全部**具备 PDF、cover letter、metadata、checklist、可复现声明 —— 无一缺项。

### 我没有重跑一遍全部构建与脚本，这是有意的

六篇的 clean build 与被点名脚本分别在 t526、t530、t533 刚验过（30 个脚本 30 通过、0 失败），
期间未再改动 `.tex`。**再扫一遍只会显得努力，不会带来任何新信息** ——
本窗口我已经反复说过不靠这种方式填 tick，这里同样适用。

已把逐篇的"何时、以何种方式验过"写进 `PRE_SUBMISSION_ACTIONS.md` 的状态头，
便于日后追溯，而不是让人再去翻一百多条 board 记录。
...
## tick 536 — 派一次冷读复审：本窗口改动最大的三份摘要，此前只做过机械检查

内存 0.84 GB。codex 可用。Oracle worker 仍离线（Queued 1，非我的任务，不动）。

### 为什么派这个

清单已空，但 window6、projection、zeck_arith 的摘要都是最近一天重写的，
而我对它们的核验**全是机械性的**：能否编译、有没有动定理、若干事实断言是否与仓库相符。
**没有人以审稿人的眼光读过这三段文字。** codex 空着，这是它现在最该做的事。

### 任务定为"只读不改"

明确要求**不许编辑任何文件**，产出就是报告；并明说是让它**找问题**，不是确认没问题。
五个问项：

1. **量词与范围** —— 摘要是否在**真实假设**下陈述定理。本项目有过前科：
   摘要把结论写在部分条件下，导致那句话本身为假。要求**比对量词而非结论**，并引原文。
2. **越权主张** —— 有没有把正文归于他人的东西说成自己的、或把计算说成证明。
   点名两处必须核：window6 引入的 Bugeaud-Cipu-Mignotte、projection 引入的 Sanna，
   **必须在摘要里就署名，而不是只在正文**。
3. **主次** —— 最强结果是不是摘要第一件事。
4. **内部一致** —— 摘要里每一个数字（胞数、次数、重数、谱、界）是否与正文逐个吻合。
5. **cover letter（仅 zeck_arith）** —— 有没有对稿件或相关工作作出仓库支持不了的断言。

这一条是有针对性的：t526 我正是在那封信里抓到过"两篇互相引用"的不实陈述。

## tick 537 — 冷读复审抓到三处真缺陷，其中两处是我 t526 亲手核过并提交的

内存 2.12 GB。复审 agent 全程**未改任何文件**（工作区 0 处改动），符合只读要求。

### 我逐条复核了它的指控，两处硬伤属实

**一、摘要丢了假设（zeck_arith）。** 摘要写
"every exact most-significant-digit-first multiplier at effective resolution n has delay at
least n-1"，而定理原文（`07_..._06_def_stable_mul.tex:102`）写的是
**"then, for every $n\ge3$"**，且附带一整套模型条件（MSDF 延迟约定、L>=n+2 的补零同步输入、
c,d in X_n、对 X_n x X_n 每一对都精确计算）。**摘要把结论从 n>=3 扩成了全体 n。**
这正是我记忆里那条"摘要丢掉要紧的假设"的复发。

**二、摘要有数学错误（zeck_arith）。** 摘要写"composite 时给出 CRT 分解"。
正文自己的三分法（`07_..._03_...:349-350`）是：素数 -> 域；**素数幂 -> 局部环、按 CRT 不分裂**；
**至少两个不同素因子 -> 才分裂**。素数幂也是合数，所以**这句话按字面是假的**。

**三、cover letter 有一句无据的时序断言。** "ITA-2026-0032 was submitted before the present
work" —— **这句是我 t526 写的**。仓库能证明姊妹稿在 RAIRO ITA 在审、且未引用本文，
但**证明不了投稿先后**。我当时是推断而非核实。

### 意义

前两处出在**我 t526 核验并提交过**的稿子里。我那轮的检查是机械性的（能否编译、有没有动定理、
若干断言是否与仓库相符），**而这两处需要把摘要的每个量词与正文逐句对齐才能发现**。
...
### 三、我也自己把 window6 的摘要与正文对了一遍：干净

不等 agent，自己从判据推了一遍分类。`S=2^(m-i)+2^(m-j)` 要是 Fibonacci，由 BCM 只能是
k in {4,5,9,12}；`k-1>m` 连同数位位置约束给出候选 {3} u {6,7} u {8,9,10}；第二个条件
`k-2>m 或 对每个 admissible N 有 z_(k-2)(N)=0` 留下 3,6,8,9，杀掉 7 和 10。
胞数也自洽：`2^(m-1) + 2*2^(m-2) = 2^m`，m=6 给 32+16=48，与正文那个 worked case 一致。
结论在 `main.tex:112` 是**有编号的正式陈述**，不只是判据。

`verify_admissible_dimensions.py` 本身带对照：它对每个 m 到 22 都**找得到**候选 (i,j) 对
（左列非空），只是 9 以后 admissible 列为空 —— 所以这个"空"是真结果，不是搜索坏掉。
...
被测对象每次都是好的，坏的是我的检查。
### 另: 自查 brocot, 全篇最大胆的那个主张是站得住的

brocot 摘要平铺直叙地说"修正了 Dushistova 印出的首项系数", 这是本冲刺风险最高的一句话。查了三件事:

**一、修正有机制, 不是靠数值。** `sec_brocot_condensation.tex:177` 指名了确切的丢失位置 ——
Dushistova Lemma 7 里丢掉了 $u>1$ 这个限制; 第 152 行把两个系数并排框出来对比。

**二、常数 8 是解析证明的, 不是拟合的。** `sec_arithmetic_critical.tex:14-39`:
$b_C = 2(\zeta(\sigma_0-1)/\zeta(\sigma_0))^2$, 在临界点 $ho_{\sigma_0}=2$, 故 $b_C=8$,
原文用的词是 **which proves**。这与我 t473 数值到 n=1000 收到 8.2186(仍在下降)一致 ——
数值是旁证, 不是依据。

**三、涉及计算的地方措辞克制。** 有限层检查明写 "a consistency check, not an input to the
proof", 并承认收敛慢。

唯一残留风险不是缺陷, 而是可预防的误解: 审稿人若在 n 约 20-30 随手算 $Z_n$, 看到的是约 15.28
(我 t458-t473 测过, 峰在 n 约 27, 到 n=1000 才降到 8.2186, 修正项系数 215.4 很大),
可能据此认定论文错了。加一句带数字的注记能挡掉它。但**记忆里"审稿轮的疤痕组织"那条警告过:
堆积的防御性文字自己会变成首要反对意见**。故留作判断题, 不自动派工;
若冷读独立提出同一条, 那就是该加的信号。
...
## tick 543 — window6 的 MSC/keywords 已补齐核实; 我自己的任务书造出一处自相矛盾

内存 1.83 GB / 42 pages/s。Oracle 仍 1 排队 0 worker, 未动。

### 一、window6 缺口已闭合(独立核实)

选的码经得起对照正文: **05C50**(图与线性代数——正文有商图邻接谱)、**11B39**(Fibonacci)、
**11D61**(指数丢番图——即 F_k = 2^a + 2^b 那步), 三个都是真实存在的 MSC 2020 条目, 且各自
能在正文找到落点。keywords 七个, 含 **perfect colouring** —— 这正是 t440 认定该社群会去搜的词。

不采信 agent 自述, 从零重建**两个引擎都跑**:
`latexmk -pdfxe` exit 0 / 0 错误 / **9 页**, `latexmk -pdf` **10 页**。
两个数与 metadata、checklist 里**分引擎标注**的记录都对得上, 所以两处都不需要改 —— agent 说
页数未变, 属实。MSC 与 Keywords 两行确认渲染在 PDF 第 1 页。

### 二、我的任务书写死了"只准改那两行", 于是留下一处自相矛盾

checklist 第 16、17 行已改成 PASS, 但第 23 行的**总结句**仍写着
"two metadata items are not met: the manuscript does not declare MSC codes or keywords"。
agent 严格照我的约束没动它 —— **这处矛盾是我的指令造成的**, 不是它的错。
恰恰就是 t542 审计在 scan_projection 抓到的那种"文件内部自相矛盾"。已改。

改的时候顺手守住一条: 新总结句写"十项通过 + 一项无数值上限", 我**先数了再写** ——
实际 `[x]` 11 项、`[ ]` 0 项, 10+1=11 对得上; 并且回查了那"一项无上限"确有其事
(第 12 行: EJC 要求摘要但不设字数上限)。**不先数就写数字, 正是 t542 那个"约 35 页"的错法。**
...
## tick 545 — 把变异测试用到论文的 verifier 上: 两篇的招牌结论确实被自己的脚本约束住

内存最低到 1.02 GB(本会话最低), 随后回到 1.62 GB; 单 agent 在跑, 并发已是最小, 未再降。
Oracle 仍 1 排队 0 worker。`missing_checklists` 仍在跑(40 万字节), 尚未动任何论文文件。

等待期间做了一件一直没做的事: **不只问"脚本跑不跑通", 而是问"脚本到底约束住了什么"**。
办法是改论文自己的常数, 看脚本会不会翻脸 —— 这正是记忆里那条"验证器的容忍度就是主张本身"。

### cubical_stokes: 招牌公式经得起变异

招牌是箱体极值 $m(R)=(2\sum_j L_j^{-1})^{-1}$。脚本不是把公式代一遍就完事 ——
它用 `scipy.optimize.linprog` 从**箱体几何**独立解离散 LP, 再与闭式比。
三个箱体 x 四种网格, LP 最优值与闭式**逐位吻合到 9 位**。

把闭式里的 `2.0` 改成 `2.1`(5% 的改动), 立刻 `-> FAIL`。
**所以那个常数 2 是被独立证据顶住的, 不是自说自话。**

### scan_projection: 常数是推出来的, 灵敏到千分之一

招牌是"相位无关的推广是假的": 偶类均值 `953/5618 = 0.169633`, 奇类 `267/(676 sqrt5) = 0.176636`,
两者不等。脚本把渐近量算到 m=90, 与闭式误差 **4.000e-59**(算术下限)。

把 `953` 改成 `954`(千分之一), `two constants` 立刻变 False, m=89 处误差从 4e-59 跳到 **3.560e-4**。
**常数是独立推导出来的, 不是打上去的。**

**但要说清一个分寸**: 同一次变异下 `means differ` 仍然 True —— 因为那一项只比较两个数值是否相等,
954/5618 和 267/(676 sqrt5) 当然也不等。**这个子检查本身是弱的**, 它成立只是因为
...
### 变异探针第一版的结论作废 —— 是我的工具, 不是论文

第一版报出 23 个脚本里 17 个 `UNCONSTRAINED`。**在把这个数字当成对论文的指控之前先查自己**,
两处都塌了:

**一、它扰动的根本不是被主张的常数。** 我把筛选条件写成"三位有效数字以上",
于是 window6 里被改的是 `while len(F)<200`、`range(2,120)`、`r=100` —— **数组容量和循环上界**。
而 window6 的结论本身就是**小整数**: 可容许维数集合 `{3,6,8,9}`、胞数 `21` 与 `48`。
**我恰好把该测的数全筛掉了。** 改这些容量参数当然不会翻转结论, 于是满屏 UNCONSTRAINED。

**二、`ALREADY-FAILING` 是我关键词匹配的锅。** 判定里写了 `False` 就算失败,
而 `verify_refinement_family.py` 每行本来就打印一个 `False` 列(表示该行不是等分的),
于是基线被读成"本来就在失败"。这几个脚本 t538 明明全过。

两处都已修: 字面量放开到任意数值、每脚本最多试 8 处; 判定只认 `-> FAIL` 这类**显式结论**,
不再认裸 `False`。window6 正在重跑。

**这正是本 tick 该记的东西**: 一个新工具第一次给出耸人听闻的结果时,
**最可能的解释是工具错了** —— 17 个验证脚本集体失效, 远不如"我筛错了数字"来得可信。

## tick 548 — 变异探针路线放弃: 它问不出我想问的问题

内存 2.39 GB。工作区已复原为 0 改动。

### 放弃的理由(第二版修完仍然不成立)

第一版筛"三位有效数字以上", 改的全是数组容量与循环上界, 满屏假 UNCONSTRAINED(t547 已记)。
第二版放开到任意数值字面量, 于是它干出了这个:

    -    FIB.append(FIB[-1] + FIB[-2])
    +    FIB.append(FIB[-2] + FIB[-2])

**它把数组下标改了, 直接破坏了 Fibonacci 递推本身。** 脚本当然会失败 ——
但那证明的是"我把它的内部算坏了会报错", 这对**任何**程序都成立,
与"它是否约束住论文声称的常数"毫无关系。

两版失败的方向相反, 但暴露的是同一件事: **正则分不清"论文主张的那个数"和"程序自己的实现细节"。**
t545 手工做的两次之所以有效, 恰恰因为我**先读懂了脚本在主张什么**, 再去改那一个数
(箱体公式里的 2.0、周期二均值里的 953)。这一步没法用模式匹配代替。

探针已停, 被改的 `verify_hidden_refinement.py` 已 `git checkout` 单文件复原,
复跑确认递推完好、`all assertions passed`。

### 改用能读出来的问题: 点名的脚本是"断言"还是"打印"

window6 可复现小节点名 6 个脚本。其中 2 个带断言(`verify_hidden_refinement.py`、
`supplement/verify_window6_streams.py`), 另外 4 个只计算并打印
(`verify_refinement_family.py`、`verify_involution_mechanism.py`、
`verify_admissible_dimensions.py`、`verify_preservation_criterion.py`)。

**判定: 不是缺陷, 不改。** 正文对这四个的描述本来就是"枚举""streams", **没有**声称它们自检;
它们各自独立重算一遍, 由读者把输出与论文比对 —— 这正是可复现该有的样子。
记在这里是为了让后来者知道**这四个跑通不等于自动确认了 {3,6,8,9}**, 需要人对一眼。

### 顺带: `agents_running.sh` 会把我自己的探针报成孤儿

它排除了 MCP server, 但没排除 sprint 自己的工具。若照第 (5) 步机械清理, 会杀掉正在跑的探针。
本 tick 是手工判断才没误杀。

## tick 549 — 论文点名的一个脚本, 单看输出像是在推翻论文自己的招牌常数

内存 2.36 GB。工作区除本条改动外 0 变更。

### 发现

手工探 brocot 的 $b_C=8$ 时, 读到 `verify_critical_tail_constant.py` 的最后一行输出:

    measured level is roughly 13.9 and still rising -> ratio to 8 is 1.733

而这个脚本**是被论文点名的**(`sec_conclusion.tex:49`), 它的 docstring 还写着
"Test that constant directly"。**审稿人照可复现小节跑一遍, 看到的是 1.733 倍的偏离,
而脚本里没有一个字解释这是预期之内的。**

论文本身措辞是干净的 —— 写的是 "**estimates** the critical tail constant from finite layers",
`verify_resolvent_recurrence.py` 也同在名单里。问题不在论文说错了什么,
而在**两个脚本在长名单里并列, 没有任何东西把它们连起来**。

### 处理: 让这个检查自己说清楚它看不见什么

我当场用 resolvent 脚本算了一遍(不是引用旧结论):

    n = 27   15.2760      n = 51   13.2728      n = 100   10.5844

n=100 与 transcript 吻合到 **3.4e-12**, 且对精确整数表的对照 PASS。加上 t472 已复现的
n=1000 = 8.2187, 这就是一条**从上方极慢下降**的序列 —— 浅处 13.9 正是它该有的样子。

已在脚本里写明: 这不构成对 $b_C=8$ 的反驳、浅处未进入渐近区、并列出上述数值、
并指明 **$b_C=8$ 根本不是数值确立的, 而是 `sec_arithmetic_critical.tex` 里从
$\zeta(\sigma_0-1)/\zeta(\sigma_0)=2$ 解析证明的**, 本脚本只是有限层估计。

**分寸**: 我明确写了两个量"相关但不相同" —— d=25 处本脚本给 13.86, resolvent 在
n=2d+1=51 给 13.27。**我没有推导过它们的对应关系, 所以只作为一致性旁证陈述, 不声称同一序列。**

这条正是本会话反复的那句话的正面用法: **检查看不见某样东西, 是检查的局限** ——
所以要让检查自己把局限说出来, 而不是改论文去迎合它。
### 顺带把这个形状扫了一遍: 还有哪些被点名的脚本会打印像是反对论文的话

扫六篇**被论文点名**的脚本源码, 找 "does not / cannot / discrepancy / disagree / ratio to"
这类措辞。命中十来处, 逐个看下来**只有 tail constant 那一处是真问题**(已修), 其余都是良性的:

- 多数是 docstring 在**主动说明本检查的局限** —— 这正是该有的样子(我这次加的那段也会被扫到)。
- `verify_correction_form.py` 值得单独说: 它输出 "every single-term fit exceeds the peak: True
  ... the FORM is what fails", 单看像负面结论。**查过, 是一致的** —— 它排除的是**单项**修正形式,
  而论文用的正是两项反号展开($215.38/n - 44.58 n^{-1.47875}$); 脚本自己也写着
  "至少需要两个反号项, n=27 附近的转折应当由它们给出"。论文对它的描述
...
差点又一次用窄模式下了大结论。
### 收割: 两份 REPRODUCE.md 已写好, 我逐条照着跑过一遍才收

不采信自述。scan_projection 那份文档里每个具体数字我都对了:

    m=89 误差 3.600e-59 / m=90 误差 4.000e-59        文档写什么就是什么
    pi = (21/89, 16/89, 16/89, 36/89), rho_2 = sqrt5/12, c_20 = 953/2809
    默认 exit 0; --negative-control exit 1; unittest 2 tests OK

cubical_stokes 四条命令同样逐条跑过, 含 `--negative-control` 确实 exit 1。

**新负对照是对的那一种。** 它只把**声称的**偶类均值 953/5618 换成 954/5618,
不动转移矩阵、不动递推、不动渐近常数 —— 证据是: 变异后前三个 CONTROL 与渐近检查**仍然 PASS**,
只有"claim 与推导常数是否一致"这一项翻成 False。**改的是主张, 不是机器。**

**而且它顺手补上了我 t545 记下的那个弱点。** 当时我写过:
"`means differ` 这一项本身是弱的, 它只比较两个数值是否相等"。原来的判据是

    ok = m0 != m1

现在是

    ok = m0 != m1 and constants_match          # 两个均值都须与独立推导的 c_20/2、c_21/2 在 1e-40 内吻合
...
## tick 555 — 派出正文级冷读; 我自己先探了两处最容易出事的地方, 都干净

内存 2.27 GB。工作区 0 改动。Oracle 中继仍下线。

### 派工: 只读, 只读正文证明

前两轮冷读只覆盖摘要与 cover letter。**正文证明从没有人以审稿人的眼光读过** ——
迄今所有核验要么是机械的(能否编译、脚本能否跑、记录与源是否一致), 要么是关于摘要的,
这与"证明是否成立"是两回事。

任务书把要猎的形状写在最前面: **一步的界被当成多步用** —— 记忆里这个项目反复产出的假证明形状。
并列了具体的迹象: 归纳里引入的常数偷偷依赖归纳变量; 单层误差直接对层求和而不论证可和;
"同理可得"而其实第一次已经把假设用掉了; 极限与求和交换却没有控制函数;
以及**前面的估计根本没建立起来的"一致性"断言**。

同时明确列出**已定案、不得重开**的四条(b_C=8 的解析证明、Sanna 行的机器辅助证明、
被刻意保留为条件性的识别、以及编译与脚本状态), 免得又收到一份重复劳动的报告。
并写明: 上两轮冷读四条具体指控里**有两条经不起复核**, 所以宁可短而实。

### 我自己先探了两处(仅两处, 不是全文)

**一、一致性断言。** 全文只有三处 `uniformly in`, 且**没有一处 "the same argument" 或 "similarly"**
—— 后两者的缺席本身就是好迹象。三处都读了, **都是论证过的, 不是断言的**:

- 第一处给出显式控制 `<= min{2, |theta||S_r - r mu_C|/m}`, 并写明"bounded by 2",
  由依概率收敛过渡到期望收敛 —— **正是那个"缺了就出错"的控制函数步骤**;
- 第二处用子列反证法证明紧集上的一致性;
- 第三处由前两处推出。

原文还自陈方法("To see this without a moment estimate"), 说明作者清楚这里需要什么。

**二、外部定理的假设是否真被核过。** 以 Omey-Van Gulck 为例, 三件通常被省略的事它都做了:
**精确定位**(第 3.2 节式 (8) 之后的第一个断言, 而非泛泛引用);
**显式记号对照**("Their u_n, mu, F_n are respectively v_j, mu, F(j) here" —— 误译最常藏在这一步);
以及**单独一条命题**"Verification for the critical letter law"逐条核对假设。
...
## tick 557 — 自己判冷读的 Finding 2 与 3(它自陈没查外部文献); 修复仍在飞

内存 1.19 GB。修复 agent 仍在跑, 已改 `sec_ledger.tex` 与 `sec_moment_kernel.tex`,
**`lean4/` 一个文件都没动**(任务书明令), 符合要求。未中途收割。

### Finding 2(projection 的转录机命题): **部分成立, 但它把范围说大了**

命题断言存在一个确定性 subsequential 转录机, 其证明只有三句话, 归结为
"standard in the automata-theoretic literature cited above"(引的是 Frougny 1991/1992 与
Ahlbach 等 2013), **没有点名任何一条定理**。这一条我同意: 存在性是被挥手带过的,
而后文确实要用它。

**但冷读列的四条"未经核实的性质"里, 有一条不成立**: 它把
"has a uniform terminal-output bound $L$" 也算作待验证的性质。
查原文第 49 行, $L$ 是**定义出来的**: $L:=\max_{s\in Q}|\tau(s)|$ ——
subsequential 转录机按定义状态集有限, 故 $L$ 自动有限, **根本不是一条需要另行验证的性质**。

所以 Finding 2 缩水成一条: **存在性命题应当点名定理或给出显式构造**。这是审稿人合理会提的,
但属于"论证不足", 不是错误 —— 与冷读自己的定级一致。

### Finding 3(brocot 的一致极点提取): **成立但轻微**

原文以"aperiodicity and analyticity"一句带过主极点的孤立性, 随后代入**随 $j$ 变动的复参数**
$y=e^{i\lambda/\sqrt j}$。冷读说这需要沿动参数的一致根分离与围道余项一致界。

我的判断: **非周期性正是给出 $|P(z)|<P(|z|)$(离开正轴)这条标准严格不等式的假设**,
根分离由此而来, 是解析组合学里的常规步骤, 只是写得极简。
**属于可补的一句话, 不威胁正确性** —— 冷读自己也是这么定级的, 我同意。

### 三条合起来看

冷读这一轮的质量明显高于前两轮: 它自陈了"哪些没查", 主动报告了自己一条疑虑
(尾递推/Hankel)**经复核不成立**, 并且**独立复现了我 t555 那两处自查的结论**
(Omey-Van Gulck 的假设在 124-160 行逐条核过、Gibbs 一致性由子列论证建立)。
真正重的那条(13 个空壳 `\leanverified`)不是它报的, 是**顺着它的 Finding 1 查下去挖到的**。

## tick 558 — 修复已核实; 同时**我必须撤回 t556 那条 BLOCKING 的定性**

内存 1.5 GB(51977 pages/sec 又是尖峰, 连采三次均为 0)。

### 先说我错的地方

t556 我把 13 个空壳 `\leanverified` 定为 **BLOCKING**, 理由是"论文主动向读者声称了一种
并不存在的验证, 审稿人看到这个宏会相信该陈述已被机器检验"。

**这个理由不成立。** `main.tex:49`:

    \newcommand{\leanverified}[1]{}

**这个宏展开为空, 在 PDF 里不产生任何输出。** 我核了成品 PDF: 全文 "lean" 出现 **0 次**。
也就是说这些标记**只存在于 LaTeX 源码里, 任何读者、任何审稿人都看不到**。
**没有人被告知过任何验证。** 定为 BLOCKING 是我过度升级了。

我当时应当在下定性之前先查这个宏 —— 我核了 13 个 Lean 文件的内容, 却**没核这个标记是否会被读者看到**,
而后者恰恰是"是否构成对读者的虚假声称"的全部依据。

### 仍然成立的部分

- **13 个 Lean 文件确实是命题逻辑恒真式**, 把数学当抽象 `Prop` 收进来。这是关于
  `lean4/` 现状的**事实**, 对项目内部"形式化覆盖率"一类说法有意义, 只是**不构成投稿阻塞**。
  摘掉这 13 个源码标记仍是对的: 源码不该记录一件不真的事。
- **Defect 2 是真的数学错误, 而且读者看得见** —— 三处把单边界写成了等式。这条才是本轮真正的收获。

### 修复核实(五项全过)

    标记        剩 8 个, 且恰是我独立判定为真的那 8 个; 13 个已摘
    三处等式     全部改为 \ge
    下游链条     补了一句 "Since the left-hand side is at most S_q(m)",
                由下界加平凡上界夹出 = exp((p_q+o(1))m) —— **补法正确**
    lean4/      0 个文件被改动
...
## tick 560 — 引文完整性闸门: 六篇引用的 **72 个 DOI 全部注册在案**

内存最低到 909 MB(本会话最低), 单 agent 已是最小并发, 未再降。修复 agent 仍在跑, 未中途收割。

### 结果

    [CONTROL-good]  10.1016/j.jnt.2021.02.010   registered   6/6
    [CONTROL-fake]  10.9999/definitely.not...   NOT-FOUND    6/6
    六篇实引 DOI                                registered   72/72

记忆里有一条: **曾有一个伪造引文熬过了好几轮审稿**。这一轮把这道闸门补上了。

### 顺带查清一件事: Sanna 的 DOI 解析不了, 但**不是论文的问题**

`10.19086/da.137601` 在 doi.org 与 Crossref 都返回 404。它出现在
Sanna 自己那篇 arXiv PDF 里、我的 board 与若干任务文件里, 以及
`verify_sanna_table_continuation.py` 的注释里 —— 而
**projection 的 bib 条目 `Sanna2025` 根本没有 DOI 字段**, 只写 journal/volume/number/pages。
所以六篇没有一篇声称过这个 DOI, **稿件不受影响**。

### 这一条的过程本身值得记: 三次跑错, 全是我的量具

1. 第一次带 `-L`, curl 跟着跳转去出版商站点、在本沙箱失败, **72 个全报 000**。
2. 去掉 `-L` 后仍全是 000, 我判断为 doi.org 限流 —— **判断错了**。
3. 加上**内嵌对照**(每 12 个插一个已知真 DOI 与一个编造 DOI)才定位到真因:
   已知真 DOI 作为**字面量**传入时 `registered`, 从**文件读入**时 `NO-RESPONSE`。
   同一个值、两种来源、两种结果 —— 只可能是文件里带了 `\r`(CRLF), URL 尾部多一个回车。
   与 t540 那次 heredoc 的 CR 是同一个根因。

**要点**: "全部 72 个都解析不了"这种结论本身就极不可信, 而**内嵌对照是唯一能把
'量具坏了'与'被测对象坏了'分开的东西** —— 前两次没有对照, 我就只能瞎猜(还猜错了一次)。

## tick 561 — 两条"论证不足"已补, 独立核实通过; 六篇内容层至此全部过了审稿人视角

内存 1.26 GB。

### projection 的转录机: 给的是**点名定理 + 显式合成**, 不是又一次挥手

原来三句话的"standard in the literature"已换成:
点名 Frougny 的 Fibonacci 归一化定理并给出**页码定位**(pp. 397--398)、
把定理的有限词形式**完整写出**、核对**权重指标与本文约定一致**、
指出多出来的 \(b_0\) 正是 \(\widetilde\Lambda\) 的前导零填充位, 最后用一个
**两状态 subsequential 转录机**(初态删首符、次态照抄)与之复合, 由复合封闭性得到所需机器。

我能离线核的那一项已核: bib 条目写的是 pp. 393--399, **定位 397--398 落在范围内**。
**核不了的那一项也说清楚**: 我没有原文, 无法确认该定理确实在这两页上。

### brocot 的极点分离: 补的正是缺的那一步

新文字给出紧参数弧 \(K_\delta\)、指出 \(e^{i\lambda/\sqrt j}\in K_\delta\)(\(j\) 充分大)、
写出非周期性给的严格不等式 \(|P(z)|<P(|z|)\)、由此得到**一致根分离** \(\epsilon\)、
围道上的**一致下界** \(\eta\), 以及外围道的 \(O(R^{-j})\) 一致估计。
这正是 t557 我判定"标准但写得极简"的那一步, 现在写全了。

### 页数变了, 两处记录都同步了

    brocot      31 -> 32     metadata 32 / checklist 32 / 实际构建 32
    projection  49 -> 50     metadata 50 / checklist 50 / 实际构建 50
...
## tick 564 — 清单重生成核实通过; 14 条未引用文献判定为**删**, 已派工

内存 1.08-1.22 GB(84423 pages/sec 又是尖峰, 连采三次 16/64/0)。

### 重生成的清单: 四处缺陷全部消失

    指着 ETDS            0 处(原 1 处), 改为 Stochastics and Dynamics
    虚假的作者 blocker    0 处
    引用不存在的证据文件   0 处
    参考文献那条          改为 REVIEW, 并写明 main.tex 内嵌 26 条、无 \bibliography 命令、12 个键被引

新的总结段是对的形状: **"9 项实测通过; 3 项需外部期刊规则或编辑判断"** ——
把**量得出来的**与**要查期刊规定的**分开列, 这正是我要的。

### 那 14 条: 证据指向删除, 不是补引

agent 报告: **14 条无一在正文中以作者姓名被讨论**。我再查了正文主题分布与已引的 12 个键:

    已引的 12 个键覆盖了相应主题, 而且**存在近重复**:
        ColletMartinezSchmitt1997 已引, 而未引的是 ...1996
        Walters1975GMeasures      已引, 而未引的是 WaltersErgodicTheory
        Seneta2006                已引, 而未引的是 DarrochSeneta1965

这三组近重复说明这 14 条是**起草期扫的一遍相关工作、后来没清理**, 而不是有意留的延伸阅读。
`thebibliography` 里未引条目**照样印出来**, 于是参考表里现在有 14 条论文从未提及的工作。
**判定: 删。** 已派工, 并要求它**先自行复核每一条确实未被引用**(我的计数可能错), 且
**编译日志必须无 undefined citation** —— 若出现, 就说明我让它删了在用的东西。

### 又一次我自己的量具错, 靠"结果不可信"抓回来

查正文主题时我先把所有 tex 拼起来、再**从第一个 `\begin{thebibliography}` 处截断**。
...
## tick 568 — \(n\ge3\) 已补齐并扫过全文; 再把同样的扫法用到 projection, 那边是干净的

内存 1.12 GB。

### zeck_arith: 扫出的比我指出的多一处

`main.tex:82` 已改成与摘要一致(\(n\ge3\)、补零同步输入、对 \(X_n\times X_n\) 每一对)。
**扫全文又抓到第 73 行**: 它排除"分辨率无关的有界延迟乘法"时**没有指明输入与扫描模型**, 现已补上。
两个源文件加 main.tex 共 **27 处匹配逐条判定**, 改 2 处, 其余给出了"为何已经正确"的理由。

我**抽查了两条它判为"已正确"的**, 都成立: 第 265 行只是罗列乘法的几种等价描述、不作延迟断言;
第 278 行说的是"在同一 MSD-first 约定下延迟至少线性增长"这一渐近结论, 而非对每个 \(n\) 断言
\(\delta_n\ge n-1\)。

重建 exit 0、**34 页不变**, 成品 PDF 里"effective resolution n ≥ 3 on padded synchronous inputs"
在**摘要与贡献点清单两处都出现**; 记录核对六篇 0 异常。

### 把同样的扫法用到 projection: 干净

t558 把三处 \(p_m(V_m(\varepsilon))=1-\exp(\cdot)\) 改成了 \(\ge\)。这次复查:

    sec_ledger.tex:35, sec_moment_kernel.tex:396, :429 —— **三处前面都是 `\ge`**, 改全了

并且按 t556 冷读的提醒, 另找了**散文层面的转述**("sharp exponential tail""exponentially small"
"all but a ...""excluded mass"等): **一处都没有**。
所以 projection 那次修正是完整的, 与 zeck_arith 不同。

### 其余四篇为什么不用做同样的扫

它们本冲刺的改动**本身就不具备"多处转述"的形态**: brocot 是封面信披露(单文件)与证明内部补一步;
window6 是 MSC/keywords 与标题摘要重写; cubical_stokes 本冲刺没有过内容更正。
**不做形式主义的补扫**, 但这个判断写在这里, 以便被推翻。
