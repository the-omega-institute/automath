...
> **TICK 62 — 四篇回验全部送出**。池子 2/6(`6e48ff00` A6、`0878ed5b` A8 在飞);codex 写 A4、A9 的回验问题;内存 1.88 GB、无孤儿、无可收割。
>
> 四份问题都按"**宁可现在被驳回**"的口径写:
>
> - **A6**:更正若错则撤回,并请他核我们把错误限定在首项系数**是否公允——有没有低估,也有没有夸大**。
> - **A8**:**现在签不签 Theorem G?** 不签就点名还差什么,并说明是否该进一步削弱而非辩护。
> - **A4**:我们自查后认定重叠**比他说的更重**(机制可由其 Remark 12 导出),问题里直接问:**我们是不是把重叠说过头了?还是仍然说轻了?存活贡献有没有被高估?**
> - **A9**:方案 A 是否按他本意实现、方案 B 是否隔离到位、以及他所说"档位相关的结果是完整局部化"是否在射程内。
>
> 这一轮的模式已经稳定:**做完必送审,送审必把最不利的问法写在前面**。本会话靠它逼出了四件本会溜走的事 —— A5 的 parity 本质性主张、A6 与 A4 两次新产出上的优先权遗漏、A8 那条为假的结论。
