...
> **TICK 15 — 引入 referee 式外审，投前先挨打**。A2（强 TIER-3、临界 TIER-2）与 A5（上 TIER-3）已有可评审的成品定理，故本轮改用**第三种问法**：不再问"该做什么"，而是把定理连同全部假设、**明确写出未证部分**、引用归属与验证证据交给专家，请他**以该档期刊审稿人身份**回答四问：定理是否正确（指出无法跟随或认为有误的步骤）、是否新颖（点名最近在先结果并说明增量）、增量对该档刊是否足够（不足则需补什么）、修订会要求什么。并明说：**宁可现在听到带理由的硬拒，也不要软性通过**；任何反对意见须同时给出"怎样才能解决"。
>
> 用意是把 referee 的反对意见提前到投稿之前拿到——这比再加一条定理更能决定 A2 能否守住 JFA。四篇新式提问（A3/A4/A6/A7）仍在 Oracle 侧；A8"恢复已撤回主张"与 A9 crossed-module 两项检验在跑。
