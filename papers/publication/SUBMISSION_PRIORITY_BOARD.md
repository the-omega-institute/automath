...
> **TICK 35 — 对照"保证 tier-2、冲刺 tier-1"结算一次**。池子空闲;仅校验和推广 agent 在跑;内存 1.08 GB,无孤儿;无可收割。
>
> **八份外审裁决摆齐后,结论必须直说:目标没达到,而且不是差一点。**
>
> | 篇 | 裁决 | 现去向 | 档 |
> |---|---|---|---|
> | A2 | **拒稿** | Potential Analysis | 3 |
> | A3 | Major Revision | DCDS-A | 2–3 |
> | A4 | 材料不全则拒稿重投 | Monatshefte | 3 |
> | A5 | **拒稿后重投** | ETDS | 2 |
> | A6 | **拒稿** | JNT | 3 |
> | A7 | **拒稿重投** | Fibonacci Quarterly | 4 |
> | A8 | **拒稿** | MCAP | 3 |
> | A9 | **拒稿** | Cahiers | 3–4 |
>
> **八篇里七篇被拒,只有 A3 是 Major Revision。tier-1 一篇没有,且没有任何一位审稿人指出通往 tier-1 的路径。** 勉强够 tier-2 的只有 A3 与 A5 两篇 —— "保证 tier-2"在八篇里兑现了两篇。
>
> 有两点必须同时讲清,否则这张表会被误读:
>
> **其一,所有裁决都是对修复前的版本作出的。** 修复后的版本无一被重新评审。而这些修复不是润色:A9 把核心证明搬回正文(48+50→34+6)、A8 砍掉 118 页(186→68)、A3 补全三处承重证明且页数反减、A6 删降三条定理、A2 把 $r=1$ 纳入定理。这些版本与被拒的版本已不是同一稿。
>
> **其二,审稿人的档位判断大多是结构性的,修复改不动。** A6 的原话是"**即使证明完全修复、优先权完全改写,我预计 TAMS 的最终裁决仍会是拒稿**";A9 的是 TAC "现在 <5%,照我推荐的修订做完仍 <10%,需要一条真正新的范畴论定理,压缩解决不了";A2 的是存活增量属"**强的专门期刊结果**"。这些是**前瞻性判断,已经把修复计入**。
>
> **可执行的结论**:继续在同一批机器上追加定理不会改变档位——这正是六篇被查出优先权遗漏所揭示的,我们的很多"新结果"是标准结论的特化包装。想上 tier-2 以上,需要的是换问题,不是改稿。
