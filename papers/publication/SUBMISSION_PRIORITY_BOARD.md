...
| **A5** `finite_parts` | 43+19 | ✅ 一般 $p$ Mahler 定理;可归约性已承认 | ETDS(重估在飞)|
...
> **TICK 81 — A3 有界零定理复核入库 `bc87de020`;余下四篇全部送交档位重估。八篇的第三代闭环至此走完。**
>
> **A3 的证伪测试是在写定理之前跑的,而且通过了** —— 无例外族,含非标准初值。在非 Condition-F 的 $x^3-3x^2+2x-1$ 根上,$D=3$、$m=9,10$ 分别精确枚举 5,764,801 与 40,353,607 个中间词,零违例。定理 2.6 的顶点集是 $([-D,D]\cap\mathbb Z)^{m-1}$,边条件是纯整数方程 $\sum e_ju_j+cu_m=0$;**我逐字读过陈述**,规范秩双射、合法词字母表、循环约化、正负实现、像移位/逆解码器五项确实全部消失 —— 这是当初立下的验收标准,不采信机械扫描。数字结构只残留在更早的碰撞商里,引理 2.5 与定理 2.6 均不使用。
>
> **四份重估已投**(问题由我写,因为每篇我都亲自复核过,呈的是核实过的事实而非论文内容):
>
> | 篇 | task id | 问的关键问题 |
> |---|---|---|
> | A6 | `7bc499c7-bbbe-498b-a1a5-990d85816fc7` | 全变差升级是否真完整(联合位置极限与分母比极限超出他说的已有部分);连分子权全局耦合的论证能否顶住熟悉条件重尾文献的审稿人 |
> | A8 | `7ba06ae2-01e4-492f-8097-f4f5fea30626` | 耦合论证是否真杀死边界周期条件化,还是把条件化藏到了别处;70 页加一条定理是帮忙还是拖累;宽等价 + 窄奇异应用是否会被判为两篇论文 |
> | A3 | `7d5ba223-ef7c-4716-b500-034afee4ff54` | **有界零表示的孔径线性瞬态界是否真不在经典文献里** —— 这是我们最无力自答的一问 |
> | A5 | `adee3043-9994-438b-90a0-0dfc29becc0b` | 判定性既已归约到已发表算法,**残余主张是否还够格当定理**,还是该降为注记;我们的归约本身是否正确;Prop 3.11 常数以我们的更正为准还是他的 |
>
> 四份都写死了同一条:**不要因为我们做了工作就放软评价;标准若未变,直说。** A5 那份更进一步:"若诚实的答案是这项增补已不承载档位,我们宁可据此行动,也不愿投出去再从审稿人那里学到。"
