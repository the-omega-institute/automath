...
> **TICK 88 — A3 两处修复入库 `4a21598e8`;落后上游的那一个提交已合并,分支不再 behind。**
>
> **反例经三方独立计算一致**:外审给出、codex 重算、我自己手算。$U=(1,2,4,\dots)$、$D=2$、$m=2$ 时边条件为 $e_0+2e_1\equiv0\ (\mathrm{mod}\ 4)$,得七条边($-2\to\{-1,1\}$、$0\to\{-2,0,2\}$、$2\to\{-1,1\}$,$\pm1$ 终止),非零顶点出发无可达圈而 $0\to0\to\cdots\to0\to2\to1$ 可任意长。锚定后的表述已入稿,七边图加入回归用例,并**先见其失败(枚举器缺失时)再见其通过**。第二处:完成系数只保证 $|c|\le K_{U,D}$,边定义在混合字母表上,已写在边条件处。
>
> **该篇文献表也不干净**:删掉一个**不存在的 DOI** `10.4134/BKMS.b170831`(文章本身为真,故只去伪 DOI 保留条目),并更正一处指向无关论文的 Frougny DOI;65 条全部按标题与第一作者确认。
>
> **git 状态查清并处理**:落后的那一个提交是 `16efc6374`(你 2026-07-30 所作),只动 `submitted_..._berstel_adder_rairo_ita` 一篇,与本冲刺八篇**零重叠**,故普通合并安全,已合并(`db5ed8a49`),现为 **ahead 503, behind 0**。`--rebase` 被脏树拒绝 —— 阻塞的正是 `tools/chatgpt-oracle/` 下 **08-03 的 15 个未提交改动**(非本冲刺产物)。**我没有把它们以自己名义提交,也没有 stash**,合并后它们原样保留。
>
> **503 个已验证提交仍未推送** —— 推送是对外动作,等你的指示。
>
> A9 文献审计在跑,是八篇中最后一篇未查的。
