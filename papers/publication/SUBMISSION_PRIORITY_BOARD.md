...
> **TICK 49 — 七篇升档质询在投;补掉我自己监控的一个漏洞**。池子 1/6;A2 抽象核定理仍在跑;内存 1.59 GB、无孤儿。
>
> **换问法的理由已查实并记录**:给七篇用的通用外审第 3 问是 "Is the increment sufficient for this journal, **and if not, which journal fits?**" —— 内建向下引力,等于请专家找降落点,七篇齐齐降档是**我问出来的**。而 A2 那份问的是"borderline TIER-2,**那个级别的审稿人会反对什么**",于是产出具名门槛,照做后他确认门槛已达。**A2 不是特殊,是被问对了一次。**
>
> 新问法为**升档质询**:要一条具名、可达的定理;禁止用改投低刊作答;禁止再加同类定理(并把六篇优先权遗漏的教训讲给他);成功率给**数字**;可达性须点明由哪些现有引理喂给它,并区分"现有证明的困难延伸"与"另起研究项目";留诚实天花板出口。七份 15–22 KB 全部写就。
>
> **本 tick 修掉自己的一个漏洞**:原发送监控每篇只发一次,`extraction_failure` 后不会重发 —— A4、A6 已因此失败并滞留。已换成覆盖完整生命周期的调度器(未发则发、失败则重发至多四次、就绪则收回),并保持池子留一个 slot。
