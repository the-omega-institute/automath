...
| 篇 | 页 | 状态 | **去向** |
|---|--:|---|---|
| **A2** `cayley_chebyshev` | **77+33** | 四项修复齐;**乘性归一化已证等价**(未走"定义掉"那条路);类边界锐性已证 | **JFA**(审稿人:credible);备选 AIHP |
...
> **TICK 48 — 八篇全清,开 A2 第 1 条路线:抽象核定理**。池子空闲、**无 agent 在跑**、无孤儿、内存 1.27 GB;无可收割。(未提交的 15 个文件仍是你 8 月 3 日的 worker 改动,未动。)
>
> 八篇已全部走完外审、修复、终态复扫、投稿包七项齐备,故按"空闲论文派最有价值方向",派出审稿人五条路线之首、也是他最初给的门槛选项 (b):**覆盖稳定、Gaussian、Poisson 及其他多项式尾核的抽象核定理**。
>
> **任务框定为"抽象"而非"逐个推广"**,这一点写死在任务书里。要问的不是"能不能对更多核重做一遍证明",而是:**这套机制到底用到了核的哪些性质?** 要求逐步拆开现有稳定核论证(Thm 4.27、双背景 KL 摄动引理、代理正性与归一化、$o(s^{-r})$ 背景估计、Hessian 辨识、三阶余项),抽出每一步**真正需要**的最小假设,再在该假设下证抽象定理,并逐一验证成员:稳定核**必须退化回 Thm 4.27**;Gaussian(即本文当前排除的 $lpha=2$)**说清覆盖与否、不覆盖则指出哪条假设失效**;Poisson;以及至少一个另外的多项式尾核。
>
> **主要失败模式已提前点名禁止**:不得交出"假设列表就是把稳定核证明所需逐条抄下来、而无其他核被证满足"的定理 —— 那是空洞抽象。上一轮审稿人正是在 admissible class 上问过同一个问题(是否为可证性量身定制),他会一眼看出。
>
> 诚实结局按价值排序也写明:①最小假设 + 多个已验成员;②假设诚实但只有一两个已知核满足,则直说哪些自然核失效、失效在哪一步;③**机制根本不可抽象,并指出那一步不可约地依赖稳定核** —— 若是这个结局,**写在报告第一行**,它同样是真结果。
