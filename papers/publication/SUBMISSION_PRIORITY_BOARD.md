...
> **TICK 41 — 八篇投稿包逐项过筛,查出三处缺件(两处是我自己造成的)**。池子空闲;A2 收尾在跑(日志 2.4 MB);内存 876 MB、无孤儿;无可收割。
>
> 八篇都修完了,但只有 A4/A5 走过完整的 `SUBMISSION_PACKAGE.md` 清单,而它们的修复还发生在那之后。于是按七项必交内容把八篇逐一过了一遍:
>
> **两处是我自己弄丢的。** A3 与 A5 的**补充材料 PDF 不在工作区** —— 因为我此前复核时跑了 `latexmk -C` 却只重建了 `main.tex`。已重建:A3 补充 15 页、A5 补充 19 页,均 exit 0、零未定义引用。这类缺件不会有任何告警,只在投稿时才发现,正是清单存在的理由。
>
> **一处是真缺**:**A9 没有 `artifacts/literature_check.md`** —— 其余七篇都有,而 `SUBMISSION_PACKAGE.md` 把它列为必交项。这在 A9 上尤其要命:它正是优先权叙述被判"不安全"的那篇。已派工,并明令记录三件事而不是只挑好听的:Giraud III.2.1.5.3 是**输入而非贡献**;审稿人说大量定理结构属教科书 gerbe 理论/UCT 自然性/有限对偶/子群算术/纤维计数,**包装可能新但推进小于定理数所暗示**;以及他那句两半话——**未见有人逐字发表过两标签 wedge 分类,但这不构成实质优先权**,因为由标准等价复合出的精确陈述可以形式上新而几乎无独立优先权。**要求两半都写,不许只引有利的一半。**
