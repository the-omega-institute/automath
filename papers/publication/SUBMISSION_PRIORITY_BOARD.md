...
| **A3** `sharp_three_window` | **67+15** | ⚠️ 跨界确认(优先权 0.82);**"Equivalently" 一句印错,在修** | TAMS(档位未跳)|
...
> **TICK 87 — 文献审计三篇完成:A4 干净、A7 又一条伪造。A4 `2425e1b2a`、A7 `14d8790aa` 入库。**
>
> **A7 查出第二条伪造条目 `Sanna2016LucasSequences`**,签名与前几处一致:标题合理、无 DOI、所占卷页属他人文章,且**内部矛盾**(键写 2016、year 字段写 2019)。我独立核实支持该判定 —— Sanna 在 IJNT 的真实论文在**第 13 卷(2017)**,不存在第 15 卷 (2016) 233–243 的 Sanna 文章,且卷号与年份本身即矛盾(第 15 卷约 2019)。该条在两份活文档中**无引用点**,故无句子改动;唯一引用点在过期构建快照里,连同快照一并移除而非改指。另五处元数据更正(Vinson 页码、Yabuta 名字与标题、Sun–Sun 实为单作者 Zhi-Wei Sun、Fulton–Morris 首字母互换、Marques 页码)。
>
> **A4 是干净的**:78 条、无伪造、无 DOI 指向他人论文,仅四处外观性更正。它还查出**存在五份文献表副本,其中两份是投稿压缩包里过期的 70 条版本**,现已统一。八篇中已查四篇:**两脏、一净、一在查** —— 问题是零散混入而非系统性,更需逐条核。
>
> **一处我险些放过的不一致**:A7 的 agent 拦下了 codex 删除 `next_A7*.txt`、`REVISION_PLAN_FQ.md`、`scope_contract`、`theorem_inventory`,而我上一 tick 刚批准 A4 删同名的 `next_A4*.txt`。查证结果:`scope_contract` / `theorem_inventory` / `research_directive` **确为 `oracle_pipeline.py` 引用的受保护文件,拦得对**;而 `next_A*.txt` **无任何脚本拥有**,删得也对。两边都不必回滚,但这条差异值得记住。
>
> **三次审计一致的自身局限**:OpenAlex 全程 429、Semantic Scholar 多数 429,**三索引冗余从未真正达成**;带 DOI 条目均做了 Crossref 身份核对,无 DOI 条目为"Crossref + 一个官方来源"。服务失败一律不算反证,未核条目原样保留并记录。
>
> A9 的 25 个 DOI 审计已派(内存回升至 2.41 GB);A3 修复在跑。
