...
> **TICK 16 — 三篇同时给出"够不到"的诚实裁定，各自指明原因**。这正是新问法要买的信息。
>
> - **A3**：「不可达到」。以现稿事实边界为限，**没有任何已定位到原始出处的同层级具名公开问题**能同时通过四道门槛（结论须落在既有标准对象而非本文的循环语言秩映射上；不得把有限 Parry 数据、simple-Parry 性或待判定自动机结构预先作为输入；参数一致证明须由现有整数递推可得；等）。
> - **A4**：根本原因**不是算术强度不足，而是量词结构不匹配**——本文的弱泵送机制从一个**已假定属于 MCFL** 的无限语言中抽取一条同步轨道；而相邻的 Cobham 型交集问题、Erdős 数字限制问题与 ultimate-periodicity 问题**要求控制任意无限交集或整个自动集合**。
> - **A6**：审计了 Weinstein 三组问题，各需整数位置次序、支撑线不等式、或三次单位根下逐系数消去；**这些信息在本文的乘数—成本更新、正实压力、层计数与无标记 ACSV 中统统被消去**。给出天花板候选（0.80 × 6.2 = 4.96）：固定有限素数支撑下、典型成本以下但未及重尾临界线的严格活动区域内的精确乘数—成本联合系数渐近。
>
> ⚠️ 该候选与它**上一条被我们验伪的推荐同属一片领域**（假相变边界，正确比值 0.9982745 vs 所提 2.0077607）。已明令检验 agent：**先查相变边界，边界不成立立即停止**，不要投入后续论证。
>
> A2/A5 的 referee 式外审已发出（`c1a53a3b` / `c887b924`）。
