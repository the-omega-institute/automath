...
| **A8** `detector_shells` | 65+19 | **70%:Le Cam 局部渐近等价**(在改)| **Bernoulli** / EJS / SPA / SISP |
...
> **TICK 75 — A2 主定理已证并复核提交 `57da02447`;A6、A8 派工;八篇的第三代问答全部回齐。**
>
> **A2 先导引理通过,且在恰好 $(d+lpha)$ 阶矩下闭合** —— 无需矩余量、无需附加密度假设,这正是外审点名的主要不确定性。$(d+lpha)$ 矩只用在上商界 $p_1(y-w)/p_1(y)\lesssim(1+|w|)^{d+lpha}$ 一处;下界与 $|
abla\log Q_t^\lambda|\le2\|
abla\log p_1\|_\infty$ 由有界稳定 score 给出;近对角 $\Lambda\lesssim|y-z|^2$ 使核局部可积。**非紧 Green 配对**由环形算子 $A_{arepsilon,R}	o(-\Delta)^{lpha/2}$($L^1$)闭合,两个配对**先各自取极限、再组装非负 $\Lambda$ 形式**,不假定无穷远相消。
> 新颖性边界诚实:正文直书"**两点对数代数不是新的**",Klimsiak–Rozkosz 的条件 Hardy–Stein 已覆盖凸函数比值;可主张的只有测度初值下的移动分母抛物定理、充分有限矩定义域、非紧与 $t=\infty$ 两处闭合、以及与 Johnson Open Problem 4 的关联。有限一阶矩与有限初始相对熵**明写为未决**,不冒充最优。79→86 页,25→26 条编号定理。
> 独立复核:两文档从零重建 exit 0、0 未定义;**6/6** 产物脚本通过;SHA256SUMS **13/13**、LF;无修订痕迹。
>
> ⚠️ **`sec_introduction.tex` 799 行,距 800 上限只剩一行** —— 下次任何编辑都会越界,须拆分。
>
> **A8:70%** —— **Le Cam 距离下的局部渐近等价**:长确定性窗口内观测平稳格点更新过程 ≅ 从其 Palm 间隔律抽 i.i.d. 样本。逻辑上强于现有 LAN 引理,且可在两种观测方案间搬运有界损失决策论与半参数有效性。他明说这已是**现有机器的外边界**(再往 Markov-renewal 或一般 MAP 需另一套似然论证)。真正的难点不是更新论,而是**把两个 deficiency 写成完全一致可测的核**并避开边界周期条件化问题($N^\gamma$ 欠冲、$\gamma>1/2$ 正是为此)。
> 同时带回五处更正:重置条件**是秩一而非必然确定性**(故"泄漏必毁更新"过于绝对);平稳似然的乘积分解**不含全零记录**(须加定义域限定);端点命名与标准更新术语**反了**($A$ 是前向剩余寿命、$R$ 是后向年龄);端点因子本身**不是** $o_P(1)$,$o_P(1)$ 的是其对数在 $\sqrt N$ 局部似然比中的增量;观测对象是**格点更新指示过程**而非一般离散观测事件数据。另须补做聚簇源/超分辨极小极大统计文献的比较。
>
> **A6:64%** —— 分母加权 Brocot 分数的**全变差凝聚定理**(完整六项结论已给定,含 $ho_s=\zeta(s-1)/\zeta(s)$ 与极限词律)。派工写死了索引平移:领域对象按数字和 $n=a_1+\cdots+a_r$ 编号,我们的字母代价用 $n=d+1$、$c=2d+1$,**必须用 Brocot 的 $n$ 陈述**,否则会悄悄变成另一族。并要求核查是否只是已知重尾条件极限定理的直接推论。
