...
> **TICK 19 — 冲刺转段：A3/A4/A6 从"升档"转入"投稿就绪"**。这三篇已各自给出定位明确的"够不到"裁定（A3 四道门槛无一问题能同过；A4 量词结构不匹配；A6 所需信息被机器消去），继续问"该做什么"已无收益。故转为 referee 式外审，问的是**现有成果按对口刊投出去会被怎么打**。
>
> 提问稿要求把主定理连同全部假设、**明确列出未证与已撤回的主张**、引用归属与机器验证证据一并交出，并特别加重了第 2 问（新颖性与优先权叙述是否准确）：**"我们一篇姊妹论文的审稿人刚发现一处被遗漏的在先结果，改变了该文的整个优先权主张，故请把此问视为四问中最重要的一问。"** 这条直接来自 A2 本轮的教训。
