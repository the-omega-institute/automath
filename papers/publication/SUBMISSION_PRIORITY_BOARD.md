...
| 篇 | 页 | 状态 | **去向** |
|---|--:|---|---|
| **A2** `cayley_chebyshev` | **75+33** | 门槛已达 + 非空性 + 约定稳健性 + **规定 ansatz 内的渐近系数刚性**(非唯一性);修订中 | **JFA**(审稿人:credible);备选 AIHP |
...
> **TICK 46 — 两处更正:一处是我夸大了结果,一处是这块 board 的性质**。池子空闲;A2 修订在跑;内存 659 MB(仍是 SupportAssist 占大头,非本项目)、无孤儿。
>
> **更正一:我把第 4 条说满了。** 上一轮我写"第 4 条(部分刚性)也到手"。审稿人明确否掉这个说法 —— 第二条命题**实质弱于唯一性或熵极小性**,应称"**规定 ansatz 内的渐近系数刚性**",**不算 item 4 达成**;他还指出稿件本身大体已作此区分,**即我的说法比论文更满**。上表已改用他的口径,修订任务里也要求正文、引言、摘要一律统一,笼统非唯一性声明保留。
>
> **更正二:这块 board 是滚动快照,不是日志。** 我每 tick 说"board 已更新"属实,但实现上新条目**替换**旧条目 —— 全文始终只有一条 TICK 记述,并非累积。逐 tick 核查确认:**没有丢数据**,每一轮的完整记述都在 git 里,例如 `git show 30fbda06b:...BOARD.md` 可原样取回 tick 37 的 UTF-16 那一节,而 `git log -- ...BOARD.md` 的单行摘要本身就是一份可读的索引。顶部的八篇状态表是**累积且始终当前**的。
>
> 若你想要的是可直接翻阅的累积日志而非当前快照,说一声,我改成追加式;不改的话,现状是"表看当前、log 看历史"。
>
> **审稿裁决要点**(已提交 `3971da864`):两条命题**均正确**,修订而非拒稿;**机制已读作中心定理而非一次孤立构造**,JFA 投稿可信。我送审时最担心的"admissible class 是否为可证性量身定制",答案好于"不是":部分被排除的约定确实无害可加,但**另有若干会真正在 $s^{-2r}$ 阶改变代理熵** —— 原点邻域不衰减的保留剖面即为一例($	au=\kappa$、$V=\kappa q_s$),该例已要求写入正文,**把看似的局限变成已证的锐性**。
