...
| 7 | `brocot` | 25 | 待定（TAMS 已拒：增量性） | — | 外审在飞：问"要证出哪一条 $X$ 才会送外审" | 收 `BR` |
| 8 | `folded_histograms` | 48 | 待定（ETDS 已拒：显著性） | — | 外审在飞：问 48 页里还剩不剩独立结果 | 收 `FH` |
| 9 | `scan_projection` | 21 | 待定 | — | **本冲刺从未被审过**，首轮冷判在飞 | 收 `SC` |
