...
| **A7** `upper_fibers` | 33+36 | **74%:大本原素因子/Wall–Sun–Sun 二择一**(在改)| FQ(待重估)|
| **A8** `detector_shells` | 65+19 | ✅ 实质数学被接受;五处修复完成 | **Bernoulli**(若围绕 F–G 重构)/ EJS / SPA / SISP |
| **A9** `homological_visibility` | 37+6 | ✅ 更正被接受;两处误引已清 | Cahiers |
...
> **TICK 74 — A5 复核通过提交 `c227bf442`;A7 天花板从 <3% 变成 74%;A6 拿到 64% 目标。**
>
> **A5 带回一个诚实的负结果,而这正是该报告预警的风险。** codex 查出该乘性方程**确可经非显然变换归约到已发表算法**:对数微分给出加性 Mahler 方程 $(\sigma-1)u=rac{z}{p}rac{H'}{H}$,再由 Arreche–Zhang 的 Mahler discrete residues 与 Chyzak–Dreyfus–Dumas–Mezzarobba 判定;逆向由"$u/z$ 在零点正则、无多项式部分、仅有整留数单极点"重构 $R$。故**"形式不可归约"不成立**,引言已直书此事。真正属于我们的只剩:归一化单系统重构、显式尖锐 $O_p(D\log D)$ 次数阶、高度与系数界、精确接受/拒绝、固定 $p$ 多项式比特复杂度。它还**纠正了外审**:命题 3.11 印的是 $b_p(D)=\lceilrac{2D}{p}m_p(D)ceil$,不是报告写的 $N_p(D)$(因 $b_p\le N_p$,3.13(iii) 仍成立)。
> 独立复核:从零重建两文档 exit 0、0 未定义;41/41 测试;2/2 verifier PASS;SHA256SUMS 15/15、LF、**归档输出不含挂钟耗时**;最长 `.tex` 742 行。顺带清掉了上一轮更正的残留(定理 3.17 证明里那句"这正是定理 3.9 的奇偶性输入")。
>
> **A7 是本轮最大的翻转。** 上一代提问给出的天花板是 $P_M<3\%$;换成"领域标准对象"问法后,他给出 **74%**(技术成功率 >95%,折价几乎全来自先例风险)的目标:**大本原素因子 / Fibonacci–Wieferich 二择一** —— 对每个 $arepsilon>0$,充分大的 $n$ 要么 $F_n$ 有本原素因子 $p\ge n^{2-arepsilon}$,要么有 $q$ 使 $q^2\mid F_n$($q$ 即 Wall–Sun–Sun 素数);从而若无 Wall–Sun–Sun 素数,$P_{m prim}(F_n)\ge n^{2-o(1)}$。**缺的唯一零件是经典的 Lucas 分圆因子定理** $\Psi_n(1)=c_nU_n^{m prim}$($c_n=1$ 或 $c_n\mid n$);我们现在只保留了单向整除,够界素数个数,不够界其根积。已派工,**文献审计前置,若已被记载即停并报告**。
>
> **A6:64%** —— 分母加权 Brocot 分数的**全变差凝聚定理**(典型连分数在权 $q^{-s}$、$s>2$ 下有一个部分商等于 $n-O_P(1)$,左右有限连分数词收敛到显式独立极限上下文)。技术成功率远高于 64%,折价来自新颖性定位与架构。证明**多半已在定理 6.3 与引理 6.5–6.6 内**,只差把逐点上下文极限、可和控制与"无唯一大数字配置可忽略"归一化成概率律。
> 他还纠正一处索引:领域对象按数字和 $n=a_1+\cdots+a_r$ 编号,而我们的字母代价用 $n=d+1$、$c=2d+1$ —— 目标定理须用 Brocot 的 $n$ 变量陈述。
>
> **A7 还逐条拆穿了我们的命名**:$F_d$ 就是标准 fibotomic 整数 $\Psi_d(1)$,不得当新对象;"atomic 素幂"误导(除子格里的 atom 是素数);"birth layer" 是本地术语,$B_n=lpha^{-1}(n)$ 只是秩映射的普通水平集。这些框架更正已并入派工。
