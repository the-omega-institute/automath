...
> **TICK 109 — 等待 A4 有效性一节;把第五种问法在其余各篇上逐一试过,确认已用尽。**
>
> "邻近结果有我们没有的东西,能否也拿到"这一问法在 A4 上换回 0.94 的可执行结果。逐篇复核是否还有同型机会:
>
> - **A7**:差距方向相反 —— 我们更强(指数 $2-o(1)$ 对 Hong 的 $(\kappa+1)n-1$)但**有条件**;去条件化等同于解决 Wall 猜想,外审已明说该定理不解决它。无机会。
> - **A5**:**我们有**对方缺的 —— 显式整数比特高度界,而 Chyzak–Dreyfus–Dumas–Mezzarobba 按系数域代数运算计复杂度。方向相反,无机会。
> - **A9**:天花板是**优先权**而非能力(即便证出也是 Jardine/Breen/Nikolaus–Waldorf 已有分类的特化),补机器不解决。无机会。
> - A2/A3/A6/A8:各自的下一步都落在待作者决断的结构选择上,不在此问法射程内。
>
> 故第五种问法已用尽。此为逐篇查证后的结论,非默认。
>
> 池 0/6,codex 一个 agent,内存 0.79 GB、缺页 68.4,八篇工作树干净。
