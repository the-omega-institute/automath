...
| 篇 | 页 | 状态 | **去向** |
|---|--:|---|---|
| **A2** `cayley_chebyshev` | **81+33** | 五条路线拿下**三条**(抽象核定理/稳健性/系数刚性)+ 门槛定理 + 非空性;核定理送审中 | **JFA**(审稿人:credible);备选 AIHP |
...
> **TICK 52 补 — 七份路线图全部到齐;A6 升档定理已证并复核入库;一处提交纪律失误**。
>
> **A6 拿下了它的升档定理**(`0c3d0cc03`):算术临界稳定更新,$b_C=8$ 为**显式**绝对收敛 continuant 上下文和(非极限代称),且**未假设正则变差** —— 这条写在 `sec_introduction.tex:467`("its proof does not assume the regular variation later used by the renewal transfer"),正是审稿人自设的防作弊条款。证法:隔离唯一 $>n/2$ 的部分商、控制收敛取上下文和、另证两类区域贡献 $o(n^{-s})$,顺带排除局部格与半稳定振荡。独立复核:65+20 页、缺陷全 0、三 verifier 零失败、20 测试、清单 15/15 且**重跑逐字节复现归档产物**。
>
> **A5 查出我们一处很可能为假的主张**:现稿称"模二行列式奇偶性对特殊值提升**本质不可缺少**"。审稿人判其**很可能错误** —— 奇偶性给出整系数,但 **Kumiko Nishioka 的印刷定理允许受控公共分母**($\log d_h\ll hL$),不要求系数为代数整数,而**我们自己的系数递推**就能给出线性指数分母界。故 parity 是方便且强的**充分**条件,非必要。他并诚实指出代价:改正后提升**更一般,但也更明显地成为两条 Nishioka 定理加一个初等分母估计的组合** —— 任务书要求新颖性口径随之下调。
>
> **A9 的一档定理必须是强形式**:把依赖覆盖的上闭链构造变为**对标准资料 $(X,A,(\omega_\lambda))$ 加细相容且与表现无关**的构造,并精确定出不可避免的 $H^1$、$H^0$ 歧义,Čech 群取**可能不连通重叠上的实际截面群**。他点名**不会升档**的:去掉 $H^1(N,\mathbb Z)=0$、加 Ext 标记例子、放宽有限群、换更大 Čech 复形。
>
> **纪律失误照记**:提交 `94a3dea75` 时我用了 `git add -A papers/publication`,把 A2/A4/A8 **仍在编辑中且未经我复核**的状态一并扫入,而提交信只写 A5/A9。没有丢失内容,各篇完成后仍会有经复核的正式提交覆盖;但这违反了本轮"复核通过才提交"的规矩。改法:此后按论文目录逐个 `git add`,不再通配。
