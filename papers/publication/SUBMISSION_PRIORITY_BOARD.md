...
> **TICK 36 — 更正上一条:A2 的审稿人确实给了一条通往 tier-2 的具名路线,我上轮漏了**。池子空闲;仅校验和推广在跑;内存 968 MB、硬缺页 0。
>
> 上一 tick 我写"没有任何一位审稿人指出通往更高档的路径",那是凭印象。本 tick 逐篇搜了八份报告,该说法要分两半:
>
> **通往 tier-1 的路径,确实无人指出** —— 这半句成立。
>
> **但 A2 的审稿人明确给了通往 JFA(tier-2)的路线**,写在"修改门槛"一节:
>
> > 仅修正文献和补充技术细节可使定理成为可信、可发表的结果,但不足以显著改变 JFA 层级判断。**要达到该层级,最有效的增加是任意阶的 law-by-law 非负尾缺陷分解,或覆盖 Gaussian、稳定及更一般核与散度的抽象机制定理。**
>
> 两条并列的具体目标:(a) **任意阶**的 law-by-law 非负尾缺陷分解;(b) 一条同时覆盖 Gaussian、稳定核与更一般核/散度的**抽象机制定理**。
>
> 这条值得追,理由有三:它是**审稿人自己认定能改变档位判断**的东西,不是我们臆想的加法;它正对"保证 tier-2"的目标;而且我们**已有 law-by-law 稳定分解**(见 `89f64eb36`),他要的是把它推到**任意阶** —— 起点已在手上,不是从零开题。这与"不要机械追加定理"并不冲突:机械追加是往边上堆命题,这是把已有机器推到审稿人指名的强度。
>
> A7 那七处 open-problem 命中查过了,全部是 BLMS 猜想与 H1/H2 的冲突,修复时已处理,不是升档路线。
>
> **暂不派工**:校验和推广 agent 正在改包括 A2 在内的七篇,同一篇不并发。待其退出后再派。
