...
> **TICK 71(续)— 第三代提问命中:A4、A5 各拿到一条"不含本文任何自造术语"的可达定理,已派 codex 去证。**
>
> | 篇 | 目标定理(领域对象) | 成功率 | 分类 |
> |---|---|--:|---|
> | **A5** | 一阶乘性 Mahler 方程 $P_0R(z)^p=P_1R(z^p)$ 有理解的有效定理(唯一性、判定算法、$N_p(D)$ 次数界、高度界、Padé 重构、固定 $p$ 多项式比特复杂度、$\Omega(D\log D)$ 尖锐) | **75%** | (A) 困难但自然的延伸 |
> | **A4** | **slender-CFL Cobham 定理**:同一整数集在两个乘法无关 weak-Perron 数系中的表示语言若都是 slender 上下文无关语言,则该集有限 | **70%** | 另起项目(复用两块现有机器) |
> |
> A5 的零件**多数已印在论文里**($p=2$ 版):命题 3.7、3.11、引理 3.5、注 3.12 本就对任意 $p$ 成立;真正缺的是一条合并的一般 $p$ 有效重构引理(五项:系数分母界、Padé 商与解的选取无关、恢复全部次数 $\le N_p(D)$ 的有理解、恒等式不符时拒绝、固定 $p$ 比特界)。
> A4 的量词与现有定理 2.25 **全线不同**(两系统 vs 一系统;整个表示语言 vs 存在无穷 MCFL 子语言;无素支撑条件;结论是集合有限)。缺的是主根转移引理,风险在**外围特征值碰撞、Jordan 块与相消须在实际仿射块矩阵层面审计**,不能由增长夹逼推断。
>
> **他还查出一处真缺陷(A5)**:定理 3.17 的证明仍写着"这正是定理 3.9 中的奇偶性输入",而 3.9 现已是**无奇偶性**版本(其证明用定理 3.8 与正性,注 3.10 明说特殊值提升不需要奇偶)。上一轮更正的残留,已列入修复。另三处:全文根本没有 Dieudonné 行列式(只有 Dieudonné–Dwork 整性判别法,是另一回事),该词须整个删去;$F_{H,2}(y)^2=\Pi_y(H)$ 的平方须显式;(OA) 须对**特征标**而非群元素量化。
>
> **两份派工都把优先权检索放在最前而非最后** —— 两位报告本身都点名必须先查(A4:"未找到不等于新";A5:"主要风险不是证明失败而是先例碰撞")。A4 另加一条:**目标若不闭合,不得制造更窄的替代品,须报告卡在哪** —— A9 那条 no-go 正是靠这条才浮出来。
>
> 在飞:A7 `8ca43d8c`、A9 `47eb976a`、A8 `3a0131d7`、A2 `985fe20b`(池 4/6)。
