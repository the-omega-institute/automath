...
> **TICK 7 — A4 给出 $\Sigma^0_1$-完全性，但需高度存疑地核验**。A4 r5（8.9 KB）声称几何同步方案承诺问题 GSR 在逐实例可计算多一归约下是 **$\Sigma^0_1$-完全**的，且限制到 $A=\{0,1\}$、$\mu_U(X)=(X^M-Q)(X^M-1)^E$（$Q\ge4$）仍成立。
>
> **这正是同一专家此前两轮明确拒答的问题**——当时说在给定有限输入模型下既证不出可判定性也证不出 $\Sigma^0_1$-困难性。这种规模的翻转要么是真进展要么是错误。已派检验并写明四个必查点：多一归约是否全函数且可计算、归约是否保持承诺条件 (U1)–(U4)（离开承诺类就不算归约）、**上界**（完全性还需 $\Sigma^0_1$ 隶属，即见证 $W(t)$ 满足 $\mathrm{val}_U(W(t))=cb^t$ 须由可计算谓词验证）、以及受限族是否真的实例化了一般构造而非悄悄假设了待证结论。
