...
> **TICK 77 — A7 定理已证并复核提交 `c7a5d7881`;A9 范围更正提交 `0d170fef5`;A2/A7 重估已投。**
>
> **A7 是换问法击穿天花板的完整闭环**:上一代给 $P_M<3\%$,这一代给出 74% 的目标并已证成 —— 对每个 $\varepsilon>0$ 及充分大 $n$,$F_n$ 要么有本原素因子 $p\ge n^{2-\varepsilon}$,要么有 $q$ 使 $q^2\mid F_n$(此时 $z(q)=n$,$q$ 为 Wall–Sun–Sun 素数);故若无此类素数则 $P_{\rm prim}(F_n)\ge n^{2-o(1)}$。缺失零件由 Granville 的 Lucas 分圆因子定理补上,判别因子 5 显式对账($z(5)=5$,故 $n\ge13$ 时 5 永不本原,残余的 5 并入 $c_n$ 且整除 $n$),并对 $13\le n\le210$ 全部 198 个指标做了精确整数抽查。
> **codex 第二次纠正外审,这次在证明枢纽上**:拒用 outline 的 $2/\log\varphi$,改用论文的 $(\frac{\log\varphi}{2}+o(1))$。手算复核:前者给 $\frac{(\log\varphi)^2}{2}\approx0.116\log n$,**推不出指数 2**;后者恰给 $2\log n$。它还补上 outline 缺失的 $O(\log n)=o(\phi_E(n))$(经 $\phi_E(n)\gg n/\log\log 3n$)。最近前驱 Kiss 1987 是"几乎所有/全局"版本,非逐点、不定位精确秩、推不出条件结论。
>
> **A9 范围更正完成**:命题 4.1 现写明需连通有限交**加上**把每个系数群与限制映射相干地化为同一固定 $A$;定理 4.2 现写明 gerbe 类是**预先给定**的固定覆盖固定带对象层构造;$u_a$ 的单射与双射等价归属定理 3.7。我独立核过**逐文件度数记号计数与 HEAD 完全一致**,12 处含 $H^1/H^2/H_1/H_2$ 的改动行全是文字重排,记号逐字未变;无编号环境边界移动。
>
> **两处复核纪律的实例**:A7 首次复核我自己误报 3 处未定义跨文档引用 —— 那是**我先建 main 后建 supplement** 的顺序问题,补完交替一轮即归零,不是论文缺陷;A7 首次提交**失败**(消息里 `"atomic prime power"` 的双引号提前终止 `-m`),改用消息文件重提,否则成果会停在暂存区而误报已入库。
>
> **重估在飞**:A2 `6e31b37a`、A7 `b46e5c70`。另 A3 第三代提问 `2a67cdc8`。池 3/6。
