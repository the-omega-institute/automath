...
| **A4** `prime_languages` | **29+43** | ✅ 残差分析写全 + Charlier–Kreczman 比较(**重叠比审稿人所述更重,已如实写入**) | Monatshefte |
...
> **TICK 63 — 回验结果开始回来,三篇的裁决都往好里走;补上 A3 这个缺口**。池子 1/6;codex 跑 A6、A8 收尾;内存 1.65 GB、无孤儿。
>
> **A6 反转**:更正**经确认成立**,他连**俄文原版**都查了(排除英译引入错误),机制定位到"**丢掉 $u>1$ 限制导致端点被计两次,多出恰为 $R_s$**";**优先权异议撤销**,并明说"**不会仅因为它更正了前人文献就预先降档**"。这是当初被判"即使完全修复 TAMS 仍拒稿"的那篇。
>
> **A4 转 minor revision**:两项异议全解,证明**无需重构即可核验**。而且我们那份**对自己不利**的自查判对了 —— 他确认 "**your derivability reading is correct**"。无具体待办,剩余风险是编辑口味,**故不派工**。
>
> **A8 实质数学被接受**,compact-uniform LAN 判 yes,且一致性**没有被误述成全局**。五处局部缺陷已派修 —— 其中最要紧的又是我们说满了:G(iii) 把"局部似然比检验"与 score 检验并列,而证明只给出**极限实验中的等价**。
>
> **补上一个缺口**:A3 的定理 A 证完复核了却**从未送审**,而审稿人当初**预先承诺**"证成即认可其足以升层"。本 tick 送回请他兑现,并把最该查的一条写在前面:**这个类是否被收窄成"披着一般外衣的我们自己的族"** —— 若是,定理一文不值;也告诉他若结果达不到他当初所指,**直说比为保持一致而兑现承诺有用得多**。
