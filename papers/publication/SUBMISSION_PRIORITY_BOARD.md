...
> **TICK 128 — 内存与缺页双双突破且这次确为持续,但元凶是 Windows Update,非本冲刺。**
>
> 首采 0.40 GB + 19606 缺页/s;复采四次为 **1195 / 986 / 1075 / 4585 每秒**,内存 0.60–0.75 GB —— 与前三次尖峰不同,**本次确实持续在 1000/s 界附近及以上**。占用前列出现此前从未有过的 **`TiWorker` 706 MB**(Windows 模块安装工作进程),持续换页正是系统更新在装。
>
> 本冲刺侧:**零 agent、零 stdin 孤儿、无未提交论文改动**,规则的"减少并发"与"清理孤儿"两项处置仍无对象;中断安装中的 `TiWorker` 是坏主意,不做。预计更新完成后自行缓解。
>
