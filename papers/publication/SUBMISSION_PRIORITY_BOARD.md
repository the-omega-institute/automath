...
> **TICK 93 — A4 重估在飞;本 tick 复查三次审计共同留下的"未核"尾巴,确认它们是查不到而非造假。**
>
> 三份审计都记了同一条局限:OpenAlex 配额为零、Semantic Scholar 多数 429,故约 12 条条目只能标"未核"。限流已过一段时间,我自己重试若干:两条 Noorani 条目(`MeslizaNoorani1999`、`Noorani1995`)在 OpenAlex 与 Semantic Scholar **均返回空**(是无记录,非报错);Csiszár 1967(Studia Sci. Math. Hungar. **2, 299–318**)在 Crossref 中只见其 1975 年及以后作品 —— 该 1967 年匈牙利期刊**早于 DOI 注册**。
>
> **结论:这批条目属"确实查不到",与 A2/A7 那两条伪造条目**(标题合理、DOI 不解析或指向他人、作者名被改动、且所占卷页归他人所有)**不同类**。审计把它们保留原样并公开标注,而不是删除或编造替代,处理正确。这一复查本身也说明:**"未核"不该被当成"可疑",否则前 DOI 时代的经典会被误删。**
>
> 池 1/6(A4 重估 `4dba0bdc`),无 agent,内存 1.15 GB、缺页 169,八篇工作树干净。
