...
> **TICK 86 — A5 配重入库 `b2b65b8fd`;A3 重估回来并查出一处印刷错误(附反例)。**
>
> **A3:跨界经确认是真的,不是把名词换掉。** 他核过引理 2.5 确已脱离数字结构,定理 2.6 的量化陈述正确,且**未暗用**规范秩双射、数字词合法性、循环折叠、原始数字的正负实现、像移位与解码器。尖锐性搬运也合法 —— $E_m$ 虽最初在碰撞问题里发现,定理 2.6 只把它们当作满足递推恒等式的有界系数词。**优先权约 0.82 成立**(这是我承认最无力自答的一问):经典文献给的是正则性、有限状态识别、正规化与各种有界延迟结果,他**未找到**这些重叠图最长无圈瞬态的孔径线性界,且该界**不由正则性routine地推出**。
>
> **但他查出一处印出来就是假的表述,并用我们自己的非标准例子给了反例。** 定理 2.6 的 "Equivalently" 一句从"起点首坐标非零的路径"滑到"任何含非零暴露系数的重叠链",在自然的无锚定读法下不成立:取 $U=(1,2,4,9,\dots)$、$D=2$、$m=2$,边为 $a\to b\iff a+2b\equiv0\ (\mathrm{mod}\ 4)$,得 $0\to0$、$0\to2$、$2\to1$,$1$ 为终点;非零顶点出发无可达圈,但 $0\to0\to\cdots\to0\to2\to1$ 可任意长且终将含非零暴露系数。**正确表述须加锚定**:"把链平移到初始暴露系数非零之后,它要么到达有向圈,要么至多再有 $C(U,D)m$ 次重叠。" 另需补一句:完成系数唯一且必满足 $|c|\le K_{U,D}$ —— 边其实定义在混合字母表 $[-D,D]^m\times[-K_{U,D},K_{U,D}]$ 上,不能让"有界表示"被读成 $|c|\le D$。
>
> 派工要求 **codex 自己重算该反例**再改写,**若算得与外审不符则停下报告**;并把它加入产物回归用例,免得无锚定读法回潮。
>
> **档位不跳**:TAMS 作为进取型首投"更可辩护了一些",ETDS 仍是强自然备选;他保留意见是全篇仍 67 页 52 条结果、标题与架构仍以循环语言秩重编码为中心。
>
> A4、A7 文献表审计在跑。
