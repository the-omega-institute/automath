...
> **TICK 22 — A4 外审暴露一个我们自己造成的投稿缺口**。裁决：**实质性大修**（若期刊不允许补交材料则拒稿后重投）。核心链条「弱同步泵送 → 模有限群返回 → 删失同余拓扑 → 素因子支撑障碍」判为**总体可信**，几何射线刻画、单位系统整除树、Perron 子类分类具可发表内容，且**摘要未夸大正文**。
>
> 但审稿人写道：**实际可得附件只有正文 PDF——补充材料、Python 脚本、单元测试、存档输出均未随附**，故其对机器验证的意见"只能作程序性判断"。**这是压缩的副作用**：我们把材料迁进补充文档以缩短正文，投稿时却没把补充文档交出去，等于把机器验证信用作废。排查确认 A2/A4/A7 三篇**都有** `supplement.pdf` 与脚本，**都没有提交清单**。已建 `SUBMISSION_PACKAGE.md` 定义必交七项与投前自检（含"硬编码 `\suppref` 编号不触发任何 LaTeX 告警、须逐条核对"一条）。
>
> 另三项待修：一处实质证明缺口；Seki–Matsumura–Fujii–Kasami 弱泵送的优先权比较需深化（审稿人确认我们**正确区分了弱/强泵送并引了 Kanazawa 反例**）；若干量词与有效性表述需精确化。
