...
> **TICK 8 — A2 否定了我们自己选的主攻方向**。A2 r7（17.3 KB）判定：继续攻 **Johnson Section 7 Open Problem 4 高影响但低成功率，不应作为首个项目**；并明说本稿机器目前**不足以完整解决任何同级具名开放问题**，Johnson 上可达的层级只是"所有对称稳定指数下的有限方差特例"，不是原问题的完整解决——**这与我们 TICK 2 记录的"部分解答"口径一致，须保持准确、不得升格**。
>
> 它给的首选是把稿中两条**正交**机器合成：(i) 一维 Cauchy 核下任意阶首失配矩的平方型 KL 渐近；(ii) 任意维、任意各向同性严格稳定核下**仅在协方差阶 $r=2$** 的临界 $L^q$ 余项、Bregman 转移与最优矩指数。合成为**全阶稳定核首失配张量定理 + 最优统一矩指数**（$d\ge1$、$0<lpha<2$、$eta=d+lpha$、整数 $r\ge2$）——判为结构性增量而非"再算两阶"。已派检验并执行，明令不得让 $r=2$ 的假设悄悄延用到一般 $r$。
