...
| **A8** `detector_shells` | **65+19** | ✅ 实质数学被接受;五处修复完成 | MCAP |
...
> **TICK 64 — A8、A9 收尾入库;回验一轮下来,四篇裁决全部改善**。池子 1/6(A3 回验在飞);无 agent、无孤儿、内存 1.20 GB。
>
> **A8 的 G(iii) 按正确方向收回**:现在只主张残差化 score 检验一致渐近达水平 $lpha$ 并达到 Gaussian 半空间功效包络,与局部似然比规则的等价**移出定理**、记为极限实验中的重合。要证有限记录版本还需有限样本局部邻域、似然上确界存在性、极大点局部化、一致 argmax 传递 —— 我们没有,就不写。另四处是定义性补全(Hankel 奇异时的全定义、score 检验的全可测定义与门控、chart 相容性由断言改为证明、"精确"距离改述为对 $n>2$ 最终精确)。**实质范围未收窄**,这是他明确说过不需要的。测试 53→58。
>
> **A9 两处误引已清**:`04TU` 本是全子范畴判据,却被我们用来支撑"Isom 层是 torsor";`06NY` 只是定义 gerbe 的一般章节,却被用作 Giraud $H^2$ 分类在带等价下自然性的依据。现改由 `02ZP` 承担其真正陈述的事实,两个 tag 全源清零;方案 B 的抽象同构已换成**由分支映射诱导的保标记具体双射**。
>
> **另一件值得记的:他指出的第三点不是论文的错,是我们提问文本里的错** —— 摘述把 $H^1(N,\mathbb Z)$ 写成 $H_1(N,\mathbb Z)$,他明说"PDF 是对的,摘述是错的"。已对全源做上下标审计,确认稿件干净。**送审材料自身的质量也会影响裁决可信度**,这条得记住。
