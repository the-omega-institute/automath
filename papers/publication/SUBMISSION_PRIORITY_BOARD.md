...
> **TICK 47 — 查投稿件装配状态,发现 A7 连投稿信与 metadata 都没有**。池子空闲;A2 修订在跑(日志 1.8 MB);内存 1.09 GB、硬缺页 120/s、无孤儿;无可收割。
>
> 八篇都"完成"了,但**完成不等于可寄出**。查装配状态,两处缺口:
>
> **A7 既无 cover letter 也无 `submission_metadata.md`** —— 其余七篇两样都有,`SUBMISSION_PACKAGE.md` 也把它们列为必交项。`find` 全盘搜过,匹配到的只是数学意义上的 "covers"(witness covers、minimal covers)章节文件,**确实一件都没有**。这在 A7 上尤其不能省:它是**拒稿重投**,投稿信必须交代改了什么 —— 补编结果已进正文并编号、Wagner 与 Hearne–Wagner 已引、H1/H2 已改名且 Corollary 6.9 转为纯条件。已派工,并要求 FJMV 那两个问题**必须从稿件里核实其确切表述后再写,不许照任务书转述**。
>
> **另六篇尚未装配 `submission_bundle/`**(仅 A4、A5 有,分别 32 与 38 个文件)。这一步等你确认是否要现在投再做 —— 装配本身不难,但没必要在投稿时间未定时先固化一份快照。
