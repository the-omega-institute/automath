...
|--:|---|--:|---|---|---|---|
| 1 | `single_primitive` | 11 | **DMTCS**（短文形式） | 现状 <10% → 修后 **30–35%** | 89→11 页重建完成；Chow–Jones 已补、Sanna 描述已改正、书目 14/14 双向配平 | 送新一轮裁决 |
| 2 | `window6` | 11 | **EJC** | 现状 18–22% → 修后 **25–30%** | 80→11 页抽取完成；第 3 节两处真缺陷（引理 3.1 假、命题 3.2 缺随机上界）**修复重跑中** | 收 agent → 独立复核 → 提交 |
...
| 6 | `joukowsky` | 13 | 复分析/位势论专业刊 | **~50%**（原 90 页版 <15%） | 90→13 页抽出定理 3.21，自评表已删，存档完整 | **补先行工作定位**（仅 4 条引用） |
| 7 | `brocot` | 25 | **JNT**（现状）／强专业刊（若证出穿越定理） | — | 常数争议已结（我错，$b_C=8$ 对）；**穿越定理攻坚中** | 收结果 |
| 8 | `folded_histograms` | 48→6–8 | **The Fibonacci Quarterly** | **~45%** | 判为一条干净观察的短 note；定理 5.14 另存为第二篇候选 | **改写中** |
| 9 | `scan_projection` | 21 | **Stochastics and Dynamics**（重建后） | 现状 <5% → 重建后 **~20%** | 判 **撤回重建**：定理 6.1 是被定义做成的恒等式 | 围绕定理 7.2 重建 11–13 页 |
