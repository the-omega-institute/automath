...
injective optimum. For an arbitrary irrational slope with irrationality
exponent $\mu(\alpha)$, the three-gap multiplicities determine the exact
logarithmic R\'enyi spectrum. More precisely, the coefficient of the
logarithmic correction is
\[
\kappa_q(\alpha)=
\begin{cases}
...
\end{cases}
\]
when \(\mu(\alpha)<\infty\), with coefficient one at \(q=0\) and zero at
\(q>0\) in the Liouville case. The formula holds
in the liminf sense of
$\bigl(m\log\varphi-D_q(\pi_m^{\mathrm{fold}}\|\pi_m)\bigr)/\log m$.
Thus slopes of irrationality exponent two retain the full logarithmic
correction at every order. This finite-$m$ formula calibrates the universal
...
The exact three-gap description of Sturmian factor frequencies is classical
\cite{Berthe1996SturmianFrequencies}. Goda recently characterized badly
approximable rotations by quasi-uniformity of their Kronecker sequences
\cite{Goda2024QuasiUniform}. Theorem~\ref{thm:diophantine-renyi-spectrum}
goes beyond this bounded-ratio dichotomy: it determines the complete
logarithmic R\'enyi spectrum for every irrational slope and transfers that
spectrum, without loss, to the Parry-divergence correction of the canonical
fold.
...
relabeling, the bounded-type asymptotics, the Diophantine R\'enyi spectrum,
and the primitive-SFT extension.
