...
of the finite-state calculation.  We also record a conjectural
H\"older block-recoding framework; a complete proof in that broader
setting would require additional operator-theoretic work that we do not
carry out here.
...
\begin{remark}[Conjectural framework for a H\"older extension after block recoding]
...
denote the depth-\(n\) Bayes error computed from block prefixes.
Part~(i) below is exact; parts~(ii)--(v) are conjectural and describe
the operator-theoretic framework that would be needed to extend
Corollary~\ref{cor:error-resolvent-cyclotomic} and
Theorem~\ref{thm:quasistationary-ambiguity} beyond the finite-state
setting.
\begin{enumerate}[label=(\roman*)]
  ...
  \item Let \(\widehat Y^{(\ell)}\) be the natural extension of
    ...
    \[
      ...
      := \int f\,\dd\nu_{\varphi^{(\ell)}}^{-},
    \]
    the block-error formula should take the form
    \[
      \varepsilon_n^{(\ell)}
      (\mathsf P_R^{(\ell)};\nu_{\varphi^{(\ell)}})
      =
  ...
  \item The isolated spectrum of
    ...
    Ruelle operator \(\mathcal L_{\varphi,H}\).  Under that
    identification one would therefore expect every pole of the
  ...
  \item If, in addition, the \(\ell\)-block first-entry residue event
    ...
    constants \(c_r\) for which one expects the asymptotic expansion
    \[
      \varepsilon_m(\mathsf P_R;\nu_\varphi)
...
\medskip\noindent\textit{Outline of strategy for parts~(i)--(v).}\quad
...
operator on the left-history space, and killing by a one-step hole
depends only on the current block state.  Disintegrating the exact
cylinder formula for Bayes error over past histories leads to the stated
operator representation.

Parts~(iii)--(v) follow the same formal structure as the finite-state
case (Corollary~\ref{cor:error-resolvent-cyclotomic}
and~\ref{thm:quasistationary-ambiguity}), but their rigorous
justification requires: (a)~verification that the killed backward
operator has a spectral gap on \(\mathcal H_\alpha\); (b)~identification
of its isolated spectrum with that of the forward open Ruelle operator
via the natural-extension pairing
\cite{Bowen2008,Ruelle1978,Walters1975GMeasures}; and (c)~H\"older
regularity of the residue-hit probability functions.  We do not carry out
these verifications here.
\end{remark}
