...
records a supplementary algebraic consequence: a resolvent reformulation
of the finite-state calculation.  We also state, as a formal
transfer-operator framework, the expected H\"older block-recoding
extension; a complete proof of the latter would require additional
operator-theoretic work on spectral gaps and regularity that we do not
carry out here.
...
\begin{theorem}[H\"older Gibbs cyclotomic lift after block recoding
  (conditional on spectral-gap verification)]
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item If, in addition, the \(\ell\)-block first-entry residue event
    satisfies the residue non-degeneracy condition from
    Theorem~\ref{thm:first-entry-escape-rate}, and if the leading open
    resonance \(e^{-\gamma_H(\varphi)}\) of \(\mathcal L_{\varphi,H}\)
    is simple and isolated, then there exist \(\eta>0\) and positive
...
For part~(ii), write \(\psi:=\varphi^{(\ell)}\).  Walters'
\(g\)-measure formalism for normalized H\"older potentials
\cite{Walters1975GMeasures} gives a H\"older backward specification
operator \(\mathcal K_\psi\) on the left-history space
\((Y^{(\ell)})^{-}\).  Because \(H^{(\ell)}\) is one-step, killing
depends only on the current block state, so restricting to safe states
defines \(\mathcal K_{\psi,H^{(\ell)}}\).  Let \(p_r^-(\xi^-)\) denote
the conditional probability, given the left history
\(\xi^-\in(Y^{(\ell)})^{-}\), that the corresponding forward orbit first
enters \(H^{(\ell)}\) in the residue class \(R-r\).  The conditional
probability of any fixed future cylinder is H\"older in \(\xi^-\), and
the first-hit decomposition of the event is a countable disjoint union
of such cylinders.  The standard cylinder expansion for the normalized
\(g\)-measure kernel therefore expresses each \(p_r^-\) as a uniformly
convergent H\"older series, so \(p_r^-\in
\mathcal H_\alpha((Y^{(\ell)})^{-})\).  Hence
\(g_r^-:=\min\{p_r^-,1-p_r^-\}\) is also H\"older.  Disintegrating the
exact cylinder formula for Bayes error over past histories yields
\[
  \varepsilon_n^{(\ell)}
  (\mathsf P_R^{(\ell)};\nu_\psi)
  =
...
\textbf{Note on parts~(iii)--(v).}\quad
The resolvent and spectral-gap arguments in parts~(iii)--(v) follow the
same formal structure as the finite-state case, but their rigorous
justification requires:
\begin{enumerate}[label=(\alph*)]
\item verification that \(\mathcal K_{\psi,H^{(\ell)}}\) acts as a
  bounded operator on \(\mathcal H_\alpha((Y^{(\ell)})^{-})\) with a
  spectral gap (this is expected by analogy with the Ruelle--Perron--Frobenius
  theorem for open operators, but requires checking the specific Banach
  space setup);
\item identification of the isolated spectrum of the backward open
  operator with that of the forward open Ruelle operator via the
  natural-extension pairing;
\item H\"older regularity of the residue-hit probability functions
  \(g_r^-\).
\end{enumerate}
We outline the strategy assuming these hold.
...
lift of \(\spec(\mathcal K_{\psi,H^{(\ell)}})\).

For part~(iv), the backward operator on past histories and the forward
open Ruelle operator on future histories are transposes of each other
with respect to the equilibrium-state pairing on the natural extension,
\[
  \langle f^-,f^+\rangle
  := \int f^-\,f^+\,\dd\widehat\nu_\psi;
\]
see \citet{Bowen2008,Ruelle1978,Walters1975GMeasures}.  Transpose
operators have the same nonzero spectrum, so the isolated spectra
coincide.  Block recoding does not change the open pressure or the
isolated resonance set, and part~(i) transfers the pole statement back
to the original error generating function.
...
representation in part~(ii).  Because the residue non-degeneracy
hypothesis implies \(g_r^->0\) on the safe-history space for every
residue class \(r\), and because the leading spectral projector
\(\Pi_H^-\) is given by a positive eigenfunction-eigenmeasure pair, the
coefficients
\[
  d_r:=\Lambda^-(\Pi_H^- g_r^-)
\]
are strictly positive.  Therefore
\[
  \varepsilon_n^{(\ell)}
  (\mathsf P_R^{(\ell)};\nu_\psi)
...
This is Theorem~\ref{thm:error-resolvent-cyclotomic}.
\end{proof}
