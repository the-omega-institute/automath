...
same normalized Gibbs state on the natural extension
\(\widehat Y^{(\ell)}\), which is a bijective system.  The key
observation is that the backward specification operator
\(\mathcal K_{\psi,H^{(\ell)}}\) acting on H\"older functions of the
past and the forward open Ruelle operator
\(\mathcal L_{\psi,H^{(\ell)}}\) acting on H\"older functions of the
future are transposes of each other with respect to the equilibrium
state pairing
\(\langle f^-,f^+\rangle
:=\int f^-\cdot f^+\,\dd\widehat\nu_\psi\).
Since transpose operators share the same nonzero spectrum (including
multiplicities), the isolated spectra coincide; see
\citet{Bowen2008,Ruelle1978,Walters1975GMeasures} for the standard
duality on the natural extension.  Block recoding does
