...
defines \(\mathcal K_{\psi,H^{(\ell)}}\).  If \(p_r^-\) denotes the
conditional probability that the corresponding forward orbit first
enters \(H^{(\ell)}\) in the residue class \(R-r\), then \(p_r^-\) is
H\"older, hence so is
\(g_r^-:=\min\{p_r^-,1-p_r^-\}\).  Disintegrating the exact cylinder
formula for Bayes error over past histories yields
\[
  \varepsilon_n^{(\ell)}
  (\mathsf P_R^{(\ell)};\nu_\psi)
  =
...
representation in part~(ii).  This gives
\[
  \varepsilon_n^{(\ell)}
  (\mathsf P_R^{(\ell)};\nu_\psi)
