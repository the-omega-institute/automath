...
- [ ] **Page-limit rule**: MEASURED -- `main.log` reports a 17-page XeTeX output, and `main.aux` records `\@abspage@last{17}`. The Stochastics and Dynamics page limit was not verified from the available paper files.
...
- [x] **Bibliography accounting**: PASS -- `main.tex` embeds 12 `\bibitem` entries and has no `\bibliography` command. The manuscript contains 12 distinct cited keys and 0 uncited entries that still print in `thebibliography`; `references.bib` is not part of the active build.
...
10 measured checks pass. 2 items require an external journal-rule check: abstract length and page limit.
