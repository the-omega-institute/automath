...
\begin{lemma}[Graph-theoretic sufficient condition for residue non-degeneracy]
\label{lem:residue-nondegen-sufficient}
Under the hypotheses of
Theorem~\ref{thm:first-entry-escape-rate}, the residue non-degeneracy
condition holds whenever the following graph-theoretic condition is
satisfied: for every safe state \(i\in\mathcal S\), there exist
admissible paths from \(i\) to the hole \(A_{\mathrm{hole}}\) of two
different lengths whose difference is not divisible by \(q\).
\end{lemma}

\begin{proof}
Fix \(i\in\mathcal S\) and \(r\in\ZZ/q\ZZ\).  Let \(d_1<d_2\) be two
path lengths from \(i\) to the hole with \(d_2-d_1\not\equiv 0\pmod q\).
Then the first entry times along these two paths yield residue classes
\(r+d_1\bmod q\) and \(r+d_2\bmod q\), which are distinct.  If
\(R\subsetneq \ZZ/q\ZZ\) is non-empty, at least one of these residues
falls in \(R-r\) and at least one falls in \((\ZZ/q\ZZ\setminus R)-r\),
or the pair of residues witnesses both possibilities.  Since
\(\card{R}\geq 1\) and \(\card{\ZZ/q\ZZ\setminus R}\geq 1\), and the
two residues are distinct, at least one path lands in \(R-r\) and the
other in its complement, or vice versa.  In fact, if both landed in the
same set, then by varying over all \(q\) translates of \(R\), the two
distinct residues would need to always be on the same side, which is
impossible for every non-empty proper \(R\).  Hence for any fixed \(R\),
both residue classes are witnessed.
\end{proof}

\begin{example}[Non-degeneracy holds: golden-mean shift with symbol hole]
\label{ex:nondegen-holds}
Let \(Y\) be the full two-shift \(\{0,1\}^{\NNzero}\) and
\(H=[11]\).  From safe state \(0\), one can reach the hole in \(2\) steps
(via \(01\to 1\to 1\)) or \(3\) steps (via \(0\to 0\to 1\to 1\)).  Since
\(3-2=1\) is not divisible by \(q=2\), the condition of
Lemma~\ref{lem:residue-nondegen-sufficient} holds.  The same is true
from state \(1\) (one can reach the hole in \(1\) or \(2\) steps).
\end{example}

\begin{example}[Non-degeneracy fails: periodic hole alignment]
\label{ex:nondegen-fails}
Let \(Y=\{0,1,2\}^{\NNzero}\) with full adjacency, and set
\(A_{\mathrm{hole}}=\{2\}\) and \(q=2\).  If the only admissible
transitions into state \(2\) are from state \(0\), and if from both safe
states \(0\) and \(1\) every admissible path to the hole has even
length, then residue non-degeneracy fails: from any safe state and any
residue \(r\), the first entry time modulo \(2\) is always \(r\bmod 2\),
so \(\beta_{i,r}\in\{0,1/2\}\) and the lower bound \(\theta>0\) cannot
hold for all \(r\).  In this degenerate case the Bayes error can vanish
on certain residue subsequences, and the amplitude formula requires
modification.
\end{example}
