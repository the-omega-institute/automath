...
\begin{theorem}[Quotient functoriality]
  ...
  \end{equation}
\leanverified{paper\_etds\_quotient\_functoriality}
\end{theorem}
...
Define the abelian shadow and the genuine non-abelian defect by
...
one-dimensional sector and hence detect exactly the abelian shadow.
...
\begin{theorem}[Exact boundary of cyclic detection]
...
\begin{proof}
  ...
  \[
    ...
    =\mathbf 1_{\{a\}}(a^j).
  \]
  Without conjugation, the same calculation with \(\psi_k(a)=\omega_3^k\) gives
  \(\frac{1}{3}\sum_k\omega_3^{k(j+1)}=\mathbf 1_{\{j\equiv -1\bmod 3\}}=\mathbf 1_{\{a^{-1}\}}(a^j)\).
\end{proof}
