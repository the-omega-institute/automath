...
We record two further consequences of the preceding analysis:
functoriality with respect to quotient groups and an explicit
\(S_3\)-example. This section closes the circle with
\Cref{sec:finite-part}: quotient functoriality identifies exactly which
part of the primitive class profile is visible in cyclic data, while
the \(S_3\)-example shows that a genuinely non-abelian primitive
contribution can remain after all cyclic shadows are accounted for. The
orthogonal decomposition used for cyclic detection is deferred to the
appendix.
...
the exact cyclic-detection boundary are given in the appendix.
...
\begin{corollary}[Primitive profile for the \(S_3\)-example]
  ...
  \end{equation}
  so \(\mathcal B_A^{\mathrm{na}}\) dominates
  \(\mathcal B_A^{\mathrm{ab}}\) in \(L^2\).
\end{corollary}
