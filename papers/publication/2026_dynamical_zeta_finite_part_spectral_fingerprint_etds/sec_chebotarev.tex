...
twisted determinant data into primitive Peter--Weyl series.
...
\begin{proof}
  ...
  \(\sigma\) (since every eigenvalue of \(A_\sigma\) is an eigenvalue of
  the extension matrix \(\tilde A\) by \Cref{prop:peter-weyl-determinant},
  and \(\rho(\tilde A)=\lambda\)). Since \(n/m\le n/2\) for \(m>1\),
  \[
...
\begin{remark}[On the twisted spectral-gap hypothesis]
  ...
  the Perron eigenvalue from the rest of the spectrum. For related
  sufficient criteria in finite-group extensions of shifts of finite
  type, compare the Chebotarev-type framework of
  \cite{ParryPollicott1986Chebotarev} and the structural results of
