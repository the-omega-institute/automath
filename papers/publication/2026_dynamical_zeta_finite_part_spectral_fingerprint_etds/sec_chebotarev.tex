...
twisted-\(L\)-function and Chebotarev framework. They are used here as
input in exactly the form needed for primitive orbit calculations.

What is not supplied by the classical periodic theory is a primitive
irreducible channel extracted directly from the twisted determinant
logarithms. Bare M\"obius inversion of a single logarithm
\(\log\det(I-zA_\varrho)^{-1}\) leaves the higher-power terms
\(\chi_\varrho(g^r)\) coming from repeated orbit labels. This is the
obstruction isolated in \Cref{prop:naive-mobius-contamination}. The new
ingredient is therefore \Cref{cor:class-function-adams-mobius}: the
Adams operations \(\psi^m\) on class functions remove the higher-power
contamination and produce the primitive determinant formulas of
...
of \Cref{thm:class-mertens-explicit,thm:class-mertens-fourier} become
conditional corollaries under the explicit hypothesis \(\eta<\lambda\).
Accordingly, the novelty of the present section lies in the primitive
Adams--M\"obius calculus and its determinant packaging, whereas the
periodic determinant identities and the leading Chebotarev asymptotics
are treated throughout as established background.
