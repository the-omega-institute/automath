...
\begin{proof}
  By \Cref{lem:class-primitive-trivial-split},
  \[
    p_{n,C}
    =
    \frac{|C|}{|G|}p_n(A)
  ...
  non-trivial \(\varrho\). Summing the identity
  ...
  \]
  Since \(\Lambda_*/\lambda\ge \vartheta\) with
  \(\vartheta=\max\{\eta/\lambda,\lambda^{-1/2}\}\), the tail estimate
  from \Cref{lem:geometric-harmonic-tail} implies
  \[
    \sum_{n>N}\pi_{n,\varrho}(A)\lambda^{-n}
    =O\!\left(\frac{(\Lambda_*/\lambda)^N}{N}\right).
  \]
  Likewise \Cref{lem:primitive-orbit-asymptotic} gives
  \[
    \sum_{n\le N}\frac{p_n(A)}{\lambda^n}
    =
...
Peter--Weyl channels at the Perron point.
