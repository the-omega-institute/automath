...
We now pass from the cyclic model to general finite-group extensions.
For a cyclic extension, the relevant channels are one-dimensional and
\Cref{sec:finite-part} reduces the problem to Fourier analysis on roots
of unity. For a general finite group, the same passage from
determinant data to primitive data takes place in the space of class
functions, that is, along the full Peter--Weyl decomposition. At the
periodic orbit-counting level, the Chebotarev theorem for finite
homogeneous extensions of shifts is due to Noorani--Parry
...
What is needed here is a primitive refinement of those periodic
determinant identities. The main point of the section is that, after
composing M\"obius inversion with the Adams operations \(g\mapsto g^m\),
the twisted determinant logarithms recover the primitive Peter--Weyl
series directly. We use the standard representation-ring formalism of
Serre \cite{Serre1977LinearRepresentations} and remain throughout in
the finite-dimensional twisted-matrix setting.
