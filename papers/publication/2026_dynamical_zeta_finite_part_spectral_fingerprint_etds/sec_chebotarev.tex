...
We now turn to finite-group extensions. For cyclic extensions the
relevant channels are one-dimensional, and the discussion of
\Cref{sec:finite-part} amounts to Fourier analysis on roots of unity.
For a general finite group \(G\), the same passage from periodic data
to primitive data takes place in the space of class functions, that is,
along the full Peter--Weyl decomposition. At the periodic level, the
Chebotarev theory for shifts of finite type goes back to
Parry--Pollicott \cite{ParryPollicott1986Chebotarev} and
Noorani--Parry \cite{NooraniParry1992ChebotarevShifts}; see also
\cite{PollicottSharp2007Chebotarev,BoyleSchmieding2017FiniteGroupExtensions,Parry1999LivsicNonAbelian}.
Our aim here is to obtain the corresponding primitive formulas.
...
\begin{remark}[Historical note]
  ...
  The novelty of \Cref{thm:class-mertens-explicit} is therefore not the
  leading density itself but the identification of the constant term in
  ...
  classwise constants. The graph-covering refinements of
  Hasegawa--Saito--Sato
  \cite{HasegawaSaitoSato2015AbelianCovering,HasegawaSaitoSato2018GaloisCovering}
  treat Mertens constants for Ihara and Artin--Ihara zeta functions of
  abelian and Galois coverings. \Cref{thm:class-mertens-secondary-term}
  adds a finite-state refinement in the present symbolic setting: above
  the square-root barrier, the first subleading term of the primitive
  class counts and of the harmonic remainder is governed by the
  spectral edge of the twisted Peter--Weyl channels.
\end{remark}

\begin{theorem}[Classwise Mertens asymptotics]
