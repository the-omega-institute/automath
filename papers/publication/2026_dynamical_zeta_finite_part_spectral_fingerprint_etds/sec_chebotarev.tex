...
The root-of-unity formulas of \Cref{sec:finite-part} provide the
abelian model for the theory developed here. For finite-group
extensions, Fourier analysis on roots of unity is replaced by the
Peter--Weyl decomposition of class functions. The key point is that
primitive extraction is governed by M\"obius inversion together with the
Adams operations \(g\mapsto g^m\), and this turns the twisted
determinant logarithms into primitive Peter--Weyl series. At the
orbit-counting level, the Chebotarev theorem for finite homogeneous
extensions of shifts is due to Noorani--Parry
\cite{NooraniParry1992ChebotarevShifts}, building on the Axiom~A
framework of Parry--Pollicott \cite{ParryPollicott1986Chebotarev}; see
also \cite{PollicottSharp2007Chebotarev,BoyleSchmieding2017FiniteGroupExtensions,Parry1999LivsicNonAbelian}.
For the character-ring background and Adams operations we use the
standard representation-ring formalism of Serre
...
The next corollary is the key step: it passes from periodic determinant
data to primitive orbit data.

\begin{proposition}[Standard class-function determinant linearisation]
...
\begin{proposition}[Failure of direct trace-level M\"obius inversion]
...
\subsection{Twisted \texorpdfstring{$L$}{L}-functions and primitive inversion}
\label{subsec:dictionary-twisted-L}

The formalism of this section is closely related to the twisted
\(L\)-function and twisted Perron--Frobenius framework of
Adachi--Sunada and Sunada, the Chebotarev-type orbit counting of
Parry--Pollicott, and the graph-covering zeta theory of
Kotani--Sunada and Stark--Terras
...
The determinant factorisation of
\Cref{prop:kernel-peter-weyl-block-diagonalisation}, the
class-function linearisation of
\Cref{thm:class-function-linearisation}, and the leading asymptotic
\(p_{n,C}\sim(|C|/|G|)\lambda^n/n\) all belong to this classical
twisted-\(L\)-function and Chebotarev framework.

For primitive orbit questions, however, one needs a formula for an
irreducible primitive channel directly in terms of the twisted
determinant logarithms. A direct M\"obius transform of a single
\(\log\det(I-zA_\varrho)^{-1}\) does not suffice, because it retains
the terms \(\chi_\varrho(g^r)\) arising from repeated orbit labels; see
\Cref{prop:naive-mobius-contamination}. The Adams operations remove
this obstruction and lead to the determinant and trace formulas of
\Cref{cor:primitive-peter-weyl-determinant,cor:primitive-peter-weyl-trace}.

Once the primitive channels are available, the Perron-point statements
of \Cref{thm:class-mertens-explicit,thm:class-mertens-fourier} follow
by evaluating them at \(z=\lambda^{-1}\) under the hypothesis
\(\eta<\lambda\).
...
\begin{remark}[Abelian model: cyclic lifts]
...
\]
Its role is to justify evaluation at the Perron point in the
non-trivial Peter--Weyl channels. The next five lemmas prepare the
proof of the classwise Mertens theorem.
...
\begin{remark}[Historical note]
  ...
  \Cref{thm:class-mertens-explicit} identifies the constant term in
  terms of the primitive Peter--Weyl channels
  \(\Pi_\varrho(\lambda^{-1})\), and
  \Cref{thm:class-mertens-fourier} reconstructs these values from the
  classwise constants.
\end{remark}

\begin{theorem}[Classwise Mertens asymptotics under a twisted spectral gap]
...
\begin{proposition}[Perron-point evaluation of the primitive channels]
...
\subsection{On the hypothesis \texorpdfstring{$\eta<\lambda$}{eta<lambda}}
...
\begin{theorem}[Twisted Perron rigidity and line fields]
  \textbf{[New-3 | 2026-04-17]}
...
\begin{remark}
  Canonical cyclic lifts provide an explicit equality case:
  \Cref{rem:abelian-toy-model-cyclic} shows that
  \(A_{\chi_j}=\omega_q^jA\), so every non-trivial cyclic character has
  spectral radius \(\lambda\). Thus the sentence ``strict Perron
  dominance holds by primitivity'' is false in this setting. Thus
  \(\eta<\lambda\) is not automatic, even
...
\begin{remark}[A \texorpdfstring{$C_2$}{C2}-example with \texorpdfstring{$\eta<\lambda$}{eta<lambda}]
  \label{rem:c2-example-explicit-gap}
  Let \(G=C_2=\{e,s\}\), let
  \[
    A=\begin{pmatrix}
      1 & 1 \\
      1 & 1
    \end{pmatrix},
    \qquad \lambda=\rho(A)=2,
  \]
  and label the four edges of the full two-shift by
  \[
    \tau(0\to 0)=e,\qquad \tau(0\to 1)=s,\qquad
    \tau(1\to 0)=e,\qquad \tau(1\to 1)=e.
  \]
  If \(\varepsilon\) denotes the non-trivial character of \(C_2\), then
  \[
    A_\varepsilon
    =
    \begin{pmatrix}
    ...
    \end{pmatrix},
    \qquad
    \det(I-zA_\varepsilon)=1-2z+2z^2.
  \]
  Hence \(\spec(A_\varepsilon)=\{1+i,1-i\}\), so
  \[
    \rho(A_\varepsilon)=\sqrt2<2=\lambda,
    \qquad
    \eta=\sqrt2.
  \]
  Thus strict inequality can also occur, even in an abelian extension.
  Moreover,
  \[
    \mathcal L_\varepsilon(z)=\log(1-2z+2z^2)^{-1},
  \]
  and, since \(\psi^m\varepsilon=\varepsilon\) for \(m\) odd and
  \(\psi^m\varepsilon=\mathbf 1\) for \(m\) even,
  \[
    \Pi_\varepsilon(z)
    =
    \sum_{\substack{m\ge 1\\ m\ \mathrm{odd}}}
      \frac{\mu(m)}{m}\log(1-2z^m+2z^{2m})^{-1}
    +
    \sum_{\substack{m\ge 1\\ m\ \mathrm{even}}}
      \frac{\mu(m)}{m}\log(1-2z^m)^{-1}.
  \]
  Consequently,
  \[
    \mathcal M_{\{e\}}(A)
    =\frac12\mathcal M(A)+\frac12\Pi_\varepsilon(1/2),
    \qquad
    \mathcal M_{\{s\}}(A)
    =\frac12\mathcal M(A)-\frac12\Pi_\varepsilon(1/2).
  \]
  This provides a concrete strict case in which the Perron-point formulas of
  \Cref{thm:class-mertens-explicit,thm:class-mertens-fourier} apply
  with an explicitly checked twisted gap.
\end{remark}
...
\begin{theorem}[Fourier reconstruction of class Mertens constants]
