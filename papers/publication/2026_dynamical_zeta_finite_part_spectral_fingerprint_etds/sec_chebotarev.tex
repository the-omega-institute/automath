...
\begin{proposition}[Direct trace-level M\"obius inversion is weaker]
...
\subsection{Comparison with classical twisted \texorpdfstring{$L$}{L}-functions}
\label{subsec:dictionary-twisted-L}

The formalism of this section is closely related to the twisted
\(L\)-function and twisted Perron--Frobenius framework introduced by
Adachi--Sunada \cite{AdachiSunada1987TwistedPerronFrobenius} and
Sunada \cite{Sunada1989DynamicalLFunctions}, the Chebotarev-type
orbit counting of Parry--Pollicott
\cite{ParryPollicott1986Chebotarev}, and the graph-covering zeta
theory of Kotani--Sunada and Stark--Terras
\cite{KotaniSunada2000ZetaFiniteGraphs,%
StarkTerras1996ZetaGraphCoverings,%
StarkTerras2000ZetaGraphCoveringsII,%
TerrasStark2007ZetaGraphCoveringsIII}.
To locate the exact increment of the present paper, we provide a
theorem-by-theorem comparison.

\medskip
\noindent\textbf{Theorem-by-theorem comparison.}

\begin{enumerate}[label=(\roman*)]
\item \textbf{Peter--Weyl determinant factorisation
  (\Cref{prop:kernel-peter-weyl-block-diagonalisation}).}
  Known input gives the identity
  \[
    \det(I-z\widetilde A)
    =\prod_\varrho\det(I-zA_\varrho)^{\dim\varrho}.
  \]
  This is the finite-state specialisation of the factorisation established by
  Adachi--Sunada \cite[Theorem~1.1]{AdachiSunada1987TwistedPerronFrobenius}
  for the twisted Perron--Frobenius operator acting on sections of a
  flat bundle over a Riemannian manifold. In the SFT setting, the same
  factorisation appears in Parry--Pollicott
  \cite[\S3]{ParryPollicott1986Chebotarev} and, for finite homogeneous
  extensions, in Noorani--Parry
  \cite{NooraniParry1992ChebotarevShifts}. On the graph side, the
  corresponding covering-zeta factorizations are developed in
  \cite{KotaniSunada2000ZetaFiniteGraphs,%
  StarkTerras1996ZetaGraphCoverings,%
  StarkTerras2000ZetaGraphCoveringsII,%
  TerrasStark2007ZetaGraphCoveringsIII}. This part of our paper is
  therefore classical input.

\item \textbf{Class-function linearisation
  (\Cref{thm:class-function-linearisation}).}
  Known input also gives the identity
  \(\mathcal L_\phi(z)
  =\sum_\varrho\widehat\phi(\varrho)\log\det(I-zA_\varrho)^{-1}\).
  This is the logarithmic version of the Adachi--Sunada twisted
  \(L\)-function \cite[Theorem~2.3]{AdachiSunada1987TwistedPerronFrobenius}.
  In the notation of Sunada \cite{Sunada1989DynamicalLFunctions},
  this corresponds to the factorisation of the dynamical \(L\)-function
  \(L(z,\varrho)\) into a product over the spectrum of the twisted
  operator. The content here is classical; we record it only to fix
  notation for the primitive formulas that follow.

\item \textbf{Chebotarev equidistribution
  (\Cref{thm:class-mertens-explicit}, leading term).}
  Known input further gives the asymptotic
  \(p_{n,C}\sim(|C|/|G|)\lambda^n/n\). This is the SFT version of the
  Chebotarev density theorem for Axiom~A systems proved by
  Parry--Pollicott \cite[Theorem~1]{ParryPollicott1986Chebotarev}
  and carried out for finite homogeneous SFT extensions by
  Noorani--Parry \cite[Theorem~1]{NooraniParry1992ChebotarevShifts}.
  The leading asymptotic in \Cref{thm:class-mertens-explicit} is
  therefore known.

\item \textbf{Adams--M\"obius primitive inversion
  (\Cref{cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant}).}
  The missing step begins here. The Adams operation
  \(\psi^m\) on class functions converts the \(m\)-fold repetition
  of an orbit into evaluation of the class function at the \(m\)-th
  power. Standard periodic determinant factorisation plus direct
  trace-level M\"obius inversion yields only the contaminated transform
  \(\mathcal N_\varrho\) of
  \Cref{prop:naive-mobius-contamination}, not the primitive channel
  \(\Pi_\varrho\): higher powers \(\chi_\varrho(g^r)\) for \(r\ge 2\)
  remain in the coefficients. The Adams--M\"obius formula
  \(\Pi_\phi(z)=\sum_{m\ge 1}(\mu(m)/m)\mathcal L_{\psi^m\phi}(z^m)\)
  is precisely the correction that removes this contamination. It
  operates on the character-ring level and thereby linearises the
  primitive inversion into a sum of twisted determinant logarithms.
  This formula does not appear in
  \cite{AdachiSunada1987TwistedPerronFrobenius,%
  Sunada1989DynamicalLFunctions,%
  ParryPollicott1986Chebotarev,%
  NooraniParry1992ChebotarevShifts,%
  StarkTerras1996ZetaGraphCoverings,%
  StarkTerras2000ZetaGraphCoveringsII,%
  TerrasStark2007ZetaGraphCoveringsIII}.

\item \textbf{Class Mertens constants and Fourier reconstruction
  (\Cref{thm:class-mertens-explicit} constant term,
  \Cref{thm:class-mertens-fourier}).}
  Our contribution at the Perron point is then the constant-term
  packaging
  \(\mathcal M_C(A)\) and the recovery of the non-trivial twisted
  primitive values \(\Pi_\varrho(\lambda^{-1})\) by non-abelian Fourier
  inversion. The leading asymptotic remains classical; what is new is
  the explicit determinant/class-function packaging of the constant term
  under the extra hypothesis \(\eta<\lambda\).
\end{enumerate}
...
The remaining results of this section are conditional on the explicit
twisted spectral-gap hypothesis
\[
  \eta:=\max_{\varrho\neq\mathbf 1}\rho(A_\varrho)<\lambda.
\]
Its role is only to permit evaluation at the Perron point in the
non-trivial Peter--Weyl channels. The present paper does not derive this
strict inequality from separate dynamical criteria; the results below
should therefore be read as a conditional Perron-point package built on
the primitive inversion formulas proved above.

The conditional class Mertens theorem is assembled from the next five
lemmas: class decomposition, the non-trivial channel estimate, absolute
...
\begin{remark}[Known input versus new consequences]
  ...
  The contribution of \Cref{thm:class-mertens-explicit} beyond the
  leading term is the explicit class Mertens constant-term packaging
  \(\mathcal M_C(A)\) in terms of the primitive Peter--Weyl channels
  \(\Pi_\varrho(\lambda^{-1})\), and the subsequent Fourier
  reconstruction in \Cref{thm:class-mertens-fourier}. Both results
  depend on the twisted spectral-gap hypothesis \(\eta<\lambda\).
\end{remark}

\begin{theorem}[Conditional class Mertens theorem under twisted spectral gap]
...
\begin{proposition}[Class-constant deviations and Perron-point
...
\begin{proof}
  ...
  only non-trivial \(\sigma\), while for \(m\ge 2\) the point
  \(\lambda^{-m}\) lies strictly inside the Perron disc. Therefore
...
\subsection{When can one expect \texorpdfstring{$\eta<\lambda$}{eta<lambda}?}
\label{subsec:eta-gap}

\begin{remark}[Role of the hypothesis]
  ...
  The hypothesis therefore marks the boundary between formal primitive
  inversion and Perron-point asymptotics.
\end{remark}

\begin{proposition}[One-dimensional coboundaries force equality]
  \label{prop:eta-gap-coboundary-obstruction}
  Let \(\chi:G\to\TT\) be a one-dimensional character. Assume that there
  exists a function \(b:V\to\TT\) such that for every admissible edge
  \(e:i\to j\),
  \[
    \chi(\tau(e))=b(i)^{-1}b(j).
  \]
  ...
  \[
    A_\chi=D^{-1}AD.
  \]
  In particular \(\rho(A_\chi)=\rho(A)=\lambda\).
\end{proposition}

\begin{proof}
  For each admissible pair \(i\to j\), the cocycle condition gives
  \[
    (A_\chi)_{ij}
    =
    ...
    =
    b(i)^{-1}
      \left(\sum_{e:i\to j}1\right)
      b(j)
    =
    b(i)^{-1}A_{ij}b(j).
  \]
  Thus \(A_\chi=D^{-1}AD\), so \(A_\chi\) is similar to \(A\) and has
  the same spectral radius.
\end{proof}

\begin{remark}[Equality and strict inequality both occur]
  ...
  dominance holds by primitivity'' is false in this setting. The
  strict inequality \(\eta<\lambda\) is therefore not automatic, even
  for finite abelian extensions.
\end{remark}

\begin{remark}[Broader dynamical background]
  The equality mechanism of
  \Cref{prop:eta-gap-coboundary-obstruction} is the finite-state analogue
  of the cohomological obstructions studied by Parry
  ...
  We cite these works as structural background: the present paper does
  not derive \(\eta<\lambda\) from their hypotheses.
\end{remark}

\begin{remark}[A \texorpdfstring{$C_2$}{C2}-extension with explicit twisted gap]
...
\subsection{Conditional non-abelian Fourier reconstruction}
...
Peter--Weyl channels at the Perron point, so the reconstruction theorem
below starts from an explicitly defined class function.

\begin{theorem}[Conditional non-abelian Fourier reconstruction of class
  Mertens constants]
