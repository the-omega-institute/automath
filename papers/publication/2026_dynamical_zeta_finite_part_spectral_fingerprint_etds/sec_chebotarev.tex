...
We now turn to finite-group extensions. This section is the
non-abelian continuation of \Cref{sec:finite-part}: for cyclic
extensions the relevant channels are one-dimensional and the earlier
discussion reduces to Fourier analysis on roots of unity, whereas for a
general finite group \(G\) the same passage from periodic data to
primitive data takes place along the full Peter--Weyl decomposition.
At the periodic level, the Chebotarev theory for shifts of finite type
...
Our aim is not to reprove those periodic factorisations or the leading
density theorem, but to pass from them to primitive orbit data.

The essential point is that direct M\"obius inversion on a single
twisted logarithm does not remove the repetitions of the group labels.
The missing ingredient is the Adams operation \(g\mapsto g^m\) on class
functions. Once this is inserted, the twisted determinant logarithms
recover the primitive irreducible channels directly. Throughout we use
the standard representation-ring formalism of Serre
...
The next proposition gives the passage from periodic determinant data to
primitive orbit data.
...
The formalism developed above belongs to the classical twisted
