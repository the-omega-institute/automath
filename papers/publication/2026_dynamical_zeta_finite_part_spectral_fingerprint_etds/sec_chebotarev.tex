...
From this point onward we impose the explicit twisted spectral-gap
hypothesis
\[
...
strict inequality from separate dynamical criteria.

The class Mertens theorem is assembled from the next five lemmas: class
decomposition, the non-trivial channel estimate, absolute convergence at
the Perron point, a geometric-harmonic tail bound, and the trivial
Perron-channel contribution.
...
\begin{theorem}[Explicit class Mertens theorem under twisted spectral gap]
...
\begin{proof}
  ...
  For the Parseval identity, multiply \eqref{eq:class-mertens-fourier-expansion}
  by \(|G|/|C|\), take the modulus squared, and sum over conjugacy
  classes. Since
  \[
    |\Delta_C(A)|^2
    =
  ...
  \]
  Expanding, using \(\overline{\chi_\sigma(C^{-1})}=\chi_\sigma(C)\) for
  unitary representations, and applying the character orthogonality
  \(\sum_{C}\frac{|C|}{|G|}\chi_\varrho(C^{-1})\chi_\sigma(C)=\delta_{\varrho,\sigma}\)
