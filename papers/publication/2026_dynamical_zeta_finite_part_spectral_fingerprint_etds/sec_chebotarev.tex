...
\subsection{Dictionary with classical twisted \texorpdfstring{$L$}{L}-functions}
...
Sunada \cite{Sunada1989DynamicalLFunctions}. To help the reader locate
the exact increment, we translate the present notation into theirs.

\begin{enumerate}[label=(\roman*)]
\item The class-function logarithm \(\mathcal L_\phi(z)\) of
  \eqref{eq:class-logarithm-def} is the dynamical analogue of the
  logarithmic derivative of the Adachi--Sunada twisted
  \(L\)-function. For an irreducible character \(\chi_\varrho\), the
  identity
  \(\mathcal L_{\chi_\varrho}(z)=\log\det(I-zA_\varrho)^{-1}\)
  (\Cref{thm:class-function-linearisation}) is the
  Peter--Weyl determinant factorisation, which was already implicit in
  the Parry--Pollicott Chebotarev theory
  \cite{ParryPollicott1986Chebotarev}; the corresponding
  Noorani--Parry result for finite homogeneous SFT extensions is in
  \cite{NooraniParry1992ChebotarevShifts}.

\item The primitive series \(\Pi_\phi(z)\) of
  \eqref{eq:primitive-class-series-def} corresponds to passing from the
  periodic orbit sum to the prime orbit sum. In the classical
  \(L\)-function language this is the passage from
  \(\log L_\varrho(z)\) to the sum over prime orbits of
  \(\chi_\varrho\).

\item The Adams operation \(\psi^m\) on class functions
  (\eqref{eq:adams-class-function}) converts the \(m\)-fold repetition
  of an orbit into an evaluation of the class function at the \(m\)-th
  power, which is the standard Adams power in the representation ring
  \cite{Serre1977LinearRepresentations}. The new element of the
  present paper is the combination of this operation with M\"obius
  inversion to produce the formula
  \(\Pi_\phi(z)=\sum_{m\ge 1}(\mu(m)/m)\mathcal L_{\psi^m\phi}(z^m)\).
  In the existing literature the passage from periodic to primitive
  data uses direct M\"obius inversion on the trace level; the
  Adams--M\"obius formula operates instead on the character-ring level
  and thereby linearises the inversion into a sum of twisted
...
\begin{proof}
  ...
  classes. Since \(|\Delta_C(A)|^2=(|C|/|G|)^2|\sum_\varrho\chi_\varrho(C^{-1})\Pi_\varrho|^2\),
  one obtains
  \[
