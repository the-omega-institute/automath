...
\(\overline{\chi_\varrho(C)}\), equivalently \(\chi_\varrho(C^{-1})\);
we do not replace this by \(\chi_\varrho(C)\) unless the character is
real-valued.}
\[
  \operatorname{conj}\!\bigl(\chi_\varrho(\cdot)\bigr)
  :=\overline{\chi_\varrho(\cdot)}
  =\chi_\varrho((\cdot)^{-1}).
\]
...
\begin{proof}
  ...
  \]
  where the last equality uses the unitary identity
...
\begin{proof}
  ...
  \[
    ...
    \sum_{n\ge 1}\frac{\Tr(A^n)}{n}
      \bigl(-\log(1-|z|^n)\bigr).
  \]
  By \Cref{lem:primitive-orbit-asymptotic}, one has
  \(\Tr(A^n)=\lambda^n+O(\Lambda(A)^n)\), so there is a constant
  \(C_A>0\) such that \(\Tr(A^n)\le C_A\lambda^n\) for all \(n\ge 1\).
  Also,
  \[
  ...
  \begin{align*}
    ...
    \sum_{\gamma\in\cP}\sum_{s\ge 1}
      \phi(\tau(\gamma)^s)z^{s|\gamma|}
      \sum_{m\mid s}\frac{\mu(m)}{m}\frac{1}{s/m} \\
    &=
    \sum_{\gamma\in\cP}\sum_{s\ge 1}
...
\begin{lemma}[Non-trivial Peter--Weyl channel bound]
  ...
  For every non-trivial \(\varrho\in\Irr(G)\), define
  \[
    K_\varrho
    :=
    \sup_{n\ge 1}\frac{\abs{\Tr(A_\varrho^n)}}{\eta^n},
    \qquad
    K_0
    :=
    \max_{\sigma\in\Irr(G)}
      \sup_{\ell\ge 1}\frac{\abs{\Tr(A_\sigma^\ell)}}{\lambda^\ell}.
  \]
  These constants are finite. Moreover, for every \(n\ge 1\),
  \[
    \abs{\pi_{n,\varrho}(A,\tau)}
    \le
    \frac{K_\varrho\eta^n}{n}
    +\frac{K_0\,\dim\varrho\,\abs{\Irr(G)}}{n}
...
\begin{proof}
  Fix \(\varrho\neq\mathbf 1\). Every eigenvalue \(\mu\) of
  \(A_\varrho\) satisfies \(\abs{\mu}\le \rho(A_\varrho)\le \eta\) by
  \Cref{prop:twisted-spectral-radius-bound}, and
  \(\Tr(A_\varrho^n)\) is the sum of the \(n\)-th powers of these
  eigenvalues with algebraic multiplicity. Hence \(K_\varrho<\infty\).
  The same eigenvalue argument, together with
  \(\rho(A_\sigma)\le \lambda\) for every \(\sigma\in\Irr(G)\), gives
  \(K_0<\infty\).
  ...
  \(K_\varrho\eta^n/n\). For the divisor sum, use
  ...
  \[
    ...
    \le
    K_0\,\dim\varrho\,\abs{\Irr(G)}\,\lambda^{n/m}.
  \]
...
\begin{theorem}[Main Theorem II(a): class Mertens theorem under the
  ...
  \end{equation}
  Choose \(C_A>0\) such that
  \begin{equation}\label{eq:class-mertens-base-constant}
    \abs{p_n(A)-\lambda^n/n}
    \le
    \frac{C_A}{n}\max\{\Lambda(A)^n,\lambda^{n/2}\}
    \qquad (n\ge 1),
  \end{equation}
  which exists by \Cref{lem:primitive-orbit-asymptotic}. For each
  non-trivial \(\varrho\), let \(K_\varrho\) be as in
  \Cref{lem:nontrivial-peter-weyl-channel-bound}, and let \(K_0\) denote
  the same global constant from that lemma. Set
  \begin{equation}\label{eq:class-mertens-explicit-constant}
    C_{\mathrm{cls}}(A,\tau;G)
    :=
    C_A
    +\sum_{\varrho\neq\mathbf 1}\dim\varrho\,K_\varrho
    +\frac{\lambda K_0\,\abs{\Irr(G)}}{\lambda-1}
  ...
  class \(C\) and in \(n\). The adjective ``stated'' refers only to the
  finite-dimensional spectral-gap input \(\eta\) and \(\Lambda_*\).
\end{theorem}
...
\begin{proof}
  ...
  \[
    \frac{|C|}{|G|}
      \sum_{\varrho\neq\mathbf 1}\overline{\chi_\varrho(C)}
      \sum_{n\le N}\pi_{n,\varrho}(A,\tau)\lambda^{-n}.
  \]
  For each non-trivial \(\varrho\),
  \Cref{lem:nontrivial-peter-weyl-channel-bound} gives
  \[
    \pi_{n,\varrho}(A,\tau)\lambda^{-n}
    =O\!\left(\frac{\vartheta_*^n}{n}\right),
  \]
...
\begin{remark}[Comparison with classical Mertens and Chebotarev
  ...
  constant term \(\mathcal M_C(A,\tau;G)\) after the leading
  \(\frac{|C|}{|G|}\log N\) behaviour has already been separated.
\end{remark}
...
\begin{remark}[On the twisted spectral-gap hypothesis]
  ...
  the primitive Peter--Weyl series into class Mertens
  ...
  the hypothesis is kept as a separate stated assumption and marks the boundary between formal
  ...
  constants and the explicit classwise constant term
...
The constants \(\mathcal M_C(A,\tau;G)\) therefore encode more than the
leading
...
\begin{theorem}[Main Theorem II(b): class-constant readout]
  ...
  \[
    \operatorname{conj}\!\bigl(\chi_\varrho(C)\bigr)
    =\overline{\chi_\varrho(C)}
    =\chi_\varrho(C^{-1}).
  \]
\end{theorem}
...
\begin{proof}
  ...
  \[
    \sum_{C\subset G}\frac{|C|}{|G|}
      \operatorname{conj}\!\bigl(\chi_\varrho(C)\bigr)\,\chi_\sigma(C)
    =\delta_{\varrho,\sigma}
  \]
