...
root-of-unity formulas of \Cref{sec:finite-part} are the cyclic
one-dimensional shadow of the construction developed here, not a
separate source of the non-abelian result.
...
The periodic-data viewpoint for finite group actions on SFTs goes back
to Fiebig \cite{Fiebig1993PeriodicFiniteGroupActions}, while the
classification and flow-equivalence theory of \(G\)-SFTs is developed
in Boyle--Carlsen--Eilers
\cite{BoyleCarlsenEilers2020FlowEquivalenceGSFT}. Our use of the
finite group is complementary to those works: the action/cocycle is
fixed, and the problem is to pass from twisted determinant data to
primitive conjugacy-class data.
...
\begin{proof}
  Write a length-\(n\) closed edge path as
  \(e_0\cdots e_{n-1}\), with
  \(e_k:i_k\to i_{k+1}\) and \(i_n=i_0\). By the definition of the
  twisted matrix,
  \[
    (A_\varrho^n)_{i_0i_0}
    =
    \sum_{e_0\cdots e_{n-1}:\, i_n=i_0}
      \varrho(\tau(e_0))\cdots\varrho(\tau(e_{n-1})).
  \]
  Taking the trace over the representation space and summing over
  \(i_0\) gives
  \[
    \Tr(A_\varrho^n)
    =
  ...
  \]
  The trace on the representation space is the character
  \(\chi_\varrho\). Length-\(n\) closed edge paths are exactly
  \(n\)-periodic points of the edge shift with a distinguished starting
  time, and their group labels are \(\tau_n(x)\). This gives the stated
  identity.
\end{proof}
...
\begin{proof}
  ...
  \]
  This double series is absolutely convergent for
  \(|z|<\lambda^{-1}\), because \(\phi\) is bounded and
  \[
    \sum_{\gamma\in\cP}\sum_{r\ge 1}
      \frac{|z|^{r|\gamma|}}{r}
    =
  ...
  \[
    ...
    <\infty,
  \]
  so the \(m,r,\gamma\) sums below may be rearranged absolutely.
  Hence
  \begin{align*}
