...
\begin{array}{c}
  ...
  \to\text{class-count readout}.
\end{array}
\]
The next proposition records this assembly as a theorem-level interface,
so that the introduction, the Peter--Weyl bridge, and the later Mertens
section all refer to the same package rather than to an informal chain of
aliases.

\begin{proposition}[Main Theorem I assembly map]
  ...
  \begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
    ...
    \item \Cref{cor:primitive-peter-weyl-determinant,%
    thm:primitive-peter-weyl-trace} specialize the preceding identity to
    ...
    channel.
    \item \Cref{thm:class-count-trace-readout} applies finite character
  ...
  package consisting of the twisted matrices \(A_\pi\), the finite
...
\begin{proof}
  ...
  precisely the determinant, primitive-channel, and class-readout assertions
...
\begin{theorem}[Fixed-label Euler transform]
  ...
  \(z=\lambda^{-1}\) by the branch convention fixed at the beginning of
  this section, together with the local exclusion of the single scalar
  Perron boundary summand \((r,m,\pi)=(1,1,\mathbf1)\).  Equivalently,
  all later Perron-boundary determinant evaluations use the same
  normalized branch and the same scalar exclusion recorded explicitly in
  \Cref{prop:fixed-label-determinant-perron}.  That omitted summand
  would contain \(\log\det(I-\lambda^{-1}A_{\mathbf1})^{-1}\), and its
  coefficient is zero in the non-trivial Peter--Weyl summand.  Its later
  product-constant role, and the precise
  obstruction to substituting \(\mathcal L_\phi\), are isolated in
