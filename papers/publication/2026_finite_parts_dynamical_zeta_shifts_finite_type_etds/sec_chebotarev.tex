...
\begin{proof}
  ...
  By \Cref{lem:nontrivial-perron-point-convergence}, each
  \(\Pi_\varrho(\lambda^{-1})\) is absolutely convergent, and the proof of
  that lemma gives \(\Pi_\varrho(\lambda^{-r})=O(\lambda^{-r})\)
  uniformly for \(r\ge2\). Hence
  \(\sum_{r\ge1}\Pi_\phi(\lambda^{-r})/r\) converges absolutely. For the
  determinant expansion of the same boundary value, apply
...
\begin{proof}
  ...
  \(rm=1\) is excluded before any logarithm is evaluated. If
  \(rm=1\) and \(\pi\neq\mathbf1\), then \(r=m=1\), and the twisted gap
  gives \(\rho(A_\pi)<\lambda\). Hence
  \(I-t\lambda^{-1}A_\pi\) is invertible for \(0\le t\le1\), and the
  normalized logarithm continued from \(0\) remains bounded on this compact
  Abel path.

  It remains to bound the tail \(rm\ge2\). For every \(\pi\) and every
  \(k\ge2\), the spectral-radius bound \(\rho(A_\pi)\le\lambda\) implies
  \(\rho(\lambda^{-k}A_\pi)\le\lambda^{1-k}<1\). Because the set of
  twisted matrices is finite, choose \(D>0\) larger than all matrix
  sizes. Since the eigenvalues of \(A_\pi\) have modulus at most
  \(\lambda\), one has
  \(\abs{\Tr(A_\pi^\ell)}\le D\lambda^\ell\) for every \(\pi\) and
  \(\ell\ge1\). For
  \(0\le t\le1\), the determinant logarithm is given by its power series,
  and
  \[
    \begin{aligned}
    \abs{\log\det(I-t^k\lambda^{-k}A_\pi)^{-1}}
    &\le
    \sum_{\ell\ge1}\frac{t^{k\ell}\lambda^{-k\ell}}{\ell}
      \abs{\Tr(A_\pi^\ell)} \\
    &\le
    D\sum_{\ell\ge1}\frac{\lambda^{(1-k)\ell}}{\ell}
    \le C\lambda^{1-k},
    \end{aligned}
  \]
  where the final constant is uniform for all \(k\ge2\). Multiplying by
  \(\abs{b_{\phi,\pi}^{(m)}}/(rm)\), summing over the finite set
  \(\Irr(G)\), and putting \(k=rm\) gives the majorant in
  \eqref{eq:fixed-label-perron-double-majorant-bound}. Finally,
  \(\sum_{rm=k}1/(rm)=d(k)/k\), so
  \(\sum_{k\ge2}d(k)\lambda^{1-k}/k<\infty\). This proves the displayed
  dominated-convergence bound and the branch exclusion statement.
\end{proof}
