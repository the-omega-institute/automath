...
This section contains the main construction of the paper. For
finite-group extensions, the twisted matrices \(A_\varrho\) record
periodic data in Peter--Weyl channels. Passing from those periodic traces
to primitive orbit labels is the point at which the non-abelian
structure enters: a repeated primitive orbit with label \(g\) contributes
the powers \(g^m\), so M\"obius inversion must be combined with the
Adams operations on class functions. This Adams--M\"obius mechanism turns
twisted determinant data into primitive Peter--Weyl series. The
linear span of cyclic quotient observables is only the one-dimensional
shadow of this construction; the root-of-unity formulas of
\Cref{sec:finite-part} correspond to the special canonical cyclic-lift
model described in \Cref{rem:abelian-toy-model-cyclic}, not to arbitrary
cyclic quotients of arbitrary cocycles.

At the orbit-counting level, the Chebotarev theorem for finite
...
For group-action zeta functions in a broader orbit-counting setting see
Lind \cite{Lind1996ZetaZdActions}.
Finite group actions on SFTs were studied systematically by
Adler--Kitchens--Marcus
\cite{AdlerKitchensMarcus1985FiniteGroupActions}. The periodic-data
viewpoint most directly relevant here is Fiebig's work
\cite{Fiebig1993PeriodicFiniteGroupActions}: fixed subsystems, quotient
systems, and their zeta functions are organised as invariants of finite
actions. Silver--Williams
\cite{SilverWilliams2005InvariantFiniteGroupActions} further develop
reduced-shift invariants for finite group actions and the quotient-zeta
viewpoint. The classification and flow-equivalence theory of \(G\)-SFTs is
developed in Boyle--Carlsen--Eilers
\cite{BoyleCarlsenEilers2020FlowEquivalenceGSFT}. Our use of the finite
group is complementary to those works: the action/cocycle is fixed, and
the problem is to pass from twisted determinant data to primitive
conjugacy-class data. The new point is not the leading Chebotarev
density, but the Adams--M\"obius extraction of primitive Peter--Weyl
channels from periodic determinant data.
The determinant and \(L\)-function viewpoint is parallel to the
twisted Perron--Frobenius theory of Adachi--Sunada
\cite{AdachiSunada1987TwistedPF} and to the graph-covering
factorisations of Stark--Terras
\cite{StarkTerras1996FiniteGraphsCoverings,StarkTerras2000FiniteGraphsCoveringsII}.
...
The section is organised in two labelled main blocks. The first block,
...
is Main Theorem~I: it is the Adams--M\"obius determinant mechanism and
does not use a twisted spectral gap. The second block,
\Cref{thm:class-mertens-explicit,thm:class-mertens-fourier}, is Main
Theorem~II: it starts only after the gap
...
intervening class-decomposition
formulas are bridge statements between these two blocks.
Thus all genuinely finite-group primitive information is produced before
the auxiliary root-of-unity material of \Cref{sec:finite-part} is used.
The latter section only supplies the scalar normalisation and the
one-dimensional comparison case.

\subsection{Main Theorem I: Adams--M\"obius primitive determinant
linearisation}

The first main block has four steps. The twisted trace identity converts
...
terms of the determinant data. The preliminary class-indicator and
...
Together with the determinant linearisation and its two primitive
...
\begin{proof}
  ...
  directed graph underlying \(A\), a choice of \(i_0\) and of a closed
...
\begin{theorem}[Main Theorem I(a): class-function determinant
  ...
  \end{equation}
  \leanverified{paper_etds_class_function_linearisation}
\end{theorem}
...
\begin{corollary}[Main Theorem I(b): Adams--M\"obius primitive
  ...
  \end{equation}
  \leanverified{paper_etds_class_function_adams_mobius}
\end{corollary}
...
\begin{proof}
  ...
  \]
  The resulting \(m,r,\gamma\) sums may be rearranged absolutely, because
  \[
...
\begin{corollary}[Main Theorem I(c): irreducible-channel determinant
  ...
  \end{equation}
  \leanverified{paper_etds_primitive_peter_weyl_determinant}
\end{corollary}
...
rewrites it as twisted determinant data, and the two corollaries above
perform the primitive Adams--M\"obius extraction. Neither the twisted
spectral gap nor cyclic root-of-unity reconstruction enters this first
main package.
  ...
  \begin{equation}\label{eq:primitive-peter-weyl-trace}
    ...
    \frac{1}{n}\sum_{m\mid n}\mu(m)
      \sum_{\sigma\in\Irr(G)}
        ...
        \Tr(A_\sigma^{n/m}).
  \end{equation}
\leanverified{paper_etds_primitive_peter_weyl_trace}
\end{corollary}
...
\begin{proposition}[Conjugacy-class primitive decomposition]
  ...
  \end{equation}
\leanverified{paper_etds_class_primitive_decomposition}
\end{proposition}
      ...
      \sum_{\varrho\in\Irr(G)}
        ...
        \sum_{m\mid n}\mu(m)
          \sum_{\sigma\in\Irr(G)}
            ...
            \Tr(A_\sigma^{n/m}).
  \end{equation}
\leanverified{paper_etds_class_artin_mobius_trace}
\end{corollary}
...
\Cref{thm:class-mertens-fourier} are conditional on the displayed gap.
The one-layer condition \(q\ge d\) from \Cref{sec:finite-part} is not
used anywhere in this finite-group Perron-point argument.
...
readout of the constants by character orthogonality, not a second
analytic mechanism. The proof is assembled from the next five lemmas:
...
\end{proof}
\leanverified{paper_etds_class_primitive_trivial_split}
...
\begin{lemma}[Non-trivial Peter--Weyl channel bound]
  ...
  \]
\leanverified{paper_etds_nontrivial_peter_weyl_channel_bound}
\end{lemma}
...
\leanverified{paper_etds_nontrivial_perron_point_convergence}
\end{proof}
...
\begin{lemma}[Geometric-harmonic tails]
  ...
  \]
\leanverified{paper_etds_geometric_harmonic_tail}
\end{lemma}
...
\end{proof}
\leanverified{paper_etds_perron_channel_contribution}
...
\begin{theorem}[Main Theorem II(a): explicit class Mertens theorem under
  ...
  or \(N\), respectively.
\leanverified{paper_etds_class_mertens_explicit}
\end{theorem}
...
\begin{remark}[On the twisted spectral-gap hypothesis]
  ...
  the analysis of Parry and Liv\v{s}ic obstructions in
  ...
  primitive inversion and Perron-point asymptotics.
\end{remark}
...
\begin{theorem}[Main Theorem II(b): class-constant readout]
  ...
  \end{equation}
  \leanverified{paper_etds_class_mertens_fourier}
\end{theorem}
...
\begin{proof}
  ...
  the standard class orthogonality relation
  \[
    \sum_{C\subset G}\frac{|C|}{|G|}
      \chi_\varrho(C^{-1})\chi_\sigma(C)
    =\delta_{\varrho,\sigma}.
