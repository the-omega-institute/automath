...
\begin{remark}[Literature comparison for the three main theorems]
  \label{rem:main-theorem-literature-comparison}
  \label{rem:theorem-by-theorem-comparison-calibration}
  The proof inputs for Main Theorems~I--III are internal finite determinant,
  character-table, Adams--M\"obius, and Perron-boundary arguments.  The
  comparison references serve different roles.  Parry--Pollicott and
  Noorani--Parry supply the classical Chebotarev and Artin--Ruelle setting for
  finite symbolic extensions, Sharp supplies the dynamical Mertens scale, and
  Mesliza--Noorani supply the Frobenius-class product comparison; these sources
  account for density, exponent, and product-existence layers.  The contribution
  used here is the primitive fixed-label determinant constant and the obstruction
  to substituting \(\mathcal L_\varrho(\lambda^{-1})\) for
  \(F_\varrho(\lambda^{-1})\).  The recent comparison papers cited in the
  introduction calibrate nearby topics--geometric periodic-orbit invariants,
  Ruelle resonances, finite-data rigidity, and non-commutative skew products--but
  are not hypotheses in the theorem chain.
\end{remark}
