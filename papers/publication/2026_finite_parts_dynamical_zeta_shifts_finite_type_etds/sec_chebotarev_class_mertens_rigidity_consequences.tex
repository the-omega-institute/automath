...
\begin{remark}[Comparison with the literature for the main theorems]
  \label{rem:main-theorem-literature-comparison}
  \label{rem:theorem-by-theorem-comparison-calibration}
  The comparison references below locate the inherited symbolic-dynamical
  layers of the argument, while the finite-label determinant interfaces are the
  contributions used in the main theorem chain.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \(F_\varrho(\lambda^{-1})\).
    \item The eta-gap, shadow, and \(S_3\) rational-check results support
    the same determinant/product-coordinate picture.  The recent comparison
    papers on geometric periodic-orbit invariants, Ruelle resonances,
    finite-data rigidity, and non-commutative skew products are not hypotheses
    and are not claimed as consequences.
  \end{enumerate}
\end{remark}
