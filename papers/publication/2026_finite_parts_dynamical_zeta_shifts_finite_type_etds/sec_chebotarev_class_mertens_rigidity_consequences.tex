...
\begin{proposition}[Adams correction to Frobenius-class Mertens products]
  ...
  constant have been separated.  The leading exponent \(|C|/|G|\) and the
  existence of a Frobenius-class product constant are the classical layer
  calibrated by
  ...
  layer and the scalar base \(\gamma+\log C(A)\) are fixed, the constants are
  exactly the fixed-label values \(F_\varrho(\lambda^{-1})\), these values are
  recovered from finite twisted determinants by Adams--M\"obius inversion, and
  every proposed replacement is measured by the zero-mean Peter--Weyl error
  vector below.  The substitution test is downstream and is not a hypothesis
