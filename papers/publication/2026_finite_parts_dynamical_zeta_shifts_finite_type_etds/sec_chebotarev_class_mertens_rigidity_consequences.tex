...
\begin{theorem}[Rank-sharp fixed-coordinate obstruction skeleton]
  ...
  theorem records the fixed-coordinate obstruction skeleton for recovering the
  scalar-normalised class-product vector inside the branch-compatible
  determinant-coordinate chart used in Main Theorem~III and in the
  Adams-obstruction theorem.  It compares already-proved scalar-normalised
  product constants after the finite determinant coordinates and their branches
  have been fixed.  Let
...
\begin{proposition}[Theorem-by-theorem attribution]
  ...
  normalisation, or comparison context, while the finite-state determinant
  formulae are proved inside the present paper.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \cite{MeslizaNoorani1999FrobeniusMertens}, for product-existence and
  ...
  formula, and Artin-substitution obstruction, with the external results
  providing the surrounding Chebotarev--Mertens and modern comparison context.
\end{proposition}
...
\begin{proof}
  ...
  contain no geometric orbit-invariant input, Banach/Ruelle resonance
  hypotheses, finite-data rigidity assumptions, or operator-algebraic
  skew-product classifications.  The comparison papers listed in item
  \textup{(iv)} therefore serve as context for neighbouring directions.  The
  four displayed items exhaust the labelled main theorem chain described
  immediately before the proposition, so the attribution statement follows.
\end{proof}
