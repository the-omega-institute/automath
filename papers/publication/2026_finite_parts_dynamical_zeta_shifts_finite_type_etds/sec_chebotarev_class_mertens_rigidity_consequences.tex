...
\begin{proposition}[Adams correction to Frobenius-class Mertens products]
  ...
  Equivalently, Main Theorem~III first uses the classical Frobenius product
  layer only for the exponent and existence of \(K_C\); it then computes the
  remaining constant from the fixed-label determinant coordinates
  \(F_\varrho(\lambda^{-1})\); and only afterwards tests proposed substitutes
  such as \(\mathcal L_\varrho(\lambda^{-1})\) by the zero-mean obstruction
  vector and Parseval identity.  The substitution test is downstream and is not
  a hypothesis for Main Theorems~I--III.
