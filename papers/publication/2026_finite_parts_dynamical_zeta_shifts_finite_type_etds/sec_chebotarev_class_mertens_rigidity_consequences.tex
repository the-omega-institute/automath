...
\begin{corollary}[Finite-rank obstruction from partial class data]
...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\subsubsection{Finite determinant formula and classical comparison}
...
\Cref{thm:eta-gap-rigidity,cor:eta-gap-boundary-classifier,prop:eta-gap-finite-state-certificate}, and the \(S_3\) finite verifications
...
\begin{remark}[Comparison with the literature for the main theorems]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \(F_\varrho(\lambda^{-1})\).
    \item The eta-gap, shadow, and \(S_3\) finite-verification results support
