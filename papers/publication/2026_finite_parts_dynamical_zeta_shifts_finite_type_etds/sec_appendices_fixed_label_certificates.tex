...
the relevant finite constants checkable by rational inequalities.  The main
text uses only the finite determinant computability theorem, the
...
are the mathematical core; the certificate propositions below are appendix
support describing one finite rational normal form for reproducing those
formulas in the local \(S_3\) calculation.  Optional files in the
\texttt{certificates/} directory are not hypotheses for any theorem in this
appendix; the load-bearing assertions are the finite records and rational
inequalities displayed here.
...
\begin{proposition}[Finite rational reproducibility record]\label{prop:fixed-label-rational-certificate-record}
  ...
  finite record gives an exact reproducibility certificate for the constant
  used in
