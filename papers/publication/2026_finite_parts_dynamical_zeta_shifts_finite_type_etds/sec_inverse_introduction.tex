...
\Cref{thm:effective-rational-mahler-coboundary} proves this effective
degree, height, reconstruction, and fixed-base complexity theorem for every
integer Mahler base \(p\ge2\), using one finite Pad\'e system.  The logarithmic divisor estimate in
\Cref{prop:logarithmic-mahler-divisor-bound} has optimal order at the level of
general rational divisor equations.  The relative realization lemma
\Cref{lem:relative-sign-determinant-realization} further shows that this
\(O(D\log D)\) certificate degree has optimal order for standard realizable
\(C_2\)-cover zeta ratios.  The collision radii are exactly the zeros of
\(R-1\), giving
...
solution space by \((\sigma-1)u=f\).  Conversely, a rational solution \(u\)
of \eqref{eq:intro-logarithmic-derivative-reduction} comes from a normalized
rational \(R\) exactly when \(u/z\) has no polynomial part, is regular at
zero, and has only simple poles with integer residues.  In that case the
...
\cite{Pegis1996NonlinearMahler}.  Thus bare decidability is not claimed here.
The effective increment is the direct input-only \(O(D\log D)\) degree bound,
the explicit height and coefficient bounds, one normalized Pad\'e system, and
the sharp degree order for this multiplicative equation.
...
dynamical setting, direct effective rational-coboundary bounds and reconstruction,
and cross-base finite radial determination for odd-Adams-invariant finite
abelian \(2\)-groups, together with realizable certificate-degree sharpness
and the linear sampling lower bound on standard \(C_2\)-covers.  The
