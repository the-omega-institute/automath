\subsection{Abelian shadow projection}

The next statement is a support projection fact used to identify which part of
the already constructed primitive profile is visible to abelian data.  The
decomposition
\eqref{eq:abelian-shadow-defect-sum} is the orthogonal splitting into the
one-dimensional and higher-dimensional Peter--Weyl sectors.

\begin{theorem}[Projection fact: abelian shadow and non-abelian defect]
...
\subsection{Cyclic quotient support span}

The cyclic input needed later is only the following elementary span
calculation.  Taken over all cyclic quotients, quotient pullbacks span exactly
the one-dimensional sector; an individual quotient sees only the characters
that factor through it.  This concerns quotient pullbacks on the fixed
cocycle.  The canonical cyclic lift of \Cref{sec:finite-part} is separate:
it supplies a fixed-matrix scalar root-of-unity comparison model.
...
\begin{corollary}[Projection fact: cyclic quotient support span]
