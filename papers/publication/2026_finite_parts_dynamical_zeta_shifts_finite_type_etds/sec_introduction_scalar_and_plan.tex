...
\begin{proposition}[Introductory scalar finite-part computation]
  ...
  \(\lambda>1\).  The scalar finite part used in the main theorem chain is
  ...
  \[
    ...
    \log C(A)+\sum_{n\ge1}p_n(A)
      \bigl(\lambda^{-n}+\log(1-\lambda^{-n})\bigr).
  \]
  Consequently the scalar orbit-Mertens normalisation is
  \(\mathcal M(A)=\gamma+\FP(A)\).  This scalar package contributes exactly
  the trivial Peter--Weyl coordinate to Main Theorems~II--III and determines
  no non-trivial fixed-label Perron value
  \(\Pi_\varrho(\lambda^{-1})\), \(\varrho\ne\mathbf1\), from fixed-matrix
  scalar data alone.
\end{proposition}

\begin{proof}
  The determinant and primitive-orbit formulae are
  \Cref{lem:finite-part-primitive-closed-form}.  Adding Euler's constant gives
  \(\mathcal M(A)=\gamma+\FP(A)\), and
  \Cref{cor:finite-part-mertens-asymptotic} proves that this is precisely the
  scalar orbit-Mertens constant.  The last assertion follows from
  \Cref{thm:finite-part-scalar-interface-no-descent,cor:fixed-matrix-scalar-interface-only}:
  fixed-matrix scalar records project the normalised class profile onto the
  constant class functions, while all non-trivial coordinates are the
  independent Perron-point primitive channels supplied by the finite twisted
  matrices.
\end{proof}
...
\begin{corollary}[Introductory quotient-shadow boundary]
  ...
  Under the hypotheses of Main Theorem~II, every finite quotient
  \(q:G\twoheadrightarrow Q\) pulls primitive and periodic channels back by
  \(q^\ast f=f\circ q\):
  \[
    ...
    \quad(|z|<\lambda^{-1}).
  \]
  At \(z=\lambda^{-1}\), cyclic quotients detect exactly the
  one-dimensional Peter--Weyl shadow of the normalised primitive class
  profile, while the sum of the higher-dimensional irreducible channels is
  the non-abelian defect.  The scalar root-of-unity layers of the canonical
  cyclic lift are therefore a separate fixed-matrix comparison model and do
  not descend from arbitrary cyclic quotients of \(\tau\).
\end{corollary}

\begin{proof}
  The channel identities are \Cref{thm:quotient-functoriality}.  The
  orthogonal decomposition of the normalised primitive class profile into its
  one-dimensional abelian shadow and higher-dimensional non-abelian defect is
  \Cref{thm:abelian-shadow-defect}, and exact cyclic detection of the
  one-dimensional sector is
  \Cref{cor:abelian-cyclic-shadow,lem:cyclic-quotients-one-dimensional-characters}.
  Finally, \Cref{prop:canonical-cyclic-lift-root-layer,lem:cyclic-quotient-versus-root-layer}
  prove that the root-of-unity determinant layers occur only after the
  canonical reset \(\tau_q(e)=1\), not for an arbitrary inherited cyclic
  quotient of the fixed finite-group cocycle.
\end{proof}
...
\Cref{sec:finite-part} isolates the scalar finite part and canonical
cyclic-lift comparison needed for the trivial-channel normalization.
...
Theorems~II--III.  \Cref{app:cyclic-lift-comparison,app:scalar-boundaries}
contain only scalar normalization and comparison material.  The \(S_3\)
appendices \Cref{app:s3-model-computations,app:s3-log-certificates} display
the finite matrices and rational inequalities used to certify the concrete
