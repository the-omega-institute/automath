\subsection{Squarefree certificates and prime-primary linear rigidity}
...
\begin{theorem}[Prime-primary collision--jet rigidity]
...
\end{align}
at distinct radii \(0<y_i<x\), at least one of which is algebraic.
If
...
forces \(F_\chi\) to be algebraic over \(\CC(z)\).
\Cref{prop:algebraic-mahler-coboundary} applied to
\eqref{eq:prime-primary-critical-equation} now gives
\(F_\chi\in\CC(z)\).  Since its Taylor coefficients are rational,
...
\begin{corollary}[Linear radial determination]
\label{cor:prime-primary-linear-radial-determination}
Under the hypotheses of \Cref{thm:prime-primary-collision-jet-rigidity},
with \(L=0\),
...
\begin{corollary}[Binary linear sampling bound]
