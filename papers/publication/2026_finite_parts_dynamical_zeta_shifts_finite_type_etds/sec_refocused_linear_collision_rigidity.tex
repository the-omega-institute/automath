...
\(f(z^q)=f(z)^q/J(z)\) belongs to \(\CC(z)\).  Under the remaining
...
\(J\in\CC(z)^\times\), every convergent Laurent-series germ
\(f\in\CC((z))\) that is algebraic over \(\CC(z)\) and satisfies
\(f(z^q)=f(z)^q/J(z)\) belongs to \(\CC(z)\).  Then, for \(V\ge6\),
