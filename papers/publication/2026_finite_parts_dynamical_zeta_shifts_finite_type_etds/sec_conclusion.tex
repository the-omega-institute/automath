...
secondary spectral-tail refinements, and non-abelian root-layer reconstructions.
The present manuscript uses only the finite interfaces proved above: the
eta-gap certificate, the finite-tail non-interference
\Cref{prop:main-chain-secondary-noninterference}, the local fixed-label
certificate records, and the fixed-matrix scalar boundary theorem.  Sibling
finite-observation, address-semantics, and universality materials remain
outside local theorem ownership.  The explicit \(S_3\) calculation should
therefore be read only as a finite witness for the Adams correction in the
determinant model, not as the start of a separate root-layer or universality
programme.
