...
the paper diagnoses the boundary case \(\eta=\lambda\) but does not prove
broad structural criteria forcing a strict gap for arbitrary cocycles.
Likewise, no H\"older transfer-operator determinant, Ruelle-resonance theorem,
pressure-window statement, completed zeta normalisation, xi-structure,
functional equation, or external determinant-normalisation theorem is imported
into the proof of Main Theorems~I--III.
  ...
  inverse problem.  Secondary spectral-edge tail refinements are retained only
  as conditional appendix material after those constants have been fixed; their
  interface with the main line is the non-interference statement
  \Cref{prop:main-chain-secondary-noninterference}, not a second proof of
  Main Theorems~I--III.  The explicit \(S_3\) certificate is a witness for the
  Adams correction in this determinant model and is used as a finite tool only;
  no general finite-group certificate taxonomy, certificate-complexity theory,
  or universality hierarchy is claimed in this article.

Natural extensions to H\"older/Ruelle transfer determinants, completed zeta
and xi normalisations, structural strict-gap criteria, secondary-edge theory,
fixed-matrix cyclic-lift inverse problems, sibling Fredholm/kernel and
language-obstruction packages, or broader computational certification are
therefore left to separate projects rather than folded into the finite-state
