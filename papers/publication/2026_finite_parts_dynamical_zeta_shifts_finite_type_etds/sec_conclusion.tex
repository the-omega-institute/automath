...
proved here: infinite-dimensional transfer-operator versions, completed-zeta
...
formal verification.  The present manuscript uses only the finite interfaces
proved above: the eta-gap certificate, the finite-tail non-interference
\Cref{prop:main-chain-secondary-noninterference}, and the fixed-matrix scalar
boundary theorem.  Sibling finite-observation, address-semantics, and
universality materials remain outside local theorem ownership.  The explicit
\(S_3\) calculation should therefore be read only as a finite witness for the
Adams correction in the determinant model.
