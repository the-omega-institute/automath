...
proved here.  The H\"older/Ruelle transfer-operator version would require
Banach-space hypotheses and determinant-convention comparisons not used in the
finite-matrix proof.  Secondary spectral-edge terms beyond the Perron-boundary
constants require their own separation hypotheses; this article keeps only the
noninterference firewall.  Broad sufficient criteria for the strict twisted gap
would be a cocycle-classification problem, whereas the present eta-gap material
is only a finite Perron-point gate and boundary obstruction.  Fixed-matrix
cyclic-lift reconstruction, scalar-boundary finite-observation limits, and
inverse problems from partial class observations are likewise companion
directions, not suppressed hypotheses for the present results.

The appendix material is subject to the same boundary.  This article retains
only the auxiliary scalar and certificate interfaces needed by the finite-group
determinant constants: the scalar constant \(C(A)\), the finite part
\(\FP(A)\), the no-descent warning, the fixed-label finite reproducibility
records, and the explicit \(S_3\) obstruction certificate.  The arguments above
use only the finite interfaces proved here: the eta-gap criterion, the
robustness statement \Cref{prop:main-chain-secondary-noninterference}, the
fixed-label records, and the fixed-matrix scalar boundary theorem.  The
uncompiled cyclic-lift and scalar-boundary packages are therefore companion
material unless a separate descent or finite-observation theorem is supplied.
The explicit \(S_3\) calculation should be read only as a finite witness for the
Adams correction in the determinant model, not as a root-layer reconstruction,
secondary-asymptotic theorem, scalar-certification theorem, or universality
theorem.
