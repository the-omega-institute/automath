...
versions and completed-zeta or \(\xi\)-type normalisations.  Structural
extensions include broader sufficient criteria for the strict twisted gap and
inverse problems from partial class observations.  Asymptotic extensions
include secondary spectral-edge terms beyond the Perron-boundary constants, and
example-building questions include systematic families illustrating the Adams
correction.  These are future directions rather than suppressed hypotheses for
the present results.

The appendix material is subject to the same boundary.  The cyclic-lift
comparison and scalar certificate statements are kept only as fixed-matrix
normalisation and reproducibility tools.  If venue length requires a shorter
submission, those packages can be extracted as companion notes while leaving
the scalar constant \(C(A)\), the finite part \(\FP(A)\), and the no-descent
warning in place.  The arguments above use only the finite interfaces proved in
this article: the eta-gap criterion, the robustness statement
\Cref{prop:main-chain-secondary-noninterference}, the fixed-label finite
reproducibility records, and the fixed-matrix scalar boundary theorem.  The
...
reconstruction, secondary-asymptotic theorem, scalar-certification theorem, or
