...
\Cref{prop:eta-gap-finite-state-certificate}.  Lower-order spectral-tail
refinements require their own secondary-edge hypotheses; cyclic root-layer
sampling would require a stable descent theory across resolutions.  The present
manuscript uses the former only through the finite-tail non-interference
\Cref{prop:main-chain-secondary-noninterference} and the latter only through the
fixed-matrix scalar boundary theorem.  Sibling determinant, synchronisation, and
automata-zeta packages remain cited background rather than local theorem
ownership, and prior single-primitive universality/certificate hierarchies are
separate projects.  The explicit \(S_3\) calculation should therefore be read
only as a finite witness for the Adams correction in the determinant model.
