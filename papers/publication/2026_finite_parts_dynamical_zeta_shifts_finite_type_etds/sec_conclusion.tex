...
The appendix material is subject to the same boundary. In this venue version,
the cyclic-lift recovery and scalar certificate packages are retained as
fixed-matrix auxiliary material, while the scalar constant \(C(A)\), the finite
part \(\FP(A)\), and the no-descent warning remain in the body. The arguments
above use only the finite interfaces proved in this article: the eta-gap
criterion, the robustness statement
...
reconstruction, secondary-asymptotic theorem, universal scalar-certification
theorem, or universality theorem.
