...
comparison in \Cref{prop:class-product-comparison}, and the comparison theorem
...
\begin{proposition}[Class-product fixed-label readout]
...
\begin{proposition}[Finite-dimensional determinant reduction]
  ...
  the finite-state theory but are not used in the proof of these three
  theorems.
\end{proposition}
\begin{proof}
...
comparison interface.  Reconstruction from cyclic lifts, one-layer Fourier
recovery, scalar finite-part variation or truncation certificates, secondary
spectral-edge asymptotics, and H\"older/Ruelle transfer-operator extensions
require additional hypotheses and are not inputs to the theorem chain.  The
...
\subsection{Auxiliary cyclic lifts}
...
These implications are made precise by
...
\end{array}
\]
Thus determinant recovery of these same coordinates is a theorem-chain step,
not an additional definition of the class product.  The cited product results
supply the Frobenius exponent and existence statement in the finite SFT group
extension described above.  The standing strict twisted gap \(\eta<\lambda\)
leaves a single scalar Perron pole and excludes competing non-trivial
Perron-boundary poles; all branch choices and the displayed value of \(K_C\) are
then derived from the Adams--M\"obius and determinant arguments below.
...
\begin{proposition}[Comparison with existing symbolic-dynamical results]
...
\begin{proof}
  The assertion follows by reading the cited theorem statements in the order of
  the main chain.  Main
  ...
  comparisons account for the external inputs used by the main theorem chain
  and prove the stated comparison.
\end{proof}
...
\begin{theorem}[Branch-compatible determinant readout for Main Theorem III]
  ...
  \begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
    \item The leading Frobenius-class product statement used in
    \Cref{thm:class-euler-product-mertens} supplies
...
\begin{proof}
  Assertion \textup{(a)} is the comparison statement in
  ...
  restatement of the proved equivalence together with the explicit comparison
  of the Frobenius-class product exponent and determinant coordinates in
  \Cref{thm:class-euler-product-mertens}.
...
\noindent
  ...
  variation, finite scalar truncation, or reconstruction from lift data need
  additional hypotheses beyond this scalar normalisation.  The product-side
...
The Euler-product constants require the same distinction. The comparison
theorem recalls the Frobenius-class product existence statement and leading
exponent \(|C|/|G|\).  Once these are fixed, the non-trivial coordinates of the constant are the fixed-label Euler transform values
...
part alone.  Further finite-matrix projects include cyclic-lift spectrum
reconstruction from branch-lifted records and effective scalar finite-part
variation, truncation, and certification.  Each of these questions requires
additional structure beyond the hypotheses used in this article.
...
\label{cor:fixed-label-five-field-rational-record}
  ...
  \(\varrho\in\Irr(G)\), by a single normal form with the following five finite
