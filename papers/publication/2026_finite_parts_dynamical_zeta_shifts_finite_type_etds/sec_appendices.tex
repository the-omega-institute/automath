...
This appendix collects the scalar cyclic-lift comparison arguments used
only as fixed-matrix background for the normalisation and comparison
statements in the main text.  The finite-group theorem spine does not use
the reconstruction conclusions below; it uses only the scalar reduced
constant, the multiple-sum identity, and the resulting branch-lifted
comparison record.  The determinant identities, M\"obius inversion on
multiples, and one-layer Fourier calculations are therefore kept here as
auxiliary fixed-matrix material rather than as a parallel theorem chain.

For reference, the appendix material has the following dependency status.
\begin{center}
\begin{tabular}{@{}L{0.30\textwidth}L{0.25\textwidth}L{0.35\textwidth}@{}}
\toprule
Appendix material & Dependency class & Relation to Main Theorems~I--III \\
\midrule
\Cref{app:cyclic-lift-package}
& Scalar normalisation and fixed-matrix comparison
& Supplies reduced-determinant and cyclic-lift calculations; not an
additional finite-group hypothesis. \\
\Cref{app:scalar-boundaries}
& No-descent safeguard
...
& Example and obstruction certificate
...
displayed \(S_3\) windows. \\
\Cref{sec:secondary-term}
& Conditional refinement
& Refines remainders under extra secondary-edge separation; the main
constants and theorem spine do not depend on it. \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Auxiliary fixed-matrix reconstruction outputs}
...
\begin{theorem}[Auxiliary fixed-layer Fourier recovery]
