...
This appendix supports the finite-group theorem chain by fixing the scalar
normalisation \(\mathcal M(A)\), recording the canonical cyclic-lift identities
used for fixed-matrix comparison, and stating the no-descent boundary between
scalar records and the non-trivial Peter--Weyl class profile.  It supports the
scalar term in Main Theorems~II--III and does not claim a second determinant
recovery theorem for finite-group labels.
...
the body of the paper.  For a shorter venue version, the part needed here is
only the trivial-channel normalisation \(C(A)\), \(\FP(A)\), \(\mathcal M(A)\),
and the no-descent warning; the longer cyclic-lift reconstruction may be read
as companion-note material.  No result in this appendix is used to recover a
finite-group edge cocycle or a non-trivial Peter--Weyl primitive profile.
\begin{center}
