\section{Appendix: scalar normalisation and cyclic-lift comparison}
\label{app:cyclic-lift-comparison}

This appendix is retained as support for the finite-group theorem chain.  Its
role is to fix the scalar normalisation \(\mathcal M(A)\), record the
canonical cyclic-lift identities used for fixed-matrix comparison, and state
the no-descent boundary separating scalar records from the non-trivial
Peter--Weyl class profile.  The cyclic-lift statements below are therefore
normalisation and comparison tools only; they are not an alternate route to
Main Theorems~I--III.

The fixed-matrix reconstruction material is included only to make the scalar
comparison transparent.  It does not recover non-abelian primitive coordinates
and does not compete with the finite-group Adams--M\"obius inversion,
Perron-point class constants, or fixed-label Euler-product formula proved in
the body of the paper.  If extracted into a companion note, the part needed
here is only the trivial-channel normalisation \(C(A)\), \(\FP(A)\),
\(\mathcal M(A)\) and the no-descent warning; the cyclic-lift reconstruction
proofs should move as a unit rather than being used as local main-line input.
No result in this appendix is used to recover a finite-group edge cocycle or a
non-trivial Peter--Weyl primitive profile.
\begin{center}
...
& Scalar no-descent boundary
& Main Theorems~II--III
& Records the trivial-channel boundary: scalar fixed-matrix records do not by
themselves recover non-trivial class profiles. \\
...
& Finite fixed-label records
& finite algorithm
& Records finite rational data for the same \(S_3\) witness; it is not a
general verification theory. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{lemma}[Fixed-matrix branch ambiguity in multiples
  ...
  \]
  Thus the reconstruction statement uses supplied logarithmic branches,
...
\begin{proposition}[Fixed-matrix cyclic-lift comparison record]
  ...
  normalisation model; it is not a coequal finite-group reconstruction result
...
\begin{proof}[Fixed-matrix reconstruction does not descend to class-product coordinates]
  ...
  the proposition is not a finite-group class-product reconstruction result and
...
\begin{proposition}[Fixed-layer Fourier comparison]
  \label{thm:finite-part-cyclic-lift-single-q-torsion-reconstruction}
  \label{thm:appendix-one-layer-reconstruction}
