...
particular, the proofs below are the auxiliary proof bodies for
...
\begin{proposition}[Auxiliary formula for the reduced determinants of cyclic lifts]
  \label{prop:auxiliary-finite-part-cyclic-lift-reduced-constant-closed}
  For every \(q\ge 1\),
  \[
...
\begin{proof}
  By \Cref{prop:auxiliary-finite-part-cyclic-lift-reduced-constant-closed},
  \[
    \Psi_q(A)
...
\begin{proposition}[M\"obius inversion on multiples]
  ...
  \]
  Its proof is deferred only to insert
  \Cref{lem:finite-part-cyclic-lift-mobius-absolute}, the absolute
  convergence lemma needed for the Tonelli--Fubini interchange; the
  labelled proof of this proposition follows that lemma directly.
\end{proposition}
...
\begin{theorem}[Auxiliary appendix comparison: fixed-matrix cyclic-lift recovery of the
  ...
  \[
    ...
    \qquad q\ge1,
  \]
  with a specified branch for every \(q\). This record determines the
