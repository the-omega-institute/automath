...
This appendix is auxiliary to the finite-group theorem chain.  The scalar
statements used in the main article are the reduced-determinant finite part
\(\mathcal M(A)\), the canonical cyclic-lift multiple-sum identity, and the
fixed-matrix branch comparison that separates scalar normalization from the
non-trivial Peter--Weyl class profile.  These facts support the trivial
channel and comparison paragraphs in the main text; they are not an alternate
route to Main Theorems~I--III.

The remaining cyclic-lift material is included for normalization checks and
for comparison with familiar scalar root-of-unity sampling.  It does not
recover non-abelian primitive coordinates and it does not compete with the
finite-group Adams--M\"obius inversion, Perron-point class constants, or
fixed-label Euler-product formula proved in the body of the paper.
\begin{center}
