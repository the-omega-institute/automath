...
interface with the theorem chain is narrow: it supplies the trivial-channel
finite part \(\mathcal M(A)\), the canonical cyclic-lift multiple-sum
identity, and the fixed-matrix branch comparison used to separate scalar
data from non-trivial Peter--Weyl class profiles.  It never supplies a
descent theorem for non-abelian primitive class data; that information
comes only from the finite-group determinant and Adams--M\"obius machinery
in the main text.

The remaining appendices are organized as follows.  This table is also the
dependency map for the auxiliary theorem environments: every auxiliary
result is used only in the listed role and is not an additional main
theorem spine.  In particular, the cyclic-lift, scalar-variation, and
secondary-edge material below is kept as compact support for the
finite-state Adams--M\"obius article, not as a parameter-family,
inverse-problem, or secondary-asymptotic theory developed here.
\begin{center}
\begin{tabular}{@{}L{0.24\textwidth}L{0.21\textwidth}L{0.19\textwidth}L{0.26\textwidth}@{}}
\toprule
Appendix material & Classification & Feeds & Use in the paper \\
\midrule
\Cref{app:cyclic-lift-package}
& Proof inputs and scalar checks
