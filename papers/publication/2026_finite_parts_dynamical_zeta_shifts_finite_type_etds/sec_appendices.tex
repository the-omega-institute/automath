...
The appendix does not assert the cyclic-block Fredholm, synchronisation-kernel,
completed-determinant, endpoint-jet, large-parameter, or automata-zeta theorem
families assigned to sibling papers; it keeps only the finite scalar identities
needed for the fixed-matrix normalisation in this article.

For the present article we keep the exhaustive finite certificates in the appendix,
because they are short enough to make the determinant recovery and the concrete
\(S_3\) obstruction reproducible without a separate supplement.  A shorter
venue version could move the tabulated rational certificates to supplementary
material without changing Main Theorems~I--III.
\begin{center}
