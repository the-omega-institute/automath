\section{Appendix: cyclic-lift comparison package}
...
separate scalar data from non-trivial Peter--Weyl class profiles.  It is
not an input to Main Theorems~I--III and never supplies a descent theorem
for non-abelian primitive class data; that information comes only from the
finite-group determinant and Adams--M\"obius machinery in the main text.

The remaining appendices are organized as follows.  This table is the
dependency map for support material only: Main Theorems~I--III are the
main sequence, and each auxiliary result is used only in the listed
role.  The cyclic-lift checks, scalar boundary records, \(S_3\)
certificates, and secondary-edge material are therefore local support for
the finite-state Adams--M\"obius argument, not a parameter-family,
inverse-problem, or secondary-asymptotic article nested inside this one.
\begin{center}
...
\Cref{sec:secondary-term}
& Conditional post-constant tail
& Post-constant remainders
& Refines finite-length remainders only after the constants have been
constructed from the main theorem chain. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{theorem}[Fixed-matrix cyclic-lift recovery of the
  reduced spectrum]
