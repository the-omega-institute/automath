...
\begin{proof}
  ...
  \(F_\varrho(\lambda^{-1})\) is exactly the convergent determinant sum
  \eqref{eq:assembled-product-determinant-evaluation}.  In that sum the
  scalar singular term \((r,m,\pi)=(1,1,\mathbf1)\) is omitted because its
  character coefficient is zero for \(\varrho\ne\mathbf1\).  The coefficients
