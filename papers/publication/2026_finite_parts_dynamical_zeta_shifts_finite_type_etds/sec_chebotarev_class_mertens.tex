...
chain.  These are synthesis statements: for hypotheses, branch conventions,
...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\begin{proposition}[Tiered finite-determinant closure of the main theorem chain]
...
\begin{corollary}[Downstream status of the obstruction refinements]
  ...
  scalar-normalised product skeleton, the same class constants, and the same
  Artin-substitution obstruction bound.  Conversely, equality of the
  scalar-normalised product skeleton forces equality of all non-trivial
  fixed-label boundary values.  Hence the skeleton theorem and the Parseval
...
\begin{proof}
  ...
  If the periodic Artin logarithms also agree, then the difference vector
  agrees, and so the Parseval bound agrees.  No additional asymptotic input,
  determinant channel, or Perron-point hypothesis is introduced.  This proves
  the asserted downstream status.
\end{proof}
