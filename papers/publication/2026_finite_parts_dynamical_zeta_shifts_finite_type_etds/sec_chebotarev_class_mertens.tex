...
This subsection has three successive layers.  First,
\Cref{thm:class-mertens-explicit} proves the additive class-Mertens
constant under the stated twisted spectral gap.  Second,
\Cref{thm:fixed-label-euler-transform,prop:fixed-label-determinant-perron,thm:class-euler-product-mertens}
separate the fixed primitive-label Euler transform from the periodic Artin
logarithm and use it to evaluate Frobenius-class product constants.  The
remaining labelled statements are assembly and readout consequences of this
spine: they identify exactly which part of the product constant is new, prove
the associated rigidity and Artin-substitution obstruction, and classify the
allowable class-product vectors after scalar normalisation.  Thus the theorem
chain for the Perron-point part is additive constants \(\Rightarrow\) product
constants \(\Rightarrow\) fixed-label Adams--M\"obius determinant data, not a
survey of independent Mertens-type asymptotics.
...
\begin{proposition}[Finite-data closure of the main theorem spine]
  ...
  scalar cyclic-lift record, or auxiliary appendix invariant is an input to
...
\begin{proof}
  ...
  completing the finite-data closure of the main theorem spine.
\end{proof}
...
\begin{lemma}[Hermitian normalisation of the class-product constant]
  ...
  Equivalently,
  \begin{equation}\label{eq:class-product-normalisation-audit}
    -\log K_C
    =
    \frac{|C|}{|G|}
    \left(
...
\begin{proof}
  ...
  \eqref{eq:class-product-normalisation-audit} is multiplied by
...
\begin{corollary}[Readout consequence: character-hyperplane classification of
  product vectors]
  \label{cor:assembled-adams-frobenius-product-package}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}, and put
  \(\alpha_A:=\gamma+
  \log C(A)\).  This is the classification consequence of
  \Cref{cor:class-product-constant-rigidity}: it describes the affine
  hyperplane on which the already constructed constants live and adds no new
  product asymptotic.  Let \(\mathscr K_A(G)\) be the set of positive real
