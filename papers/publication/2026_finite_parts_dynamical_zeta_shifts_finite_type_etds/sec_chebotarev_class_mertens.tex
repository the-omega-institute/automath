...
\Cref{prop:fixed-label-certificate-manuscript-scope,thm:class-product-comparison} then separates the certified finite
...
\begin{corollary}[Hyperplane classification and fixed-coordinate stability]
  \label{cor:class-product-hyperplane-stability}
  \label{cor:class-product-stability-scalar-independence}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}.
  After the scalar value \(\alpha_A\) is fixed, the identities of
  \Cref{thm:class-product-determinant-rigidity}
  classify all positive class-product vectors satisfying the forced scalar
  product constraint.  Namely let
  \[
    \mathscr K_{\alpha_A}:=
    \left\{(L_C)_C:\ L_C>0,
  ...
  \begin{equation}\label{eq:class-product-stability-isometry}
    ...
    \sum_{\varrho\ne\mathbf1}
      ...
      \right|^2 .
  \end{equation}
  Hence two admissible determinant-coordinate data sets with the same scalar
  normalisation and branch convention give the same Frobenius-class product
  constants if and only if their non-trivial fixed-label Perron vectors agree.
  ...
  non-trivial Peter--Weyl coordinates.
\end{corollary}
...
\begin{proof}
  ...
  \eqref{eq:class-product-stability-forward} and its Parseval identity.
...
\begin{proposition}[Finite determinant-to-product certificate interface]
  \label{prop:fixed-label-perron-finite-algorithm}
  \label{prop:fixed-label-certificate-protocol}
  \label{thm:fixed-label-perron-finite-certificate-sufficiency}
  ...
  below.  The same proposition is also the manuscript's complete finite
  certificate interface for the determinant coordinates in Main Theorem~III:
  the certificate record, the sufficiency statement, the bookkeeping schema,
  and the product-constant enclosure are the finite consequences of the single
  Adams-corrected determinant formula proved here, not separate theorem-level
  mechanisms.
...
\end{proposition}
  ...
  The proof is included to make clear that the certificate interface is only a finite reformatting of the Adams-corrected determinant formula.
...
\medskip
\noindent\textit{Finite rational certificate record.}
...
\medskip
\noindent\textit{Sufficiency and necessity for the manuscript interface.}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}, and fix a
  non-trivial irreducible character \(\varrho\).  The following finite record is
  sufficient, and is necessary for the certificate interface used in this
  paper, to certify the fixed-label Perron coordinate
  ...
  \Cref{prop:fixed-label-certificate-protocol} reduces to such a record after
...
\begin{proof}
  The sufficiency direction is exactly the finite truncation theorem and the
  protocol interface in compressed form.  Items \textup{(i)} and
  ...
  Conversely, the certificate protocol consists precisely of the determinant
...
\medskip
\noindent\textit{Bookkeeping schema.}
  Under the hypotheses and notation of
...
\medskip
\noindent\textit{Product-level use and scope.}
  The certificate protocol of
  \Cref{prop:fixed-label-certificate-manuscript-scope}
