...
\Cref{prop:fixed-label-perron-finite-algorithm,thm:class-product-comparison,prop:class-product-coordinate-observation-budget} then separates the certified finite
...
For dependency checking, the large proof spine should be read in the following
...
evaluate the non-trivial Perron coordinates from the finite determinant data
and record the effective truncation convention.  Finally,
\Cref{prop:class-product-comparison,cor:class-product-minimal-obstruction-skeleton,prop:class-product-coordinate-observation-budget,thm:adams-obstruction-class-product,cor:adams-obstruction-bound}
are comparison and obstruction consequences of the already recovered product
vector.  Thus the later rigidity, stability, and \(S_3\) certificate statements
do not feed back into the proofs of Main Theorems~II--III; they only test or
separate the product constants produced by this ordered chain.
...
\begin{proposition}[Isolated reduction to Main Theorem II and the product tail]
  ...
  auditable spine: Main Theorem~II controls the \(r=1\) logarithm, the
...
linear algebra in this fixed coordinate chart; they are recorded for audit and
reproducibility, not as additional main theorems or as new hypotheses for the
class-product asymptotic.
...
\begin{proposition}[Theorem-by-theorem comparison calibration]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.24\textwidth}
    ...
    \toprule
    theorem package in this paper & comparison source & separated contribution proved here\\
    \midrule
    Main Theorem~I:
    \Cref{thm:class-function-linearisation,cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant,thm:primitive-peter-weyl-trace}
      & Artin--Ruelle and Chebotarev transfer-operator framework of
      ...
      & finite character-algebra inversion from periodic twisted determinants
        to primitive Peter--Weyl channels, with Adams powers keeping primitive
        repetitions separate\\
    Main Theorem~II:
    \Cref{thm:class-mertens-explicit,prop:class-mertens-deviations,thm:class-mertens-fourier}
      & Sharp's orbit-Mertens model and the Frobenius-class Mertens comparison
        of Mesliza--Noorani
        \cite{Sharp1991Mertens,MeslizaNoorani1999FrobeniusMertens}
      & finite-state determinant formula for the class-indexed constant term
        after the classical density and Perron-gap layer is fixed\\
    Main Theorem~III:
    \Cref{thm:class-euler-product-mertens,prop:fixed-label-determinant-perron,thm:class-product-determinant-rigidity,prop:class-product-comparison,thm:adams-obstruction-class-product}
      & Frobenius-class product existence and ordinary Artin-logarithm
        determinant analogues in the same symbolic-dynamical lineage
        \cite{MeslizaNoorani1999FrobeniusMertens,ParryPollicott1990Zeta}
      & Adams-corrected fixed-label Euler value
        \(F_\varrho(\lambda^{-1})\), its determinant recovery, and the
        obstruction to replacing it by \(\mathcal L_\varrho(\lambda^{-1})\)\\
    body finite-state readouts and obstructions:
    \Cref{thm:quotient-functoriality,thm:abelian-shadow-defect,thm:s3-certified-nonabelian-defect,thm:eta-gap-rigidity}
      & Coles, Humbert, O'Hare, and Crismale--Del Vecchio--Griseta--Rossi
      ...
      & venue calibration only: the present results are finite SFT cocycle
        determinant-coordinate statements, not geometric periodic-orbit
        invariants, Ruelle-resonance theorems, finite-data rigidity theorems,
        or operator-algebraic skew-product classifications\\
    \bottomrule
  \end{tabular}
  \end{center}
...
\begin{proof}
  ...
  theorem.  The Parry--Pollicott and Noorani--Parry references therefore give
  the ambient Artin--Ruelle and Chebotarev setting, while the primitive-channel
  inversion is the internal finite-label step.
  ...
  \Cref{thm:class-mertens-fourier}.  This proves the second row: Sharp and
  Mesliza--Noorani supply the relevant Mertens comparison scale, but the
  determinant expression for \(\mathcal M_C(A,\tau;G)\) is the internal output.
  ...
  Finally, the modern comparison papers cited in the last row concern geometric
  periodic-orbit observables, Ruelle-resonance behaviour, finite-data rigidity,
  and non-commutative skew-product structure.  None of their results appears in
  the hypotheses or proofs of the theorem packages just listed: the body
  arguments use finite adjacency matrices, finite-group character algebra,
  Perron-boundary evaluation, quotient/shadow maps, and explicit \(S_3\)
  certificates.  This proves both the venue calibration and the asserted
...
\begin{proposition}[Finite reproducibility support for determinant constants]
  \label{prop:fixed-label-perron-finite-algorithm}
  The displayed label
  \verb|prop:fixed-label-perron-finite-algorithm| is the single reader-facing
  label for this finite reproducibility proposition.
  ...
  branch-specified reproducibility procedure.  More precisely, from the
...
\noindent\textit{Appendix-level reproducibility records.}
  The local rational reproducibility record and five-field \(S_3\) audit format are
