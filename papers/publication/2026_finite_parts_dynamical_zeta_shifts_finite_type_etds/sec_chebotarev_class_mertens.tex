...
\begin{theorem}[Determinant rigidity and Artin obstruction for
  ...
  \label{thm:retargeted-adams-product-novelty}
  ...
  product vector
  \begin{equation}\label{eq:retargeted-product-log-vector}
    H_K(C):=-\frac{|G|}{|C|}\log K_C-\alpha_A .
  \end{equation}
  Then
  \begin{equation}\label{eq:retargeted-product-constraint}
  ...
  \begin{equation}\label{eq:retargeted-product-fourier-coordinates}
    H_K(C)=
    \sum_{\varrho\ne\mathbf1}
      \chi_\varrho^\ast(C)F_\varrho(\lambda^{-1}),
    \qquad
  ...
  \end{equation}
  In particular,
  \begin{equation}\label{eq:retargeted-product-parseval}
  ...
  \eqref{eq:retargeted-product-constraint} determines a unique zero-mean
...
\begin{proof}
  ...
  \eqref{eq:retargeted-product-fourier-coordinates}.  Averaging this
  identity over conjugacy classes with weights \(|C|/|G|\) gives the second
  identity in \eqref{eq:retargeted-product-constraint}, because every
  ...
  \eqref{eq:retargeted-product-constraint}.  The inverse Fourier identity in
  \eqref{eq:retargeted-product-fourier-coordinates} and the Parseval identity
  \eqref{eq:retargeted-product-parseval} are the standard Hermitian
  ...
  \eqref{eq:retargeted-product-parseval}: the displayed norm vanishes if and
  only if all non-trivial fixed-label coordinates vanish, and then
  \eqref{eq:retargeted-product-log-vector} gives
