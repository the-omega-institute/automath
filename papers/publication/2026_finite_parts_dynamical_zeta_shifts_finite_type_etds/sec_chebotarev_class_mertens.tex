...
record the readout order without adding a new rigidity theorem; the formal
dependency spine below also records the corresponding two-extension source
closure for the finite determinant data.  The
...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\begin{remark}[Finite determinant readout interface for the three main theorems]
  \label{prop:finite-data-main-spine-closure}
  Assume \(\lambda>1\) and the twisted spectral gap
  \(\eta<\lambda\) of \Cref{thm:class-mertens-explicit}.  This remark
