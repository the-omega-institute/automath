...
\begin{theorem}[Adams-corrected Frobenius-class Euler-product Mertens
  constants]
...
\begin{proposition}[Comparison with classical Frobenius-class products]
  \label{prop:class-product-comparison}
  The exponent \(|C|/|G|\) in
  \eqref{eq:class-euler-product-asymptotic} is the classical Frobenius
  density exponent arising from Chebotarev/Mertens orbit asymptotics for
  finite homogeneous extensions, in the sense of
  \cite{Sharp1991Mertens,ParryPollicott1986Chebotarev,%
  NooraniParry1992ChebotarevShifts}. The new finite-state contribution
  of \Cref{thm:class-euler-product-mertens} is the Adams-corrected
  constant: the class-product constant is controlled by the fixed-label
...
\begin{proof}
  The cited Chebotarev and dynamical Mertens theorems give the leading
  Frobenius-class density and product exponent in the usual orbit-counting
  framework. In the present finite-state setting the same exponent also
  follows directly from \eqref{eq:class-mertens-log}, and the proof of
  \Cref{thm:class-euler-product-mertens} shows that all information beyond
  this leading exponent is contained in the constant
  \eqref{eq:class-euler-product-k-f}. By
  \Cref{thm:fixed-label-euler-transform} this constant uses
  \(F_\varrho=\sum_{r\ge1}\Pi_\varrho(z^r)/r\), whereas the Artin
  logarithm is
  \(\mathcal L_\varrho=\sum_{r\ge1}\Pi_{\psi^r\varrho}(z^r)/r\). The two
  are generally different because repetitions apply Adams operations to
  labels. Finally \Cref{prop:fixed-label-determinant-perron} expresses
  these corrected constants in terms of the finite twisted determinant
  data. This proves both the classical attribution of the exponent and the
  advertised Adams-corrected determinant content of the present theorem.
\end{proof}
