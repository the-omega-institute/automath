...
\begin{lemma}[Audit identity for class Euler-product constants]
  ...
  \begin{equation}\label{eq:class-product-constant-audit-identity}
    \mathcal M_C(A,\tau;G)
    +\sum_{r\ge2}\frac{\Pi_{\mathbf1_C}(\lambda^{-r})}{r}
    =
    \frac{|C|}{|G|}
    ...
    \right).
  \end{equation}
  All sums in \eqref{eq:class-product-constant-audit-identity} are
...
\begin{proof}
  ...
  term by term, and
  \begin{equation}\label{eq:class-product-tail-split-audit}
    \sum_{r\ge2}\frac{\Pi_{\mathbf1_C}(\lambda^{-r})}{r}
    =
  ...
  \[
    ...
    \frac{|C|}{|G|}
      \sum_{\varrho\neq\mathbf1}
        \chi_\varrho^\ast(C)\Pi_\varrho(\lambda^{-1}).
  \]
  Adding this display to \eqref{eq:class-product-tail-split-audit} and
  ...
  \begin{equation}\label{eq:scalar-product-normalisation-audit}
    \mathcal M(A)+\sum_{r\ge2}\frac{\Pi_{\mathbf1}(\lambda^{-r})}{r}
    =
    \gamma+\FP(A)+
  ...
  \eqref{eq:scalar-product-normalisation-audit} gives
  \[
    \mathcal M(A)+\sum_{r\ge2}\frac{\Pi_{\mathbf1}(\lambda^{-r})}{r}
    =\gamma+\log C(A).
  \]
  ...
  \eqref{eq:class-product-constant-audit-identity}.

  Finally, \eqref{eq:class-euler-product-k-mc} is the exponential of the
  negative left-hand side of
  \eqref{eq:class-product-constant-audit-identity}, and
...
\begin{proof}
  ...
  The whole Abel family of double determinant series is controlled by
  \Cref{lem:fixed-label-perron-double-majorant}, applied to
  \(\phi=\chi_\varrho\). In particular, the only scalar Perron logarithm,
  corresponding to \((r,m,\pi)=(1,1,\mathbf1)\), has coefficient zero;
  the finitely many remaining \(rm=1\) logarithms are regular along the
  Abel path; and the \(rm\ge2\) tail is dominated by the displayed
  summable majorant
  \(C_\varrho\sum_{rm\ge2}\lambda^{1-rm}/(rm)\). Dominated convergence as
...
\begin{corollary}[Structural rigidity of the Adams-corrected class-product constants]
  ...
  exactly equivalent, after the
  scalar normalisation \(C(A)\), to the non-trivial fixed-label
  ...
  \begin{equation}\label{eq:class-product-rigidity-forward}
    ...
    \sum_{\varrho\neq\mathbf1}
      \chi_\varrho^\ast(C)F_\varrho(\lambda^{-1}),
  \end{equation}
  and, for every non-trivial \(\pi\in\Irr(G)\),
...
\begin{proof}
  ...
  \[
    ...
    \sum_{\varrho\neq\mathbf1}
      \chi_\varrho^\ast(C)F_\varrho(\lambda^{-1}),
  \]
  which is \eqref{eq:class-product-rigidity-forward}.  Multiplying this
