...
fixed-label Perron values from the twisted determinant package.  The later
rigidity, finite-data closure, Artin-obstruction, hyperplane-classification,
and stability statements are consequences of this chain: they describe how
the product vector can be read back, when the periodic Artin logarithm gives
the wrong answer, and which non-trivial Peter--Weyl coordinates remain after
...
\begin{theorem}[Determinant rigidity and Artin obstruction for
  ...
  Abel branches of \Cref{prop:fixed-label-determinant-perron}.  More
...
\begin{corollary}[Readout consequence: determinant coordinates after scalar normalisation]
  ...
  \(\alpha_A:=\gamma+
  \log C(A)\).  This corollary is only the structural readout following
  \Cref{thm:class-euler-product-mertens}: after the scalar normalisation is
