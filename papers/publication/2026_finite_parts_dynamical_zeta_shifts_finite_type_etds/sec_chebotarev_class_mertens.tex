...
\Cref{prop:fixed-label-certificate-artifact-interface,thm:class-product-comparison} then separates the certified finite
...
\begin{corollary}[Canonical finite certificate schema for fixed-label Perron
  readout]
  \label{cor:fixed-label-perron-certificate-schema}
  Under the hypotheses and notation of
  \Cref{prop:fixed-label-certificate-artifact-interface}, the preceding
  analytic certificate can be serialized without any extra mathematical
  choices by the following five-field schema:
  \begin{center}
  \small
  ...
  \end{tabular}
  \end{center}
  The checker accepting this schema returns exactly the interval inclusion
  \eqref{eq:fixed-label-certificate-interface-inclusion}.  Conversely, every
  finite certificate record admitted by
  \Cref{prop:fixed-label-certificate-artifact-interface} determines the five
  ...
  text, are reproducible arithmetic transcripts for this schema, not new
  analytic assumptions.
\end{corollary}

\begin{proof}
  Starting from the four records in
  \Cref{prop:fixed-label-certificate-artifact-interface}, put the base matrix,
  ...
  The budget field is record \textup{(iv)}.  Therefore the schema contains no
  datum beyond the analytic certificate proposition.

  The verifier checks listed in the last column are precisely the proof steps
  of \Cref{prop:fixed-label-certificate-artifact-interface}: finite character
  ...
  Conversely, any accepted four-record certificate already specifies the same
  ...
  normalization needed to place it in the displayed schema.  The cited
  \(S_3\) files instantiate the same fields with exact rational arithmetic,
  so their role is a serialized transcript of this finite verification.
\end{proof}
