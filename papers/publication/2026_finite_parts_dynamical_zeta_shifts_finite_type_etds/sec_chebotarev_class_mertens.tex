...
\begin{proposition}[Certified finite algorithm for fixed-label Perron evaluation]
  ...
  \begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
    ...
    \item Given \(\epsilon>0\), choose \(M\) so that the computable tail
      ...
      package and \(\varrho\).  For an exact certificate one may replace the
...
\begin{proof}
  ...
  \(Q_\varrho\) are finite comparisons in the algebraic determinant package.
...
\begin{proposition}[Finite certificate protocol for Perron readout]
  ...
  algorithm of \Cref{prop:fixed-label-perron-finite-algorithm} admits the
  following script-independent certificate protocol.  A certificate record is
  accepted if it consists only of finite algebraic model data and rational
  inequalities of the following four kinds:
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item for every listed triple, a branch tag specifying either the Abel
      ...
      \(\Log\det(I-\lambda^{-rm}A_\pi)^{-1}\);
    \item two rational error budgets \(R_M\) and \(R_{\mathrm{num}}\): the
      tail budget \(R_M\) is certified either directly from
  ...
  \end{equation}
  where all sides of \(I_M\) and both radii are rational.  Consequently a
  supplemental certificate file or script for a concrete model is not an
  additional hypothesis: it is a
  mechanically checkable serialization of the same theorem-chain data used in
