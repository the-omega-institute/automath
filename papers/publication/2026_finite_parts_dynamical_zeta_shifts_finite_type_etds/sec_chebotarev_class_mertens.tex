...
\begin{proposition}[Adams correction to Frobenius-class Mertens products]
  ...
  is the Adams-corrected determinant recovery of the constant
  ...
  More precisely, the correction is exactly the finite class function
  \eqref{eq:adams-obstruction-class-function}: if a naive formula replaces
  \(F_\varrho\) by \(\mathcal L_\varrho\), then the logarithmic error in
  the product constant is the Peter--Weyl sum displayed in
...
\begin{proof}
  ...
  the classical product asymptotic. The displayed error formula follows
  from \Cref{thm:adams-obstruction-class-product}, which subtracts the
  naive Artin replacement constant from the corrected constant.
\end{proof}
...
\begin{theorem}[Dependency-transparent Adams-corrected Frobenius-class
  ...
  Frobenius-class zeta-product constants
  \((K_C)_{C\subset G}\) satisfying the scalar constraint
  \(\prod_C K_C=\exp(-\gamma-\log C(A))\) are equivalent, after the scalar
  normalisation \(C(A)\), to the non-trivial fixed-label Perron vector
  ...
  \(a_{\varrho,\pi}^{(m)}\), determines both of these equivalent data sets.
  More explicitly,
  ...
  fixed-label boundary vector in class coordinates.  This assembled
  statement is the body theorem corresponding to the unnumbered
  Frobenius-class zeta-product formulation in the introduction.  Its
  only inputs are the previously proved component results
...
\begin{proof}
  ...
  \eqref{eq:assembled-product-inverse}.  This proves the equivalence of
  the constrained product vector and the non-trivial fixed-label Perron
  vector; it is the same rigidity statement recorded in
