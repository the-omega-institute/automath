...
\begin{theorem}[Determinant rigidity and Artin obstruction for
  ...
  following is an assembly statement for the product theorem, not a separate
  leading-asymptotic theorem.  It records the exact information retained by
  the constant term once the classical Frobenius exponent has been removed.
  ...
  In short, this theorem does not assert novelty for Frobenius-class
  product asymptotics themselves.  Its new assertion is that, after the
  classical exponent and existence theorem have been separated, the
  fixed-label Adams--M\"obius determinant data are exactly the missing
  non-scalar coordinates of the product constant.
\end{theorem}
...
\begin{corollary}[Core determinant readout and scalar normalisation boundary]
  \label{cor:assembled-adams-frobenius-product-package}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}, and put
  \(\alpha_A:=\gamma+
  \log C(A)\).  This is the finite-dimensional readout part of
  Main Theorem~III: the product asymptotic has already been proved in
  \Cref{thm:class-euler-product-mertens}, and the assertion here is the exact
  recovery of the non-scalar determinant coordinates after the scalar
  normalisation has been removed.  Let \(\mathscr K_A(G)\) be the set of
  positive real
...
\begin{corollary}[Readout consequence: stability and scalar independence]
  ...
  \[
    ...
    +\frac{|G|}{|C|}\log K_C^{\tau'}.
  \]
  This readout statement compares two already constructed product vectors;
  its role is to isolate the non-trivial Peter--Weyl coordinates that scalar
  finite-matrix records cannot see.  Then
