...
first two subsubsections prove Main Theorems~II and III: additive class
constants, then Adams-corrected class Euler products.  Everything after
the determinant readout is a structural consequence, comparison theorem,
or obstruction certificate for the same constants, not a fourth main
spine.  The core chain is
\[
\begin{gathered}
...
prove Main Theorem~III.  The determinant readout is
\Cref{prop:fixed-label-determinant-perron,thm:class-product-determinant-rigidity};
the subsequent stability, obstruction, and quotient-shadow statements use
that readout without introducing further product constants.
...
\begin{theorem}[Finite-determinant readout for Adams-corrected constants]
...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\begin{proposition}[Finite determinant-data readout for the main theorem spine]
  ...
  readout statement from the finite determinant polynomials and the
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  From these finite data, the algebraic part of the readout consists only of
  coefficient extraction, character-table linear algebra, Adams-coefficient
  computation, and finite Fourier inversion.  The constants at the Perron
  boundary are then obtained by the explicitly specified logarithm branches
  and absolutely convergent determinant series of
  \Cref{thm:class-mertens-explicit,prop:fixed-label-determinant-perron,thm:class-product-determinant-rigidity}.
  With this convention, \(\mathfrak D(A,\tau;G)\) determines the complete
  output of the main theorem packages:
  ...
  \end{enumerate}
  In the last two items the phrase ``determines'' means this analytic readout
  from finite determinant polynomials, not evaluation by finitely many
  algebraic operations alone.  The values at \(z=\lambda^{-1}\) are exactly
  the Abel-path limits, normalized logarithm branches, and convergent series
  supplied by
  \Cref{thm:class-mertens-explicit,prop:fixed-label-determinant-perron,thm:class-product-determinant-rigidity};
  the singular scalar determinant logarithm is never evaluated, and the
  scalar normalisation uses the positive Perron constant \(C(A)\).  No
  secondary spectral-edge datum, scalar cyclic-lift record, or auxiliary
  appendix invariant is needed for this determination.  Conversely, after
...
\begin{proof}
  ...
  normalized Abel-path continuation stated in that proposition, and its
  determinant series is absolutely convergent after the local exclusion of the
  singular scalar Perron term.  Thus these constants are analytic functions of
  the finite determinant data with the specified branch convention, rather
  than finite algebraic expressions in the polynomial coefficients.  Then
...
\begin{corollary}[Readout consequence: rigidity of the Adams-corrected class-product constants]
  ...
  a coordinate readout in the fixed determinant model; equality of these
