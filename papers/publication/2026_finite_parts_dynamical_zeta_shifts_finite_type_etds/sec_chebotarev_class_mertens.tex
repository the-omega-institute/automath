...
\begin{lemma}[Explicit product-tail budget]
  \label{lem:class-euler-product-tail-budget}
  ...
  \]
  Then
  \begin{equation}\label{eq:class-product-tail-budget}
    T_C(N)=
      \sum_{r\ge2}\frac{\Pi_{\mathbf1_C}(\lambda^{-r})}{r}
      +O(\lambda^{-N}/N),
  \end{equation}
  uniformly in \(C\). Consequently, if
  \begin{equation}\label{eq:class-product-log-budget-input}
    \sum_{n\le N}\frac{p_{n,C}}{\lambda^n}
    =
    \frac{|C|}{|G|}\log N+\mathcal M_C(A,\tau;G)+O(N^{-1}),
  \end{equation}
  then
  \begin{equation}\label{eq:class-product-log-budget-output}
    \log P_C(N)=
    -\frac{|C|}{|G|}\log N
    -\mathcal M_C(A,\tau;G)
    -\sum_{r\ge2}\frac{\Pi_{\mathbf1_C}(\lambda^{-r})}{r}
...
\begin{proof}
  ...
  \eqref{eq:class-product-tail-budget}.  Taking logarithms in the finite
  Euler product gives
  \[
    \log P_C(N)=
    -\sum_{n\le N}\frac{p_{n,C}}{\lambda^n}-T_C(N).
  \]
  Substituting \eqref{eq:class-product-log-budget-input} and
  \eqref{eq:class-product-tail-budget} gives
  \eqref{eq:class-product-log-budget-output}, because
...
\begin{proof}
  ...
  \Cref{lem:class-euler-product-tail-budget}.  Substituting the logarithmic
  form \eqref{eq:class-mertens-log} from \Cref{cor:class-mertens-log-form}
  into \eqref{eq:class-product-log-budget-output} gives
  \[
    \log P_C(N)
    =
    -\frac{|C|}{|G|}\log N
...
\begin{lemma}[Singular-triple and branch ledger]
  \label{lem:fixed-label-singular-triple-ledger}
  ...
  \begin{equation}\label{eq:fixed-label-singular-ledger-abel}
...
\begin{proof}
  ...
  formula as \eqref{eq:fixed-label-singular-ledger-abel} with the scalar
...
\begin{proof}
  Apply \Cref{lem:fixed-label-singular-triple-ledger} to the Abel identity
  \eqref{eq:fixed-label-singular-ledger-abel}.  That lemma makes the local
  ...
  lies strictly inside the Perron disc.  The same ledger supplies the branch
