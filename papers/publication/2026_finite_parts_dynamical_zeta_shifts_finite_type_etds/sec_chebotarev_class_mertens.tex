...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\begin{proposition}[Finite-data readout for the main theorem spine]
  ...
  readout statement from the determinant and branch-compatible formulas already
  proved in Main Theorems~I--III.  The following data determine, by finite
  algebraic operations and the normalized determinant evaluations specified
  below, the complete output of those theorem packages:
  \begin{equation*}
    \begin{aligned}
      \mathfrak D(A,\tau;G):=
  ...
  \end{enumerate}
  In the last two items the values at \(z=\lambda^{-1}\) are exactly the
  Abel-path limits and normalized branches supplied by
...
\begin{theorem}[Quantitative Adams obstruction skeleton]
  ...
  \[
    ...
    :=-
      ...
      \qquad(C\subset G).
  \]
  Then the possible classwise Artin-replacement error is completely
  confined to the zero-mean class-function skeleton:
