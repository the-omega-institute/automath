...
This subsection separates the product theorem from its diagnostics.  The
core chain is
\[
\begin{gathered}
...
\Cref{thm:fixed-label-euler-transform,thm:class-euler-product-mertens}
prove Main Theorem~III, and
\Cref{prop:fixed-label-determinant-perron} evaluates the required
fixed-label Perron values from the twisted determinant package.  The single
finite-determinant message is
\Cref{thm:class-product-determinant-rigidity}: after the product asymptotic
has been proved, the finite-data closure, Artin obstruction,
hyperplane-classification, stability, and quotient-shadow statements are
readouts of that theorem rather than additional sources of constants.
...
\begin{corollary}[Logarithmic readout from Main Theorem II]
...
\begin{theorem}[Main Theorem III: Adams-corrected Frobenius-class
  ...
  logarithm \(\mathcal L_\varrho(\lambda^{-1})\); the local comparison and
  ...
  the branch-compatible Adams--M\"obius determinant readout of that
...
\begin{theorem}[Determinant rigidity and Artin obstruction for
  Adams-corrected constants]
  \label{thm:class-product-determinant-rigidity}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}.  After the
  classical Frobenius exponent and the scalar base normalisation have been
  removed, the Adams-corrected Frobenius-class product constants are a
  rigid finite-determinant readout: their non-scalar coordinates are exactly
  the fixed-label Perron values, and no further class information is present.
  Put \(\alpha_A:=\gamma+\log C(A)\), and define the normalised logarithmic
  product vector
  ...
  \end{equation}
  The
  Frobenius-class Euler-product vector \((K_C)_{C\subset G}\) of
  \Cref{thm:class-euler-product-mertens} is completely determined by the
  scalar normalisation \(C(A)\) and the finite twisted determinant family
  \[
    \{\det(I-zA_\pi):\pi\in\Irr(G)\}
    \]
  through the fixed-label Adams--M\"obius values
  \(F_\varrho(\lambda^{-1})\), \(\varrho\ne\mathbf1\), with the normalized
  Abel branches of \Cref{prop:fixed-label-determinant-perron}.
  More precisely, this is a classification theorem.  The map
  ...
  \end{equation}
  This is the stability and finite-data rigidity interface used by the later
  readout corollaries.

  More
  concretely, for \(M\ge1\) set
  ...
  Conversely, after
  the same scalar normalisation, any class-product vector satisfying
  \eqref{eq:class-product-normalisation-constraint} determines a unique zero-mean
  Peter--Weyl coordinate vector by
  \[
    \widehat H(\pi)
    =
    \sum_{C\subset G}\frac{|C|}{|G|}
  ...
  \(\widehat H(\pi)=F_\pi(\lambda^{-1})\).
  Thus the constants form an affine hyperplane whose scalar coordinate is
  fixed by the base shift, and the whole non-scalar coordinate system is the
  fixed-label Adams--M\"obius Peter--Weyl vector.  The scalar baseline
  \(K_C=\exp(-|C|\alpha_A/|G|)\) for all \(C\) occurs if and only if
  \(F_\varrho(\lambda^{-1})=0\) for every non-trivial \(\varrho\).
  If the displayed fixed-label values are replaced by regular periodic
  Artin logarithms \(\mathcal L_\varrho(\lambda^{-1})\), then the resulting
  class constants agree with the true constants for every conjugacy class
  if and only if
  \[
    F_\varrho(\lambda^{-1})=
    ...
    \qquad(\varrho\ne\mathbf1).
  \]
  Thus the classical part is only the Frobenius density
  \(|C|/|G|\), the exponent \(N^{-|C|/|G|}\), and existence of a
  class-product constant; the additional finite-state assertion proved in
  this article is the determinant recovery of the primitive-label
  fixed-Euler constant together with the Peter--Weyl obstruction to an
  Artin-logarithm substitution.

  In short, the Frobenius density and leading product exponent are the
  classical input.  The theorem proved here is the finite-state rigidity of
  the constant term: the Adams-corrected determinant coordinates are exactly
  the missing non-scalar coordinates, they are computably exhausted by
  \eqref{eq:class-product-finite-truncation-coordinate}--\eqref{eq:class-product-finite-truncation-error},
  and the obstruction to substituting periodic Artin logarithms is precisely
  the failure of these coordinates to agree.
\end{theorem}
...
\begin{proof}
  ...
  \(F_\varrho(\lambda^{-1})\).  Taking logarithms in that formula gives
  \eqref{eq:class-product-fourier-coordinates}.  Averaging this
  identity over conjugacy classes with weights \(|C|/|G|\) gives the second
  identity in \eqref{eq:class-product-normalisation-constraint}, because
  every non-trivial irreducible character is orthogonal to the trivial
  character.
  ...
  The same orthogonality proves the classification.  If
  ...
  forward determination of \((K_C)_C\).
  ...
  \]
  Taking logarithms in \eqref{eq:class-euler-product-k-f} gives
  \[
    \mathfrak K(C)=
    \sum_{\varrho\ne\mathbf1}
      \chi_\varrho^\ast(C)F_\varrho(\lambda^{-1}).
  ...
  formula for the coordinate \(\widehat H(\pi)\).  For the actual dynamical
  vector the preceding Fourier identity identifies this coordinate with
  \(F_\pi(\lambda^{-1})\), \(\pi\ne\mathbf1\).  This is also the inverse
  readout in \Cref{cor:class-product-constant-rigidity}.
  ...
  \(K_C=\exp(-|C|\alpha_A/|G|)\) for every class \(C\).

  Finally, \Cref{thm:adams-obstruction-class-product} compares the true
  fixed-label constants with the naive constants obtained by substituting
  \(\mathcal L_\varrho(\lambda^{-1})\).  Its ratio formula shows that all
  class constants agree if and only if the non-trivial Peter--Weyl
  expansion of the discrepancy is zero.  Since the irreducible characters
  form an orthonormal basis of class functions, this is equivalent to
  \(F_\varrho(\lambda^{-1})=\mathcal L_\varrho(\lambda^{-1})\) for every
  non-trivial \(\varrho\).  This proves both the determinant-recovery
  statement and the obstruction to Artin substitution.
\end{proof}

\begin{proposition}[Finite-data determination of the main constants]
  \label{prop:finite-data-main-spine-closure}
  Assume \(\lambda>1\) and the twisted spectral gap
  \(\eta<\lambda\) of \Cref{thm:class-mertens-explicit}.  The following
  data determine, by finite algebraic operations and the normalized
  determinant evaluations specified below, the complete output of Main
  Theorems~I--III:
  \begin{equation*}
    \begin{aligned}
      \mathfrak D(A,\tau;G):=
    ...
    \end{aligned}
  \end{equation*}
  More precisely, \(\mathfrak D(A,\tau;G)\) determines:
  ...
  \end{enumerate}
  In the last two items the values at \(z=\lambda^{-1}\) are the Abel-path
  limits in the non-trivial channels and the scalar normalisation uses the
  positive Perron constant \(C(A)\).  No secondary spectral-edge datum,
  scalar cyclic-lift record, or auxiliary appendix invariant is needed for
  this determination.  Conversely, after the scalar normalisation
...
\begin{proof}
  ...
  \(\Pi_\varrho(\lambda^{-1})\) and the formula
  \eqref{eq:class-mertens-constant} for
  \(\mathcal M_C(A,\tau;G)\).  The scalar term in that formula is
  ...
  the Abel limits supplied by the gap, the class-Mertens constants are
  determined.  This proves (ii).

  The fixed-label values are determined from the same determinant package by
  \Cref{prop:fixed-label-determinant-perron}; its branch convention is the
  normalized Abel-path continuation stated in that proposition, and the
  singular scalar Perron logarithm is absent in non-trivial channels.  Then
  \Cref{thm:class-euler-product-mertens} gives the constants \(K_C\) by
  \eqref{eq:class-euler-product-k-f}.  The inputs listed in the statement
  are exactly the inputs used in these formulas, so no secondary edge,
  cyclic-lift, or other appendix datum enters the determination.
...
\begin{proof}
  ...
  classwise.  This proves \textup{(C)} and completes the calibration.
\end{proof}
...
\begin{lemma}[Sign and normalisation audit for \texorpdfstring{$K_C$}{KC}]
  \label{lem:class-product-sign-normalisation-audit}
...
\begin{proof}
  ...
  \[
    ...
    =\gamma+
      \log C(A)+
      \sum_{\gamma_0\in\cP}
      \left(x_{\gamma_0}+\log(1-x_{\gamma_0})+
      \sum_{r\ge2}\frac{x_{\gamma_0}^r}{r}\right),
  \]
...
\begin{corollary}[Readout consequence: rigidity of the Adams-corrected class-product constants]
...
\begin{corollary}[Quantitative Adams obstruction budget]
  \label{cor:adams-obstruction-budget}
  Assume the hypotheses and notation of
  \Cref{thm:adams-obstruction-class-product}.  For
  \(\varrho\neq\mathbf1\), put
  \[
    D_\varrho
  ...
  \end{equation}
  In particular, if a finite certificate proves
...
\begin{proof}
  ...
  \[
    ...
    \sum_{C\subset G}\frac{|C|}{|G|}f(C)\overline{h(C)}.
  \]
  Since the expansion has no trivial-character term, Parseval's identity
  gives \eqref{eq:adams-obstruction-budget-parseval}.  If the right-hand
  side is at least \(\delta^2\), then the weighted average of
  \(|\mathcal E(C)|^2\) is at least \(\delta^2\). Because the weights
  \(|C|/|G|\) are positive and sum to one, some class has
  \(|\mathcal E(C)|^2\ge\delta^2\), which is exactly
...
\begin{corollary}[Readout consequence: determinant coordinates after scalar normalisation]
  ...
  coordinates.  Let \(\mathscr K_A(G)\) be the set of positive real
  ...
  coordinates.  Any real linear readout on the logarithmic product side that
...
\begin{corollary}[Readout consequence: stability in non-scalar coordinates]
...
\begin{remark}[On the twisted spectral-gap hypothesis]
  ...
  the hypothesis is kept as a separate stated assumption because this exact
  spectral input is what permits passage from the open disc to the Perron
  point. In the present finite-state setting it
