...
logarithm and use it to evaluate Frobenius-class product constants.  Third,
\Cref{thm:retargeted-adams-product-novelty,prop:class-product-comparison,thm:class-product-constant-rigidity,thm:adams-obstruction-class-product,thm:assembled-adams-frobenius-product-package}
record the determinant recovery, rigidity, obstruction, and hyperplane
classification consequences.  This order is the main theorem chain for the
Perron-point part of the paper: additive constants feed product constants,
and product constants retain exactly the fixed-label Adams--M\"obius data
that cannot be replaced by ordinary Artin logarithms.
...
\begin{theorem}[Main Theorem III: determinant rigidity and Artin
  obstruction for Adams-corrected constants]
  ...
  the same scalar normalisation, the class-product vector determines every
  non-trivial fixed-label value by
  \[
    F_\pi(\lambda^{-1})
    =
...
\begin{proof}
  ...
  \(F_\varrho(\lambda^{-1})\).  Each of these values is recovered from the
  finite twisted determinant family by
  \Cref{prop:fixed-label-determinant-perron}.  Substitution gives the
  asserted forward determination.
  ...
  formula for \(F_\pi(\lambda^{-1})\), \(\pi\ne\mathbf1\).  This is also
  the inverse readout in \Cref{thm:class-product-constant-rigidity}.
...
\begin{theorem}[Main Theorem III consequence: rigidity of the Adams-corrected
...
\begin{theorem}[Character-hyperplane classification of Frobenius-class
  product vectors]
  \label{thm:assembled-adams-frobenius-product-package}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}, and put
  \(\alpha_A:=\gamma+
  \log C(A)\).  Let \(\mathscr K_A(G)\) be the set of positive real
...
\begin{proof}
  ...
  For the dynamical constants, \Cref{thm:class-product-constant-rigidity}
