...
\Cref{prop:finite-data-main-spine-closure,prop:assembled-readout-dependency-order}
record the readout order without adding a new rigidity theorem.  The
...
thm:adams-obstruction-class-product,thm:adams-obstruction-budget,%
...
Parry--Pollicott, Noorani--Parry, and Mesliza--Noorani.  The new assertion
...
\begin{corollary}[Corollary-level minimal obstruction skeleton]
  ...
  following extremal interpretation: the determinant data in
  ...
  notation, by keeping the classical exponent \(|C|/|G|\), or by using only
  scalar fixed-matrix data.  This statement is a structural refinement of
  Main Theorem~III: it assumes the scalar-normalised product constants and
  their Adams-corrected Peter--Weyl boundary have already been identified,
  and it adds no new product asymptotic, determinant evaluation, or Perron
  hypothesis to the locked main theorem package.
\end{corollary}
...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\begin{proposition}[Finite determinant readout interface for the three main theorems]
  \label{prop:finite-data-main-spine-closure}
  Assume \(\lambda>1\) and the twisted spectral gap
  \(\eta<\lambda\) of \Cref{thm:class-mertens-explicit}.  This proposition
  is a readout interface from the finite determinant polynomials and the
  branch-compatible analytic formulas already proved in Main
  Theorems~I--III; it is not an additional rigidity theorem for cocycles or
  skew products.  The finite input data are
  \begin{equation*}
    \begin{aligned}
      \mathfrak D(A,\tau;G):=
...
\begin{proposition}[Dependency order of the assembled readout]
  \label{prop:assembled-readout-dependency-order}
...
\begin{proof}
  The proof of \Cref{prop:finite-data-main-spine-closure} gives the first
  arrow explicitly: the polynomials \(\det(I-zA_\pi)\) determine the trace
  coefficients at the origin, \Cref{thm:primitive-peter-weyl-trace} applies
  Adams--M\"obius inversion to recover the primitive Peter--Weyl
  coefficients, and \Cref{prop:class-indicator-expansion} gives the class
  counts.  This part is algebraic and uses no Perron-boundary continuation.

  The second arrow is exactly \Cref{thm:class-mertens-explicit}: under
  \(\eta<\lambda\), the normalized non-trivial determinant branches continue
  along the Abel path to \(\lambda^{-1}\), giving the non-trivial
  Perron-point primitive values and then the class-Mertens constants through
  \eqref{eq:class-mertens-constant}.  The scalar term is supplied by
  \(C(A)\) through \Cref{lem:finite-part-primitive-closed-form}, so the
  singular scalar determinant logarithm is not evaluated.

  The third arrow follows from \Cref{prop:fixed-label-determinant-perron},
  which evaluates every \(F_\varrho(\lambda^{-1})\) from the same finite
  determinant family with the normalized Abel branch, and from
  \Cref{thm:class-euler-product-mertens}, which substitutes these values in
  \eqref{eq:class-euler-product-k-f}.  The designation of
  \Cref{thm:class-product-determinant-rigidity} as the minimal rigidity
  theorem follows from its identities
  \eqref{eq:class-product-normalisation-constraint}--\eqref{eq:class-product-parseval}
  and its finite-truncation estimate
  \eqref{eq:class-product-finite-truncation-error}.  Those identities are
  precisely the zero-mean Peter--Weyl coordinate classification after the
  scalar normalisation has been fixed.

  Therefore equality of finite determinant packages gives equality at each
  successive stage of \eqref{eq:assembled-readout-dependency-order}.  The
  final equivalence between the product-constant vector and the non-trivial
  fixed-label Perron vector is the inverse Fourier identity
  \eqref{eq:comparison-fixed-label-inverse}, which is also the last assertion
  of \Cref{prop:finite-data-main-spine-closure}.  This proves the ordered
  factorisation and the readout status of the assembled interfaces.
\end{proof}
...
\begin{proposition}[Adams correction to Frobenius-class Mertens products]
  ...
  \begin{enumerate}[label=\textup{(\roman*)}]
    ...
    constant.
    \item The same separation locates these results
    against the modern comparison papers
    \cite{Coles2025VassilievWritheAxiomA,Humbert2025FirstRuelleResonance,%
    OHare2025FiniteDataRigidity,%
    CrismaleDelVecchioGrisetaRossi2025NoncommutativeSkewProduct}:
    \begin{enumerate}[label=\textup{(\alph*)}]
      \item Main Theorems~I--III are finite-state determinant-to-primitive,
      Perron-point, and Adams-corrected product-constant theorems; they do
      not assert a new geometric periodic-orbit invariant, a Ruelle-resonance
      theorem, a finite-data rigidity theorem, or an operator-algebraic
      skew-product classification.
      \item The shadow results
      \Cref{thm:quotient-functoriality,thm:abelian-shadow-defect,%
      thm:cyclic-detection-boundary} records quotient functoriality,
      abelian-shadow loss, and cyclic-detection boundaries for the same
      primitive class profile, rather than a reconstruction theorem from
      arbitrary finite samples.
      \item The eta-gap results
      \Cref{thm:eta-gap-rigidity,cor:eta-gap-boundary-classifier,%
      prop:eta-gap-loop-subgroup-obstruction} is a finite-matrix rigidity
      test for the hypothesis \(\eta<\lambda\), rather than a Banach-space
      resonance analysis.
      \item The \(S_3\) certificates
      ...
      are finite rational witnesses for the failure of naive Artin
      substitution at the product-constant level.
    \end{enumerate}
  \end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  For \textup{(E)}, Main Theorems~I--III have just been identified in
  \textup{(A)}--\textup{(D)} as finite determinant, Perron-point, and
  fixed-label product-constant statements.  The four cited modern works have
  the theorem scopes named in \textup{(E)(a)}: geometric observables for
  periodic orbits of Axiom~A flows, Ruelle resonances for Anosov flows,
  finite-data rigidity for expanding maps, and operator-algebraic
  non-commutative skew products.  None of these outputs is the fixed-label
  class Euler transform or the Adams-corrected constant in
  \eqref{eq:class-euler-product-k-f}.  The shadow, eta-gap, and \(S_3\)
  clauses in \textup{(E)(b)}--\textup{(E)(d)} are exactly the internally
  cited results; their statements show respectively that they are
  quotient/no-descent results for the same class profile, finite-matrix
  diagnostics for \(\eta<\lambda\), and explicit finite witnesses for the
  non-Artin product constant.  This proves the comparison with the cited
  literature.
\end{proof}
...
\begin{proposition}[Finite algorithm for fixed-label Perron evaluation]
  ...
  convergent truncation algorithm.
  \begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
    \item The determinant inputs are the finite character table of \(G\),
      the power maps \(g\mapsto g^m\) on conjugacy classes, and the finite
      determinant polynomials
      \(\det(I-zA_\pi)\), \(\pi\in\Irr(G)\).
    ...
    \item For \(M\ge1\), form the finite truncation
      \begin{equation}\label{eq:fixed-label-perron-finite-algorithm-truncation}
        ...
        \sum_{\substack{\pi\in\Irr(G)\\(r,m,\pi)\neq(1,1,\mathbf1)}}
          ...
          \Log\det(I-\lambda^{-rm}A_\pi)^{-1},
      \end{equation}
      with the branch convention in (d).  Then
      ...
      interval enclosures by truncating after any prescribed tail budget.
  \end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  displayed tail tends to zero exponentially.  Therefore the finite
  determinant truncations converge to the Perron value, and any prescribed
  enclosure follows by choosing \(M\) so that the explicit tail is below the
  requested tolerance and enclosing the finitely many determinant logarithms
  using their specified branches.
\end{proof}
...
\begin{theorem}[Main-spine rigidity against secondary-edge refinements]
  \label{thm:main-spine-secondary-noninterference}
...
\begin{proof}
  The additive constants \(\mathcal M_C(A,\tau;G)\) are already defined by
  \Cref{thm:class-mertens-explicit}; the formula there is exactly
  \eqref{eq:secondary-noninterference-mc}.  Taking the limit of
  \eqref{eq:secondary-noninterference-refined-sum} after subtracting
  \((|C|/|G|)H_N-(|C|/|G|)\gamma\) gives the same constant, because the
  hypothesis says \(T_C(N)\to0\) and the remaining error is \(o(1)\).  Thus
  the auxiliary finite exponential sums can change only finite-length
  compensated tails; they do not change the limiting class Mertens constants.

  With \(\mathcal M_C(A,\tau;G)\) fixed, \Cref{thm:class-euler-product-mertens}
  gives the Euler-product constant in the two equivalent forms
  \eqref{eq:class-euler-product-k-mc} and \eqref{eq:class-euler-product-k-f}.
  Formula \eqref{eq:secondary-noninterference-k} is the second of those forms.
  Its only non-scalar coordinates are the fixed-label Perron-boundary values
  \(F_\varrho(\lambda^{-1})\), and
  \Cref{prop:fixed-label-determinant-perron} evaluates those values from the
  finite twisted determinant package with the stated branch convention.
  Therefore no auxiliary secondary-edge parameter appears in the formula for
  \(K_C\).

  Finally, \Cref{cor:class-product-constant-rigidity} proves the converse
  Fourier recovery
  \[
    F_\pi(\lambda^{-1})
    =
    \sum_{C\subset G}\frac{|C|}{|G|}
      \left(-\frac{|G|}{|C|}\log K_C-\gamma-\log C(A)\right)
      \chi_\pi(C)
    \qquad (\pi\neq\mathbf1).
  \]
  Since the preceding paragraphs show that the auxiliary package leaves
  \((K_C)_C\) and \(C(A)\) unchanged, the recovered non-trivial fixed-label
  vector is unchanged as well.  This proves the asserted closure and
  non-interference of all secondary-edge refinements satisfying the displayed
  vanishing-tail condition.
\end{proof}
...
\begin{corollary}[Corollary-level product-constant Adams obstruction norm]
...
\begin{corollary}[Locked-spine status of the obstruction refinements]
  ...
  budget of \Cref{thm:adams-obstruction-budget} are determined functorially by
...
\begin{proof}
  ...
  \Cref{thm:adams-obstruction-budget} are all functions of the already fixed
