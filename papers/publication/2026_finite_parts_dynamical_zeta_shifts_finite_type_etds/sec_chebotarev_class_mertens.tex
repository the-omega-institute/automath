...
\Cref{prop:fixed-label-certificate-protocol,prop:fixed-label-certificate-manuscript-scope,thm:class-product-comparison} then separates the certified finite
manuscript proof interface, its \(S_3\) witness role, and the classical
...
record the formal finite-data closure and recovery order for the main theorem
chain.  The
...
\Cref{cor:class-product-artin-obstruction-recovery,thm:adams-obstruction-class-product,cor:adams-obstruction-bound,cor:main-iii-obstruction-refinements-downstream}; they compare the
...
For reference, the formal dependency chain of this subsection is recorded in
\Cref{prop:class-mertens-product-dependency-chain}.  It is placed at the
...
\begin{proposition}[Dependency chain for Main Theorems II--III]
...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\begin{proposition}[Finite determinant recovery closure for the main chain]
...
\begin{remark}[Dependency order of the assembled recovery]
  ...
  are packaging recoverys of Main Theorems~I--III, while
  ...
  In particular, equality of two finite determinant datas implies equality
...
\begin{proposition}[Finite rational certificate for Perron recovery]
  ...
  \Cref{prop:fixed-label-perron-finite-algorithm} is certified, for the finite
  determinant interfaces used in this manuscript, by the following printed
  finite data.  A certificate consists only of finite algebraic model data and
  ...
  mathematical certificate is the displayed finite record itself; any external
...
\begin{corollary}[Finite bookkeeping schema for the fixed-label Perron recovery]
...
\begin{proposition}[Manuscript scope of the fixed-label certificate interface]
  \label{prop:fixed-label-certificate-manuscript-scope}
  The certificate protocol of
  \Cref{prop:fixed-label-perron-finite-algorithm,prop:fixed-label-certificate-protocol,cor:fixed-label-perron-certificate-schema}
  is exactly a finite proof interface for the determinant-coordinate values
  used in Main Theorem~III and for the printed \(S_3\) witness in the
  appendices.  More precisely, it proves the following three statements and no
  broader software-infrastructure claim.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    \item For the fixed finite matrices \(A_\pi\), fixed character table, fixed
      branch convention, and fixed tolerance appearing in this manuscript, the
      finite records of \Cref{prop:fixed-label-certificate-protocol} are
      sufficient to verify rational enclosures for
      \(F_\varrho(\lambda^{-1})\) and hence, by finite Fourier inversion, for
      the class-product constants of \Cref{thm:class-euler-product-mertens}.
    \item The \(S_3\) appendix records instantiate this finite interface as a
      witness to the non-abelian Artin-substitution obstruction; they add no
      new analytic hypothesis to Main Theorem~III.
    \item Nothing in the protocol asserts a general-purpose certified
...
\begin{proof}
  Statement (i) is a direct combination of the two preceding finite-interface
  results.  \Cref{prop:fixed-label-certificate-protocol} proves that the four
  finite records enclose the truncated determinant sum and its discarded tail,
  giving the inclusion
  \eqref{eq:fixed-label-certificate-interface-inclusion} for each non-trivial
  \(F_\varrho(\lambda^{-1})\).  The values involved are precisely the
  fixed-label determinant coordinates appearing in
  \Cref{prop:fixed-label-determinant-perron}.  Applying the finite Fourier
  inversion of \Cref{cor:class-product-constant-rigidity} then transports those
  enclosures to the logarithms of the class-product constants in
  \Cref{thm:class-euler-product-mertens}.  No limiting orbit data or external
  numerical assumption is introduced in this passage.

  Statement (ii) follows from
  \Cref{cor:fixed-label-perron-certificate-schema}: the appendix files are
  serialized arithmetic transcripts for the same five bookkeeping fields.  The
  ...
  For (iii), inspect the inputs permitted in
  ...
  \Cref{prop:fixed-label-certificate-protocol} checks only algebraic equalities,
  ...
  certification theory for all finite groups and matrices.  Thus the protocol
  is a manuscript-level proof interface and an \(S_3\) witness interface, not a
...
\begin{corollary}[Main-theorem status of the obstruction refinements]
  ...
  obstruction bound.  Conversely, equality of the scalar-normalised product
