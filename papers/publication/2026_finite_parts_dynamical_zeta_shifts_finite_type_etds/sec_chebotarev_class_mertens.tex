...
\Cref{prop:fixed-label-determinant-perron,prop:fixed-label-perron-finite-algorithm,thm:class-product-determinant-rigidity,prop:class-product-fixed-coordinate-equivalence};
...
\begin{proposition}[Three logarithmic interfaces at the product constant]
  ...
  last sentence sharp: after the scalar coordinate is fixed, every bad
  replacement of the determinant boundary data is exactly a non-zero vector
  in the \((q-1)\)-dimensional zero-mean class-function skeleton, where
  \(q\) is the number of conjugacy classes of \(G\), and no scalar,
  exponent-level, or notation-level record can hide it.
\end{proposition}
...
\begin{theorem}[Finite-determinant recovery of Adams-corrected constants]
  ...
  \end{equation}
  The vector \((K_C)_{C\subset G}\) is therefore determined by precisely the
  finite determinant-coordinate input listed in the first sentence.  This is a
  rigidity statement only for the Adams-corrected constant vector in those
  fixed coordinates and branch/scalar normalisation: it does not assert
  invariance under arbitrary presentations, covers, folds, gauge changes, or
  conjugacies of the cocycle \(\tau\) or of the skew product.  More
  concretely, for
  \(M\ge1\) set
...
\begin{proposition}[Fixed-coordinate equivalence class for product constants]
  ...
  and the same normalized Abel branch convention.  If their non-trivial
  fixed-label Perron vectors agree,
  \[
  ...
  coordinate vector modulo this finite Fourier equivalence, not a presentation-,
  cover-, or fold-invariant classification of the underlying cocycle.
\end{proposition}
...
\begin{proof}
  ...
  character table and the fixed scalar/branch normalisation, so no conclusion
  about alternative presentations or extensions is introduced.
\end{proof}
...
\begin{corollary}[Hyperplane classification and stability]
  ...
  \begin{equation}\label{eq:class-product-coordinate-map-inverse}
    ...
    \qquad \pi\ne\mathbf1 .
  \end{equation}
  Hence, in the fixed finite-coordinate model with the same scalar
  normalisation and branch convention, two admissible data sets have the same
...
\begin{proof}
  ...
  non-trivial \(\varrho\).
\end{proof}
\begin{proposition}[Finite determinant data determine the main-chain outputs]
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  Together with the Abel-path branch convention fixed in
  \Cref{thm:class-mertens-explicit,prop:fixed-label-determinant-perron}, this
  finite datum determines, and is used here only to determine, the following
  outputs of the three main theorems:
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
...
\begin{proposition}[Finite certificate theorem for fixed-label product data]
  ...
  \Cref{prop:fixed-label-certificate-protocol}.  If these finite records pass
  ...
  the logarithms of all class-product constants \(K_C\).  This is a
  reproducibility interface for exactly the displayed certificate records and
  the appendix \(S_3\) witnesses that instantiate them.  Thus the statement
  proves the following four assertions and no broader
  software-infrastructure claim.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
...
\begin{proof}
  ...
  \Cref{prop:fixed-label-certificate-protocol}.  That proposition proves that
  the four
