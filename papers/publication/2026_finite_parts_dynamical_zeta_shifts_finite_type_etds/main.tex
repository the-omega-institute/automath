...
\(\mathcal E_g(y)=\sum_{n\ge1}p_{n,g}\log(1-y^n)\).  Under
\textup{(KN85)} and without a twisted-gap hypothesis, equality of every
element profile at
\[
...
unable to consult; the exact statement required is quoted in the paper.  The
sharp squarefree Mahler bound, the collision--jet inequality, the odd-prime
multi-collision theorem, and the sampling lower bound require no such input.
Determinant parity and the
