...
\begin{proof}
  ...
  cor:class-product-stability-scalar-independence}; see
...
\begin{theorem}[Scalar interface for trivial-channel normalisation]
  ...
  canonical cyclic-lift record named in \Cref{app:cyclic-lift-package}.
...
\begin{corollary}[Rank-zero scalar budget for the main theorem chain]
  \label{cor:scalar-record-rank-zero-budget}
  ...
  named in \Cref{app:cyclic-lift-package} have rank zero on
  \(\mathcal Z(\mathscr T)\): they are constant on \(\mathscr T\) and
  annihilate all profile differences.

  Consequently, any finite package of real linear invariants whose restriction
  to profile differences is linear and which, together with the scalar
  records, recovers the Main Theorem~II normalised class-Mertens profile on
  \(\mathscr T\) must have real rank at least
  \(\dim\mathcal Z(\mathscr T)\) on the non-trivial Peter--Weyl sector.  The
  same lower bound applies to any such linear recovery package for the Main
  Theorem~III product constants \((K_C)_{C\subset G}\) after the scalar
...
\begin{proof}
  ...
  Let \(I\) be any additional finite real invariant package whose restriction
