...
Theorems~II--III. We therefore keep the finite-part formula and the
body-level scalar projection boundary in the main text, and record the
fixed-matrix cyclic-lift recovery and scalar-certificate calculations in
\Cref{app:cyclic-lift-comparison,app:scalar-boundaries}. These auxiliary
statements are part of the compiled article as scalar support and are
kept separate from the Adams--M\"obius primitive-inversion theorem chain.
...
root-of-unity layers of \(F_A\) are fixed-matrix records. Their longer
reconstruction package is retained in \Cref{app:cyclic-lift-comparison}; the
main text uses such scalar records only through the following boundary
statement.
...
\noindent
The fixed-matrix cyclic-lift reconstruction and one-layer Fourier inversion
package is retained in \Cref{app:cyclic-lift-comparison} as auxiliary scalar
support. Its role here is bounded by the structural warning already proved
above: scalar records identify a fixed scalar comparison model, not the
non-trivial branch-compatible class-product coordinates. The product-side
finite-state obstruction is recorded in
