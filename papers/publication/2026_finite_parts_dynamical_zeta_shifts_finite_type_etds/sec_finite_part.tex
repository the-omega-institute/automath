...
This section records only the finite-part formula and the scalar projection
boundary needed for that normalisation.  Fixed-matrix cyclic-lift and
root-of-unity records appear only as comparison records for this scalar layer.
...
statement isolates their scalar normalisation role for
\Cref{thm:renormalized-class-mertens,thm:renormalized-frobenius-product} and
the strict-gap specialization.
...
\begin{remark}[Fixed-matrix scalar comparison data and the trivial channel]
  ...
  package and its strict-gap specialization.  In the setting of \Cref{thm:finite-part-scalar-interface-no-descent},
...
\begin{corollary}[Scalar comparison is constant on class-profile differences]
  ...
  scalar comparison data, recovers the normalised class-Mertens profile in the
  strict-gap specialization on \(\mathscr T\) must have real rank at least
