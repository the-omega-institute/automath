...
body-level scalar projection boundary, but leave the cyclic-lift recovery
and scalar-certificate packages out of the compiled article. Those
fixed-matrix calculations are recorded in \texttt{split\_candidates.json}
as companion-note material and are not part of the Adams--M\"obius
primitive-inversion proof.
...
reconstruction package is split from this compiled article; the main text uses
such scalar records only through the following boundary statement.
...
inversion package has been left out of the compiled article.  Its role here is
only the structural warning already proved above: scalar records identify a
fixed scalar comparison model, not the non-trivial branch-compatible
class-product coordinates.  The product-side finite-state obstruction is
recorded in \Cref{cor:class-product-stability-scalar-independence}.
