...
projection boundary needed for that normalization. Broader fixed-matrix
cyclic-lift recovery, one-layer Fourier reconstruction, DFT recovery, and
non-abelian root-of-unity shadow questions are not theorem obligations in this
paper and are not part of the Adams--M\"obius primitive-inversion proof.
...
non-trivial class-product coordinates or cross-resolution invariants: by
...
through a scalar reconstruction, DFT-recovery, truncation-certificate, or
descent theorem.
...
\begin{corollary}[Only the trivial channel descends from fixed-matrix scalar data]
  ...
  \(\mathcal B_{A,\tau;G}\).  Thus scalar finite-part formulas and canonical
  cyclic-lift scalar records enter the finite-group theorem chain only through
  the trivial-channel normalisation \(\mathcal M(A)\).  They determine the
  non-trivial class profile on a family \(\mathscr T\) only in the independence
  case already characterized in
...
\begin{proof}
  ...
  \(\mathcal B_{A,\tau;G}\).  Every scalar finite-part or canonical cyclic-lift
  record named here depends only on \(A\), hence is constant as \(\tau\) varies
  in \(\mathscr T\).  Such a record can therefore determine a single
  non-trivial profile on the whole family precisely when the profiles
  \(\mathcal B_{A,\tau;G}\) are independent of \(\tau\), which is the
...
\begin{corollary}[Rank-zero scalar obstruction for class profiles]
  ...
  scalar records, recovers the normalised class-Mertens profile of Main
...
inversion, and reusable scalar certificate algorithms require hypotheses beyond
the scalar normalisation used here.  In this article the cyclic-lift material has
only the structural role proved above: scalar records identify a fixed scalar
comparison model, not the non-trivial branch-compatible class-product
coordinates.  The product-side finite-state obstruction is recorded in
