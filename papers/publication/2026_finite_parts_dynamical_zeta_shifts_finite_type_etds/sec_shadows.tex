...
\begin{theorem}[Structural consequence: quotient functoriality]
  ...
  \end{equation}
\leanverified{paper_etds_quotient_functoriality}
\end{theorem}
...
\leanverified{paper_etds_abelian_cyclic_shadow}
\end{corollary}
...
\begin{theorem}[Structural consequence: abelian shadow and genuine
  ...
  \end{enumerate}
\leanverified{paper_etds_abelian_shadow_defect}
\end{theorem}
...
\begin{lemma}[Cyclic quotients and one-dimensional characters]
  ...
  \]
\leanverified{paper_etds_cyclic_quotients_one_dimensional_characters}
\end{lemma}
...
\leanverified{paper_etds_cyclic_detection_boundary}
...
\begin{proof}
  ...
  \(\log((1+u)/(1-u))\). Applying this with \(J=3\) to every rational
  logarithm in \(E_7\) and \(R_{15}\) gives the following exact rational
  certificate.
  \begin{center}
  \small
