...
\begin{proof}
  The orthonormal basis vectors here are the conjugated character
  values \(C\mapsto \overline{\chi_\varrho(C)}=\chi_\varrho(C^{-1})\),
  not the unconjugated values \(C\mapsto \chi_\varrho(C)\) unless the
  character is real-valued. Accordingly, the functions
  \(C\mapsto \chi_\varrho(C^{-1})\) are orthonormal for this Hermitian
  inner product, since
  \[
...
\begin{proposition}[Primitive channels for the \(S_3\)-example]
  ...
  \end{equation}
  and the exact rational enclosure
  \[
    -\frac{341079}{10^6}-\frac{1}{1105920}
    <\Pi_\varepsilon(1/2)<
    -\frac{341071}{10^6}+\frac{1}{1105920}
  \]
  is proved in \Cref{lem:s3-example-perron-certificates}.
...
\begin{proof}
  ...
  convergent series at \(z=1/2\) yields the decimal value in
  \eqref{eq:s3-example-pi-eps-value}; the exact rational enclosure
  quoted after it is proved in
  ...
  value in \eqref{eq:s3-example-pi-rho-value}; the exact rational
  enclosure quoted after it is proved in
  \Cref{lem:s3-example-perron-certificates}.
\end{proof}
...
\begin{proof}
  ...
  It remains only to enclose the finite logarithmic sums. For each
  rational \(q>1\) appearing in the displayed formulas for \(E_7\) and
  \(R_{15}\), put \(u=(q-1)/(q+1)\). Using the \(J=3\) truncation of the
  positive series for \(\log((1+u)/(1-u))\), one has
  \[
    ...
    +\frac{2u^{9}}{9(1-u^2)}.
  \]
  Appendix~\ref{app:s3-log-certificates} lists the resulting exact
  rational lower and upper bounds for every logarithm in \(E_7\) and
  \(R_{15}\), together with the coefficient-weighted intervals and their
  sums. Summing those rational intervals yields
  \[
  ...
  \eqref{eq:s3-example-rho-tail-certificate} gives exactly
  \eqref{eq:s3-example-certified-intervals-sharp} and
  \eqref{eq:s3-example-certified-intervals-sharp-rho}. These sharp
  intervals lie inside the coarse windows
  \eqref{eq:s3-example-certified-intervals}, so both primitive channels
  are non-zero.
\end{proof}
...
\(\Delta_T(A,\tau;S_3)\approx 0.1705389472374253\) \\
\bottomrule
\end{tabular}
\end{center}
