...
\begin{theorem}[Finite-state projection boundary for scalar inputs]
  ...
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}, and set
  \[
    \mathcal M^\#_{A,\tau;G}(C)
    :=\frac{|G|}{|C|}\mathcal M_C(A,\tau;G)
  \]
  for conjugacy classes \(C\subset G\). In the Hermitian class-function
  space of \(G\),
  ...
  The auxiliary fixed-matrix data
  \(\FP(A)\), \(\mathcal M(A)\), \(F_A\), \(\{\Psi_q(A)\}_{q\ge1}\), and
  the root-of-unity layers of \(\Cref{sec:finite-part}\) therefore
  supply only the scalar projection in
  \eqref{eq:scalar-normalisation-projection}. They do not determine, and
  cannot replace, the non-trivial class-profile vector
  \(\mathcal B_{A,\tau;G}\) without extra data tying those scalar records
  to the fixed cocycle \(\tau\).

  More precisely, fix \(A\) and \(G\), and let \(\mathscr T\) be any
  ...
  \begin{enumerate}[label=\textup{(\roman*)}]
    \item Every scalar fixed-matrix record depending only on
      \(A\), including
      \[
        \FP(A),\quad \mathcal M(A),\quad F_A,\quad
        \{\Psi_q(A)\}_{q\ge 1},
      \]
      determines the full normalised class profile
      \(\mathcal M^\#_{A,\tau;G}\) uniformly for \(\tau\in\mathscr T\).
    \item The non-trivial class-profile vector
      \(\mathcal B_{A,\tau;G}\) is independent of
      \(\tau\in\mathscr T\).
    \item For every non-trivial irreducible representation
      \(\varrho\in\Irr(G)\), the Perron-point primitive channel
      \(\Pi_\varrho^\tau(\lambda^{-1})\) is independent of
      \(\tau\in\mathscr T\).
  \end{enumerate}
  Hence any two gap cocycles over the same \(A\) with different
  non-trivial Perron-point primitive channels form a finite-state
  obstruction to scalar normalisation: all scalar fixed-matrix records
  agree for the pair, but the normalised primitive class profiles do not.
\end{theorem}
...
\begin{proof}
  ...
  \(\tau\in\mathscr T\). Thus such records determine
  \(\mathcal M^\#_{A,\tau;G}\) uniformly on \(\mathscr T\) if and only if
  ...
  \(\varrho\neq\mathbf 1\). The final obstruction statement follows by
