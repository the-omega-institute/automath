...
\begin{equation}\label{eq:abelian-shadow-defect}
  \begin{aligned}
    ...
    \sum_{\substack{\chi\in\Irr(G)\\ \dim\chi=1,\ \chi\neq \mathbf 1}}
      \overline{\chi(C)}\,\Pi_\chi(\lambda^{-1}), \\
...
\begin{proof}
  ...
  values \(C\mapsto \overline{\chi_\varrho(C)}\). By unitarity,
  \(\overline{\chi_\varrho(C)}=\chi_\varrho(C^{-1})\), but we keep the
  conjugated notation to avoid suggesting inversion-invariance of the
  unconjugated class value outside the real-valued case.
  These basis vectors are orthonormal for this Hermitian inner product,
  since
  \[
    \sum_{C\subset G}\frac{|C|}{|G|}
      \overline{\chi_\varrho(C)}\,\overline{\overline{\chi_\sigma(C)}}
    =
    \sum_{C\subset G}\frac{|C|}{|G|}
  ...
  \[
    \begin{aligned}
      ...
      \frac{|C|}{|S_3|}
        \Bigl(
          \Tr(A^n)
          +\overline{\chi_\varepsilon(C)}\Tr(A_\varepsilon^n)
          +\overline{\chi_\rho(C)}\Tr(A_\rho^n)
        \Bigr) \\
      &=
      \frac{|C|}{|S_3|}
        \Bigl(2^n+\overline{\chi_\rho(C)}(-1)^n\Bigr),
    \end{aligned}
  \]
  ...
  \begin{enumerate}[label=\textup{(\roman*)}]
    \item The non-trivial cyclic quotient shadow is exactly the sign
      ...
      \[
        ...
        \mathcal B_{A,\tau;G}^{\mathrm{ab}}(C)
        =
        \overline{\varepsilon(C)}\,\Pi_\varepsilon(1/2).
      \]
    ...
    \item The genuine non-abelian defect is the standard channel:
      \[
        \mathcal B_{A,\tau;G}^{\mathrm{na}}(C)
        =
        \overline{\chi_\rho(C)}\,\Pi_\rho(1/2),
        \qquad
        -\frac{719}{1000}
        <\Pi_\rho(1/2)<-\frac{718}{1000}.
      \]
