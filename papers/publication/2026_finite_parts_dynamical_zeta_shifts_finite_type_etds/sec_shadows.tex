...
This section identifies exactly which part of the primitive class
profile is already visible after abelianisation and which part is
genuinely non-abelian. Quotient functoriality shows that the primitive
profile behaves naturally under surjections \(G\twoheadrightarrow Q\),
while the orthogonal splitting into one-dimensional and
higher-dimensional Peter--Weyl channels yields an intrinsic
decomposition into abelian shadow and non-abelian defect. The
\(S_3\)-example shows that this defect is genuine: a channel may be
invisible at the periodic level and still contribute non-trivially to
primitive bias.
These results are structural consequences of the finite-group main
theorems from \Cref{sec:chebotarev}. They do not use the scalar
finite-part or one-layer DFT arguments to produce non-abelian primitive
data. There are two cyclic notions in the paper, and they are kept
separate throughout this section: a cyclic quotient of a fixed cocycle
produces a quotient primitive profile, while the root-of-unity layer of
\Cref{sec:finite-part} belongs only to the canonical cyclic-lift cocycle
of \Cref{rem:abelian-toy-model-cyclic}. No cyclic quotient of a general
abelianised cocycle is identified with that layer.
...
\begin{proof}
  ...
  \(G\). Applying \Cref{thm:quotient-functoriality} to \(q_{\mathrm{ab}}\)
  gives
  \[
