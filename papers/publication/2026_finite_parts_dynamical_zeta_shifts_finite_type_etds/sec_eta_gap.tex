...
The classifier \Cref{cor:eta-gap-boundary-classifier} records what any
boundary failure would have to look like on loop labels.
Fix a vertex \(i\). If
\[
...
\]
on the loop labels arising from closed paths based at \(i\). Thus
equality at the boundary would force a one-dimensional constituent
in the restricted loop-label action of the offending representation.
This observation is used here only as a boundary certificate. Turning it
into broad structural hypotheses that guarantee \(\eta<\lambda\) is a
separate strict-gap problem, not part of the theorem chain of this
paper.
