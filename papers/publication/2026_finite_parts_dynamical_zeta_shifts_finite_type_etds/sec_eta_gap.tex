...
\begin{theorem}[Diagnostic rigidity at the Perron boundary]
  ...
  theorem is only a boundary diagnostic for the fixed finite-state cocycle just
  specified: it identifies exactly when equality \(\rho(A_\varrho)=\lambda\)
  occurs in a non-trivial channel, and it does not supply an independent
  mechanism forcing \(\eta<\lambda\) for arbitrary cocycles.
\end{theorem}
