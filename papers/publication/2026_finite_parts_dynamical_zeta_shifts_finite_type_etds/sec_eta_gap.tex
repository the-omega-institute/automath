...
\begin{corollary}[Diagnostic boundary for Perron-point obstruction]
  ...
  \end{enumerate}
  In particular, within this fixed datum the strict-gap endpoint chart is
  available exactly when these diagnostics eliminate every possible boundary
  line field.  The general
...
\begin{corollary}[Perron-point availability for the main constants]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
  \item The finite-state diagnostic certificate of
      \Cref{prop:eta-gap-finite-state-certificate} is a pass/fail check for
      the fixed cocycle.  It succeeds if and only if the
  ...
  \end{enumerate}
  The explicit line-field exclusions proved in this section are only the
  finite-state checks for this strict-gap Perron-point gate.
\end{corollary}
...
\begin{remark}[Strict-gap diagnostic boundary]
  ...
  formulas.  They are finite diagnostics for the present fixed datum, not a
  general gap-forcing theory for variable cocycles.  More
