...
\begin{proposition}[Perron-point firewall for Main Theorems II--III]
  \label{prop:eta-gap-perron-firewall}
  ...
  \item For every non-trivial class function \(\phi\) with zero average on
    ...
    \Cref{thm:class-mertens-explicit,thm:class-euler-product-mertens} are
    computed entirely inside the finite determinant package.
  \end{enumerate}
...
\begin{proof}
  ...
  are available from the determinant package precisely when the
...
\begin{corollary}[Boundary obstruction is the only eta-gap export]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item or a non-trivial boundary line field exists, equivalently
      ...
      the Perron circle, so the Perron-point determinant readout required by
...
\begin{proof}
  ...
  second alternative, \Cref{prop:eta-gap-perron-firewall} shows that the same
...
\begin{corollary}[Eta-gap diagnostic interface]
  ...
  \Cref{prop:eta-gap-perron-firewall}.  This section has no further input
...
\begin{proof}
  ...
  \Cref{prop:eta-gap-perron-firewall} identifies this same condition with
