...
\(0\).  This section gives finite-state criteria for that strict-gap branch
and describes the equality case \(\eta=\lambda\).  The equality case is rigid
...
loop-character criteria feed \Cref{cor:strict-gap-specialization} and the older
...
\begin{proposition}[Finite-state certificate for the Perron-point
  ...
  specialization may require the gap operationally by asking for either the
  spectral-radius check in \textup{(ii)}/\textup{(iv)} or the line-field
...
\begin{corollary}[Line fields are the only Perron-boundary obstruction]
  ...
  section export exactly the following finite-state alternative to the
  strict-gap endpoint chart:
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item or a non-trivial boundary line field exists, equivalently
      ...
      the Perron circle, marking failure of the ordinary Perron-point gate
      for the strict-gap class-Mertens and class-product constants.
  \end{enumerate}
  In particular, the stated hypotheses give strict-gap criteria exactly when
  they eliminate every possible boundary line field.  The general
  Perron-boundary product theorem uses the certified peripheral data instead
  of this strict exclusion.
\end{corollary}
...
\begin{proof}
  ...
  non-trivial ordinary Perron branches used by the strict-gap product formula
  are available precisely in the first alternative.  The displayed line-field
  equations are the only obstruction data exported from this section to the
  strict-gap product-constant chart.
\end{proof}
...
\begin{corollary}[Perron-point availability for the main constants]
  ...
  paper, the eta-gap section supplies \Cref{cor:strict-gap-specialization} and
  the strict-gap computational formulas with the following boundary interface:
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    \item The finite-state criterion of
      ...
      strict-gap specialization hypothesis \(\eta<\lambda\) is available and if and only if
    ...
    \item On the failed branch, a non-trivial boundary line field exists and
      ...
      the branch-compatible Perron-point vector needed for the strict-gap
      computational formulas is absent, while Main Theorem~I remains unchanged because it uses
      open-disc determinant identities.
      The failed branch is therefore a boundary obstruction for the fixed
      cocycle's ordinary endpoint chart; with a finite peripheral
      semisimplicity certificate it is precisely the situation treated by
      the renormalized Perron-boundary theorems.
  \end{enumerate}
  The explicit line-field exclusions proved in this section are the finite-state
  checks for this strict-gap Perron-point gate.
\end{corollary}
...
\begin{proof}
  ...
  branch-compatible Perron-point vector required by the strict-gap
  computational formulas is absent.  The open-disc determinant
  identities of Main Theorem~I do not evaluate at \(\lambda^{-1}\), so they are
  unaffected.  The renormalized boundary theorem uses reduced finite parts
  rather than the absent ordinary endpoint values.  This proves item~(iii).
...
\begin{remark}[Strict-gap criteria boundary]
  ...
  \(\eta=\lambda\) by a finite line-field test and provide the hypothesis check
  needed for \Cref{cor:strict-gap-specialization} and the older strict-gap formulas.  They are finite criteria for the present
  fixed datum, not a general gap-forcing theory for variable cocycles.  More
  elaborate loop-subgroup and representation-theoretic sufficient-condition
  packages are deliberately outside the scope of this article.
\end{remark}
