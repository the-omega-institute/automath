...
\begin{proof}
  ...
  equation. Fix a vertex \(i\).  The equality case in the norm triangle
  inequality says that all non-zero summands
  \(\varrho(\tau(e))v_j\), \(e:i\to j\), have one common argument in the
  Hilbert space: there is a unit vector \(y_i\) such that
  \(\varrho(\tau(e))v_j=u_jy_i\) for every outgoing edge.  Since their
  sum is \(\alpha v_i\) and \(Au=\lambda u\),
  \[
    \alpha v_i
    =\sum_{e:i\to j}u_jy_i
    =\lambda u_i y_i,
