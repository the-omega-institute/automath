...
\begin{corollary}[Perron-point availability for the main constants]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    \item The operational finite-state certificate of
      \Cref{prop:eta-gap-finite-state-certificate} succeeds if and only if
      the strict-gap hypothesis \(\eta<\lambda\) is available and if and only
      if all non-trivial determinant logarithms have the Perron-point branches
