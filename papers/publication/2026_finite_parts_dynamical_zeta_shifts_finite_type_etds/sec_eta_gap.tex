...
a non-trivial Perron-boundary channel.  Thus the boundary criterion below is
...
\begin{proof}
  ...
  its immediate structural specialization.  None of these boundary criteria
...
\begin{corollary}[Line fields are the only Perron-boundary obstruction]
  ...
  fixed finite input.  They are not a general structural classification of
  cocycles with \(\eta<\lambda\), except where the stated hypotheses explicitly
  eliminate every possible boundary line field.
\end{corollary}
...
\begin{corollary}[Perron-point availability for the main constants]
  ...
  paper, the eta-gap section exports to Main Theorems~II--III exactly the
  following boundary interface:
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item On this successful branch, every non-trivial fixed-label value
      ...
      Thus the eta-gap section contributes only an availability flag; it
      contributes no independent numerical coordinate to the product vector.
    \item On the failed branch, a non-trivial boundary line field exists and
      ...
      is not supplied by the hypotheses of this paper, while Main Theorem~I
      remains unchanged because it uses only open-disc determinant identities.
      The failed branch is therefore a certificate of obstruction for the
      fixed cocycle, not an invitation to choose alternate Perron-point
      branches.
  \end{enumerate}
  Broader structural criteria forcing \(\eta<\lambda\) for general cocycle
  classes are outside the scope of this article; only the explicit line-field
  exclusions proved in this section are retained as finite-state checks.
\end{corollary}
...
\begin{proof}
  ...
  the scalar normalisation, not of any additional line-field datum.  Therefore
  the eta-gap section has rank zero as a numerical source for the product
  constants: after success it supplies only the permission to evaluate the
  already fixed determinant branches.  This proves item~(ii).
  ...
  absent from the hypotheses of this paper.  The open-disc determinant
  identities of Main Theorem~I do not evaluate at \(\lambda^{-1}\), so they are
  unaffected.  This proves item~(iii).  The final scope sentence follows from
  \Cref{cor:eta-gap-only-boundary-export,prop:eta-gap-conditional-scope}, which
  state that this section proves only the finite alternative and its explicit
  line-field exclusions, not a broad sufficient-condition theory.
\end{proof}

\begin{proposition}[Fixed-cocycle scope of the eta-gap certificate]
  \label{prop:eta-gap-conditional-scope}
  The eta-gap results in this section give, for the finite edge cocycle fixed
  in the paper, exactly the following finite equality test and Perron-point
  gate, and no broader classification of gap-producing cocycles.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item If the certificate fails, then some non-trivial irreducible channel
      ...
      Theorem~I.
    \item The only sufficient implications for the strict inequality proved
      here are fixed-cocycle eliminations of the classified boundary line
      fields, such as the loop-character obstruction and the
      perfect-loop-group consequence.  Structural sufficient criteria forcing
      \(\eta<\lambda\) for general cocycle classes form a separate
      classification problem and are not used as hypotheses, conclusions, or
      hidden inputs in this article.
  \end{enumerate}
\end{proposition}

\begin{proof}
  The finite-state certificate is equivalent to the strict gap by
  \Cref{prop:eta-gap-finite-state-certificate}.  Under that equivalent
  condition, \Cref{prop:eta-gap-perron-separation} supplies exactly the
  branch-compatible Perron-point determinant values used in the class-Mertens
  theorem and in the Adams-corrected class-product theorem, proving item~(i).

  If the certificate fails, then
  \Cref{prop:eta-gap-finite-state-certificate,thm:eta-gap-rigidity} give a
  non-trivial boundary line field.  By \Cref{prop:eta-gap-perron-separation},
  this line field is equivalent to a non-trivial twisted determinant pole on
  the Perron circle.  The hypotheses needed for the Perron-point evaluations in
  \Cref{thm:class-mertens-explicit,thm:class-euler-product-mertens} are
  therefore absent, while the open-disc determinant identities of Main
  Theorem~I remain inside their domain of proof.  This proves item~(ii).

  The last assertion follows from
  \Cref{cor:eta-gap-boundary-classifier,cor:eta-gap-only-boundary-export}:
  every obstruction at the boundary is one of the classified line fields, and
  the sufficient criteria proved here are precisely hypotheses that exclude
  those line fields for the fixed finite input.  No statement in the cited
  chain classifies all cocycle classes with strict gap, so no such structural
  classification is exported to the main theorem chain.  This proves item~(iii).
\end{proof}

\begin{remark}[Strict-gap criteria boundary]
  \label{rem:eta-gap-criteria-scope}
  \label{rem:eta-gap-criteria-boundary-note}
  The criteria above characterize the boundary \(\eta=\lambda\) by a
  ...
  chain.  These statements are therefore criteria for a fixed finite-state
  input, not a promised classification of cocycles with
  \(\eta<\lambda\).  General structural sufficient criteria for strict
  twisted spectral gaps belong to a separate classification problem and are not
  part of the present theorem chain.
\end{remark}
