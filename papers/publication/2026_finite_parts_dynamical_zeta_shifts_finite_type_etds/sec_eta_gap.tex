...
\begin{corollary}[Perron-point availability for the main constants]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item On the failed branch, a non-trivial boundary line field exists and
      ...
      fixed cocycle.
  \end{enumerate}
  The explicit line-field exclusions proved in this section are finite-state
...
\begin{proof}
  ...
  record the finite alternative and its explicit line-field exclusions.
\end{proof}

\begin{remark}[Fixed-cocycle use of the eta-gap certificate]
  \label{prop:eta-gap-conditional-scope}
  The eta-gap results in this section give, for the finite edge cocycle fixed
  in the paper, the following finite equality test and Perron-point gate.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item If the certificate fails, then some non-trivial irreducible channel
      admits the boundary line field of \Cref{thm:eta-gap-rigidity}.  The
      open-disc determinant identities of Main Theorem~I remain available for
      that cocycle.
    \item The only sufficient implications for the strict inequality proved
      here are fixed-cocycle eliminations of the boundary line fields detected
      by this finite test, such as the loop-character obstruction and the
      perfect-loop-group consequence.
  \end{enumerate}
\end{remark}
...
\begin{proof}[Verification]
  ...
  the Perron circle.  The open-disc determinant identities of Main Theorem~I
  remain inside their domain of proof.  This proves item~(ii).

  The last assertion follows from
  \Cref{cor:eta-gap-boundary-classifier,cor:eta-gap-only-boundary-export}:
  every obstruction at the boundary is one of the line fields detected by the
  finite criterion, and the sufficient criteria proved here are precisely
  hypotheses that exclude those line fields for the fixed finite input.  This
  proves item~(iii).
\end{proof}
...
\begin{remark}[Strict-gap criteria boundary]
  ...
  The finite boundary obstruction and the perfect-loop-group elimination of that
  obstruction are the structural checks used in the finite-determinant
  product-constant theorem chain.  Broader sufficient conditions for strict
  twisted spectral gaps form a separate classification problem.
\end{remark}
