...
\begin{proposition}[Finite-state certificate for the Perron-point
  ...
  \item No non-trivial irreducible representation admits a boundary line
    ...
    \]
    has no solution.
  \item The deterministic Perron-circle audit returns \(\mathsf{Pass}\).  This
    audit computes \(\chi_{\widetilde A}\), removes the distinguished factor
    ...
    one of these comparisons fails, the audit returns \(\mathsf{Fail}\) and the
...
\begin{proof}
  ...
  Item~\rm(iv) is the same test written as a finite deterministic audit.
  ...
  comparisons.  Therefore the audit returns \(\mathsf{Pass}\) exactly when item
...
\begin{corollary}[Perron-point availability for the main constants]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    \item The operational finite-state certificate of
      \Cref{prop:eta-gap-finite-state-certificate} is a deterministic
      pass/fail audit of the fixed cocycle.  It succeeds if and only if the
...
\begin{proposition}[Fixed-cocycle scope of the eta-gap certificate]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item The only sufficient implications for the strict inequality proved
      ...
      \(\eta<\lambda\) for general cocycle classes are split material and are
      not used as hypotheses, conclusions, or hidden inputs in this article.
  \end{enumerate}
\end{proposition}
