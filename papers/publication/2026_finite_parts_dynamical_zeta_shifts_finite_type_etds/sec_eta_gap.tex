...
\begin{proof}
  Assume, for contradiction, that \(\eta=\lambda\). By
  \Cref{cor:eta-gap-boundary-classifier}, there are a non-trivial
  irreducible representation \(\varrho\), a phase \(\omega\in\TT\), and a
  boundary line field \((\xi_i)_i\). Apply
  \Cref{cor:eta-gap-character-classification} at the vertex \(i_0\). The
  line \(L_{i_0}:=\CC\xi_{i_0}\) is a one-dimensional representation of the
  perfect group \(\Gamma_{i_0}(\tau)\). Since every homomorphism from a
  perfect group to the abelian group \(\TT\) is trivial, this character is
  trivial. Hence every element of \(\Gamma_{i_0}(\tau)\) fixes
  \(\xi_{i_0}\), contradicting the assumed absence of non-zero fixed vectors
  in \(\varrho|_{\Gamma_{i_0}(\tau)}\). Therefore \(\eta\neq\lambda\), and
  \Cref{cor:eta-gap-boundary-classifier} gives \(\eta<\lambda\).
