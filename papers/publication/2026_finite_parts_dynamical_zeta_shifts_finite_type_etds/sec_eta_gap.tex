...
\begin{proposition}[Finite-state certificate for the Perron-point
  ...
  \]
  The following finite-state conditions are equivalent.  Thus a main theorem
  may require the gap operationally by asking for either the spectral-radius
  check in \textup{(ii)}/\textup{(iv)} or the line-field exclusion in
  \textup{(iii)}, rather than treating \(\eta<\lambda\) as an unverifiable
...
\begin{remark}[Strict-gap criteria boundary]
  ...
  Main Theorems~II--III.  They are finite criteria for the present fixed
