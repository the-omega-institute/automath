...
empty maximum is \(0\).  This section supplies only a diagnostic package for
checking that hypothesis and for understanding the boundary case
\(\eta=\lambda\).  It supports Main Theorems~II--III by certifying the
line-field obstruction and its finite loop-character consequences; it does
not provide another source of class-product constants, a general
classification of strict-gap cocycles, or a broad rigidity theorem for
arbitrary finite-group extensions.  The boundary case is rigid in the precise
finite-state sense below: equality is equivalent to the existence of a
cocycle-equivariant line field.  The corollaries are therefore operational
checks for the already stated main-theorem hypothesis.
