...
The finite data retained in the main text are only the model, the determinant
polynomials, and the four rational windows.  The full two-shift
\(A=\begin{psmallmatrix}1&1\\1&1\end{psmallmatrix}\) is labelled by
...
\[
  ...
  \end{pmatrix}.
\]
Their determinant polynomials are
\[
  P_{\mathbf1}(z)=1-2z,\qquad
  P_\varepsilon(z)=1,\qquad
  P_{\varrho_{\mathrm{std}}}(z)=1+z,
\]
so \(\rho(A_\varepsilon)=0\), \(\rho(A_{\varrho_{\mathrm{std}}})=1<2\),
\(\mathcal L_\varepsilon(z)=0\), and
\(\mathcal L_{\varrho_{\mathrm{std}}}(z)=-\log(1+z)\).  The Adams
coefficients are the finite \(m\bmod 6\) data in
\eqref{eq:s3-example-adams-rho}--\eqref{eq:s3-example-alpha-beta}.  The
compact reproducibility record in
\Cref{prop:s3-compact-rational-verification-record,prop:s3-finite-exact-arithmetic-reproducibility}
contains the rational endpoints, tail radii, and cleared-denominator
comparisons for the four intervals quoted below; the expanded termwise
fractions are ancillary in
\texttt{certificates/s3\_log\_certificates.py} and
...
\begin{proof}
  ...
  in \Cref{thm:class-product-determinant-rigidity}:
