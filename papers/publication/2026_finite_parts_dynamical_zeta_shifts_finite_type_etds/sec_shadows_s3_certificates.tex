...
symbolic skew product and survives at the product-constant level of
\Cref{thm:renormalized-frobenius-product} through the strict-gap specialization.
The main-text mechanism is that Adams powers erase the periodic
...
checking.  No step of the Perron-boundary product theorem depends on those
appendix details; they certify the single displayed non-abelian witness.  For orientation only, the appendix check chain is as follows.  The finite model,
...
fixed-label windows used in the strict-gap product specialization are bounded in
...
\eqref{eq:s3-f-rho-appendix-window}.\\[0.8ex]
Replay files &
The paper-local Python replay script and its printed certificate in the
\texttt{certificates/} directory. &
Recomputes the determinant logarithm brackets, finite truncations, and rational
tail comparisons used in \Cref{lem:appendix-s3-sharp-certificates,lem:appendix-s3-f-level-certificates}.\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{theorem}[Adams erasure mechanism in the \(S_3\) witness]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \]
    \item the normalized strict-gap class-product error obtained by
...
\begin{proof}
  ...
  The strict-gap coordinate readout identifies the non-trivial Fourier
  coordinates of the scalar-normalised class-product constants with the
  fixed-label values \(F_\varrho(1/2)\), whereas
...
\begin{corollary}[Product-constant obstruction is already finite-state]
  ...
  the Adams-corrected constants of the strict-gap product specialization by periodic Artin
