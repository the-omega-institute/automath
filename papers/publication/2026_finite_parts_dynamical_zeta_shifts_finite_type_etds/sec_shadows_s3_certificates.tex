...
abelian/cyclic support boundary.  It shows that the higher-dimensional
Peter--Weyl defect occurs in a small
...
non-zero.  Accordingly the main text records the model, the certified
...
The appendices retain the finite rational arithmetic for checking the displayed
non-abelian witness.  The appendix check chain is as follows.  The finite model,
...
The self-contained rational record in
\Cref{prop:s3-finite-exact-arithmetic-reproducibility}. &
The appendix prints the determinant logarithm brackets, finite truncations,
rational tails, endpoint-selection rule, and cleared-denominator comparisons
used in
\Cref{lem:appendix-s3-sharp-certificates,lem:appendix-s3-f-level-certificates};
no ancillary program or certificate file is needed to verify the displayed
windows.\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{proof}
  ...
  \eqref{eq:class-product-fourier-coordinates} from
