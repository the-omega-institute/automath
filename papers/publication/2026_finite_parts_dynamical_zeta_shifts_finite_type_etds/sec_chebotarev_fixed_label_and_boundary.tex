\begin{lemma}[Primitive expansion of class-function logarithms]
  ...
  In the finite-group sections, \(A\) is the primitive adjacency matrix of a
  finite directed multigraph, so that \(\cP\), \(\tau(\gamma)\), and the
  primitive orbit counts below have their literal orbit meaning.  The label
  \(\tau(\gamma)\) is the conjugacy class of a chosen cyclic representative;
  the class-function hypothesis makes the choice immaterial.
...
\begin{corollary}[Adams--M\"obius primitive inversion]
  ...
  This is the primitive-inversion step inside Main Theorem~A, not a separate
  main theorem.  It is a finite-directed-multigraph statement: \(A\) is the
  primitive adjacency matrix fixed for the finite-group orbit and cocycle
  theorems, not merely a primitive non-negative real matrix.  For every class
  function \(\phi:G\to\CC\) and every
  \(|z|<\lambda^{-1}\),
  ...
  \end{equation}
  The inversion is an identity of absolutely convergent open-disc series.  It
  does not assert a Perron-boundary value; endpoint values are supplied only
  later by the strict-gap or renormalized boundary conventions.
\end{corollary}
...
\begin{proof}
  ...
  \]
  This is the precise cancellation point in the primitive inversion.  The
  extra factor \(1/m\) in the Adams--M\"obius sum and the \(1/r\) already
  present in the periodic logarithm multiply to \(1/(mr)=1/s\).  Hence there
  is no additional cyclic-orbit normalization left at this stage: the earlier
  factor \(\ell/(r\ell)=1/r\) came from the \( \ell \) basepoints of a
  primitive orbit inside the fixed-point trace, and the divisor sum below is
  only the ordinary M\"obius identity.
  Hence
  \[
    \sum_{m\ge 1}\frac{\mu(m)}{m}\mathcal L_{\psi^m\phi}(z^m)
    =
...
The next theorem is the notational hinge between Main Theorem~A and the
...
constant.  For non-trivial characters its Perron-point determinant value is
read with the strict-gap specialization of
\Cref{def:perron-boundary-logarithm-convention}: the scalar Perron
determinant is deleted only in the singular triple
...
\begin{theorem}[Fixed-label Euler transform with Perron branch convention]
  ...
  \end{equation}
  The endpoint reading of this Abel limit is the strict-gap specialization of
  \Cref{def:perron-boundary-logarithm-convention}.  In particular, the
  deletion is performed before any branch or finite value is chosen: the single
  scalar Perron summand \((r,m,\pi)=(1,1,\mathbf1)\) is not assigned a finite
  part.  Non-trivial \(rm=1\) summands use that Abel branch, and every
  \(rm\ge2\) summand, including scalar summands created by Adams powers,
  remains in the formula as an ordinary interior-disc determinant logarithm.  Thus
