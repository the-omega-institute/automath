...
\begin{theorem}[Roadmap and role separation for the main theorem package]
  \label{thm:main-theorem-package-roadmap}
  Under the hypotheses of \Cref{prop:class-mertens-product-dependency-chain},
  the theorem-like statements used in and after the class-Mertens core split
  into the following ordered roadmap.
  \begin{center}
  \small
  \begin{tabular}{@{}p{0.17\textwidth}|p{0.36\textwidth}|p{0.28\textwidth}@{}}
    tier & load-bearing statements & navigation or certificate role\\
    \hline
    ...
    \Cref{prop:main-iii-proof-owning-spine,cor:main-iii-convention-compatibility}\\[0.4ex]
    finite availability of branches &
    \Cref{prop:eta-gap-finite-state-certificate,prop:eta-gap-perron-separation} &
    \Cref{cor:eta-gap-diagnostic-interface}\\[0.4ex]
  ...
  \end{tabular}
  \end{center}
  More precisely, the statements in the right-hand column do not introduce a
  further product constant, Perron branch, determinant coordinate, or quotient
  invariant.  Each either assembles previously proved formulas, records a
  branch or convention certificate, or identifies the functorial loss incurred
  when the already recovered primitive class profile is tested only through
  abelian or cyclic quotients.  Consequently every output used by Main
  Theorems~II--III and by the quotient-shadow package factors through the
  scalar normalisation \(C(A)\) and the non-trivial fixed-label determinant
  coordinates \((F_\varrho(\lambda^{-1}))_{\varrho\ne\mathbf1}\) supplied by
  the load-bearing chain above.
\end{theorem}

\begin{proof}
  The first three rows are exactly the finite determinant dependence proved in
  \Cref{prop:class-mertens-product-dependency-chain}.  Main Theorem~I gives
  primitive Peter--Weyl extraction on \(|z|<\lambda^{-1}\) through
  \Cref{prop:main-theorem-i-assembly-map}.  After imposing
  \(\lambda>1\) and \(\eta<\lambda\),
  \Cref{thm:class-mertens-explicit,prop:class-mertens-deviations,thm:class-mertens-fourier}
  give the additive class constants and their Hermitian Fourier profile.  The
  fixed-label Euler transform and the product theorem then give the constants
  \(K_C\), and \Cref{prop:fixed-label-determinant-perron} evaluates their
  non-trivial coordinates from branch-compatible twisted determinants.  The
  proof-owning spine \Cref{prop:main-iii-proof-owning-spine} is therefore an
  assembly device for these arrows, and
  \Cref{cor:main-iii-convention-compatibility} only records that the same
  branch, Hermitian, and scalar-normalisation conventions are used in the
  displayed formulas; neither statement contains a new numerical input.

  For the fourth row,
  \Cref{prop:eta-gap-finite-state-certificate,prop:eta-gap-perron-separation}
  prove the finite-state equivalence between the strict eta gap and the
  availability of the non-trivial Perron-point determinant branches.  The
  diagnostic interface \Cref{cor:eta-gap-diagnostic-interface} merely exports
  this pass/fail information to Main Theorems~II--III: on the passing branch
  the determinant coordinates are the ones already supplied by the preceding
  rows, and on the failing branch a boundary line field gives the obstruction
  to those Perron-point values.  Thus the eta-gap interface changes only the
  availability of the already named branches, not the formula for the product
  vector.

  Finally, \Cref{thm:abelian-shadow-defect} decomposes the normalized class
  profile into its one-dimensional and higher-dimensional Peter--Weyl parts,
  and \Cref{thm:cyclic-detection-boundary} proves that cyclic quotient tests
  see exactly the one-dimensional span.  These statements are applied after
  the profile has been recovered from the determinant chain.  They therefore
  identify what quotient shadows forget, rather than adding a second source for
  \(\mathcal M_C\), \(K_C\), or \(F_\varrho(\lambda^{-1})\).  Combining the
  four rows proves the asserted factorisation and the claimed separation
  between load-bearing endpoints and navigation or certificate statements.
\end{proof}
...
\begin{proof}[Assembly of the three main statements]
  ...
  the role separation of \Cref{thm:main-theorem-package-roadmap}.
