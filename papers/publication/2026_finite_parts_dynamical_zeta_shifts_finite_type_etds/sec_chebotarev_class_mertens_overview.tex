...
follow all concern the same scalar-normalised vector.  The proof route is:
\[
\begin{array}{c}
...
coordinate and branch verification
...
\(\mathcal L_\varrho(z)\) & Periodic Artin determinant logarithm \(\log\det(I-zA_\varrho)^{-1}\) & Comparison object; it is not substituted for \(F_\varrho\) in \(K_C\). \\
...
Branch & Principal determinant branches in the open disc; Abel-path boundary values at \(\lambda^{-1}\); positive real branch for \(K_C\) & Keeps the scalar singular determinant deleted and the class products positive. \\
\bottomrule
\end{tabular}
\end{center}
