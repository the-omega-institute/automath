...
This is the obstruction part of the Main Theorem~III block.  All constants in
this subsection are the product constants already defined above; the new
question is whether a proposed branch-compatible replacement for the
fixed-label vector can give the same class products.  The finite \(S_3\)
rational windows later in the paper are used only as exact witnesses to this
obstruction, rather than as decimal approximations.
...
\begin{theorem}[Branch-compatible determinant readout for Main Theorem III]
  ...
  those non-trivial coordinates.  The theorem uses the classical
  Frobenius-class product layer only for the already cited existence, exponent,
  and scalar context; the lossless coordinate readout proved here is the
  Adams-corrected finite determinant statement.
\end{theorem}
