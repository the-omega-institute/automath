...
\begin{proposition}[Strict-gap class-constant deviations,
  ...
  \(\log\det(I-\lambda^{-m}A_\pi)^{-1}\) is taken in the strict-gap
  specialization of \Cref{def:perron-boundary-logarithm-convention}: for
  \(m=1\) and non-trivial \(\pi\) this is the Abel-path boundary value of the
  normalized determinant germ, and for \(m\ge2\) it is the same germ evaluated
  at \(z=\lambda^{-m}\).
  ...
  \(\mathfrak m_C(A,\tau;G)\).  The determinant logarithms are taken in the
  branch fixed by \Cref{def:perron-boundary-logarithm-convention}; the
  formally singular zero-coefficient term
...
\begin{proof}
  ...
  Adams coefficients, and the branch convention cited in the statement.
\end{proof}
