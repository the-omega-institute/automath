...
\begin{theorem}[Adams obstruction criterion for class-product constants]
  ...
  define the naive Artin replacement constant with the product scalar baseline
  \(\gamma+\log C(A)
  =\mathcal M(A)+\sum_{r\ge2}\Pi_{\mathbf1}(\lambda^{-r})/r\), rather than the
  additive baseline \(\mathcal M(A)=\gamma+\FP(A)\), by
...
\begin{theorem}[Strict-gap branch-compatible determinant readout]
  \label{thm:main-iii-certificate-chain}
  Assume the hypotheses of \Cref{cor:strict-gap-specialization}, equivalently
  the strict-gap hypotheses of
  \Cref{thm:class-mertens-explicit,thm:class-euler-product-mertens}.  Fix the
  strict specialization
  \(\mathfrak D_{\mathrm{strict}}(A,\tau;G)\) of
  \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\), together with the
  scalar normalisation \(\alpha_A:=\gamma+\log C(A)\), the positive logarithm
  branch, the Abel/interior/deleted branch tags \(\mathfrak b\), and the
  certified gap \(\eta<\lambda\).  The full renormalized coordinate readout is
  \Cref{thm:renormalized-coordinate-readout}; the present statement is its
  strict-gap finite-coordinate verification.  For every non-trivial
  ...
  non-classical constant-coordinate part of the strict-gap specialization.
  \begin{enumerate}[label=\textup{(\alph*)},leftmargin=*]
    \item The leading Frobenius-class product statement in
    \Cref{thm:class-euler-product-mertens} is derived internally from the
    additive estimate and product-tail reduction and lies on the classical
    exponent scale \(|C|/|G|\).  It fixes the positive product branch and the
    scalar normalisation channel \(\alpha_A\), but supplies no non-trivial
  ...
  \end{enumerate}
  Thus the strict-gap determinant contribution is a single finite
  ...
  supplies comparison for the cited exponent and scalar context; the lossless
...
\begin{proof}
  ...
  \Cref{thm:class-euler-product-mertens}: the internal product-tail argument provides the leading asymptotic and positive
  product branch, while the cited dynamical Mertens and Chebotarev product
  theorems provide comparison conventions and all non-trivial coordinates are supplied by the
