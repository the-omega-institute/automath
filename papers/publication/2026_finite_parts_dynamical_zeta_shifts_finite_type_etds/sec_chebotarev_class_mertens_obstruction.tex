...
\begin{theorem}[Branch-compatible determinant readout for Main Theorem III]
  ...
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}.  Fix the full
  Perron-boundary data tuple \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\) of
  \eqref{eq:perron-boundary-data-tuple}.  In particular this fixes the scalar
  normalisation \(\alpha_A:=\gamma+\log C(A)\), the positive logarithm branch,
  the Abel/interior/deleted branch tags \(\mathfrak b\), and the certified gap
  \(\eta<\lambda\).  For every non-trivial
  ...
  non-classical constant-coordinate part of Main Theorem~III.
