...
The proof separates open-disc primitive inversion from the later Perron-point
evaluation.  The first part,
...
mechanism and does not use a twisted spectral gap. The second block,
  \Cref{thm:class-mertens-explicit,prop:class-mertens-deviations,thm:class-mertens-fourier}, is the strict-gap additive specialization and starts only
...
non-abelian character orthogonality. The intervening class-decomposition
formulas translate between primitive Peter--Weyl coordinates and class counts.
...
\subsection{Main Theorem A: Adams--M\"obius primitive determinant
linearisation}

The next proposition packages the open-disc primitive recovery: twisted traces
are linearised by determinant logarithms, Adams--M\"obius inversion removes
primitive repetitions, and finite character orthogonality recovers class
counts.  Later Perron-point arguments cite this package without reintroducing
its proof.

\begin{proposition}[Primitive class-count recovery interface]
  \label{prop:main-theorem-i-assembly-map}
  The following implications give the primitive determinant recovery used in
  Main Theorem~A without any use of the Perron-boundary hypothesis
  \(\eta<\lambda\).
  ...
  \end{enumerate}
  Consequently Main Theorem~A is exactly the finite-state determinant
...
\toprule
Symbol & Orbit datum & Role in the proof \\
\midrule
\(\mathcal L_\phi(z)\)
...
fixed in every Euler factor.
  & Supplies the fixed-label coordinates used in the Frobenius-class product constants. \\
\bottomrule
\end{tabular}
\end{center}
...
\[
  ...
  =\chi_\varrho(C^{-1}).
\]
\textbf{We keep this \(\ast\)-notation throughout, and the inverse recovery
is always written separately with \(\chi_\pi(C)\).}
...
corollaries below, it forms Main Theorem~A from the introduction.
...
\begin{theorem}[Class-function determinant linearisation]
  ...
  \begin{equation}\label{eq:class-logarithm-determinant-linearisation}
    ...
    \sum_{\varrho\in\Irr(G)}
      ...
      \log\det(I-zA_\varrho)^{-1}.
  \end{equation}
  Here each determinant logarithm is its power-series logarithm at the origin;
  no Perron-boundary finite part is being used in this open-disc identity.
\end{theorem}
