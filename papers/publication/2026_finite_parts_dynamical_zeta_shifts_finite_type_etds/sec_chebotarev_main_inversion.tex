...
The first main argument has five steps. The twisted trace identity converts
...
For reference, the finite-dimensional passage is
\[
\begin{array}{c}
...
\end{array}
\]
The next proposition records the finite construction with its hypotheses and
outputs, so that the later Perron-point arguments can cite the primitive
recovery in a single form.

\begin{proposition}[Assembly note for primitive class-count recovery]
  \label{prop:main-theorem-i-assembly-map}
  This is an organizational assembly of the local identities used in Main
  Theorem~I, not an additional theorem.  The following implications prove the
  primitive determinant recovery without any use of the Perron-boundary
