...
\(V\) vertices, \(2V\lceil\log_2(4V)\rceil\) radial samples in the common
open Perron interval determine the complete primitive length--element profile,
provided that one sample is algebraic.  This count is independent of \(r\),
and the bases need not coincide.  The strict twisted gap is needed only if a
Perron endpoint is sampled in the binary scalar theorem.  The determinant ratio governing the
...
and denominator satisfy an essentially optimal \(O(D\log D)\) degree bound
and an explicit height bound,
...
parity coordinate by coordinate.  Nishioka's theorem converts an algebraic special value into
