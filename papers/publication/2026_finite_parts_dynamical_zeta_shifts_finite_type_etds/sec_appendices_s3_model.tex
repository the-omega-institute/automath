...
used in \Cref{sec:shadows}. The rational details are recorded here so that the
abelian-shadow calculations do not obscure the Adams--M\"obius class-Mertens
argument.  The appendix verifies the finite obstruction witness whose rational
windows are used in the main text.
...
\begin{equation}\label{eq:s3-character-table}
  \begin{array}{c|ccc}
    ...
    \mathbf 1 & 1 & 1 & 1 \\
    \varepsilon & 1 & -1 & 1 \\
    \varrho_{\mathrm{std}} & 2 & 0 & -1
  \end{array}
\end{equation}
where \(\varepsilon\) is the sign character and \(\varrho_{\mathrm{std}}\) is the
...
\begin{aligned}
  \varrho_{\mathrm{std}}((12))
    &=
    \begin{pmatrix}
      -1 & 0 \\
      0 & 1
    \end{pmatrix}, \\
  \varrho_{\mathrm{std}}((23))
    &=
    \begin{pmatrix}
      \frac12 & \frac32 \\
      \frac12 & -\frac12
    \end{pmatrix}, \\
  \varrho_{\mathrm{std}}((123))
    &=
    \begin{pmatrix}
      -\frac12 & -\frac32 \\
      \frac12 & -\frac12
...
relations, determinant polynomials, and spectral-radius statements used
below are invariant under equivalence of representations.  Thus the
displayed rational matrices are the exact block model used for the
computations in this appendix.  They are not the Hilbert model used in the
line-field domination proof of \Cref{thm:eta-gap-rigidity}; if that proof is
invoked, one first transfers the same representation to an equivalent
unitary basis as in \Cref{lem:eta-gap-exact-block-models}.  In this
\(S_3\) example the rational field \(\QQ\subset\QQ(\zeta_6)\) already
contains the displayed block matrices, so determinant and rank checks are
...
\begin{proposition}[Twisted channels for the \(S_3\)-example]
  ...
  \begin{equation}\label{eq:s3-example-arho-charpoly}
    \chi_{A_{\varrho_{\mathrm{std}}}}(t)=t^3(t+1),
    \qquad
  ...
  \begin{equation}\label{eq:s3-example-logarithms}
    ...
    \qquad
    \mathcal L_{\varrho_{\mathrm{std}}}(z)=\log\det(I-zA_{\varrho_{\mathrm{std}}})^{-1}=-\log(1+z).
  \end{equation}
...
\begin{proof}
  ...
  \[
    ...
    =
    \begin{pmatrix}
      \varrho_{\mathrm{std}}((23)) & I_2 \\
      \varrho_{\mathrm{std}}((12)) & \varrho_{\mathrm{std}}((123))
    \end{pmatrix},
  \]
  which is exactly \eqref{eq:s3-example-arho}. To compute its
  characteristic polynomial, write
  \[
    tI_4-A_{\varrho_{\mathrm{std}}}
    =
    \begin{pmatrix}
      tI_2-\varrho_{\mathrm{std}}((23)) & -I_2 \\
      -\varrho_{\mathrm{std}}((12)) & tI_2-\varrho_{\mathrm{std}}((123))
    \end{pmatrix}.
  \]
  Since the upper-right block \(-I_2\) commutes with the lower-right
  block, the block determinant identity gives
  \[
    \det(tI_4-A_{\varrho_{\mathrm{std}}})
    =
    \det\!\bigl((tI_2-\varrho_{\mathrm{std}}((123)))(tI_2-\varrho_{\mathrm{std}}((23)))-\varrho_{\mathrm{std}}((12))\bigr).
  \]
  A direct multiplication using
  \eqref{eq:s3-standard-representation-matrices} shows that
  \[
    (tI_2-\varrho_{\mathrm{std}}((123)))(tI_2-\varrho_{\mathrm{std}}((23)))-\varrho_{\mathrm{std}}((12))
    =
    \begin{pmatrix}
      t^2 & 0 \\
      -t & t(t+1)
  ...
  Hence
  \[
    \chi_{A_{\varrho_{\mathrm{std}}}}(t)=\det(tI_4-A_{\varrho_{\mathrm{std}}})=t^3(t+1),
  \]
  so the eigenvalues of \(A_{\varrho_{\mathrm{std}}}\) are \(0,0,0,-1\). This gives
  \(\rho(A_{\varrho_{\mathrm{std}}})=1\), \(\det(I-zA_{\varrho_{\mathrm{std}}})=1+z\), and
  \(\Tr(A_{\varrho_{\mathrm{std}}}^n)=(-1)^n\) for every \(n\ge 1\).

  Since \(\Tr(A^n)=2^n\), \(\Tr(A_\varepsilon^n)=0\), and
  \(\Tr(A_{\varrho_{\mathrm{std}}}^n)=(-1)^n\), the logarithmic channels are
  \[
  \begin{aligned}
    \mathcal L_\varepsilon(z)
      &=\log\det(I-zA_\varepsilon)^{-1}=0,\\
    \mathcal L_{\varrho_{\mathrm{std}}}(z)
      &=\log\det(I-zA_{\varrho_{\mathrm{std}}})^{-1}=-\log(1+z).
  \end{aligned}
  \]
    ...
    \begin{aligned}
      ...
      \frac{|C|}{|S_3|}
        \Bigl(
          ...
          +\chi_{\varrho_{\mathrm{std}}}^\ast(C)\Tr(A_{\varrho_{\mathrm{std}}}^n)
        \Bigr) \\
      &=
      \frac{|C|}{|S_3|}
        \Bigl(2^n+\chi_{\varrho_{\mathrm{std}}}^\ast(C)(-1)^n\Bigr),
    \end{aligned}
  \]
...
\begin{proposition}[Primitive channels for the \(S_3\)-example]
  ...
  \begin{equation}\label{eq:s3-example-adams-rho}
    \psi^m\varrho_{\mathrm{std}}=
    \begin{cases}
      \varrho_{\mathrm{std}}, & m\equiv \pm 1 \pmod 6, \\
      \mathbf 1-\varepsilon+\varrho_{\mathrm{std}}, & m\equiv \pm 2 \pmod 6, \\
...
\begin{proof}
  ...
  For \(\varrho_{\mathrm{std}}\), the Adams powers are computed from the character table
  ...
  the class values of \(\psi^m\varrho_{\mathrm{std}}\) are
  \[
    \begin{array}{c|ccc|c}
      m \bmod 6
      & (\psi^m\chi_{\varrho_{\mathrm{std}}})(e)
      & (\psi^m\chi_{\varrho_{\mathrm{std}}})(T)
      & (\psi^m\chi_{\varrho_{\mathrm{std}}})(K)
      & \psi^m\varrho_{\mathrm{std}} \\ \hline
      \pm 1 & 2 & 0 & -1 & \varrho_{\mathrm{std}} \\
      \pm 2 & 2 & 2 & -1 & \mathbf 1-\varepsilon+\varrho_{\mathrm{std}} \\
      3 & 2 & 0 & 2 & \mathbf 1+\varepsilon \\
      0 & 2 & 2 & 2 & 2\mathbf 1
    \end{array}
  \]
  and decomposing these four class functions in the basis
  \(\{\mathbf 1,\varepsilon,\varrho_{\mathrm{std}}\}\) gives
...
\begin{proof}
  ...
  \(F_\varepsilon(1/2)\) and \(F_{\varrho_{\mathrm{std}}}(1/2)\) are proved by exact rational
...
\begin{corollary}[Primitive profile and deviations for the
  ...
  \begin{equation}\label{eq:s3-example-primitive-classes}
    \begin{aligned}
      p_{n,e}
      &=\frac{p_n(A)+c_{n,\varepsilon}(A,\tau)+2c_{n,\varrho_{\mathrm{std}}}(A,\tau)}{6}, \\
      p_{n,T}
      &=\frac{p_n(A)-c_{n,\varepsilon}(A,\tau)}{2}, \\
      p_{n,K}
      &=\frac{p_n(A)+c_{n,\varepsilon}(A,\tau)-c_{n,\varrho_{\mathrm{std}}}(A,\tau)}{3}.
    \end{aligned}
  \end{equation}
  where
  ...
  \begin{equation}\label{eq:s3-example-delta-values}
    \begin{aligned}
      \Delta_e(A,\tau;S_3)
      &=\frac16\Pi_\varepsilon(1/2)+\frac13\Pi_{\varrho_{\mathrm{std}}}(1/2), \\
      \Delta_T(A,\tau;S_3)
      &=-\frac12\Pi_\varepsilon(1/2), \\
      \Delta_K(A,\tau;S_3)
      &=\frac13\Pi_\varepsilon(1/2)-\frac13\Pi_{\varrho_{\mathrm{std}}}(1/2).
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}\label{eq:s3-example-nonabelian-defect}
    ...
    =
    \Bigl(
      \frac13\Pi_{\varrho_{\mathrm{std}}}(1/2),
      0,
      -\frac13\Pi_{\varrho_{\mathrm{std}}}(1/2)
    \Bigr).
  \end{equation}
  ...
  \begin{equation}\label{eq:s3-example-energy-ratio}
    ...
    =
    \frac{\abs{\Pi_{\varrho_{\mathrm{std}}}(1/2)}^2}
...
\begin{proof}
  ...
  \(c_{n,\mathbf1}(A,\tau)=p_n(A)\), character orthogonality gives
  \[
    \begin{aligned}
    p_{n,e}
      &=\frac{1}{6}\bigl(p_n(A)+c_{n,\varepsilon}(A,\tau)
        +2c_{n,\varrho_{\mathrm{std}}}(A,\tau)\bigr),\\
    p_{n,T}
      &=\frac{3}{6}\bigl(p_n(A)-c_{n,\varepsilon}(A,\tau)
        +0\cdot c_{n,\varrho_{\mathrm{std}}}(A,\tau)\bigr),\\
    p_{n,K}
      &=\frac{2}{6}\bigl(p_n(A)+c_{n,\varepsilon}(A,\tau)
        -c_{n,\varrho_{\mathrm{std}}}(A,\tau)\bigr),
    \end{aligned}
  \]
  ...
  \[
    \begin{aligned}
      \Delta_e(A,\tau;S_3)
      &=\frac{1}{6}\Pi_\varepsilon(1/2)+\frac{1}{3}\Pi_{\varrho_{\mathrm{std}}}(1/2), \\
      \Delta_T(A,\tau;S_3)
      &=-\frac{1}{2}\Pi_\varepsilon(1/2), \\
      \Delta_K(A,\tau;S_3)
      &=\frac{1}{3}\Pi_\varepsilon(1/2)-\frac{1}{3}\Pi_{\varrho_{\mathrm{std}}}(1/2).
    \end{aligned}
  \]
  ...
  \((718/342)^2>4\). The following table is therefore only a derived summary
  of the identities just proved and of the certified windows used in the last
  step.
\end{proof}
