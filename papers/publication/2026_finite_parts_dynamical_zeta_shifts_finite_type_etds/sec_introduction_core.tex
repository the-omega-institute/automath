...
\begin{proposition}[Introductory class-product calibration]
...
comparison model.  The finite-tail invariance statement
\Cref{prop:main-chain-secondary-noninterference} records that later
lower-order refinements do not alter the constants.  None of these auxiliary
ingredients provides another route to the finite-group product constants or
adds a hypothesis to Main Theorems~I--III.  Thus the proofs of Main
...
The proof spine of the three main statements is as follows.  Main Theorem~I is
proved by the trace identity, finite Peter--Weyl linearisation, and
Adams--M\"obius primitive inversion summarized in
\Cref{prop:main-theorem-i-assembly-map}.  Main Theorem~II is obtained from
\Cref{thm:class-mertens-explicit,thm:class-mertens-fourier} after imposing the
strict finite-dimensional gap and using \Cref{sec:eta-gap} only to verify that
hypothesis for a fixed cocycle.  Main Theorem~III is the product-level
assembly in \Cref{thm:class-euler-product-mertens}: it uses the fixed-label
transform \(F_\varrho(\lambda^{-1})\), its determinant evaluation in
\Cref{prop:fixed-label-determinant-perron}, and the comparison theorem
separating it from the periodic Artin logarithm.  The concrete \(S_3\) witness
later in the paper uses this same finite determinant machinery and adds no
hypothesis to the product theorem.
...
\(\psi^m\phi(g)=\phi(g^m)\), and the spectral gap stated in Main Theorem~II.
On the open Perron disc every determinant logarithm is the ordinary
finite-matrix logarithm \(\log\det(I-zA_\varrho)^{-1}\), and its periodic trace
coefficients are the fixed-point sums of \Cref{lem:twisted-trace-identity}.
The passage to primitive fixed-label series is finite Peter--Weyl expansion
followed by Adams--M\"obius inversion, as proved in
...
The Perron-point evaluations use only
\(\eta=\max_{\varrho\ne\mathbf1}\rho(A_\varrho)<\lambda\), equivalently the
finite skew-product peripheral condition of
\Cref{lem:twisted-gap-skew-product-peripheral}.  Consequently the theorem chain
requires no infinite-dimensional transfer-operator extension, completed-zeta
normalisation, functional equation, secondary spectral-tail theorem,
non-abelian root-layer reconstruction, universality theorem, or general
finite-verification theory.

The paper's main contribution is recorded in the following three numbered
headline theorems.  The classical symbolic-dynamical input is the
Chebotarev/Mertens scale: Frobenius classes have density \(|C|/|G|\), and the
corresponding products have leading power \(N^{-|C|/|G|}\) under the usual
mixing and spectral hypotheses.  The new finite-state layer in this paper is the
constant left after that scale is removed.  Main Theorem~I converts periodic
twisted determinants into primitive Peter--Weyl channels by Adams--M\"obius
inversion.  Main Theorem~II evaluates the resulting non-trivial primitive
channels at the Perron point.  Main Theorem~III identifies the Euler-product
constant by the fixed-label transform \(F_\varrho(\lambda^{-1})\), proves its
finite determinant formula, and shows that the ordinary periodic Artin--Ruelle
logarithm gives the wrong coordinate in non-abelian examples.

This positioning is meant literally.  Parry--Pollicott's transfer-operator and
Chebotarev work \cite{ParryPollicott1990Zeta,ParryPollicott1986Chebotarev},
Noorani--Parry's Chebotarev theorem for homogeneous extensions of shifts
\cite{NooraniParry1992ChebotarevShifts}, Sharp's dynamical Mertens and homology
class orbit results \cite{Sharp1991Mertens,Sharp1993ClosedOrbitsHomologyAnosov},
and Mesliza--Noorani's Frobenius-class Mertens theorem
\cite{MeslizaNoorani1999FrobeniusMertens} supply the comparison layer for
density, exponent, and product existence.  Recent ETDS papers by Coles,
Humbert, O'Hare, and Crismale--Del Vecchio--Griseta--Rossi calibrate the
surrounding symbolic-dynamics landscape
...
they are not proof inputs.  The novelty claimed here is therefore the
Adams-corrected primitive-label determinant constant and its \(S_3\)
obstruction, not the classical Frobenius-class Mertens exponent.

Lower-order tail analysis is treated only as downstream refinement after
the constants have been identified; the retained finite-length comparison is
not a fourth main theorem.

The scope is similarly finite-matrix at the level of determinant identities.
Cyclic-block Fredholm realisation, synchronisation-kernel completed determinants,
endpoint-jet or large-parameter expansions, and automata-prime zeta dichotomies
are not used as hypotheses or proof inputs here.  When scalar determinant
identities are used below, they are used only in the fixed-matrix normalisation
needed for the finite-group class-product constants.

In relation to this literature, Main Theorem~I is a finite-state
Adams--M\"obius linearisation of primitive Peter--Weyl channels from the
twisted determinant polynomials \(\det(I-zA_\varrho)\); it uses the
Parry--Pollicott and Noorani--Parry setting but does not replace their
Chebotarev theory.  Main Theorem~II assumes the Perron gap and evaluates the
class-indexed Mertens constants by the resulting non-trivial determinant values
at \(z=\lambda^{-1}\).  Main Theorem~III then identifies the Frobenius-class
product constant through the fixed-label transform
\(F_\varrho(z)=\sum_{r\ge1}\Pi_\varrho(z^r)/r\), whose Adams-corrected
determinant expression differs in general from the ordinary periodic
Artin--Ruelle logarithm.  The Hermitian readout, quotient-shadow statements,
and the concrete \(S_3\) witness are finite-state consequences inside this
fixed SFT cocycle model; the recent symbolic-dynamics papers cited above are
comparison points rather than proof inputs.
