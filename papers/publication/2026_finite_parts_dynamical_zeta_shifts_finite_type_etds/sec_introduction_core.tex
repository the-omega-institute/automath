...
certifies the concrete Artin-substitution obstruction.  These auxiliary units
are used only through the displayed finite interfaces; they do not import a
H\"older/Ruelle transfer theorem, a completed-zeta or \(\xi\)-type package, a
secondary spectral-edge asymptotic, or a general certificate methodology into
the proof of Main Theorems~I--III.
...
The only split-paper interfaces used below are correspondingly narrow: the
H\"older/Ruelle transfer-operator chain is not reconstructed here; sibling
cyclic-block, synchronisation-kernel, completed-zeta or \(\xi\)-type,
automata-density, and prime-language results are not locally reproved;
secondary spectral-edge asymptotics enter only through the noninterference
firewall \Cref{prop:main-chain-secondary-noninterference}; scalar cyclic-lift
reconstruction remains an appendix comparison tool for the fixed matrix \(A\),
not a second main theorem chain; eta-gap diagnostics are used only as finite
tests or boundary obstructions, not as generic strict-gap criteria; and finite
rational records are used only as checkable witnesses for the single
determinant-constant obstruction, not as a general verification framework or a
...
\begin{proof}
  ...
  proposition.  The only external or split interfaces named in the paragraph
  are cited either as inherited background for existence and leading terms or
