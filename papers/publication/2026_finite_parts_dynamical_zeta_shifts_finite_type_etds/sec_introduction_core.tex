...
\begin{proposition}[Finite-dimensionality of the main transfer data]
  ...
  chain does not require a H\"older/Ruelle transfer-operator theorem,
  Banach-space determinant convention, pressure-boundary rigidity statement,
  completed-zeta or xi normalisation, functional equation, spectral-window
  continuation theorem, or any single-primitive universality/certificate
  hierarchy.
\end{proposition}
\begin{proof}
  ...
  resonance theorem, a completed zeta function, a functional equation, or any
  Zeckendorf/EML/Richardson-style universality construction, so no such theorem
  is a hidden input to Main Theorems~I--III.
\end{proof}
...
\begin{table}[t]
  ...
  \begin{tabular}{L{0.21\textwidth}L{0.34\textwidth}L{0.36\textwidth}}
    ...
    Hermitian readout, quotient/shadow boundaries, and the \(S_3\) witness
    ...
    Auxiliary finite-tail boundary
      & No classical comparison input and no hypothesis for Main Theorems~I--III
      & The retained statement
        \Cref{prop:main-chain-secondary-noninterference} records only that
        lower-order finite tails do not change \(\mathcal M_C\),
        \(F_\varrho(\lambda^{-1})\), or \(K_C\); secondary-edge
        detection itself is split out. \\
    \bottomrule
  \end{tabular}
...
\paragraph{Proof guide.}
  ...
  remain outside the article; in particular, no secondary-edge skeleton,
  isolation theorem, tail-refinement theorem, product consequence, or
  dominant-modulus detection statement is used below.  The core protection
  against leakage is the finite-tail
