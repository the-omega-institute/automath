...
been separated.  Under the strict twisted gap this constant is governed by
...
fixes the trivial-channel normalization, the fixed-matrix cyclic-lift record
serves only as a scalar comparison boundary, the quotient and shadow
statements identify which coordinates are visible through abelian data, the
eta-gap tests verify the Perron-point hypothesis in finite examples, and the
\(S_3\) calculation certifies the concrete Artin-substitution obstruction.
These auxiliary units are used only through the displayed finite interfaces;
they do not import a H\"older/Ruelle transfer theorem, a completed-zeta or
\(\xi\)-type package, a secondary spectral-edge asymptotic, a scalar recovery
theorem, or a general certificate methodology into the proof of Main
Theorems~I--III.
...
The proof inputs for Main Theorems~I--III are the primitive matrix \(A\), the
...
evaluation allowed by \(\rho(A_\varrho)<\lambda\); no Banach-space transfer
operator or thermodynamic limiting theorem is a hidden input to the constants.
The only external directions named below are correspondingly narrow.  The
H\"older/Ruelle transfer-operator chain is not reconstructed here; related
cyclic-block, synchronisation-kernel, completed-zeta or \(\xi\)-type,
automata-density, and prime-language results are not local inputs; secondary
spectral-edge asymptotics enter only through the noninterference firewall
\Cref{prop:main-chain-secondary-noninterference}; fixed-matrix cyclic-lift
records are used only to mark a scalar comparison boundary; eta-gap diagnostics
are used only as finite tests or boundary obstructions, not as generic
strict-gap criteria; and finite rational records are used only as checkable
witnesses for the single determinant-constant obstruction, not as a general
verification framework or a family classification.
\end{proposition}
\begin{proof}
  Main Theorem~I is proved from the finite twisted determinant family and
  Adams--M\"obius inversion in \Cref{prop:main-theorem-i-assembly-map}.  Main
  Theorem~II is the strict-gap Perron-point package
  \Cref{thm:class-mertens-explicit,thm:class-mertens-fourier}.  Main
  Theorem~III is assembled from the fixed-label determinant evaluation and
  class-product rigidity in
  \Cref{thm:class-euler-product-mertens,prop:fixed-label-determinant-perron,thm:class-product-determinant-rigidity}.
  These cited statements have precisely the finite inputs listed in the
  proposition.  The only external material named in the paragraph
  is cited either as inherited background for existence and leading terms or
  as the noninterference statement
  \Cref{prop:main-chain-secondary-noninterference}; hence none is an additional
  hidden input to the determinant recovery of the constants.
\end{proof}
...
\begin{theorem}[Main Theorem III: Adams-corrected Frobenius-class Euler-product constants]
  ...
  Fixed-label product & \Cref{thm:fixed-label-euler-transform,thm:class-euler-product-mertens} & From \(\Pi_\varrho(\lambda^{-1})\) to \(F_\varrho(\lambda^{-1})\) and the positive constants \(K_C\). \\
