...
calculations, and the finite certificates in the appendices support the
normalisation and examples rather than providing another route to the product
constants.  The scalar calculation supplies the trivial-channel value
\(\mathcal M(A)\), while the canonical cyclic lift is a fixed-matrix
comparison model.  The optional secondary spectral-edge appendix refines
finite-length remainders after the constants have been identified.  Thus the
proofs of Main Theorems~I--III use finite-matrix determinant identities,
finite-group character orthogonality, Adams--M\"obius inversion, and
Perron--Frobenius theory for finite non-negative matrices.
...
Secondary-edge asymptotics are treated as a conditional refinement of the tail
after the constants have been identified; the appendix records the precise
finite-length statement used for that comparison.
...
\begin{table}[t]
  ...
  \begin{tabular}{L{0.21\textwidth}L{0.34\textwidth}L{0.36\textwidth}}
    ...
    Optional secondary-edge appendix
      & No classical comparison input and no hypothesis for Main Theorems~I--III
      & Gives a conditional finite-length tail refinement only after the
        constants have been determined; by
        \Cref{prop:secondary-spectral-edge-noninterference,prop:main-chain-secondary-noninterference}
        it does not change \(\mathcal M_C\), \(F_\varrho(\lambda^{-1})\), or
        \(K_C\). \\
    \bottomrule
  \end{tabular}
