\subsubsection{Strict-gap additive class constants}
...
\noindent\textbf{Operational gap certificate.}
Before applying the next theorem one must verify one of the finite certificates
of \Cref{prop:eta-gap-finite-state-certificate}.  Equivalently,
\[
  \operatorname{spec}(\widetilde A)\cap\{|z|=\lambda\}=\{\lambda\}
  \quad\text{with algebraic multiplicity }1,
\]
or, in the line-field form, there is no non-trivial irreducible
\(\varrho\), no \(\omega\in\TT\), and no non-zero vertex vectors
\(\xi_v\) satisfying
\[
  \varrho(\tau(e))\xi_{t(e)}=\omega\xi_{o(e)}
  \qquad(e\in E_A).
\]
This certificate is part of the theorem hypothesis, not a consequence of
primitivity of the base adjacency matrix.

\begin{theorem}[Class Mertens theorem conditional on the strict twisted
  spectral gap]
  \label{thm:class-mertens-explicit}
  Assume that \(A\) is the primitive adjacency matrix of a finite directed
  multigraph and that its Perron root satisfies \(\lambda>1\).  Conditional on
  the strict twisted spectral gap
  \[
  ...
  certified equivalently by the spectral-radius check or the line-field
  exclusion in \Cref{prop:eta-gap-finite-state-certificate}; it is not asserted
  for all cocycles.  This theorem is the strict-gap specialization of the
  Perron-boundary renormalized class theorem
  \Cref{thm:renormalized-class-mertens,cor:strict-gap-specialization}; its role
  here is to record the clean \(\eta<\lambda\) branch used by the determinant
  computations, not to impose the maximal boundary of the paper.  Perron-boundary
  constants below are functions of the
  branch-sensitive tuple \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\) in
  \eqref{eq:perron-boundary-data-tuple}, not of the determinant polynomials
  alone.
  ...
  class \(C\) and in \(n\).  The summatory conclusion of this strict-gap
  additive specialization is the harmonic form
...
\begin{corollary}[Logarithmic form conditional on the strict twisted gap]
  \label{cor:class-mertens-log-form}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}, in particular
  the strict twisted spectral gap \(\eta<\lambda\).  Then, for every
  ...
  internal proof of \Cref{thm:class-euler-product-mertens}; no external
  Frobenius-class product theorem is used as an input at this step.
\end{corollary}
