...
This section is a support section for the Adams obstruction used at the end
of Main Theorem~III.  After the constants of Main Theorems~II--III have
already been constructed, quotient functoriality identifies the abelianised
primitive profile, and Peter--Weyl orthogonality locates the part that no
cyclic quotient can see.  The final \(S_3\) model then exhibits the
...
paper.  Cyclic quotients of the fixed cocycle are kept separate from the
canonical cyclic-lift root-of-unity layer used later in \Cref{sec:finite-part};
neither construction is treated as data varying over graph resolutions or
folds.  The only role of these boundary results is to show where the
fixed-label Euler vector, and hence the Adams obstruction, escapes abelian or
cyclic shadow data.  Throughout this section, ``boundary'' means the exact
linear-algebraic boundary of what cyclic quotients can detect inside the
class-function space of \(G\).
...
\begin{theorem}[Finite-state projection boundary for scalar inputs]
  ...
  \begin{enumerate}[label=\textup{(\roman*)}]
    \item The map
      ...
      \]
      is constant on \(\mathscr T\). Equivalently, any fixed scalar record
      depending only on \(A\), such as
      \(\FP(A)\), \(\mathcal M(A)\), \(F_A\), or a canonical cyclic-lift
      scalar comparison record, is compatible with at most one normalised class
      profile on \(\mathscr T\).
...
\begin{corollary}[No scalar cyclic-lift descent without a constant
  ...
  \(\mathcal R(A)\) be any collection of fixed-matrix scalar comparison records
  formed from \(A\), for instance
  \(\FP(A)\), \(\mathcal M(A)\), \(F_A\), or canonical cyclic-lift scalar
  root-of-unity layers attached to \(A\). These records are attached to the
  single matrix \(A\), with no fold map, resolution change, spectrum
  reconstruction claim, or cocycle descent structure included. Then the
  following are
  equivalent.
  \begin{enumerate}[label=\textup{(\roman*)}]
  ...
  fixed-matrix scalar or cyclic-lift comparison record of \(A\) descends to a stable
  ...
  such fixed-matrix scalar records.
\end{corollary}
