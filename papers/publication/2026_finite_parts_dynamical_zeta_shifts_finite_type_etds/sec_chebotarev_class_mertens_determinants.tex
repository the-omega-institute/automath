...
\begin{corollary}[Finite verification of the product constants]
  ...
  \Cref{prop:fixed-label-perron-finite-algorithm}.  This is a finite-verification
  ...
  necessary and sufficient for the certificate records used in this paper.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
...
\begin{proposition}[Robustness of the Perron-point constants]
  ...
  coordinates and are unchanged by auxiliary lower-order finite tails whose
  compensated weighted tails vanish.
  ...
  \begin{equation}\label{eq:secondary-noninterference-refined-sum}
    ...
    -T_C(N)+o(1),
  \end{equation}
  then the auxiliary tail lies below the Perron-boundary level and
  contributes no new Frobenius-class product constant.  More explicitly, the
