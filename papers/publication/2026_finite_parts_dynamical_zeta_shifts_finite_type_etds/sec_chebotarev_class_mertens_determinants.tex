...
\noindent\textit{Product-level finite consequence.}
  ...
  the logarithms of all class-product constants \(K_C\).  This follows directly from \Cref{prop:fixed-label-perron-finite-algorithm}:
  the displayed enclosure is the finite truncation plus the certified discarded
  tail, with the scalar Perron logarithm deleted before any boundary value is
  taken.  Repeating the same accepted-record argument over all
  \(\varrho\ne\mathbf1\) gives enclosures for the full non-trivial vector
  ...
  character-table operations once \(C(A)\) has fixed the scalar coordinate, so
  those enclosures transport to the logarithms of the class-product constants
  in \Cref{thm:class-euler-product-mertens}.  The appendix records the same
  finite rational fields for the \(S_3\) witness; it supplies transcripts for
  the obstruction, not a general parser, branch tracker, interval-arithmetic
  library, or universal certification theory for all finite groups and
  matrices.
