...
This appendix is reproducibility support for the \(S_3\) obstruction used in
...
\begin{corollary}[Displayed rational certificate windows]
  ...
  \[
    ...
    -\frac{923}{1000}<F_\rho(1/2)<-\frac{922}{1000}.
  \]
  In particular the \(S_3\) witness is an auditable rational certificate for
...
\begin{remark}[Exact-arithmetic reproducibility]
  ...
  comparisons.  The auxiliary files with basename
  \texttt{s3\_log\_certificates} in the \texttt{certificates/} directory are
  serializations of this exact-arithmetic transcript, including the script,
  certificate, output, and run note.  The run note records the two exact
  regeneration commands: one writes the certificate file
  \texttt{certificates/s3\_log\_certificates.cert}, and one redirects the same
  exact-rational transcript to
  \texttt{certificates/s3\_log\_certificates.out}.  Both recorded commands have
  exit status zero; the output transcript terminates with the message that the
  fixed-label windows have been verified.  The stored certificate file records every
  rational logarithm bracket, every signed rational coefficient, the four
  rational tail bounds, and the final cleared-denominator comparisons backing
  the displayed intervals.  They are not additional mathematical inputs for the
  appendix, and none of the inequalities above depends on independently
  validating those external serializations.
\end{remark}
