...
\begin{remark}[Finite exact-arithmetic check]
  ...
  above is self-contained from the displayed rational data, and the printed
  rational inequalities are the certificate used in the argument.

  The calculation can also be checked from the auxiliary files
  \path{s3_log_certificates.py}, \path{s3_log_certificates.cert},
  \path{s3_log_certificates.out}, and \path{s3_log_certificates.run.txt} in
  \path{certificates/}.  These files reproduce the displayed rational endpoints
  and interval comparisons; they are reproducibility aids and not additional
