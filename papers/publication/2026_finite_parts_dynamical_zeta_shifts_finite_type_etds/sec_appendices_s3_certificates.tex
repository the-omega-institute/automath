...
\begin{proof}
  ...
  below.  No decimal approximation is used.  The optional exact-arithmetic
  files described in \Cref{prop:appendix-s3-reproducibility-check} serialize
  the same calculation; the proof in the main text depends only on the
  displayed rational endpoints, coefficients, and inequalities.
...
\begin{proposition}[Optional exact-arithmetic reproducibility check for the
  \(S_3\) certificates]
  \label{prop:appendix-s3-reproducibility-check}
  The source file
  \[
  ...
  The following optional reproducibility data identify the serialization:
  ...
  \begin{center}
    ...
    \texttt{certificates/s3\_log\_certificates.cert}.
  \end{center}
  More precisely, the auxiliary check contains no floating-point input to the
...
\begin{proof}
  ...
  coefficient reverses it; hence the signed endpoint selection in the check
  ...
  order-preserving finite integer calculation.  Thus the auxiliary file is an
  optional serialization of the appendix proof.  Together with
  \Cref{prop:fixed-label-certificate-protocol}, it verifies the
  finite Perron-readout certificate used by this \(S_3\) class-product
  obstruction, without adding a separate computational theorem to the
  paper.
\end{proof}
