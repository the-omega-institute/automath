...
coarse rational non-vanishing windows; here we record the truncations,
tail estimates, and the final rational enclosures needed for this example.  The appendix is not a catalogue of finite-group obstruction examples and not a
general finite-verification theory for arbitrary finite groups. This is the exhaustive-appendix certificate policy for the submitted article:
every rational truncation, tail estimate, and interval enclosure used by the
example is displayed here, while the main text cites only the coarse windows.
...
\begin{proof}
  ...
  \[
    ...
    -\frac1{32}.
  \]
  These displayed rational bounds are the finite verification used here: for
...
\begin{remark}[Exact-arithmetic reproducibility]
  ...
  comparisons.  Any machine transcript is only a serialization of this finite
  rational record; it is not an additional mathematical input for the appendix.
\end{remark}
