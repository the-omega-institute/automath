...
termwise rational arithmetic is recorded in the included certificate file
\texttt{certificates/s3\_log\_certificates.cert}; its SHA256 digest is
\[
  \texttt{22c4a0f015f8c579680b87b378bfac19351ca66421c3988a025fea9b433cff62}.
\]
The short script \texttt{certificates/s3\_log\_certificates.py}
reproduces the same rational interval arithmetic; it is a reproducibility
aid, while the proof below is self-contained and prints the finite-sum
endpoints needed for clearing denominators.
...
\begin{lemma}[Exact rational certificate for the \(S_3\) fixed-label
  Euler transforms]
  \label{lem:appendix-s3-f-level-certificates}
  In the \(S_3\)-example of \Cref{sec:shadows},
...
\begin{proof}
  We use the same rational logarithm bounds as in the proof of
  \Cref{lem:appendix-s3-sharp-certificates}: for rational \(q>1\),
  \[
    L_3(q)<\log q<U_3(q),
  \]
  where
  ...
  \eqref{eq:s3-example-pi-rho} with
  \(F_\rho(z)=\sum_{r\ge1}\Pi_\rho(z^r)/r\). Grouping by
  \(k=rm\) gives
  \[
    F_\rho(1/2)=
    \sum_{k\ge1}\frac{1}{k}
  ...
  logarithms in \(S_{32}\) gives rational endpoints
  \(S_{32}^-<S_{32}<S_{32}^+\)
  and hence
  \[
    S_{32}^- -B_\rho < F_\rho(1/2)<S_{32}^+ +B_\rho.
  \]
  The exact values of \(S_{32}^-\), \(S_{32}^+\),
  \(S_{32}^--B_\rho\), and \(S_{32}^++B_\rho\) are printed in
  \texttt{certificates/s3\_log\_certificates.cert} with the checksum given
  at the start of this appendix. The shorter denominator-cleared margins
  needed in the manuscript are
  \[
  \begin{array}{c|c|c}
    \text{comparison} & \text{exact positive margin} &
      \text{rational lower bound}\\
