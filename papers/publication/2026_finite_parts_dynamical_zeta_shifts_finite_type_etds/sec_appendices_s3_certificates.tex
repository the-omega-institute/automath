...
certificate, not a catalogue of finite-group obstruction examples. The
proof below is self-contained: every rational truncation, tail estimate,
and interval enclosure used by the example is displayed in the appendix.
...
\begin{proof}
  ...
  \[
    ...
    -\frac1{32}.
  \]
  The exact rational endpoint certificate for these substitutions is also
  recorded in the reproducibility artifact
  \texttt{certificates/s3\_log\_certificates.cert}, generated by
  \texttt{certificates/s3\_log\_certificates.py}.  In the block headed
  \texttt{S32 termwise certificates}, the artifact prints for every term its
  rational logarithm bounds and weighted rational contribution; in the block
  headed \texttt{Fixed-label total certificates}, it prints the exact
  fractions labelled \texttt{S32 lower}, \texttt{S32 upper}, \texttt{B\_rho},
  \texttt{F\_rho lower}, and \texttt{F\_rho upper}.  Thus the manuscript
  inequalities below, and not any decimal approximation, are checkable from
  the printed rational numerators and denominators.
