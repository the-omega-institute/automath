...
tail estimates, and the final rational enclosures. The proof below is
self-contained. A certificate file and a companion script in the
\texttt{certificates} directory record and reproduce the same termwise
rational arithmetic; the certificate SHA256 begins
\texttt{22c4a0f015f8c579}.
...
\begin{proof}
  ...
  \end{aligned}
  \]
  Equivalently, the two margins themselves are larger than
  \(789/10^6\) and \(204/10^6\). These inequalities are obtained by
  clearing the positive denominators in the rational expressions produced
  by the displayed finite list of logarithmic arguments.
  Thus the interval check no longer depends on an external certificate
  file.  The shorter numerical consequences are
  \[
  \begin{array}{c|c|c}
    \text{comparison} & \text{exact positive margin} &
      \text{rational lower bound}\\
