...
\begin{proof}
  ...
  below.  No decimal approximation is used.  The exact-arithmetic check described below serializes the same calculation; the proof in the main text depends only on the
...
\begin{remark}[Exact-arithmetic reproducibility]
  ...
  An auxiliary exact-arithmetic script can serialize the same calculation by
  computing the rational logarithm endpoints \(L_3(q)\le\log q\le U_3(q)\),
  multiplying them by the signed rational coefficients in \(E_7\), \(R_{15}\),
  \(Q_8\), and \(S_{32}\), adding the four rational tails, and clearing
  positive denominators in the four final interval comparisons. This
  reproducibility check is not an additional hypothesis or a separate
  computational theorem: all inequalities needed for the \(S_3\) obstruction
  are printed above as rational inequalities.
\end{remark}
