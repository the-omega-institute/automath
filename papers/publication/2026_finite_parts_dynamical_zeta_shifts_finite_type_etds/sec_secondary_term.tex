...
define or recover the Adams-corrected constants.  Its interface with the
main chain is only after Main Theorems~II--III and the determinant readout
of \Cref{cor:assembled-adams-frobenius-product-package} have supplied the
constants \(\mathcal M_C(A,\tau;G)\), \(K_C\), and their non-scalar
Peter--Weyl coordinates: when a non-Perron spectral edge is isolated above
the square-root contribution from repeated primitive orbits, the finite
Peter--Weyl trace formula identifies the first oscillating tail around
those constants.  The hypotheses are the explicit finite-dimensional ones
used below: \(\lambda>1\), \(\eta<\lambda\), \(R>\lambda^{1/2}\), and, for
the sharper tail estimate, \(\Gamma_{\mathrm{sec}}<R\).  Without these
separation hypotheses no secondary expansion is asserted.
