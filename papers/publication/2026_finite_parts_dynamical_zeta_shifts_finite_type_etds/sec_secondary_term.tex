\section{Optional conditional secondary spectral-edge tail refinement}
\label{sec:secondary-term}

This appendix gives an optional finite conditional refinement of the
remainders in the class-Mertens and product formulae. It is not used to
define or recover the Adams-corrected constants, and no result in the main
theorem spine depends on the hypotheses introduced here. Its interface
with the main chain is only after Main Theorems~II--III and the determinant
readout of \Cref{cor:assembled-adams-frobenius-product-package} have
supplied the constants \(\mathcal M_C(A,\tau;G)\), \(K_C\), and their
non-scalar Peter--Weyl coordinates: when a non-Perron spectral edge is
isolated above the square-root contribution from repeated primitive
orbits, the finite Peter--Weyl trace formula identifies the first
oscillating tail around those constants. The hypotheses are the explicit
finite-dimensional ones used below: \(\lambda>1\), \(\eta<\lambda\),
\(R>\lambda^{1/2}\), and, for the sharper tail estimate,
\(\Gamma_{\mathrm{sec}}<R\). Without these separation hypotheses no
secondary expansion is asserted.
...
\begin{theorem}[Conditional refinement above the square-root
  barrier]
  ...
  \(\Gamma_{\mathrm{sec}}<R\), neither of which is used to establish the
  constants in Main Theorems~II--III, the series
