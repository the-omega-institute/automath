...
This appendix is retained as a finite conditional refinement of the
remainders in the class-Mertens and product formulae.  When a non-Perron
spectral edge is isolated above the square-root contribution from repeated
primitive orbits, the finite Peter--Weyl trace formula identifies the first
oscillating tail after the main constants.  The hypotheses are the explicit
finite-dimensional ones used below:
\(\lambda>1\), \(\eta<\lambda\), \(R>\lambda^{1/2}\), and, for the sharper
tail estimate, \(\Gamma_{\mathrm{sec}}<R\).  The constants
\(\mathcal M_C(A,\tau;G)\) and \(K_C\) remain the Perron-point constants
defined in the main text.  Without these separation hypotheses no secondary
expansion is asserted.  Broader questions about sharpness, resonance
collisions, and general secondary-edge theory are left outside the present
finite Adams-corrected class-product calculation.
