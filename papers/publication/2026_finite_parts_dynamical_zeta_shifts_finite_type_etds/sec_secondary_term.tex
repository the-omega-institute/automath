\section{Optional appendix: secondary spectral-edge tail refinement}
\label{sec:secondary-term}

This appendix is a conditional refinement, not a component of the main
theorem spine. It records the finite-tail refinement that is available
when a non-Perron spectral edge is isolated above the square-root
contribution from repeated primitive orbits. Its support role is precise:
under the
explicit hypotheses \(\lambda>1\), \(\eta<\lambda\),
\(R>\lambda^{1/2}\), and \(\Gamma_{\mathrm{sec}}<R\) where required
below, the same finite Peter--Weyl trace formula identifies the first
oscillating tail in the remainder of \Cref{thm:class-mertens-explicit}
and of the product asymptotic in \Cref{thm:class-euler-product-mertens}.
The constants \(\mathcal M_C(A,\tau;G)\) and \(K_C\) remain the
Perron-point constants already defined in the main text, and the
finite-group Adams--M\"obius, class-Mertens, and Adams-corrected product
theorems do not use any result from this appendix. Without these separation
hypotheses, no secondary expansion or spectral readout is asserted. Thus
the appendix is not a general secondary spectral-edge theory; it only
records the compact refinement needed to verify that such tails do not
change the primary Adams-corrected constants.
...
\begin{proposition}[Optional appendix finite edge skeleton under
  ...
  \begin{equation}\label{eq:secondary-edge-inverse}
    ...
    \quad(\pi\neq\mathbf 1).
  \end{equation}
  Hence this appendix secondary correction, only under the conditional
...
\begin{proposition}[Optional secondary edge refines only the appendix
  product tail]
  ...
  the remainder terms while keeping the Perron-point constants fixed. For every conjugacy class
  ...
  \(K_C\) as in \Cref{thm:class-euler-product-mertens}, with finite-length secondary
  ...
  constants, and none of the main Adams--M\"obius, class-Mertens, or
  Adams-corrected product assertions depends on this refinement.
\end{proposition}
...
\begin{corollary}[Optional appendix detection of the conditional dominant
  secondary modulus]
  \label{cor:class-secondary-term-detects-spectrum}
  Under the additional separation hypotheses of the appendix refinement
\Cref{prop:class-mertens-secondary-term}, and only in that conditional
appendix regime,
  \[
    R
    =
