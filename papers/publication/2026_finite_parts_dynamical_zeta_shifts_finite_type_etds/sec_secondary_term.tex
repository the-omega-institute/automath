...
This appendix gives a finite conditional refinement of the remainders in
the class-Mertens and product formulae.  Its interface with the main chain
is only after Main Theorems~II--III have supplied the constants
\(\mathcal M_C(A,\tau;G)\) and \(K_C\): when a non-Perron spectral edge is
isolated above the square-root contribution from repeated primitive orbits,
the finite Peter--Weyl trace formula identifies the first oscillating tail
around those constants.  The hypotheses are the explicit
