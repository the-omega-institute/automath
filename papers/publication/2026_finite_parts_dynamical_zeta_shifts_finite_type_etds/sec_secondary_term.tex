...
This optional appendix has one supporting role and is not part of, and is
not cited as an input to, Main Theorems~I--III: when a non-Perron spectral
edge is isolated above the square-root contribution from repeated
primitive orbits, it records the corresponding first oscillating tail in
the remainder of \Cref{thm:class-mertens-explicit}. It does not alter the constants
...
the Adams--M\"obius determinant spine. Under the explicit appendix
hypotheses \(\lambda>1\), \(\eta<\lambda\), \(R>\lambda^{1/2}\), and
\(\Gamma_{\mathrm{sec}}<R\) where required below, the same finite
Peter--Weyl trace formula identifies that tail. Without these separation
hypotheses, no secondary expansion or spectral readout is asserted.
...
\begin{proposition}[Optional appendix secondary edge is tail-only]
  ...
  \Cref{thm:class-euler-product-mertens}. This is a conditional appendix
  refinement of the remainder only, not an additional input to Main
  Theorems~I--III. For every conjugacy class
