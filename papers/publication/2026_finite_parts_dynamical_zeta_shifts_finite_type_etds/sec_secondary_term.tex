\section{Appendix: conditional tail refinement at an isolated secondary edge}
\label{sec:secondary-term}

This appendix is not part of the numbered main theorem spine.  It records only an optional
conditional finite-dimensional refinement of the remainder in
\Cref{thm:class-mertens-explicit}. It is subordinate to the class-Mertens
theorem: it changes neither the constants
\(\mathcal M_C(A,\tau;G)\), the Euler-product constants \(K_C\), nor the
Adams--M\"obius theorem spine. Under the explicit hypotheses
\(\lambda>1\), \(\eta<\lambda\), and \(R>\lambda^{1/2}\), a non-Perron
spectral edge is isolated above the square-root contribution from
repeated primitive orbits, and the same Peter--Weyl trace formula
identifies the first oscillating tail. Without this separation hypothesis,
no secondary expansion is asserted.
