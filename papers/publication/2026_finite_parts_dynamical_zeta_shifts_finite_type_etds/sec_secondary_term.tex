\section{Optional appendix refinement: secondary spectral edge}
\label{sec:secondary-term}

This section is not part of the two-block theorem spine.  Main
Theorem~I supplies the Adams--M\"obius primitive-channel recovery, and
Main Theorem~II supplies the class Mertens and Adams-corrected Euler
product constants.  The refinement below is retained only as an
appendix check on the error term in \Cref{thm:class-mertens-explicit}:
when a non-Perron spectral edge lies strictly above the square-root
contribution from repeated primitive orbits, the same Peter--Weyl trace
formula identifies the first oscillating remainder.  Without that extra
separation hypothesis, this section asserts no additional main theorem.
It may be omitted from a shorter symbolic-dynamics version without
changing Main Theorems~I--II, the quotient/shadow boundary, the
eta-gap rigidity statements, or the \(S_3\) obstruction certificates.
The sole interface with the main theorem chain is the already-proved
Peter--Weyl primitive trace formula: the optional skeleton below records
...
\begin{theorem}[Optional appendix edge skeleton]
  ...
  \begin{equation}\label{eq:secondary-edge-inverse}
    ...
    \quad(\pi\neq\mathbf 1).
  \end{equation}
  Hence the optional secondary correction is completely exhausted by
...
\begin{theorem}[Optional appendix refinement: secondary term above the
  square-root barrier]
...
\begin{corollary}[Optional appendix detection of the dominant modulus]
