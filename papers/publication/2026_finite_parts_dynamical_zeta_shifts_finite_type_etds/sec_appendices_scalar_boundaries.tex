...
This appendix supplies only the scalar normalisation used in Main
Theorems~II--III and the no-descent safeguards needed to keep that
normalisation from being mistaken for a class-product invariant.  The scalar
finite part \(\mathcal M(A)=\gamma+\FP(A)\), its local boundary variation, and
the canonical cyclic-lift comparison live in the trivial channel; the
non-trivial fixed-label coordinates \((F_\varrho(\lambda^{-1}))_{\varrho\ne\mathbf1}\)
come from the finite-group determinant theory in the main text.  The results
below therefore normalise and delimit scalar data only: they do not recover
non-abelian class products, assert root-of-unity sampling for arbitrary fixed
cocycles, or supply cross-resolution descent invariants.
