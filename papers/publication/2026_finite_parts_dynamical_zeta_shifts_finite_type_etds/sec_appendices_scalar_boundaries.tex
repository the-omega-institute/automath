\section{Auxiliary scalar boundary checks}
...
\[
  ...
  \qquad q\ge d .
\]
The following scalar facts are appendix support only.  They delimit the
two ways in which the trivial-channel finite part may be checked outside
the fixed matrix used in Main Theorem II: holomorphic variation in a
...
certificate.  Their purpose is instead to keep scalar normalisation from
being mistaken for a second primitive-channel theorem spine.

\begin{theorem}[Auxiliary scalar check: holomorphic variation of the
  scalar finite part]
...
\begin{proposition}[Auxiliary scalar boundary: perturbations do not
...
\begin{theorem}[Auxiliary scalar check: effective truncation of the
  scalar finite part]
...
used in the main theorem spine here, which requires only the fixed-
