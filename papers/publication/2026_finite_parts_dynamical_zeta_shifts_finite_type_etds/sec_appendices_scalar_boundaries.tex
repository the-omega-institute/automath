...
\[
  ...
  \qquad q\ge d .
\]
The following scalar facts have a supporting role. They record two
standard checks on the trivial-channel finite part beyond the single
fixed matrix used in the main finite-group argument: holomorphic
variation in a local family and effective determinant truncation for
reproducible numerics. The finite-group Adams--M\"obius and
class-product theorems themselves use only the fixed-matrix
...
\(\Lambda(A)/\lambda\), \(d\), and \(\lambda\). In this paper these
estimates serve only as scalar background around the fixed-matrix
normalisation and are not part of the finite-group class-product
mechanism.
