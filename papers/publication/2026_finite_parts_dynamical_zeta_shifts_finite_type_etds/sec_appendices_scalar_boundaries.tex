...
\[
  ...
  \qquad q\ge d .
\]
The following scalar facts explain the boundary of the scalar
normalisation used by the finite-group theorem chain.  Holomorphic
variation and effective determinant truncation provide checks on the
trivial-channel finite part, while the Adams--M\"obius and class-product
theorems use from this appendix only the fixed-matrix normalisation
\(\mathcal M(A)=\gamma+\FP(A)\) and the canonical cyclic-lift comparison
from \Cref{sec:finite-part}.
...
the relative non-Perron spectrum from scalar root-of-unity data. Arithmetic
refinements of the same data remain statements about the canonical cyclic
lift of the base matrix. Their interface with the main theorem chain is
the scalar normalisation; cyclic quotients of a fixed cocycle, treated in
\Cref{sec:shadows}, are different functorial pullbacks and detect only the
