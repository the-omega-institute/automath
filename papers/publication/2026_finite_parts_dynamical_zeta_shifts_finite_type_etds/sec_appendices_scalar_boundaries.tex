...
root-of-unity data. Any finer arithmetic refinements of the same data are
therefore scalar statements about the canonical cyclic lift. They are not
class-profile invariants of the fixed cocycle: the cyclic quotients in
\Cref{sec:shadows} are pullbacks along quotient maps and detect only the
one-dimensional Peter--Weyl sector.
...
\(\Lambda(A)/\lambda\), \(d\), and \(\lambda\). In the present
fixed-matrix class-product theorem these estimates have only a boundary
role: they document the scalar normalisation regime and do not enter the
finite-group Adams--Frobenius mechanism.
