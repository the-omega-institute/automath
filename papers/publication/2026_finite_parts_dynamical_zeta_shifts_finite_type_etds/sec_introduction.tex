...
recovery, the Hermitian class transform, and the \(S_3\) obstruction showing
...
\begin{proposition}[Theorem-by-theorem novelty calibration]
  ...
  \midrule
  Main Theorem~I
  & Uses only the finite determinant identity behind the usual Artin--Ruelle
    zeta formalism of Parry--Pollicott.
  & Recovers primitive fixed-label Peter--Weyl channels from finite twisted
    ...
    \(\psi^m\phi(g)=\phi(g^m)\). \\
  Main Theorem~II
  & Takes the leading Frobenius density and Mertens exponent as the same
    classical layer represented by Parry--Pollicott, Noorani--Parry, Sharp,
    and Mesliza--Noorani.
  ...
  & Keeps the classical Euler-product exponent \(|C|/|G|\) and the existence
    form of Frobenius-class product asymptotics.
  ...
  Secondary appendix and recent comparisons
  & Coles, Humbert, O'Hare, and Crismale--Del Vecchio--Griseta--Rossi give
    current comparison points for zeta, resonance, finite-data, and
    skew-product questions.
  & The appendix is conditional and non-interfering; it supplies no input to
    Main Theorems~I--III and no additional product constant. \\
  \bottomrule
  \end{tabular}
  \end{center}
...
\begin{proof}
  Main Theorem~I is proved from the finite-matrix determinant expansion
  ...
  \(g\mapsto g^m\).  These are the body results cited in its proof, so the
  only output not already present in the classical determinant formalism is
  the fixed-label primitive recovery formula.

  Main Theorem~II uses the leading density term
  ...
  channels at \(z=\lambda^{-1}\).  The cited classical works provide the
  comparison class for the leading prime-orbit, Chebotarev, and Mertens
  layers; the theorem's extra content is the finite determinant expression
  for \(\mathcal M_C(A,\tau;G)\) under the stated strict twisted gap.

  Main Theorem~III exponentiates the classwise Mertens expansion, so the
  exponent \(|C|/|G|\) is the classical part.  Its non-scalar coordinate is
  the fixed-label Euler value \(F_\varrho(\lambda^{-1})\), and
  ...
  product constants, and supplies the obstruction criterion.  Finally,
  \Cref{thm:secondary-spectral-edge-noninterference,%
  thm:secondary-package-supporting-classification} show that the conditional
  secondary appendix is downstream from the three headline theorems.  The
