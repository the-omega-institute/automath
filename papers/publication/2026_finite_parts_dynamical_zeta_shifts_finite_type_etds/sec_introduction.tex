...
strict Perron-point input of Main Theorems~II--III.  The scalar dynamical \(\zeta\)-function
\[
  \zeta_A(z)
...
computations, and the finite certificates in the appendices are supporting
material rather than a second main theorem chain.  The scalar calculation
supplies the trivial-channel normalisation \(\mathcal M(A)\).  The cyclic
lift is a fixed-matrix comparison model and records a no-descent boundary; it
is not an invariant of arbitrary cyclic quotients of the cocycle.  The
retained secondary spectral-edge refinement in \Cref{sec:secondary-term} is
not used in the main theorem chain: it assumes additional spectral separation
and refines only post-constant error terms, not
...
Banach-space transfer theorem or Ruelle resonance theorem is a premise of the
paper.
...
\begin{theorem}[Main Theorem III: Adams-corrected Frobenius-class Euler-product constants]
  ...
  logarithm.  More sharply, the non-trivial Peter--Weyl boundary is a
  minimal coordinate skeleton for the scalar-normalised product constants:
...
The main theorem chain is rigid under later secondary-edge refinements:
\Cref{thm:main-spine-secondary-noninterference} proves that any finite
secondary package whose compensated tails vanish leaves
...
\begin{proof}
  ...
  comparison, the determinant-coordinate form, the obstruction to
...
\paragraph{Proof guide.}
  ...
  The conditional secondary refinement retained in
  ...
  statements are retained as a conditional refinement and do not supply any
...
\begin{proposition}[Modern calibration of the theorem packages]
  \label{prop:modern-theorem-package-calibration}
  The theorem packages retained in this paper have the following
  theorem-local scope relative to the recent comparison works
  ...
  \toprule
  Package & Output proved here & Calibration against the modern works \\
  \midrule
  Main Theorems~I--III & Finite twisted determinants determine primitive
  Peter--Weyl channels, Perron-point class Mertens constants, and
  Adams-corrected fixed-label Euler-product constants. & Not a geometric
  periodic-orbit observable theorem, not a Ruelle-resonance theorem, not a
  finite-data rigidity theorem for expanding maps, and not an
  operator-algebraic skew-product classification. \\
  Quotient and shadow package & The class profile has quotient functoriality,
  abelian-shadow defect, and a boundary for cyclic detection. & A
  no-descent theorem for the fixed finite determinant package, rather than a
  reconstruction or classification theorem from arbitrary finite samples. \\
  Eta-gap diagnostic package & Equality at the Perron boundary is detected by
  finite-dimensional line fields, loop labels, and character obstructions. &
  A finite-matrix diagnostic for the hypothesis \(\eta<\lambda\), not an
  assertion about Banach-space resonance spectra. \\
  \(S_3\) certificates & Rational finite-state certificates show that
  fixed-label product constants are not obtained by naive Artin
  substitution. & A concrete obstruction inside one finite SFT extension, not
  a general geometric invariant or operator-algebraic skew-product invariant.
  \\
  \bottomrule
  \end{tabular}
  \end{center}
  Consequently the novelty claimed in Main Theorems~I--III and their
  supporting diagnostic packages is exactly the finite-state Adams--M\"obius recovery
  and product-constant obstruction described in the second column.
\end{proposition}

\begin{proof}
  Main Theorem~I is proved from the finite trace identity, finite character
  linearisation, and Adams--M\"obius primitive inversion in
  \Cref{lem:twisted-trace-identity,thm:class-function-linearisation,%
  cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant}.  Main
  Theorems~II--III are proved from the finite gap evaluation and fixed-label
  product formula in
  \Cref{thm:class-mertens-explicit,thm:class-euler-product-mertens,%
  prop:class-product-comparison}.  Those statements have as input a fixed
  finite SFT matrix cocycle and as output primitive channels,
  \(\mathcal M_C(A,\tau;G)\), \(F_\varrho(\lambda^{-1})\), and \(K_C\).
  The comparison works cited in the statement concern, respectively,
  geometric periodic-orbit observables for Axiom~A flows, first Ruelle
  resonances for Anosov flows with smooth potentials, finite-data rigidity
  for one-dimensional expanding maps, and operator-algebraic
  non-commutative skew-product systems.  None has the fixed-label transform
  \(F_\varrho(z)=\sum_{r\ge1}\Pi_\varrho(z^r)/r\), the Adams--M\"obius
  determinant inversion, or the class-product constant \(K_C\) as its
  theorem output.

  The shadow assertions in the table are exactly
  \Cref{thm:quotient-functoriality,thm:abelian-shadow-defect,%
  thm:cyclic-detection-boundary}; they compare images or shadows of the
  same finite determinant package and therefore do not supply a competing
  finite-data rigidity theorem.  The eta-gap assertions are exactly
  \Cref{thm:eta-gap-rigidity,cor:eta-gap-boundary-classifier,%
  prop:eta-gap-loop-subgroup-obstruction}; they classify when the finite
  twisted matrices can touch the Perron boundary and are used only to test
  the hypothesis \(\eta<\lambda\).  The finite certificates are
  ...
  they certify a specific obstruction to replacing \(F_\varrho\) by
  \(\mathcal L_\varrho\).  These cited internal theorems prove every output
  listed in the table, while the cited recent works occupy the distinct
  scopes named in the third column.  The claimed calibration follows.
\end{proof}
...
obstruction.  The retained conditional secondary-edge tail refinement is
