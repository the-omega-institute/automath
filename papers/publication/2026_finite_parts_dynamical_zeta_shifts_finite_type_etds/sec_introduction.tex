...
loop-subgroup obstruction, quotient, cyclic-detection, and certificate statements serve
...
cocycle.  The conditional secondary refinement in \Cref{sec:secondary-term}
is appendix material only: it assumes additional spectral separation and
refines post-constant error terms, not \(\Pi_\varrho(\lambda^{-1})\),
\(\mathcal M_C(A,\tau;G)\), \(F_\varrho(\lambda^{-1})\), or \(K_C\). Thus the
...
normalisation, or sibling determinant package is a premise of the paper.
...
Theorem~II adds the strict Perron-point gap and is proved by
\Cref{thm:class-mertens-explicit,prop:class-mertens-deviations,%
thm:class-mertens-fourier}.  Main Theorem~III is the product-level assembly
from \Cref{thm:class-euler-product-mertens,prop:fixed-label-determinant-perron,%
thm:class-product-determinant-rigidity,prop:class-product-comparison,%
thm:adams-obstruction-class-product}; the concrete \(S_3\) obstruction is
certified by \Cref{prop:s3-naive-artin-obstruction,%
lem:s3-example-perron-certificates,lem:appendix-s3-f-level-certificates} and
...
\begin{proposition}[Finite-dimensional reduction]
  ...
  Completed zeta normalisations and functional-equation statements are also
  absent from this finite determinant package.
\end{proposition}
...
OHare2025FiniteDataRigidity,CrismaleDelVecchioGrisetaRossi2025NoncommutativeSkewProduct};
...
\begin{proposition}[Theorem-by-theorem novelty calibration]
  \label{prop:intro-theorem-novelty-calibration}
  Under the hypotheses stated in the three headline theorems below, their
  dependence on previous dynamical-zeta and symbolic-dynamics results is
  exactly as follows.
  \begin{center}
  \begin{tabular}{@{}L{0.23\textwidth}L{0.32\textwidth}L{0.35\textwidth}@{}}
  \toprule
  Result & Classical background & New finite-state output \\
  \midrule
  Main Theorem~I
  & Uses the finite determinant identity and transfer-operator bookkeeping in
    the Parry--Pollicott Artin--Ruelle framework, not a new zeta rationality
    theorem.
  & Recovers primitive fixed-label Peter--Weyl channels from finite twisted
    determinants by Adams--M\"obius inversion; repetitions are corrected by
    \(\psi^m\phi(g)=\phi(g^m)\). \\
  Main Theorem~II
  & Takes the leading Frobenius density from the Parry--Pollicott and
    Noorani--Parry Chebotarev layer, and the scalar/class Mertens comparison
    from Sharp and Mesliza--Noorani.
  & Gives the classwise constant term from the non-trivial primitive channels
    and evaluates it at the Perron point from finite determinant data under
    the explicit twisted gap. \\
  Main Theorem~III
  & Keeps the classical Euler-product exponent \(|C|/|G|\) and the existence
    form of Frobenius-class product asymptotics from the preceding Mertens
    literature.
  & Identifies the product constant with the fixed-label Euler transform,
    proves determinant recovery of that constant, and separates it from the
    ordinary periodic Artin logarithm. \\
  Appendix-only comparisons
  & Coles, Humbert, O'Hare, and Crismale--Del Vecchio--Griseta--Rossi mark
    current neighbouring questions: periodic-orbit invariants, Ruelle
    resonances, finite-data rigidity, and non-commutative skew products.
  & The secondary appendix is conditional and non-interfering; it is not a
    novelty driver, supplies no input to Main Theorems~I--III, and contributes
    no additional product constant. \\
  \bottomrule
  \end{tabular}
  \end{center}
  Consequently the paper does not claim a new leading prime-orbit theorem,
  a new Chebotarev density theorem, or a new Tauberian Mertens theorem of
  the classical kind.  Its in-scope novelty is the finite-state
  Adams--M\"obius passage from periodic determinant logarithms to fixed-label
  primitive data and the determinant recovery of the resulting class-product
  constants.
\end{proposition}

\begin{proof}
  The citations in the first row have exactly the following role.  The
  finite-matrix determinant expansion
  \(\log\det(I-zA_\varrho)^{-1}=\sum_{n\ge1}\Tr(A_\varrho^n)z^n/n\), the
  twisted trace identity, finite character orthogonality, and ordinary
  M\"obius inversion applied after the Adams operation records the power
  \(g\mapsto g^m\) are the finite-state analogue of the determinant side of
  the Artin--Ruelle formalism developed systematically by Parry--Pollicott.
  The present proof then adds the Adams-corrected primitive extraction; this
  is the fixed-label primitive recovery formula, not another proof of the
  classical determinant formalism.

  For the second row, the leading density term
  \((|C|/|G|)\lambda^n/n\) and then isolates the constant term by subtracting
  the scalar harmonic contribution and inserting the non-trivial primitive
  channels at \(z=\lambda^{-1}\).  Parry--Pollicott and Noorani--Parry are
  used as the Chebotarev source class for finite and hyperbolic extensions;
  Sharp is the closed-orbit Mertens source class; Mesliza--Noorani is the
  Frobenius-class Mertens source class.  The theorem's extra content is the
  finite determinant expression for \(\mathcal M_C(A,\tau;G)\) under the
  stated strict twisted gap.

  For the third row, exponentiating the classwise Mertens expansion keeps the
  exponent \(|C|/|G|\) in the classical Mertens layer.  Its non-scalar
  coordinate is the fixed-label Euler value \(F_\varrho(\lambda^{-1})\), and
  \Cref{prop:three-logarithm-product-interface,prop:fixed-label-determinant-perron,%
  thm:class-product-determinant-rigidity,thm:adams-obstruction-class-product}
  prove respectively that this value is distinct from the periodic Artin
  logarithm in general, is recovered from the determinant package, gives the
  product constants, and supplies the obstruction criterion.  Finally, Coles,
  Humbert, O'Hare, and Crismale--Del Vecchio--Griseta--Rossi are comparison
  points only: none is cited as a lemma in the proof chain.  The appendix
  results \Cref{prop:secondary-spectral-edge-noninterference,%
  prop:secondary-package-supporting-classification} show that the conditional
  secondary tail package is downstream from the three headline theorems.  The
  table is therefore a theorem-by-theorem restatement of proved dependencies,
  not an additional unproved input.
\end{proof}
...
The proof of Main Theorem~I is forwarded to the body results
\Cref{lem:twisted-trace-identity,thm:class-function-linearisation,%
cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant,%
thm:primitive-peter-weyl-trace,thm:class-count-trace-readout}; the following
paragraph only assembles those labelled statements.
...
\begin{proof}
  ...
  formula, trace-coordinate formula, and class-count formula proved in
  \Cref{lem:twisted-trace-identity,thm:class-function-linearisation,%
  cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant,%
  thm:primitive-peter-weyl-trace,thm:class-count-trace-readout}.
  These body results are stated with the same hypotheses as the displayed
  theorem and introduce no additional finite-group assumption.
\end{proof}
...
The proof of Main Theorem~II is forwarded to
\Cref{thm:class-mertens-explicit,prop:class-mertens-deviations,%
thm:class-mertens-fourier}; the following paragraph records the assembly from
those body statements.
...
\begin{proof}
  ...
  Hermitian Fourier--Parseval identities are proved in
  ...
  non-trivial primitive channels at the Perron point.
\end{proof}
...
\begin{theorem}[Main Theorem III: Adams-corrected Frobenius-class Euler-product constants]
  ...
  Peter--Weyl boundary is a minimal coordinate skeleton for the
  scalar-normalised product constants:
  \Cref{thm:class-product-minimal-obstruction-skeleton} proves that every
  real linear record determining all branch-compatible constants has rank
...
The product constants are rigid under later secondary-edge refinements:
\Cref{thm:main-spine-secondary-noninterference} proves that any finite
secondary refinement whose compensated tails vanish leaves
\(\mathcal M_C(A,\tau;G)\), \(K_C\), and the recovered values
\(F_\varrho(\lambda^{-1})\) unchanged.  The conditional appendix
\Cref{sec:secondary-term} is an application of that non-interference theorem,
not a fourth main theorem.

The proof of Main Theorem~III is forwarded to
...
cor:class-product-hyperplane-stability,%
  ...
  prop:s3-naive-artin-obstruction,lem:s3-example-perron-certificates,%
  lem:appendix-s3-f-level-certificates}; the
following paragraph assembles exactly those body results and uses the cited
\(S_3\) certificates only as finite witnesses for the Artin-substitution
obstruction, not as an appendix refinement of the hypotheses.
...
\begin{proof}
  ...
  thm:class-product-determinant-rigidity,%
    prop:class-product-comparison,%
    cor:class-product-hyperplane-stability,%
    cor:class-product-artin-obstruction-readout,%
    cor:assembled-adams-frobenius-product-package,%
  thm:adams-obstruction-class-product} prove, respectively, determinant
  recovery at the Perron point, finite product-constant recovery,
  comparison with the classical Frobenius-class product input,
  hyperplane stability, scalar-baseline and Artin-coordinate
  comparison, the determinant-coordinate form, and the obstruction to
  substituting the periodic Artin logarithm for the fixed-label transform.
  The finite \(S_3\) certificates
  \Cref{prop:s3-naive-artin-obstruction,lem:s3-example-perron-certificates,%
  lem:appendix-s3-f-level-certificates} give the stated concrete witness to
  that obstruction.  Thus Main Theorem~III is exactly the conjunction of
  these routed body results: they prove the displayed formula, the
  determinant recovery of its constant, and the classwise
  Artin-substitution obstruction.
\end{proof}

The corollary-level refinements
\Cref{thm:class-product-minimal-obstruction-skeleton,thm:adams-obstruction-budget}
then package the already proved constants into a minimal Peter--Weyl
coordinate skeleton and a Parseval obstruction budget.  They are used below
only as structural readouts and finite-witness interfaces, not as additional
members of the locked Main Theorem~III label set.
...
\paragraph{Proof guide.}
  ...
  \Cref{prop:fixed-label-determinant-perron} and the coordinate corollaries
  following it identify the constant without adding hypotheses to the main theorems.
  The eta-gap classifier \Cref{thm:eta-gap-rigidity} explains when the
  strict-gap hypothesis fails; it is not a separate mechanism for product
  constants.

  The conditional secondary refinement in
  \Cref{sec:secondary-term} is downstream from this proof guide.  The core
  protection against leakage is the main-spine rigidity theorem
  \Cref{thm:main-spine-secondary-noninterference}.  Under the extra
  hypotheses \(R>\lambda^{1/2}\) and \(\Gamma_{\mathrm{sec}}<R\), the appendix
  refinement \Cref{thm:class-mertens-secondary-term,thm:secondary-edge-isolation,%
  cor:class-secondary-term-detects-spectrum} isolates the first finite
  spectral edge below \(\lambda\), identifies its vanishing product tail, and
  shows how the classwise pointwise remainders detect its modulus.  These
  statements are presented as a conditional refinement and do not supply any
  hypothesis or constant used by Main Theorems~I--III.
...
\begin{proof}
  ...
  cor:class-product-stability-scalar-independence}; the rank-zero boundary of
  these scalar records for the non-trivial class profile is isolated in
  \Cref{cor:scalar-record-rank-zero-budget}.
\end{proof}
...
This is the front-matter summary of the body results in
  \Cref{thm:quotient-functoriality,cor:abelian-cyclic-shadow,%
  lem:cyclic-quotient-versus-root-layer,thm:abelian-shadow-defect,%
  lem:cyclic-quotients-one-dimensional-characters}.  The quotient theorem
...
\cite{LindMarcus1995}. Appendix~\ref{app:cyclic-lift-package} records the
...
The appendices collect supporting finite-state computations.
\Cref{app:cyclic-lift-package,app:scalar-boundaries} give the scalar checks
and no-descent safeguards used to keep the main theorems from being misread
as scalar inverse statements.
...
obstruction.  The conditional secondary-edge tail refinement is
proved in \Cref{sec:secondary-term} after the main constants are fixed.
