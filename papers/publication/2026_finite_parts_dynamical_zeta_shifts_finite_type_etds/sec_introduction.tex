...
is not an invariant of arbitrary cyclic quotients of the cocycle.  The final
appendix on secondary spectral edges is retained as a conditional refinement
of the error term only: it assumes additional spectral separation and does
not change \(\Pi_\varrho(\lambda^{-1})\),
...
\begin{proposition}[Proof assembly for the main theorems]
  \label{prop:intro-main-theorem-assembly}
  Each headline theorem stated above is proved by a labelled result in the
  body together with the finite readouts cited below.  The labels
  \Cref{thm:intro-finite-group,thm:intro-adams-mobius} point to the same
  displayed Main Theorem~I statement, and
  \Cref{thm:intro-class-mertens} points to the same displayed Main
  Theorem~II statement; these introduction labels are aliases for convenient
  reference, not additional theorem namespaces.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    \item Main Theorem~I is the assembled determinant-to-primitive statement
    \Cref{prop:main-theorem-i-assembly-map}.  Its proof uses
    \Cref{lem:twisted-trace-identity,thm:class-function-linearisation,%
    cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant,%
    thm:primitive-peter-weyl-trace,thm:class-count-trace-readout}.
    \item Main Theorem~II is the strict-gap class-Mertens theorem
    \Cref{thm:class-mertens-explicit}.  The normalised deviation profile is
    \Cref{prop:class-mertens-deviations}, the Hermitian Fourier inversion
    and Parseval clauses are \Cref{thm:class-mertens-fourier}, and the scalar
    normalisation entering the displayed constant is fixed in
    ...
    \item Main Theorem~III is the Frobenius-class Euler-product theorem
    ...
    finite \(S_3\) witness for this obstruction is the appendix package
...
\begin{proof}
  For (i), the trace identity identifies labelled fixed points with the
  finite Peter--Weyl family of twisted matrices.  The determinant
  linearisation then expresses \(\mathcal L_\phi\) as a finite
  character-table combination of the logarithms
  \(\log\det(I-zA_\varrho)^{-1}\), and Adams--M\"obius inversion gives the
  displayed formula for \(\Pi_\phi(z)\) on the open Perron disc.  The
  determinant-germ, trace-coordinate, and class-count forms cited in (i)
  are the corresponding coefficient readouts.  This is precisely the
  assembly recorded in \Cref{prop:main-theorem-i-assembly-map}, so the
  introduction aliases for Main Theorem~I introduce no independent claim.

  For (ii), \Cref{thm:class-mertens-explicit} proves the stated harmonic
  Mertens asymptotic and constant \(\mathcal M_C(A,\tau;G)\) under
  \(\lambda>1\) and \(\eta<\lambda\).  The deviation and Fourier theorems
  cited in (ii) identify this constant with the non-trivial Perron-point
  primitive vector and recover that vector by Hermitian character
  orthogonality.  The strict gap is exactly the convergence condition that
  permits evaluating \(\Pi_\varrho(\lambda^{-1})\) for
  \(\varrho\ne\mathbf1\).
...
The appendices collect supporting computations and refinements.
...
\Cref{app:s3-model-computations,app:s3-log-certificates} give the finite
  ...
  obstruction.  \Cref{sec:secondary-term} gives a conditional tail refinement
  under additional separation hypotheses; it does not alter the primitive
  Perron values, class Mertens constants, fixed-label Euler values, or product
  constants of Main Theorems~I--III.
