...
\begin{proof}
  ...
  thm:adams-obstruction-budget} prove, respectively, determinant
  ...
  readout, the assembled determinant package, the obstruction to
  substituting the periodic Artin logarithm for the fixed-label transform,
  and the quantitative zero-mean obstruction skeleton.  These results prove
...
\begin{proposition}[Proof locations for the main theorems]
  ...
  The following clause-level dependency map is part of the stable proof
  interface for the three headline theorems.  The labels
  \Cref{thm:intro-finite-group,thm:intro-adams-mobius} are source aliases
  for the same numbered Main Theorem~I statement, and
  \Cref{thm:intro-class-mertens} is a source alias for the same numbered
  Main Theorem~II statement.  No body result is renamed by these aliases;
  the body proof locations are the labels listed below.
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    \item The determinant recovery, Adams--M\"obius formula, determinant-germ
    specialization, trace-coordinate formula, and class-count extraction
    clauses of \Cref{thm:intro-adams-mobius-linearisation} are exactly the
    labelled assembly map \Cref{prop:main-theorem-i-assembly-map}.  Its
    constituent steps are
    \Cref{lem:twisted-trace-identity,thm:class-function-linearisation,%
    cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant,%
    thm:primitive-peter-weyl-trace,thm:class-count-trace-readout}.
    \item The logarithmic Mertens asymptotic, strict-gap Perron-point
    convergence, explicit class constant, and error term in
    \Cref{thm:intro-class-mertens-constants} are those of
    \Cref{thm:class-mertens-explicit}.  The normalised deviation profile is
    \Cref{prop:class-mertens-deviations}, and the Hermitian Fourier
    inversion and Parseval clauses are exactly
    \Cref{thm:class-mertens-fourier}.  The scalar normalisation entering
    the displayed constant is fixed in
    \Cref{lem:finite-part-primitive-closed-form,prop:finite-part-residue-reduced-determinant,cor:finite-part-mertens-asymptotic}.
    \item The Frobenius-class Euler product, exponent, fixed-label
    constant, and distinction from the periodic Artin logarithm in
    \Cref{thm:intro-adams-frobenius-products} are those of
    \Cref{thm:class-euler-product-mertens}, using the fixed-label transform
    of \Cref{thm:fixed-label-euler-transform}.  Determinant evaluation at
    the Perron point and finite product-constant readout are supplied by
    \Cref{prop:fixed-label-determinant-perron,%
    thm:class-product-determinant-rigidity,%
    cor:assembled-adams-frobenius-product-package}.  The comparison with
    the ordinary Artin substitute is \Cref{prop:class-product-comparison},
    while stability, classwise obstruction, and the zero-mean budget
    skeleton are supplied by
    \Cref{cor:class-product-hyperplane-stability,%
    cor:class-product-artin-obstruction-readout,%
    thm:adams-obstruction-class-product,thm:adams-obstruction-budget}.  The
    finite \(S_3\) witness for this obstruction is the proposition-level
    appendix package
    \Cref{prop:quadratic-adams-obstruction,prop:s3-naive-artin-obstruction}
    together with the certificate readout
    \Cref{lem:s3-example-perron-certificates,cor:s3-example-profile}.
  \end{enumerate}
\end{proposition}

\begin{proof}
  For (i), \Cref{prop:main-theorem-i-assembly-map} records the complete
  dependency map for Main Theorem~I.  In detail,
  \Cref{lem:twisted-trace-identity} identifies the labelled
  ...
  Thus the aliases \Cref{thm:intro-finite-group,thm:intro-adams-mobius}
  are not independent claims: they are exactly the four-stage assembly
  packaged in \Cref{prop:main-theorem-i-assembly-map}.
  ...
  \Cref{prop:class-product-comparison} isolates the Adams-corrected
  comparison with the naive Artin substitute.
  ...
  The quantitative zero-mean skeleton is \Cref{thm:adams-obstruction-budget},
  and the finite \(S_3\) witness used for the paper's concrete obstruction
  is the proposition-level package
  \Cref{prop:quadratic-adams-obstruction,prop:s3-naive-artin-obstruction,%
  lem:s3-example-perron-certificates,cor:s3-example-profile}.
...
\begin{corollary}[Introductory quotient-shadow theorem package]
  \label{thm:intro-quotient-shadow}
  Under the twisted spectral gap, the body proves a quotient-shadow theorem
  package for the normalised primitive class profile.  It includes scalar
...
\begin{proof}
  This is the front-matter summary of the body package in
  \Cref{thm:quotient-functoriality,cor:abelian-cyclic-shadow,%
  ...
  readouts with the one-dimensional character span.  Finally,
  ...
  a proved structural theorem package supporting Main Theorems~II--III, not
...
matrix comparison problem.  As already specified in
...
non-abelian class-profile invariant.  The terminology of cyclic, torsion,
...
\Cref{app:s3-model-computations,app:s3-log-certificates} give the finite
  ...
  obstruction.  For the present symbolic-dynamics submission we retain
  \Cref{sec:secondary-term} only as an appendix support module: it is a
  conditional post-constant tail refinement under additional separation
  hypotheses, and it does not alter the primitive Perron values, class
  Mertens constants, fixed-label Euler values, or product constants of Main
  Theorems~I--III.
