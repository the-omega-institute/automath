...
\begin{proof}
  ...
  logarithm in general, is recovered from the determinant data, gives the
  ...
  secondary tail refinement is downstream from the three headline theorems.  The
...
\paragraph{Proof guide.}
  ...
  conditional refinement \Cref{thm:class-mertens-secondary-term,thm:secondary-edge-isolation,%
...
fixed in the proof chain.  \Cref{app:cyclic-lift-package,app:scalar-boundaries}
support the scalar finite-part and cyclic-lift comparisons used around the
finite-part normalisation; they are safeguards against reading the main
theorems as scalar inverse statements.  \Cref{app:s3-model-computations,%
app:s3-log-certificates} supply the finite \(S_3\) matrices and printed
rational inequalities used only as the concrete witness for the
Artin-substitution obstruction in Main Theorem~III.  The conditional
secondary-edge tail refinement is proved in \Cref{sec:secondary-term} after
the main constants are fixed and supplies only downstream error-term
information.
