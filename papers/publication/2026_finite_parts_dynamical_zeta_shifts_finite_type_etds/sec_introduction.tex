...
cocycle.  Secondary spectral-edge refinements are deliberately appendix-only
and conditional on the extra hypotheses \(\lambda>1\), \(\eta<\lambda\),
and \(R>\lambda^{1/2}\):
\Cref{prop:main-chain-secondary-noninterference,prop:secondary-spectral-edge-noninterference}
prove that any such compensated tail with vanishing contribution cannot change
...
chain.  The conditional secondary-tail appendix \Cref{sec:secondary-term} is
similarly supporting material downstream from the three headline theorems and
is invoked only under \(\lambda>1\), \(\eta<\lambda\),
\(R>\lambda^{1/2}\), and, for the sharper edge expansion,
\(\Gamma_{\mathrm{sec}}<R\).
...
described once in the dependency table.  The conditional secondary-edge
appendix \Cref{sec:secondary-term} is retained after the main theorem chain;
its hypotheses are \(\lambda>1\), \(\eta<\lambda\),
\(R>\lambda^{1/2}\), and, where the sharper tail estimate is used,
\(\Gamma_{\mathrm{sec}}<R\).  It contributes only the non-interfering
refinement of
\Cref{prop:secondary-spectral-edge-noninterference,thm:secondary-edge-isolation,cor:class-secondary-term-detects-spectrum}.
