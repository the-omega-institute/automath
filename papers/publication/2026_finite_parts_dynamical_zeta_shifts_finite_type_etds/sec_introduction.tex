...
\begin{proposition}[Proof locations for the main theorems]
  ...
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    \item The determinant recovery, Adams--M\"obius formula, and
    class-count extraction clauses of
    \Cref{thm:intro-adams-mobius-linearisation} are exactly the labelled
    assembly map \Cref{prop:main-theorem-i-assembly-map}.  Its constituent
    steps are
    \Cref{lem:twisted-trace-identity,thm:class-function-linearisation,%
    cor:class-function-adams-mobius,cor:primitive-peter-weyl-determinant,%
    thm:primitive-peter-weyl-trace,thm:class-count-trace-readout}.
    \item The logarithmic Mertens asymptotic and displayed constant in
    \Cref{thm:intro-class-mertens-constants} are those of
    \Cref{thm:class-mertens-explicit}.  The normalised deviation profile,
    determinant boundary evaluation, Hermitian Fourier inversion, and
    Parseval clauses are supplied by
    \Cref{prop:class-mertens-deviations,thm:class-mertens-fourier}.
    ...
    \Cref{thm:class-euler-product-mertens}.  Determinant evaluation and
    finite product-constant readout are supplied by
    \Cref{prop:fixed-label-determinant-perron,%
    thm:class-product-determinant-rigidity,%
    cor:class-product-hyperplane-stability,%
    cor:class-product-artin-obstruction-readout,%
    cor:assembled-adams-frobenius-product-package}, and the classwise
    Artin-substitution obstruction and its zero-mean budget skeleton are
    supplied by
    \Cref{thm:adams-obstruction-class-product,thm:adams-obstruction-budget}.
  \end{enumerate}
\end{proposition}
