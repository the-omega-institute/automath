...
\(\Lambda_\perp\) and \(\Lambda_\partial\), reduced finite-part branches,
and, when one wants it, the finite semisimplicity certificate for the
twisted boundary blocks.
...
The following objects form the preliminary finite-state support data used
  ...
  checks, reduced determinant finite-part branch tags, and scalar
  normalization.  Consequently the preliminary data determine the formal
...
\begin{proof}
  ...
  they require the peripheral lists with algebraic multiplicities, reduced
  finite-part branch tags, and scalar normalization recorded in
  \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\).  With those
...
representation labels.  The symbol \(\pi_{n,\varrho}\) below is a primitive
coefficient, not a representation named \(\pi\).\\
...
\(\psi^m\phi\), \(a_{\varrho,\pi}^{(m)}\) & Adams operation
...
\(\pi_{n,\varrho}(A,\tau)\) & Coefficient of \(\Pi_{\chi_\varrho}\):
...
\((|C|/|G|)\sum_\varrho\chi_\varrho^\ast(C)\pi_{n,\varrho}\).\\
\bottomrule
\end{tabular}
\end{center}
...
\(\{P_\pi\}\), \(\lambda\), the peripheral factorizations, \(\Gamma_\pi\),
\(m_{\pi,\zeta}\), \(s_\pi\), \(\Log_0\), and \(n_\partial\).  It is larger
than the determinant polynomials alone.\\
\(n_\partial\) & Positive scalar/product logarithm normalization. & This
fixes the real logarithm branch used when the product constants are
exponentiated.\\
