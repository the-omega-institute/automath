...
\]
This is notation only for optional split-stage refinements of secondary
orbit-counting bounds.  The main theorem chain in this paper uses only the
Perron gap \(\Lambda(A)<\lambda\), the finite twisted gap \(\eta<\lambda\),
and the finite-tail non-interference principle of
\Cref{prop:main-chain-secondary-noninterference}; no conclusion of Main
Theorems~I--III assumes \Cref{def:kernel-rh}.
\end{definition}
