...
\begin{lemma}[Primitive-orbit asymptotic from M\"obius inversion]
...
\begin{proof}
  ...
  \]
  The divisor \(m=1\) contributes
  \(\lambda^n/n+O(\Lambda(A)^n/n)\). For the remaining divisors,
  \(m>1\) implies \(n/m\le n/2\), and the spectral estimate
  \(\Tr(A^k)=O(\lambda^k)\) gives
  \[
    \sum_{\substack{m\mid n\\ m>1}}\abs{\Tr(A^{n/m})}
    \le
    C\sum_{1\le k\le n/2}\lambda^k
    =
    O(\lambda^{n/2}).
  \]
  Dividing by \(n\) gives the displayed primitive-orbit asymptotic.
