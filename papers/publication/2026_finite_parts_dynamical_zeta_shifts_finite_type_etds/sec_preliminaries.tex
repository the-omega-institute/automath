...
\paragraph{Future square-root notation.}
For possible refinements outside the submitted theorem chain, one may say that
\(A\) satisfies the \emph{square-root bound} if
\[
  \Lambda(A)\le \lambda^{1/2}.
\]
This paragraph is not a hypothesis, theorem environment, or hidden secondary
term assertion.  Main Theorems~I--III use only the Perron gap
\(\Lambda(A)<\lambda\), the finite twisted gap \(\eta<\lambda\), and the
finite-tail invariance principle of
