...
notation come afterwards as the scalar normalisation and canonical comparison
interface.
...
as \(0\).  The Perron-boundary data used below are broader than this
strict exclusion of non-trivial boundary channels:
\Cref{def:peripheral-peter-weyl-data,lem:finite-cover-peripheral-expansion}
record the peripheral sets \(\Gamma_\pi\), multiplicities
\(m_{\pi,\zeta}\), logarithmic ranks \(s_\pi\), the exceptional lists
\(\Lambda_\perp\) and \(\Lambda_\partial\), the Abel branch families
\(\Log_{\pi,\zeta}\) and \(\Log_{\partial,\pi}\), and, when one wants it,
the finite semisimplicity certificate for the twisted boundary blocks.
These data form the renormalized tuple
\(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\) in
\eqref{eq:perron-boundary-data-tuple}.  The clean inequality
\(\eta<\lambda\) is retained as the diagnostic strict-gap specialization of
\Cref{cor:strict-gap-specialization}, where all non-trivial boundary phases
are absent.  For a class function \(\phi\),
...
written with the star notation
\[
  \chi_\varrho^\ast(C)
  :=\overline{\chi_\varrho(C)}
  =\chi_\varrho(C^{-1}),
\]
while inverse recovery uses \(\chi_\sigma(C)\) without conjugation.  The
scalar normalisations are
...
\(C(A)\).  Non-trivial channels on the Perron boundary are evaluated in the
main theorem chain by the reduced finite-part branches and semisimplicity
certificate in \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\).  In the
strict-gap specialization, those reduced branches collapse to ordinary
regular values at \(z=\lambda^{-1}\).
...
\(\chi_\pi(C)\). The two symbols coincide exactly for real-valued
characters.
Equivalently, on class functions we use the Hermitian inner product
\[
  \langle f,h\rangle_{\mathrm{cl}}
  :=
  \sum_{C\subset G}\frac{|C|}{|G|}f(C)\overline{h(C)}.
\]
The functions \(C\mapsto\chi_\varrho^\ast(C)\) are orthonormal for this
inner product, and recovering the coefficient of a forward expansion
\(F(C)=\sum_\varrho b_\varrho\chi_\varrho^\ast(C)\) means
\[
  b_\pi
  =
  \langle F,\chi_\pi^\ast\rangle_{\mathrm{cl}}
  =
  \sum_{C\subset G}\frac{|C|}{|G|}F(C)\chi_\pi(C).
\]
Thus displayed products such as
\(\chi_\varrho^\ast(C)\chi_\pi(C)\) are the Hermitian orthogonality
relation written after expanding the complex conjugate in the second
slot; they are not a separate bilinear pairing.
...
\begin{remark}[Preliminary finite-state determinant support data]
  \label{prop:finite-state-data-collection}
  \label{prop:finite-state-data-package}
The following objects form the preliminary finite-state support data used
  for the open-disc determinant identities and for the strict-gap
  specialization:
  \[
  ...
  \eqref{eq:prelim-adams-character-coefficients}. These objects are not,
  by themselves, the boundary datum of the main theorem.  The primary
  Perron-boundary datum is the renormalized tuple
  \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\) of
  \eqref{eq:perron-boundary-data-tuple} and
  \eqref{eq:renormalized-finite-data-tuple}, which additionally records
  \(\Gamma_\pi\), \(m_{\pi,\zeta}\), \(s_\pi\),
  \(\Lambda_\perp\), \(\Lambda_\partial\), optional finite semisimplicity
  checks, and reduced determinant finite-part branches.  Scalar/product
  logarithms are the ordinary real logarithm only after positivity has been
  proved.  Consequently the preliminary data determine the formal
  open-disc traces and primitive coefficients, while the full renormalized
  datum determines the boundary quantities
  \(\delta_C\), \(M_C^{\mathrm{ren}}\),
  \(F_\pi^{\mathrm{ren}}\), and \(K_C^{\mathrm{ren}}\) as proved in
  \Cref{prop:finite-data-main-chain-closure}.  The old
  \(\eta<\lambda\) record is only the strict-gap specialization supplied by
...
\begin{proof}
  ...
  polynomials.  Boundary evaluations at \(z=\lambda^{-1}\), however, are
  not determined by determinant polynomials alone as branch-free data:
  they require the peripheral lists with algebraic multiplicities and the
  reduced Abel/interior branch convention recorded in
  \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\).  Product constants
  then use the real logarithm on \(\mathbb R_{>0}\) after positivity.  With those
  additions, \Cref{prop:finite-data-main-chain-closure} determines
  the renormalized boundary constants.  Imposing \(\eta<\lambda\) simply
  collapses this datum to the ordinary strict-gap endpoint chart.
\end{proof}
