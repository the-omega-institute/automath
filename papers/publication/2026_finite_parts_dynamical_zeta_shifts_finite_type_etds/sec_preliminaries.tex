...
spine. The reduced determinant, finite part, and cyclic-lift notation
...
=\chi_\varrho(C^{-1})\), while inverse readout uses
...
\cite{ArtinMazur1965PeriodicPoints,BowenLanford1970Zeta,%
LindMarcus1995,ParryPollicott1990Zeta}.  The proof is included to fix
normalisations for the later finite-group Euler products.
...
here in the precise form used below; compare the standard treatment of
mixing shifts of finite type in \cite{LindMarcus1995,ParryPollicott1990Zeta}.
The bound remains valid even when \(\lambda=1\).
...
classical orbit-counting setting.
...
\begin{proposition}[Finite-state data package]
  \label{prop:finite-state-data-package}
  The following objects form the finite-state input package used in
  Main Theorems~I--III:
  \[
  ...
  and class-Mertens constant in the main theorem chain is a function of
  this finite package, together with the additional Perron-boundary
...
\begin{definition}[Fixed-matrix cyclic-lift records]
  ...
  \]
  These are records of this one matrix \(A\). They are not invariants of
  a varying family and are not class-profile data for a finite-group
  cocycle unless additional descent information is supplied. In
