...
\begin{equation}\label{eq:primitive-peter-weyl-series}
  ...
  =\sum_{n\ge 1}c_{n,\varrho}(A,\tau)z^n,
  \qquad
  c_{n,\varrho}(A,\tau)
...
\begin{corollary}[Irreducible-channel determinant
  formula]
  ...
  Let \(e_G=\exp(G)\).  The open-disc primitive coefficients below are
  determined by the finite table
  \[
    (G,\mathrm{Pow}_G,\{P_\pi(z)=\det(I-zA_\pi)\}_\pi),
    \qquad
    \mathrm{Pow}_G:=\{C\mapsto C^a:0\le a<e_G\}.
  \]
  At Perron-boundary points the determinant polynomials alone are not the
  data.  The branch-sensitive renormalized boundary tuple is a finite recipe:
  \begin{equation}\label{eq:perron-boundary-data-tuple}
    \begin{aligned}
    ...
    \Bigl(&G,\mathrm{Pow}_G,\{P_\pi(z)\}_{\pi\in\Irr(G)},\lambda,\\
      &\{\Gamma_\pi\}_\pi,\{m_{\pi,\zeta}\}_{\pi,\zeta},
        \{\Log_{\pi,\zeta}\}_{\pi,\zeta},
        \{\Log_{\partial,\pi}\}_{\pi}\Bigr).
    \end{aligned}
  \end{equation}
  where \(\mathrm{Pow}_G\) records the class-power maps modulo \(e_G\) used in
  all Adams coefficients.  For each \(\pi\), the factorization
  \[
    P_\pi(t/\lambda)=
  ...
  \]
  defines the finite factor \(Q_{\partial,\pi}\).  The branches
  \(\Log_{\pi,\zeta}(1-t\zeta)\) are the unique continuous branches on
  \(0\le t<1\) with value \(0\) at \(t=0\), and
  \(\Log_{\partial,\pi}Q_{\partial,\pi}(1)\) is obtained by continuing
  \(\Log Q_{\partial,\pi}(t)\), normalized at \(0\), along that segment.  For
  \(k\ge2\) the determinant layer is the ordinary power-series branch
  \(\Log_0P_\pi(\lambda^{-k})^{-1}\).  Product constants use the real logarithm on
  \(\mathbb R_{>0}\) after the positivity lemma, not an additional tuple
  entry.  The reduced values \(B_{k,\pi}^{\mathrm{ren}}\) are then computed
  from this tuple by \eqref{eq:perron-boundary-renormalized-b-data}; they are
  not independent boundary entries.  The auxiliary quantities
  \(\Lambda_\perp\), \(\Lambda_\partial\), and the optional semisimplicity
  certificate \(\mathfrak s_\partial\) are finite verification data for
  estimates and Jordan structure; they are not substitutes for the boundary
  logarithm recipe of \(\mathfrak D_{\partial}^{\mathrm{ren}}\).  The strict-gap
  record with \(\eta<\lambda\) is the specialization recovered in
...
Theorem~A.  The next theorem is the coefficient recovery from the same
...
\begin{theorem}[Primitive Peter--Weyl trace formula]
  ...
  Theorem~A.  It is not a separate structural theorem about the extension, and
  it is purely coefficient-level: no twisted spectral-gap hypothesis or
  Perron-boundary branch enters the statement.
...
\begin{corollary}[Finite trace formula for primitive coefficients]
  ...
  Peter--Weyl coefficient \(c_{n,\varrho}(A,\tau)\). Equivalently, for
  every \(\varrho\in\Irr(G)\), the series \(\Pi_\varrho(z)
  =\sum_{n\ge1}c_{n,\varrho}(A,\tau)z^n\) is determined by the finite
...
\begin{proof}
  ...
  \(c_{n,\varrho}(A,\tau)\), and therefore determines the whole series
...
\noindent
This is the coefficient-level form of Main Theorem~A, not a preliminary
...
\subsection{Bridge to classes and Perron-boundary constants}
...
translates the primitive channels of Main Theorem~A into class counts,
...
\begin{proposition}[Conjugacy-class primitive decomposition]
  ...
  \begin{equation}\label{eq:class-primitive-coefficients}
    ...
    \frac{|C|}{|G|}
      \sum_{\varrho\in\Irr(G)}
        \chi_\varrho^\ast(C)\,c_{n,\varrho}(A,\tau).
  \end{equation}
\end{proposition}
...
\begin{theorem}[Class-count trace recovery]
  ...
  This is the final finite-character readout step of Main Theorem~A.  For
  ...
  any primitive class count \(p_{n,C}\).  The factor \(1/n\) is the
  coefficient-extraction factor from the Adams--M\"obius primitive channel
  formula; the earlier twisted trace identity is a basepointed fixed-point
  identity and carries no primitive-orbit quotient.
\end{theorem}
...
\noindent
This completes Main Theorem~A in the sense used in the introduction:
...
recovery that feeds both the Perron-boundary class constants and their
strict-gap specialization.

The next strict-gap additive statements assume \(\lambda>1\) and impose the
gap hypothesis
\[
...
The class Mertens theorem below is the strict-gap additive specialization of
the Perron-boundary package. The leading density term is included
...
\begin{proof}
  ...
  \(c_{n,\mathbf 1}(A,\tau)=p_n(A)\), the displayed identity follows.
\end{proof}
...
\begin{lemma}[Absolute convergence at the Perron point]
  ...
  \[
    \Pi_\varrho(\lambda^{-1})
    =\sum_{n\ge 1}c_{n,\varrho}(A,\tau)\lambda^{-n}
  \]
