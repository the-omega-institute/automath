...
\begin{corollary}[Main Theorem I(c): irreducible-channel determinant
  ...
  coefficients, \(\mathfrak s_\partial\) is an optional finite semisimplicity
  certificate, \(\mathfrak b_{\partial}^{\mathrm{ren}}\) records the reduced
  determinant finite-part branch at \(k=1\) and the ordinary interior branches
  at \(k\ge2\), and \(\mathfrak n_\partial\) records the scalar/product
  logarithm normalization.  The strict-gap record with \(\eta<\lambda\) is the
  specialization recovered in \Cref{cor:strict-gap-specialization}.
