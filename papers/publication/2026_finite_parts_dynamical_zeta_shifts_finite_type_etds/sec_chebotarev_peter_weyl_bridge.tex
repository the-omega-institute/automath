...
\begin{corollary}[Main Theorem I(c): irreducible-channel determinant
  ...
  \begin{equation}\label{eq:perron-boundary-data-tuple}
    ...
    \Bigl(&G,\mathrm{Pow}_m,\{P_\pi(z)\}_{\pi\in\Irr(G)},\lambda,
      ...
      &\{\mathcal B_{1,\pi}^{\mathrm{ren}}\}_\pi,\Log_0,n_\partial\Bigr).
    \end{aligned}
  \end{equation}
  where \(\mathrm{Pow}_m\) records the class-power maps used in the Adams
  coefficients, \(\{\mathcal B_{1,\pi}^{\mathrm{ren}}\}_\pi\) records the
  reduced Abel determinant finite parts at \(k=1\), \(\Log_0\) is the
  determinant-logarithm germ normalized at \(0\) and continued along the stated
  Abel path or evaluated at interior points, and \(n_\partial\) records the
