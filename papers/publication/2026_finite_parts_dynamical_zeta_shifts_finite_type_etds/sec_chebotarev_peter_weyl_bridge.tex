...
\begin{corollary}[Irreducible-channel determinant
  ...
  The open-disc primitive coefficients below are determined by
  \[
    (G,\mathrm{Pow}_m,\{P_\pi(z)=\det(I-zA_\pi)\}_\pi).
  \]
  ...
  \begin{equation}\label{eq:perron-boundary-data-tuple}
    ...
    \Bigl(&G,\mathrm{Pow}_m,\{P_\pi(z)\}_{\pi\in\Irr(G)},\lambda,\\
      &\{P_\pi(t/\lambda)=
        ...
        Q_{\partial,\pi}(t)\}_\pi,\\
      &\{\Gamma_\pi\}_\pi,\{m_{\pi,\zeta}\}_{\pi,\zeta},\{s_\pi\}_\pi,
        \Log_0,n_\partial\Bigr).
    \end{aligned}
  \end{equation}
  where \(\mathrm{Pow}_m\) records the class-power maps used in the Adams
  coefficients, the displayed factorisations identify the peripheral factors
  and the finite \(Q_{\partial,\pi}\), \(\Log_0\) is the determinant-logarithm
  germ normalized at \(0\) and continued by the stated Abel/interior rule, and
  \(n_\partial\) records the scalar/product logarithm normalization.  The
  reduced values \(B_{k,\pi}^{\mathrm{ren}}\) are then computed from this
  tuple by \eqref{eq:perron-boundary-renormalized-b-data}; they are not
  independent boundary entries.  The auxiliary quantities
