...
\begin{proposition}[Strict-gap downstream finite-coordinate readout]
...
\begin{proposition}[Downstream status of the product-coordinate chart]
  ...
  scalar product normalisation \(\alpha_A=\gamma+\log C(A)\) and the
  strict-gap scalar-triple deletion convention of
  \Cref{def:perron-boundary-logarithm-convention}.  Then every assertion in
  \Cref{thm:class-product-determinant-rigidity} factors through the single
  strict-gap product formula
  \[
    C\longmapsto
    H_K(C):=-\frac{|G|}{|C|}\log K_C-\alpha_A
    =
  ...
  \]
  and through finite Hermitian Fourier inversion on the zero-mean class
  functions of \(G\).  In particular the proposition introduces no additional
  orbit-counting asymptotic, no new product constant, and no invariant under
  folds, gauges, covers, or changes of logarithmic branch.  Its exact content
  is the finite linear algebra of the already constructed Adams-corrected
  determinant vector
  \((F_\varrho(\lambda^{-1}))_{\varrho\ne\mathbf1}\).
\end{proposition}

\begin{proof}
  \Cref{thm:class-euler-product-mertens} gives the displayed formula for
  \(H_K\) after the positive product branch and \(\alpha_A\) are fixed.  The
  functions \(\chi_\varrho\), \(\varrho\ne\mathbf1\), form an orthonormal basis
  of the zero-mean class-function subspace with respect to
  \[
    \langle f,h\rangle_G
    =
    \sum_{C\subset G}\frac{|C|}{|G|}f(C)\overline{h(C)} .
  \]
  Therefore Fourier inversion recovers the non-trivial coefficients of
  \(H_K\), Parseval gives the displayed norm identities, and exponentiating the
  real logarithmic chart with the fixed scalar \(\alpha_A\) gives the positive
  product vector.  The determinant equalities used for the coefficients are
  exactly the boundary readout
  \(\mathfrak D_\varrho=F_\varrho(\lambda^{-1})\) from
  \Cref{prop:fixed-label-determinant-perron}; they do not change the product
  asymptotic proved in \Cref{thm:class-euler-product-mertens}.  Since the proof
  uses only this fixed branch chart and finite character orthogonality, it
  supplies none of the broader invariance or observation-system claims
  excluded in the statement.
\end{proof}

\begin{corollary}[Branch-compatible hyperplane readout and fixed-coordinate stability]
  ...
  identities of \Cref{thm:class-product-determinant-rigidity} describe all
  positive class-product vectors satisfying the forced scalar product
  constraint in this fixed logarithmic chart.  Namely let
  \[
    \mathscr K_{\alpha_A}:=
    \left\{(L_C)_C:\ L_C>0,
