...
\begin{proposition}[Downstream finite-coordinate readout of the main product constants]
  \label{thm:class-product-determinant-rigidity}
  Assume the hypotheses of \Cref{thm:class-mertens-explicit}.  The input
  data for this coordinate-readout statement are exactly the
  branch-sensitive tuple \(\mathfrak D_{\partial}^{\mathrm{ren}}(A,\tau;G)\) of
  \eqref{eq:perron-boundary-data-tuple}, together with the character table of
  \(G\) and the resulting finite Adams coefficient tensors
  \(a_{\varrho,\pi}^{(m)}\).  Thus the scalar Perron normalisation \(C(A)\), the
  class-power maps, the determinant polynomials, the branch tags, and the gap
  certificate are all part of the data.  For every
  ...
  \begin{equation}\label{eq:class-product-rigidity-determinant-coordinate}
    ...
    \sum_{r,m\ge1}\frac{\mu(m)}{rm}
      \sum_{\substack{\pi\in\Irr(G)\\(r,m,\pi)\neq(1,1,\mathbf1)}}
        ...
        \mathcal L_{rm,\pi}.
  \end{equation}
  The logarithms are interpreted by the Perron-boundary convention of
  ...
  replacing the Main Theorem~III coordinate vector.  If
  ...
  \end{equation}
  Consequently a replacement vector can give all Main Theorem~III constants
...
\begin{proposition}[Downstream status of the product-coordinate chart]
  ...
  Perron-boundary branch convention of
