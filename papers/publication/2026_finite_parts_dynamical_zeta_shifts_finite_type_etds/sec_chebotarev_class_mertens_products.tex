...
\(\tau(\gamma)\), and the Mertens weight is normalised by the Perron root as
  ...
  fixes the displayed orientation of the internally computed constant.
...
\begin{proposition}[Classical layer versus Adams-corrected constant]
  ...
  the fixed-label vector in \textup{(i)}.  Replacing that vector by a proposed
  vector \((B_\varrho)_{\varrho\neq\mathbf1}\) changes the scalar-normalised
  constants by the class function
  ...
  \end{equation}
  and this error is identically zero if and only if
...
\begin{proof}
  ...
  that this class function vanishes identically if and only if all of its
  non-trivial Fourier coefficients vanish, namely
  \(B_\varrho=F_\varrho(\lambda^{-1})\) for every non-trivial
  \(\varrho\).  This proves both the equivalence of the three records and the
  claimed calibration: the classical product layer fixes the exponent and
  positive branch, while the branch-compatible Adams--M\"obius determinant
  vector is an additional finite-state coordinate record.
\end{proof}
