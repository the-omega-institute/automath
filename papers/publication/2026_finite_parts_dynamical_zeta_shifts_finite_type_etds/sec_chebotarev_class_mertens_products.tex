...
\(F_\varrho(\lambda^{-1})\).  The compact proof spine is the following finite
map:
\[
\begin{gathered}
\text{classical product layer}
\xrightarrow{\ \Cref{lem:class-euler-product-tail-bound}\ }
\text{primitive product tail}\xrightarrow{\ \Cref{lem:class-product-constant-equivalence}\ }
\text{fixed-label coordinates}\xrightarrow{\ \Cref{prop:fixed-label-determinant-perron}\ }
\text{twisted determinants},\\
\text{with the reversible closed-coordinate form recorded in }
\Cref{prop:main-iii-fixed-label-proof-spine}
\text{ and the literature calibration in }
\Cref{prop:class-product-comparison}.
\end{gathered}
\]
