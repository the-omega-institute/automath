...
\(F_\varrho(\lambda^{-1})\).  The compact proof spine is the finite map
from the classical product layer to the primitive product tail, then to the
fixed-label coordinates, and finally to twisted determinants; the arrows are
made precise by
\Cref{lem:class-euler-product-tail-bound,lem:class-product-constant-equivalence,prop:fixed-label-determinant-perron,prop:main-iii-fixed-label-proof-spine,prop:class-product-comparison}:
\[
\begin{array}{c}
\text{classical product layer}\longrightarrow\text{primitive product tail}\\[2pt]
...
same theorem chain, not an additional definition of the class product.
