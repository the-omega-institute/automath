...
\begin{proposition}[Internal product reduction from Main Theorem II]
  ...
  verifiable internal spine: Main Theorem~II controls the \(r=1\) logarithm,
  the product-tail lemma controls all \(r\ge2\) repetitions, and the
  fixed-label transform supplies the determinant-ready coordinates for the
  same constant.  The classical Frobenius-class product results cited around
  \Cref{thm:class-euler-product-mertens} calibrate the inherited existence and
  exponent; they are not used to identify the displayed value of \(K_C\).
\end{proposition}
...
\begin{proposition}[Internal certificate for the displayed product constant]
  ...
  \]
  In particular the displayed Main Theorem~III formula for \(K_C\) is an
  internal consequence of Main Theorem~II plus the product tail.  Any external
  Frobenius-class Mertens product theorem supplies only a literature
  calibration of the already separated product exponent and the existence
  layer.
\end{proposition}
...
\begin{proof}
  ...
  classical exponent/existence layer rather than the internal constant formula.
\end{proof}
...
\(F_\varrho(\lambda^{-1})\).  The compact proof spine is internal: the
...
existence layer, while the displayed value of \(K_C\) is proved by the internal
reduction and determinant evaluation below.
...
\begin{proposition}[Dependency map for Main Theorem~III]
  ...
  Under the hypotheses of \Cref{thm:class-euler-product-mertens}, the proof of
  the product theorem factors through the following finite list of interfaces,
  and no other analytic or certification input is used:
  \begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
    ...
    \item \Cref{prop:main-iii-fixed-label-proof-spine,thm:class-product-minimal-obstruction-skeleton,prop:class-product-comparison}
      ...
      the inherited classical product theorem and the new Adams-corrected
...
used in the sequel.
The internal derivation above proves the displayed \(K_C\)-formula from Main
