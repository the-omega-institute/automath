...
Before stating the product theorem, we separate the two inputs.  The exponent
\(|C|/|G|\) and the existence of a Frobenius-class product asymptotic are the
classical Mertens layer, used here through the additive estimate of Main
Theorem~II and the elementary product-tail reduction above.  The new
finite-state contribution begins with the constant: after the scalar
normalisation \(C(A)\) is separated, the class vector \((K_C)_C\) is recovered
from the fixed-label coordinates \(F_\varrho(\lambda^{-1})\), not from the
periodic Artin logarithms \(\mathcal L_\varrho(\lambda^{-1})\).
