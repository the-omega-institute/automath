...
| Paper or route | Venue | Status | Next step |
|---|---|---|---|
| `2026_finite_parts_dynamical_zeta_shifts_finite_type_etds` | retarget dynamical systems / symbolic dynamics venue | ETDS rejected 2026-05-26, Submission ID `ETDS-2026-0139` (Ian Melbourne: needs a true advance). 2026-06-17: implemented the Adams-corrected Frobenius-class Euler-product Mertens package as Main Theorem III and compressed the theorem spine; reopened with a fresh round budget (post-rejection retarget reopen) to flow A→B→C automatically. | Material theorem now in place; let the pipeline re-review. Do not restart from zero Stage A content. |
...
| `2026_self_dual_synchronisation_kernel_completed_determinant_cyclotomic_twists` | Experimental Mathematics (was IMRN) | IMRN rejected (novelty: single 10-state case study). 2026-06-17: retargeted to Experimental Mathematics — honest case-study framing (no generic-family claim), Prop 3.2 spectral-radius fix via Perron-Frobenius + ‖B‖∞, displayed self-duality/determinant/smoothness certificates with traceable runner keys, bibliography added; back in Stage-B Oracle review. | Run certificates/verify_certificates.py under Sage for the executable certificate check. |
