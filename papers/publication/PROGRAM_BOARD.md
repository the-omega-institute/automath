...
| `2026_upper_fibers_witness_covers_fibonacci_apparition_fq` | Fibonacci Quarterly | A-BLOCKED (FQ deepening audit real block score=6) | RJ→FQ深改候选；current automatic deepening insufficient; needs new substantive result / human route decision before rerun; not in manual submission queue |
...
| `2026_coefficient_sup_radial_homotopy_monomial_forms_jdde` | JDDE | A-BLOCKED (Stage A real block: central homotopy theorem not self-contained) | triaged 2026-05-13: canonical route for centered homotopy / cubical Stokes coefficient-bound manuscript; structural fallback was empty, but Codex math audit found real blockers in `stage_a_audit.json`; integrate self-contained homotopy identity/coefficient estimate/sharpness proof before rerun |
