...
\[
\operatorname{Val}(u)=F_{n+1}^2,
\qquad
\operatorname{Val}(u')=(F_{n+1}+1)F_{n+1},
\]
where $u:=c\otimes d$ and $u':=c'\otimes d$. Hence
\[
\operatorname{Val}(u')-\operatorname{Val}(u)=F_{n+1}.
\]
Suppose, for contradiction, that the two output Zeckendorf words agree at every position $k\ge n$. Their value difference would then come entirely from positions $1,\ldots,n-1$. By Lemma~\ref{lem:finite-window-max}, an admissible word supported on those positions has value at most $F_{n+1}-1$. Therefore two such lower portions cannot differ in value by $F_{n+1}$, contradicting the displayed identity. Thus the stable products differ at some output position $k\ge n$.
