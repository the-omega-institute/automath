...
\begin{proof}
The statement is just the finite-set chain rule, once one notes that the reference measure factors along the fibers of \(\pi\). Indeed, by construction \(\mu_n^{(1)}=\pi_\ast\mu_n^{(2)}\). Since \(\pi\) is a surjective homomorphism of finite groups, every fiber has cardinality \(|N|\), and hence
\[
...
\]
If \(\mu_n^{(1)}(c_1)=0\), then \(\mu_n^{(2)}\) vanishes on the whole fiber \(\pi^{-1}(c_1)\), because \(\mu_n^{(1)}(c_1)\) is the sum of the nonnegative terms \(\mu_n^{(2)}(c_2)\) over that fiber. Such fibers make no contribution.
...
\begin{proof}
The key point is that fiberwise averaging is the orthogonal projection onto the subspace of functions that are constant on the fibers of \(\pi\). For \(c_1\in G_1\), the pushforward relation reads
\[
r_n^{(1)}(c_1)
=
...
\end{aligned}
\]
Thus \(r_n^{(1)}\circ\pi\) is the orthogonal projection of \(r_n^{(2)}\) onto \(\mathcal H_\pi\). The Pythagorean identity in \(L^2(\mathrm{unif}_{G_2})\) gives
\[
\|r_n^{(2)}-1\|_{L^2(\mathrm{unif}_{G_2})}^2
=
...
\begin{remark}
The formal ingredients are classical. What is special in the present setting is that the coarse counts arise by exact pushforward from the primitive layer, so the discarded term has a canonical arithmetic interpretation: it is precisely the character energy outside \(N^\perp\).
\end{remark}
