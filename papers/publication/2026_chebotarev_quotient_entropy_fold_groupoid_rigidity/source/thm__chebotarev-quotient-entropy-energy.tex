...
\begin{proof}
By construction, \(\mu_n^{(1)}=\pi_\ast\mu_n^{(2)}\). Since \(\pi\) is a surjective homomorphism of finite groups, every fiber has cardinality \(|N|\), and therefore
\[
...
\]
If \(\mu_n^{(1)}(c_1)=0\), then \(\mu_n^{(2)}\) vanishes on the whole fiber \(\pi^{-1}(c_1)\), because \(\mu_n^{(1)}(c_1)\) is the sum of the nonnegative masses \(\mu_n^{(2)}(c_2)\) over that fiber. Such fibers contribute nothing to the claimed identity.
...
\]
Multiply by \(\mu_n^{(2)}(c_2)\), sum over \(c_2\in G_2\), and regroup by fibers of \(\pi\). This is exactly the asserted identity.
\end{proof}
...
\begin{proof}
For \(c_1\in G_1\), the pushforward relation reads
\[
r_n^{(1)}(c_1)
=
\frac{1}{|N|}\sum_{c_2\in\pi^{-1}(c_1)}r_n^{(2)}(c_2).
\]
Let \(\mathcal H_\pi\subset L^2(\mathrm{unif}_{G_2})\) be the subspace of functions that are constant on the fibers of \(\pi\). The displayed identity shows that \(r_n^{(1)}\circ\pi\in\mathcal H_\pi\). If \(h\in\mathcal H_\pi\) and \(h(c_2)=h_0(\pi(c_2))\), then
\[
\begin{aligned}
...
\end{aligned}
\]
Thus \(r_n^{(1)}\circ\pi\) is the orthogonal projection of \(r_n^{(2)}\) onto \(\mathcal H_\pi\). The Pythagorean theorem in \(L^2(\mathrm{unif}_{G_2})\) yields
\[
\|r_n^{(2)}-1\|_{L^2(\mathrm{unif}_{G_2})}^2
=
...
\]
Since \(\chi\mapsto\overline\chi\) permutes \(\widehat G_2\), Parseval gives
\[
\|r_n^{(2)}-1\|_{L^2(\mathrm{unif}_{G_2})}^2
=
...
\bigl|B_n^{(2)}(\chi;u)\bigr|^2.
\]
Subtracting these two identities proves the hidden-energy formula.
\end{proof}

\begin{remark}
The identities used above are classical. What is specific here is that the coarse counts are obtained by exact pushforward from the primitive layer, so the discarded term has a canonical arithmetic meaning: it is precisely the character energy outside \(N^\perp\).
\end{remark}
