\begin{theorem}[Pole order of a unique dominant Artin factor]\label{thm:kernel-artin-dominant-channel-pole-order}
...
\]
has a simple zero at \(z=\mu_\star^{-1}\). Hence \(L_{K,\rho_\star}(z,u)\) has a simple pole there, and raising it to the power \(\dim\rho_\star\) gives a pole of order \(\dim\rho_\star\). By assumptions (2) and (3), no other nontrivial factor \(L_{K,\rho}(z,u)\) has a zero of its denominator at \(z=\mu_\star^{-1}\): in the channel \(\rho_\star\), every spectral value distinct from \(\mu_\star\) has modulus \(<\eta(u)\), and in every other nontrivial channel the value \(\mu_\star\) does not occur at all. The remaining factors are therefore analytic and nonzero at \(z=\mu_\star^{-1}\), so they do not affect the pole order.
\end{proof}
...
\]
The same theorem gives a constant \(c_0(A)>0\) such that
\[
