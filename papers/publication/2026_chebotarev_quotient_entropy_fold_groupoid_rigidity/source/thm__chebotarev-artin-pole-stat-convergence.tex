...
\]
has a simple zero at \(z=\mu_\star^{-1}\). Equivalently,
\[
\det(I-\mu_\star^{-1}M_{K,\rho_\star}(u))
=
\prod_{\nu\in\mathrm{spec}(M_{K,\rho_\star}(u))}\left(1-\frac{\nu}{\mu_\star}\right),
\]
with algebraic multiplicities. For \(\rho=\rho_\star\), exactly one factor vanishes, and it vanishes simply. Hence \(L_{K,\rho_\star}(z,u)\) has a simple pole at \(z=\mu_\star^{-1}\), and raising it to the power \(\dim\rho_\star\) produces a pole of order \(\dim\rho_\star\). For every other nontrivial channel \(\rho\), assumptions (2) and (3) imply that \(\nu\neq\mu_\star\) for all \(\nu\in\mathrm{spec}(M_{K,\rho}(u))\), so every factor above is nonzero. Thus the remaining Artin factors are analytic and nonvanishing at \(z=\mu_\star^{-1}\), and the pole order is exactly \(\dim\rho_\star\).
\end{proof}
...
\]
Because \(\tau(u)<1\), for all sufficiently large \(n\) the second factor lies in \([1/2,3/2]\); in particular \(B_n(\ind;u)>0\), and its reciprocal is \(1+O(\tau(u)^n)\). It follows that
\[
