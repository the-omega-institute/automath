...
\]
has a simple zero at \(z=\mu_\star^{-1}\). Hence \(L_{K,\rho_\star}(z,u)\) has a simple pole there, and raising it to the power \(\dim\rho_\star\) produces a pole of order \(\dim\rho_\star\). By assumptions (2) and (3), no other nontrivial factor \(L_{K,\rho}(z,u)\) vanishes in the denominator at \(z=\mu_\star^{-1}\): in the channel \(\rho_\star\), every spectral value distinct from \(\mu_\star\) has modulus \(<\eta(u)\), and in every other nontrivial channel the value \(\mu_\star\) does not occur. The remaining factors are therefore analytic and nonzero at \(z=\mu_\star^{-1}\), so they do not alter the pole order.
\end{proof}

\begin{theorem}[Convergence to uniformity]\label{thm:kernel-chebotarev-statistical-convergence}
...
O\!\bigl(\tau(u)^n\bigr),
\]
which proves the total-variation bound.
