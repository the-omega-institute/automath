...
Specialize the algebraic notation of Theorem \ref{thm:pullback-factorization} to complex coefficients.  Let \(P(z)\in\CC[z]\) have degree \(n\ge 1\) and satisfy \(P(0)\neq 0\).  For each scale \(t>1\), let \(Q_t=Q_t(P)\) denote the transport polynomial attached to \(P\) by Definition \ref{def:reciprocal-transport-normalization}.  Fix \(r>1\), and set
\[
...
\]
Thus \(\mathcal F\) is the logarithmic scale parametrization of the same transported mean, with the polynomial \(P\) held fixed and the transport scale specialized to \(t=e^s\).
