...
Specialize the algebraic notation of Theorem \ref{thm:pullback-factorization} to complex coefficients.  Let \(P(z)\in\CC[z]\) have degree \(n\ge 1\) and satisfy \(P(0)\neq 0\).  Write \(P(z)=a_n\prod_{j=1}^{n}(z-z_j)\).  In this theorem the reciprocal is the root-normalized polynomial
\[
P^\vee(u)=a_n^{-1}u^nP(1/u)=\prod_{j=1}^{n}(1-u z_j),
\]
so that \(P^\vee(r^2z)\) carries the reflected scale \(r^{-2}\) with no extra leading-coefficient factor.  For each scale \(t>1\), let \(Q_t=Q_t(P)\) denote the transport polynomial attached to \(P\) by Definition \ref{def:reciprocal-transport-normalization}.  Fix \(r>1\), and set
\[
