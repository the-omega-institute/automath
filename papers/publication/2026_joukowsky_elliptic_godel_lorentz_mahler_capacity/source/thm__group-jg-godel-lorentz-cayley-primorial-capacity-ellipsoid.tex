...
Throughout this section, \(\mathrm{SO}^+(1,1)\) denotes the identity component of the real Lorentz group preserving \(x^2-y^2\) and both time and space orientations.  Equivalently,
\[
\mathrm{SO}^+(1,1)=\{B(t):t\in\RR\},
\qquad
B(t)=\begin{pmatrix}\cosh t&\sinh t\\ \sinh t&\cosh t\end{pmatrix},
\]
and the parameter \(t\) is the rapidity.  The matrix identity \(B(s)B(t)=B(s+t)\) is the only Lorentz-group fact used below.

\begin{theorem}[Capacity-scaled Lorentz representation of prime data]\label{thm:capacity-scaled-prime-representation}
For \(L\ge 1\), define
\[
...
\begin{theorem}[Rigidity of the capacity-scaled Lorentz representation]\label{thm:capacity-scaled-lorentz-rigidity}
For \(t\in\RR\), write
\[
B(t):=
\begin{pmatrix}
...
\in\mathrm{SO}^+(1,1).
\]
Let \(\Psi:\mathsf P\to\mathrm{SO}^+(1,1)\) be a monoid homomorphism. Assume that the condition number exactly recovers the multiplicative norm,
\[
...
\Psi(r)=B\!\left(-\frac12\log N(r)\right)\qquad(r\in\mathsf P).
\]
In particular, if \(\Psi(e_p)_{12}\ge 0\) for each prime generator \(e_p\), then \(\Psi=\Lambda\). Thus the assignment in Theorem \ref{thm:capacity-scaled-prime-representation} is not merely a reparameterization: it is the unique positive Lorentz representation whose singular-value distortion is exactly the multiplicative multiplicative code.
\end{theorem}
...
\begin{corollary}[Condition number identity]\label{cor:capacity-condition-number-identity}
In the notation of Theorem \ref{thm:capacity-scaled-prime-representation},
\[
...
\begin{theorem}[Area-preserving normalization and Einstein addition]\label{thm:joukowsky-area-preserving-cayley}
For \(a>0\), let
\[
...
\begin{proposition}[Primorial lower bounds for \(p\)-ary square-free encodings]\label{prop:primorial-ellipse-bound}
Let \(p\) be a prime and \(b\ge 1\). Choose distinct primes \(q_1,\dots,q_b\), and define the scalar square-free encoding
\[
...
\begin{corollary}[Primorial forcing for Lorentz condition numbers]\label{cor:primorial-condition-number-bound}
Under the hypotheses of Proposition \ref{prop:primorial-ellipse-bound}, let \(r_{\max}\in\mathsf P\) be any prime register satisfying \(N(r_{\max})=L_{\max}\). Then
\[
...
\]
The claim now follows immediately from Proposition \ref{prop:primorial-ellipse-bound}(1).
\end{proof}

\begin{corollary}[Capacity, modulus, and rapidity]\label{cor:capacity-modulus-koenigs}
...
The next result is used as the volume interface for the primorial codebook theorem, not as an independent excursion into higher-dimensional convex geometry.  A one-dimensional Joukowsky ellipse records only the scalar norm \(N(r)\).  The diagonal ellipsoid below keeps the prime coordinates separated until the final determinant is taken; this is exactly what lets the pairwise-coprime codebook constraint turn into a product-volume minimization in Theorem \ref{thm:primorial-min-total-log-cost}.

\begin{proposition}[Auxiliary ellipsoid-volume character for prime registers]\label{prop:prime-ellipsoid-volume}
...
\dots,r_n\) are supported in \(P_0\), then
  ...
  \]
  In particular, for a pairwise-coprime square-free codebook \(\iota:X\hookrightarrow\NN_{\mathrm{sf},>1}\) with exponent registers \(r_x\), the ellipsoid-volume product is a strictly increasing function of the total logarithmic cost \(C(\iota)=\sum_x\log\iota(x)\). Thus the ellipsoid functor descends the prime-register decomposition to the primorial capacity minimization problem without losing the coordinatewise coprimality data before the determinant is evaluated.
\end{enumerate}
\end{proposition}
...
=\mathrm{Vol}(B_d)^n\exp\!\left(\frac12\sum_{k=1}^{n}\log N(r_k)\right).
\]
If the registers come from a pairwise-coprime square-free codebook, then \(N(r_x)=\iota(x)\), so the exponent is \(C(\iota)/2\). Since the exponential function is strictly increasing, minimizing the ellipsoid-volume product is equivalent to minimizing the total logarithmic cost. This is the precise interface used in Theorem \ref{thm:primorial-min-total-log-cost}.
\end{proof}

\begin{corollary}[Volume gap and ellipse area]\label{cor:ellipse-area-ellipsoid-gap}
...
\begin{proof}
By Theorem \ref{thm:capacity-scaled-prime-representation}, the semiaxes of \(E_J(r)\) are
\[
...
\begin{theorem}[Capacity-normalized primorial extremality]\label{thm:capacity-normalized-primorial-extremality}
Let \(X\) be a finite set with \(|X|=n\ge1\), and let
\[
\iota:X\hookrightarrow \NN_{\mathrm{sf},>1}
\]
be pairwise-coprime square-free.  For each \(x\in X\), let \(r_x\in\mathsf P\) be the exponent register of \(\iota(x)\), so that \(N(r_x)=\iota(x)\). Define the normalized ellipsoid-volume character
\[
...
\begin{proof}
The volume formula in Proposition \ref{prop:prime-ellipsoid-volume}, applied in the coordinate space of \(\operatorname{supp}(r_x)\), gives
\[
\frac{\mathrm{Vol}(\mathcal E_{D_x}(r_x))}{\mathrm{Vol}(B_{D_x})}
=\sqrt{N(r_x)}=\sqrt{\iota(x)}.
\]
Corollary \ref{cor:capacity-modulus-koenigs} gives
\[
...
For the fixed ambient statement, Proposition \ref{prop:prime-ellipsoid-volume} gives
\[
...
\begin{theorem}[Primorial minimization for pairwise-coprime square-free codebooks]\label{thm:primorial-min-total-log-cost}
Let \(X\) be a finite set with \(|X|=n\ge 1\), and let
\[
\iota:X\hookrightarrow \NN_{\mathrm{sf},>1}
\]
be a pairwise-coprime square-free codebook in the sense of Definition \ref{def:coprime-squarefree-codebook}. Let \(C(\iota)\) denote its total logarithmic cost. Then:
\begin{enumerate}
  \item \textbf{Minimal total cost.}
  Among all pairwise-coprime square-free codebooks,
  \[
...
\textbf{(2)} This is the normalized and fixed-ambient specialization of Theorem \ref{thm:capacity-normalized-primorial-extremality}. The fixed-dimensional factor \(\mathrm{Vol}(B_D)^n\) is constant only because the ambient prime set \(P_0\) is fixed before the minimization begins.

\textbf{(3)} By Corollary \ref{cor:ellipse-area-ellipsoid-gap},
\[
...
\begin{remark}[Necessity of the coprimality constraint]\label{rem:min-total-godel-cost-coprime-necessity}
The primorial minimization theorem is specific to the pairwise-coprime square-free class. If one merely asks for a set of distinct square-free integers, shared prime factors can reduce the total product. For instance,
\[
