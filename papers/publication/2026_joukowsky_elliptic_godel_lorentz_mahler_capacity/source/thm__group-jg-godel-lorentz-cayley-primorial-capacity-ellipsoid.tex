...
\begin{theorem}[Capacity-scaled Lorentz representation of prime data]\label{thm:godel-lorentz-algebraization}
For \(L\ge 1\), define
\[
...
\begin{theorem}[Rigidity of the capacity-scaled Lorentz representation]\label{thm:godel-lorentz-dictionary-rigidity}
For \(t\in\RR\), write
\[
B(t):=
\begin{pmatrix}
...
\Psi(r)=B\!\left(-\frac12\log N(r)\right)\qquad(r\in\mathsf P).
\]
In particular, if \(\Psi(e_p)_{12}\ge 0\) for each prime generator \(e_p\), then \(\Psi=\Lambda\). Thus the assignment in Theorem \ref{thm:godel-lorentz-algebraization} is not merely a reparameterization: it is the unique positive Lorentz representation whose singular-value distortion is exactly the multiplicative G\"odel code.
\end{theorem}
...
\begin{corollary}[Condition number identity]\label{cor:godel-condition-number-identity}
In the notation of Theorem \ref{thm:godel-lorentz-algebraization},
\[
...
\begin{proposition}[Primorial lower bounds for \(p\)-ary G\"odel encodings]\label{prop:boundary-primorial-ellipse-bound}
...
\begin{corollary}[Primorial forcing for Lorentz condition numbers]\label{cor:dyadic-boundary-godel-ellipse-illconditioning}
Under the hypotheses of Proposition \ref{prop:boundary-primorial-ellipse-bound}, let \(r_{\max}\in\mathsf P\) be any prime register satisfying \(N(r_{\max})=L_{\max}\). Then
\[
...
\]
The claim now follows immediately from Proposition \ref{prop:boundary-primorial-ellipse-bound}(1).
\end{proof}
...
The next result is used as the volume interface for the primorial codebook theorem, not as an independent excursion into higher-dimensional convex geometry.  A one-dimensional Joukowsky ellipse records only the scalar norm \(N(r)\).  The diagonal ellipsoid below keeps the prime coordinates separated until the final determinant is taken; this is exactly what lets the pairwise-coprime codebook constraint turn into a product-volume minimization in Theorem \ref{thm:primorial-min-total-godel-cost}.

\begin{proposition}[Auxiliary ellipsoid-volume character for prime registers]\label{prop:godel-ellipsoid-volume}
...
=\mathrm{Vol}(B_d)^n\exp\!\left(\frac12\sum_{k=1}^{n}\log N(r_k)\right).
\]
If the registers come from a G\"odel-decomposable codebook, then \(N(r_x)=\iota(x)\), so the exponent is \(C(\iota)/2\). Since the exponential function is strictly increasing, minimizing the ellipsoid-volume product is equivalent to minimizing the total G\"odel cost. This is the precise interface used in Theorem \ref{thm:primorial-min-total-godel-cost}.
\end{proof}
...
\begin{proof}
By Theorem \ref{thm:godel-lorentz-algebraization}, the semiaxes of \(E_J(r)\) are
\[
...
\begin{proof}
The volume formula in Proposition \ref{prop:godel-ellipsoid-volume}, applied in the coordinate space of \(\operatorname{supp}(r_x)\), gives
\[
...
For the fixed ambient statement, Proposition \ref{prop:godel-ellipsoid-volume} gives
\[
...
\begin{theorem}[Primorial minimization for G\"odel-decomposable codebooks]\label{thm:primorial-min-total-godel-cost}
