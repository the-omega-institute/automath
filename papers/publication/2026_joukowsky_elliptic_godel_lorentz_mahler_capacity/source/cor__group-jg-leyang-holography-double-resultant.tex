...
\]
is injective on this exact Lee--Yang class. Moreover, \(P\) can be reconstructed uniquely from the noiseless full pair \((r,Q_r)\): compute \(S_r\) from \(Q_r\), and then \(P\) is the unique monic degree-\(n\) factor of \(S_r\) whose zeros all lie on the unit circle.
\end{corollary}
