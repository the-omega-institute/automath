...
\begin{lemma}[Parametrized-cover gauge for the speed spectrum]\label{lem:group-jg-elliptic-gate-parametrized-gauge}
The Fourier family in Definition \ref{def:group-jg-elliptic-gate-logspeed-spectrum} is the spectral data of the normalized covering coordinate \(\theta\) on \(\TT\), not a datum of the unparametrized set \(E_c\) alone.  More precisely, let
\[
...
\]
is the formula in the normalized gauge of Definition \ref{def:group-jg-elliptic-gate-logspeed-spectrum}.  No assertion in this section treats Fourier coefficients after an arbitrary smooth reparametrization of the boundary as invariants of the unparametrized ellipse.
\end{lemma}
...
\begin{definition}[Gate isomorphism for the classification]\label{def:group-jg-equivariant-elliptic-gate-isomorphism}
...
In the minimal equivariant-register setting of Definition \ref{def:group-jg-equivariant-elliptic-gate-isomorphism}, a rational elliptic projection gate \(J_{c,m}\) with \(c\in(0,1)\cap\QQ\) and \(m\in\NN\) is determined, up to gate isomorphism, by the valuation data
\[
...
\qquad (1\le n\le N).
\]
Moreover \(\widehat u_{c,m}(k)=\widehat u_{c',m}(k)=0\) for every \(k\notin 2m\ZZ\). Thus, even when the sheet number \(m\) is fixed in advance and all coefficients up to the \(N\)-th nonzero harmonic are audited modulo \(p^K\), no finite congruence audit can exclude all sufficiently deep \(p\)-adic perturbations of the elliptic parameter.
\end{theorem}
...
\qquad (1\le n\le N).
\]
By Theorem \ref{thm:group-jg-elliptic-gate-prime-spectrum-rigidity}(1),
\[
\widehat u_{c,m}(2mn)=-\frac{c^n}{2n},
\qquad
