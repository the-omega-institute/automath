...
\[
F_{p,s}|_{\mathcal C_a}:=\pi_0^{\mathrm{pre}}(\mathfrak L_r).
\]
Since $\mathfrak L_r$ is already a stack, we may take
\[
\mathfrak P_r:=\mathfrak L_r
\]
as the realization prestack of an admitted reference $r$, so
\[
\pi_0^{\mathrm{pre}}(\mathfrak P_r)=\pi_0^{\mathrm{pre}}(\mathfrak L_r),\qquad
a_{p,s}F_{p,s}|_{\mathcal C_a}\cong\pi_0(\mathfrak L_r).
\]
Since both $\mathfrak G_{v_1}(a)$ and $\mathfrak G_{v_2}(a)$ are empty (both gerbes are non-neutral),
\[
F_{p,s}(a)=\pi_0^{\mathrm{pre}}(\mathfrak L_r)(a)=\varnothing,
\qquad
a_{p,s}F_{p,s}|_{\mathcal C_a}(a)\cong\pi_0(\mathfrak L_r)(a)\neq\varnothing.
\]
By \Cref{lem:nonvacuous-stackified-components}, this gives
\[
...
\]
translated through \textup{(i)} and \textup{(ii)}. The admissibility conjunct is already true by the standing hypothesis that $r$ is admitted at $p$, so the remaining content is exactly compatible local sectionability together with failure of global sectionability.
