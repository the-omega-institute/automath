...
\qquad
F_{p,s}|_{\mathcal C_a}:=\pi_0(\mathfrak G).
\]
Then $\mathfrak P_r$ is already a stack, so the stackification map is the identity and global conservativity holds automatically. Because $\mathfrak G$ is locally nonempty and connected, there is exactly one visible branch
\[
\Vis_{p,s}(r)=\{v\},
\]
the branch gerbe is $\mathfrak G_v=\mathfrak G$, and
\[
M,p\Vdash \CompSec_s(r)
\qquad\text{but}\qquad
M,p\nVdash \Sec_s(r),
\]
since $\mathfrak G$ is non-neutral. The good cover $\mathcal U$ trivializes the constant band, all finite intersections are contractible, hence the $H^1$-vanishing and \v{C}ech-to-derived comparison hypotheses of \Cref{def:cech-gluing-obstruction,def:finite-nerve-presentation} are satisfied.
