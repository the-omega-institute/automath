...
In the finite nerve presentation of \Cref{def:finite-nerve-presentation}, the initiality of the intrinsic visible quotient among coefficient quotients whose pushed-forward obstruction is pure $\operatorname{Ext}$ is exactly \cite[Theorem~6.13]{MaZhang2026ConditionalGluingFailure}. Applied here with the quotient map $\pi_\omega:A\twoheadrightarrow A_{\mathrm{vis}}^\omega$, it says that a surjection $q:A\twoheadrightarrow B$ factors uniquely through $\pi_\omega$ precisely when $q_*(\omega)$ has zero $H_2$-evaluation, equivalently when it lies in the pure $\operatorname{Ext}$ part of the universal coefficient sequence for $H^2(N(\mathcal U),B)$.
\end{proposition}
...
\]
for every abelian group $B$. Thus, in the criterion cited in \Cref{thm:intrinsic-pure-ext-initiality}, the pure-extension condition is equivalent to $q_*(\omega)=0$, and the conclusion follows.
\end{proof}
