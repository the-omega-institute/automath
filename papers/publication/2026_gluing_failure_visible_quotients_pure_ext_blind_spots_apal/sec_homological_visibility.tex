...
\]
\end{enumerate}
If these conditions hold and $\omega\neq 0$, then the chosen branch gerbe $\mathfrak G_v$ is non-neutral, while every character of $A$ is class-admissible for that branch. In particular, if $|\Vis_{p,s}(r)|=1$ and the chosen realization prestack is globally conservative at $a$, then
\[
...
If, in addition, $|\Vis_{p,s}(r)|=1$ and the chosen realization prestack is globally conservative at $a$, then this unique visible branch exists and its component gerbe is non-neutral. The branched gerbe semantics of \Cref{thm:gerbe-null-semantics} therefore gives
\[
M,p\Vdash \Null^{\mathrm{glue}}_s(r).
\]
\end{proof}
...
\]
Thus the obstruction is nontrivial, but only its parity quotient is intrinsically visible. If $\omega$ is the unique visible branch obstruction of the reference under consideration and the chosen realization prestack is globally conservative at the terminal fibre, then \Cref{thm:gerbe-null-semantics} yields a genuine gluing-level absence with a nontrivial visible quotient.
\end{example}
...
A_{\mathrm{vis}}^{\omega}=A,
\]
by \Cref{thm:character-blind-obstructions}. Thus no class-level quotient collapse occurs at all, yet the obstruction class is nonzero. Hence this branch is completely character-blind. If it is the unique visible branch of the admitted reference under consideration and the chosen realization prestack is globally conservative at the terminal fibre, then \Cref{thm:gerbe-null-semantics} yields a genuine gluing-level absence that is invisible to every character channel.
\end{example}
