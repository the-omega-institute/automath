...
In the setting of \Cref{def:finite-nerve-presentation}, define the
\emph{intrinsic hidden subgroup}
\[
...
=
\operatorname{Ann}(K_{\omega}),
\]
where
\[
\operatorname{Ann}(K_{\omega})
:=
\{\chi\in\widehat A\mid \chi(k)=0 \text{ for every }k\in K_{\omega}\}.
\]
Consequently,
\[
K_{\omega}
...
\begin{proof}
For every character $\chi:A\to\mathbb T$, naturality of the universal
coefficient sequence gives a commutative square
\[
\begin{CD}
...
\]
\cite[Sec.~2.3]{Weibel1994}. Thus $\eta_{\mathbb T}$ is an isomorphism by the
universal coefficient exact sequence. Therefore
\[
\chi_*(\omega)=0
\Longleftrightarrow
...
\]
This proves the first displayed equality. Because
$K_{\omega}=\mathrm{Im}(\mathrm{ev}_{\omega})$, the condition
$\chi\circ \mathrm{ev}_{\omega}=0$ is equivalent to $\chi$ vanishing on
$K_{\omega}$, so
\[
X_{\omega}=\operatorname{Ann}(K_{\omega}).
\]

Let $H:=K_{\omega}$. By finite abelian duality,
\[
\bigcap_{\chi\in \operatorname{Ann}(H)}\ker\chi=H
\]
\cite[Part~I, Chaps.~2, 12]{Terras1999}. Since
$X_{\omega}=\operatorname{Ann}(K_{\omega})$, this gives
\[
K_{\omega}
=
\bigcap_{\chi\in X_{\omega}}\ker\chi.
\]
The quotient description now follows immediately:
\[
A_{\mathrm{vis}}^{\omega}
=
A/K_{\omega}
...
X_{\omega}=\operatorname{Ann}(K_{\omega}).
\]
Characters of $A_{\mathrm{vis}}^{\omega}=A/K_{\omega}$ are exactly the
characters of $A$ that vanish on $K_{\omega}$, so precomposition with
$\pi_{\omega}$ identifies $\widehat{A_{\mathrm{vis}}^{\omega}}$ with
$X_{\omega}$. The final equivalence is the usual separation property of the
character group of the finite abelian quotient $A_{\mathrm{vis}}^{\omega}$
\cite[Part~I, Chap.~2]{Terras1999}.
\end{proof}
...
\]
be a surjective homomorphism onto a finite abelian group $B$. In the setting of
\Cref{def:finite-nerve-presentation}, the following are equivalent:
\begin{enumerate}[label=(\roman*),nosep]
\item the pushed-forward class $q_*(\omega)\in H^2(N(\mathcal U),B)$ lies in
the subgroup
\[
...
\]
\end{enumerate}
Hence $A_{\mathrm{vis}}^{\omega}$ is the initial coefficient quotient through
which the $H_2$-visible part of the obstruction disappears, leaving only a pure
$\operatorname{Ext}$-type remnant.
\end{theorem}

\begin{proof}
The equivalence of \textup{(i)} and \textup{(ii)} is exactly the universal
coefficient sequence for $B$ \cite[Sec.~3.1]{Hatcher2002}. Naturality gives
\[
\eta_B(q_*(\omega))=q\circ \eta_A(\omega)=q\circ \mathrm{ev}_{\omega},
\]
so \textup{(ii)} and \textup{(iii)} are equivalent. Conditions \textup{(iii)}
and \textup{(iv)} are equivalent by definition of the image of a homomorphism.
By \Cref{def:intrinsic-visible-quotient},
$\mathrm{Im}(\mathrm{ev}_{\omega})=K_{\omega}$, so \textup{(iv)} is equivalent
to
\[
K_{\omega}\subseteq \ker q.
\]
By the universal property of the quotient by $K_{\omega}$, this is equivalent
to unique factorization of $q$ through $\pi_{\omega}$, proving \textup{(v)}.
\end{proof}
...
\]
Then, for every surjective homomorphism $q:A\twoheadrightarrow B$, the
...
\]
\end{enumerate}
Hence, when $H_1(N(\mathcal U),\mathbb Z)=0$, the intrinsic visible quotient is
initial among coefficient quotients that annihilate the obstruction class
itself.
\end{corollary}
...
\]
for every abelian group $B$. Thus the pure-extension condition in
\Cref{thm:intrinsic-pure-ext-initiality}\textup{(i)} is equivalent to
$q_*(\omega)=0$, and the conclusion follows.
\end{proof}
...
In the setting of \Cref{def:finite-nerve-presentation}, the following are
equivalent:
\begin{enumerate}[label=(\roman*),nosep]
\item $K_{\omega}=0$;
...
\]
\end{enumerate}
If these conditions hold, then every character of $A$ is class-admissible for
$\omega$. In particular, if also $\omega\neq 0$, then the branch obstruction is
nonzero while every character channel is blind to it.
\end{theorem}
...
K_{\omega}=\mathrm{Im}(\mathrm{ev}_{\omega}),
\]
so \textup{(i)} and \textup{(iii)} are equivalent. Since
$A_{\mathrm{vis}}^{\omega}=A/K_{\omega}$, conditions \textup{(i)} and
\textup{(ii)} are equivalent. By the universal coefficient exact sequence,
\textup{(iii)} holds exactly when $\omega\in\ker(\eta_A)$, and this kernel is
the stated $\operatorname{Ext}$-subgroup. Hence all four conditions are
equivalent.

If they hold, then $\mathrm{ev}_{\omega}=0$, so
\[
\chi\circ \mathrm{ev}_{\omega}=0
\]
for every $\chi\in\widehat A$. By
\Cref{thm:intrinsic-visible-quotient}, every character of $A$ is therefore
class-admissible. If moreover $\omega\neq 0$, then the obstruction class
remains nontrivial even though all character channels annihilate it.
\end{proof}
...
\]
Every nonzero branch obstruction class on such a nerve is therefore
character-blind.
\end{corollary}
...
\]
so $\mathrm{ev}_{\omega}=0$ for every $\omega$. Apply
\Cref{thm:character-blind-obstructions}.
\end{proof}
...
\omega=0.
\]
Equivalently, every nonzero branch obstruction class has a nontrivial
$H_2$-visible part, and hence every nonzero obstruction class has a proper
intrinsic visible quotient.
\end{corollary}
...
\]
Hence the universal coefficient map $\eta_A$ is injective. By
\Cref{thm:character-blind-obstructions},
\[
K_{\omega}=0
\quad\Longleftrightarrow\quad
...
\eta_A(\omega)=0.
\]
Injectivity of $\eta_A$ therefore gives $K_{\omega}=0$ if and only if
$\omega=0$. If $\omega\neq 0$, then $K_{\omega}\neq 0$, so
$A_{\mathrm{vis}}^{\omega}=A/K_{\omega}$ is a proper quotient of the finite
group $A$.
\end{proof}
