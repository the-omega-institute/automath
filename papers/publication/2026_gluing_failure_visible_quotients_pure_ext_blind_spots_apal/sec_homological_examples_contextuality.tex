...
intersection after refinement. The comparison theorem for a basis says that
restriction from sheaves on the topological site of $X$ to sheaves on this
basis site is an equivalence of topoi; under this equivalence the derived
global section functors agree, so sheaf cohomology computed on the basis site is
the same as sheaf cohomology on the original space
\cite[Chap.~III, Secs.~2--4]{MacLaneMoerdijk1994},
\cite[Sec.~4.1]{Bredon1997}. The same statement applies to every basis object,
viewed with its induced basis of smaller nonempty intersections. Since the
members and nonempty finite intersections of a good cover are contractible,
the constant finite abelian sheaf has vanishing positive cohomology on those
pieces.
...
$A$-valued \v{C}ech $2$-cocycle $g=(g_{ijk})$ representing $\omega$. The
version of the standard classification used here is the Giraud classification
of $A$-banded gerbes on the chosen slice site by the sheaf cohomology group
$H^2(\mathcal C_a,A)$, with the cover-\v{C}ech class mapped to sheaf cohomology
by the comparison map of \Cref{def:gerbe-adapted-cech-comparison}
\cite[Chap.~IV]{Giraud1971}, \cite[Tags 06NY and 06O9]{StacksProject}. Under
the present gerbe-adapted cofinal system this comparison identifies the chosen
class $\omega\in H^2(N(\mathcal U),A)\cong \Cech^2(\mathcal U,A)$ with the
corresponding sheaf cohomology class on the slice. Hence there is an
$A$-banded gerbe $\mathfrak G_\omega$ on that slice whose Giraud class is the
image of $\omega$, and it is neutral exactly when this class is zero.
