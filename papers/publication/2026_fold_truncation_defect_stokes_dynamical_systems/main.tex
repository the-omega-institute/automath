...
We note that the finite-state realizability and character nondegeneracy
conditions are structural hypotheses that must be verified for each specific
$m$; the theorem provides the general mechanism.  For small values of $m$, one
can in principle construct the hidden Markov chain and its transition matrices
explicitly and check both conditions by direct computation.  We leave this
verification to future work and emphasize that the deterministic Stokes
...
like $O(\rho_m^{\,n-m})$.  Since Haar measure kills all nontrivial characters,
the same exponential bound holds in total variation; see
\cite[Chapters~12--13]{LevinPeresWilmer2009MarkovMixing} and the transfer
operator framework of \cite{Baladi2000}.
\end{proof}
