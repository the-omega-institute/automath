...
resolution band $m,\dots,n-1$.  From this identity we derive auditable
probability bounds, a Hamming--Lipschitz budget, a cocycle formula across
intermediate scales, and a simple inverse-limit lift criterion under eventual
adjacent-defect extinction.  The final section is explicitly conditional: it
records a general Haar-mixing criterion for finite-state additive defect
models, but no such model is constructed or verified here for the Fold tower.
The contribution of the paper is therefore the deterministic defect calculus
and its immediate consequences, not a statistical theorem for Fold itself.
\end{abstract}
...
section then records a separate conditional harmonic-analysis criterion for any
finite-state additive realization of a defect process.  That criterion is
included as a template for future Fold applications, not as a theorem
established here for the Fold tower itself.
...
prefix restriction, and it records the exact telescoping identity satisfied by
that observable.  The accompanying probability and Hamming estimates are direct
deterministic consequences of that bookkeeping identity.  In that sense the
statistical discussion in Section~7 should be read as a conditional framework
appended to the deterministic theory, not as an independently verified theorem
about the Fold tower itself.
...
\section{Conditional Statistical Framework}

\subsection{Conditional Haar-Mixing Criterion}
...
\cite{Norris1997,LevinPeresWilmer2009MarkovMixing,Terras1999}.  The statements
below are conditional implications about any finite-state additive realization
of the deterministic defect process.  We do not construct or verify such a
model for the Fold tower here.
...
\begin{corollary}[Conditional parity polarization and binomial limit at full generation]
...
auditable probability bounds, Lipschitz budgets, and cocycle inequalities.
