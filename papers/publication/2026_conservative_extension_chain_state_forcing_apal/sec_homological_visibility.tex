...
The strict quotient $A_{\mathrm{vis}}=A/H_\alpha$ still depends on the chosen cocycle. To isolate the part determined by the class itself, we pass to the homological evaluation map.

Throughout the section we work with a fixed finite cover and a fixed finite nerve presentation. The only comparison maps used are the explicit identification of \v{C}ech cochains with simplicial cochains on the finite nerve and the resulting universal coefficient sequence. In this sense the intrinsic hidden subgroup and intrinsic visible quotient are intrinsic only relative to the homology-equivalent changes of presentation singled out below.
