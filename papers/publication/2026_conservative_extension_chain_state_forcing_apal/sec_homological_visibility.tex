\section{Homological Visibility}
\label{sec:homological-visibility}

The strict quotient $A_{\mathrm{vis}}=A/H_\alpha$ depends on the chosen cocycle representative. To isolate the part of the obstruction determined by the cohomology class itself, we pass to the homological evaluation map and take it as the primary invariant.

Throughout the section we work with a fixed finite cover and a fixed finite nerve presentation. All cochain groups are computed on that presentation, and the only comparison maps used are the explicit identification of \v{C}ech cochains with simplicial cochains on the finite nerve and the resulting universal coefficient sequence for that simplicial model. Accordingly, the intrinsic hidden subgroup and intrinsic visible quotient are intrinsic only relative to the homology-equivalent comparison operations isolated below.

The algebraic input is standard: the universal coefficient exact sequence, finite Pontryagin duality, and elementary quotient theory. What matters here is the semantic interpretation of the resulting maps and quotients, and in particular the identification of the pure $\operatorname{Ext}$ regime in which a nonzero obstruction remains invisible to every admissible character.

We also assume that the chosen cover $\mathcal U$ trivializes the band, so that the coefficients may be treated as a constant finite abelian group $A$ on the nerve. The case of nontrivial monodromy, which would require local coefficients, is left aside.
