\section{Intrinsic visibility and homological quotients}
\label{sec:homological-visibility}

The quotient $A_{\mathrm{vis}}=A/H_\alpha$ attached to a chosen cocycle still depends on the presentation. To isolate the part determined by the cohomology class itself, we pass to the homological evaluation map. Nothing in this section asserts that every branch obstruction class admits such a presentation; the discussion is conditional on one fixed finite nerve presentation.

Throughout the section we work with a fixed finite cover and a fixed finite nerve presentation. The only comparison maps we use are the identification of \v{C}ech cochains with simplicial cochains on the finite nerve and the resulting universal coefficient sequence. Thus the intrinsic hidden subgroup and visible quotient are intrinsic only with respect to the homology-equivalent changes of presentation singled out below. The underlying homological algebra is classical; the point of the section is to package it into branchwise semantic invariants within this restricted comparison class.

We also assume that the chosen cover $\mathcal U$ trivializes the band, so that the coefficients may be regarded as a constant finite abelian group $A$ on the nerve. The case of nontrivial monodromy, which would require local coefficients, is not treated here.
