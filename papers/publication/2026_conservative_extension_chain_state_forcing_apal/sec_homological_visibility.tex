...
The quotient $A_{\mathrm{vis}}=A/H_\alpha$ attached to a chosen cocycle still depends on the presentation. To isolate the part determined by the cohomology class itself, we pass to the homological evaluation map.

Throughout the section we work with a fixed finite cover and a fixed finite nerve presentation. The only comparison maps we use are the identification of \v{C}ech cochains with simplicial cochains on the finite nerve and the resulting universal coefficient sequence. Thus the intrinsic hidden subgroup and visible quotient are intrinsic only with respect to the homology-equivalent changes of presentation singled out below.
