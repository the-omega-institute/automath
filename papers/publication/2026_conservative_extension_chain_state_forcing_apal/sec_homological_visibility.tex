...
Fix a finite nerve presentation of a branch obstruction class. The quotient attached to a chosen cocycle still depends on that presentation, so to extract the part determined by the cohomology class itself we pass to the homological evaluation map. Throughout the section we work with one fixed finite cover and the comparison maps arising from the identification of \v{C}ech cochains with simplicial cochains on its finite nerve. The resulting hidden subgroup and visible quotient are therefore intrinsic only with respect to the homology-equivalent changes of presentation singled out below.
