...
The algebraic ingredients themselves are standard: the universal coefficient exact sequence, finite Pontryagin duality, and elementary quotient theory. The point of the subsection is therefore not to present these facts as new algebra, but to isolate the semantic quotient detected by $\mathrm{ev}_{\omega}$ and the pure $\operatorname{Ext}$-type regime in which a nonzero obstruction remains character-blind. In particular, the discussion below does not claim a cohomological completeness theorem of the sort pursued by Car\`u and Montanhano \cite{Caru2018,Montanhano2021}; it isolates one precise blind regime inside the universal-coefficient decomposition.
...
\subsubsection*{Algebraic background}
...
\]
inside a concrete site-theoretic example, once the realization stack has been fixed by the chosen
cocycle data. It is the opposite extreme from the $\mathbb{R}P^2$ blind case of
\Cref{ex:rp2-character-blind}:
\begin{itemize}[nosep]
\item $\mathbb{R}P^2$-type nerve: $K_\omega=0$, every character is class-admissible, the obstruction is purely $\operatorname{Ext}$-type with $A_{\mathrm{vis}}^\omega = A$.
\item $S^2$-type nerve: $K_\omega=A$, only the trivial character is class-admissible, the obstruction is purely $H_2$-type and maximally visible with $A_{\mathrm{vis}}^\omega = 0$.
\end{itemize}
\end{example}
