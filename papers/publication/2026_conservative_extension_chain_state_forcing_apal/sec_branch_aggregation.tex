...
\begin{enumerate}[label=(\roman*),nosep]
\item $M,p\Vdash \CompSec_s(r)$ iff $S_e$ has a compatible family on the measurement cover;
\item $M,p\Vdash \Sec_s(r)$ iff $S_e$ has a global section.
\end{enumerate}
\end{proposition}

\begin{proof}
The two clauses are direct translations of \Cref{def:existence-levels}(ii),(iii) under the stated identification of local object functors and the definition of sections of a presheaf on the terminal object of the cover site.
\end{proof}
...
\begin{proof}
By \Cref{prop:ab-compatibility-translation}, the first two clauses are equivalent to compatible families and global sections of the support presheaf, respectively.

Statement \textup{(iii)} is then the definition of $\Null^{\mathrm{glue}}_s(r)$. Since there is one visible branch, \Cref{cor:single-branch-reduction} gives the quotient identifications in \textup{(iv)}. Character detection is then by \Cref{cor:intrinsic-character-detection}.

For \textup{(v)}, the first equivalence is the statement that $\mathrm{ev}_{\omega}=0$ exactly when $\omega$ lies in the $\operatorname{Ext}$-summand in \Cref{thm:character-blind-obstructions}; the second is by the same theorem for $K_{\omega}=0$ in the unique-branch case. The remaining implication is the interpretation of $|\Vis_{p,s}(r)|=1$.
\end{proof}
