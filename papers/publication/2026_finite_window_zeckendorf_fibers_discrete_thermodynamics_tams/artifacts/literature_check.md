...
## Brocot condensation and conditioned heavy tails (2026-08-15)

This pass checked the proposed total-variation theorem before it was added.
Primary texts were downloaded and searched in full where available.  The
arXiv API queries included exact titles and combinations of `Brocot`,
`Stern-Brocot`, `continued fraction`, `partial quotient`, `fixed sum`,
`continuant`, `condensation`, `Gibbs partitions`, and `total variation`.
Crossref confirmed the DOI metadata until its public endpoint returned HTTP
429; the arXiv records and publisher DOI redirects independently confirmed
the metadata used below.

### Fixed-order Brocot and Farey literature

- A. A. Dushistova, *Partitioning of the interval [0,1] induced by the
  Brocot sequences*, Sb. Math. 198 (2007), 661--690,
  DOI 10.1070/SM2007v198n05ABEH003854; arXiv:math/0512598v3.  The full text
  defines Brocot fractions of order `n` by canonical regular continued
  fractions with `a_1 + ... + a_r = n`, introduces the fixed-sum continuant
  sum in Theorem 3, and derives a fuller scalar asymptotic expansion.  Its
  large-partial-quotient decomposition is a partition-sum argument.  It does
  not define the denominator-weighted probability law, state total-variation
  convergence of the two contexts, give a defect distribution, or give the
  joint location and denominator-ratio limits.
- N. Moshchevitin and A. Zhigljavsky, *Entropies of the partitions of the
  unit interval generated by the Farey tree*, Acta Arith. 115 (2004), 47--58,
  DOI 10.4064/aa115-1-4.  This is the scalar entropy/partition-sum predecessor
  cited by Dushistova.  The search also checked the later two-dimensional
  extension N. Moshchevitin and M. Vielhaber, *Moments for generalized
  Farey-Brocot partitions*, arXiv:math/0703566.  Neither source advertises or
  states a fixed-order probability law for the left and right continued-
  fraction contexts.
- M. Kessebohmer and B. O. Stratmann, *Stern-Brocot pressure and multifractal
  spectra in ergodic theory of numbers*, Stoch. Dyn. 4 (2004), 77--84,
  DOI 10.1142/S0219493704000948, and *A multifractal analysis for Stern-Brocot
  intervals, continued fractions and Diophantine growth rates*, J. Reine
  Angew. Math. 605 (2007), 133--163,
  DOI 10.1515/CRELLE.2007.029; arXiv:math/0509603v2.  The full 2007 source
  studies Stern-Brocot pressure, interval sizes, denominators, Birkhoff growth
  rates, and Hausdorff dimensions of level sets.  It does not study the
  denominator-weighted law on the finite fixed-digit-sum set or the contexts
  around a unique maximal partial quotient.

No searched source in this line states the theorem now labelled
`thm:brocot-condensation`.  This is evidence from direct queries and primary
texts, not an assertion that a priority search can be logically exhaustive.

### Does a general condensation theorem imply it?

- I. Armendariz and M. Loulakis, *Conditional distribution of heavy tailed
  random variables on large deviations of their sum*, Stochastic Process.
  Appl. 121 (2011), 1138--1147,
  DOI 10.1016/j.spa.2011.01.011; arXiv:0912.1516v4.  Theorems 1--2 and
  Propositions 1--4 give total-variation conditional product limits after
  deleting the maximum of an i.i.d. subexponential sample, for tail or local
  conditioning in the stated regimes.
- B. Stufler, *Unlabelled Gibbs partitions*, Combin. Probab. Comput. 29
  (2020), 293--309, DOI 10.1017/S0963548319000336;
  arXiv:1610.01401v2.  Theorem 3.1 and Corollaries 3.3--3.4 give
  total-variation convergence of the small fragments after formation of a
  giant component in a weighted combinatorial composition.  The hypotheses
  are expressed through component weights and a subexponential generating
  series.  The newer comparison N. Bosio, M. Kuba, and B. Stufler, *Gibbs
  partitions and lattice paths*, arXiv:2411.03930v3, likewise starts from the
  product composition weight `v_k product_i w_{n_i}` and obtains a unique
  giant component under regular-variation assumptions.

These theorems do **not** directly imply the Brocot result.  Under the target
law, a word `(a_1,...,a_r)` has weight

    K(a_1,...,a_r)^(-s),

which does not factor as a product of one-digit or component weights.  The
continuant couples every digit and also distinguishes the left unrestricted
word space from the right canonical word space.  Consequently there is no
i.i.d. conditioning relation or Gibbs composition scheme to which the cited
theorems can be applied verbatim.  The required substitute is arithmetic:

1. the fixed-context affine expansion
   `K(u,a,v) = a K(u) K(v) + O_{u,v}(1)`;
2. the summable majorant
   `n^s K(u,a,v)^(-s) <= 2^s {K(u)K(v)}^(-s)`;
3. the `o(n^(-s))` estimate for all words without a unique digit exceeding
   `n/2`.

After those model-specific facts are proved, the last normalization step is
the elementary discrete Scheffe lemma.  Thus the *form* of the conclusion is
standard condensation theory, but the theorem is not an immediate corollary
of a known conditioned-limit theorem.

### Index audit

All searches and the final theorem use the Brocot order
`n = a_1 + ... + a_r`.  The paper's renewal letter indexed by `d` is linked
only through the exact identity

    b_{2d+1}(s) = sigma_{s/2}(d+1) = Z_{d+1}(s),

so `n = d+1` and the odd renewal cost is `c = 2d+1 = 2n-1`.  No limit in the
Brocot theorem is stated in the renewal-cost variable.
