...
\begin{proposition}[Characteristic polynomial and residue constant]
  ...
  \[
    ...
    \rho(B)\le \max_i\sum_j B_{ij}=3,
  \]
  so \(\rho(B)=3\). The Perron residue constant \(C_B\) of
  \Cref{def:perron-residue-constant} satisfies
  \[
    C_B=\frac{243}{272}.
  \]
\end{proposition}
...
factorisation is then immediate. The spectral-radius statement is a
separate Perron--Frobenius consequence of the non-negative matrix
itself. Every row of \(B=B(1)\) has sum \(3\), and hence
\[
  B\mathbf 1=3\mathbf 1,\qquad
...
This argument deliberately does not identify the spectral radius by
selecting a reciprocal root from the displayed factorisation. Such an
argument would be invalid here: the factor \(z^3-z^2+z+1\) has a zero
inside the unit disc. The factorisation is used only after the
row-sum proof of \(\rho(B)=3\), namely for the residue computation
\[
  \begin{aligned}
  C_B
  &=\lim_{z\to1/3}
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \]
  If \(\Phi_m^+(s):=\prod_{\alpha\in A_m}(s-\alpha)\) and
  \(d_m:=|A_m|\), then the standard Sylvester resultant satisfies
  \[
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
    =P_m(w).
  \]
  The order is part of the formula. In the reversed order one has
  \[
...
slot. This is the only resultant order identified with the displayed
root product. Since \(\deg_s\widehat\Delta=3\), the reversed-order
identity in \Cref{lem:ordered-cyclotomic-resultant} gives
\[
  \Res_s\bigl(\widehat\Delta(w,s),\Phi_m^+(s)\bigr)
  =(-1)^{3d_m}
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr),
...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  \[
    \Phi_m^+(s):=\prod_{\alpha\in A_m}(s-\alpha),
  \]
  and define the normalized real cyclotomic factor by the root product
  \[
    R_m(w):=\prod_{\alpha\in A_m}H(w,\alpha).
  \]
  Equivalently, under the standard Sylvester convention and the order
  fixed by \Cref{lem:ordered-cyclotomic-resultant},
  \[
    R_m(w)=\Res_s\bigl(\Phi_m^+(s),H(w,s)\bigr).
  \]
  This order is part of the theorem: the cyclotomic polynomial is the
  first argument, so the resultant is the root product itself. If the
...
\]
where \(A_m\) is the conjugate set displayed below. Equivalently,
\[
  R_m(w)=\Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
\]
with \(\Phi_m^+\) in the first resultant slot.
The reversed-order resultant is not used to define \(R_m\).
...
relation between the ordered resultant and the normalized primitive
real factor is
\[
  R_m(w)=
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
...
\[
  ...
  =(-1)^{3d_m}R_m(w),
\]
not \(R_m(w)\) unless the sign is \(+1\).
...
\Cref{lem:ordered-cyclotomic-resultant} gives the exact identity
\[
  R_m(w)
  =\Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
  =
    \prod_{\alpha\in A_m}\widehat\Delta(w,\alpha).
\]
The order displayed here is essential: the same lemma gives
\[
  \Res_s\bigl(\widehat\Delta(w,s),\Phi_m^+(s)\bigr)
  =(-1)^{3d_m}R_m(w)
\]
