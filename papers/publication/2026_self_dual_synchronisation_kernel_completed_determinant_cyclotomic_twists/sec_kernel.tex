...
produces the displayed formula; the exact command certificate is
...
This metadata is already a nonsquare certificate: every square in
\(\QQ(s)\) has even order at infinity and positive leading sign when it
is a nonzero polynomial, whereas this polynomial has even degree and
negative leading coefficient. Equivalently, \(D(s)\to-\infty\) as
...
Thus \(19\nmid D(3)\), so the specialisation is separable at \(19\) and
Dedekind's theorem applies to this factorisation.
Here \(\operatorname{lc}_w\widehat\Delta(w,3)=8\not\equiv0\pmod{19}\).
Modulo \(19\),
\[
  \widehat\Delta(w,3)
...
\(c_0\ne0\) in \Cref{lem:sign-quotient-normal-form} is satisfied; hence
...
\[
  ...
  \qquad a_0=\frac13 .
\]
Substituting this series into \(\widehat\Delta(w(s),s)\) and setting the
coefficient of \((s-2)^n\) equal to \(0\), for \(1\le n\le6\), gives a
triangular recursion because the coefficient of the new unknown \(a_n\)
is always \(\partial_w\widehat\Delta(1/3,2)\ne0\). Solving these six
linear equations and then inverting \(w(s)\) yields the displayed jet of
