...
factorisation is then immediate. The spectral-radius statement is
deduced before using the factorisation to compute the residue. We do
not infer it from the reciprocal roots of the factors: indeed the
factor \(z^3-z^2+z+1\) has a zero inside the unit disc, so such a
comparison would be false. Instead we use the non-negative matrix
directly. Every row of \(B=B(1)\) has sum \(3\), so
\[
