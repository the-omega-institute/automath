...
zero with \(\abs{z}<1\), so a comparison of zero moduli in the
factorisation is not a valid Perron argument. The Perron conclusion
above comes only from the row-sum bound and primitivity. The
factorisation is used here solely for the residue computation
\[
  \begin{aligned}
  C_B
  &=\lim_{z\to1/3}
...
\begin{corollary}[Endpoint theorem applied to the ten-state kernel]
...
\(\eta=1/100\) and \(\varepsilon=1/200\) are exactly the assertions
