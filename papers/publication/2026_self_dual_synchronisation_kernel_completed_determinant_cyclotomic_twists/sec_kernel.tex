...
\begin{proposition}[Untwisted Perron root and residue constant]
  ...
  This conclusion is obtained from the row-sum Perron bound, not from
  any ordering of the reciprocal zeros in the determinant factorisation.
  In addition,
  \[
...
self-loop \(\st{000}\to\st{000}\), the period is \(1\), so \(B\) is primitive.
  The Perron-root calculation comes first and is independent of any
  factorisation of \(\det(I-zB)\). Inspecting the transition table at
  \(u=1\), every row of \(B=B(1)\) has sum \(3\), hence
\[
  B\mathbf 1=3\mathbf 1,\qquad
  \rho(B)\le \max_i\sum_j B_{ij}=\|B\|_\infty=3.
\]
Equivalently, \Cref{lem:row-sum-perron-bound} applies with \(q=3\). The
first displayed equality shows that \(3\) is itself an eigenvalue, while
the row-sum bound shows that no eigenvalue has modulus larger than
\(3\). Thus \(\rho(B)=3\). Since \(B\) is primitive, the
Perron--Frobenius theorem gives that this eigenvalue is simple and that
no other eigenvalue lies on the circle \(\abs{\lambda}=3\). This is the
entire spectral-radius argument; it does not use, and does not require,
any comparison of the reciprocal zeros of
\[
  (z-1)(z+1)(3z-1)(z^3-z^2+z+1).
\]
The determinant factorisation is used only after the Perron conclusion
has already been proved. Setting \(u=1\) in
...
\(\Phi_m^+\) in the first slot. In particular, the notation \(P_m\)
...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  \]
  This product is the primary definition. Under the standard Sylvester
  convention it is represented by the ordered resultant fixed in
...
definition of \(R_m\). With the standard Sylvester convention it is also
