...
The Perron-root calculation uses only the non-negative matrix \(B\),
not the factorisation of its characteristic polynomial. Inspecting the
transition table at \(u=1\), every row of \(B=B(1)\) has sum \(3\), and
hence
\[
  B\mathbf 1=3\mathbf 1,\qquad
...
modulus \(3\).
Only after this row-sum argument do we use the determinant
factorisation. Setting \(u=1\) in \Cref{prop:kernel-delta-explicit}
gives
\[
  \det(I-zB)=(z-1)(z+1)(3z-1)(z^3-z^2+z+1).
\]
This factorisation is used here only for the residue computation
\[
  \begin{aligned}
  C_B
  &=\lim_{z\to1/3}
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \]
  and \(d_m:=|A_m|\).
  Then the normalized primitive real factor is the root product
  \[
    P_m(w):=\prod_{\alpha\in A_m}\widehat\Delta(w,\alpha).
  \]
  Equivalently, it is the ordered resultant
  \[
    P_m(w)=
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr),
  \]
  where \(\Phi_m^+\) is in the first argument. The reversed order is
  not used to define \(P_m\); with the standard Sylvester convention it
  is
  \[
...
\(\Phi_m^+\) in the first slot. Since \(\deg_s\widehat\Delta=3\), the
...
The theorem is meant as an application template, not as a statement
about the particular ten-state matrix. For a new self-dual kernel the
