...
\begin{remark}[Boundary to companion numeration interfaces]
  \label{rem:companion-numeration-boundary}
  Some companion notes use signed Zeckendorf normal forms to name
  integer data in a larger comparison construction.  That numeration
  protocol is not used in this section.  The determinant, geometry,
  cyclic-factor, endpoint-selection, and asymptotic results below depend
  only on the displayed ten-state kernel, the completed determinant, and
  the certificates recorded in \Cref{sec:appendix-computation}.
\end{remark}
...
\begin{definition}[Admissible cyclic covers]
  ...
  finite-\(m\) branch-selection check for \(4\le m\le32\); each is
...
canonical output-cocycle covers \(B_0\otimes I_\ell+B_1\otimes P_\ell\), and
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \item on \(2-\eta<s\le2\) the branch \(w(s)\) through \(w_0\) is
        the unique minimal-modulus root and is strictly decreasing;
  \item on \(|s|\le2-\eta\), every root has modulus at least
        \(\beta\).
\end{enumerate}
If \(m_0\) satisfies \(s_{m_0}=2\cos(\pi/m_0)>2-\eta\) and
\(w(s_{m_0})<\beta\), then for every \(m\ge m_0\) the
...
\begin{lemma}[Root separation]
  ...
  \(4\le m\le32\) are verified by exact algebraic interval comparison in
...
\(\texttt{endpoint\_interval\_certificate: true}\) verifies, on
  \(199/100<s\le2\), that this branch is the unique root of modulus
  \(<1/2\) and is strictly decreasing. The same certificate records
  \(w^*(2\cos(\pi/33))<167/500\).
...
For \(m\ge33\), \(s_m=2\cos(\pi/m)>199/100\).  Among the conjugates in
...
\(\texttt{finite\_m\_branch\_selection\_4\_to\_32: true}\) for the
remaining finite range \(4\le m\le32\).
\else
Define
\[
...
\(\texttt{finite\_m\_branch\_selection\_4\_to\_32: true}\); the
large-\(m\) asymptotic branch is used only from \(m\ge33\).
\else
