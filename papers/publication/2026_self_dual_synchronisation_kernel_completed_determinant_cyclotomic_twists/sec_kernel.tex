...
The Perron-root calculation is independent of the determinant
factorisation. Inspecting the transition table at \(u=1\), every row of
\(B=B(1)\) has sum \(3\), hence
\[
  B\mathbf 1=3\mathbf 1,\qquad
  \rho(B)\le \max_i\sum_j B_{ij}=\|B\|_\infty=3.
\]
Equivalently, \Cref{lem:row-sum-perron-bound} gives \(\rho(B)=3\).
The primitive Perron--Frobenius theorem then gives simplicity of the
eigenvalue \(3\) and excludes all other eigenvalues on the circle
\(\abs{\lambda}=3\). No assertion about the moduli of the roots of the
non-Perron factors is used here. Only after this row-sum argument do we
use the determinant factorisation. Setting \(u=1\) in
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \]
  and \(d_m:=|A_m|\).
  Then the normalized primitive real factor is, by definition, the root
  product
  \[
    P_m(w):=\prod_{\alpha\in A_m}\widehat\Delta(w,\alpha).
  \]
  Equivalently, and only in the following order, it is the Sylvester
  resultant
  \[
    P_m(w)=
...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  \]
  Under the standard Sylvester convention this root product is represented
  by the ordered resultant fixed in
