...
\begin{proposition}[Untwisted Perron root and residue constant]
  ...
  \(3\). Consequently the spectral radius is determined by the
  row-sum argument
  \[
...
can have modulus greater than \(3\). Hence \(\rho(B)=3\). Since \(B\)
is primitive, the Perron--Frobenius theorem gives that this eigenvalue
is simple and that no other eigenvalue lies on
\(\abs{\lambda}=3\).
...
\[
  \det(I-zB)=(z-1)(z+1)(3z-1)(z^3-z^2+z+1).
\]
This factorisation is not a proof of the spectral radius: the cubic
factor also has a reciprocal zero inside the unit circle. It is used
here for the residue computation
\[
  \begin{aligned}
  C_B
  &=\lim_{z\to1/3}
...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  convention the same polynomial is represented by the ordered
  resultant fixed in
