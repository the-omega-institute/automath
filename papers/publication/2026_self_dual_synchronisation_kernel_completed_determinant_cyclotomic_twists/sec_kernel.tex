...
deduced from the matrix itself before using the factorisation to compute
the residue. We do not infer it by comparing the reciprocal roots of
the displayed factors: the factor \(z^3-z^2+z+1\) has a zero inside the
unit disc, so that argument would be false. Instead, every row of the
non-negative matrix \(B=B(1)\) has sum \(3\), and hence
\[
  B\mathbf 1=3\mathbf 1,\qquad
