...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  \]
  This ordered definition is part of the theorem. It uses the standard
  Sylvester convention in the order fixed by
  \Cref{lem:ordered-cyclotomic-resultant}: the cyclotomic polynomial is
  the first argument, so the resultant is the root product itself. If
  the two resultant arguments are reversed, the product on the right is
  multiplied by \((-1)^{(\deg_s H)d_m}\).
  Assume further that
  \[
    \rho(B(e^{it}))<q
    \qquad (t\not\equiv0\pmod{2\pi}),
