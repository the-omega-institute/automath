...
factorisation is then immediate. The spectral radius is not determined
from reciprocal moduli of the factors; indeed
\[
  z^3-z^2+z+1
\]
has a zero inside the unit circle, so that shortcut would give the
wrong criterion. We instead use the non-negative matrix itself. Every
row of \(B=B(1)\) has sum \(3\), so
\[
...
slot. Reversing the two arguments would multiply this product by the
Sylvester sign \((-1)^{3d_m}\), since \(\deg_s\widehat\Delta=3\).
\end{proof}
...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  \]
  If the two resultant arguments are reversed, the product on the
  right is multiplied by \((-1)^{(\deg_s H)d_m}\).
  Assume further that
  \[
    \rho(B(e^{it}))<q
    \qquad (t\not\equiv0\pmod{2\pi}),
...
displayed product formula for \(R_m\) follows from the standard
Sylvester convention because
