...
factorisation is not a valid Perron argument; the factor \(3z-1\) is
not distinguished by such a reciprocal-modulus test. The Perron
conclusion above comes only from \(B\mathbf1=3\mathbf1\), the row-sum
bound, and primitivity. The factorisation is used here solely for the
residue computation
\[
  \begin{aligned}
  C_B
...
\begin{theorem}[Endpoint selection and asymptotic transfer]
  ...
  \[
    ...
    \rho_m=\rho(s_m).
  \]
  This conclusion is kernel-independent: after the displayed
  hypotheses have been verified for a completed determinant \(H\), the
  original labelled graph enters no further.
