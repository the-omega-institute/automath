...
lies outside the unit circle, \(\rho(B)=3\). The residue is the exact
...
\[
  x=w^2,\qquad y=sw
\]
identify the invariant function field birationally, and the same lemma
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  pairing conjugate characters and applying the character-dependent
  change of variables \(w=zr_a\), give the completed, normalized
...
small values at \(100\)-bit precision, then rounding each displayed
decimal to \(12\) digits after the decimal point and each displayed
absolute error to three significant figures, gives the table below.
