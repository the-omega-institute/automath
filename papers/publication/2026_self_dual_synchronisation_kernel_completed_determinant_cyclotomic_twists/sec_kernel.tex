...
\[
  ...
  \qquad d_m=|A_m|.
\]
Thus \(R_m\) denotes the root product above; the resultant is only this
ordered representation of it. Since \(\deg_s\widehat\Delta=3\), the
reversed-order identity in
...
\begin{theorem}[Admissible-cover endpoint selection and asymptotic transfer]
  ...
  auxiliary cover.
  This product is the definition of \(R_m\). With the convention of
  \Cref{lem:ordered-cyclotomic-resultant},
  \[
    R_m(w)=\Res_s\bigl(\Phi_m^+(s),H(w,s)\bigr).
  \]
  If the two resultant arguments are reversed, the product above is
...
create or remove factors in \(R_m\). With the convention of
\Cref{lem:ordered-cyclotomic-resultant}, the displayed product formula
for \(R_m\) is the corresponding ordered resultant, and the reversed
order has the sign \((-1)^{(\deg_s H)d_m}\).
...
\begin{proof}
Since \(\Phi_2^+(s)=s\), the convention of
\Cref{lem:ordered-cyclotomic-resultant} gives
\[
