...
\ifshortcicm
This subsection is the software-facing protocol example.  It is included
only to specify the exact integer interface used when the kernel is
embedded in a larger audit pipeline; no determinant proof below depends
on this protocol.
...
The operations
\(\operatorname{NF}_\infty,\operatorname{Val},\operatorname{Add},
\operatorname{Mul},\Phi_0,\Phi_1\) defined above are total on their
stated input grammars.  If the printed raw inputs have at most \(n+1\)
...
\begin{definition}[Admissible cyclic covers]
  ...
  \[
    \widetilde B_q=B_0\otimes I_q+B_1\otimes P_q.
  \]
  The paper-local certificate data determine this category: the
  matrices \(B_0\) and \(B_1\) are read from the printed transition
  table and from the displayed matrix \(B(u)\) in
  \Cref{sec:appendix-computation}, and the determinant, self-duality,
  endpoint-separation, and finite-\(m\) branch-selection checks are
  the named outputs of
  \(\texttt{certificates/verify\_certificates.py}\).  Concrete
...
\begin{theorem}[Endpoint selection and asymptotic transfer]
  \label{thm:self-dual-endpoint-selection}
\ifshortcicm
Let \(H(w,s)\in\ZZ[s][w]\) be the completed determinant of a primitive
self-dual labelled kernel with row-sum Perron root \(q\).  For
\[
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \item on \(2-\eta\le s\le2\) the branch \(w(s)\) through \(w_0\) is
        the unique root of modulus \(<q^{-1}+\varepsilon\) and is
        strictly decreasing;
...
\begin{corollary}[Verified-triple-relative endpoint selection]
...
\begin{theorem}[Verified-triple-relative twisted Perron-root hierarchy]
