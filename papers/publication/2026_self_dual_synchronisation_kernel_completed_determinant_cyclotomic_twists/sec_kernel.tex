...
\begin{proof}
The polynomial has degree \(6\) in \(w\), so the Galois group \(G\)
is a transitive subgroup of \(S_6\). We identify \(G\) in four steps.
...
\begin{remark}
The asymptotic expansion is already numerically accurate for moderate values of \(m\). Each tabulated value \(\rho_m\) below is obtained by solving the specialised resultant \(R_m(w)=0\) (equivalently \(\widehat\Delta(w,s_m)=0\)) for the minimal-modulus root using the SageMath script recorded in \Cref{sec:appendix-computation}.
values of \(m\). Solving the specialised equations \(R_m(w)=0\) for the
minimal-modulus root and comparing with the truncation of
