...
then immediate. We do not infer the spectral radius from the reciprocal
moduli of the displayed factors; the cubic factor also has a reciprocal
outside the unit circle. Instead the spectral radius is determined
directly from Perron--Frobenius and the row sums. Every row of
\(B=B(1)\) has sum \(3\), so
\[
  B\mathbf 1=3\mathbf 1,\qquad \|B\|_\infty=3.
\]
Thus \(3\) is an eigenvalue and every eigenvalue \(\lambda\) satisfies
\(\abs{\lambda}\le \|B\|_\infty=3\). Since \(B\) is primitive,
Perron--Frobenius makes the eigenvalue of modulus \(3\) simple and
positive, hence \(\rho(B)=3\). The residue is the exact one-line
computation
\[
  \begin{aligned}
  C_B
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \]
  With the standard Sylvester resultant convention used throughout this
  paper,
  \[
...
Fourier blocks is the signed resultant
\((-1)^{3d_m}\Res_s(\widehat\Delta(w,s),\Phi_m^+(s))\). The sign is a
unit and has no effect on the zero set used below.
\end{proof}
...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  \]
  Then \(H(w,s)\in\ZZ[s][w]\). Let \(D_s:=\deg_s H\). Assume further that
  \[
    \rho(B(e^{it}))<q
    \qquad (t\not\equiv0\pmod{2\pi}),
  \]
  ...
  \(m\) with \(s_m=2\cos(\pi/m)>2-\eta\) the normalized real
  cyclotomic resultant
  \[
    R_m(w)=\Res_s\bigl(H(w,s),\Phi_m^+(s)\bigr)
  \]
  has its minimal-modulus roots exactly in the factors \(H(w,s_m)\) and,
...
\begin{proof}
The inclusion \(H\in\ZZ[s][w]\) is exactly
\Cref{lem:self-dual-compression}. For \(\alpha=2\cos\theta\in[-2,2]\)
...
minimal-modulus roots of the product
\[
  (-1)^{D_s d_m}R_m(w)=\prod_{\alpha}H(w,\alpha),
  \qquad d_m:=\deg\Phi_m^+,
\]
...
\]
All later uses of \(R_m\) depend only on this signed product's zero set
and degree.
...
\begin{proof}
Since \(\Phi_2^+(s)=s\) and \(\deg_s\widehat\Delta=3\), the Sylvester
resultant gives
\[
  R_2(w)=\Res_s(\widehat\Delta(w,s),s)=-\widehat\Delta(w,0).
\]
