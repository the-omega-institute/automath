...
Set \(u=1\) in \Cref{prop:kernel-delta-explicit}. The displayed
factorisation is then immediate. To identify the Perron root we do not
compare reciprocal moduli of the displayed factors. We use the
non-negative matrix itself: every row of \(B=B(1)\) has sum \(3\), so
\[
  B\mathbf 1=3\mathbf 1,\qquad
  \rho(B)\le \max_i\sum_j B_{ij}=\|B\|_\infty=3.
\]
The first identity shows that \(3\) is an eigenvalue; the row-sum bound
shows that no eigenvalue has larger modulus. Therefore
\(\rho(B)=3\). Since \(B\) is primitive, Perron--Frobenius also gives
that this Perron eigenvalue is simple and is the unique eigenvalue of
modulus \(3\). The residue is the exact one-line computation
\[
  \begin{aligned}
  C_B
  &=\lim_{z\to1/3}
...
The birational quotient identified at the start of the proof is
therefore represented by this nonsingular projective quartic, which is
the smooth projective model of the quotient.
\end{proof}
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \[
    P_m(w):=\prod_{\alpha\in A_m}\widehat\Delta(w,\alpha).
  \]
  We use the standard Sylvester resultant convention
  \[
    \Res_s(f,g)=a_f^{\deg g}\prod_{f(\alpha)=0}g(\alpha),
    \qquad f(s)=a_f\prod_{f(\alpha)=0}(s-\alpha).
  \]
  With this convention, and with the real cyclotomic factor placed in
  the first slot,
  \[
...
With the convention in the statement, a monic polynomial
\(f(s)=\prod_i(s-\alpha_i)\) satisfies
\(\Res_s(f,g)=\prod_i g(\alpha_i)\). By definition,
\[
  \Phi_m^+(s)=\prod_{\alpha\in A_m}(s-\alpha),
\]
so the product formula for the Sylvester resultant gives
\[
  \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
  =
  ...
  \qquad d_m=|A_m|.
\]
Thus the real primitive factor obtained from the paired primitive
Fourier blocks is exactly the resultant with \(\Phi_m^+\) in the first
slot. In the reversed order the leading coefficient of
\(\widehat\Delta(w,s)\) as a polynomial in \(s\) is \(w^5\), while
\(\Phi_m^+\) is monic of degree \(d_m\). The general symmetry law for
resultants gives
\[
  \Res_s\bigl(\widehat\Delta(w,s),\Phi_m^+(s)\bigr)
  =(-1)^{3d_m}
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr),
\]
because \(\deg_s\widehat\Delta=3\). This proves the displayed
reversed-order sign formula.
\end{proof}

\subsection{A general endpoint-selection theorem}

\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  \label{thm:self-dual-endpoint-selection}
  Let \(B(u)=B_0+uB_1\) be a finite non-negative
  output-labelled kernel with \(B(1)\mathbf 1=q\mathbf 1\), and assume
  \(B(1)\) is primitive. Suppose there is a permutation matrix \(\Pi\)
  with
  \[
  ...
  \]
  Then \(H(w,s)\in\ZZ[s][w]\) and \(H(w,-s)=H(-w,s)\). For \(m\ge2\)
  set
  \[
    A_m=\Bigl\{2\cos\!\tfrac{\pi a}{m}:
      a\in(\ZZ/2m\ZZ)^\times/\{\pm1\}\Bigr\},
  ...
  \[
    \Phi_m^+(s):=\prod_{\alpha\in A_m}(s-\alpha),
  \]
  and define the normalized real cyclotomic resultant by the ordered
  Sylvester resultant
  \[
    R_m(w):=\Res_s\bigl(\Phi_m^+(s),H(w,s)\bigr)
       =\prod_{\alpha\in A_m}H(w,\alpha).
  \]
  This ordered definition is part of the theorem. If the two resultant
  arguments are reversed, the product on the right is multiplied by
  \((-1)^{(\deg_s H)d_m}\).
  Assume further that
  \[
    \rho(B(e^{it}))<q
  ...
  satisfies
  \[
    \lambda(s)=w(s)<q^{-1}+\varepsilon
       \quad (2-\eta\le s\le2),
    \qquad
    \lambda(s)\ge q^{-1}+\varepsilon
       \quad (0\le s\le 2-\eta),
  \]
  and \(w(s)\) is strictly decreasing on \([2-\eta,2]\), then for every
  \(m\) with \(s_m=2\cos(\pi/m)>2-\eta\) the resultant \(R_m(w)\) has
  ...
  \[
    ...
    \rho_m=\rho(s_m).
  \]
  If the endpoint branch has a Taylor expansion to order \(N\), then
  substituting the Taylor expansion of \(2-2\cos(\pi/m)\) gives the
  corresponding asymptotic expansion of \(\rho_m\) through order
  \(m^{-2N}\).
\end{theorem}

\begin{proof}
The inclusion \(H\in\ZZ[s][w]\), together with the parity
\(H(w,-s)=H(-w,s)\), is exactly \Cref{lem:self-dual-compression}. We
use the standard Sylvester convention
\[
  \Res_s(f,g)=a_f^{\deg g}\prod_{f(\alpha)=0}g(\alpha),
  \qquad f(s)=a_f\prod_{f(\alpha)=0}(s-\alpha).
\]
The displayed product formula for \(R_m\) follows because
\(\Phi_m^+(s)=\prod_{\alpha\in A_m}(s-\alpha)\) is monic. The sign for
the reversed resultant is the identity
\[
...
gives \(\lambda(\alpha)\ge q^{-1}+\varepsilon>\lambda(s_m)\). Hence the
minimal-modulus roots of \(R_m(w)=\prod_{\alpha\in A_m}H(w,\alpha)\)
come exactly from the endpoint factor(s). The final assertion follows
by formal substitution of the Taylor series.
\end{proof}
...
define
\[
  R_m(w):=\Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)\in\ZZ[w].
\]
...
primitive character. With the Sylvester resultant convention, the exact
relation between \(R_m\) and the normalized primitive real factor is
\[
  R_m(w)=
    \prod_{\alpha\in A_m}\widehat\Delta(w,\alpha),
...
product formula for the Sylvester resultant \cite{CoxLittleOShea2005}
gives
\[
  R_m(w)
  =\Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
...
\(\lambda(-s)=\lambda(s)\) for all \(s\). By the Sylvester-resultant
product formula,
\[
  R_m(w)=
    \prod_{\alpha\in A_m}\widehat\Delta(w,\alpha),
...
taken to be \(\eta=1/100\) and \(\varepsilon=1/200\).
...
\begin{proof}
By \Cref{lem:root-separation}, \(\rho_m=\rho(s_m)\) for every
\(m\ge32\), and this is the range needed for the asymptotic statement.
