...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \]
  Equivalently, with the ordered resultant convention of
  \Cref{lem:ordered-cyclotomic-resultant},
  \[
    R_m(w)=
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr),
  \]
  where \(\Phi_m^+\) is in the first argument.
...
\begin{theorem}[Admissible-cover endpoint selection and asymptotic transfer]
  ...
  auxiliary cover.
  This product is the definition of \(R_m\). Under the standard
  Sylvester convention the same polynomial is represented by the ordered
  resultant fixed in
  \Cref{lem:ordered-cyclotomic-resultant},
  \[
...
definition of \(R_m\), and
\Cref{lem:ordered-cyclotomic-resultant} represents it as
\[
...
primitive character. Equivalently,
\[
  R_m(w)=
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
    =
...
\[
  ...
  =
    \prod_{\alpha\in A_m}\widehat\Delta(w,\alpha).
\]
The second displayed expression equals the same product. The same lemma
also gives
\[
  \Res_s\bigl(\widehat\Delta(w,s),\Phi_m^+(s)\bigr)
  =(-1)^{3d_m}R_m(w)
