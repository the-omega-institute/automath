...
factorisation is then immediate. The spectral-radius statement is proved
from the non-negative matrix, not from a comparison of the reciprocal
roots of the displayed factors. Every row of \(B=B(1)\) has sum \(3\), so
\[
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \]
  With the ordered convention of
  \Cref{lem:ordered-cyclotomic-resultant}, namely with the real
  cyclotomic factor placed in the first slot,
  \[
...
slot. Since \(\deg_s\widehat\Delta=3\), the reversed-order identity in
...
\begin{theorem}[Endpoint asymptotics for completed self-dual determinants]
  ...
  \]
  This ordered definition is part of the theorem. It uses the convention
  of \Cref{lem:ordered-cyclotomic-resultant}; if the two resultant
...
\begin{proof}
  ...
  displayed product formula for \(R_m\), including the reversed-order
  sign, is exactly \Cref{lem:ordered-cyclotomic-resultant} with
  \(A=A_m\).
