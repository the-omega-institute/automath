...
modulus \(3\).
The displayed factorisation is not used to identify the Perron root;
it is used only after the row-sum proof of \(\rho(B)=3\), namely for
...
\begin{proposition}[Primitive real factors of the cyclic lifts]
  ...
  \[
    ...
    =P_m(w).
  \]
  This equation is a definition of the ordered elimination convention
  used below: the monic real cyclotomic polynomial is placed in the
  first argument. In the reversed order one has
  \[
...
Fourier blocks is exactly the ordered resultant with \(\Phi_m^+\) in
the first slot. The reversed-order resultant is a different polynomial
when \(3d_m\) is odd. Since \(\deg_s\widehat\Delta=3\), the
reversed-order identity in \Cref{lem:ordered-cyclotomic-resultant} gives
\[
  \Res_s\bigl(\widehat\Delta(w,s),\Phi_m^+(s)\bigr)
  =(-1)^{3d_m}
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr),
...
primitive character. With the Sylvester resultant convention, the
definition is the ordered identity
\[
  R_m(w)=
    \Res_s\bigl(\Phi_m^+(s),\widehat\Delta(w,s)\bigr)
    =
...
\[
  ...
  =(-1)^{3d_m}R_m(w),
\]
and this reversed-order resultant is not used as \(R_m(w)\).
