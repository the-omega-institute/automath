...
\end{tabularx}
\end{center}
The runner and the interval certificate record are the authoritative
transcripts for the computer-assisted steps.  Items \textup{(a)}--\textup{(f)} in
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \(\rho_m=\rho(2\cos(\pi/m))\) is proved for every \(m\ge33\), and on
