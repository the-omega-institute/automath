...
part of the determinant and cyclotomic theorem proved here.
...
displayed ten-state matrix.  External expression-layer and Richardson
questions are not part of these theorems; a conditional boundary
statement is recorded after the determinant results.
...
\end{enumerate}
Then the supplied terms provide a conditional EML realisation of the
Richardson fragment used by that external reduction.  Without the two
displayed real-domain identities for \(U47\) and \(U111\), and without
...
\(A=\operatorname{Abs}\) with the three real-domain identities needed
for the conditional translation.  The remaining arithmetic primitives
used inside such a translation have the following domain check.  In the
real-safe principal-branch EML system
\[
...
present paper uses it only through the conditional statement in
\Cref{cor:conditional-printed-name-richardson}.  The EML
representation language used there is external to the determinant
proof: the manuscript records the elementary
