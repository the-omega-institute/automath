...
The headline hierarchy is a typed interface statement.  It separates the
external expression layer, the Zeckendorf protocol layer, the restricted
same-level readout obstruction, and the finite-fibre cover layer before
the paper-local determinant surface is attached to the displayed
ten-state matrix.
...
\Cref{rem:kernel-import-ledger}.  Let \(\mathsf{ExtExpr}\) be an
...
covers over the fixed normal-form sets \(X_m\).  Then:
\begin{enumerate}[label=\textup{(\roman*)}]
  \item \(\mathsf{ExtExpr}\) gives \(L0\) expression universality and no
  equality, normal-form, or compiler data.
  \item \(Z\) gives an \(L1\) computable protocol for
  \((\NN,+,\times)\).
  \item For every verified \(\operatorname{Trip}(P,S,A)\), no
  computable semantic normalizer into finite decidable objects reflects
  equality on the translated Richardson fragment.
  ...
  \item There exists a computable object of \(\operatorname{Cover}(Z)\)
  with fixed-resolution finite-fibre certificates but no fixed
...
\]
on their principal-branch real domains.  Then
...
applies.  Without these two displayed real-domain identities for
\(U47\) and \(U111\), this manuscript proves no printed \(U47/U111\)
Richardson instance.
\else
Assume that finite principal-branch EML terms denoted \(U47\) and
...
Richardson fragment.  The last sentence is just the contrapositive
scope audit: this paper does not prove the two displayed identities, so
without them it has supplied no printed \(U47/U111\) instance.
\else
