...
products. For this kernel, explicit
...
SageMath~10.7; the endpoint recurrence was also cross-checked in
...
the paper sources and certificate script, and the verification summaries
used in the proofs, including
\begin{center}
\small
...
\end{tabularx}
\end{center}
Items \textup{(a)}--\textup{(f)} in \Cref{sec:appendix-computation}
record the command map, exact polynomials, interval constants, and
displayed coefficients used in the corresponding proofs; no fitted
parameters are used.
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \]
  and \(B(1)\mathbf1=3\mathbf1\), hence
  ...
  \item The degree-six morphism
  \(\overline X\to\mathbb P^1_s\) has twenty finite simple branch
  values, which form one Galois orbit, and two simple ramification
  points over infinity. The Galois action on the ten antipodal pairs of
  finite branch values is the full symmetric group \(S_{10}\).
\end{enumerate}
\end{theorem}
...
\begin{remark}[Scope of companion interfaces]
  \label{rem:expression-interface-boundary}
  Companion projects attach this finite kernel to expression-language
  and numeration questions.  No external elementary-function catalogue,
  Richardson-style translation, or Zeckendorf-type numeration protocol is
  used in the determinant or cyclotomic proofs of this article.  The
  present main results are exactly the two theorems above and their
  consequences for the displayed ten-state kernel.
\end{remark}
...
For finite automata and rational-series context, the closest standard
interfaces are weighted automata and rational series
\cite{Schutzenberger1961,BerstelReutenauer2011} and symbolic-dynamics
determinant formulas
\cite{BowenLanford1970Zeta,Manning1971Axiom,LindMarcus2021}.  The
certificate runner is included only as reproducibility support: it
verifies the printed determinant, smoothness, Galois, and endpoint
checks for the finite kernel, and it supplies no additional theorem
beyond those computations.
