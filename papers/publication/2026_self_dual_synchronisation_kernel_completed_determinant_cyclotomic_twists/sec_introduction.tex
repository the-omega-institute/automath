...
This article proves a verified-triple-relative hierarchy for the
explicit ten-state weighted synchronisation kernel displayed in
\Cref{def:sync-kernel}.  The kernel has
...
The scope is intentionally concrete but relative.  We do not classify
...
catalogue name yields an elementary-function obstruction.  The main
hierarchy is relative only to a verified triple internal to the paper:
the printed transition table, the completed-determinant identity, and
the effective cyclotomic endpoint-separation witnesses.  Every assertion
below is made for the displayed kernel, for this verified triple, or for
an explicitly stated auxiliary criterion that is then verified for this
kernel.  The point of the paper is that this one kernel admits a
complete audit trail: the transition table gives the matrix, the matrix
gives the determinant, the determinant gives the completed curve and the
cyclotomic factors, and the Sage runner
\[
...
These are the admissible covers used in the certificate claim: they are
...
intrinsic Fold\(_m\)-type behaviour of the displayed kernel and are not
part of the paper-local obstruction claim.
...
The headline hierarchy is a typed four-interface statement.  Its
assumptions and conclusions live separately in the partial
principal-branch \(\mathsf{EML}\) term algebra, the Zeckendorf
normal-form protocol, one-orbit ambient-product readouts, and computable
finite-fibre covers before the paper-local determinant surface is
attached to the displayed ten-state matrix.

\begin{theorem}[Typed verified-triple-relative four-interface hierarchy]
\label{thm:typed-verified-core-hierarchy}
Let \(\mathsf{EML}\) be the partial principal-branch term algebra used in
the external \(L0\) ledger recorded in
\Cref{rem:kernel-import-ledger}.  Let \(\mathsf{B36Ext}\) be an
external expression-layer statement asserting that each primitive in
the ambient source calculus has at least one \(\mathsf{EML}\)
representative on its stated source domain.  Let
\(\operatorname{Trip}(P,S,A)\) mean finite pure \(\mathsf{EML}\) terms
satisfying
\[
  P=\pi,\qquad S(x)=\sin x\quad(x\in\RR),\qquad
  A(x)=\abs{x}\quad(x\in\RR\setminus\{0\}).
\]
Let \(Z\) be the Zeckendorf normal-form protocol with
\[
  \operatorname{Val}:X_{\mathrm{fin}}\to\NN,\qquad
  \operatorname{NF}_{\infty},
\]
and transported operations
\[
  ...
  \operatorname{Mul}(c,d)
    =\operatorname{NF}_{\infty}(\operatorname{Val}(c)\operatorname{Val}(d)).
\]
Let \(\operatorname{Orbit}(g)\) be the category of
one-free-monogenic-orbit ambient-product readouts.  Let
\(\operatorname{Cover}(Z)\) be the category of computable finite-fibre
covers over the fixed normal-form sets \(X_m\); in the determinant
application this means the admissible cyclic covers of
\Cref{def:admissible-cyclic-cover}, not arbitrary auxiliary covers.
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
  \item \(\mathsf{B36Ext}\) gives \(L0\) expression universality and no
  equality, normal-form, or compiler data.
  \item \(Z\) gives an \(L1\) computable protocol for
  \((\NN,+,\times)\).
  \item For every verified \(\operatorname{Trip}(P,S,A)\), and for every
  effective Richardson-to-\(\mathsf{EML}\) translation preserving the
  stated real branch-domain semantics, no computable semantic normalizer
  into finite decidable objects reflects equality on the translated
  Richardson fragment.
  \item No faithful same-level ambient-product readout of multiplication
  on all of \(\NN_{\ge1}\) exists inside \(\operatorname{Orbit}(g)\).
  \item In the admissible finite-fibre cover category there exists a
  computable object with fixed-resolution certificates but no fixed
  finite-state transfer formula; arbitrary one-spike or otherwise
  auxiliary covers are only stress tests and are not intrinsic
  \(\operatorname{Fold}_m\) behaviour.
\end{enumerate}
The paper-local ten-state kernel below supplies the finite-state
completed-determinant instance of this typed separation; it does not by
itself instantiate any external expression-layer or Richardson object.
\end{theorem}

We next record the determinant, spectral radius, completion, quotient,
and generic Galois group for the ten-state matrix, all as paper-local
consequences of the verified transition-table, determinant, and
certificate data.
...
\begin{theorem}[Verified-triple-relative hierarchy: geometry]
...
The second part of the verified-triple-relative hierarchy concerns
cyclotomic twists.

\begin{theorem}[Verified-triple-relative hierarchy: cyclotomic twists]
...
\begin{corollary}[Conditional printed Richardson branch]
\label{cor:conditional-printed-name-richardson}
\ifshortcicm
Assume that finite principal-branch EML terms denoted \(U47\) and
\(U111\) have been supplied, and that the following real-domain
identities have been proved independently:
\[
  U47=\pi,\qquad U111(x)=\sin x\quad(x\in\RR).
\]
Let \(\operatorname{Abs}(u)=G(M(u,u))\), where the supplied EML terms
\(G\) and \(M\) satisfy
\[
  \operatorname{Abs}(u)=\abs{u}\qquad(u\ne0)
\]
on their principal-branch real domains.  Assume also that the relevant
Richardson expressions are translated effectively to partial
\(\mathsf{EML}\) terms with the same values on their stated real
branch domains.  Then \((U47,U111,\operatorname{Abs})\) satisfies
\(\operatorname{Trip}(P,S,A)\), so the verified Richardson-fragment
part of \Cref{thm:typed-verified-core-hierarchy} applies.  Without
these two displayed real-domain identities for \(U47\) and \(U111\) and
the effective branch-domain translation, this manuscript proves no
printed \(U47/U111\) Richardson instance.
\else
Assume that finite principal-branch EML terms denoted \(U47\) and
\(U111\) have been supplied, and that the following two real-domain
identities have been proved independently:
\[
  U47=\pi,\qquad U111(x)=\sin x\quad(x\in\RR).
\]
...
\[
  \operatorname{Abs}(u)=\abs{u}\qquad(u\ne0)
\]
on their principal-branch real domains.  Suppose also that:
\begin{enumerate}[label=\textup{(\alph*)}]
  \item \(U47\), \(U111\), \(G\), and \(M\) are finite terms of the
  stated principal-branch EML system on the displayed real domains; and
  \item the corresponding Richardson translation is effective: for each
  Richardson expression \(E\) used in the reduction it produces a
  partial EML term \(\tau(E)\) such that
  \(\lbrack\!\lbrack\tau(E)\rbrack\!\rbrack(x)=E(x)\) for every
  \(x\in D_E\), where \(D_E\) is the stated real branch domain.
\end{enumerate}
Then \((U47,U111,\operatorname{Abs})\) satisfies
\(\operatorname{Trip}(P,S,A)\), so the verified
Richardson-fragment part of \Cref{thm:typed-verified-core-hierarchy}
applies.  Without the two displayed real-domain identities for \(U47\)
and \(U111\), this manuscript proves no printed \(U47/U111\)
Richardson instance.
\fi
\end{corollary}
...
\begin{proof}
\ifshortcicm
The definition of \(\operatorname{Trip}(P,S,A)\) in
\Cref{thm:typed-verified-core-hierarchy} requires exactly a
\(\pi\)-term, a sine term on all real inputs, and an absolute-value term
away from \(0\).  The displayed hypotheses give those three terms with
\(P=U47\), \(S=U111\), and \(A=\operatorname{Abs}\).  Thus the triple
satisfies \(\operatorname{Trip}(P,S,A)\), and item~\textup{(iii)} of
\Cref{thm:typed-verified-core-hierarchy} applies to the translated
Richardson fragment because the corollary assumes the required
effective branch-domain translation.  The last sentence is just the
contrapositive scope audit: this paper does not prove the two displayed
identities or the translation hypothesis, so without them it has
supplied no printed \(U47/U111\) instance.
\else
The displayed hypotheses give \(P=U47\), \(S=U111\), and
\(A=\operatorname{Abs}\) with the three real-domain identities required
in the definition of \(\operatorname{Trip}(P,S,A)\) in
\Cref{thm:typed-verified-core-hierarchy}.  Hence
\((U47,U111,\operatorname{Abs})\) satisfies
\(\operatorname{Trip}(P,S,A)\), and item~\textup{(iii)} of
\Cref{thm:typed-verified-core-hierarchy} applies to the effective
Richardson translation.  The remaining arithmetic primitives used
inside such a translation have the following domain audit.  In the
real-safe
principal-branch EML system
\[
  \operatorname{eml}(x,y)=\exp(x)-\Log(y),
\]
all logarithm arguments displayed below are positive on the stated
domains.  Put
\[
  E(u)=\operatorname{eml}(u,1)=e^u .
\]
For \(u>0\),
\[
  \operatorname{eml}(1,u)=e-\log u,\qquad
  E(\operatorname{eml}(1,u))=\exp(e-\log u)=e^e/u>0,
\]
and hence
\[
  L(u)=\operatorname{eml}(1,E(\operatorname{eml}(1,u)))
      =e-\Log(e^e/u)=\log u .
\]
For \(a>0\) and real \(b\),
\[
  D(a,b)=\operatorname{eml}(L(a),E(b))
        =\exp(\log a)-\Log(e^b)=a-b .
\]
Thus, for real \(u\),
\[
  P(u)=\operatorname{eml}(u,E(u))=e^u-u>0,\qquad
  N(u)=D(P(u),E(u))=-u,
\]
and for real \(u,v\),
\[
  A(u,v)=D(E(u),D(P(u),v))=u+v,\qquad
  S(u,v)=A(u,N(v))=u-v .
\]
These identities justify the internal addition, subtraction, and
negation primitives needed after an effective Richardson-to-EML
translation has been supplied externally.  They do not themselves prove
the displayed \(U47=\pi\) or \(U111=\sin\) identities.  If either
displayed identity or the effective translation is absent, this
corollary supplies no printed \(U47/U111\) Richardson instance.
\fi
\end{proof}
...
present paper uses it only through the conditional translation boundary
in \Cref{cor:conditional-printed-name-richardson}.  The EML
representation language used in that boundary is treated here as a
paper-external interface: the manuscript records the elementary
principal-branch arithmetic identities above, but it does not import a
catalogue-wide EML representation theorem or a semantic equality
procedure.  The \(L1\) numeration protocol is likewise separated from
the determinant proof: Zeckendorf's uniqueness theorem
\cite{Zeckendorf1972} and efficient Zeckendorf arithmetic
\cite{AhlbachUsatineFrougnyPippenger2013} explain the numeration
interface, while \Cref{sec:completion-twists} depends only on the
...
the certificate runner belongs to the CICM-style ecosystem of
formalization and software artifacts \cite{CICM2024}; it is a
