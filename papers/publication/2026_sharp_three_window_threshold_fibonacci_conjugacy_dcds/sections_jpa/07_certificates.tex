...
Assume that the depth-$(m+n-1)$ atom partition for the blurred readout has
total interval complexity at most $m+n$, so its empirical raw-word law differs
from its limiting raw-word law in total variation by at most
...
increment used here is the interval-partition hypothesis: the same certificate
holds whenever the blurred depth atoms have the same total interval-complexity
discrepancy input, even if the blurred readout is not itself a two-interval
coding.
\end{remark}
