...
Assume that the depth-$(m+n-1)$ atoms of the blurred readout obey the same
interval discrepancy estimate used in Theorem~\ref{thm:tv-certificate}; this
includes, but is not limited to, the case in which $R_\rho$ remains a
two-interval coding of the original irrational rotation. Then, for the
empirical blurred stabilized law $\hat{\eta}_{m,n,N}^{(\rho)}$,
\[
...
form above records the local increment needed here: the same certificate is
valid for any blurred atom partition with the identical interval-discrepancy
input.
\end{remark}

\begin{proof}
Combine the assumed discrepancy estimate for the blurred empirical law with
