...
The local fibers become a dynamical problem only after overlap is imposed. We construct the labeled overlap graph, prove that the sharp injectivity threshold is $m=3$, classify the exceptional branch at $m=2$, and then pass from finite blocks to a two-sided finite-memory conjugacy. The terminology for sofic shifts and labeled graph presentations is standard; see \cite{LindMarcus1995,Kitchens1998,Nasu1995}.
...
\begin{enumerate}[label=(\roman*),leftmargin=*,itemsep=2pt]
  \item The shift $Y_m$ is the label shift presented by $\mathcal G_m$. In particular, $Y_m$ is a sofic shift.
  \item The labeled graph $\mathcal G_m$ is resolving in both time directions: for each fixed vertex $u$, the two outgoing edges corresponding to $c=0$ and $c=1$ have distinct labels, and for each fixed terminal vertex the two incoming edges also have distinct labels.
...
\]
defines a unique bi-infinite binary sequence $a$ whose length-$(m-1)$ windows are the $u_t$. The corresponding label sequence is exactly $\Phi_m(a)$. This proves that $Y_m$ is the label shift of $\mathcal G_m$.
...
\]
Since $0<F_{m+1}<F_{m+2}$, the two residues modulo $F_{m+2}$ are distinct. By Proposition~\ref{prop:finite-window-range}, distinct residues correspond to distinct labels in $X_m$, and the two outgoing labels are distinct.
...
Since $m\ge 4$, one has $F_m\ge 3$ and $F_m+2<F_m+F_{m+1}=M$. Hence the sets
\[
\{0,1,2\}
\qquad\text{and}\qquad
\{F_m,F_m+1,F_m+2\}
\]
are disjoint in $\{0,\dots,M-1\}$. With residues represented in this interval, $q_t\in\{0,1,2\}$ is equivalent to $a_{t+m+1}=0$, and $q_t\in\{F_m,F_m+1,F_m+2\}$ is equivalent to $a_{t+m+1}=1$. In either case the remaining offset gives $s_t=a_t+a_{t+1}$. Thus each $q_t$ uniquely determines both
\[
a_{t+m+1}\in\{0,1\}
\qquad\text{and}\qquad
...
\Fold_3(110)=001.
\]
The two raw blocks $0001$ and $0110$ therefore have the same stabilized output pair.
\end{proof}
...
If $y\neq (00)^n$, choose $t$ with $y_t\in\{01,10\}$. Then $y_t$ determines the pair $(a_t,a_{t+1})$ exactly. For $s=t+1,\dots,n-1$, the symbol $y_s$ determines $a_{s+1}$ from $a_s$: if $y_s=00$ then $a_{s+1}=a_s$, if $y_s=01$ then $(a_s,a_{s+1})=(0,1)$, and if $y_s=10$ then $(a_s,a_{s+1})=(1,0)$. The same rule propagates uniquely backward for $s=t-1,\dots,0$. It follows that the entire raw block is uniquely determined, so every other fiber is a singleton.

The bi-infinite statement is the same. If $y=(\ldots,00,00,\ldots)$, then every adjacent raw pair is constant, so the only preimages are $0^{\mathbb Z}$ and $1^{\mathbb Z}$. Otherwise choose $t$ with $y_t\in\{01,10\}$ and propagate the reconstruction uniquely in both time directions.
\end{proof}
...
\end{aligned}
\end{equation}
Here terms with $p_i=0$ are interpreted as $0$. If $q>0$ and $s=0$, both the raw and pushed-forward divergences are infinite, and no finite defect identity is asserted.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:m2-branch-locus}, every conditional fiber of $\Phi_{2,n}$ is a singleton except the branch fiber over $(00)^n$, which is exactly $\{0^{n+1},1^{n+1}\}$. Hence $H_P(A\mid Y=y)=0$ for $y\neq (00)^n$, while on the branch fiber the conditional law is
\[
...
\]
In particular, if $\varepsilon_n$ decays exponentially, then the conditional entropy loss of the two-window stabilized readout decays exponentially for this family.
\end{corollary}
...
\]
Since $\Phi_2$ is continuous, this restricted map is Borel, and its inverse on the image is Borel by the Lusin--Souslin theorem for injective Borel maps between Polish spaces \cite[Theorem~15.1]{Kechris1995}. Both the branch set and its image have measure zero under $\nu$ and $\eta$, respectively. The restriction of $\Phi_2$ therefore gives an invertible, shift-equivariant correspondence modulo null sets, which is precisely a measure-theoretic isomorphism.
\end{proof}
...
The conditions ``$w_t\in\mathcal L_m(Y_m)$'' and \eqref{eq:decoded-overlap} depend only on the length-$(m+1)$ block $(y_t,\dots,y_{t+m})$. Since there are only finitely many such blocks, they define a finite forbidden list. It follows that $Y_m$ is a subshift of finite type of memory at most $m$.
\end{proof}
...
\begin{proof}
The subcritical statement is precisely the path-space part of
Theorem~\ref{thm:m2-branch-locus}.  That theorem proves both that
$0^{\mathbb Z}$ and $1^{\mathbb Z}$ map to the constant $00$ output and that
no other output point has more than one preimage.  Hence the $m=2$ failure is a
single branch pair, and since both preimages are fixed points this is a single
shift-orbit pair.

For $m\ge3$, Theorem~\ref{thm:two-sided-conjugacy}(i) gives injectivity,
continuity, shift equivariance and surjectivity onto $Y_m$, hence a
topological conjugacy.  The local inverse with memory $0$ and anticipation
$m-1$ is Theorem~\ref{thm:two-sided-conjugacy}(ii), equivalently
Corollary~\ref{cor:explicit-decoder}.  The finite-type assertion is
Theorem~\ref{thm:two-sided-conjugacy}(iii).  These two alternatives exhaust all
integers $m\ge2$: the only subthreshold value is $m=2$, while every $m\ge3$ is
in the conjugacy regime.  Thus there is no remaining value of $m$ at which a
nontrivial finite-to-one, non-conjugate, or non-SFT intermediate degeneration
can occur.
\end{proof}
...
In the terminology of \cite{LindMarcus1995}, $\mathcal G_m$ is a right-resolving presentation of the sofic shift $Y_m$, and $\Phi_m$ has degree at most $2^{m-1}$ by Theorem~\ref{thm:finite-state-lift}(iii). A right-resolving factor of degree~1 between irreducible shifts of finite type is a conjugacy \cite[Theorem~8.1.16]{LindMarcus1995}; see also \cite{Ashley1991} for the equal-entropy resolving theory and \cite{Boyle1983} for the lower-entropy obstruction. The threshold theorem says that degree~1 is achieved exactly at $m=3$. This is an arithmetic fact, not a formal consequence of the sofic framework, and it uses the explicit decoder of Lemma~\ref{lem:m3-decoder}.
\end{remark}
