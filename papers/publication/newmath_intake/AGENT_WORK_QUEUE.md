...
| `bedc_finite_kernel_calculus` | Refine exact theorem-spine summaries; maintain `current_declaration_map.md`; keep `groundcompiler_placement_decision.md`, `blocker_ledger.md`, `bibliography_scope_seed.md`, and `short_note_route_memo.md` aligned with the theorem spine; draft source-update note shell without changing source commit | Source-side work in `D:/omega/newmath`; later journal/workshop promotion | Stop before editing Lean/source files, claiming a packaging theorem exists, or choosing the short-note route for the human |
