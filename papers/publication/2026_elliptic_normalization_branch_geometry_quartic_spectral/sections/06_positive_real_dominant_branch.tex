...
=(y-x^2)^2+y+x^3-x^2-x.
\]
The function $h(x)=x^3-x^2-x$ has derivative
\[
h'(x)=3x^2-2x-1=(3x+1)(x-1),
\]
so on $x\ge0$ its unique minimum occurs at $x=1$, with $h(1)=-1$.
Therefore $y+h(x)>0$ for every $y>1$.  Hence $\Pi(-x,y)>0$ for all $x\ge0$.
\end{proof}
...
roots for every $y>1$.  Lemma~\ref{lem:ygt1-no-negative-roots} excludes negative real roots.
