...
Let $y_0=(17+\sqrt{297})/2$, the positive root of $y^2-17y-2$.  The primitive rational Sturm chain for \(\Pi(\lambda,y)\), displayed in Proposition~\ref{prop:positive-axis-sturm-chain}, has sign-controlling factors
...
\begin{proof}
The primitive rational chain is the one displayed in Appendix~\ref{app:positive-axis-sturm}; its only positive denominator wall is the positive root \(y_0\) of the corrected factor \(y^2-17y-2\).  The signs of the primitive chain at \(\pm\infty\) on the three open positive cells are exactly those in \eqref{eq:positive-axis-sturm-sign-table-main}.  Since \(y_0\ne1\) and the cubic factor in the discriminant
\[
\Disc_\lambda\Pi=-y(y-1)(256y^3+411y^2+165y+32)
\]
...
$\Pi(\cdot,y)$.  Lemma~\ref{lem:positive-axis-sturm-count} gives
...
\[
\Omega_U=Q_U\setminus\bigcup_{\nu_j^{(0)}<U}\operatorname{int}D_j.
\]
After a harmless arbitrarily small enlargement of $Q_U$ if a zero of $G_m$ lies on $\Re u=0$, the boundary of \(\Omega_U\) is piecewise smooth and contains no zero of \(G_m\).  The set consisting of \(\Omega_U\) and the finitely many circle boundaries is compact, and $G_m$ has no zeros there; hence \(|G_m|\ge c_U>0\) on that set for all sufficiently large $m$.  The uniform perturbation in \eqref{eq:quartic-corollary-cosine} is then smaller than $c_U$ on the same compact set.  Rouch\'e's theorem on each \(D_j\) gives exactly one zero of \(F_m\) in \(D_j\), with multiplicity one.  Applying the argument-principle form of Rouch\'e's theorem to the multiply connected domain \(\Omega_U\) gives the same number of zeros for \(F_m\) and \(G_m\) in \(\Omega_U\), namely none.  Therefore all zeros in $Q_U$ are the unique zeros in the discs.

The polynomial $Z_m(y)$ has real coefficients, so the function \(u\mapsto Z_m(-u^2/m^2)\), and hence also \(F_m\), satisfies \(F_m(\overline u)=\overline{F_m(u)}\).  Since each $D_j$ is invariant under conjugation and contains exactly one zero, that zero is fixed by conjugation and hence real.  The zeros are simple because the local Rouch\'e count preserves the multiplicity-one count of the shifted cosine zeros.  At every counted zero one has $u>0$, so the map $u\mapsto y=-u^2/m^2$ is locally biholomorphic there, with $dy/du=-2u/m^2\ne0$; the conjugate zero at $-u$ of the even function $Z_m(-u^2/m^2)$ represents the same $y$-zero and lies outside the chosen positive $u$-rectangle.
