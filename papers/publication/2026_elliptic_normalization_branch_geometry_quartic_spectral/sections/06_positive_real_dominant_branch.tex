...
$\Pi(\cdot,y)$.  The Sturm variation table in \eqref{eq:positive-axis-sturm-sign-table-main} gives
...
\end{equation}
\item Let $y_0=(17+\sqrt{297})/2$, the positive root of $y^2-17y-2$.  The Sturm divisions may be read only on intervals avoiding the denominator wall $y_0$; on those intervals the sign-controlling factors are
\begin{equation}\label{eq:positive-axis-sturm-sign-factors}
y,\quad y-1,\quad 256y^3+411y^2+165y+32,\quad y^2-17y-2.
\end{equation}
\phantomsection\label{eq:positive-axis-sturm-initial-terms}
The real-root count is constant on each component of $(0,\infty)\setminus\{1,y_0\}$.  Since $\Disc_\lambda\Pi(y_0)\ne0$, the count extends by continuity across $y_0$.  Evaluating the corrected primitive Sturm chain of Proposition~\ref{prop:positive-axis-sturm-chain} gives the variation table
...
obtained from $\Pi(\lambda,y)=0$, so $p$ and $q$ are roots of $\Pi(\cdot,y)$.  The Sturm
variation table in \eqref{eq:positive-axis-sturm-sign-table-main} gives exactly two real
roots for every $y>1$, hence Lemma~\ref{lem:ygt1-no-negative-roots} excludes any negative real root.
...
\end{lemma}
\begin{proof}
Away from $s=0$ the displayed residue formula is the usual simple-root partial-fraction coefficient.  Appendix~\ref{app:endpoint-branching}, especially Lemmas~\ref{lem:endpoint-coefficient-computation} and~\ref{lem:plus-branch-residue-cancellation}, computes the Laurent numerator and denominator along the colliding branches and shows that their common linear zero cancels.  The cancellation identity is
\[
2A\left(A n_1+\frac54\right)-5A^2=\frac12,
\qquad A=\frac1{\sqrt2},\quad n_1=\frac{5A}{2},
\]
and the next coefficient gives the odd term $3\sqrt2/16$.  Thus the singularity at $s=0$ is removable and \eqref{eq:endpoint-amplitude-removable-main} holds.  The nonvanishing radius follows from continuity of the holomorphic extensions and the value $C_\pm(0)=1/2$.
\end{proof}
...
\]
be the real simple zeros of the shifted cosine $\cos((a-B_*/(A_*m))u)$.  Since $U$ is not a limiting threshold, choose $\eta>0$ so that, for all sufficiently large $m$, the closed discs
\[
D_j=\{u\in\CC:|u-\nu_j^{(m)}|\le\eta\},
\qquad \nu_j^{(0)}<U,
\]
are pairwise disjoint, conjugation-invariant, contained in $Q_U$, and avoid $\partial Q_U$.  On the complement of their interiors in $Q_U$, together with the disc boundaries, the shifted cosine is bounded below by a positive constant.  The uniform $O(m^{-2})$ perturbation in \eqref{eq:quartic-corollary-cosine} therefore gives, by Rouch\'e's theorem, exactly one zero in each $D_j$ and none elsewhere in $Q_U$, for all sufficiently large $m$.  Since each $D_j$ is invariant under conjugation and contains exactly one zero, that zero is fixed by conjugation and hence real.  The zeros are simple because Rouch\'e's theorem preserves the multiplicity-one count of the shifted cosine zeros.  At every counted zero one has $u>0$, so the map $u\mapsto y=-u^2/m^2$ is locally biholomorphic there, with $dy/du=-2u/m^2\ne0$; the conjugate zero at $-u$ of the even function $Z_m(-u^2/m^2)$ represents the same $y$-zero and lies outside the chosen positive $u$-rectangle.
...
\end{theorem}
\begin{proof}
The estimate \eqref{eq:endpoint-scaling} is Proposition~\ref{prop:fixed-window-endpoint-isolation}.  The same proposition gives the Rouch\'e count in $Q_U$ and the locations \eqref{eq:quartic-u-location}.  The change $y=-u^2/m^2$ maps $0<u\le U$ bijectively to $[-U^2/m^2,0)$, and the nonvanishing derivative $dy/du=-2u/m^2$ at the counted zeros transfers both their number and their simplicity to $Z_m(y)$.  Squaring \eqref{eq:quartic-u-location} gives \eqref{eq:endpoint-zero-quantization}.
\end{proof}
