...
\end{equation}
Here the positive denominator wall is exactly the positive root of $y^2-17y-2$, namely $y_0=(17+\sqrt{297})/2$.
...
\]
be the real simple zeros of the shifted cosine $\cos((a-B_*/(A_*m))u)$.  Since $U$ is not a limiting threshold, choose $\eta>0$ so that, for all sufficiently large $m$, the closed real-centred discs
\[
D_j=\{u\in\CC:|u-\nu_j^{(m)}|\le\eta\},
\qquad \nu_j^{(0)}<U,
\]
