...
$\Pi(\cdot,y)$.  The variation table already proved in Lemma~\ref{lem:positive-axis-sturm-count} gives
...
Throughout this proposition the window parameters $K$, $U$, and the index $j$ are fixed before $m\to\infty$; every $O_K(\cdot)$ constant is uniform on that fixed compact set and does not cover growing windows or growing index sets.
