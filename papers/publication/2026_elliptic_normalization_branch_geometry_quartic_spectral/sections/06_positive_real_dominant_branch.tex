...
\end{theorem}
\begin{proof}
For $y\ne1$ this is Lemma~\ref{lem:positive-axis-dominance-criterion}, whose Sturm table uses the denominator wall given by the positive root of the corrected factor $y^2-17y-2$ and crosses it only through the non-discriminant continuity lemma.  In the case $y>1$, the proof writes the two real roots as $1<p<q$ and the nonreal pair as $z,\bar z$; Vieta gives $z+\bar z=1-p-q$ and $pq(1-p-q)+(p+q)|z|^2=-1$, hence
\[
q^2-|z|^2=\frac{q(q-p)(p+q)+pq+1}{p+q}>0,
\]
