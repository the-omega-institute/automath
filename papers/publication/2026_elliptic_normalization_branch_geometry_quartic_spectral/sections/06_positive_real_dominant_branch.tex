...
\end{theorem}
\begin{proof}
For $y\ne1$ this is Lemma~\ref{lem:positive-axis-dominance-criterion}, whose Sturm table uses the denominator wall given by the positive root of the corrected factor $y^2-17y-2$ and crosses it only through the non-discriminant continuity lemma.  In the case $y>1$, the proof writes the two real roots as $1<p<q$ and the nonreal pair as $z,\bar z$; with \(r=|z|^2=z\bar z\), Vieta gives \(z+\bar z=1-p-q\) and \(pq(1-p-q)+(p+q)r=-1\), hence
\[
r=\frac{pq(p+q-1)-1}{p+q},
\qquad
q^2-r=\frac{q(q-p)(p+q)+pq+1}{p+q}>0,
\]
so the dominant branch is the larger positive root $q$.  At $y=1$ the factorization $(\lambda-2)(\lambda-1)(\lambda+1)^2$ gives the unique largest modulus root $2$, and the reduced recurrence keeps precisely this surviving factor by Proposition~\ref{prop:minimal-order}.
\end{proof}
...
\end{proposition}
\begin{proof}
Appendix~\ref{app:endpoint-branching} is used only through the fixed local data in Proposition~\ref{prop:endpoint-expansion} and the removable spectral decomposition \eqref{eq:removable-endpoint-spectral-decomposition}.  Thus the substitution below is valid on the whole fixed $u$-window, including the point $u=0$ where $s=0$ and $y=0$.  With $y=s^2$ and $s=iu/m$, the endpoint expansion gives the two colliding branches and amplitudes in the common branch coordinate
\[
...
The spectral decomposition used here is the removable one \eqref{eq:removable-endpoint-spectral-decomposition}: \(C_\pm(0)=1/2\), \(\lambda_\pm(0)=1\), and \(Z_m(0)=1\), so the estimate \(R_m(-u^2/m^2)=O_K(m^{-2})\) applies on rectangles that include \(u=0\).
\end{lemma}
\begin{proof}
