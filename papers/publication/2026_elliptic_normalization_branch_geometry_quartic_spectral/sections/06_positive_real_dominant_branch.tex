...
\end{proposition}
\begin{proof}
The formula is the minus-sign sheet of \eqref{eq:y-from-E}.  Direct differentiation gives \eqref{eq:dy-dlambda}; after squaring, positivity is equivalent to
$4\lambda^2(4\lambda^3-4\lambda+1)>(3\lambda^2-1)^2$, i.e. $(\lambda-1)^3(7\lambda^2+5\lambda+1)>0$.  The limits are $0$ as $\lambda\downarrow1$ and $+\infty$ as $\lambda\to\infty$, so the map is bijective.
\end{proof}
...
For every $y>0$, the branch $\lambda_{\rm dom}(y)>1$ of Proposition~\ref{prop:positive-maximal-modulus-branch} is a root of $\Pi(\cdot,y)$ and is the unique root of maximal modulus.  At $y=1$, this branch gives the surviving simple spectral root $\lambda_{\rm dom}(1)=2$.
\end{theorem}
\begin{proof}
...
With $y=s^2$ near $s=0$, the two branches colliding at $\lambda=1$ satisfy
...
Use the endpoint constants computed in Appendix~\ref{app:endpoint-branching},
...
\end{equation}
Here $K$, $U$, and any finite set of indices are fixed before $m\to\infty$; the theorem makes no assertion for growing endpoint windows or growing $k$.
\end{theorem}
\begin{proof}
