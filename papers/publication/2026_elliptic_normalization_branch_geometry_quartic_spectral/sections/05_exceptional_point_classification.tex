...
\begin{proposition}[Canonical finite comparison skeleton for Theorem B]\label{prop:canonical-theorem-b-finite-comparison-skeleton}
...
endpoint values & \(F(\beta_i)=0\) gives \(y(\beta_i,0)=\beta_i^2-1/2\) and the sign isolations in Corollary~\ref{cor:quartic-real-projection-framework} & endpoint candidates lie strictly between the critical extrema except for the listed transition values.\\[3pt]
ramification values & the real ramification abscissae are \(-1,1,\alpha\), where \(\alpha\in(-1/2,0)\) is the real root of \(16\lambda^3-9\lambda^2+1\) & the critical values are \(1\), \(0\), and \(y_{\mathrm{sub}}\).\\[3pt]
subdominant threshold & Lemma~\ref{lem:cubic-branch-real-root} identifies \(y_{\mathrm{sub}}\) as the unique real zero of \(256y^3+411y^2+165y+32\), with \(y_{\mathrm{sub}}\in(-2,-1)\) & the compact oval minimum is \(y_{\mathrm{sub}}\).\\[3pt]
overlap test & \(M_{\mathrm c}=1\) and \(m_{\mathrm n}=0\) & the compact image \([y_{\mathrm{sub}},1]\) and the noncompact image \([0,\infty)\) overlap on \([0,1]\), so the total image is one interval.\\
\bottomrule
\end{tabular}
\endgroup
\end{center}
...
\]
Theorem~\ref{thm:theorem-b-real-projection-criterion} is then obtained from the fixed-member criterion by adding only the two-to-one covering count on the interiors of these two intervals; no parameter-space chamber comparison and no additional dominance assertion enter the proof of Theorem~B.
\end{proposition}

\begin{proof}
The balanced fixed-member criterion, Theorem~\ref{thm:balanced-real-projection}, reduces the real projection of one fixed smooth member to the number of real roots of \(F\), the endpoint values over zeros of \(F\), the finite ramification values, and the single two-oval overlap comparison \(M_{\mathrm c}\ge m_{\mathrm n}\).
Corollary~\ref{cor:quartic-real-projection-framework} performs these computations for the canonical member.
Its proof gives the sign isolations for \(F\), hence the two intervals \(J_{\mathrm c}\) and \(J_{\mathrm n}\); evaluates the endpoint values as \(\beta_i^2-1/2\); locates the real ramification abscissae \(-1\), \(1\), and \(\alpha\); and identifies the corresponding critical values as \(1\), \(0\), and \(y_{\mathrm{sub}}\).
The same corollary proves \(\min\mathcal V(J_{\mathrm c})=y_{\mathrm{sub}}\), \(\max\mathcal V(J_{\mathrm c})=1\), and \(\min\mathcal V(J_{\mathrm n})=0\).
On the compact oval the continuous function \(y\) attains its minimum and maximum at the displayed ramification values, so its image is \([y_{\mathrm{sub}},1]\).
On the noncompact oval the displayed finite minimum is \(0\), and the fixed-member criterion supplies the pole of \(y\) along the unbounded end; hence the image is \([0,\infty)\).
...
\begin{theorem}[Canonical real projection (Theorem B)]\label{thm:theorem-b-real-projection-criterion}
...
\]
Let \(y_{\mathrm{sub}}\) denote the unique real zero of
\[
256y^3+411y^2+165y+32.
\]
The finite comparison skeleton is the one certified in Proposition~\ref{prop:canonical-theorem-b-finite-comparison-skeleton}.
For reference, the proof uses exactly the following finite comparisons from that proposition: \(F\) has three real roots, so the real components lie over \(J_{\mathrm c}\) and \(J_{\mathrm n}\); the endpoint values are the three numbers \(\beta_i^2-1/2\); the only real ramification values are \(1\), \(0\), and \(y_{\mathrm{sub}}\); these values satisfy \(\min\mathcal V(J_{\mathrm c})=y_{\mathrm{sub}}\), \(\max\mathcal V(J_{\mathrm c})=1\), and \(\min\mathcal V(J_{\mathrm n})=0\).  These comparisons give \(y(E_{\mathrm c}(\RR))=[y_{\mathrm{sub}},1]\) and \(y(E_{\mathrm n}(\RR))=[0,\infty)\); the remaining part of the theorem is only the two-to-one covering count over the interiors of these two images.
Then the real projection of the normalized canonical quartic is the following canonical specialization of the fixed-member criterion Theorem~\ref{thm:balanced-real-projection}.  This theorem concerns only the single canonical member just displayed, not a chamber classification for all balanced quartics.  The model is smooth, has no exceptional $\lambda=0$ critical fibre, and falls in the nondegenerate overlap case of the one-interval/two-interval alternative: the compact image maximum is $1$, the noncompact image minimum is $0$, and therefore $1>0$ gives one real-image interval rather than two disjoint intervals.  The only endpoint degeneracies for this canonical statement are the three branch values $y_{\mathrm{sub}}$, $0$, and $1$; the component-image touching degeneracy $M_{\mathrm c}=m_{\mathrm n}$ does not occur.
\begin{enumerate}
...
y(E_{\mathrm n}(\RR))=[0,\infty).
\]
\item Consequently the canonical member is in the genuine one-interval alternative of Theorem~\ref{thm:balanced-real-projection}: since $M_{\mathrm c}=1$ and $m_{\mathrm n}=0$, the two component images overlap nontrivially rather than merely touching or separating.  The total real image is
\[
y(E(\RR))=[y_{\mathrm{sub}},\infty),
\]
...
y\ge y_{\mathrm{sub}};
\]
the number of such roots is the number in the last column of the displayed table.  Thus the only algebraic obstruction to real solvability below the regular branch levels is the single endpoint value \(y_{\mathrm{sub}}\), while the only transition values in the real image are \(y_{\mathrm{sub}},0,1\).
\item The transition set \(\{y_{\mathrm{sub}},0,1\}\) is minimal for the canonical real-fibre phase diagram.  On each connected component of
\[
\RR\setminus\{y_{\mathrm{sub}},0,1\}
\]
both the number of real roots and the contributing real oval or ovals are constant; deleting any one of the three transition values merges two adjacent intervals with different real-root counts.  Thus no smaller finite transition set records the same regular real-fibre classification.
\end{enumerate}
This theorem contains no second real-projection criterion beyond Theorem~\ref{thm:balanced-real-projection} and no assertion about how the finite comparison changes under parameter variation.  It is the canonical specialization used later in the paper: the fixed-member criterion supplies the interval alternative and the finite transition sets, while Corollary~\ref{cor:quartic-real-projection-framework} supplies the canonical algebraic comparisons.  No recurrence-theoretic or dominance input enters this result, and those inputs begin only with Theorem~\ref{thm:canonical-branch-dominance}.
\end{theorem}

\begin{proof}
The discriminant of \(F(\lambda)=4\lambda^3-4\lambda+1\) is \(37\), so \(F\) has three distinct real roots.  The fixed-member criterion, Theorem~\ref{thm:balanced-real-projection}, therefore gives two real components lying above the bounded and unbounded intervals \(J_{\mathrm c}\) and \(J_{\mathrm n}\), and it reduces the complete real-image and fibre-count problem to the finite sets \(\mathcal V(J_{\mathrm c})\) and \(\mathcal V(J_{\mathrm n})\).  Corollary~\ref{cor:quartic-real-projection-framework} is the promised specialization step: it computes these two finite sets exactly as displayed in the statement, with entries given by the endpoint values \(\beta_i^2-1/2\), the non-endpoint ramification values \(1\), \(0\), and \(y_{\mathrm{sub}}\), and with no exceptional \(\lambda=0\) contribution for this specialization.  The same corollary orders these algebraic values and gives
\[
y(E_{\mathrm c}(\RR))=[y_{\mathrm{sub}},1],
\qquad
y(E_{\mathrm n}(\RR))=[0,\infty).
\]
Their union is \([y_{\mathrm{sub}},\infty)\), and their intersection is \([0,1]\).  In the notation of the fixed-member interval alternative, $M_{\mathrm c}=1$ and $m_{\mathrm n}=0$, so $M_{\mathrm c}>m_{\mathrm n}$.  Hence the canonical real image is the nondegenerate one-interval case, not the adjacent-touching case $M_{\mathrm c}=m_{\mathrm n}$ and not the two-disjoint-interval case $M_{\mathrm c}<m_{\mathrm n}$.  The only remaining endpoint degeneracies are the actual ramified endpoint levels $y_{\mathrm{sub}}$, $0$, and $1$, proving the asserted finite real-solvability criterion and the transition-value list.
...
The four regular cells have real-root counts \(0,2,4,2\) in increasing order.  Hence crossing \(y_{\mathrm{sub}}\) changes the count from \(0\) to \(2\), crossing \(0\) changes it from \(2\) to \(4\), and crossing \(1\) changes it from \(4\) to \(2\).  Since no transition can occur off the finite obstruction framework supplied by Theorem~\ref{thm:balanced-real-projection} and Corollary~\ref{cor:quartic-real-projection-framework}, the set \(\{y_{\mathrm{sub}},0,1\}\) is complete; since each of its three points separates adjacent cells with different counts, it is minimal for this phase diagram.
Corollary~\ref{cor:real-phase-diagram} retains the component source of the calculation.  Thus each clause of Theorem~B is either the fixed-member criterion applied to the canonical member or the finite canonical comparison supplied by Corollary~\ref{cor:quartic-real-projection-framework}; the stated dependency chain is complete.
\end{proof}

\begin{proposition}[Canonical real-solvability obstruction]\label{prop:canonical-real-solvability-obstruction}
For a real number \(y\) which is not one of \(y_{\mathrm{sub}},0,1\), the following are equivalent:
\begin{enumerate}
\item the polynomial \(\Pi(\lambda,y)\) has a real root;
\item the normalized real fibre \(E(\RR)\cap y^{-1}(y)\) is nonempty;
\item \(y>y_{\mathrm{sub}}\).
\end{enumerate}
Moreover the number of real roots is \(2\) on \((y_{\mathrm{sub}},0)\), \(4\) on \((0,1)\), and \(2\) on \((1,\infty)\), while no real root occurs on \((-\infty,y_{\mathrm{sub}})\).
\end{proposition}

\begin{proof}
The normalization map in Theorem~\ref{thm:elliptic-reduction} identifies the affine normalized curve with \(\Pi(\lambda,y)=0\), and away from the branch values \(y_{\mathrm{sub}},0,1\) the fibre consists of simple points.  Hence real roots of \(\Pi(\cdot,y)\) are exactly real points of the normalized fibre.  Theorem~\ref{thm:theorem-b-real-projection-criterion} gives the total real image \([y_{\mathrm{sub}},\infty)\) and the regular-fibre table, which proves the equivalence and the three root counts.
\end{proof}
...
\begin{theorem}[Real branch points and local spectral roles]\label{thm:ep-classification}
...
\begin{corollary}[Real-oval image diagram]\label{cor:real-phase-diagram}
Let $\beta_1 < \beta_2 < \beta_3$ be the three real roots of
\[
