...
Theorem~\ref{thm:theorem-b-real-projection-criterion} is the specialization of Theorem~\ref{thm:balanced-real-projection} to the single canonical member.  It is not a chamber classification for varying balanced quartics, and no recurrence-theoretic or dominance input enters its proof.
\end{remark}

\begin{proof}
The discriminant of \(F(\lambda)=4\lambda^3-4\lambda+1\) is \(37\), so \(F\) has three distinct real roots.  The fixed-member criterion, Theorem~\ref{thm:balanced-real-projection}, therefore gives two real components lying above the bounded and unbounded intervals \(J_{\mathrm c}\) and \(J_{\mathrm n}\), and it reduces the real-image and fibre-count problem to the finite sets \(\mathcal V(J_{\mathrm c})\) and \(\mathcal V(J_{\mathrm n})\).  Corollary~\ref{cor:quartic-real-projection-framework} computes these two finite sets exactly as displayed in the statement, with entries given by the endpoint values \(\beta_i^2-1/2\), the non-endpoint ramification values \(1\), \(0\), and \(y_{\mathrm{sub}}\), and with no exceptional \(\lambda=0\) contribution for this specialization.  The same corollary orders these algebraic values and gives
\[
y(E_{\mathrm c}(\RR))=[y_{\mathrm{sub}},1],
\qquad
y(E_{\mathrm n}(\RR))=[0,\infty).
...
The four regular cells have real-root counts \(0,2,4,2\) in increasing order.  Hence crossing \(y_{\mathrm{sub}}\) changes the count from \(0\) to \(2\), crossing \(0\) changes it from \(2\) to \(4\), and crossing \(1\) changes it from \(4\) to \(2\).  Since no transition can occur off the finite obstruction framework supplied by Theorem~\ref{thm:balanced-real-projection} and Corollary~\ref{cor:quartic-real-projection-framework}, the set \(\{y_{\mathrm{sub}},0,1\}\) is complete; since each of its three points separates adjacent cells with different counts, it is minimal for this real-root interval diagram.
Corollary~\ref{cor:real-root-interval-diagram} retains the component source of the calculation.  Thus each clause of Theorem~B follows from the fixed-member criterion applied to the canonical member and the finite canonical comparison supplied by Corollary~\ref{cor:quartic-real-projection-framework}.
\end{proof}
