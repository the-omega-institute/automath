...
This section is proof-bearing preparation for Theorem~\ref{thm:consolidated-branch-dominance-package}: it determines the local collision structure at each real branch point and compares the modulus of the colliding eigenvalues against the remaining roots.
The assembled branch--dominance classification is cited canonically as Theorem~\ref{thm:consolidated-branch-dominance-package}; the local statements below are its inputs, not competing theorem packages.
...
\begin{theorem}[Canonical finite real-projection criterion (Theorem B)]\label{thm:theorem-b-real-projection-package}
...
\item The complete finite transition skeleton is
\[
...
\end{enumerate}
This is the canonical specialization of the reusable fixed-member certificate in Theorem~\ref{thm:balanced-real-projection}; no recurrence-theoretic or dominance input enters this package, and those inputs begin only with Theorem~\ref{thm:consolidated-branch-dominance-package}.
\end{theorem}

\begin{proof}
The discriminant of \(F(\lambda)=4\lambda^3-4\lambda+1\) is \(37\), so \(F\) has three distinct real roots.  The fixed-member criterion, Theorem~\ref{thm:balanced-real-projection}, therefore gives two real components lying above the bounded and unbounded intervals \(J_{\mathrm c}\) and \(J_{\mathrm n}\).  Proposition~\ref{prop:quartic-real-projection-skeleton} computes the complete finite sets \(\mathcal V(J_{\mathrm c})\) and \(\mathcal V(J_{\mathrm n})\) in the statement: their entries are exactly the endpoint values \(\beta_i^2-1/2\), the non-endpoint ramification values \(1\), \(0\), and \(y_{\mathrm{sub}}\), and there is no exceptional \(\lambda=0\) contribution for this specialization.  The same proposition orders these algebraic values and gives
\[
y(E_{\mathrm c}(\RR))=[y_{\mathrm{sub}},1],
\qquad
y(E_{\mathrm n}(\RR))=[0,\infty).
...
\begin{proof}
The normalization map in Theorem~\ref{thm:elliptic-reduction} identifies the affine normalized curve with \(\Pi(\lambda,y)=0\), and away from the branch values \(y_{\mathrm{sub}},0,1\) the fibre consists of simple points.  Hence real roots of \(\Pi(\cdot,y)\) are exactly real points of the normalized fibre.  Theorem~\ref{thm:theorem-b-real-projection-package} gives the total real image \([y_{\mathrm{sub}},\infty)\) and the regular-fibre table, which proves the equivalence and the three root counts.
\end{proof}
...
\begin{theorem}[Proof-bearing real branch-point preparation for Theorem D]\label{thm:ep-classification}
...
Their local spectral roles, used later as inputs to the canonical Theorem~\ref{thm:consolidated-branch-dominance-package}, are as follows.
\begin{enumerate}
