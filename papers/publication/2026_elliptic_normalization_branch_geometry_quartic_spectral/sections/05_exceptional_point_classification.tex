...
\begin{theorem}[Theorem B: canonical specialization of the family real-projection theorem]\label{thm:theorem-b-real-projection-package}
...
\]
the real-projection theorem package is the following single chain of assertions.
\begin{enumerate}
\item The family-level finite criterion is Theorem~\ref{thm:balanced-real-projection}: for each real component of a smooth balanced member, the real \(y\)-image is determined by the finite algebraic set \(\mathcal V(J)\) of endpoint values, non-endpoint ramification values, and the exceptional \(\lambda=0\) values.
...
\[
\begin{array}{c|c|c|c}
\text{range of }y & \#(E_{\mathrm c}(\RR)\cap y^{-1}(y)) & \#(E_{\mathrm n}(\RR)\cap y^{-1}(y)) & \#\{\lambda\in\RR:\Pi(\lambda,y)=0\}\\
\hline
...
\end{array}
\]
Equivalently, Corollary~\ref{cor:real-phase-diagram} is exactly the canonical real-spectral phase diagram obtained from the finite skeleton.
\end{enumerate}
...
Thus the table is exactly Corollary~\ref{cor:real-phase-diagram}, with the component source retained explicitly, and the stated dependency chain is complete.
\end{proof}
...
\begin{corollary}[Preparatory real-spectral phase diagram]\label{cor:real-phase-diagram}
...
\end{cases}
\end{equation}
In particular, the transfer family is completely real-rooted if and only if $y\in[0,1]$, agreeing with Corollary~\ref{prop:real-fibers-specialization}.
\end{corollary}
