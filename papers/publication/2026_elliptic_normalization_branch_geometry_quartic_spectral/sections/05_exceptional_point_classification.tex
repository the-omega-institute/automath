...
-4\cdot256\cdot165^3
-4\cdot411^3\cdot32
-27\cdot256^2\cdot32^2
+18\cdot256\cdot411\cdot165\cdot32
=-699868431<0.
\]
...
The comparison is certified inside the cubic field \(\QQ(\alpha)\), without numerical approximation.
Using \eqref{eq:q-formula},
...
Equivalently, the modulus gap satisfies the exact identity
\[
16\alpha\,(q-\alpha^2)+8(\alpha^2+1)=0,
\]
whose right-hand summand is positive and whose coefficient \(16\alpha\) is negative.
Thus \(q-\alpha^2>0\), so the residual conjugate pair has modulus \(\sqrt q>|\alpha|\).
This is the claimed strict algebraic certificate that the double root at $\lambda = \alpha$ is subdominant.
\end{proof}
