...
\begin{theorem}[Canonical real projection and fibre count (Theorem B)]\label{thm:theorem-b-real-projection-criterion}
...
\end{enumerate}
This is the canonical specialization of the reusable fixed-member criterion in Theorem~\ref{thm:balanced-real-projection}; no recurrence-theoretic or dominance input enters this result, and those inputs begin only with Theorem~\ref{thm:canonical-branch-dominance}.
\end{theorem}

\begin{proof}
The discriminant of \(F(\lambda)=4\lambda^3-4\lambda+1\) is \(37\), so \(F\) has three distinct real roots.  The fixed-member criterion, Theorem~\ref{thm:balanced-real-projection}, therefore gives two real components lying above the bounded and unbounded intervals \(J_{\mathrm c}\) and \(J_{\mathrm n}\).  Proposition~\ref{prop:quartic-real-projection-framework} computes the complete finite sets \(\mathcal V(J_{\mathrm c})\) and \(\mathcal V(J_{\mathrm n})\) in the statement: their entries are exactly the endpoint values \(\beta_i^2-1/2\), the non-endpoint ramification values \(1\), \(0\), and \(y_{\mathrm{sub}}\), and there is no exceptional \(\lambda=0\) contribution for this specialization.  The same proposition orders these algebraic values and gives
\[
y(E_{\mathrm c}(\RR))=[y_{\mathrm{sub}},1],
\qquad
y(E_{\mathrm n}(\RR))=[0,\infty).
