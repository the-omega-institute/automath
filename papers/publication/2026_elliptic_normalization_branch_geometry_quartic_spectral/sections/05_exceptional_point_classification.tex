...
The same proposition proves \(\min\mathcal V(J_{\mathrm c})=y_{\mathrm{sub}}\), \(\max\mathcal V(J_{\mathrm c})=1\), and \(\min\mathcal V(J_{\mathrm n})=0\).
...
\item The transition set \(\{y_{\mathrm{sub}},0,1\}\) is minimal for the canonical real-root interval diagram.  On each connected component of
\[
...
\begin{remark}[Scope of Theorem B]\label{rem:theorem-b-scope}
...
The four regular cells have real-root counts \(0,2,4,2\) in increasing order.  Hence crossing \(y_{\mathrm{sub}}\) changes the count from \(0\) to \(2\), crossing \(0\) changes it from \(2\) to \(4\), and crossing \(1\) changes it from \(4\) to \(2\).  Since no transition can occur off the finite obstruction framework supplied by Lemma~\ref{prop:balanced-real-projection-fixed-extrema} for this member and evaluated in Proposition~\ref{cor:quartic-real-projection-framework}, the set \(\{y_{\mathrm{sub}},0,1\}\) is complete; since each of its three points separates adjacent cells with different counts, it is minimal for this real-root interval diagram.
Corollary~\ref{cor:real-root-interval-diagram} retains the component source of the calculation.  Thus each clause of Theorem~B follows from the auxiliary fixed-member test applied only to the canonical member and the finite canonical comparison supplied by Proposition~\ref{cor:quartic-real-projection-framework}.
\end{proof}
...
\begin{proof}
The normalization map in Proposition~\ref{prop:elliptic-reduction} identifies the affine normalized curve with \(\Pi(\lambda,y)=0\), and away from the branch values \(y_{\mathrm{sub}},0,1\) the fibre consists of simple points.  Hence real roots of \(\Pi(\cdot,y)\) are exactly real points of the normalized fibre.  Theorem~\ref{thm:theorem-b-real-projection-criterion} gives the total real image \([y_{\mathrm{sub}},\infty)\) and the regular-fibre table, which proves the equivalence and the three root counts.
\end{proof}
...
\(\ySub\eta\overline\eta=-1/8\).  The interval \(-2<\ySub<-1\) gives
...
Proposition~\ref{prop:ep-classification} shows that $y_{\mathrm{sub}}$ fails this dominance test: the colliding eigenvalue pair at $\lambda = \alpha$ is exceeded in modulus by the complex-conjugate pair of modulus $\sqrt{q}$.
...
By Proposition~\ref{prop:ep-classification}, the cubic factor has a unique real root $\alpha\in(-1/2,0)$, and its image is $y_{\mathrm{sub}}$.
