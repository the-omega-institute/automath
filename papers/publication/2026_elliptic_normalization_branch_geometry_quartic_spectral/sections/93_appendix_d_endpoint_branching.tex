\section{Verification of the endpoint branching hypotheses at \texorpdfstring{\(y=0\)}{y=0}}\label{app:endpoint-branching}

This appendix is a proof-bearing component of Theorem~\ref{thm:endpoint-zero-quantization}: Proposition~\ref{prop:endpoint-expansion-package} supplies the branch expansions, amplitudes, and compact-uniform remainder estimates used in Proposition~\ref{prop:endpoint-cosine-checklist}.
...
The same radius $r=1/4$, the same branch coordinate $y=s^2$, and the same constants in \eqref{eq:appendix-cosine-constants} are the ones used in Proposition~\ref{prop:endpoint-cosine-checklist}; no independent local constants are introduced in Section~\ref{sec:positive-maximal-modulus-branch}.
...
C_{-1}(y)=O_K(m^{-2}),
\qquad
\lambda_{-1}(y)^m=(-1)^m\exp\!\left(-\frac{u^2}{4m}+O_K(m^{-3})\right)=O_K(1),
\]
and
\[
