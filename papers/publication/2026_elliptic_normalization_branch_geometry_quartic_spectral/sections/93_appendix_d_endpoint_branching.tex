...
This appendix is part of the proof of Theorem~\ref{thm:endpoint-zero-quantization}: Proposition~\ref{prop:endpoint-expansion} supplies the branch expansions, amplitudes, and compact-uniform remainder estimates used in Proposition~\ref{prop:endpoint-cosine-hypotheses}.
...
The coefficient comparisons below give the exact constants \(A=1/\sqrt2\), \(B=5/8\), \(A_*=1/2\), \(B_*=3\sqrt2/16\), and the compact-uniform estimate \(R_m(-u^2/m^2)=O_K(m^{-2})\) imported by Proposition~\ref{prop:endpoint-cosine-hypotheses}, Remark~\ref{rem:endpoint-zero-law-data-package}, and Theorem~\ref{thm:endpoint-zero-quantization}.
Fix the disc
\[
...
The body uses Proposition~\ref{prop:endpoint-expansion} in the following fixed form: Section~\ref{sec:positive-maximal-modulus-branch} uses only this branch-coordinate domain, the constants \((a,b,A_*,B_*)\), the symmetries \(\lambda_-(s)=\lambda_+(-s)\) and \(C_-(s)=C_+(-s)\), the nonvanishing of the colliding branches and amplitudes after shrinking inside \(|s|<1/2\), and the compact-uniform bound \(R_m(-u^2/m^2)=O_K(m^{-2})\).
...
Although the fibre derivative \(\partial_\lambda\Pi\) vanishes at the double root \((\lambda,y)=(1,0)\), this does not create a pole in the branch-coordinate amplitudes.
After writing \(y=s^2\), Weierstrass preparation factors the colliding part as \((\lambda-\lambda_+(s)) (\lambda-\lambda_-(s))\) with \(\lambda_+(s)-\lambda_-(s)=2s/\sqrt2+O(s^3)\), while the numerator in the residue formula is evaluated on the same holomorphic branches.
Thus each residue is a meromorphic function of \(s\) with at worst a simple pole at \(s=0\), and the possible pole is odd under the involution \(s\mapsto -s\) that interchanges the two branches.
The sum of the two singular-looking residue terms is holomorphic in $s^2$ because it equals the part of the rational function obtained after subtracting the two non-colliding simple-root terms; hence the symmetric combination of the two residues is holomorphic and the odd pole in the antisymmetric combination is cancelled by the factor \(\lambda_+(s)-\lambda_-(s)\) in the reconstructed numerator.
Equivalently, both residue amplitudes have removable singularities at the coalescence and are holomorphic in the square-root parameter.
Equivalently, substituting the Puiseux roots into the identity
\begin{align*}
Z_m(s^2)
&=C_+(s)\lambda_+(s)^m+C_-(s)\lambda_-(s)^m\\
&\quad{}+C_{-1}(s^2)\lambda_{-1}(s^2)^m
 +C_{\mathrm{small}}(s^2)\lambda_0(s^2)^m
\end{align*}
...
Each such disc, or any finite family of such discs, is fixed before the limit \(m\to\infty\); the constant \(M_K\) is not asserted uniformly over a growing collection of centres.
...
\right].
\end{equation}
This identity is recorded here only as a check that the local constants give the model cosine scale.
The zero isolation and fixed-index endpoint law are not part of this appendix proposition; they are obtained in Section~\ref{sec:positive-maximal-modulus-branch} from Proposition~\ref{prop:endpoint-cosine-hypotheses} and Lemma~\ref{lem:two-branch-oscillation-law}.
...
These local expansions and the remainder estimate are precisely the proof inputs supplied to Proposition~\ref{prop:endpoint-cosine-hypotheses}; the compact-window cosine law and the endpoint zero quantization are proved later in Theorem~\ref{thm:endpoint-zero-quantization}, with no moving-window or growing-index conclusion supplied by this appendix.
\end{proposition}
...
For the amplitudes, the residue formula \eqref{eq:partial-fractions}, interpreted through the removable-singularity argument above, gives holomorphic amplitudes in the same branch coordinate.
...
\]
is uniform on compact \(K\), and converts \eqref{eq:appendix-scaling} into \eqref{eq:appendix-shifted-cosine}; this last displayed cosine form is a recorded consequence of the local inputs, not an additional appendix endpoint-zero theorem.
\end{proof}
