...
The local calculations below give the exact branch constants \(A=1/\sqrt2\), \(B=5/8\), the direct residue constants \(A_*=1/2\), \(B_*=3\sqrt2/16\), and, for each compact \(K\subset\CC\), the estimate \(R_m(-u^2/m^2)=O_K(m^{-2})\).
Fix the disc
\[
...
The noncolliding branches contribute \eqref{eq:appendix-remainder-bound} by the endpoint-scale remainder bound, which is the same statement as \eqref{eq:appendix-remainder-bound-restated}.  The logarithm of the branch expansion is \eqref{eq:log-lambda-plus-minus}.  Substituting \(s=iu/m\) gives \eqref{eq:lambda-power}; substituting the amplitude expansion gives \eqref{eq:amplitude-scale}.  Adding the two colliding terms in the spectral decomposition \eqref{eq:spectral-decomposition} and then adding the remainder gives \eqref{eq:appendix-scaling}.  Finally,
\[
