\section{Proof component: endpoint branching at \texorpdfstring{\(y=0\)}{y=0}}\label{app:endpoint-branching}
...
It supplies the theorem-critical hypotheses of Lemma~\ref{lem:two-branch-oscillation-law} for the quartic transfer family at the square-root endpoint $y_c=0$ and records the branch expansion predicted by Lemma~\ref{lem:generic-ep2-branching}.
The coefficient comparisons below are proof material: the endpoint zero theorem uses the exact constants \(A=1/\sqrt2\), \(B=5/8\), \(A_*=1/2\), \(B_*=3\sqrt2/16\), and the compact-uniform estimate \(R_m(-u^2/m^2)=O_K(m^{-2})\).
Fix the disc
\[
...
\begin{proposition}[Endpoint expansion theorem]\label{prop:endpoint-expansion}
