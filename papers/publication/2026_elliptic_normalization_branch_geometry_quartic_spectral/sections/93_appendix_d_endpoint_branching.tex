...
\end{equation}
because Lemma~\ref{lem:plus-branch-residue-cancellation} and the symmetry calculation above give holomorphic extensions of $C_\pm$ with $C_\pm(0)=1/2$, while \eqref{eq:lambda-plus-minus} gives holomorphic extensions of $\lambda_\pm$ with $\lambda_\pm(0)=1$.  At $s=0$ the right side is $1/2+1/2+R_m(0)$, and $R_m(0)=0$ from \eqref{eq:other-amplitudes}, hence it agrees with the exact value $Z_m(0)=1$.

The estimate \eqref{eq:appendix-remainder-bound} is therefore a holomorphic estimate on the compact $u$-window including $u=0$, not merely a punctured-window estimate; substituting $s=iu/m$ in \eqref{eq:removable-endpoint-spectral-decomposition} is legitimate for every $u\in K$, including $u=0$.
