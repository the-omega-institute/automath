...
The comparison of the $s^2$ coefficients for the two initial values $m=2$ and $m=3$ is included to record compatibility with the exact spectral identities.
...
\]
Substitution back into \eqref{eq:Bstar-m2} gives $E=7/16$, agreeing with the direct computation in \eqref{eq:direct-colliding-residue-expansion}.
This gives
...
For the amplitudes, Lemma~\ref{lem:plus-branch-residue-cancellation} and the involution $s\mapsto -s$ prove that the individual residue formulae have removable singularities at $s=0$ and give holomorphic amplitudes in the same branch coordinate.
The coefficient comparison \eqref{eq:Bstar-m2}--\eqref{eq:Bstar-m3} then gives the displayed quadratic coefficient:
\[
...
The exact spectral identities for \(Z_2(s^2)=1+2s^2+O(s^4)\) and \(Z_3(s^2)=1+3s^2+O(s^4)\), including the noncolliding contributions from \eqref{eq:other-branches}--\eqref{eq:other-amplitudes}, then satisfy the two displayed linear equations \eqref{eq:Bstar-m2} and \eqref{eq:Bstar-m3}; this is the consistency check leading to \eqref{eq:C-plus-minus}, not the source of removability.
