...
\begin{proof}
The Puiseux expansion of the two colliding branches is equation~\eqref{eq:lambda-plus-minus}, obtained by substituting \(\lambda=1\pm As+Bs^2\pm Cs^3+O(s^4)\) and \(y=s^2\) in \(\Pi(\lambda,y)=0\) and comparing coefficients.
The amplitude expansion is equation~\eqref{eq:C-plus-minus}; it is forced by the exact spectral identities for \(Z_0,Z_1,Z_2,Z_3\) together with the analytic expansions of the two noncolliding branches in \eqref{eq:other-branches}--\eqref{eq:other-amplitudes}.
The noncolliding branches obey the uniform estimate \eqref{eq:appendix-remainder-bound} after the substitution \(y=-u^2/m^2\), because \(C_{-1}(y)=O(y)\), \(\lambda_{-1}(y)^m=O_K(1)\), and both \(C_{\mathrm{small}}(y)\) and \(\lambda_0(y)\) are \(O_K(m^{-2})\) on the microscopic scale.
Finally, substituting the logarithmic form \eqref{eq:lambda-power}, the amplitude form \eqref{eq:amplitude-scale}, and the remainder estimate into the partial-fraction decomposition \eqref{eq:spectral-decomposition} gives \eqref{eq:appendix-scaling}; the elementary identity \(\cos(\theta-\delta)=\cos\theta+\delta\sin\theta+O(\delta^2)\), uniformly for \(u\in K\), converts it into \eqref{eq:appendix-shifted-cosine}.
\end{proof}
