...
For the quartic transfer family, after writing \(y=s^2\) near \(0\), the two branches issuing from the double root \(\lambda=1\) and their partial-fraction amplitudes may be labelled so that
\[
\lambda_\pm(s)
=
...
\begin{proof}
All assertions have been derived in the preceding computations; we collect the derivation to make the logical dependence explicit.
...
\Pi_y(1,0)=-1.
\]
Thus \(y=s^2\) gives two holomorphic branches with leading coefficient \(A^2=1/2\).
Substitution of
\[
