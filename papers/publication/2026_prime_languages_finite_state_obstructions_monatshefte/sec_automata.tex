...
\begin{proof}
  ...
  \[
    ...
    \qquad (x\ge5393)
  \]
  \cite{Dusart2010}. We use this source only for the displayed explicit
  numerical inequality. This is enough for the present lemma because no
  later argument depends on sharp constants; any published explicit-PNT
  estimate with comparable constants may be substituted without changing
  the argument. Since \(\log 5393>2\), for \(x\ge5393\) we have
  \[
...
\begin{proof}
  ...
  \]
  The endpoint correction is the visible inequality
  \[
    \left|
      \abs{\mathcal P_m^{(b)}}-
...
\begin{lemma}[Certified spectral decay for a fixed residue class]
  ...
  rational numbers \(K_r>0\), \(\theta_r\in(0,1)\), and an integer
  \(N_r\) such that
  \[
  ...
  proof object also records, for each residue, a rational monotonicity
  threshold and the finite list of exact rational matrix inequalities
  checked below that threshold. We assert computability of these data;
  no polynomial bit-complexity or output-sensitive complexity bound is
  claimed.
\end{lemma}
...
\begin{proof}
  ...
  of roots is empty, take \(\theta_r=1/2\). Otherwise choose a rational
  \(\theta_r\in(0,1)\) such that all eigenvalues of \(T\) have modulus
  \(<\theta_r\) and all eigenvalues of every \(E_{j,t}\) have modulus
  \(<\theta_r^{d_j}\). Each strict comparison is certified by rational
  isolating rectangles whose squared moduli lie below the corresponding
  rational threshold. This gives the spectral-radius part of the
  certificate, including the case in which there are no non-peripheral
  roots to control.
  ...
  and let \(\eta\in(0,1)\) be the corresponding target rate
  (\(\eta=\theta_r\) for \(T\), and
  \(\eta=\theta_r^{d_j}\) for \(E_{j,t}\)). Choose a rational
  ...
  matrices. The resolvent bounds above dominate every such product by a
  rational multiple of either \(\theta_r^m\) or of a convolution
  \(\sum_{\ell=0}^{m-1}\theta_r^\ell\theta_r^{m-1-\ell}\). Since
  \(m\theta_r^{m-1}\le C_{\theta_r,N}\theta_r^m\) for all \(m\ge N\)
  after an effectively found monotonicity threshold, these finitely many
  estimates give rational \(K_r\) and \(N_r\). The certificate records
  the explicit value of this monotone-tail threshold. The finitely many
  powers in the residue class below that threshold are checked directly
  by rational matrix powering and are included as exact inequalities;
  if necessary, \(K_r\) is enlarged before the certificate is printed.
  Thus the certificate verifies the displayed inequality for all
...
\begin{theorem}[Algorithmic fixed-DFA prime-slice obstruction]
  ...
  \begin{enumerate}[label=\textup{(\alph*)}]
    ...
    \item Rational constants \(K_r>0\), \(\theta_r\in(0,1)\), and
    integer thresholds \(N_r\) such that
    \[
      \abs{a_m(\mathcal A)/b^m-c_r}\le K_r\theta_r^m
      \qquad(m\equiv r\pmod {D_A},\ m\ge N_r).
    \]
    The certificate records the strict spectral-radius inequalities
    and rational resolvent bounds of
    \Cref{lem:effective-spectral-constants}, including the rational
    numbers \(\theta_r\), rational upper bounds \(K_r\), the monotone
    tail threshold, and the finite list of exact rational inequalities
    checked before the tail. These data are enough to verify the
    inequality mechanically, without reconstructing an algebraic Jordan
    basis.
  \end{enumerate}
  ...
  \[
    \mathcal C=(D_A,\{R_j,d_j,(R_{j,t})_t,\pi_{j,t},A_{j,t}\}_{j,t},
      \{K_r,\theta_r,N_r,M_r,c_r\}_{r\bmod D_A})
  \]
  ...
  residue-evaluation procedure for \(r\mapsto c_r,K_r,\theta_r,N_r,M_r\)
  and the corresponding proof object from part~\textup{(e)};
...
\begin{proof}
  ...
  \(D_A\) gives \(K_r,\theta_r,N_r\) and
  \[
    \abs{a_m(\mathcal A)/b^m-c_r}\le K_r\theta_r^m
    \qquad(m\equiv r\bmod {D_A},\ m\ge N_r).
  \]
  The associated certificate consists of rational \(\theta_r\), a
  rational bound \(K_r\), the spectral-radius proof objects for the
  transient and return-error blocks, the rational resolvent bounds, and
  the finite rational power checks preceding the monotone tail. Thus the
  quantified inequality used below is part of the output, not an
  unverifiable analytic side condition.
