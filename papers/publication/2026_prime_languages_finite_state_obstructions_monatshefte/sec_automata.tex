...
\begin{lemma}[Effective spectral constants for a fixed residue class]
  \label{lem:effective-spectral-constants}
  Let \(P\) be a rational stochastic matrix of dimension \(n\) and bit
  height \(H\). Suppose that the recurrent classes and their cyclic
  decompositions have been computed, and let \(D_A\) be a common
  multiple of the recurrent periods. For each residue class
  \(r\bmod D_A\) one can compute algebraic real numbers
  \(K_r>0\), \(\theta_r\in(0,1)\), and an integer \(N_r\) such that
  \[
    \|P^m-S_r\|\le K_r\theta_r^m
    \qquad(m\equiv r\bmod D_A,\ m\ge0),
  \]
  where \(S_r=\lim_{k\to\infty}P^{kD_A+r}\). The constants are given by
  minimal polynomials and isolating intervals, and the computation is
  effective in the rational bit model.
\end{lemma}

\begin{proof}
  Put \(P\) in canonical Markov-chain block form. The blocks that can
  contribute non-peripheral decay are the transient block \(P_{TT}\) and
  the primitive return blocks \(P^{d_j}|_{R_{j,t}}\) after the
  Perron--Frobenius rank-one limits have been subtracted. Compute their
  characteristic polynomials over \(\QQ\), factor them over \(\QQ\), and
  isolate all complex roots by standard algebraic root-isolation
  algorithms. This gives exact algebraic representations of every
  eigenvalue and separation bounds between their moduli.

  Let \(\Theta\) be the finite set of moduli of the eigenvalues of
  \(P_{TT}\) and of the non-unit eigenvalues of the return blocks. If
  \(\Theta=\varnothing\), set \(\theta_r=1/2\); the only remaining
  errors are nilpotent or finite-prefix errors, handled below. If
  \(\Theta\ne\varnothing\), use the isolating intervals and separation
  bounds to choose a rational number
  \[
    \theta_r\in(\max\Theta,1).
  \]

  For each eigenvalue \(\mu\) with \(|\mu|<\theta_r\), and for each
  Jordan block size \(s\), the binomial expansion of
  \((\mu I+N)^m\) bounds the corresponding block by
  \(C_{\mu,s}m^{s-1}|\mu|^m\), with \(C_{\mu,s}\) computed from a
  transition matrix between the rational basis and the algebraic Jordan
  basis. Put \(a=|\mu|/\theta_r<1\). Since
  \(x\mapsto x^{s-1}a^x\) is decreasing for
  \(x\ge (s-1)/\log(1/a)\), the finite maximum
  \[
    C(\mu,\theta_r,s):=
    \max_{0\le m\le N_{\mu,s}} m^{s-1}
       \left(\frac{|\mu|}{\theta_r}\right)^m,
    \qquad
    N_{\mu,s}:=
    \left\lceil\frac{s-1}{\log(\theta_r/|\mu|)}\right\rceil+1
  \]
  satisfies
  \[
    m^{s-1}|\mu|^m\le C(\mu,\theta_r,s)\theta_r^m
    \qquad(m\ge0).
  \]
  The finite maximum is evaluated with algebraic interval arithmetic.
  Nilpotent parts vanish after at most \(n\) steps; enlarge the same
  finite maximum to cover those initial values. Summing the finitely
  many block estimates gives a computable \(K_r\). If the sum is \(0\),
  replace it by \(1\). Finally, verify all finitely many values before
  the block estimates begin by direct rational matrix powering and
  enlarge \(K_r\) if necessary. The resulting constants prove the
  displayed inequality for all \(m\ge0\); the finite verification also
  records an explicit threshold \(N_r\).
\end{proof}
...
\begin{theorem}[Algorithmic fixed-DFA prime-slice obstruction]
  ...
  \begin{enumerate}[label=\textup{(\alph*)}]
    ...
    \[
      ...
      \qquad(m\equiv r\pmod {D_A},\ m\ge0).
    \]
  \end{enumerate}
  ...
  \(c_0,\dots,c_{D_A-1}\) has output size at least \(D_A\); no polynomial
...
\begin{proof}
  ...
  \[
    ...
    B_b/m\le c_r/4
  \]
  for all \(m\ge M_r\) with \(m\equiv r\bmod {D_A}\). This is computed
  in the same real bit model.
  ...
  size, which is necessary because \(D_A\) may be super-polynomial in the
  number of states.
\end{proof}
