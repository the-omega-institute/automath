...
\begin{proof}
  ...
  two exponential bounds is
  \[
    \tau_r^{u+D_Aq}\tau_r^{d_jH_{m,u}-D_Aq}
      =\tau_r^{m-1-a},
  \]
  ...
  \(\tau_r^{-1-a}\) into \(C\). The infinite tail above is bounded by the
  ...
  \begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \[
      C_u=\|X_u\|_\infty L_T\|Y_u\|_\infty
        L_{E_{j,t_u}}\|Z_u\|_\infty\tau_r^{-1-a}
    \]
    for the corresponding residue \(u,a\); if \(a=0\), this means the
    same displayed formula with the factor \(\tau_r^{-1}\). Indeed
    ...
    identity \(m-1-u=d_jH_{m,u}+a\) gives a contribution bounded by
    \(C_u\tau_r^m\) for each \(q\). The number of terms is at most \(m\),
