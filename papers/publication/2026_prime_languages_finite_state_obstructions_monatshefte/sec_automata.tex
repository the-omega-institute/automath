...
\begin{lemma}[Prime counts in base-\(b\) slices]
  ...
  There exist constants \(A_b,B_b>0\) such that, for all sufficiently
  large \(m\),
  \[
    A_b\,\frac{b^m}{m}
...
\begin{proof}
  ...
  \[
    \pi(b^m)-\pi(b^{m-1})+O(1),
  \]
  since only the two endpoints can differ. The prime number theorem with
  first-order error term gives
  \[
    \pi(x)=\frac{x}{\log x}
      +O\!\left(\frac{x}{\log^2 x}\right).
  \]
  Applying this at \(x=b^m\) and \(x=b^{m-1}\), and absorbing the endpoint
  \(O(1)\), yields
  \[
    \pi(b^m)-\pi(b^{m-1})
    =
  ...
  \]
  and the factor in parentheses tends to \(1-1/b>0\), the main term is
  bounded above and below by positive multiples of \(b^m/m\) for all
  large \(m\). The error term is \(o(b^m/m)\), so reducing the lower
  constant and increasing the upper constant gives the asserted
  constants \(A_b,B_b\).
\end{proof}
...
\begin{theorem}[Effective fixed-DFA prime-slice obstruction]
  ...
  \(\mathcal A\) one can compute an integer \(p\ge1\), residue
  constants \(c_0,\dots,c_{p-1}\in\RR_{\ge0}\), residue alternatives
  \(c_r=0\) or \(c_r>0\), and constants
  \(C_r,D_r>0\), \(\eta_r\in(0,1)\), and \(M_r\) with the following
  property for every \(m\ge M_r\) satisfying \(m\equiv r\pmod p\).
  If \(c_r=0\), then
  \[
  ...
  \[
    ...
    \ge D_r b^m .
  \]
  Here \(B_b\) is any explicit upper constant in
  \(\abs{\mathcal P_m^{(b)}}\le B_b b^m/m\) valid for \(m\ge M_b\),
  obtained from an explicit prime number theorem bound. Consequently
  ...
  \]
  The algorithm uses polynomially many arithmetic operations in the
  number of states for the graph decomposition and the rational linear
  systems; the bit complexity is polynomial in the bit size of the
  rational data produced from \(P=B/b\), apart from the chosen explicit
  prime-counting threshold.
\end{theorem}
...
\begin{proof}
  ...
  Choose explicit constants \(A_b,B_b>0\) and \(M_b\) from any explicit
  PNT estimate so that
  \[
    A_b\,\frac{b^m}{m}
    \le\abs{\mathcal P_m^{(b)}}
    \le B_b\,\frac{b^m}{m}
