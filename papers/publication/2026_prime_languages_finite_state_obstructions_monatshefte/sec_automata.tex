...
\begin{proof}
  The leading-digit convention gives an exact count
  \[
    \abs{\mathcal P_m^{(b)}}=\pi(b^m)-\pi(b^{m-1}).
  \]
  The prime number theorem with first-order error term gives
  \[
    \pi(x)=\frac{x}{\log x}
      +O\!\left(\frac{x}{\log^2 x}\right).
  \]
  Applying this at \(x=b^m\) and \(x=b^{m-1}\) yields
  \[
    \pi(b^m)-\pi(b^{m-1})
    =
    \frac{b^m}{m\log b}
...
\begin{proof}
  ...
  finite-state transient/recurrent decomposition used in
  \Cref{thm:dfa-density-dichotomy} to \(\widehat P\). The cemetery state
  forms one closed recurrent class. Its peripheral projector is supported
  on the cemetery coordinate, so it contributes zero to the
  \(Q'\times Q'\) block. Any other closed recurrent class of
  \(\widehat P\), if present, lies inside \(Q'\) and has the usual
  periodic Perron--Frobenius projectors. Transient blocks, including
  paths that eventually enter the cemetery state, contribute only an
  exponentially decaying remainder on the \(Q'\times Q'\) block. Thus,
  after restricting the expansion to this block, there are an integer
  \(p'\ge 1\), non-negative matrices \(S_0,\dots,S_{p'-1}\), a constant
  ...
  \[
    ...
    \|E_m\|\le C\eta^m.
  \]
  The residue matrices are non-negative because, for each residue class,
  \[
    S_r=\lim_{k\to\infty}P'^{kp'+r}
  \]
  entrywise; this also covers the case in which the restricted block is
  purely transient, where every \(S_r\) is the zero matrix.
...
\begin{remark}[Relation to conditional-density literature]
  ...
  new contribution is the conversion of this residue-class dichotomy
  into the finite-budget prime obstructions recorded in
