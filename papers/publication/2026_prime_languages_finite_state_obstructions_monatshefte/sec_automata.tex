...
\begin{proof}
  ...
  Eigenvalue \(0\) contributes only nilpotent Jordan terms, which vanish
  after at most \(s\) steps and are absorbed into the finite-prefix
  maximum below. For each non-zero eigenvalue \(\mu\) with
  \(|\mu|<\theta_r\), and for each Jordan block size \(s\), the binomial
  expansion of
  \((\mu I+N)^m\) bounds the corresponding block by
  \(C_{\mu,s}(m+1)^{s-1}|\mu|^m\), with \(C_{\mu,s}\) computed from a
  transition matrix between the rational basis and the algebraic Jordan
  basis. Put \(a=|\mu|/\theta_r<1\). Since
  \(x\mapsto (x+1)^{s-1}a^x\) is decreasing for
  \(x+1\ge (s-1)/\log(1/a)\), the finite maximum
  \[
    C(\mu,\theta_r,s):=
    \max_{0\le m\le N_{\mu,s}} (m+1)^{s-1}
  ...
  satisfies
  \[
    (m+1)^{s-1}|\mu|^m\le C(\mu,\theta_r,s)\theta_r^m
    \qquad(m\ge0).
  \]
