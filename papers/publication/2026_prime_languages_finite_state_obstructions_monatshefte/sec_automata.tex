...
\begin{proof}
  ...
  numerical inequality; any later published explicit-PNT estimate with
  comparable constants may be substituted without changing the
  argument. Since \(\log 5393>2\), for \(x\ge5393\) we have
  \[
...
\begin{lemma}[Certified spectral decay for a fixed residue class]
  ...
  \(\Pi_{j,t}=\mathbf 1\pi_{j,t}\), plus rational resolvent bounds and
  a monotonicity threshold proving the displayed estimate.
\end{lemma}
...
\begin{proof}
  ...
  isolate their complex roots. If the finite list of roots is empty,
  take \(\theta_r=1/2\). Otherwise choose a rational
  \(\theta_r\in(0,1)\) such that all eigenvalues of \(T\) have modulus
  \(<\theta_r\) and all eigenvalues of every \(E_{j,t}\) have modulus
  \(<\theta_r^{d_j}\). The strict comparisons are certified by the
  isolating intervals. This gives the spectral-radius part of the
  ...
  estimates give rational \(K_r\) and \(N_r\). The finitely many earlier
  powers in the residue class are checked directly by rational matrix
  powering and, if necessary, \(K_r\) is enlarged. Thus the certificate
  verifies the displayed inequality for all
...
\begin{theorem}[Algorithmic fixed-DFA prime-slice obstruction]
  ...
  \begin{enumerate}[label=\textup{(\alph*)}]
    ...
    \Cref{lem:effective-spectral-constants}. These data are enough to
...
\begin{proof}
  ...
  The periodic Perron--Frobenius entry asymptotic is sharper than mere
  eventual positivity: if \(q_N\in C_i\), \(f\in C_{i+r}\), and \(h,\nu\)
  are positive right and left Perron vectors on the corresponding
  cyclic classes, then
  \[
    \lambda^{-(kd_N+r)}(B_N^{kd_N+r})_{q_N f}
      \longrightarrow h_f\nu_{q_N}>0 .
  \]
  Hence \((S_r)_{q_N f}>0\), and therefore
...
\begin{proof}
  ...
  For completeness we spell out the reduction used below.
