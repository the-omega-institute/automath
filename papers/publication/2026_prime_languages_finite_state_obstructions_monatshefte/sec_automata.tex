...
\begin{lemma}[Explicit prime-counting input]
  ...
  \[
    X_0=5393,\qquad C_{\pi,1}=2,\qquad C_{\pi,2}=2.
  \]
  For every \(x\ge X_0\),
  \[
...
\begin{proof}
  ...
  \cite{RosserSchoenfeld1962}. Dusart's explicit upper estimate gives
  \[
    \pi(x)\le
    ...
    \qquad (x>1)
  \]
  by Dusart \cite[Theorem~1.10]{Dusart2010}. We use this source only
  for the displayed explicit numerical inequality. This is enough for
  the present lemma because no later argument depends on sharp
  constants; any published explicit-PNT estimate with comparable
  constants may be substituted without changing the argument. Since
  \(\log 5393>8\), for \(x\ge5393\) we have
  \[
