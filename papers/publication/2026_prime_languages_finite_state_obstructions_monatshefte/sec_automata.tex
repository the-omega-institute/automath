...
\begin{lemma}[Substochastic product expansion]
  ...
  \(\mathcal A\) over the same alphabet there exist an integer
...
\begin{proof}
  Form the synchronous product automaton
  ...
  \Cref{thm:dfa-density-dichotomy} to \(\widehat P\). After restricting
  the resulting expansion to the \(Q'\times Q'\) block, there are an
  integer \(p'\ge 1\), non-negative matrices
  \(S_0,\dots,S_{p'-1}\), a constant \(C>0\), and \(\eta\in(0,1)\) such
  that
  \[
    P'^m=S_{m\bmod p'}+E_m,
    \qquad
