...
\begin{proof}
  ...
  The standard finite-chain block formula writes \(P^m-S_r\), on
  \(m\equiv r\bmod D_A\), as a finite sum of products involving powers
  of \(T\), powers of the \(E_{j,t}\), and fixed rational connecting
  matrices. The resolvent bounds above dominate every direct block
  product by a rational multiple of \(\tau_r^m\). A
  transient-to-recurrent term with one entrance time is bounded by a
  rational multiple of
  \[
    \sum_{\ell=0}^{m-1}\tau_r^\ell \tau_r^{m-1-\ell}
    =m\tau_r^{m-1}.
  \]
  The finitely many higher-order block-chain convolutions that occur in
  the canonical block formula are similarly bounded by
  \(C_rm^{J_r}\tau_r^m\), for effectively computed rational \(C_r>0\)
  and integer \(J_r\ge0\), after increasing \(C_r\) to absorb the
  harmless factor \(\tau_r^{-1}\) from one-step entrance terms. This is
  the only polynomial loss retained in the certificate: the block and
  convolution estimates are first made at the strict rate \(\tau_r\),
  and the final advertised rate \(\theta_r\) is introduced only after
  choosing \(\theta_r\in(\tau_r,1)\). Since
  \(\tau_r/\theta_r<1\), the rational sequence
  \(m^{J_r}(\tau_r/\theta_r)^m\) is eventually decreasing and tends to
  \(0\). A finite rational search therefore produces \(K_r\) and
  \(N_r\) such that
  \[
...
\begin{theorem}[Algorithmic fixed-DFA prime-slice obstruction]
  ...
  \begin{enumerate}[label=\textup{(\roman*)}]
    ...
    class \(m\equiv r\pmod {D_A}\), together with the coefficient bound
    proving
    \[
      \|P^m-S_r\|_\infty\le C_rm^{J_r}\tau_r^m
      \qquad(m\equiv r\pmod {D_A}).
    \]
    The verifier obtains this bound by multiplying the listed rational
    connector matrices and the recorded resolvent bounds in the
    \(\ell^\infty\) operator norm, i.e. maximum absolute row sum.
