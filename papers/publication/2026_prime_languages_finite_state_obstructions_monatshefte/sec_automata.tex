...
\begin{proof}
  ...
  intervals we compute a rational \(\delta>0\) such that
  \[
    |\det(zI-A)|\ge\delta\qquad(|z|=\gamma).
  \]
  Indeed, the product formula for the determinant and the strict root
  separation from the circle \(|z|=\gamma\) give such a lower bound.
  The adjugate entries of \(zI-A\) are polynomials in \(z\), so their
  absolute values on the same circle have an elementary rational upper
  bound \(U\) obtained from the coefficients of \(A\). Cauchy's formula
...
\begin{theorem}[Algorithmic fixed-DFA prime-slice obstruction]
  ...
  \begin{enumerate}[label=\textup{(\roman*)}]
    ...
    checks, using the same isolating intervals and rational interval
    arithmetic, that
    \[
      |\det(zI-A)|\ge\delta_A\quad(|z|=\gamma_A),
      \qquad
    ...
    verifier further splits \(\ell=u+D_Aq\), records the exact affine
    exponent \(h(q)=q+h_0\) attached to the relevant return-error block,
    and checks only the resulting finite residue list
    \[
      \sum_{q=0}^{Q(m,u)}
        X_{u}T^{D_Aq}Y_{u}E_{j,t_u}^{q+h_0}Z_{u},
    \]
    with all matrices \(X_u,Y_u,Z_u\) rational and with the finitely
    many exceptional \(h_0<0\) terms moved to the direct finite-check
    list. This is the normal form for every convolution term.
    ...
    \(C\,m\tau_r^{m-1}\), and for every tail it lists the rational
    coefficient in
