...
\begin{lemma}[Substochastic product expansion]
  ...
  \]
  This is a background product-expansion lemma for the fixed trim
  strongly connected presentation setting; it supplies constants for
  \Cref{thm:parry-density} and not a broader multicomponent theorem.
\end{lemma}
...
\begin{remark}
  ...
  dominant root of the corresponding recurrence. This is only a
  deterministic, trim, strongly connected presentation statement. We do
  not claim an extension to broader Parry systems, to presentations with
  multiple strongly connected components, or to nondeterministic or
  weighted automata; see \cite{CharlierRampersadRigoWaxweiler2010} for
  examples where multicomponent behaviour genuinely occurs.
\end{remark}
...
\begin{remark}[Relation to conditional-density literature]
  ...
  as background statements for the trim, strongly connected deterministic
  setting. The contribution is the conversion of this residue-class
  dichotomy into the finite-budget prime obstructions recorded in
  \Cref{cor:relative-finite-budget-obstruction,cor:dfa-prime-symmetric-diff,cor:dfa-prime-recall-precision}.
\end{remark}
