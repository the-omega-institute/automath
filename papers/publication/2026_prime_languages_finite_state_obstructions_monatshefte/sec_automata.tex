...
\begin{lemma}[Explicit prime-counting input]
  ...
  There are absolute effectively computable constants \(X_0,K_1,K_2>0\)
  such that for every \(x\ge X_0\),
  \[
  ...
  \]
  For instance, Rosser--Schoenfeld
  \cite{RosserSchoenfeld1962} gives one such choice.
\end{lemma}

\begin{proof}
  Rosser--Schoenfeld give explicit upper and lower estimates for
  \(\pi(x)\) of the form \(x/\log x\) multiplied by rational functions
  of \(1/\log x\) for all sufficiently large \(x\). Increasing \(X_0\),
  if necessary, absorbs the finitely many exceptional small values and
  gives the two displayed inequalities with absolute constants
  \(K_1,K_2\).
\end{proof}
...
\begin{proof}
  ...
  \[
    K_1\left(\frac{1}{mL_b}+\frac{b}{(m-1)^2L_b}\right)
      +\frac{2m}{b^m}
    \le
    \frac{1}{2L_b}
...
\begin{theorem}[Effective fixed-DFA prime-slice obstruction]
  \label{thm:effective-fixed-dfa-prime-obstruction}
  Fix \(b\ge2\) and a complete DFA
  \(\mathcal A=(Q,\Sigma_b,\delta,q_0,F)\). From the transition table of
  \(\mathcal A\) one can compute the following finite output:
  the recurrent classes of the Markov chain \(P=B/b\), their periods
  \(d_i\), cyclic classes, stationary distributions, absorption
  probabilities from transient states, the common period
  \(p=\operatorname{lcm}_i d_i\), the residue constants
  \(c_r=\lim_{k\to\infty}u^{\mathsf T}P^{kp+r}v\), the alternatives
  \(c_r=0\) or \(c_r>0\), and constants
  \(C_r,D_r>0\), \(\eta_r\in(0,1)\), and \(M_r\). If the residues
  \(c_0,\dots,c_{p-1}\) are listed explicitly, this listing is part of
  the output. With this output, the following holds for every
  \(m\ge M_r\) satisfying \(m\equiv r\pmod p\).
  If \(c_r=0\), then
  \[
    a_m(\mathcal A)\le C_r\eta_r^m b^m
    \quad\text{and}\quad
  ...
  \]
  The arithmetic complexity is polynomial in the input size plus the
  size of the produced output. Equivalently, the compressed Markov-chain
  output above avoids charging the algorithm for explicitly printing
  \(p\) residue records when \(p=\operatorname{lcm}_i d_i\) is large.
\end{theorem}

\begin{proof}
  Compute the matrix \(B=\sum_{d\in\Sigma_b}T_d\), the stochastic matrix
  \(P=B/b\), the recurrent classes of \(P\), their periods, and the
  common period \(p\) as described after
  \Cref{thm:dfa-density-dichotomy}. Exact rational linear algebra gives
  the residue limits \(c_r\) and decides which of them vanish. The same
  finite-dimensional computation also gives a number
  \(\theta\in(0,1)\) and constants \(K_r\) such that
  \[
    \abs{a_m(\mathcal A)/b^m-c_r}\le K_r\theta^m
    \qquad(m\equiv r\bmod p).
  \]
  For example, one may put \(P\) in rational canonical or Jordan form
  over an algebraic extension, separate the peripheral eigenvalues from
  the remaining eigenvalues, and take any rational \(\theta\) strictly
  between the largest non-peripheral modulus and \(1\); standard
  eigenvalue isolation makes this effective. Equivalently, the same
  bound follows from the canonical transient/recurrent Markov-chain
  decomposition and explicit bounds for powers of the transient blocks.
  ...
  \[
    ...
    \qquad(m\ge M_b).
  \]
  If \(c_r=0\), take \(C_r=K_r\) and \(\eta_r=\theta\), then enlarge
  \(M_r\ge M_b\) until \(K_r\theta^m\le A_b/(2m)\) on the residue class.
  For \(m\ge M_r\),
  \[
  ...
  If \(c_r>0\), enlarge \(M_r\ge M_b\) until
  \(K_r\theta^m\le c_r/2\) and \(B_b/m\le c_r/4\) on the residue class.
  ...
  The complexity statement follows from the standard algorithms for
  strongly connected components, periods of strongly connected directed
  graphs, rational linear systems, and algebraic eigenvalue isolation in
  finite dimension. Explicitly printing \(c_0,\dots,c_{p-1}\) has cost at
  least proportional to \(p\); therefore the bit bound is polynomial in
  the input plus output size. If instead the output is kept in the
  compressed recurrent-class form from the statement, it consists of the
  class data, cyclic classes, stationary distributions, absorption
  probabilities, and the formula defining \(c_r\) from these data.
\end{proof}
...
\begin{corollary}[No simultaneous recall and precision for fixed DFA]
  ...
  \begin{enumerate}[label=\textup{(\alph*)}]
    ...
    \(m\equiv r\pmod p\).
  \end{enumerate}
  Consequently every DFA satisfies one of the following global
  alternatives:
  \begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{corollary}[Finite-budget obstruction in a trim presentation]
...
\noindent
This corollary is the finite-budget skeleton behind the prime-slice
results: the base-\(b\) and Zeckendorf prime slices are exactly the
