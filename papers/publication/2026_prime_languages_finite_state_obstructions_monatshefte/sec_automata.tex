...
\begin{proof}
  ...
  \(p:=\operatorname{lcm}(d_1,\dots,d_s)\). By the periodic
  Perron--Frobenius theorem for the recurrent blocks, each sequence
  \(C_i^{kp+r}\) converges exponentially fast as \(k\to\infty\), for
  ...
  classes. Consequently there are \(C>0\) and \(\theta\in(0,1)\) such
  that
  \[
    P^m=S_{m\bmod p}+E_m,
    \qquad
  ...
  class \(r\bmod p\), the case \(c_r=0\) contradicts the lower bound
...
\(d_i\), and hence \(p=\operatorname{lcm}(d_i)\), are obtained by the
standard linear-time graph algorithms. For each residue class \(r\bmod
p\), the limit
\[
  c_r=\lim_{k\to\infty}u^{\mathsf T}P^{kp+r}v
\]
...
limit of the rational matrices \(P^{kp+r}\). Thus the residue
...
\begin{lemma}[Explicit prime-counting input]
  ...
  \[
    X_0=5393,\qquad K_1=2,\qquad K_2=2.
  \]
  For every \(x\ge X_0\),
  \[
    \left|\pi(x)-\frac{x}{\log x}\right|
      \le K_1\frac{x}{(\log x)^2},
    \qquad
    \pi(x)\le K_2\frac{x}{\log x}.
  \]
\end{lemma}
...
\begin{proof}
  ...
  \[
    \pi(x)\le \frac{x}{\log x-1}
    \qquad (x\ge5393)
  \]
  \cite{Dusart2010}. Since \(\log 5393>2\), for \(x\ge5393\) we have
  \[
    0\le \pi(x)-\frac{x}{\log x}
    \le \frac{x}{\log x-1}-\frac{x}{\log x}
    =\frac{x}{\log x(\log x-1)}
    \le 2\,\frac{x}{(\log x)^2},
  \]
  and also
  \[
...
\begin{proof}
  ...
  \[
    \pi(b^m)-\pi(b^{m-1})+O(1),
  \]
  since only the two endpoints can differ. For \(m\ge M_b\), the
  explicit estimate in \Cref{lem:explicit-pnt-input} gives
  \[
    \pi(b^m)-\pi(b^{m-1})
    \ge
    \frac{b^m}{m\log b}
    -\frac{b^{m-1}}{(m-1)\log b}
    -K_1\frac{b^m}{m^2(\log b)^2}
    -K_1\frac{b^{m-1}}{(m-1)^2(\log b)^2}.
  \]
  Since
  \[
    \frac{b^m}{m\log b}
  ...
  \[
    \abs{\mathcal P_m^{(b)}}\le\pi(b^m)
      \le K_2\frac{b^m}{m\log b},
  \]
  so \(B_b:=K_2/\log b\) is admissible for all \(m\ge M_b\).
\end{proof}
...
\begin{theorem}[Algorithmic fixed-DFA prime-slice obstruction]
  ...
  \begin{enumerate}[label=\textup{(\alph*)}]
    ...
    \[
      ...
      :=\sum_{\ell\ge0}
        ...
        S_{j,t-r-1-\ell}\mathbf 1 ,
    \]
    where indices are taken modulo \(d_j\) and
    \(S_{j,u}\) is the cyclic projection onto \(R_{j,u}\).
    The displayed series is evaluated exactly as a finite family of
    linear solves after grouping \(\ell\) modulo \(d_j\).
    \item For each residue \(r\) modulo
    \(p=\operatorname{lcm}_j d_j\), the compressed formula
    \[
      c_r=
  ...
  \]
  whenever \(\mathrm{Prec}_m\) is defined, and
  \[
    \abs{(L(\mathcal A)\cap\Sigma_b^m)\triangle\mathcal P_m^{(b)}}
    \ge \frac{c_r}{4} b^m .
  \]
  ...
  \]
  In the rational bit model, the graph decomposition and all rational
  linear solves are polynomial in the input size plus the size of the
  produced certificate. The optional full list
  \(c_0,\dots,c_{p-1}\) has output size at least \(p\); no polynomial
  bound in the input size alone is asserted for printing that list.
  The constants \(K_r,\theta_r,M_r\) are represented by algebraic
  numbers together with isolating intervals, and their real
  bit-complexity is polynomial in the input size, the certificate size,
  and the requested output precision.
\end{theorem}
...
\begin{proof}
  ...
  their cyclic decompositions, and the common period \(p\) by standard
  ...
  \(H_{j,t}^{(r)}\) by \(\ell\equiv u\pmod{d_j}\). Each group is a
  rational expression in
  \[
    (I-P_{TT}^{d_j})^{-1}P_{TT}^uP_{T,R_j}S_{j,t-r-1-u}\mathbf 1,
  \]
  ...
  of \(u^{\mathsf T}P^m v\) along \(m\equiv r\pmod p\): after absorption
  into \(R_{j,t}\), \(r\) further steps place the recurrent chain in
  \(R_{j,t+r}\), and the limiting conditional distribution there is
  \(\pi_{j,t+r}\).

  It remains to record a verifiable construction of
  \(K_r,\theta_r\). Put \(P\) in block form with \(P_{TT}\) and the
  primitive return blocks \(P^{d_j}|_{R_{j,t}}\). Let \(\Theta\) be the
  finite set consisting of the moduli of all eigenvalues of
  \(P_{TT}\) and of all non-unit eigenvalues of the return blocks. Every
  element of \(\Theta\) is an algebraic number effectively represented
  by its minimal polynomial and an isolating interval. Since the
  transient spectral radii and the non-peripheral recurrent moduli are
  \(<1\), choose an algebraic or rational
  \[
    \theta_r\in\bigl(\max\Theta,1\bigr).
  \]
  In Jordan form, each non-peripheral block contributes terms bounded by
  \(C_\mu m^{s_\mu-1}|\mu|^m\), where \(s_\mu\) is the largest Jordan
  block size for \(\mu\). Real-root isolation and exact algebraic
  arithmetic give explicit constants \(C_\mu\). For this chosen
  \(\theta_r\), compute an integer \(N_\mu\) with
  \(m^{s_\mu-1}|\mu|^m\le\theta_r^m\) for all \(m\ge N_\mu\), and enlarge
  the coefficient to cover the finitely many \(m<N_\mu\). Summing these
  finitely many block bounds gives \(K_r\) and
  \[
    \abs{a_m(\mathcal A)/b^m-c_r}\le K_r\theta_r^m
    \qquad(m\equiv r\bmod p).
  \]
  ...
  \[
    ...
    \qquad(m\ge M_b).
  \]
  If \(c_r=0\), take \(M_r\ge M_b\) to be any integer in the residue
  class tail satisfying
  \[
    K_r\theta_r^m\le \frac{A_b}{2m}
    \qquad(m\ge M_r,\ m\equiv r\bmod p).
  \]
  This threshold is computable by real algebraic interval arithmetic:
  check the finitely many values before monotonicity of
  \(mK_r\theta_r^m\) begins, and then solve the remaining one-sided
  inequality by bisection on isolating intervals.
  For \(m\ge M_r\),
  \[
    a_m(\mathcal A)\le \frac{A_b}{2}\frac{b^m}{m},
  \]
  so the symmetric difference is at least
  \(A_b b^m/(2m)\). Thus \(D_r=A_b/2\) is admissible in the sparse case.

  If \(c_r>0\), take \(M_r\ge M_b\) so that
  \[
    K_r\theta_r^m\le c_r/2,
    \qquad
    B_b/m\le c_r/4
  \]
  for all larger \(m\equiv r\bmod p\). This is computed in the same
  real bit model.
  ...
  \[
    ...
    \ge \frac{c_r}{4}b^m.
  \]
  Thus \(D_r=c_r/4\) is admissible in the positive-density case.
  ...
  final all-large-\(m\) constant after replacing \(D_r b^m\) by the
  weaker \(D_r b^m/m\) in the positive-density residue classes.
  The complexity statement follows from polynomial-time strongly
  connected component and period algorithms, polynomial bit-complexity
  rational linear algebra, and standard real-algebraic eigenvalue
  isolation with requested precision. The certificate is compressed by
  recurrent class and cyclic class. If the user asks for an explicit
  printed table of all \(c_r\), that table is charged to the output
  size, which is necessary because \(p\) may be super-polynomial in the
  number of states.
\end{proof}
...
\begin{proof}
  ...
  Let \(p\), \(c_0,\dots,c_{p-1}\), and \(\theta\) be supplied by
  \Cref{thm:dfa-density-dichotomy}. By the pigeonhole principle, some
  residue class \(r\bmod p\) contains infinitely many lengths from
...
\begin{proof}
  ...
  \(B_b/(pk+r)\le c_r/4\). Then for all such \(k\),
  \[
    \abs{(L(\mathcal A)\cap\Sigma_b^{pk+r})\triangle \mathcal P_{pk+r}^{(b)}}
...
\begin{corollary}[No simultaneous recall and precision for fixed DFA]
  ...
  \Cref{prop:dfa-quantitative-obstruction}, let \(p\) and
  \(c_0,\dots,c_{p-1}\) be the period and residue constants supplied by
  \Cref{thm:dfa-density-dichotomy}. For each residue class
  \(r\bmod p\) exactly one of the following alternatives holds:
  \begin{enumerate}[label=\textup{(\alph*)}]
    ...
    \(m\equiv r\pmod p\) for which \(a_m(\mathcal A)>0\).
  \end{enumerate}
  Consequently every DFA satisfies one of the following explicitly
  global alternatives:
  \begin{enumerate}[label=\textup{(\roman*)}]
    \item \(\mathrm{Rec}_m\to 0\) exponentially fast.
    \item There exists at least one residue class
    \(r\bmod p\) with \(c_r>0\) such that
    \(\mathrm{Prec}_m\le C_{\mathcal A,b}/m\) for all sufficiently large
    \(m\equiv r\pmod p\) for which precision is defined.
  \end{enumerate}
...
\begin{proof}
  ...
  irreducible non-negative matrix of period \(d_N\), the cyclic
  decomposition \(C_0,\dots,C_{d_N-1}\) has the property that
  \(B_N\) maps \(C_j\) only to \(C_{j+1}\), and for each \(j\) the
  diagonal block of \(B_N^{d_N}\) on \(C_j\) is primitive
  \cite[Sec.~4.5]{LindMarcus1995}; equivalently, all sufficiently long
  paths in an admissible residue class occur between the corresponding
  cyclic classes. Therefore
  \((B_N^{kd_N+r})_{q_N f}>0\) for all sufficiently large \(k\), and
  the limit
  \[
    (S_r)_{q_N f}
    =\lim_{k\to\infty}\lambda^{-(kd_N+r)}
      (B_N^{kd_N+r})_{q_N f}
  \]
  is strictly positive. Hence \(d_r=u_N^{\mathsf T}S_rv_N>0\).
...
\begin{lemma}[Substochastic product expansion]
  ...
  \(p'\ge 1\), non-negative constants \(e_0,\dots,e_{p'-1}\), and
...
\begin{proof}
  ...
  holds with \(p'=1\) and \(e_0=0\). Otherwise let \(Q'\) be the state
  ...
  \(p'=\operatorname{lcm}(d_1,\dots,d_s)\), the entrywise limits
  \[
    S_r:=\lim_{k\to\infty}P'^{kp'+r}
    \qquad(0\le r<p')
  \]
...
The preceding theorem is the standard conditional-density conclusion
for a deterministic trim strongly connected presentation, specialized
to the notation used here; compare
...
\begin{proof}
  Let \(p\), \(c_0,\dots,c_{p-1}\), and \(\theta\in(0,1)\) be supplied
  by \Cref{thm:parry-density}. Enlarge \(p\), if necessary, so that it
  is also a multiple of the period in \Cref{lem:trim-density}. Then for
  every residue class \(r\bmod p\) containing infinitely many lengths
  ...
  large \(m\equiv r\bmod p\) with \(z_m>0\). Since
