...
\begin{proof}
  ...
  \cite{RosserSchoenfeld1962}. Dusart's explicit upper estimate
  \cite[Theorem~1.10]{Dusart2010} gives
  \[
    \pi(x)\le
...
\begin{proof}
  Put \(L_b:=\log b\). Choose \(M_b\ge2\) so that
  \(b^{M_b-1}\ge X_0\) and, for every \(m\ge M_b\),
  \[
  ...
  \]
  This is an effective finite search, because the left-hand side is
  \(O(1/m)\) and the factor on the right tends to
  \((1-1/b)/(2L_b)>0\). Concretely, one enumerates
  \(M=2,3,\ldots\), tests the displayed rational-logarithmic inequality
  for all \(m\ge M\) after the elementary monotonicity threshold of its
  two sides, and then directly checks the finitely many preceding
  \(m\)'s. This produces a reproducible value of \(M_b\), not an
  unspecified asymptotic threshold.
  ...
  \]
  After increasing \(M_b\) once more so that
  \(1-m/(b(m-1))\ge (1-1/b)/2\), one may take
  \[
