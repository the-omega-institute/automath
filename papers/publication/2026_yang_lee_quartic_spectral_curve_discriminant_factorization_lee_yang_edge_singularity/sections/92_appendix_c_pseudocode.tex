...
For the figure in Section~\ref{sec:verification}, the implementation uses IEEE double precision (\texttt{numpy.float64}/\texttt{complex128}) together with \texttt{numpy.roots} for both the quartic spectral polynomial and the polynomials \(Z_m(y)\).
...
For the center panel one inserts the known multiplicity-two contribution at \(y=-1\) explicitly after factoring out \((y+1)^2\).
