...
$q=\prod_{C\in\Pi(q)}q_C$ indexed by the connected components of its support
hypergraph, where $\Pi(q)$ is a partition of $\mathcal P(n)$ and
$q_C\in B_{n_C}$ is connected. Conversely, products of connected block
elements over any partition of $\mathcal P(n)$ lie in $B_n$. Hence
\[
A(n)=\sum_{\Pi\in\operatorname{Part}(\mathcal P(n))}
\prod_{C\in\Pi}A^{\mathrm{conn}}(n_C),
\]
with the analogous formula for $\#\mathcal M_n$. These statements are proved
in Theorems~\ref{thm:birth-connected-factorization}
and~\ref{thm:mingenerator-connected-factorization}.
\end{theorem}
...
The empty hypergraph does not occur for $n\ge 3$; at the boundary index
$n=2$ we set $A^{\mathrm{conn}}(2)=M^{\mathrm{conn}}(2)=0$. A one-vertex
support hypergraph is connected. Finally,
$M^{\mathrm{conn}}(n)$ denotes the number of connected elements of
$\mathcal M_n$.
\end{definition}
...
factors are coprime and the lcm property in Proposition~\ref{prop:rank-basics}
yields
...
the lcm property in Proposition~\ref{prop:rank-basics} gives
$\alpha(q_C)=n_C$ for each~$C$:
