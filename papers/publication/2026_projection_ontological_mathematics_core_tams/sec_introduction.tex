...
that the answer is unexpectedly rigid.  After an explicit affine permutation
...
partition constant of Sanna~\cite{Sanna2025}.  For $q\ge 2$ the Perron root of
the finite-state collision kernel equals $\lambda_q$.
\end{theoremB}

\begin{theoremC}
For $q\ge 2$, the constant $\lambda_q$ is the spectral radius of a nonnegative
integer matrix of dimension polynomial in~$q$; in particular, $\lambda_q$ is
an algebraic integer.  The pressure sequence $P_0:=\log\varphi$,
...
established the uniqueness of Fibonacci representations.  Chow and
...
Sanna~\cite{Sanna2025} proved that $\sum_{n<N}R(n)^q\asymp_q
