...
sequence to Sanna's partition constants and gives an independent
finite-window kernel realization of constants that Sanna had already proved
to be Perron roots and algebraic integers; the transferred pressure then
yields convexity, canonical concentration, and zero-temperature asymptotics.
Third, it extends Sanna's Table~1 from \(p\le 8\) to an audited arithmetic
window \(q=9,\dots,17\), in which the recurrence polynomials are
computationally certified to be irreducible with full symmetric Galois groups,
together with audited discriminant, linear-disjointness, and product
...
$k=|Q|(2^{L+1}-1)$, the raw collision matrix has at most $k^q$ states and its
permutation quotient has at most $\binom{k+q-1}{q}$ states.  The latter is
polynomial in $q$ only for fixed $Q,L$, not jointly in $q,L$.  In particular,
$\lambda_q$ is an algebraic integer.  The pressure sequence $p_0:=\log\varphi$,
...
Jones~\cite{ChowJones2024}.  Extending Sanna's table beyond its endpoint
$p=8$, the audited recurrence polynomials of $S_q(m)$ for $q=9,\dots,17$ are
computationally certified to be irreducible over~$\QQ$ with full symmetric
Galois groups on their roots (Section~\ref{sec:chebotarev}).  The same
criterion gives full symmetric groups for Sanna's nonlinear rows, so this is
a continuation of the earlier pattern rather than an exceptional Galois
phenomenon.  For $q\in\{12,13,14,15\}$ the discriminant audit indicates linear
disjointness and the corresponding product Chebotarev density.  Exact root
isolation in this window also singles out a negative real subdominant root of
each displayed recurrence polynomial.  Conditional on the
audited identification of those polynomials as the minimal recurrences of the
actual moment sequences, every $S_q(m)$ with $9\le q\le17$ then has an
eventually alternating second-order term.
...
Sanna~\cite[Thm.~1]{Sanna2025} proved both that
$\sum_{n<N}R(n)^q\asymp_q N^{(\log\lambda_q)/\log\varphi}$ and that
$\lambda_q$ is the Perron--Frobenius eigenvalue of the transition matrix of an
automaton constructed from parallel copies of Berstel's automaton; in
particular, he proved that $\lambda_q$ is an algebraic integer.  His Table~1
records $\lambda_p$ and its minimal polynomial for $p=1,\dots,8$.
Theorem~B transfers his constants to the collision moments, our collision
kernel gives an independent finite-window realization, and the audited
$q=9,\dots,17$ window extends his table and supplies the arithmetic analysis
not undertaken there.  The finite-state realization of
