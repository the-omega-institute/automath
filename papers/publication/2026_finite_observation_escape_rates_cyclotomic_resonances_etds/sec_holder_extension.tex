...
\begin{proof}
Boundedness is the usual Ruelle estimate.  If \(x,x'\) agree for \(N\)
symbols, their admissible safe preimages can be paired by the prepended
symbol.  The paired preimages agree for \(N+1\) symbols, while the
variation of \(e^\psi\) on such pairs is \(O(d_\theta(x,x'))\).  Hence
\[
  \norm{\mathcal L_{\psi,H}f}_\theta
  \leq C\norm{f}_\theta
\]
for a constant \(C\) depending only on \(Z,\psi,H\), and \(\theta\).
...
H\"older functions on the ambient symbolic metric space.  Moreover
\[
...
\]
The weighted sum of all \(n\)-step safe preimages is bounded uniformly in
the terminal point by \(C_1e^{nP_{Z_H}(\psi)}\): this is the same
bounded-distortion and finite-suffix comparison used in
Lemma~\ref{lem:holder-safe-word-pressure}.  Therefore
\[
  \norm{\mathcal L_{\psi,H}^n f}_\infty
  \leq
  C_2\theta^n e^{nP_{Z_H}(\psi)}\norm{f}_\theta .
\]
The H\"older seminorm satisfies the same bound.  To see this, pair
\(n\)-step preimages over two terminal points that agree for \(N\)
symbols.  The paired preimages agree for \(n+N\) symbols.  The difference
of the two summands is bounded by the preceding
\(\theta^{n-K}\)-smallness of \(f\), the \(\theta^{n+N}\)-smallness of
\(f(y)-f(y')\), and the usual Bowen distortion estimate for the
\(n\)-step weights.  After division by \(d_\theta(x,x')=\theta^N\) and
summation over the safe preimages, one obtains
\[
...
\[
  ...
  \longrightarrow 0
\]
now identifies the spectrum of \(\mathcal L_{\psi,H}\) outside the disk
of radius
\(\max\{\theta\lambda_H,r_H\}\) with the corresponding spectrum of the
survivor operator.  Hence \(\lambda_H\) is a simple isolated eigenvalue
of the open operator, and all remaining spectrum has modulus strictly
smaller than \(\lambda_H\).  The Riesz projection at \(\lambda_H\) is
rank one; writing it as
\(\Pi_H f=\eta_H(f)h_H\) gives the stated right eigenfunction, adjoint
eigenfunctional, normalization, and spectral decomposition.
\end{proof}
