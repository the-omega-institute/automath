...
cyclotomic resolvent statement.
...
eigenspace for \(T\).  The same reasoning excludes a generalized
eigenvector in the ambient space: its quotient would be a generalized
eigenvector for the survivor operator, whose leading eigenvalue is
simple, and any remaining vector in \(\mathcal E\) would contradict
\(\lambda_H\notin\spec(T_{\mathcal E})\).  The Riesz projection at
\(\lambda_H\) is therefore rank one.  If \(h_H\) spans its range and
