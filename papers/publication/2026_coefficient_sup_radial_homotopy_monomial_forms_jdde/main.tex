...
\title{Boundary trace rigidity and stability for coefficient-$L^\infty$ primitives on boxes}
\author{
...
principle. A companion cubical Stokes-inverse manuscript uses the same
rectangular-box calibration as a boundary readout endpoint for linear inverse
operators. In the present article the calibration is retained as a load-bearing
main theorem and is pushed further: we extract a sharp coefficient-$L^\infty$
...
the trace-kernel minimizer skeleton, and the sliced active-coordinate
...
Let $\theta\in C^1(I^n;\RR)$ and assume that $\cob\theta$ is Whitney
