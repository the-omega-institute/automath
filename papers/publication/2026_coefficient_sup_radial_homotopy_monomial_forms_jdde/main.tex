...
Let $1\le k\le n$ and let $K_{n+1}:=0$. Then
\begin{equation}
\label{eq:radial-homotopy-identity}
\dd K_k\omega+K_{k+1}\dd\omega=\omega
\qquad
(\omega\in\Omega^k_{\mathrm{sm}}(I^n)).
\end{equation}
If, in addition, $\omega$ is coordinate-monomial, then
\begin{equation}
\label{eq:sharp-homotopy-bound}
...
\begin{proof}
Let $H(t,x)=c+t(x-c)$ and write $H_t(x)=H(t,x)$. The operator in
Definition~\ref{def:homotopy-operator} is
\[
K_k\omega=
\int_0^1 H_t^*(\iota_{\partial_tH}\omega)\,\dd t,
\qquad
\partial_tH(t,x)=x-c.
\]
Indeed, the pullback $H_t^*$ contributes one factor $t$ for each of the
$k-1$ surviving differentials after contraction, giving the factor $t^{k-1}$
in \eqref{eq:K-definition}. Cartan's homotopy formula for the map
$H:[0,1]\times I^n\to I^n$ gives
\[
\frac{\dd}{\dd t}H_t^*\omega
=
H_t^*(\dd\iota_{\partial_tH}\omega+
\iota_{\partial_tH}\dd\omega).
\]
Integrating from $0$ to $1$ yields
\[
H_1^*\omega-H_0^*\omega=\dd K_k\omega+K_{k+1}\dd\omega .
\]
Here $H_1$ is the identity and, because $k\ge1$, $H_0^*\omega=0$. This proves
\eqref{eq:radial-homotopy-identity}; when $k=n$ the term
$K_{n+1}\dd\omega$ is zero because $\dd\omega$ is an $(n+1)$-form.

Now let $\omega=f\,\dd x_I$ be coordinate-monomial, with
$I=\{i_1<\cdots<i_k\}$. The coefficient formula
\eqref{eq:K-coefficient-formula} has exactly one contribution to each output
coefficient $\dd x_{I\setminus\{i_r\}}$. Hence, for each $x\in I^n$,
\[
\abs{(K_k\omega)_{I\setminus\{i_r\}}(x)}
\le
\int_0^1 t^{k-1}\abs{x_{i_r}-\tfrac12}\,
\norm{\omega}_{\coeff,\infty}\,\dd t
\le
\frac1{2k}\norm{\omega}_{\coeff,\infty}.
\]
Taking the maximum over $r$ and the supremum over $x$ proves
\eqref{eq:sharp-homotopy-bound}.

For sharpness of this estimate, take
\[
\omega_0:=\dd x_1\wedge\cdots\wedge\dd x_k.
\]
It is closed and coordinate-monomial, $\norm{\omega_0}_{\coeff,\infty}=1$, and
\eqref{eq:explicit-minimizer} gives
\[
\norm{K_k\omega_0}_{\coeff,\infty}=\frac1{2k}.
\]

It remains to prove the operator-independent lower bound. Let $\eta$ be any
