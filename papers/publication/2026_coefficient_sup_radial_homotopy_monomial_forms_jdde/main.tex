...
\begin{proof}
If $\abs{A}=0$, then the claim is trivial. If $\abs{A}>0$, let $a$ and $b$ be
the left and right essential endpoints of $A$. The zero extension has to pass
from the value $0$ outside $[0,L]$ to the value $1$ on the essential interior of
$A$, and then back from $1$ to $0$ after the right endpoint. These two changes
are counted in the total variation on $\RR$, including the cases $a=0$ or
$b=L$ where the jump is an endpoint jump of the zero extension. Hence
$|D\widetilde{\mathbf 1_A}|(\RR)\ge2$. Since $\abs{A}\le L$, this implies
...
\abs{\eta(F_{j,\lambda})}/A_j\le M$. Hence $M\ge m_R$.
