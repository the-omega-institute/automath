...
$\norm{\eta}_{\coeff,\infty}-m(R)$. The resulting boundary error satisfies a necessary
capped mean-zero trace skeleton: its positive and negative variations agree,
and all exact minimizers differ from $\eta_R$ precisely by closed perturbations
with zero coordinate-face trace which remain in the same coefficient box. A
slicing argument extends the readout mechanism to constant coordinate-monomial
forms on higher-dimensional boxes. On one cube, the oriented cubical coboundary
gives the corresponding discrete boundary readout, and the Whitney construction
gives the matching sharp decomposition analogue.
\end{abstract}
...
perturbation, and the signed near-minimizer error is a capped mean-zero
boundary function. Thus the rectangular readout theorem is not used merely as
an isolated sharp constant statement; it is the calibration input for a
rigidity and stability skeleton on the quotient by the zero-trace kernel. On
...
\begin{theorem}[Necessary trace-error skeleton and minimizer kernel]
...
\]
Then $\delta\ge0$ and the signed trace error satisfies the following necessary
skeleton conditions
\begin{equation}
...
Theorem~\ref{thm:box-boundary-stability}. By the divergence theorem, or
equivalently by summing the signed face-trace coefficients in the proof of
that theorem,
\[
\sum_{j=1}^k\sum_{\lambda\in\{0,L_j\}}
...
\]
which is exactly \eqref{eq:trace-skeleton-cap}. The mean-zero identity implies
