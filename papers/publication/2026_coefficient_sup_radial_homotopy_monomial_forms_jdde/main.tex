...
$\norm{\eta}_{\coeff,\infty}-m(R)$. The resulting boundary error is an exact
...
end-point jumps at $0$ and $L_j$ are included in $P(E_{x'}^j;\RR)$. For every
measurable $A\subset[0,L_j]$ of finite perimeter, this one-dimensional perimeter
satisfies
\[
...
(A\subset[0,L_j]),
\]
because a non-null subset of the interval has at least two boundary jumps after
extension by zero, while the right-hand side is at most $2$. Applying this to
almost every slice and using \eqref{eq:box-sliced-variation} and Fubini yields
\[
\int_{\partial E}\abs{(\nu_E)_j}\,\dd\Haus^{k-1}
\ge
\frac{2}{L_j}\Vol(E).
...
\begin{theorem}[Trace-kernel skeleton for box minimizers]
...
\]
Then $\delta\ge0$ and the signed trace error satisfies the exact skeleton
conditions
\begin{equation}
...
\]
Then $d\eta_{\cell,R}=\Theta_R$ and
$\norm{\eta_{\cell,R}}_{\cell,L,\infty}=m_R$. Moreover, if
$\eta\in C^{k-1}(R;\RR)$ satisfies $d\eta=\Theta_R$ and
...
minimizing cochain with $d\eta=\Theta_R$ has the canonical weighted boundary
values
\[
...
$d\eta_{\cell,R}=\Theta_R$. Its weighted face norm is plainly $m_R$.

Now let $d\eta=\Theta_R$. Since $\eta(\partial R)=\abs{R}$, we have
\[
\sum_{j=1}^k\sum_{\lambda\in\{0,L_j\}}
...
and
\[
\mathcal I(\dd\alpha)=d\mathcal I(\alpha)
\qquad
(\alpha\in\Omega^k_{\mathrm{sm}}(I^n)),
\]
where the second identity is read in degree $k+1$. Also,
\[
...
The identity $\mathcal I\dd=d\mathcal I$ is Stokes' theorem with the same
orientation convention. If $\sigma$ is an oriented $(k+1)$-cell, then
\[
...
$(d\theta)(\sigma)=\theta(\partial\sigma)$.
