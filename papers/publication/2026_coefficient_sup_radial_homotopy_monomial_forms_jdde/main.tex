...
$\norm{\eta}_{\coeff,\infty}-m(R)$. The resulting boundary error is exactly
confined by a sharp capped mean-zero trace skeleton: every primitive produces
such a skeleton, the skeleton already implies the optimal trace-variation
bound, and no smaller pointwise cap or total-variation budget can hold in
general. All exact minimizers differ from $\eta_R$ precisely by closed
perturbations with zero coordinate-face trace which remain in the same
coefficient box. A slicing argument extends the readout mechanism to the active
coefficient of constant coordinate-monomial forms on higher-dimensional boxes;
inactive face components are not part of this inverse datum, and this limitation
is sharp. On one cube, the oriented cubical coboundary gives the corresponding
discrete boundary readout, and the Whitney construction gives the matching
sharp decomposition analogue.
\end{abstract}

\bigskip
\noindent\textbf{2020 Mathematics Subject Classification.} Primary 58A12; Secondary 35R30, 49Q20, 65N30.

\noindent\textbf{Keywords.} inverse problem, stability estimate,
differential forms, homotopy operator, cubical cochains, Whitney forms.
...
yes, and the determination is quantitative. In this terminology, the
measurement map studied here sends an interior norm saturation gap, such as
$\norm{\eta}_{\coeff,\infty}-m(R)$, to the admissible boundary trace error
$\Tr_{\partial R}(\eta-\eta_R)$. The inverse question is whether small, or
zero, interior saturation gap forces small, or zero, boundary error in a
...
The analysis has three components. First, we identify the class on which the
...
while near-minimizers determine that trace with linear stability. The local
increment used in this paper is the associated trace-kernel classification:
after the common rectangular-box calibration theorem fixes the canonical trace,
the only remaining exact-minimizer freedom is a closed zero-face-trace
perturbation. For nonzero deficit we prove the sharp necessary boundary
skeleton seen by this calibration: the signed trace error is capped, has zero
mean, factors through a nonnegative saturation-deficit density, and has the
resulting linear variation budget. We do not use this necessary skeleton as a
stand-alone realization theorem for arbitrary prescribed boundary errors. Thus
the rectangular readout theorem is not used merely as an isolated sharp
constant statement; it is the calibration input for a rigidity and stability
skeleton on the quotient by the zero-trace kernel. On
the unit cube this gives the explicit boundary formula
\[
...
estimates for differential forms are also connected to $L^1$-Poincare and
$L^\infty$ primitive inequalities, including work of Baldi--Franchi--Pansu
\cite{BaldiFranchiPansu2019,BaldiFranchiPansu2020} and
Gol'dshtein--Dubrovskiy~\cite{GoldshteinDubrovskiy2009}. On the anisotropic side,
...
principle. Rather, it is the use of the rectangular-box calibration as a
boundary-readout endpoint for linear inverse operators. In the present article
the calibration is retained as a load-bearing main theorem and is pushed
further: we extract a sharp coefficient-$L^\infty$
estimate on the restricted class $\omega=f\,\dd x_I$, identify the
combinatorial obstruction beyond that class, prove quantitative boundary
determination on boxes, classify the quotient of all smooth minimizers by the
closed zero-face-trace kernel, and give the corresponding one-cell cubical
...
coordinate-monomial definitions and the centered homotopy operator lead to the
...
the box self-Cheeger computation, the boundary readout theorem on boxes, the
trace-kernel description of minimizers, and the sliced active-coordinate
extension. These continuous results are then used as input for the one-cube
cubical boundary readout theorem and the Whitney decomposition results below.
...
and a small saturation gap bounds the trace error. Section~5 gives the one-cell
cochain form of this mechanism. A global cubical complex requires compatibility
of the local homotopies across adjacent cells and is not proved here.
...
statement in Proposition~\ref{thm:homotopy-decomposition} for $k=1$.
\end{proof}
...
\]
In the general-domain statements in this section the ambient dimension is the
displayed dimension $n$. In the rectangular-box statements the ambient dimension
is the box dimension $k$; the same notation $P_1$ and $C_\infty$ is used with
that displayed ambient dimension.
...
\begin{proof}
We use the standard one-dimensional structure theorem for sets of finite
perimeter: after modification on a null set, $A$ is a finite disjoint union of
open intervals relative to $[0,L]$, and the variation of the zero extension to
$\RR$ is the number of endpoints of these intervals counted in $\RR$; endpoints
lying at $0$ or $L$ are counted because the function is zero outside $[0,L]$
\cite[Sec.~5.10]{EvansGariepy2015}. Thus, if $A$ is null, both sides vanish.
If $\abs{A}>0$, at least one interval occurs and contributes two endpoint jumps
to the zero extension, possibly at $0$ or $L$. Hence
...
\label{cor:box-minimizer-structure}
...
\eqref{eq:trace-skeleton-lower-cap}. The mean-zero identity implies
that the total positive and negative variations of the family
$(q_{j,\lambda})$ agree, hence the equality in
...
Corollary~\ref{thm:boundary-rigidity} follows when $M=1/(2k)$.
\end{corollary}
...
formula in Corollary~\ref{thm:boundary-rigidity}.
\end{corollary}
...
Proposition~\ref{thm:discrete-stokes-poincare}. Extending them to a multi-cell
cubical complex requires compatibility of the local homotopies and traces across
adjacent cells and is not asserted here.

Let $C^k(I^n;\RR)$ be the space of cubical $k$-cochains on the standard cell
decomposition of $I^n$, with coboundary $\cob$. For $\theta\in C^k(I^n;\RR)$ we
set
\[
...
\begin{corollary}[Hypercube one-cochain primitive correction]
...
Proposition~\ref{thm:discrete-stokes-poincare} with $k=1$:
\[
