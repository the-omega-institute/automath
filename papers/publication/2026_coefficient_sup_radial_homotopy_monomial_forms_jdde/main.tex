...
\frac1{2k}\norm{\omega}_{\coeff,\infty};
\]
these are the sharp linear-inverse estimates cited in the following remark. The
new assertion here is the same obstruction without linearity: if
...
\begin{remark}
The homotopy identity, the coefficient estimate, and sharpness for linear right
inverses are Theorem~3.1 of \cite{MaZhangCubicalStokes2026}. The increment in
Theorem~\ref{thm:optimal-continuous-inverse} is the same lower bound for an
arbitrary, not necessarily linear, right inverse on the closed
coordinate-monomial class.
\end{remark}

\begin{proof}
The cited theorem supplies the displayed identity, the displayed bound, and the
linear right-inverse sharpness statement. It remains only to remove the
linearity assumption in the lower bound. Take
\[
\omega_0:=\dd x_1\wedge\cdots\wedge\dd x_k.
\]
