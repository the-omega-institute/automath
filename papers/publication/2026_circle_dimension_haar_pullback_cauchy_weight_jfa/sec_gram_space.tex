...
\]
Hence $P_1*f=0$ also holds in~$\mathscr S'(\RR)$.  Taking Fourier
transforms in~$\mathscr S'(\RR)$ gives
\[
e^{-|\xi|}\widehat f(\xi)=\widehat{P_1*f}(\xi)=0.
\]
Since the multiplier $e^{-|\xi|}$ never vanishes, $\widehat f=0$ in
$\mathscr S'(\RR)$, hence $f=0$.  Thus
...
=\int_{\RR}
\bigl(h_t(\bar\gamma+ty)-g_t(\bar\gamma+ty)\bigr)\,t\,\dd y
