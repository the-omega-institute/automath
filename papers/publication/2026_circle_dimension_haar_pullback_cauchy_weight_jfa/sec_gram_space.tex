...
\]
Hence the distributional convolution $P_1*f$ also vanishes in
$\mathscr S'(\RR)$.  Taking Fourier transforms in~$\mathscr S'(\RR)$ gives
\[
e^{-|\xi|}\widehat f(\xi)=\widehat{P_1*f}(\xi)=0.
\]
Since $e^{-|\xi|}$ is a smooth strictly positive function,
multiplication by $e^{|\xi|}$ is again continuous on~$\mathscr S'(\RR)$.
Therefore $\widehat f=0$ in $\mathscr S'(\RR)$, hence $f=0$.  Thus
