...
\section{The Mode Gram Space and Centered Reconstruction}%
...
\begin{lemma}[Poisson convolution in $L^2(\omega)$]%
\label{lem:poisson-L2-omega}
Let $f\in L^2(\omega)$.  Then $f$ defines a tempered distribution via
\[
\langle f,\psi\rangle_{\mathscr S',\mathscr S}
:=\int_{\RR}f(y)\psi(y)\,\omega(\dd y)
\qquad(\psi\in\mathscr S(\RR)),
\]
the convolution $P_1*f$ is well-defined in $\mathscr S'(\RR)$, and its
Fourier transform satisfies
\[
\widehat{P_1*f}(\xi)=e^{-|\xi|}\widehat f(\xi)
\]
in $\mathscr S'(\RR)$.  In particular, $P_1*f=0$ in $\mathscr S'(\RR)$
if and only if $f=0$ in~$L^2(\omega)$.
\end{lemma}

\begin{proof}
Since $\omega(\dd y)=\pi^{-1}(1+y^2)^{-1}\dd y$, one has for
$\psi\in\mathscr S(\RR)$:
\[
\left|\int_{\RR}f(y)\psi(y)\,\omega(\dd y)\right|
\le \|f\|_{L^2(\omega)}\|\psi\|_{L^2(\omega)}.
\]
The map $\psi\mapsto\psi/(1+y^2)^{1/2}$ is continuous on
$\mathscr S(\RR)$, so the linear functional above is continuous and
$f\in\mathscr S'(\RR)$.

For $P_1*f$, write $g:=f\cdot\omega/\dd y=f/(\pi(1+y^2))$ so that
$g\in L^1(\RR)\cap\mathscr S'(\RR)$.  Then
$P_1*f$ in the distributional sense equals the Poisson regularization of
the tempered distribution~$f$.  Since $\widehat{P_1}(\xi)=e^{-|\xi|}$ is
a Schwartz multiplier, the identity
$\widehat{P_1*f}=e^{-|\xi|}\widehat f$ holds in $\mathscr S'(\RR)$.
Because $e^{-|\xi|}$ never vanishes, $\widehat{P_1*f}=0$ implies
$\widehat f=0$, hence $f=0$.
\end{proof}
...
identically.  By Lemma~\ref{lem:poisson-L2-omega}, $f\in L^2(\omega)$
defines a tempered distribution and
$\widehat{P_1*f}=e^{-|\xi|}\widehat f=0$ in $\mathscr S'(\RR)$.
Since $e^{-|\xi|}$ never vanishes, $\widehat f=0$, hence $f=0$.  Therefore $\mathcal S_0^\perp=\{0\}$ in $L^2_0(\omega)$, so
\[
...
\]
Now we verify the differentiation under the integral sign that produces
$B(t)$.  The derivative of the Poisson kernel in~$x$ is
\[
\partial_x P_t(x-\gamma)
=-\frac{1}{\pi}\frac{2t(x-\gamma)}{((x-\gamma)^2+t^2)^2}.
\]
For fixed $t>0$ and any $x,\gamma\in\RR$, set $u=(x-\gamma)/t$.  Then
\[
\left|\partial_x P_t(x-\gamma)\right|
=\frac{2}{\pi t^2}\frac{|u|}{(u^2+1)^2}.
\]
Since $\sup_{u\in\RR}|u|/(u^2+1)^2=3\sqrt{3}/16$ (attained at
$u^2=1/3$), one obtains the uniform bound
\[
\left|\partial_x P_t(x-\gamma)\right|
\le \frac{3\sqrt{3}}{8\pi t^2}
\qquad(x,\gamma\in\RR).
\]
This is integrable against~$\nu$ (which is a probability measure), so
...
\[
B(t)=\pi\,\partial_x h_t(x)\big|_{x=\bar\gamma}
=-2t\int_{\RR}\frac{\Delta}{(\Delta^2+t^2)^2}\dd\nu(\gamma).
\]
For the Fubini step, note that $B(s)$ involves $\Delta/(\Delta^2+s^2)^2$.
Since
\[
...
\le \frac{1}{2t},
\]
the double integral $\int_{\RR}\int_t^\infty|B(s)|\dd s\dd\nu(\gamma)$
converges.  Fubini's theorem therefore applies, and integrating
the identity
\[
-\int_t^\infty \frac{2s\,\Delta}{(\Delta^2+s^2)^2}\dd s
=\frac{\Delta}{\Delta^2+t^2}
...
Under the hypotheses of Proposition~\ref{thm:poisson-central-two-channel}, the
...
\begin{proposition}[Observation channels as evaluation functionals]%
...
Proposition~\ref{thm:poisson-central-two-channel} gives
\[
...
\begin{remark}\label{rem:cauchy-transform}
The previous proposition recovers the centred Cauchy transform on the
imaginary axis:
\[
G_{\nu_c}(\ii t)
...
parameter.  The proof of
Proposition~\ref{thm:poisson-central-two-channel} is better viewed as a
Laplace-uniqueness plus characteristic-function-inversion argument than
as a classical Stieltjes inversion; for the latter approach and its
relation to Cauchy-transform recovery see
\cite{Widder1941,Lukacs1970}.
\end{remark}
