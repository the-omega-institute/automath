...
$\operatorname{span}\{\Psi_\varepsilon:\varepsilon\in\RR\}$ is dense
in~$L^2_0(\omega)$.
...
the algebraic span of the secant family is dense in~$L^2_0(\omega)$.
Since $\mathcal S$ is exactly the Hilbert closure of that span, it follows
that $\mathcal S=L^2_0(\omega)$.
...
therefore $(A,B)$, determines $\phi_{\nu_c}(r)$ for every $r\ge 0$.
No evenness assumption is used here: every characteristic function
satisfies
\[
\phi_{\nu_c}(-r)=\overline{\phi_{\nu_c}(r)}
\qquad(r\in\RR).
\]
Hence the negative half-line is recovered from the positive half-line, so
the entire characteristic function is determined.  The centred law then
follows from the standard inversion theorem for characteristic functions
