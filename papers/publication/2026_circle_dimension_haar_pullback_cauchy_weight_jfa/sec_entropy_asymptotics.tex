...
Lemma~\ref{lem:remainder-bookkeeping} (with $M=2N$ and $N$ as in the
statement), one may substitute the $N$-term truncation of $\Phi$ and
integrate term by term in $L^1(\omega)$, yielding
\[
D_{\mathrm{KL}}(h_t\|g_t)
...
Lemma~\ref{lem:remainder-bookkeeping} with $M=6$ and $N=4$ justifies the
...
eighth-order coefficients are new:
...
whereas the present contribution is a large-time asymptotic
\emph{expansion} of
...
Lemma~\ref{lem:rkhs-delta} shows that the centred fluctuation profile
produced in Section~\ref{sec:precision-potential} lives in the RKHS of
Appendix~\ref{app:gram-space}, so the kernel formulas feed directly into
...
and cardinal norm formula for the integer secant orbit; the structural
results are direct specializations of
