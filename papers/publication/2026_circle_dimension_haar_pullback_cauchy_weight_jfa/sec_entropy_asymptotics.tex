...
moment exists.  Only the explicit weighted integrals needed for the
coefficients of orders $t^{-6}$ and $t^{-8}$ are deferred to the
appendix; the later defect identities are then algebraic consequences of
the expansion.
...
\begin{remark}
This proposition is standard and is included only to fix the
...
$\pi^{-1}(1+y^2)^{-1}\dd y$.
Appendix~\ref{app:coefficient-assembly} records the coefficient
bookkeeping, and the weighted identities from
Appendix~\ref{app:entropy-integrals} give
\[
