...
$\overline w$, not on~$w$, in the second slot.  The purpose of this
section is to realize $\mathcal H_K$ as a Poisson image on a strip and
then to exploit that realization to obtain explicit projection and
cardinal interpolation formulas on uniform lattices.  The underlying
shift-invariant theory is standard; the special feature of the present
kernel is that every fiber quantity can be summed in closed form.
...
$K(a,b)=2/(4+(a-b)^2)$, of the standard fiberization description of a
principal shift-invariant space and its orthogonal projection; see
\cite{deBoorDeVoreRon1994,RonShen1995,AldroubiGrochenig2001}.  We spell
out the computation because in the present model the periodized weight
and the projector admit closed formulas.
