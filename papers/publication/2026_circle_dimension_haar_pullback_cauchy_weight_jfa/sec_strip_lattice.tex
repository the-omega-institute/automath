...
$\overline w$, not on~$w$, in the second slot.  The aim of the section
is to realize this kernel space concretely as a Poisson image on a strip
and then to record, for this specific kernel, the resulting projection
and cardinal interpolation formulas on uniform lattices.  The general
shift-invariant framework is classical; what matters here is that every
fiber quantity can be written in closed form.
...
The next theorem is the specialization, for the kernel
$K(a,b)=2/(4+(a-b)^2)$, of the usual fiberization description of a
principal shift-invariant space and its orthogonal projection; see
\cite{deBoorDeVoreRon1994,RonShen1995,AldroubiGrochenig2001}.  We record
the calculation because the periodized weight and the projector can be
summed explicitly in this model.
...
Related explicit nod-function constructions are known for Gaussian and
other structured generators
\cite{RomeroUlanovskiiZlotnikov2024,BaranovBelovGrochenig2022,GrochenigRomeroStockler2020},
while the Cauchy/Lorentz and rational-function literature contains
...
In the present section, the point is that for the
kernel $K(a,b)=2/(4+(a-b)^2)$ the Poisson-strip representation makes the
symbol, the projector, and the interpolation norm completely explicit,
yielding in particular \eqref{eq:lattice-symbol}
and~\eqref{eq:cardinal-norm-formula}.
\end{remark}
