...
and then to develop the resulting sampling and cardinal interpolation
theory on the integer orbit.
...
therefore built into the kernel itself.  The holomorphic extension and
pointwise estimate above are the familiar $H^2$ boundary statements
transported to the present translation-invariant setting; see
...
\begin{theorem}[Uniform lattice alias projection]\label{thm:uniform-lattice-projection}
...
The point of the present section is more specific: for the kernel
$K(a,b)=2/(4+(a-b)^2)$ the Poisson-strip representation makes both the
symbol and the interpolation norm explicit, yielding the closed formula
for $\sigma$ and the exact norm identity
