...
\begin{theorem}[Sharp faithful submonoids for one-orbit readouts]
...
\(S=\{p\}\) for one prime \(p\) with \(w(p)>0\).  Consequently every faithful
same-level one-orbit multiplication readout is confined to a single prime-power
ray; the full positive-integer monoid fails because all prime exponents are collapsed to one weighted sum.
\end{theorem}
...
\Cref{thm:one-orbit-faithful-submonoid-classification} is sharp: only a single
prime-power ray can remain faithful.  As a scoped refinement,
...
claim is exactly the faithful-submonoid classification, and the finite-rank
extension is \Cref{thm:finite-rank-product-readout-lower-bound}.
\end{proof}
...
faithful same-level restrictions inside the one-orbit model are the
single-prime-power rays of
