...
Under this level-zero monogenic-orbit restriction, there is no injective
multiplicative readout \(\eta\colon\NN_{\ge1}\to\{g^n:n\in\NN\}\) whose
multiplication is represented by the ambient product on one free monogenic
orbit.
\end{theorem}
...
prime-power ray can remain faithful.
\end{corollary}
...
claim is exactly the faithful-submonoid classification.
\end{proof}
...
readout level such as the power-operator level of \Cref{thm:two-level}.  More
sharply, the only faithful same-level restrictions inside the one-orbit model
are the single-prime-power rays of
