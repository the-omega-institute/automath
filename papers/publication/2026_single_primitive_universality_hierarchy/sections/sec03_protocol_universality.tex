...
be read at level zero by the ambient product on \(C\).  Under exactly this
one-free-monogenic-orbit restriction, there is no injective multiplicative
readout \(\eta\colon\NN_{\ge1}\to C\) satisfying
\(\eta(ab)=\eta(a)\eta(b)\) for all \(a,b\ge1\).
\end{theorem}
...
\begin{theorem}[Sharp faithful coordinate submonoids for one-orbit readouts]
\label{thm:one-orbit-faithful-submonoid-classification}
Let \(C=\{g^n:n\in\NN\}\) be a free monogenic orbit and let
\(\eta_w\colon\NN_{\ge1}\to C\) be the multiplicative readout associated in
\Cref{thm:one-orbit-readout-classification} to weights
\(w\colon\mathcal P\to\NN\).  If \(S\subseteq\mathcal P\) and
\(\NN_S\) denotes the multiplicative submonoid generated by \(S\), then
\(\eta_w\) is injective on \(\NN_S\) if and only if either \(S=\varnothing\), or
\(S=\{p\}\) for one prime \(p\) with \(w(p)>0\).  Consequently, among
prime-coordinate submonoids, the only nontrivial faithful same-level one-orbit
restrictions are single-prime-power rays; the full positive-integer monoid fails
because all prime exponents are collapsed to one weighted sum.
\end{theorem}

\begin{proof}
By \Cref{thm:one-orbit-readout-classification}, equality of readout values on
\(\NN_S\) is equality of the weighted sums
\[
  \sum_{p\in S} v_p(n)w(p).
\]
If \(S=\varnothing\), then \(\NN_S=\{1\}\), so the restriction is injective.  If
\(S=\{p\}\) and \(w(p)>0\), then
\(\eta_w(p^a)=g^{aw(p)}\), and the infinite order of \(g\) implies
\(aw(p)=bw(p)\), hence \(a=b\); thus the restriction is injective.

Conversely, suppose \(\eta_w\) is injective on \(\NN_S\).  If some
\(p\in S\) has \(w(p)=0\), then \(\eta_w(p)=g^0=\eta_w(1)\), contradicting
injectivity.  Hence all primes in \(S\) have positive weight.  If
\(p,r\in S\) are distinct, then
\[
  \eta_w(p^{w(r)})=g^{w(p)w(r)}=\eta_w(r^{w(p)}),
\]
while unique factorization gives \(p^{w(r)}\ne r^{w(p)}\).  This again
contradicts injectivity.  Therefore \(S\) contains at most one prime, and the
one-prime case must have positive weight.  The final sentence is the same
classification applied to \(S=\mathcal P\), which contains two distinct primes.
\end{proof}

\begin{theorem}[Extremal faithful restrictions for one-orbit readouts]
\label{thm:one-orbit-faithful-cyclic-classification}
Let \(C=\{g^n:n\in\NN\}\) be a free monogenic orbit and let
\(\eta_w\colon\NN_{\ge1}\to C\) be a multiplicative one-orbit readout with
exponent homomorphism
\[
  e(n)=\sum_p v_p(n)w(p).
\]
For a multiplicative submonoid \(H\subseteq\NN_{\ge1}\), the restriction
\(\eta_w|_H\) is injective if and only if either \(H=\{1\}\), or there is an
integer \(q>1\) such that
\[
  H\subseteq \{q^k:k\in\NN\}
  \qquad\hbox{and}\qquad e(q)>0 .
\]
Thus the one-orbit obstruction is extremal: a same-level ambient-product readout
can be faithful only on a single cyclic multiplicative ray, while the two-level
power-operator construction of \Cref{thm:two-level} is the sharp positive
replacement used in the main hierarchy.
\end{theorem}

\begin{proof}
Assume first that \(\eta_w|_H\) is injective.  If \(h\in H\setminus\{1\}\) and
\(e(h)=0\), then \(\eta_w(h)=g^0=\eta_w(1)\), contradicting injectivity.  Hence
every nonunit in \(H\) has positive exponent weight.

Let \(a,b\in H\setminus\{1\}\).  The preceding paragraph gives
\(e(a),e(b)>0\).  Since \(e\) is additive under multiplication,
\[
  \eta_w(a^{e(b)})=g^{e(a)e(b)}=\eta_w(b^{e(a)}).
\]
Injectivity on \(H\) gives \(a^{e(b)}=b^{e(a)}\).  Writing prime factorizations,
this says
\[
  e(b)v_p(a)=e(a)v_p(b)\qquad\hbox{for every prime }p,
\]
...
rational ray through \((v_p(h))_p\), and put
\(q=\prod_p p^{\alpha_p}\).  The support is finite because \(h\) has finite prime
support, and \(q>1\).  For every \(x\in H\), the preceding paragraph shows that
\((v_p(x))_p\) lies on the same rational ray.  Since \(\alpha\) is primitive, the
integrality of all \(v_p(x)\) implies \((v_p(x))_p=k\alpha\) for some
\(k\in\NN\).  Thus \(x=q^k\), so \(H\subseteq\{q^k:k\in\NN\}\).  Also
\(e(q)>0\), because otherwise the nonunit \(q^k\in H\) represented by the chosen
element \(h\) would have exponent weight zero.

Conversely, suppose \(H\subseteq\{q^k:k\in\NN\}\) and \(e(q)>0\).  If
\(q^a,q^b\in H\) and \(\eta_w(q^a)=\eta_w(q^b)\), then the infinite order of
\(g\) gives \(ae(q)=be(q)\), hence \(a=b\).  Therefore the restriction to \(H\) is
injective.  The final sentence combines this cyclic-ray classification with the
positive level-shift identity \(\Phi_1(c_{ab})=\Phi_1(c_a)\circ\Phi_1(c_b)\) from
\Cref{thm:two-level}.
\end{proof}

\begin{theorem}[Scoped finite-rank product-readout refinement]
\label{thm:finite-rank-product-readout-lower-bound}
Let \(C_d\) be a free commutative monoid of finite rank \(d\), written
\[
  C_d=\{g_1^{a_1}\cdots g_d^{a_d}:a_i\in\NN\}.
\]
For a set \(S\) of primes, let \(\NN_S\) be the multiplicative submonoid of
\(\NN_{\ge1}\) generated by \(S\).  Every multiplicative readout
\(\eta\colon\NN_S\to C_d\) is determined by weight vectors
\(w(p)\in\NN^d\) through
\[
  \eta\Bigl(\prod_{p\in S}p^{a_p}\Bigr)
  =g_1^{\sum_pa_pw_1(p)}\cdots g_d^{\sum_pa_pw_d(p)} .
\]
It is injective on \(\NN_S\) if and only if the vectors \(w(p)\in\ZZ^d\),
\(p\in S\), are \(\ZZ\)-linearly independent.  Consequently an ambient-product
readout into rank \(d\) can be faithful on at most \(d\) independent prime
generators, and no finite-rank ambient-product readout is injective on all of
\(\NN_{\ge1}\).  This finite-rank statement is only a scoped refinement of the
load-bearing obstruction: the main hierarchy uses the rank-one one-free-monogenic
orbit case \(\Cref{thm:level-zero-obstruction}\).
\end{theorem}

\begin{proof}
Unique factorization identifies \(\NN_S\) with the free commutative monoid of
finite-support exponent vectors \((a_p)_{p\in S}\).  Unique expression in the
free monoid \(C_d\) identifies the value of \(\eta\) with the vector
\(\sum_pa_pw(p)\in\NN^d\), so the displayed formula follows from the
multiplicative law and the values \(\eta(p)\).

If the vectors \(w(p)\) are \(\ZZ\)-linearly independent and
\(\eta(\prod p^{a_p})=\eta(\prod p^{b_p})\), then
\(\sum_p(a_p-b_p)w(p)=0\).  Linear independence gives \(a_p=b_p\) for every
\(p\), and the two source integers are equal.

Conversely, suppose the weights are linearly dependent.  Choose a nonzero
finite integer relation \(\sum_pc_pw(p)=0\).  Write
\(c_p=c_p^+-c_p^-\) with \(c_p^+,c_p^-\in\NN\) and disjoint supports.  Then
\[
  \eta\Bigl(\prod_pp^{c_p^+}\Bigr)
  =\eta\Bigl(\prod_pp^{c_p^-}\Bigr),
\]
while the two products are distinct by unique factorization because some
\(c_p\ne0\).  Thus injectivity fails.  Since at most \(d\) vectors can be
linearly independent in \(\ZZ^d\), any faithful finite-prime restriction has at
most \(d\) prime generators.  Applying this to every \((d+1)\)-element subset
of the prime numbers excludes an injective readout on all of \(\NN_{\ge1}\).
For \(d=1\), the only nonempty independent subsets consist of one prime with
positive weight, which recovers
\Cref{thm:one-orbit-faithful-submonoid-classification}.  The theorem is not used
to enlarge the main obstruction beyond the one-orbit model; it only records the
finite-rank skeleton that contains the rank-one proof as its first case.
\end{proof}
...
\Cref{thm:two-level}.  Within the one-orbit model,
\Cref{thm:one-orbit-faithful-cyclic-classification} is sharp: only a single
cyclic multiplicative ray can remain faithful.  The prime-coordinate case is
\Cref{thm:one-orbit-faithful-submonoid-classification}.  As a scoped refinement,
\Cref{thm:finite-rank-product-readout-lower-bound} shows that an ambient-product
readout into any finite-rank free commutative monoid has a rank budget: rank
\(d\) can faithfully carry at most \(d\) independent prime generators.
\end{corollary}

\begin{proof}
\Cref{thm:two-level} reads multiplication by composition of power operators in
\(\End(\End(A))\), not by the ambient product on \(\{g^n\}\).  The sharpness
claim is exactly the cyclic faithful-restriction classification, the
prime-coordinate case is the faithful-submonoid classification, and the
finite-rank extension is \Cref{thm:finite-rank-product-readout-lower-bound}.
\end{proof}
...
as the power-operator level of \Cref{thm:two-level}.  More sharply, the only
faithful same-level restrictions inside the one-orbit model are cyclic
multiplicative rays, as classified by
\Cref{thm:one-orbit-faithful-cyclic-classification}.
\end{theorem}
...
\(I_n\) in \(\End(\End(A))\), not by the ambient product on \(C\).  The sharper
