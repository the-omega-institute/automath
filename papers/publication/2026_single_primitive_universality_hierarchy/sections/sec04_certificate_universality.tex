...
restricting to \(\{-1,0,1\}^2\).  The only exits from this square in one step are
\[
  (0,-2),(1,-2),(1,-3),(-1,2),(-1,3),(0,2).
\]
A finite check of these six forms under the three maps
\((p,r)\mapsto(r,p+a-r)\), \(a=-1,0,1\), shows that their forward orbit is
contained in the two disjoint cones
\(r\ge p+2\ge2\) and \(r\le p-2\le -2\), together with their one-step images;
in particular it never meets \(\{-1,0,1\}^2\).  Hence an accepting path from
\((0,0)\) to \((\sigma,0)\) cannot leave the square and later return.  This
proves that the displayed finite state set counts all and only the target
solutions.  Multiplying the scalar construction over the \(q-1\) difference
coordinates proves the lemma.
\end{proof}
...
\[
  ...
  ={2z+2z^2-2z^3\over 1-2z-2z^2+2z^3}
\]
and
\[
  \sum_{m\ge1}S_2^{\mathrm{int}}(m)z^m
  ={6z+2z^2-4z^3\over 1-2z-2z^2+2z^3}.
\]
\end{theorem}
...
\end{aligned}
\]
The initial values are obtained by the same automaton for lengths \(1,2,3\).
...
recurrence, rational-generating-series, and exponential-growth constraints of
\Cref{prop:finite-state-coefficient-budget}; therefore any computable cover
whose degree-\(q\) moment violates every exponential bound has no fixed
finite-state transfer formula in that degree.
\end{proposition}
...
sequence \(b_m=S_q^{(m)}\) gives a constant-coefficient recurrence, a rational
generating series, and constants \(C,\rho\) with
\(|S_q^{(m)}|\le C\rho^m\).  Hence a sequence which eventually exceeds
\(C\rho^m\) for every pair of constants \(C,\rho\) cannot admit the fixed matrix
representation in \Cref{def:fixed-finite-state-transfer}.  This is precisely
the budget obstruction used later for the one-spike covers.
\end{proof}
...
\[
  \{d_m^\sharp(x):x\in X_m\}\qquad(0\le m\le M)
\]
...
\Cref{lem:cover-preserves-protocol}.  For every \(m\le M\) its fibre-size multiset
is exactly the singleton multiset of the first cover.  Therefore every predicate
or numerical invariant depending only on those multisets in the window has the
same value for the two covers.  In particular, all displayed moments agree for
\(0\le q\le Q\) and \(0\le m\le M\).
