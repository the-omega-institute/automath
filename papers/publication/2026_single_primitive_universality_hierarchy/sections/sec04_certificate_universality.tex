...
\begin{theorem}[Source-audited ordinary maximum formula]
...
\cite[Thm.~4.7]{KocabovaMasakovaPelantova2005}.  In that source
...
Theorem~4.7 of the cited source gives, after this normalization and interval
conversion,
\[
...
independent maximum theorem proved in this manuscript.  The theorem environment
records the citation boundary and the normalization needed later; its only
...
introduction of \cite{KocabovaMasakovaPelantova2005}: its representations have
...
\Cref{thm:intrinsic-fold-fibre-height,cor:linfty-fold-entropy,%
thm:zero-temperature-fold-pressure,thm:diagonal-zero-temperature-fold-endpoint,%
thm:moment-height-cover-trichotomy}
...
\begin{theorem}[Source support versus intrinsic height theorem]
\label{thm:ordinary-maximum-supports-intrinsic-height}
The exact fibre-height theorem \Cref{thm:intrinsic-fold-fibre-height} is not an
imported ordinary Fibonacci maximum theorem.  Its external support is precisely
the normalized ordinary interval formula \((E)\) of
\Cref{thm:source-audited-ordinary-maximum-import}; its intrinsic conclusion
\[
  M_m=H_{m+1},
  \qquad
  M_{2s-1}=F_{s+1},\qquad M_{2s}=2F_s
\]
is obtained in this manuscript by the fold fibre--representation pairing and the
half-interval conversion.  Consequently any later reference to the
\(q=\infty\) height package may cite the external source only for \((E)\), and
must cite \Cref{thm:half-interval-fibonacci-representation-max,%
thm:intrinsic-fold-fibre-height} for the intrinsic fold statement.
\end{theorem}

\begin{proof}
By \Cref{thm:source-audited-ordinary-maximum-import}, the only imported assertion
is the positive-index ordinary interval formula \((E)\) for
\(C_k=\max\{R(N):F_k\le N<F_{k+1}\}\).  That theorem also proves the normalization
from the cited source's Fibonacci convention to this paper's convention and
excludes any imported classification of maximizing arguments.

Starting from \((E)\), \Cref{thm:half-interval-fibonacci-representation-max} proves
inside the manuscript that the shifted half-interval maxima
\(H_n=\max_{0\le r<F_{n+1}}R_{n+1}(F_{n+1}+r)\) have the displayed parity values.
The proof of that theorem uses only the ordinary interval values, the recursion
\eqref{eq:R-recursion}, and the elementary tail bound in its proof; it does not
appeal to any external fibre theorem.

Finally, \Cref{lem:fold-fibre-representation-pairing} identifies every intrinsic
fold fibre over \(X_m\) with exactly one shifted half-interval representation
count \(R_{m+2}(F_{m+2}+r)\), and \(\Val_m\) ranges bijectively over
\(0\le r<F_{m+2}\).  Taking maxima gives \(M_m=H_{m+1}\), and substituting the
half-interval parity formulas gives the two displayed formulas for
\(M_{2s-1}\) and \(M_{2s}\).  This proves that the external theorem is source
support only for \((E)\), while the intrinsic height theorem itself is the
in-paper consequence of the conversion chain just described.
\end{proof}
...
\item At each fixed resolution, the fibre-size multiset gives the finite-atomic
  ...
  \Cref{prop:classical-finite-atomic-skeleton,thm:hankel-psd,thm:prony}.
...
\item In the computable finite-fibre cover category over the same normal-form
  ...
  transfer: there is a computable cover preserving all \(L1\) readouts whose
  second cover moment has no fixed finite-state transfer formula, and there are
  two covers over the same intrinsic data \((X_m,\Fold_m)\) with opposite
  finite-state-transfer behaviour.
\end{enumerate}
...
The height formula is \Cref{thm:intrinsic-fold-fibre-height}, with the entropy and
large-moment endpoints recorded in
\Cref{cor:linfty-fold-entropy,thm:zero-temperature-fold-pressure,thm:diagonal-zero-temperature-fold-endpoint}.

For the third item, \Cref{thm:L2-cover-skeleton-strictness} realizes, for every
computable excess sequence, a one-spike computable finite-fibre cover over the
unchanged normal-form sets, and \Cref{lem:cover-preserves-protocol} shows that
all normal forms and \(L1\) readouts are preserved.  Taking the excess to be
\(m!\) gives the second cover moment \((m!+1)^2+F_{m+2}-1\), which grows faster
than \(C\rho^m\) for every fixed pair \(C,\rho\).  By
\Cref{prop:fixed-transfer-interface-budget}, no fixed finite-state transfer
formula can produce such a sequence.  The same skeleton also gives the two-cover
boundary of \Cref{thm:L2-two-cover-intrinsic-boundary}: the singleton cover has
the Fibonacci transfer sequence \(F_{m+2}\), while the factorial one-spike cover
over the same \((X_m,\Fold_m)\) violates the exponential budget.
