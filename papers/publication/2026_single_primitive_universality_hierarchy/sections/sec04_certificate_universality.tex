...
\begin{lemma}[Imported ordinary maximum theorem and positive-index conversion]
...
The imported source statement is used exactly in the following form, and no
classification of maximizing arguments from the cited paper is used:
\[
...
\(R(2)=1\).  This ordinary interval maximum formula is the only external
maximum formula imported for \Cref{thm:half-interval-fibonacci-representation-max}.
\end{lemma}
...
The representation function in the cited theorem counts sums of distinct
positive-index Fibonacci parts and its maximum \(\operatorname{Max}(k)\) is taken
over the half-open interval \([\mathcal F_k,\mathcal F_{k+1})\).  Hence the
admissible parts and intervals coincide term-by-term with this paper's
\([F_k,F_{k+1})\), and \(\operatorname{Max}(k)=C_k\).  Substituting
\(k=2t+1\) and \(k=2t+2\) in Theorem~4.7 of the cited paper gives exactly the
two displayed parity clauses, including the source restrictions \(t\ge0\) and
\(t\ge1\).  The exceptional value \(C_2\) is not imported from a parity clause:
...
or as an endpoint in the later half-interval argument.
\end{proof}
