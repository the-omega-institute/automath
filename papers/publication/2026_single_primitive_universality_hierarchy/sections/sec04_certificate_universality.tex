...
The proof interface for this lemma is the adjacent three-step block that follows.
First, \Cref{lem:scalar-signed-fibonacci-exit-classification} classifies every
possible scalar exit from the bounded square.  Second,
\Cref{lem:coordinatewise-signed-fibonacci-exit-deletion} applies that scalar
classification coordinate by coordinate and proves that deleting transitions
leaving \(\mathcal C_q\) loses no target solution.  Third, the proof headed
``Proof of \Cref{lem:bounded-signed-fibonacci-carry}'' rewrites equality of
\(\Fold_m\)-fibres as \eqref{eq:q-column-fib-target} and applies that deletion
lemma.  Thus the bounded state restriction is not an independent hypothesis: the
displayed automaton, the two exit lemmas, and the immediately following delayed
proof are one closed proof package, and every later reference to
