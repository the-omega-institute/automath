...
\begin{theorem}[Height-squeeze lower/upper endpoint for intrinsic fold moments]
...
envelopes used in the displayed squeeze.  They are defined separately because the
argument proves no equality \(p_q^-=p_q^+\) and no existence of
\(\lim_{m\to\infty}m^{-1}\log S_q^{\mathrm{int}}(m)\) for fixed \(q\).  Thus the
assertion does not define a full pressure function and does not assert a
...
whose second moment violates all exponential bounds must have an unbounded spike
height in the sense that
\[
  \max_{x\in X_m}\bigl(d_m^\sharp(x)-1\bigr)
\]
is not exponentially bounded.  In particular, a one-spike cover with \(E_m=m!\)
has no fixed finite-state transfer formula for its second covered moment, although
all fixed-resolution finite-atomic certificates of
...
Let \(H_m=\max_x e_x\).  Since \(e_x^2\le H_m e_x\), every cover satisfies
\[
...
\(S_2^\sharp(m)\) exponentially bounded.  Therefore a cover whose second moment
is not exponentially bounded must have spike height \(H_m\) that is not
exponentially bounded, proving the announced budget lower bound.
