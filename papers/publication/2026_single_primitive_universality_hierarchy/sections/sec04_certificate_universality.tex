...
\(n\ge2\), we use the ordinary maximum theorem of
Koc\'abov\'a--Mas\'akov\'a--Pelantov\'a \cite{KocabovaMasakovaPelantova2005}:
for the Fibonacci representation function, the maximum on the interval
\([F_k,F_{k+1})\) is \(C_2=1\) and, for \(t\ge1\),
\[
...
\begin{theorem}[Moment--height--cover trichotomy]
\label{thm:moment-height-cover-trichotomy}
The \(L2\) finite-fibre interface for the Zeckendorf fold has three distinct
layers.
\begin{enumerate}
\item For every fixed finite \(q\), the intrinsic moment sequence
  \((S_q^{\mathrm{int}}(m))_{m\ge1}\) has fixed finite-state transfer.
\item At \(q=\infty\), the intrinsic fibre height has the exact law
  \(M_{2s-1}=F_{s+1}\), \(M_{2s}=2F_s\), and entropy
  \((1/2)\log\varphi\), with the zero-temperature pressure envelope of
  \Cref{thm:zero-temperature-fold-pressure}.
\item In the arbitrary computable finite-fibre cover category over the same
  normal-form sets, fixed finite-state transfer can fail by the one-spike
  construction.
\end{enumerate}
Thus the paper proves fixed-\(q\) intrinsic transfer and the intrinsic
\(L^\infty\) height endpoint, while the negative strictness statement remains
cover-relative and does not assert a full intrinsic fibre-histogram or
large-deviation classification.
\end{theorem}

\begin{proof}
Item (1) is \Cref{thm:intrinsic-fold-moment-transfer}.  Item (2) is
\Cref{thm:intrinsic-fold-fibre-height,cor:linfty-fold-entropy,thm:zero-temperature-fold-pressure}.
Item (3) is \Cref{thm:L2-finite-fiber-no-transfer-content}.  The final sentence
is the nonimplication boundary of
\Cref{thm:intrinsic-cover-transfer-dichotomy,thm:L2-cover-intrinsic-nonimplication}:
cover multiplicities are auxiliary data over fixed normal forms, whereas the
height law above concerns the original maps \(\Fold_m\).
\end{proof}
