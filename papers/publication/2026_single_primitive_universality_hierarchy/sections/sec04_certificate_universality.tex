...
Theorem~4.7 of Koc\'abov\'a--Mas\'akov\'a--Pelantov\'a
\cite{KocabovaMasakovaPelantova2005}, normalized by
...
Moreover the two parity clauses of that theorem, and only those clauses,
translate verbatim to
\[
  C_{2t+1}=F_{t+1}\quad(t\ge0),
  \qquad
  C_{2t+2}=2F_t\quad(t\ge1),
\]
and this is the only external maximum formula imported for
...
\(n\ge2\), we use only the interval-maximum formula in Theorem~4.7 of
Koc\'abov\'a--Mas\'akov\'a--Pelantov\'a \cite{KocabovaMasakovaPelantova2005},
with the normalization checked in
\Cref{lem:fibonacci-maximum-normalization-conversion}.  This is the only
external input in the height theorem: the remainder of the proof is the local
conversion of that theorem's ordinary interval maximum to the half-interval
notation used here.  The cited theorem is stated for Fibonacci numbers indexed by
