...
For each fixed resolution, \Cref{prop:classical-finite-atomic-skeleton} gives the
standard Hankel positivity, log-convexity, and finite Prony reconstruction facts
for the finite fibre-size multiset.  Applied to the Zeckendorf fold, one may use
the computable bound \(2|X_m|-1=2F_{m+2}-1\) for reconstruction.  The same finite
atomic data also give the elementary Gram-kernel description
\(K_m(i,j)=S_{i+j}(m)\) and its finite diagonal nonnegative compression.  These are
recorded only as fixed-resolution support: they impose no recurrence in the
resolution variable and are not the \(L2\) novelty claim of this article.
\end{remark}
