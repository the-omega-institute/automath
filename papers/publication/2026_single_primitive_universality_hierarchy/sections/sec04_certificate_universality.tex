...
restricting to \(\{-1,0,1\}^2\).  We verify this explicitly.  From a state
\((p,r)\in\{-1,0,1\}^2\), the first coordinate of the next state is \(r\), and
the second is \(p+a-r\), with \(a\in\{-1,0,1\}\).  Hence the only one-step exits
from the square are
\[
  (0,-2),(1,-2),(1,-3),(-1,2),(-1,3),(0,2).
\]
Separate the positive and negative exits.  The positive exits
\((-1,2),(-1,3),(0,2)\) lie in
\[
  U_+=\{(p,r):p\le0,\ r\ge2\}.
\]
For every \((p,r)\in U_+\) and every \(a\in\{-1,0,1\}\),
\[
  \tau_a(p,r)=(r,p+a-r)
\]
has first coordinate at least \(2\) and second coordinate at most \(-1\), so it
lies in
\[
  V_+=\{(p,r):p\ge2,\ r\le -1\}.
\]
Conversely, if \((p,r)\in V_+\), then \(\tau_a(p,r)\) has first coordinate at
most \(-1\) and second coordinate at least \(2\), and therefore lies in
\(U_+\).  Thus \(U_+\cup V_+\) is forward invariant and is disjoint from
\(\{-1,0,1\}^2\).

The negative exits are symmetric.  They lie in
\[
  U_-=
  \{(p,r):p\ge0,\ r\le -2\}.
\]
For \((p,r)\in U_-\), every image \(\tau_a(p,r)\) has first coordinate at most
\(-2\) and second coordinate at least \(1\), hence lies in
\[
  V_-=
  \{(p,r):p\le -2,\ r\ge 1\}.
\]
For \((p,r)\in V_-\), every image lies back in \(U_-\).  Hence
\(U_-\cup V_-\) is also forward invariant and disjoint from the square.  Once a
path leaves \(\{-1,0,1\}^2\), it is trapped in one of these two invariant
regions and can never return to an accepting state \((\sigma,0)\).  This proves
that the displayed finite state set counts all and only the target solutions.
...
with the same normalization \(F_0=F_1=1\) used in this paper.  In the notation
\[
