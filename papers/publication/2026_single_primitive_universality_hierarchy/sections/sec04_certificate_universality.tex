...
The proof of this lemma is deliberately delayed only until the next lemma.  The
only sentence not proved directly from the displayed transition rule is the
boundedness assertion that no target solution is lost when states outside
\(\{-1,0,1\}^{q-1}\times\{-1,0,1\}^{q-1}\) are deleted; that assertion is exactly
the coordinatewise consequence of the scalar exit-trapping classification in
\Cref{lem:scalar-signed-fibonacci-exit-classification}.  Thus the present lemma,
the scalar exit lemma, and the proof headed ``Proof of
\Cref{lem:bounded-signed-fibonacci-carry}'' below form a single proof interface.
No independent unproved carry boundedness is being assumed, and no later use of
\Cref{lem:bounded-signed-fibonacci-carry} refers merely to the displayed
statement without that delayed proof.
...
This is the delayed proof announced above, and it closes exactly the lemma
statement displayed before \Cref{lem:scalar-signed-fibonacci-exit-classification}.
The only extra input used between the statement and this proof is the scalar exit
classification just proved in \Cref{lem:scalar-signed-fibonacci-exit-classification}.
...
normalization recorded above.  The source check is against the DOI and Numdam
copy named in the bibliography: Theorem~4.7 there states the two displayed parity
formulas for
