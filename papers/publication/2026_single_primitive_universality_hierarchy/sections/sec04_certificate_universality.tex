...
Moreover the two parity clauses of their Theorem~4.7 translate verbatim to
\[
  C_{2t+1}=F_{t+1}\quad(t\ge0),
  \qquad
  C_{2t+2}=2F_t\quad(t\ge1),
\]
and this is the only external maximum formula used in
\Cref{thm:half-interval-fibonacci-representation-max}.
\end{lemma}
...
with \([F_k,F_{k+1})\).  Thus \(\operatorname{Max}(k)=C_k\).  Substituting
\(k=2t+1\) and \(k=2t+2\) in the cited theorem gives exactly the two displayed
parity clauses, including the source restrictions \(t\ge0\) and \(t\ge1\).  No
quantity with index \(0\) in the source sequence is used as an admissible part or
as an endpoint in the later half-interval argument.
\end{proof}
...
This is the only external input in the height theorem: the remainder of the proof
is the local conversion of that theorem's maximum to the half-interval notation
used here.  That theorem is stated for Fibonacci numbers indexed by
...
\begin{theorem}[Diagonal zero-temperature intrinsic fold endpoint]
...
is a height-squeeze consequence of the exact maximal-fibre law, not an additional
thermodynamic formalism.  It controls the normalized diagonal large-moment slope
for every diverging moment degree.  This is an intrinsic theorem about the
unmodified maps \(\Fold_m\), uses no auxiliary cover fibres, and supplies only the
\(q=\infty\) endpoint needed in \Cref{thm:main-hierarchy}.
\end{theorem}
...
pressure assertions are precisely
...
\(L2\).  They are not the \(L2\) novelty claim of this article and, by
...
bound follows from \(|X_m|=F_{m+2}\), since the number of distinct fibre sizes is
at most the number of normal forms.
\end{proof}
...
the fixed-resolution finite-fibre certificates factor through the finite multiset
at the single resolution \(m\).  Therefore the positive
