...
\[
  ...
  ={\frac12}\log\varphi .
\]
Equivalently, the zero-temperature endpoint detected in
\Cref{thm:zero-temperature-fold-pressure} is not only an iterated
fixed-\(q\) pressure squeeze: it is the actual diagonal large-moment slope for
every diverging moment degree.  This is an intrinsic theorem about the
unmodified maps \(\Fold_m\), and it uses no auxiliary cover fibres.
\end{theorem}
...
Vandermonde star feature map attached to the fixed fibre-size atoms.
\end{proposition}
...
\begin{remark}[Subordinate fixed-fibre consequences]
\label{rem:hankel-psd-generic}
The preceding Hankel, Prony, and star-kernel statements are fixed-resolution
support for \(L2\).  They are not the \(L2\) novelty claim of this article and,
by \Cref{thm:hankel-prony-transfer-independence}, they impose no recurrence in
