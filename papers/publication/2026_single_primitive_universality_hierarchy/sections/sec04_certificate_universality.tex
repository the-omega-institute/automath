...
\item the matrix \(T_q\) is exactly the weighted adjacency matrix of this bounded
  ...
  \(\sum_{	au_a(c)=c'}w_q(a)\), and the vectors \(\lambda_q,\rho_q\) are the
...
\(\lambda_q^{\mathsf T}T_q^{m+1}\rho_q\) equal to the weighted number of bounded
accepting paths of length \(m+1\), which is exactly
\(S_q^{\mathrm{int}}(m)\) by \Cref{lem:bounded-signed-fibonacci-carry}.  The
construction uses only the fixed fold data \((\Omega_m,X_m,\Fold_m)\); the
...
article: it is exactly the positive-index formula \((E)\) isolated and cited in
...
external theorem only through the named quantities \(C_k\) and then works entirely
with \(R_{n+1}\), \eqref{eq:R-recursion}, and the fold-fibre pairing.
\end{theorem}
...
\item For the unmodified maps \(\Fold_m\), every fixed finite intrinsic moment
  ...
  interface of \Cref{thm:audited-fixed-degree-transfer-interface}; in particular the intrinsic second
