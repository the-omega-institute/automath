...
\begin{theorem}[Source-audited ordinary maximum import]
...
\]
over positive integers represented as sums of distinct positive-index Fibonacci
parts is exactly the quantity
\[
  C_k=\max\{R(N):F_k\le N<F_{k+1}\}
\]
used below, with no index shift for \(k\ge1\).
The imported source statement, after this normalization and interval conversion,
is used exactly in the following form, and no classification of maximizing
arguments from the cited paper is used:
\[
...
The source defines Fibonacci representations using distinct parts whose indices
are at least \(1\), and its quantity \(\operatorname{Max}(k)\) is the maximum of
the same representation count over the half-open interval
\([\mathcal F_k,\mathcal F_{k+1})\).  Hence the admissible parts and intervals
coincide term-by-term with this paper's \([F_k,F_{k+1})\), and
\(\operatorname{Max}(k)=C_k\) for every \(k\ge1\).  Theorem~4.7 in the cited source
states precisely
\[
...
\begin{proposition}[Exact dependency cut for the intrinsic height package]
