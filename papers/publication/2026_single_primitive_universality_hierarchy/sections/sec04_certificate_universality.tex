...
\begin{equation}\label{eq:q-column-fib-target}
  ...
  \qquad(1\le j\le q-1).
\end{equation}
These weighted words are recognized by a finite automaton whose carry states are
\[
...
\]
Thus a scalar word with target \(\sigma F_{n+1}\) is accepted exactly when its
final carry is \((\sigma,0)\), provided no accepting path has been lost by
restricting to \(\{-1,0,1\}^2\).  This is exactly
\Cref{lem:scalar-signed-fibonacci-exit-classification}: every scalar path that
leaves the square is trapped outside the square and cannot later reach an
accepting state.  Thus the displayed finite state set counts all and only the
target solutions.
Multiplying the scalar construction over the \(q-1\) difference coordinates
proves the lemma.
\end{proof}
...
Let \((\mathcal F_k)_{k\ge0}\) denote the Fibonacci sequence used in
Koc\'abov\'a--Mas\'akov\'a--Pelantov\'a, normalized by
\(\mathcal F_0=\mathcal F_1=1\) and
\(\mathcal F_{k+1}=\mathcal F_k+\mathcal F_{k-1}\).  Then
\(\mathcal F_k=F_k\) for every \(k\ge1\), where this paper uses
\(F_1=1,F_2=2,F_{k+2}=F_{k+1}+F_k\).  Consequently their interval maximum
\[
...
\begin{proof}
The two normalizations agree at indices \(1\) and \(2\):
\(\mathcal F_1=1=F_1\) and
\(\mathcal F_2=\mathcal F_1+\mathcal F_0=2=F_2\).  The same second-order
recurrence then gives \(\mathcal F_k=F_k\) for all positive indices by induction.
The representation function in the cited theorem uses distinct positive-index
parts \(\mathcal F_1,\ldots,\mathcal F_k\), hence the admissible parts and the
half-open intervals \([\mathcal F_k,\mathcal F_{k+1})\) coincide term-by-term
with \([F_k,F_{k+1})\).  Thus \(\operatorname{Max}(k)=C_k\).
\end{proof}
...
\(n\ge2\), we use Theorem~4.7 of
Koc\'abov\'a--Mas\'akov\'a--Pelantov\'a \cite{KocabovaMasakovaPelantova2005}.
That theorem is stated for Fibonacci numbers indexed by
\(\mathcal F_0=\mathcal F_1=1\) and for maxima over the half-open intervals
\([\mathcal F_k,\mathcal F_{k+1})\).  By
\Cref{lem:fibonacci-maximum-normalization-conversion}, this is the same
positive-index convention used here.  Thus, in the notation
\[
  C_k=\max\{R(N):F_k\le N<F_{k+1}\},
\]
that theorem gives \(C_1=C_2=1\) and, for \(t\ge1\),
\[
...
from those formulas and \(F_{j+1}=F_j+F_{j-1}\).
\end{proof}
...
\begin{theorem}[Zero-temperature intrinsic fold pressure envelope]
...
\(\lim_{m\to\infty}m^{-1}\log F_{m+2}=\log\varphi\), gives the displayed pressure
envelope.  Dividing by \(q\) and letting \(q\to\infty\) squeezes both normalized
endpoints to \((1/2)\log\varphi\).
\end{proof}
...
\item At \(q=\infty\), the intrinsic fibre height is exactly
  ...
  \[
    \lim_{m\to\infty}{\frac1m}\log M_m={\frac12}\log\varphi .
  \]
  Moreover the intrinsic zero-temperature pressure endpoint is
  \((1/2)\log\varphi\) in the sense of
  \Cref{thm:zero-temperature-fold-pressure}.
...
\(m=2s\) and \(m^{-1}\log(2F_s)\to(1/2)\log\varphi\).  The zero-temperature
pressure assertion is precisely \Cref{thm:zero-temperature-fold-pressure}.
...
finite certificates of \Cref{prop:classical-finite-atomic-skeleton}.  Fixed
...
Hankel positivity, log-convexity, Prony reconstruction data, star-kernel data, or
finitely many collision moments can certify finite-state transfer for all
resolutions.
\end{theorem}
...
Hankel positivity, log-convexity, finite Prony reconstruction, and the
star-kernel tests at any fixed resolution factor through the finite fibre-size
multiset by
\Cref{prop:classical-finite-atomic-skeleton,prop:finite-fiber-certificate-taxonomy,thm:star-kernel}.
...
\begin{remark}[Terminology bridge for the \texorpdfstring{\(L2\)}{L2} clause]
...
universality itself.  The theorem-named label on this remark is a dependency-cut
alias for this terminology boundary; the load-bearing statements are the cited
definition and transfer theorems.
...
\begin{theorem}[Extremal one-spike classification of the \texorpdfstring{\(L2\)}{L2} cover layer]
...
strictness assertion is governed by the following extremal theorem in the
computable finite-fibre cover category over the same normal forms.  Fix a
resolution \(m\), write \(n_m=|X_m|=F_{m+2}\), and let a cover have fibre sizes
\(d_m^\sharp(x)=1+e_x\) with nonnegative integer excesses satisfying
\(\sum_{x\in X_m}e_x=E_m\).  Then
\[
  S_2^\sharp(m)=\sum_{x\in X_m}(1+e_x)^2
  \le (E_m+1)^2+n_m-1,
\]
with equality if and only if all excess is concentrated on one normal form.  In
particular, for every computable sequence \(a_m\ge1\), the extremal cover with
\(E_m=a_m\) preserves all \(L1\) normal forms and readouts and has
\[
  S_2^\sharp(m)=(a_m+1)^2+|X_m|-1 .
\]
Consequently the worst possible second-moment obstruction at a fixed certificate
excess has the one-spike skeleton, and choosing any superexponential computable
excess sequence, for instance \(a_m=m!\), violates every fixed finite-state
transfer budget.  This is not an intrinsic theorem about the asymptotics of the
original Zeckendorf fibres of \(\Fold_m\).
\end{theorem}
...
For the extremal assertion, fix \(m\) and write the cover sizes as
\(1+e_x\).  Since the excesses are nonnegative integers and
\(\sum_x e_x=E_m\),
\[
  S_2^\sharp(m)=\sum_x(1+e_x)^2
  = n_m+2E_m+\sum_x e_x^2 .
\]
For nonnegative numbers with fixed sum \(E_m\), one has
\(\sum_x e_x^2\le E_m^2\), because
\(E_m^2-\sum_xe_x^2=2\sum_{x<y}e_xe_y\ge0\).  This gives the displayed upper
bound.  Equality holds exactly when all cross-products \(e_xe_y\) vanish, i.e.
when at most one excess is nonzero; since the total excess is \(E_m\), this is
the one-spike cover.

To realize the extremal cover effectively, choose one normal form
\(x_m\in X_m\) effectively: for example the all-zero Zeckendorf word belongs to
every \(X_m\).  Attach a cover fibre of cardinality \(a_m+1\) over \(x_m\), and
attach singleton cover fibres over every other element of \(X_m\).  The
assignment is computable because \((a_m)\) is computable and the normal-form sets
are effectively enumerable.  By construction the projection to \(X_m\) is
unchanged, and hence all \(L1\) normal-form operations and readouts are
preserved.  The covered second collision moment is the square of the one enlarged
fibre plus one square for each of the remaining \(|X_m|-1\) fibres, giving the
displayed formula.
...
\begin{remark}[Fibre multiset factorization]\label{thm:L2-fiber-multiset-factorization}
...
\(\mathcal D_m\) emphasized.  The theorem-named label on this remark is a
dependency-cut alias for that proposition, not a separate factorization theorem.
\end{remark}

\begin{remark}[Finite-fibre transfer pairing]\label{thm:L2-finite-fiber-transfer-pairing}
If a finite-fibre cover certificate system also has a fixed finite-state transfer
formula for \(S_q^{(m)}\) in the sense of
\Cref{def:fixed-finite-state-transfer}, then its fixed-resolution certificate
data and its resolution-wise growth data are paired by a constant-coefficient
recurrence.
...
\Cref{def:fixed-finite-state-transfer,prop:finite-state-coefficient-budget}.  The
second is the factorial one-spike construction in
\Cref{thm:L2-finite-fiber-no-transfer-content}.  The theorem-named label on this
remark is a dependency-cut alias for those two cited interfaces.
\end{remark}
