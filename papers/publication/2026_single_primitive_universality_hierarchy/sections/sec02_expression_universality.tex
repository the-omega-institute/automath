...
the v2 revision date and the ancillary file
\texttt{SupplementaryInformation.pdf}.  Thus every occurrence of the citation in
this manuscript means that fixed v2 source object, not a later mutable arXiv
version, a repository checkout, or a regenerated table.  The ancillary file named
in \Cref{ass:HW} is used only for the optional printed-string hypothesis; the main
theorem chain treats the full \(\mathcal B_{36}\) result as the single external
existence assertion stated here.  No later proof may appeal to an unprinted row of
the \(\mathcal B_{36}\) table; all self-contained EML calculations used later are
