...
\Cref{prop:internal-core-eml-witnesses}, the verified-triple Richardson interface
...
\Cref{thm:level-zero-obstruction}, and the cover-relative \(L2\) finite-state
obstruction of \Cref{thm:finite-state-strictness}.  If one deletes both
\begin{enumerate}
\item the unprinted term-by-term \(\mathcal B_{36}\) representative table, keeping
  only the internal core witnesses; and
\item the optional printed-string hypothesis \((H_{\mathcal W})\) for
  \((U_{47},U_{111},U_{126})\),
\end{enumerate}
...
\(\mathcal B_{36}\) expression-universality headline and the concrete
printed-string Richardson specialization.  In particular, the cover-relative
\(L2\) strictness theorem remains a theorem about computable covers over the fixed
normal-form sets \(X_m\), and no intrinsic asymptotic classification of the
original \(\Fold_m\) fibres is introduced by the deletion.
\end{theorem}
...
\(\mathcal B_{36}\) table.  The Richardson theorem used unconditionally is the
schema of \Cref{thm:separation}: its hypotheses are a specified finite triple
\((\mathsf P,\mathsf S,\mathsf R)\) and proofs of the three real-domain identities
\(\mathsf P=\pi\), \(\mathsf S(x)=\sin x\) on \(\RR\), and
\(\mathsf R(t)=\sqrt t\) on \(\RR_{\ge0}\).  Thus deleting the printed names
\((U_{47},U_{111},U_{126})\) removes only the specialization supplied by
\((H_{\mathcal W})\); it does not remove the universal verified-triple
implication.  This is exactly the interface identified in
\Cref{thm:L0-richardson-exact-load-bearing-cut}.
...
as \(a_m=m!\).  None of these steps uses the intrinsic multiplicities of
\(\Fold_m\); by \Cref{thm:L2-cover-intrinsic-nonimplication} a cover-moment
transfer statement cannot be transported to the original fold fibres without an
additional equality or comparison theorem.  Therefore the stated deletion leaves
