...
data from the \(\mathcal B_{36}\) import alone.  The Richardson theorem is
relative to any supplied punctured triple with those identities proved, and it
proves the absolute-value row internally, so the endpoint-root string
\(U_{126}\) is not load-bearing for \Cref{thm:main-hierarchy}.
\end{proof}
...
\item the unprinted term-by-term \(\mathcal B_{36}\) representative table, keeping
  the internal core witnesses and a supplied punctured Richardson triple,
\end{enumerate}
...
term \(\Abs\).  This is exactly the interface identified in
