...
algorithm for EML terms, not a semantic compiler, and not an endpoint-root Richardson
package for \(\pi\), sine on \(\RR\), and nonnegative square root on
...
endpoint-root triple for \(\pi\), sine, and nonnegative square root; these are exactly
...
\(\mathcal B_{36}\) import and the endpoint-root string \(U_{126}\) are not
load-bearing substitutes for those identities.\end{theorem}
...
data from the \(\mathcal B_{36}\) import alone.  The repaired Richardson theorem
supplies its own punctured witnesses, so the endpoint-root string \(U_{126}\) is
not load-bearing for \Cref{thm:main-hierarchy}.
\end{proof}
...
obstruction of \Cref{thm:finite-state-strictness}.  If one deletes
\begin{enumerate}
\item the unprinted term-by-term \(\mathcal B_{36}\) representative table, keeping
  the internal core witnesses and the printed punctured Richardson witnesses,
\end{enumerate}
then every assertion in the core theorem package remains proved with the same
statements.  The only assertion removed by this deletion is the global cited
\(\mathcal B_{36}\) expression-universality headline.  In particular, the cover-relative
...
\(\mathcal B_{36}\) table.  The Richardson theorem used unconditionally is
\Cref{thm:separation}, whose witnesses for \(\pi\), sine, and punctured absolute
value are verified in \Cref{lem:eml-core-witness-table}.  This is exactly the
interface identified in \Cref{thm:L0-richardson-exact-load-bearing-cut}.
