...
proposition is a citation-only representative-existence theorem, pinned to that
exact public version and to the source's own branch-domain conventions.  Its logical
content inside this paper is exactly the following finite-existence assertion:
...
\texttt{10.48550/arXiv.2603.21852}; the arXiv metadata for that version records
the submission history, the v2 revision timestamp, and the ancillary file
\texttt{SupplementaryInformation.pdf}.  Thus every occurrence of the citation in
this manuscript means that fixed v2 source object, its main-text Table~1
\(\mathcal B_{36}\) list, and its versioned ancillary verification interface, not
a later mutable arXiv version, a repository checkout, a generated-code artifact,
or a regenerated table.  The principal-branch domains are likewise imported only
...
serving only as the cited verification record for that representative-existence
claim.
...
record pins the arXiv version, DOI, ancillary file name, and provenance snapshot;
hence the manuscript has a stable object to cite, but the object cited is still
only the stated source assertion and its cited Part~II verification record.  The
pinning has two parts: the main-text Table~1 list is the sole \(\mathcal B_{36}\)
object being cited, and the ancillary Part~II material is used only as the public
verification interface for representative existence for that fixed list.  The
manuscript does not import any additional row, generated code artifact, mutable
repository state, regenerated table, or term-by-term proof of the thirty-six representatives.  In
...
thirty-six-row \(\mathcal B_{36}\) representative table.
\end{proposition}
...
pinned arXiv v2 record with its stated principal-branch domains and ancillary
verification interface.  Thus the full headline factors through that external
anchor and through no unversioned table, repository state, generated artifact, or
endpoint-extended branch convention.

Conversely, \Cref{thm:eml-universality} is precisely the statement that the
displayed source basis has finite representatives over \((\eml,1)\) on those
domains, so invoking it with the cuts of \Cref{thm:B36-import-exact-scope} gives
exactly the full calculator-basis \(L0\) assertion and no stronger data.  The
self-contained calculations in \Cref{prop:internal-core-eml-witnesses} prove only
the smaller package named there.  The dependency check in
...
bearing for the later internal theorem chain.
\end{proof}
