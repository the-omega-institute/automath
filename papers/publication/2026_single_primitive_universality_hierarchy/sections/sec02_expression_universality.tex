...
\Cref{thm:level-zero-obstruction}, the intrinsic \(L2\) transfer package of
\Cref{thm:intrinsic-fold-moment-transfer}, and the cover-relative \(L2\)
finite-state obstruction of \Cref{thm:finite-state-strictness}.  If one deletes
\begin{enumerate}
...
normal-form sets \(X_m\), while the positive intrinsic \(\Fold_m\) finite-state
transfer package remains the one proved directly in \Cref{sec:L2}; no further
intrinsic asymptotic classification of the original fibres is introduced by the
deletion.
\end{theorem}
