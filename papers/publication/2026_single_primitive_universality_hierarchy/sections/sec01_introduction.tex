...
\(U_{47}\)/\(U_{111}\) specialization of the relative theorem.  Unless
...
\begin{theorem}[Source-gate rigidity of the hierarchy]
...
\((E)\).  Then
\Cref{cor:citation-sensitive-half-interval-conversion,thm:intrinsic-height-source-dependency-cut,thm:ordinary-maximum-supports-intrinsic-height,thm:audited-shifted-half-interval-height-conversion}
...
\Cref{thm:ordinary-maximum-supports-intrinsic-height,thm:intrinsic-fold-fibre-height} give the dependency firewall and the two displayed height formulas,
...
\begin{theorem}[Relative single-primitive hierarchy]\label{thm:main-hierarchy}
...
whereas Zeckendorf normal forms and Church iteration give an internal computable
\(L1\) protocol for \((\NN,+,\times)\).  The strictness is witnessed by two primary
obstructions: verified finite EML \(\pi\)/sine/absolute-value triples transport
Richardson's identity-to-zero theorem and therefore forbid equality-reflecting
...
The Richardson clause is relative: this article proves the punctured
absolute-value row internally, but the exact printed pair \((U_{47},U_{111})\)
instantiates the clause only under \((H_{\mathcal W})\) or separate proofs of
\(U_{47}=\pi\) and \(U_{111}(x)=\sin x\) on the stated real principal-branch
domains.  The endpoint-root string \(U_{126}\), a term-by-term proof of the full
\(\mathcal B_{36}\) table, and a full intrinsic fibre-histogram or
large-deviation theory for the unmodified Zeckendorf fold are not consequences of
the theorem.
\end{theorem}
...
\(L1\) positive protocol is \Cref{thm:zeckendorf-protocol-univ}.  The primary
...
deletions the remaining theorem chain is exactly the cited \(L0\)
representative-existence anchor, the internally proved core EML witnesses, the
replacement-triple punctured Richardson implication, the internal Zeckendorf
\(L1\) protocol, the one-orbit multiplication obstruction, the intrinsic \(L2\)
fixed-\(q\) transfer and height package, and the cover-relative \(L2\)
finite-state obstruction.  Conversely, an unconditional printed-string
...
support isolated in
\Cref{thm:intrinsic-height-source-dependency-cut,thm:ordinary-maximum-supports-intrinsic-height}; the intrinsic fold statement itself is
...
\begin{theorem}[Verified-core projection of the hierarchy]
...
\Cref{thm:zeckendorf-protocol-univ} together with the restricted obstruction
\Cref{thm:level-zero-obstruction,thm:restricted-level-zero-protocol-dichotomy};
