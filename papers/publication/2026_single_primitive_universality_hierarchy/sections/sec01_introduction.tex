...
The dependencies in the compact statement are as follows.  The \(L0\) anchor is
the pinned external representative-existence theorem
\Cref{thm:eml-universality,thm:B36-import-exact-scope}; it imports no equality
algorithm, semantic compiler, or Richardson witness package.  The \(L1\) positive
protocol is \Cref{thm:zeckendorf-protocol-univ}.  The Richardson obstruction is
the verified-triple theorem
\Cref{thm:separation,thm:proc-ams-relative-richardson-theorem}, with the printed
name boundary recorded in
\Cref{ass:HW,cor:no-unconditional-printed-richardson-triple,thm:unconditional-submission-richardson-closure}.
The same-level multiplication obstruction is
\Cref{thm:level-zero-obstruction,thm:restricted-level-zero-protocol-dichotomy}.
The \(L2\) layer is consolidated in
...
\Cref{thm:level-zero-obstruction}.
Finally, the consolidated \(L2\) interface is
\Cref{thm:L2-consolidated-interface}.  It packages the finite-resolution
Hankel--Prony certificates, the positive intrinsic fixed-degree transfer and
height package, and the cover-relative finite-state separation, while
\Cref{thm:L2-cover-intrinsic-nonimplication} excludes interpreting the cover
example as an intrinsic failure or nontransfer theorem for the original fibres.  The
...
explicit hypotheses--the printed \((U_{47},U_{111})\) Richardson specialization,
the term-by-term \(\mathcal B_{36}\) table, endpoint-root claims, and full
intrinsic fold-histogram or large-deviation assertions--the remaining theorem
chain still proves \Cref{thm:main-hierarchy}.  More precisely, the surviving core
is the following directed chain of load-bearing assertions:
\[
\begin{gathered}
\text{external }L0\text{ representative existence}
\longrightarrow
\text{verified-triple Richardson obstruction},\\
...
By \Cref{thm:unconditional-submission-richardson-closure}, that Richardson theorem
is formulated for arbitrary finite triples whose three semantic rows have been
proved, and deleting \((H_{\mathcal W})\) removes only the named printed-pair
specialization.  The second arrow is the positive protocol
...
\Cref{thm:main-hierarchy-exact-scope-closure}; hence the surviving core proves the
