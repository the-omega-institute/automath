...
\]
for the printed pure EML strings on their stated principal-branch domains.  Then
\(\mathcal C\) proves \Cref{thm:main-hierarchy} with its Richardson obstruction
quantified over verified finite triples and without using the printed names
\(U_{47}\) and \(U_{111}\) as analytic identities.  Moreover, relative to the
structural Richardson translation used in this paper and to the internally proved
punctured row \(\Abs(u)=|u|\) for \(u\ne0\), the printed
...
and leaves the verified-core hierarchy unchanged.
\end{theorem}
...
the \(L2\) assertion is
\Cref{thm:L2-consolidated-interface,thm:finite-state-strictness}.  Therefore the
cited arrows are exactly the proof of \Cref{thm:main-hierarchy} in verified-core
form.
