...
intrinsic height, \(L^\infty\) entropy, zero-temperature lower/upper envelope,
moment-height dichotomy, and cover-relative finite-state strictness are the
labelled statements just named, independent of any later compression of the
theorem stream.  The word ``pressure'' in the label of
\Cref{thm:zero-temperature-fold-pressure} is only mnemonic for this liminf/limsup
envelope and is not a claim that a fixed-\(q\) pressure limit exists.
...
\begin{theorem}[Verified core and printed specialization split]
...
obstruction, and the intrinsic/cover-relative \(L2\) interface.  Then
\(\mathcal C\) proves \Cref{thm:main-hierarchy} without using the printed names
\(U_{47}\) and \(U_{111}\) as analytic identities.  If, in addition,
\((H_{\mathcal W})\) is assumed, or the two exact real-domain identities
\(U_{47}=\pi\) and \(U_{111}(x)=\sin x\) are proved for the printed terms on the
stated principal-branch domains, then the Richardson clause of
\(\mathcal C\) specializes to that printed pair.  Conversely, deleting this
additional printed-pair hypothesis deletes only that specialization and leaves the
verified-core hierarchy unchanged.
\end{theorem}
...
internally by \(G(M(u,u))=|u|\) for \(u\ne0\); the proof of the Richardson
obstruction then proceeds by the effective translation of
\Cref{lem:richardson-translation} and Richardson undecidability, and nowhere
requires the syntactic names \(U_{47}\) or \(U_{111}\).  The remaining arrows of
\(\mathcal C\) are independent of those names: the positive protocol is
...
the \(L2\) assertion is
\Cref{thm:L2-consolidated-interface,thm:finite-state-strictness}.
Therefore the cited arrows are exactly the proof of \Cref{thm:main-hierarchy}.

Under \((H_{\mathcal W})\), or under separate proofs of the two displayed
real-domain identities for the exact printed terms, the pair
\((U_{47},U_{111})\) satisfies the first two semantic rows of the verified-triple
Richardson interface.  Substituting that verified pair into
\Cref{thm:separation} gives the printed specialization.  If the extra hypothesis
is removed, \Cref{cor:printed-string-free-richardson-spine,cor:no-unconditional-printed-richardson-triple}
remove precisely this substitution step; all preceding quantified verified-triple
statements remain available for any replacement pair whose identities have been
proved.  Thus the printed branch is a conditional instance of the core interface,
not a hidden premise or consequence of the unconditional hierarchy.
\end{proof}

The verified-core/printed-specialization decomposition of
