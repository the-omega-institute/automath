...
\item Finite fibres give the \(L2\) certificate layer over the fixed Zeckendorf
  ...
  fixed finite-state transfer formula.  Thus finite-fibre certificates do not
  force the cross-resolution finite-state transfer property.
\end{enumerate}
The Richardson item is relative to supplied finite pure EML witnesses for
\(\pi\), sine on \(\RR\), and \(|x|\) on \(\RR\setminus\{0\}\); the paper proves
the punctured absolute-value row internally, while the printed
...
semantic normalizer into finite decidable objects or an equality-reflecting
compiler into finite decidable protocol objects existed on one of these verified
fragments, equality of the resulting finite objects would decide identity to
zero in the Richardson fragment.  This contradiction is the decidability
...
The positive \(L1\) protocol is \Cref{thm:zeckendorf-protocol-univ}.  The
same-level obstruction is \Cref{thm:level-zero-obstruction}, refined by the
classification in
\Cref{thm:one-orbit-readout-classification,thm:one-orbit-faithful-submonoid-classification,thm:one-orbit-faithful-cyclic-classification}.  Those results prove that
under the one-free-monogenic-orbit and ambient-product readout hypotheses every
readout factors through the prime-weight exponent skeleton and every faithful
...
\Cref{thm:intrinsic-fold-moment-transfer,thm:intrinsic-second-moment-recurrence,cor:polynomial-intrinsic-fiber-statistics,thm:intrinsic-fold-fibre-height,cor:linfty-fold-entropy,thm:moment-height-cover-trichotomy}.  In particular
\Cref{thm:intrinsic-fold-fibre-height} gives the two displayed height formulas,
