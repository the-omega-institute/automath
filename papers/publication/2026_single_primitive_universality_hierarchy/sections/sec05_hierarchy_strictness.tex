...
\Cref{thm:main-hierarchy}.  The Richardson part is stated relative to any finite
pure EML triple whose three real-domain identities have been proved.  The paper
proves the punctured absolute-value row internally; the printed strings
...
No conclusion about \(\pi\) or sine is drawn from finite syntax alone.
\end{proof}

\begin{proposition}[Exact printed-string instantiation criterion]
\label{prop:printed-pi-sine-instantiation-criterion}
For the exact printed strings \(U_{47}\) and \(U_{111}\), the Richardson
obstruction in \Cref{thm:separation,thm:proc-ams-relative-richardson-theorem}
...
\]
are supplied for those exact strings in the principal-branch EML semantics.  The
finite catalogue and parser certificate alone supply only membership in the pure
EML grammar, so they do not by themselves give an unconditional printed-string
Richardson instance.
\end{proposition}
...
Conversely, the concrete printed-string specialization of that interface uses
\(U_{47}\) in the \(\pi\)-clause and \(U_{111}\) in the sine clause of the
structural translation.  Those clauses are exactly the first two displayed
semantic hypotheses in
...
which are the \(U_{47},U_{111}\) part of \((H_{\mathcal W})\).
\end{proof}
...
translation in \Cref{lem:richardson-translation,lem:richardson-translation-bookkeeping}.
...
agree with \(|x|\) only away from zero.
\end{proof}
...
on the two printed-string identities
\(W_{\pi}=\pi\) and \(W_{\sin}(x)=\sin x\) for \(x\in\RR\).  All other rows needed
...
precisely the two semantic identities named in the statement.  Richardson's
...
\begin{theorem}[Conditional Richardson fragment]
...
and therefore the translated EML fragment has undecidable zero identity.
\end{theorem}
...
zero-identity algorithm for the translated EML fragment would decide the
undecidable source identity-to-zero problem.
\end{proof}
...
such pair \((\mathsf P,\mathsf S)\).  The printed triple
\((U_{47},U_{111},U_{126})\) is not used unless the same punctured identities are
independently proved for those exact strings.
\end{theorem}
...
\Cref{thm:eml-punctured-richardson-core}.  If the printed strings are proposed as
a replacement, the same identities must be proved for them; provenance alone
supplies no semantic equality.
\end{proof}
...
exactly matching Richardson's hypothesis on \(\mu\).
\end{proof}
...
evaluated, the value of \(F\) is nonzero.  Then
\(\tau_{\mathsf P,\mathsf S,\mathsf A}(E)\) is defined on \(D_E\) and equals
\(E\) there.  In particular,
\[
  E=0\hbox{ on }D_E
  ...
  \hbox{ on }D_E .
\]
Zeros of absolute-value arguments therefore delete the same input points from
both sides and do not invalidate the many-one reduction.
\end{theorem}

\begin{proof}
Proceed by structural induction on \(E\).  Constants, the variable, arithmetic,
composition, and \(\exp\) use the internally proved core identities and their
ordinary real domains.  The \(\pi\) and \(\sin\) clauses use the first two
displayed hypotheses.  For a subterm \(\mu(F)\), the recursive definition of
\(D_{\mu(F)}\) requires \(x\in D_F\) and \(F(x)\ne0\).  By the induction
hypothesis,
...
EML \(\mathsf A\)-clause is used; no value at that point is needed.  Thus each
...
For each fixed finite replacement triple \((\mathsf P,\mathsf S,\mathsf A)\),
there is a constant \(C\), depending only on the internally proved core macros
and on that triple, such that every Richardson expression \(E\) with \(|E|\)
source nodes translates to a pure EML term
\(\tau_{\mathsf P,\mathsf S,\mathsf A}(E)\) with at most \(C|E|\) macro nodes
before expansion and effectively computable finite expanded EML syntax.
\end{lemma}
...
\begin{theorem}[Triple-relative Richardson obstruction interface]
...
Fix the principal-branch pure EML semantics of \Cref{def:eml} and finite pure EML
terms \((\mathsf P,\mathsf S,\mathsf A)\) satisfying
\[
  \mathsf P=\pi,
  \qquad \mathsf S(x)=\sin x\quad(x\in\RR),
  \qquad \mathsf A(x)=|x|\quad(x\ne0).
\]
Richardson's identity-to-zero problem many-one reduces to zero-identity on the
pure EML fragment translated using this triple.  Consequently that translated
fragment admits no zero-identity algorithm, no computable semantic normalizer
into finite objects with decidable equality, and no equality-reflecting semantic
compiler into finite decidable protocol objects.
...
\[
  ...
  \hbox{ on }D_E .
\]
Since Richardson's identity-to-zero problem for \(\mathcal R_{\mathrm{core}}\) is
undecidable by \Cref{lem:richardson-fragment}, this is a many-one reduction.  A
zero-identity algorithm on the translated fragment would decide the Richardson
problem.  A computable semantic normalizer into finite objects with decidable
equality would decide the same problem by normalizing
\(\tau_{\mathsf P,\mathsf S,\mathsf A}(E)\) and
\(\tau_{\mathsf P,\mathsf S,\mathsf A}(0)\) and comparing the results; this is the
...
translated zero term on the same domain.  Since the identity-to-zero problem for
\(\mathcal R_{\mathrm{core}}\) is undecidable by \Cref{lem:richardson-fragment},
this is a many-one reduction to the replacement EML fragment.  A zero-identity algorithm
...
The Proc. AMS Richardson theorem stream is conditional on a finite punctured
triple \((\mathsf P,\mathsf S,\mathsf A)\) with proved identities for \(\pi\),
sine, and \(|x|\) off zero.  The paper proves the internal absolute-value option
\(\mathsf A=\Abs\); the printed pair \((W_{\pi},W_{\sin})\) instantiates the other
two rows only under \((H_{\mathcal W})\).  It contains no unconditional
endpoint-root instance for \(U_{126}\).
\end{corollary}

\begin{proof}
This is an immediate extraction from \Cref{thm:separation} and
\Cref{prop:three-witness-stability}.  The endpoint obstruction in
\Cref{thm:separation} shows why no pure EML term defined at \(0\) can be
substituted for the positive root term.  Thus the Richardson input to
\Cref{thm:obstruction-spine,thm:main-hierarchy} is the punctured triple interface,
while the endpoint-root string \(U_{126}\) remains outside that input.
\end{proof}

\begin{remark}[Dependency cut]
The source-locking and \(\mathcal B_{36}\)-noninstantiation clauses are separate
from the Richardson obstruction: \Cref{thm:submission-richardson-dichotomy} uses
...
\((\eml,1)\) does not provide a computable semantic normalizer for the translated
Richardson fragment.
\end{corollary}

\begin{proof}
A computable semantic normalizer for the translated punctured fragment would
contradict the normalizer obstruction in \Cref{thm:separation}.  The only
