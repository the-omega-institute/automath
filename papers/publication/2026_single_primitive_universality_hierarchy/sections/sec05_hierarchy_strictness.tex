...
\]
Equivalently, Richardson's Theorem~2 supplies the source problem needed here as a
punctured partial-function identity problem, with no separate value of \(\mu\) at
zeros of its argument.
\end{theorem}

\begin{proof}
Richardson's stated hypothesis on the auxiliary unary function is the punctured
condition \(\mu(x)=|x|\) for \(x\ne0\)
\cite[pp.~514, 520, Theorem~2]{Richardson1968}.  The theorem is therefore read on
the common real domain where all displayed operations are legitimate and where
the \(\mu\)-clause is invoked only at nonzero arguments.  That common domain is
exactly the recursive set \(D_E\) above.  The cited theorem asserts that identity
to zero for this language is not decidable; with the domain convention made
explicit, this is precisely the displayed decision problem.  No completion of
\(\mu\) at zero and no endpoint value of a square-root term is used in the cited
source problem.
\end{proof}
