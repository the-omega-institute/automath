...
proves the punctured absolute-value row internally; the printed pair
\((U_{47},U_{111})\) appears only as the conditional cited specialization
\((H_{\mathcal W})\) from \Cref{ass:HW}, and the endpoint-root string \(U_{126}\)
is not a row of the Richardson package.
...
The following decision problem is undecidable: given a Richardson expression
\(E\), determine whether
\[
  E(x)=0\qquad\hbox{for every }x\in D_E .
\]
This is the partial-domain form of Richardson's identity problem needed below:
the value of \(\mu\) at zeros of its argument is never queried.
\end{theorem}
...
that identifies exactly equality of the two partial functions after restriction
to the source domain \(D_E\).
\end{theorem}
...
\Cref{lem:richardson-translation,lem:richardson-translation-bookkeeping}.
...
\begin{theorem}[Verified-pair Richardson instantiation]
...
EML fragment.  More precisely, there is an effective translation
...
equality, and no equality-reflecting semantic compiler into finite decidable
protocol objects.  The exact printed pair \((U_{47},U_{111})\) is one such pair
only under \((H_{\mathcal W})\); without that hypothesis this theorem remains the
replacement-pair statement and has no printed-string specialization.
\end{theorem}
...
finite objects.  The final printed-pair sentence is exactly
...
fragment would decide the undecidable source identity-to-zero problem.
\end{proof}
...
used unless the same \(\pi\) and sine identities are independently proved for
those exact strings, and \(U_{126}\) is not a witness row in the punctured
Richardson interface.
\end{theorem}
...
using this verified triple.  Consequently that transported fragment admits no
...
The theorem has an unconditional concrete instance for every verified triple
\((\mathsf P,\mathsf S,\mathsf A)\).  In particular the internally proved
punctured row \(\Abs(u)=|u|\) from \Cref{thm:eml-punctured-richardson-core}
may serve as \(\mathsf A\).  The exact printed pair
\((U_{47},U_{111})\) gives the printed-string specialization of this theorem
only after the two real-domain identities
\(U_{47}=\pi\) and \(U_{111}(x)=\sin x\) for \(x\in\RR\) have been supplied in
the principal-branch semantics.  The finite syntax catalogue by itself is not
one of the verified triples covered by the unconditional part of the theorem.
...
with decidable equality would decide the same problem by normalizing the
source-indexed restrictions of
\(\tau_{\mathsf P,\mathsf S,\mathsf A}(E)\) and
...
The specialization statement is formal bookkeeping.  The preceding argument used
only the three displayed identities for the chosen triple
\((\mathsf P,\mathsf S,\mathsf A)\), so any triple for which those identities have
been proved is an unconditional instance of the interface.  The term
...
base identities for \(\pi\) and sine.  Thus those printed strings enter this
theorem only under the corresponding verified-pair hypothesis, namely the
\(U_{47},U_{111}\) part of \((H_{\mathcal W})\).
...
normalizer into finite objects with decidable equality would decide it by
normalizing the two source-restricted translated terms and comparing the normal
forms; and an equality-reflecting semantic compiler into finite decidable
protocol objects would decide it by compiling the two source-restricted
translated terms and comparing the target objects.  These are precisely the
...
\begin{theorem}[Unconditional submission closure of the Richardson clause]
...
is exactly the following verified-triple theorem and nothing stronger: for every
...
computable semantic normalizer into finite decidable objects, and no
...
are supplied as proved hypotheses.  The cited \(\mathcal B_{36}\) representative
existence statement and the finite syntax catalogue do not alter this closure.
\end{theorem}
...
specialization, not the verified-triple theorem itself.

Finally, \Cref{thm:B36-import-exact-scope,thm:B36-self-contained-replacement-core}
...
witness rows.  Consequently neither that citation nor the syntax catalogue can
create an unconditional printed \((U_{47},U_{111})\) Richardson instance, and the
submission-facing Richardson clause is precisely the verified-triple closure
stated above.
\end{proof}
