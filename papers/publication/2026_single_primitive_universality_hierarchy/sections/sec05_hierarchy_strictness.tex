...
The obstruction of \Cref{thm:separation} is unchanged if the explicit witnesses
\((W_{\pi},W_{\sin},\Abs)\) are replaced by any finite pure EML witnesses for
\(\pi\), sine on \(\RR\), and \(|x|\) on \(\RR\setminus\{0\}\).  It is not
instantiated by the strings \((U_{47},U_{111},U_{126})\) unless those exact
punctured identities are proved for those strings.
\end{proposition}

\begin{proof}
This is the replacement argument in
\Cref{thm:relative-finite-richardson-fragment}: the structural recursion and the
normalizer reduction inspect only the three semantic identities, not the syntax
or provenance of the witnesses.
\end{proof}
