...
two printed strings.  Consequently every theorem, corollary, or proof in this
paper that names the concrete pair \((U_{47},U_{111})\) as the \(\pi\)/sine rows
of a Richardson triple is to be read under \((H_{\mathcal W})\) or under a
separate proof of exactly the two displayed identities; without one of those
inputs the statement reverts to the abstract verified-pair interface.
\end{proposition}
...
on the two source-locked printed-string identities
\(W_{\pi}=U_{47}=\pi\) and
\(W_{\sin}=U_{111}\) with \(W_{\sin}(x)=\sin x\) for \(x\in\RR\).  These two
identities are exactly the optional \((H_{\mathcal W})\) clauses used for a
printed-name specialization, unless they are proved separately for the exact
printed strings.  All other rows needed
...
\end{proof}
The submission Richardson scope is therefore the following.  The Proc. AMS
Richardson theorem stream is conditional on a finite punctured triple
\((\mathsf P,\mathsf S,\mathsf A)\) with proved identities for \(\pi\), sine, and
\(|x|\) off zero.  The paper proves the internal absolute-value option
