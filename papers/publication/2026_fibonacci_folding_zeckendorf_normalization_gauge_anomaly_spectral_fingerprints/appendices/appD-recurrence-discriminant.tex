...
D(t;y).
\]
The boundary-vector realization used in Section~\ref{sec:partition} is
\[
...
\end{pmatrix}.
\]
Here \(\alpha\) singles out the synchronized initial state \(00\) forced by the high-weight padding, while \(\beta(y)\) collects the contributions of the finite completions needed to forget the terminal state: the follower sets in Proposition~\ref{prop:transitions} show that states \(01\) and \(11\) resolve after one deterministic step and state \(10\) resolves after either one or two steps, leading to the simple rational generating functions displayed above. The rational entries of \(\beta(y)\) are therefore an artifact of this compact \(4\times4\) presentation; after multiplication by \(M(y)^m\) all poles cancel and one recovers the polynomial \(Z_m(y)\).

\subsection{Numerator computation}
The generating function \(Z(t;y)=\sum_{m\ge 0}Z_m(y)t^m\) satisfies \(Z(t;y)=N(t;y)/D(t;y)\), where \(N(t;y)=Z(t;y)\cdot D(t;y)\) is determined by the initial values. Setting
\[
N(t;y)=\sum_{m=0}^{3}\left(Z_m(y)-\sum_{k=1}^{\min(m,4)}c_k(y)Z_{m-k}(y)\right)t^m,
\]
...
\emph{At \(y=0\):} \(\Pi(\lambda,0)=\lambda^4-\lambda^3-\lambda^2+\lambda=\lambda(\lambda-1)^2(\lambda+1)\). The repeated root is \(\lambda=1\), which is dominant. At this parameter value, \(Z_m(0)=\abs{\Fold_m^{-1}(0^m)}\) counts microstate words whose fold is the all-zeros word; the growth is polynomial in \(m\) (roughly \(\lceil(m+1)/2\rceil\)), reflecting the multiplicity of the dominant root.
