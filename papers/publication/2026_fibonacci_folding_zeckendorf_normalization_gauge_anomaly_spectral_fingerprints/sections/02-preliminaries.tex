...
We adopt the convention that position \(k\) carries weight \(\Fib_{k+1}\). Thus position \(1\) has weight \(\Fib_2=1\) and position \(2\) has weight \(\Fib_3=2\). This is shifted by one from some common numeration conventions; compare \cite{Fraenkel1985,Frougny1992,FrougnySolomyak1996,Berstel1985,AlloucheShallit2003}. We call this the \emph{low-to-high} (LH) convention because position \(1\) carries the lowest weight. The classical numeration-theoretic convention writes the most significant digit first (MSD-first); see Section~\ref{sec:multiscale} for a precise translation between the two. Both conventions appear in this paper: the LH convention is used for all formulas involving window spaces \(\Om_m\) and \(X_m\), while the MSD-first convention is used in the normalization rewriting system (Appendix~\ref{app:rewriting}) and in the boundary-state transducer (Appendix~\ref{app:pressure}), where left-to-right processing is natural.
\end{remark}
