...
The four-state presentation from Section~\ref{sec:gauge} also yields a transfer-matrix description of the fibers of \(\Fold_m\). We keep in the main text the consequences needed for asymptotics and for the variational interpretation, and leave the linear-algebraic eliminations to Appendix~\ref{app:recurrence}.
...
\]
so \(Z_m(y)\) is the moment generating polynomial of the output Hamming weight under the uniform measure on \(\Om_m\). We write
\[
...
\subsection{Recurrence and exceptional parameters}
...
The rational function \(Z(t;y)=N(t;y)/D(t;y)\) has the following exceptional behavior.
\begin{enumerate}
...
\item At \(y=1\), one has \(N(t;1)=-(t-1)(t+1)^2\) and \(D(t;1)=(t-1)(t+1)^2(2t-1)\). After cancellation, \(Z(t;1)=1/(1-2t)\), consistently with \(Z_m(1)=2^m\). The effective recurrence order drops to \(1\).
\item For all \(y\in\CC\setminus\{0,1\}\), the numerator and denominator are coprime, and the effective recurrence order is exactly \(4\).
\end{enumerate}
\end{corollary}

\begin{proof}
The first two assertions follow by direct factorization. For the third, compute the resultant of \(N(t;y)\) and \(D(t;y)\) with respect to \(t\); its only roots in \(y\) are \(0\) and \(1\). Hence for \(y\notin\{0,1\}\) the rational function is in lowest terms with denominator degree \(4\).
\end{proof}
...
\]
Thus the repeated-root locus consists of \(y=0\), \(y=1\), and the roots of \(256y^3+411y^2+165y+32\).
\end{proposition}

\begin{proof}
Appendix~\ref{app:recurrence} computes and factors the discriminant, giving \eqref{eq:disc}. Let
\[
q(y):=256y^3+411y^2+165y+32.
\]
Since \(q(-2)=-702<0\) and \(q(-1)=22>0\), there is a root in \((-2,-1)\). Moreover,
\[
q'(y)=768y^2+822y+165
\]
has both real roots in \((-1,0)\), so \(q'(y)>0\) for \(y\le -1\). Hence \(q\) is strictly increasing on \((-\infty,-1]\), and the root in \((-2,-1)\) is unique.
\end{proof}

For \(y>0\), Perron--Frobenius implies that the dominant root \(\lambda_\star(y)\) of \(\Pi(\lambda,y)\) is real, positive, and simple. The only nonnegative discriminant zeros are \(0\) and \(1\); at \(y=1\) the repeated root is subdominant, while \(y=0\) is a boundary point. Thus the free energy
\[
...
\]
The convergence in probability and the central limit theorem therefore follow from the standard perturbative theory for primitive matrix families \cite[Chapter~4]{ParryPollicott1990}, \cite[Chapter~6]{Ruelle2004}. Since \(\theta\mapsto M(e^\theta)\) is analytic for all \(\theta\in\RR\), the G\"artner--Ellis theorem applies and yields the stated large deviation principle \cite[Theorem~2.3.6]{DemboZeitouni1998}.
\end{proof}
...
\begin{theorem}[Microcanonical entropy profile]\label{thm:microcanonical-output}
...
Theorem~\ref{thm:microcanonical-output} shows that the weighted theory is not merely a recurrence calculation: the same transfer matrix that governs \(Z_m(y)\) also realizes a genuine entropy maximization problem on the pair shift.

For comparison, interval-based Zeckendorf statistics \cite{KologluKoppMillerWang2011,MillerWang2012,BowerInsoftLiMillerTosteson2015} study the decomposition of a uniformly chosen integer in \([1,\Fib_n]\). In that model the summand density converges to \(1/(\varphi^2+1)\approx 0.2764\), whereas the fold-pushforward measure \(w_m\) yields the density \(5/18\approx 0.2778\). The similarity of the constants is accidental: the underlying sampling measures are different.
