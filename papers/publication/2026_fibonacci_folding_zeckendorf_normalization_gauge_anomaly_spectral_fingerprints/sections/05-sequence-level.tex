...
The answer is uniform in the window length despite the unbounded carry cascades of Fibonacci normalization. The sharp whole-block threshold below supplies the arithmetic backbone; the subsequent decoder theorem extracts one current digit from only three labels and yields the conjugacy.
...
Theorem~\ref{thm:block-separation} says that $m$ consecutive labels determine their entire length-$(2m-1)$ lift. Recovering only the current digit requires fewer labels, and the required number does not grow with $m$.
