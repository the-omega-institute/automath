...
=M+\Fib_{m+1}-2.
\]
Its normalized expansion therefore has at most one digit beyond the window, at position $m+1$, and the retained digits are the Zeckendorf expansion of its value modulo $M$. Thus two length-$m$ words have the same fold precisely when their values are congruent modulo $M$.
...
\begin{theorem}[Finite-window decoder]\label{thm:window-decoder}
Fix $m\ge3$. Every admissible block of $m$ consecutive symbols in $Y_m$ has a unique lift to a binary block of length $2m-1$. Consequently there is a well-defined decoder
\[
\psi_m:\mathcal{B}_m(Y_m)\to\{0,1\},
\qquad
\eta_t=\psi_m(y_{t-m+1},\ldots,y_t)
\]
whenever $y=\Phi_m(\eta)$.
\end{theorem}

\begin{proof}
An admissible block $(y_{t-m+1},\ldots,y_t)$ is the folded-window sequence of a binary block on the coordinate interval $[t-2m+2,t]$, which has length $2m-1$. Existence follows from admissibility, and uniqueness follows from \Cref{thm:block-separation}. Define $\psi_m$ to be the last digit of this unique lift.
\end{proof}

\begin{remark}\label{rem:window-bound-essential}
The dependence on $m$ is essential. The sharpness family in \Cref{thm:block-separation} gives ambiguities of length $2m-2$ even though the elementary Zeckendorf rewrite has fixed span $3$. Thus the decoding bound is controlled by the boundary propagation encoded in \eqref{eq:window-boundary-identity}, not by the support of a single rewrite.
\end{remark}
