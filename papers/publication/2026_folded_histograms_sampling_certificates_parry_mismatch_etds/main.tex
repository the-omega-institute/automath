...
The canonical fold starts from Zeckendorf uniqueness
\cite{Zeckendorf1972} and finite-automaton normalization in Fibonacci
numeration \cite{Frougny1991FibonacciFiniteAutomata,
Frougny1992NumbersAutomata}, but Definition~\ref{def:canonical-fold} fixes a
finite-resolution histogram map rather than a general numeration system.
Proposition~\ref{prop:canonical-fold-basic} records the standard consequences
of that normalization for the present map, while
Proposition~\ref{prop:fold-residue-characterization} adds the exact modulo
\(F_{m+2}\) residue test used throughout the paper. The microstate and target
law definitions specialize the usual sliding-block and pushforward language of
symbolic dynamics \cite{LindMarcus1995} to the rotation-and-fold audit. The
endpoint-collapse formula for the golden-mean Parry law is a direct
specialization of Parry's intrinsic Markov-chain formula
\cite{Parry1964IntrinsicMarkovChains}; it is used as a building block, not as a
new theorem. The continued-fraction cylinder estimates now stated as
Proposition~\ref{thm:spg-sturmian-cylinder-measure-continued-fraction-sandwich}
are classical consequences of Slater's gap theorems
\cite{Slater1950DistributionIntegers,Slater1967GapsSteps}.

The discrepancy certificate in Theorem~\ref{thm:tv-certificate-hist} uses the
classical star-discrepancy estimates of Kuipers--Niederreiter
\cite{KuipersNiederreiter1974}, but the increment is the explicit simultaneous
\(2m\) and \(m+1\) total-variation control of both microstate and folded laws.
Corollary~\ref{cor:metallic-mean-golden-unique-optimal-tv-constant} adds the
metallic-mean leading constant and its golden-branch minimizer. The boundary
law in Theorem~\ref{prop:multiscale-residual-by-boundary} is specific to the
canonical fold and identifies the exact threshold event for failure of
truncation commutation. Proposition~\ref{prop:multiscale-residual-parry-bound}
is just triangle inequality plus projective consistency of the Parry family,
whereas Corollary~\ref{cor:multiscale-residual-audit-bound} inserts the
sampling certificate and the structural Parry gaps. Lemma~\ref{lem:kl-from-tv-qmin}
uses standard \(f\)-divergence inequalities \cite{SasonVerdu2016fDivergence},
Definition~\ref{def:diophantine-lower-bound} is classical Diophantine
terminology \cite{Cassels1957,Schmidt1969BadlyApproximableSystems}, and
Proposition~\ref{prop:qmin-lowerbound-markov} applies the rotation gap bound
to the smallest Sturmian atom. The two empirical KL certificates and
Lemma~\ref{lem:kl-monotone-pushforward} are conditional consequences of these
inputs and the standard data-processing inequality \cite{CoverThomas2006}.

The Sturmian rigidity theorem is the first genuinely model-specific structural
result. Classical Sturmian factor partitions, complexity, and higher-block
presentations are due to Morse--Hedlund and standard symbolic dynamics
\cite{MorseHedlund1940,LindMarcus1995,Fogg2002}; Theorem~\ref{thm:sturmian-no-collision-rigidity}
adds the Fibonacci-jump and modulo-\(F_{m+2}\) no-collision certificate for the
canonical fold. Corollary~\ref{cor:sturmian-exact-relabeling} converts that
certificate into exact invariance of total variation, KL divergence, and all
Renyi entropies under the realized relabeling. Theorem~\ref{thm:sequence-level-transparency}
is a conditional higher-block conjugacy statement: once the no-collision
certificate is known, the fold is a letter relabeling of the classical
\(m\)-block presentation. Corollary~\ref{cor:sturmian-pure-degeneracy-ledger}
then uses the finite fiber-uniform entropy ledger to isolate hidden
within-fiber degeneracy, and Corollary~\ref{cor:tv-decomposition-parry} is the
corresponding triangle decomposition against the Parry baseline.

The Renyi entropy and divergence definition follows Renyi's original
information measures \cite{Renyi1961EntropyInformation}. Theorem~\ref{thm:parry-kl-identity}
adds a finite-\(m\) exact KL identity for the golden-mean Parry law, including
the endpoint correction, rather than only the usual Parry cylinder formula.
Corollary~\ref{cor:no-fold-entropy-loss} inserts the Sturmian relabeling into
that identity. Proposition~\ref{prop:parry-renyi-window} is a conditional
Renyi-order window derived from endpoint collapse, while
Proposition~\ref{prop:renyi-support-conditioning} is the standard conditioning
identity for divergences. Lemma~\ref{lem:endpoint-class-counts} is the usual
Fibonacci count of endpoint classes in the golden-mean graph. Theorem~\ref{thm:parry-support-barrier}
adds the exact three-branch finite-\(m\) support envelope \(M_m(k)\), the
support-conditioned Renyi minimizers, and the matching total-variation
minimum. Corollary~\ref{cor:exact-complexity-divergence-law} specializes that
envelope to the low-complexity branch. Theorem~\ref{thm:optimal-injective-relabeling}
keeps the Sturmian weight vector fixed and identifies the exact best injective
placement in \(X_m^{00}\), while the bounded-type entropy and Renyi-window
propositions combine Slater gap estimates with the Parry window to give the
uniform \(m\log\varphi-\log m+O_\alpha(1)\) comparison.

Corollary~\ref{cor:injective-relabeling-bounded-type} and
Theorem~\ref{thm:lossless-coarse-graining-envelope} are conditional consequences
of the injective placement theorem; their increment is to state the coding
invariance of the Parry barrier for every lossless deterministic map into the
same golden-mean language. The primitive-SFT results use only the classical
Perron--Frobenius and Parry block formulae
\cite{Parry1964IntrinsicMarkovChains,LindMarcus1995,ParryPollicott1990Zeta};
Theorem~\ref{thm:primitive-sft-renyi-barrier} adds an order-uniform support
exponent barrier for arbitrary primitive shifts, and
Theorem~\ref{thm:primitive-sft-injective-barrier} adds the finite-block
heaviest-endpoint injective placement bound. Corollary~\ref{cor:low-complexity-source-barrier}
specializes that placement result to subexponential and linear-complexity
sources. Finally, Theorem~\ref{thm:audit-conjugacy-invariance} uses the
classical Denjoy conjugacy theorem for \(C^2\) circle diffeomorphisms with
irrational rotation number \cite{Denjoy1932CourbesTore,KatokHasselblatt1995}
and adds the exact transfer of the two-stage audit through the conjugating
window.
...
a nonlinear circle-map realization of the audit. The appendices collect the
optional microstate-level lift identities, a local normalization description of
the fold, and the uniqueness statement for the Fibonacci weighting.
