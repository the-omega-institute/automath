...
\begin{proof}
Apply the triangle inequality around the projectively consistent pair
$(\pi_m,(\rho_{m+1\to m})_*\pi_{m+1})$ and use monotonicity of total
variation under the deterministic map \(\rho_{m+1\to m}\):
\[
\begin{aligned}
E_{m,N}
&\le
\TV(\hat\pi_{m,N},\pi_m)
+
\TV\bigl((\rho_{m+1\to m})_*\pi_{m+1},
(\rho_{m+1\to m})_*\hat\pi_{m+1,N}\bigr)\\
&\le
\TV(\hat\pi_{m,N},\pi_m)+\TV(\pi_{m+1},\hat\pi_{m+1,N}).
\end{aligned}
\]
\end{proof}
...
\begin{corollary}[Finite-scale microstate bound]\label{cor:rotation-microstate-kl-certificate}
...
\begin{proof}
The half-open coding convention assigns every boundary point of the finite
rotation partition to one of the adjacent atoms, and each such atom has
positive Lebesgue length. Hence
\(\supp(\hat\mu_{m,N})\subset\supp(\mu_m)\), so
Lemma~\ref{lem:kl-from-tv-qmin} applies.
...
The proof uses three features special to the canonical slice \(\beta=\alpha\):
the Sturmian partition of the circle into \(m+1\) atoms, the Fibonacci weights
in \(N_m\), and the residue characterization of \(\Fold_m\) modulo
\(F_{m+2}\).  We shall not use, or claim, an analogous jumps-and-residues
statement for other folds.
\end{remark}
