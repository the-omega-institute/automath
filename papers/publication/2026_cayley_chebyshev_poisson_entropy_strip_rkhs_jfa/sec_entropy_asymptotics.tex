...
Theorem~\ref{thm:poisson-kl-eighth}; the later defect identities are
algebraic consequences of those coefficients.
...
\begin{lemma}[Uniform entropy-expansion remainder]%
...
For $t\ge T_\nu$ the function
\[
\Phi(\delta_t):=(1+\delta_t)\log(1+\delta_t)-\delta_t
\]
is integrable against $\omega(\dd y)=\pi^{-1}(1+y^2)^{-1}\dd y$, and
\begin{equation}\label{eq:uniform-Phi-expansion}
\Phi(\delta_t)
=\sum_{k=2}^{N}\frac{(-1)^k}{k(k-1)}\delta_t^k+\eta_{N,t},
\qquad
\sup_{y\in\RR}|\eta_{N,t}(y)|\le C_\nu t^{-(2N+2)}.
\end{equation}
Moreover, for every $2\le k\le N$,
...
\varepsilon_{k,N,t}(y),
\end{equation}
where $\sup_y|\varepsilon_{k,N,t}(y)|=O(t^{-(2N+1)})$.  Consequently all
remainders in \eqref{eq:uniform-Phi-expansion} and
...
Choose $T_\nu$ so large that the right-hand side of
...
one, so uniform boundedness implies the asserted integrability.

Finally write
\[
A_t(y):=\sum_{n=2}^{2N}\frac{\mu_n}{t^n}u_n(y).
\]
The modes are bounded, so $A_t=O(t^{-2})$ uniformly.  Expanding the
finite product $A_t(y)^k$ gives the displayed sum in
\eqref{eq:uniform-power-expansion} plus terms of total degree at least
$2N+1$, which are uniformly $O(t^{-(2N+1)})$.  The difference
$(A_t+\rho_{N,t})^k-A_t^k$ contains at least one factor $\rho_{N,t}$ and
is therefore $O(t^{-(2N+1)}t^{-2(k-1)})$, hence also
$O(t^{-(2N+1)})$.  Since $\omega(\RR)=1$, the same estimates hold in
$L^1(\omega)$ and are $o(t^{-2N})$.
\end{proof}
...
integrate term by term.  Therefore
\[
D_{\mathrm{KL}}(h_t\|g_t)
...
\]
Thus, before inserting the integral identities, the coefficient ledger is
\begin{align*}
[t^{-4}]\Phi(\delta_t)
&=\frac12 a_2^2,\\
[t^{-6}]\Phi(\delta_t)
...
\end{align*}
The possible odd-order terms are
\[
[t^{-5}]\Phi(\delta_t)=a_2a_3,
\qquad
[t^{-7}]\Phi(\delta_t)
=a_2a_5+a_3a_4-\frac12a_2^2a_3.
\]
By Lemma~\ref{lem:mode-parity-vanish}, their integrals against
$\pi^{-1}(1+y^2)^{-1}\dd y$ vanish.
...
$t^{-6}$ is the first coefficient beyond the variance term furnished by
...
moment relation, while \eqref{eq:defect-c8} is the first genuinely new
