% sec_gram_space.tex -- inactive archival fragment.
% Not part of the active article source graph rooted at main.tex.
% Activated as part of the main RKHS theorem package.
...
\relocatedproof{sec_gram_space.tex}{the elementary residue calculation following
Theorem ``The mode Gram kernel''}
\else
\begin{proof}
For $\varepsilon\neq\eta$ define
\[
...
\ifdefined\JFAArticleMain
The closed quadratic-mode table and its coefficient-comparison proof are in
\path{sec_gram_space.tex}, Corollary ``Closed formulas for quadratic mode
integrals,'' collected by \path{supplement_relocated_support.tex}.
\else
...
\ifdefined\JFAArticleMain
The completeness lemma for the secant modes, including the Poisson-transform
and Fourier-injectivity proof, is in \path{sec_gram_space.tex}, Lemma
``Poisson-transform completeness of the secant modes,'' collected by
\path{supplement_relocated_support.tex}.
\else
\begin{lemma}[Poisson-transform completeness of the secant modes]%
\label{lem:poisson-injectivity-weighted}
Let
\[
\omega(\dd y)=\frac{\dd y}{\pi(1+y^2)}
\]
and let \(f\in L^2(\omega)\) satisfy
\[
\int_{\RR}f(y)\,\omega(\dd y)=0 .
\]
If
\[
\langle f,\Psi_\varepsilon\rangle_{L^2(\omega)}=0
\qquad(\varepsilon\in\RR),
\]
then \(f=0\) in \(L^2(\omega)\).  Equivalently, the algebraic span of
\(\{\Psi_\varepsilon:\varepsilon\in\RR\}\) is dense in \(L^2_0(\omega)\).
\end{lemma}

\begin{proof}
For \(\varepsilon\ne0\),
\[
\Psi_\varepsilon=\frac{R_\varepsilon-1}{\varepsilon}.
\]
Since \(f\) has zero \(\omega\)-mean, the assumed orthogonality gives
\[
0=\langle f,\Psi_\varepsilon\rangle_{L^2(\omega)}
=\frac1{\varepsilon}\int_{\RR}f(y)R_\varepsilon(y)\,\omega(\dd y)
=\frac1{\varepsilon}\int_{\RR}
\frac{f(y)}{\pi(1+(y-\varepsilon)^2)}\,\dd y .
\]
The last integral is absolutely finite.  Indeed,
\[
\int_{\RR}\frac{|f(y)|}{1+(y-\varepsilon)^2}\,\dd y
\le
\left(\int_{\RR}\frac{|f(y)|^2}{1+y^2}\,\dd y\right)^{1/2}
\left(\int_{\RR}
\frac{1+y^2}{(1+(y-\varepsilon)^2)^2}\,\dd y\right)^{1/2},
\]
and the second factor is finite for every fixed \(\varepsilon\).  Thus, for
every \(\varepsilon\ne0\),
\[
\int_{\RR}P_1(\varepsilon-y)f(y)\,\dd y=0,
\qquad
P_1(x)=\pi^{-1}(1+x^2)^{-1}.
\]
The same Cauchy--Schwarz estimate, now with \(\varepsilon\) restricted to an
arbitrary compact interval, gives a locally uniform integrable majorant for
the difference quotients in \(\varepsilon\).  Hence the displayed Poisson
transform is continuous in \(\varepsilon\).  Since it vanishes on
\(\RR\setminus\{0\}\), it also vanishes at \(0\):
\[
(P_1*f)(\varepsilon)=0\qquad(\varepsilon\in\RR).
\]

The function \(f\) defines a tempered distribution.  For example
\[
\int_{\RR}\frac{|f(y)|}{1+y^2}\,\dd y
\le
\left(\int_{\RR}\frac{|f(y)|^2}{1+y^2}\,\dd y\right)^{1/2}
\left(\int_{\RR}\frac{\dd y}{1+y^2}\right)^{1/2}<\infty,
\]
so pairing \(f\) against a Schwartz test function is continuous after
bounding the test function by a Schwartz seminorm times \((1+y^2)^{-1}\).
The preceding identity is therefore the pointwise, and hence
distributional, convolution identity \(P_1*f=0\) on \(\RR\).
Taking Fourier transforms in the convention fixed in this paper gives
\[
e^{-|\xi|}\widehat f(\xi)=0
\]
as tempered distributions.  Let
\(U\Subset(-\infty,0)\cup(0,\infty)\).  On \(U\), the function
\(e^{-|\xi|}\) is \(C^\infty\) and nonvanishing, so multiplication by
\(e^{|\xi|}\) is legitimate on test functions supported in \(U\).  Hence
\(\widehat f=0\) on every such \(U\), and therefore
\[
\operatorname{supp}\widehat f\subset\{0\}.
\]
A tempered distribution supported at one point has the form
\[
\widehat f=\sum_{j=0}^{J}c_j\delta_0^{(j)} .
\]
Consequently \(f\) is a polynomial.  Since
\(f\in L^2(\omega)\), no polynomial of degree at least one can occur:
with \(\omega(\dd y)=\pi^{-1}(1+y^2)^{-1}\dd y\), the square of such a
polynomial is not \(\omega\)-integrable at infinity.  Thus \(f\) is
constant.  The zero-mean condition in \(L^2_0(\omega)\) forces that
constant to be \(0\), so \(f=0\) in \(L^2(\omega)\).
\end{proof}
\fi
...
The density assertion rests on the isolated Poisson-transform completeness
lemma immediately above: orthogonality to all secant kernels, together with
the zero-mean condition, says that the Poisson transform \(P_1*f\) vanishes
on the line.  Fourier injectivity of the strictly positive Poisson multiplier
then forces \(f=0\).
...
0=\langle f,\Psi_\varepsilon\rangle_{L^2(\omega)}.
\]
  The supplementary completeness lemma cited in the statement applies and
  gives $f=0$.
...
\ifdefined\JFAArticleMain
\relocatedproof{sec_gram_space.tex}{the proof following Proposition
``Centred Poisson reconstruction by Laplace uniqueness''}
\else
\begin{proof}
By the elementary Laplace identities
\[
\int_0^\infty e^{-tr}\cos(rX_c)\dd r=\frac{t}{t^2+X_c^2},
\qquad
\int_0^\infty e^{-tr}\sin(rX_c)\dd r=\frac{X_c}{t^2+X_c^2},
\]
one obtains
\[
A(t)
=\int_{\RR}\frac{t}{t^2+X_c^2}\dd\nu(\gamma)
...
=\frac{2}{\pi}\int_{\RR}\frac{t\,X_c}{(X_c^2+t^2)^2}\dd\nu(\gamma),
\]
in particular
\[
B(t)=2t\int_{\RR}\frac{X_c}{(X_c^2+t^2)^2}\dd\nu(\gamma).
\]
Since
\[
\int_t^\infty \frac{2s|X_c|}{(X_c^2+s^2)^2}\dd s
=\frac{|X_c|}{X_c^2+t^2}
\le \frac{1}{2t},
\]
...
\ifdefined\JFAArticleMain
The two symmetry corollaries and their proofs are in
\path{sec_gram_space.tex}, subsection ``Centred reconstruction from Poisson
data,'' collected by \path{supplement_relocated_support.tex}.
\else
...
\mathfrak d_t(y)
=\frac{1}{t}\int_{\RR}X_c\,\Psi_{X_c/t}(y)\,\dd\nu(\gamma).
\end{equation}
Consequently,
\begin{equation}\label{eq:delta-t-rkhs}
(\mathcal U\mathfrak d_t)(a)
=\frac{1}{t}\int_{\RR}X_c\,K(a,X_c/t)\,\dd\nu(\gamma),
\qquad a\in\RR.
\end{equation}
Moreover,
...
\le\frac{1}{t}\int_{\RR}|X_c|\,\left\|\Psi_{X_c/t}\right\|_{L^2(\omega)}\,\dd\nu(\gamma)
=\frac{1}{\sqrt2\,t}\int_{\RR}|X_c|\,\dd\nu(\gamma)
<\infty,
\]
...
=\frac{1}{t}\int_{\RR}X_c\frac{2}{4+(X_c/t)^2}\,\dd\nu(\gamma)
=2t\int_{\RR}\frac{X_c}{4t^2+X_c^2}\,\dd\nu(\gamma).
\]
The definition of $H$ in
Proposition~\ref{prop:poisson-central-two-channel} gives
\[
H(2t)=\int_{\RR}\frac{X_c}{4t^2+X_c^2}\,\dd\nu(\gamma),
\]
