...
\subsection{Higher secant kernels for entropy coefficients}

The multilinear mode integrals in the entropy coefficients are Taylor
coefficients at the origin of multilinear secant germs built from the same
family.
...
\item The jet $\mathfrak E_N(\nu)$ factors through the finite centred moment
      ...
      \mu_{2N})
      \]
      and through the finite germ of the secant kernels
      \[
      \left\{
      \partial_1^{n_1-1}\cdots\partial_k^{n_k-1}
      \Gamma_k(0,
...
\item Consequently no reconstruction rule
      ...
      $\nu\in\mathcal C_N$.
\end{enumerate}
Thus finite entropy jets determine the finite entropy polynomial but not the
underlying law.
\end{proposition}

\begin{proof}
For~(i), Theorem~\ref{thm:higher-secant-kernels} shows that each
