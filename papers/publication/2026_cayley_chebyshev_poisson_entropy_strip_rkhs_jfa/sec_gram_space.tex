...
\]
and the second factor is finite for every fixed \(\varepsilon\).  Thus
\[
\int_{\RR}P_1(\varepsilon-y)f(y)\,\dd y=0
\qquad(\varepsilon\ne0),
\]
where \(P_1(x)=\pi^{-1}(1+x^2)^{-1}\).  The same Cauchy--Schwarz estimate,
with \(\varepsilon\) restricted to a compact interval, gives continuity in
\(\varepsilon\); hence the equality also holds at \(\varepsilon=0\).
...
The preceding identity is therefore the distributional convolution
\[
P_1*f=0\qquad\text{on }\RR .
\]
Taking Fourier transforms in the convention fixed in this paper gives
\[
e^{-|\xi|}\widehat f(\xi)=0
\]
as tempered distributions.  Since \(e^{-|\xi|}\) is a smooth strictly
positive multiplier and \(e^{|\xi|}\) is a smooth multiplier on compact
frequency supports, this implies \(\widehat f=0\) distributionally.  Hence
\(f=0\) as a tempered distribution and therefore \(f=0\) in
\(L^2(\omega)\).
\end{proof}
