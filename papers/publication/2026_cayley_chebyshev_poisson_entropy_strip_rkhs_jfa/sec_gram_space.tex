...
\begin{corollary}[Even--odd channel dichotomy and the symmetric case]%
...
case $A$ alone determines~$\nu_c$, and the pair $(\bar\gamma,A)$ determines
the original law~$\nu$.
\end{corollary}
...
when it vanishes, the centred law agrees with its symmetrization, so the
first assertion shows that $A$ determines~$\nu_c$.  Translating by the known
mean $\bar\gamma$ then determines~$\nu$.
\end{proof}
