...
\begin{lemma}[Cayley--Fourier completeness of the secant modes]%
...
\begin{proof}
Put \(F(\theta):=f(\tan\theta)\).  The Cayley change of variables identifies
\(L^2(\omega)\) with \(L^2((-\pi/2,\pi/2),\dd\theta/\pi)\).  Thus
\[
F\in L^2((-\pi/2,\pi/2),\dd\theta/\pi),
\qquad
\int_{-\pi/2}^{\pi/2}F(\theta)\frac{\dd\theta}{\pi}=0 .
\]
For \(|\varepsilon|<1\), Corollary~\ref{cor:mode-quadratic-integrals} gives
the uniformly convergent expansion
\[
\Psi_\varepsilon(\tan\theta)
=\sum_{n\ge1}u_n(\tan\theta)\varepsilon^{n-1}.
\]
Termwise pairing against \(F\) is therefore legitimate, and for
\(|\varepsilon|<1\)
\[
0=\langle f,\Psi_\varepsilon\rangle_{L^2(\omega)}
=\sum_{n\ge1}\varepsilon^{n-1}
\int_{-\pi/2}^{\pi/2}
F(\theta)u_n(\tan\theta)\frac{\dd\theta}{\pi}.
\]
This analytic power series vanishes on the real interval \((-1,1)\), so all
coefficients vanish:
\[
\int_{-\pi/2}^{\pi/2}
F(\theta)u_n(\tan\theta)\frac{\dd\theta}{\pi}=0
\qquad(n\ge1).
\]

Together with the zero-mean condition, \(F\) is orthogonal to
\(\{1,u_1,u_2,\ldots\}\).  Write the ordinary Cayley-Fourier coefficients of
the even and odd parts of \(F\) as
\[
a_k:=\int_{-\pi/2}^{\pi/2}F(\theta)\cos(2k\theta)\frac{\dd\theta}{\pi},
\qquad
b_k:=\int_{-\pi/2}^{\pi/2}F(\theta)\sin(2k\theta)\frac{\dd\theta}{\pi}.
\]
The finite Fourier formulas
\eqref{eq:cayley-mode-even-fourier}--\eqref{eq:cayley-mode-odd-fourier}
therefore give the two binomial-transform systems
\[
\sum_{k=1}^{2m}\binom{2m-1}{k-1}a_k=0\qquad(m\ge1),
\]
and
\[
\sum_{k=1}^{2m+1}\binom{2m}{k-1}b_k=0\qquad(m\ge0).
\]
These are ordinary Abel-regularized binomial transforms of the square-summable
Fourier coefficient sequences.  The transform is injective on \(\ell^2\):
after multiplying the \(k\)th coefficient by \(0<r^k<1\), the resulting
power series is absolutely convergent for \(|z|<1/r\); the displayed
identities say that its binomial sums vanish for infinitely many integer
orders, and analytic continuation followed by \(r\uparrow1\) forces each
Fourier coefficient to be zero.  Hence \(a_k=b_k=0\) for every \(k\ge1\).

Thus \(F\) is orthogonal to the complete Fourier system
\[
\{1,\cos(2k\theta),\sin(2k\theta):k\ge1\}
\]
on \((-\pi/2,\pi/2)\).  Therefore \(F=0\) a.e., and \(f=0\) in
\(L^2(\omega)\).
\end{proof}
...
The density assertion rests on the isolated Cayley--Fourier completeness
lemma immediately above: orthogonality to all secant kernels first gives
orthogonality to every Cayley--Chebyshev mode, and the finite Fourier
formulas for those modes then give the full zero-mean Fourier system in the
Cayley angle.
