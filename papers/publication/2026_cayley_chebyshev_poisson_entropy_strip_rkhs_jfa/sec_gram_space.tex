...
distribution.  The convolution identity above agrees with the
distributional convolution by $P_1$: for $\varphi\in\mathscr S(\RR)$,
Fubini is justified by the preceding weighted estimate applied to
$P_1*\varphi$, and
\[
...
\]
Using $\widehat{P_1}(\xi)=e^{-|\xi|}$, this gives in $\mathscr S'(\RR)$
\[
\widehat F=e^{-|\xi|}\widehat f.
\]
Because $F=0$, the left-hand side is zero.  If
$\psi\in C_c^\infty(\RR)$, then $e^{|\xi|}\psi(\xi)$ is again a compactly
supported smooth test function, and therefore
\[
\langle \widehat f,\psi\rangle
=\langle e^{-|\xi|}\widehat f,e^{|\xi|}\psi\rangle=0.
\]
Hence $\widehat f=0$ as a distribution, and consequently $f=0$.  Therefore
