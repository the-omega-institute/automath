...
\qquad n\in\ZZ .
\end{equation}
They recover $\Pi_{\ZZ}F_t$ and only $\Pi_{\ZZ}F_t$ among ambient
$\mathcal H_K$ data.  More explicitly, if
\[
...
\eqref{eq:integer-orbit-projection-symbol}.  The samples recover the full
channel $F_t$ exactly if and only if
\[
F_t\in\mathcal H_K(\ZZ)
\]
if and only if $Q_t$ is $2\pi$-periodic.  This is equivalent to the law-side
lattice condition
...
It remains to translate the periodicity into the law-side condition.  Let
$\mu$ be the law of $\Delta/t$ and set $\lambda(\dd s):=s\,\mu(\dd s)$.
