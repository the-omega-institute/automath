...
\qquad n\in\ZZ .
\end{equation}
The recovery conclusions split into three distinct assertions:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=*]
\item The integer samples recover \(\Pi_{\ZZ}F_t\) and only
\(\Pi_{\ZZ}F_t\) among ambient \(\mathcal H_K\) data.
\item They recover the full channel \(F_t\) exactly under the periodic/lattice
condition displayed below.
