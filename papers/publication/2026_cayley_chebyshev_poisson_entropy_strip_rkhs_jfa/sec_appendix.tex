...
The same finite recurrence can be reproduced by the following short exact
script, whose output is the last column of the table:
\begin{verbatim}
from fractions import Fraction as F; from math import comb
def add(a,b): return (a[0]+b[0],a[1]+b[1])
def mul(a,b): return (a[0]*b[0]-a[1]*b[1],a[0]*b[1]+a[1]*b[0])
def scale(c,a): return (c*a[0],c*a[1])
def mi(n): return [(1,0),(0,-1),(-1,0),(0,1)][n%4]
def qc(n,k):
    return scale(F(comb(n-1,abs(k)-1),2**n)*(1 if k>0 else (-1)**n),mi(n))
def q(n): return {k:qc(n,k) for k in range(-n,n+1) if k}
def conv(a,b):
    c={}
    for i,x in a.items():
      for j,y in b.items(): c[i+j]=add(c.get(i+j,(0,0)),mul(x,y))
    return c
rows=[(2,2),(2,4),(3,3),(2,2,2),(2,6),(3,5),(4,4),(2,2,4),(2,3,3),(2,2,2,2)]
for row in rows:
    p={0:(1,0)}
    for n in row: p=conv(p,q(n))
    print(row,p.get(0,(0,0)))
\end{verbatim}
Only rational Gaussian-integer arithmetic is involved; the imaginary parts
cancel in the even total-order rows listed above, and odd total-order rows
vanish by parity.
