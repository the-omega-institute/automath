...
/-- Paper label: `prop:xi-radial-defect-index-offcritical-lines`. The two reciprocal
quadratic branches over an offcritical real value solve the Joukowsky radial equation and
carry the doubled multiplicity count. -/
theorem paper_xi_radial_defect_index_offcritical_lines (L S : ℝ) (m : ℕ) (hL : 1 < L)
    (hS : 2 < |S|) :
    let η := Real.arcosh (|S| / 2) / Real.log L
    let yPlus := (S + Real.sqrt (S ^ 2 - 4)) / 2
    let yMinus := (S - Real.sqrt (S ^ 2 - 4)) / 2
  ...
  have hηMinus :
    ...
    simpa [hyMinus_def] using
      paper_xi_single_scale_radial_deviation_arcosh L S yMinus hL hyMinus_ne hyMinus_eq hS
  refine ⟨?_, ?_, ?_, ?_, by simp [two_mul]⟩
  · simpa [hyPlus_def] using hyPlus_eq
  · simpa [hyMinus_def] using hyMinus_eq
  · simpa using hηPlus
  · simpa using hηMinus
