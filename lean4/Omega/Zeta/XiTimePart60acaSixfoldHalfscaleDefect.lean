...
theorem paper_xi_time_part60aca_sixfold_halfscale_defect
    ...
    (hkl : (1 / (Omega.Zeta.lucasNum d : ℚ)) ≤ klGap) :
    (1 / (Omega.Zeta.lucasNum d : ℚ)) ≤ collisionGap ∧
      (1 / (Omega.Zeta.lucasNum d : ℚ)) ≤ klGap := by
  constructor
  · by_cases hd : d = 0
    · subst d
      simpa [lucasNum] using hcollision
    · have hdpos : 1 ≤ d := Nat.succ_le_of_lt (Nat.pos_of_ne_zero hd)
      ...
      have hlucas_id : lucasNum d = Omega.lucasNum d := by
        simp [lucasNum, hd]
      have hden : (Nat.fib (2 * d) : ℚ) = (Nat.fib d : ℚ) * (lucasNum d : ℚ) := by
        rw [hlucas_id]
        exact_mod_cast Omega.fib_double_eq_mul_lucas d
      have hratio : (Nat.fib d : ℚ) / Nat.fib (2 * d) = 1 / (lucasNum d : ℚ) := by
        rw [hden]
        field_simp [hfib_ne]
      rwa [← hratio]
  · exact hkl
